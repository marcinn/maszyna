...
void render_task::run() {
    ...
    if (output != nullptr) {
        auto *outputwidth{
            PyObject_CallMethod(m_renderer, "get_width", nullptr)};
        auto *outputheight{
            PyObject_CallMethod(m_renderer, "get_height", nullptr)};
        // upload texture data
        if ((outputwidth != nullptr) && (outputheight != nullptr) && m_target) {
            ...
            if (m_target->image) delete[] m_target->image;
            ...
            if (!Global.gfx_usegles) {
                ...
                for (int y = 0; y < h; y++)
                    for (int x = 0; x < w; x++) {
                        m_target->image[(y * w + x) * 4 + 0] =
                            image[(y * w + x) * 3 + 0];
                        m_target->image[(y * w + x) * 4 + 1] =
                            image[(y * w + x) * 3 + 1];
                        m_target->image[(y * w + x) * 4 + 2] =
                            image[(y * w + x) * 3 + 2];
...
void render_task::upload() {
    if (Global.python_uploadmain && m_target->image) {
        ...
        glTexImage2D(
            GL_TEXTURE_2D, 0, m_target->format, m_target->width,
            m_target->height, 0, m_target->components, GL_UNSIGNED_BYTE,
            m_target->image);
        ...
        if (Global.python_threadedupload) glFlush();
    }
...
auto python_taskqueue::init() -> bool {
    crashreport_add_info(
        "python.threadedupload", Global.python_threadedupload ? "yes" : "no");
    crashreport_add_info(
        "python.uploadmain", Global.python_uploadmain ? "yes" : "no");
    ...
    for (auto &worker : m_workers) {
        ...
        worker = std::thread(
            &python_taskqueue::run, this, openglcontextwindow,
...
void python_taskqueue::exit() {
    ...
    // with the workers dead we don't have to worry about concurrent access
    // anymore
...
// adds specified task along with provided collection of data to the work queue.
// returns true on success
auto python_taskqueue::insert(task_request const &Task) -> bool {
    if (!m_initialized || (false == Global.python_enabled) ||
        (Task.renderer.empty()) || (Task.input == nullptr) ||
        (Task.target == 0)) {
        return false;
    }
...
auto python_taskqueue::run_file(
    std::string const &File, std::string const &Path) -> bool {
    auto const lookup{
        FileExists({Path + File, "python/local/" + File}, {".py"})};
...
void python_taskqueue::acquire_lock() { PyEval_RestoreThread(m_mainthread); }

// releases the python gil and swaps the main thread out
void python_taskqueue::release_lock() { PyEval_SaveThread(); }

auto python_taskqueue::fetch_renderer(std::string const Renderer)
    -> PyObject * {
    ...
    acquire_lock();
    {
        ...
        if (rendererarguments == nullptr) {
            ErrorLog(
                "Python Renderer: failed to create initialization arguments");
            goto cache_and_return;
        }
...
void python_taskqueue::run(
    GLFWwindow *Context, rendertask_sequence &Tasks,
    uploadtask_sequence &Upload_Tasks, threading::condition_variable &Condition,
    ...
    while (false == Exit.load()) {
        // regardless of the reason we woke up prime the spurious wakeup flag
        // for the next time
        Condition.spurious(true);
        // keep working as long as there's any scheduled tasks
        do {
            task = nullptr;
            // acquire a lock on the task queue and potentially grab a task from
            // it
            {
            ...
            if (task != nullptr) {
                ...
                PyEval_RestoreThread(threadstate);
                {
                    ...
                    if (PyErr_Occurred() != nullptr) error();
                }
                // clear the thread state
                PyEval_SaveThread();
            }
            // TBD, TODO: add some idle time between tasks in case we're on a
            // single thread cpu?
        } while (task != nullptr);
        // if there's nothing left to do wait until there is
        // but check every now and then on your own to minimize potential
        // deadlock situations
...
void python_taskqueue::update() {
    ...
    for (auto &task : m_uploadtasks.data) task->upload();
...
void python_taskqueue::error() {
    if (m_stderr != nullptr) {
        ...
        if (type == nullptr) {
            ErrorLog(
                "Python Interpreter: don't know how to handle null exception");
        }
        ...
        if (tracebacktext != nullptr) {
            ...
            WriteLog(
                "Python Interpreter: failed to retrieve the stack traceback");
        }
    }
}
