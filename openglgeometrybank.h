   ...
   private:
    ...
    struct chunk_record {
        std::size_t vertex_offset{
            0};  // beginning of the chunk vertex data as offset from the
                 // beginning of the last established buffer
        std::size_t vertex_count{
            0};  // size of the chunk in the last established buffer
        std::size_t index_offset{0};
        std::size_t index_count{0};
        bool is_good{false};  // true if local content of the chunk matches the
                              // data on the opengl end
    };
    ...
    auto draw_(
        ...
        unsigned int const Streams) -> std::size_t override;
    // release() subclass details
    void release_() override;
    void setup_buffer();
    void bind_buffer();
    void delete_buffer();
    static void bind_streams(
        gfx::stream_units const &Units, unsigned int const Streams,
        size_t offset = 0);
    static void release_streams();

    // members:
    static GLuint m_activevertexbuffer;  // buffer bound currently on the opengl
                                         // end, if any
    static unsigned int m_activestreams;
    static std::vector<GLint> m_activetexturearrays;
    GLuint m_vertexbuffer{
        0};  // id of the buffer holding vertex data on the opengl end
    GLuint m_indexbuffer{
        0};  // id of the buffer holding index data on the opengl end
    chunkrecord_sequence m_chunkrecords;  // helper data for all stored geometry
                                          // chunks, in matching order
};
   ...
   private:
    ...
    struct chunk_record {
        ...
        unsigned int streams{
            0};  // stream combination used to generate the display list
    ...
    // release () subclass details
    void release_() override;
    void delete_list(gfx::geometry_handle const &Geometry);
