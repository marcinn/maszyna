...
bool TEventLauncher::Load(cParser *parser) {  // wczytanie wyzwalacza zdarzeń
    ...
    *parser >> token;
    ...
    if ((asEvent2Name == "end") || (asEvent2Name == "condition") ||
        (asEvent2Name ==
         "traintriggered")) {  // drugiego eventu może nie być, bo są z tym
                               // problemy, ale ciii...
        token =
            asEvent2Name;  // rozpoznane słowo idzie do dalszego przetwarzania
        asEvent2Name = "none";  // a drugiego eventu nie ma
    } else {                    // gdy są dwa eventy
        parser->getTokens();
        *parser >> token;
        // str = AnsiString(token.c_str());
    }
    if (token == "condition") {  // obsługa wyzwalania warunkowego
        parser->getTokens();
        *parser >> token;
        asMemCellName = token;
        parser->getTokens();
    ...
    if (DeltaTime < 0)
    ...
    else if (DeltaTime > 0) {    // wartość dodatnia oznacza wyzwalanie o
                                 // określonej godzinie
        iMinute = int(DeltaTime) %
                  100;  // minuty są najmłodszymi cyframi dziesietnymi
        ...
        if (timeoffset != 0) {
            auto const adjustedtime{
                clamp_circular(iHour * 60 + iMinute + timeoffset, 24 * 60)};
            iHour = (adjustedtime / 60) % 24;
            iMinute = adjustedtime % 60;
        }

        WriteLog(
            "EventLauncher at " + std::to_string(iHour) + ":" +
...
bool TEventLauncher::check_activation_key() {
    ...
    if (modifier & GLFW_MOD_SHIFT) result &= Global.shiftState;
    if (modifier & GLFW_MOD_CONTROL) result &= Global.ctrlState;
...
bool TEventLauncher::check_conditions() {
    ...
    if ((iCheckMask != 0) && (MemCell != nullptr)) {
...
bool TEventLauncher::IsGlobal() const {
    return (
        (DeltaTime == 0) && (iHour >= 0) && (iMinute >= 0) &&
        (dRadius < 0.0));  // bez ograniczenia zasięgu
}

bool TEventLauncher::IsRadioActivated() const { return (iKey < 0); }

// radius() subclass details, calculates node's bounding radius
float TEventLauncher::radius_() { return std::sqrt(dRadius); }

// serialize() subclass details, sends content of the subclass to provided
// stream
void TEventLauncher::serialize_(std::ostream &Output) const {
    // TODO: implement
}
// deserialize() subclass details, restores content of the subclass from
// provided stream
void TEventLauncher::deserialize_(std::istream &Input) {
    // TODO: implement
}

// export() subclass details, sends basic content of the class in legacy (text)
// format to provided stream
void TEventLauncher::export_as_text_(std::ostream &Output) const {
    // header
    Output << "eventlauncher ";
    // location
    Output << location().x << ' ' << location().y << ' ' << location().z << ' ';
    // activation radius
    Output << (dRadius > 0 ? std::sqrt(dRadius) : dRadius) << ' ';
    // activation key
    if (iKey != 0) {
        auto key{iKey & 0xff};
        auto const modifier{iKey >> 8};

        if (key >= 'A' && key <= 'Z' && !(modifier & GLFW_MOD_SHIFT)) key += 32;
    ...
    if (DeltaTime != 0.0) {
        ...
        if ((iHour < 0) && (iMinute < 0)) {
            ...
            // NOTE: activation hour might be affected by user-requested time
            // offset
            auto const timeoffset{
                static_cast<int>(Global.ScenarioTimeOffset * 60)};
            auto const adjustedtime{
                clamp_circular(iHour * 60 + iMinute - timeoffset, 24 * 60)};
            Output << (adjustedtime / 60) % 24 << (adjustedtime % 60) << ' ';
        }
    }
    // associated event(s)
    ...
    if (false == asMemCellName.empty()) {
        // conditional event
        Output << "condition " << asMemCellName << ' ' << szText << ' '
               << ((iCheckMask & basic_event::flags::value1) != 0
                       ? to_string(fVal1)
                       : "*")
               << ' '
               << ((iCheckMask & basic_event::flags::value2) != 0
                       ? to_string(fVal2)
                       : "*")
               << ' ';
    }
