...
bool check_shader(std::string filename) {
    ...
    std::pair<GLuint, std::string> source =
        shader_processor.process_source(filename, "../shaders/");
...
bool check_shader_allopts(std::string filename) {
    ...
    if (!check_shader(filename)) suc = false;
...
bool check_shader_confs(std::string filename) {
    ...
    if (!check_shader_allopts(filename)) suc = false;
...
int main() {
    bool suc = true;
    for (auto &f :
         std::filesystem::recursive_directory_iterator("../shaders/")) {
        if (!f.is_regular_file()) continue;

        std::string relative =
            std::filesystem::relative(f.path(), "../shaders/").string();
        if (f.path().extension() != ".frag" &&
            f.path().extension() != ".geom" && f.path().extension() != ".vert")
            continue;

        std::cout << "== validating shader " << relative << ": " << std::flush;
        if (!check_shader_confs(relative)) suc = false;
    }
