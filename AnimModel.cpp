...
void TAnimContainer::SetRotateAnim(
    Math3D::vector3 vNewRotateAngles, double fNewRotateSpeed) {
    vDesiredAngles = vNewRotateAngles;
    fRotateSpeed = fNewRotateSpeed;
    iAnim |= 1;
    /* //Ra 2014-07: jeśli model nie jest renderowany, to obliczyć czas animacji
     i dodać event wewnętrzny
     //można by też ustawić czas początku animacji zamiast pobierać czas ramki i
     liczyć różnicę
    */
    if (evDone) {  // dołączyć model do listy aniomowania, żeby animacje były
                   // przeliczane również bez
        // wyświetlania
...
void TAnimContainer::SetTranslateAnim(
    Math3D::vector3 vNewTranslate, double fNewSpeed) {
...
void TAnimContainer::UpdateModel() {
    if (pSubModel)  // pozbyć się tego - sprawdzać wcześniej
    {
        if (fTranslateSpeed != 0.0) {
            auto dif =
                vTranslateTo - vTranslation;  // wektor w kierunku docelowym
            double l = LengthSquared3(
                dif);  // długość wektora potrzebnego przemieszczenia
            if (l >= 0.0001) {  // jeśli do przemieszczenia jest ponad 1cm
                auto s =
                    Math3D::SafeNormalize(dif);  // jednostkowy wektor kierunku
                s = s * (fTranslateSpeed *
                         Timer::GetDeltaTime());  // przemieszczenie w podanym
                                                  // czasie z daną prędkością
                if (LengthSquared3(s) < l)  //żeby nie jechało na drugą stronę
                    vTranslation += s;
                else
                    vTranslation = vTranslateTo;  // koniec animacji, "koniec
                                                  // animowania" uruchomi
                // się w następnej klatce
            } else {  // koniec animowania
                vTranslation = vTranslateTo;
                fTranslateSpeed = 0.0;  // wyłączenie przeliczania wektora
                if (LengthSquared3(vTranslation) <=
                    0.0001)       // jeśli jest w punkcie początkowym
        ...
        if (fRotateSpeed != 0.0) {
            ...
            // HACK: negative speed allows to work around legacy behaviour,
            // where desired angle > 360 meant permanent rotation
            if (fRotateSpeed > 0.0) {
            ...
            if ((vRotateAngles.x == 0.0) && (vRotateAngles.y == 0.0) &&
                (vRotateAngles.z == 0.0)) {
        ...
        if (fAngleSpeed != 0.f) {
            // NOTE: this is angle- not quaternion-based rotation TBD, TODO:
            // switch to quaternion rotations?
            fAngleCurrent +=
                fAngleSpeed *
                Timer::GetDeltaTime();  // aktualny parametr interpolacji
        }
    }
};

void TAnimContainer::PrepareModel() {  // tutaj zostawić tylko ustawienie
                                       // submodelu, przeliczanie ma być w
                                       // UpdateModel()
    if (pSubModel)  // pozbyć się tego - sprawdzać wcześniej
    {
        ...
        {
            if (fAngleSpeed > 0.0f) {
                if (fAngleCurrent >= 1.0f) {  // interpolacja zakończona,
                                              // ustawienie na pozycję końcową
                    qCurrent = qDesired;
                    ...
                    // normalizacja jest wymagana do interpolacji w następnej
                    // animacji
                    qCurrent = Normalize(Slerp(
                        qStart, qDesired,
                        fAngleCurrent));  // interpolacja sferyczna kąta
                    // qCurrent=Slerp(qStart,qDesired,fAngleCurrent);
                    // //interpolacja sferyczna kąta
                    if (qCurrent.w == 1.0)  // rozpoznać brak obrotu i wyłączyć
                                            // w iAnim w takim przypadku
                        iAnim &= ~4;  // kąty są zerowe
                }
            }
            mAnim->Quaternion(
                &qCurrent);  // wypełnienie macierzy (wymaga normalizacji?)
            pSubModel->mAnimMatrix =
                mAnim;  // użyczenie do submodelu (na czas renderowania!)
        }
    }
...
bool TAnimContainer::InMovement() {  // czy trwa animacja - informacja dla
                                     // obrotnicy
    return (fRotateSpeed != 0.0) || (fTranslateSpeed != 0.0);
}

void TAnimContainer::EventAssign(
    basic_event
        *ev) {  // przypisanie eventu wykonywanego po zakończeniu animacji
    evDone = ev;
};
...
TAnimModel::TAnimModel(scene::node_data const &Nodedata)
    : basic_node(Nodedata) {
    m_lightcolors.fill(glm::vec3{-1.f});
    m_lightopacities.fill(1.f);
}

bool TAnimModel::Init(
    std::string const &asName, std::string const &asReplacableTexture) {
    if (asReplacableTexture.substr(0, 1) == "*") {
        // od gwiazdki zaczynają się teksty na wyświetlaczach
        asText = asReplacableTexture.substr(
            1, asReplacableTexture.length() - 1);  // zapamiętanie tekstu
...
bool TAnimModel::is_keyword(std::string const &Token) const {
    return (Token == "endmodel") || (Token == "lights") ||
           (Token == "lightcolors") || (Token == "angles") ||
           (Token == "notransition");
}

bool TAnimModel::Load(
    cParser *parser,
    bool ter) {  // rozpoznanie wpisu modelu i ustawienie świateł
    ...
    for (int i = 0; i < iMaxNumLights; ++i)
        if (LightsOn[i] ||
            LightsOff[i])  // Ra: zlikwidowałem wymóg istnienia obu
            iNumLights = i + 1;
    ...
    do {
        ...
        if (token == "lights") {
            auto i{0};
            while (
                (false == (token = parser->getToken<std::string>()).empty()) &&
        ...
        if (token == "angles") {
            parser->getTokens(3);
            *parser >> vAngle[0] >> vAngle[1] >> vAngle[2];
        }
    ...
    } while ((false == token.empty()) && (token != "endmodel"));

    return true;
}

std::shared_ptr<TAnimContainer> TAnimModel::AddContainer(
    std::string const &Name) {  // dodanie sterowania submodelem dla egzemplarza
...
std::shared_ptr<TAnimContainer> TAnimModel::GetContainer(
    std::string const
        &Name) {  // szukanie/dodanie sterowania submodelem dla egzemplarza
    if (true == Name.empty())
        return (!m_animlist.empty())
                   ? m_animlist.front()
                   : nullptr;  // pobranie pierwszego (dla obrotnicy)

    for (auto entry : m_animlist) {
        if (entry->NameGet() == Name) return entry;
    }
...
void TAnimModel::RaAnimate(unsigned int const Framestamp) {
    ...
    // case ls_Off: ustalenie czasu migotania, t<1s (f>1Hz), np. 0.1 => t=0.1
    // (f=10Hz) case ls_On: ustalenie wypełnienia ułamkiem, np. 1.25 => zapalony
    // przez 1/4 okresu case ls_Blink: ustalenie częstotliwości migotania, f<1Hz
    // (t>1s), np. 2.2 => f=0.2Hz (t=5s)
    ...
    for (int idx = 0; idx < iNumLights; ++idx) {
        ...
        if ((modeintegral < ls_Blink) && (modefractional < 0.01f)) {
            ...
            auto const transitiontime{(
                m_transition ? std::min(1.f, std::min(fOnTime, fOffTime) * 0.9f)
                             : 0.01f)};
            ...
            // blink modes
            auto const ontime{
                ((mode == ls_Blink) ? ((modefractional < 0.01f)
                                           ? fOnTime
                                           : (1.f / modefractional) * 0.5f)
                 : (mode == ls_Off) ? modefractional * 0.5f
                 : (mode == ls_On)  ? modefractional * (fOnTime + fOffTime)
                                    : fOnTime)};  // fallback
            auto const offtime{
                ((mode == ls_Blink)
                     ? ((modefractional < 0.01f) ? fOffTime : ontime)
                 : (mode == ls_Off) ? ontime
                 : (mode == ls_On)  ? (fOnTime + fOffTime) - ontime
                                    : fOffTime)};  // fallback
            auto const transitiontime{
                (m_transition ? std::min(1.f, std::min(ontime, offtime) * 0.9f)
                              : 0.01f)};

            timer = clamp_circular<float>(
                timer + timedelta * (lsLights[idx] > 0.f ? 1.f : -1.f),
                ontime + offtime);
            ...
            if (timer < ontime) {
                ...
                // blink off
                opacity =
                    1.f - clamp((timer - ontime) / transitiontime, 0.f, 1.f);
            }
        }
    }

    // Ra 2F1I: to by można pomijać dla modeli bez animacji, których jest
    // większość
    for (auto entry : m_animlist) {
...
void TAnimModel::RaPrepare() {  // ustawia światła i animacje we wzorcu modelu
    ...
    for (int i = 0; i < iNumLights; ++i) {
        ...
        switch (lightmode) {
            ...
            case ls_Blink: {
                if (LightsOn[i]) {
                    ...
                    LightsOn[i]->SetVisibilityLevel(
                        m_lightopacities[i], true, false);
                }
                if (LightsOff[i]) {
            ...
            case ls_Home: {
                ...
                state =
                    state && ((simulationhour < 1) || (simulationhour >= 5));
                break;
            }
            default: {
                break;
        ...
        if (lightmode >= ls_Dark) {
            ...
            if (LightsOn[i]) {
                ...
                // TODO: set visibility for the entire submodel's children as
                // well
    ...
    TSubModel::iInstance = reinterpret_cast<std::uintptr_t>(
        this);  //żeby nie robić cudzych animacji
    TSubModel::pasText =
        &asText;  // przekazanie tekstu do wyświetlacza (!!!! do przemyślenia)
...
int TAnimModel::Flags() {  // informacja dla TGround, czy ma być w Render,
    ...
    if (m_materialdata.replacable_skins[1] > 0)  // jeśli ma wymienną teksturę 0
        i |= (i & 0x01010001) *
             ((m_materialdata.textures_alpha & 1) ? 0x20 : 0x10);
    return i;
}

//---------------------------------------------------------------------------

int TAnimModel::TerrainCount() {  // zliczanie kwadratów kilometrowych (główna
                                  // linia po Next) do tworznia tablicy
    return pModel ? pModel->TerrainCount() : 0;
}

TSubModel *TAnimModel::TerrainSquare(
    int n) {  // pobieranie wskaźników do pierwszego submodelu
    return pModel ? pModel->TerrainSquare(n) : 0;
}

//---------------------------------------------------------------------------
void TAnimModel::LightSet(
    int const n, float const v) {  // ustawienie światła (n) na wartość (v)
    if (n >= iMaxNumLights) {
...
std::optional<std::tuple<float, float, std::optional<glm::vec3>>>
TAnimModel::LightGet(const int n) {
    if (n >= iMaxNumLights) return std::nullopt;
    if (!LightsOn[n] && !LightsOff[n]) return std::nullopt;

    std::optional<glm::vec3> color;

    if (m_lightcolors[n].r >= 0.0f) color.emplace(m_lightcolors[n]);

    if (!color && LightsOn[n]) color = LightsOn[n]->GetDiffuse();
...
void TAnimModel::AnimUpdate(
    double dt) {  // wykonanie zakolejkowanych animacji, nawet gdy modele nie są
    ...
    acAnimList.remove_if([](std::weak_ptr<TAnimContainer> ptr) {
        ...
        container->UpdateModel();  // na razie bez usuwania z listy, bo głównie
                                   // obrotnica na nią wchodzi
        return false;
    });
}

// radius() subclass details, calculates node's bounding radius
float TAnimModel::radius_() {
...
void TAnimModel::export_as_text_(std::ostream &Output) const {
    ...
    // 3d shape
    auto modelfile{
        (pModel ? pModel->NameGet() + ".t3d"
                :  // rainsted requires model file names to include an extension
             "none")};
