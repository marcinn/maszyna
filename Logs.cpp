...
std::string filename_date() {
    ...
    std::snprintf(
        logbuffer, sizeof(logbuffer), "%d%02d%02d_%02d%02d%03d", st.wYear,
        st.wMonth, st.wDay, st.wHour, st.wMinute, st.wMilliseconds);
...
void WriteLog(const char *str, logtype const Type) {
    ...
    if (Global.iWriteLogEnabled & 1) {
        if (!output.is_open()) {
            std::string const filename =
                (Global.MultipleLogs ? "logs/log (" + filename_scenery() +
                                           ") " + filename_date() + ".txt"
                                     : "log.txt");
    ...
    if (log_scrollback.size() > 200) log_scrollback.pop_front();
    ...
    if (Global.iWriteLogEnabled & 2) {
        ...
        DWORD wr = 0;
        WriteConsole(
            GetStdHandle(STD_OUTPUT_HANDLE), str, (DWORD)strlen(str), &wr,
            NULL);
        WriteConsole(
            GetStdHandle(STD_OUTPUT_HANDLE), endstring,
            (DWORD)strlen(endstring), &wr, NULL);
#else
        printf("%s\n", str);
#endif
    }
...
void ErrorLog(const char *str, logtype const Type) {
    ...
    if (!(Global.iWriteLogEnabled & 1)) return;

    if (!errors.is_open()) {
        std::string const filename =
            (Global.MultipleLogs ? "logs/errors (" + filename_scenery() + ") " +
                                       filename_date() + ".txt"
                                 : "errors.txt");
...
void Error(const std::string &asMessage, bool box) {
    // if (box)
    //	MessageBox(NULL, asMessage.c_str(), string("EU07 " +
    //Global.asRelease).c_str(), MB_OK);
    ErrorLog(asMessage.c_str());
}

void Error(const char *&asMessage, bool box) {
    // if (box)
    //	MessageBox(NULL, asMessage, string("EU07 " + Global.asRelease).c_str(),
    //MB_OK);
    ErrorLog(asMessage);
    WriteLog(asMessage);
}
...
void WriteLog(
    ...
    logtype const Type) {  // Ra: wersja z AnsiString jest zamienna z Error()
    WriteLog(str.c_str(), Type);
};

void CommLog(const char *str) {  // Ra: warunkowa rejestracja komunikatów
    WriteLog(str);
...
void CommLog(const std::string
                 &str) {  // Ra: wersja z AnsiString jest zamienna z Error()
    WriteLog(str);
};
