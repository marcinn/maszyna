...
void opengl_particles::update(opengl_camera const &Camera) {
    ...
    for (auto const &source : sources) {
        ...
        for (auto const &particle : particles) {
            ...
            vertex.color[3] =
                clamp<std::uint8_t>(particle.opacity * 255, 0, 255);
    ...
    if (m_buffercapacity < m_particlevertices.size()) {
        ...
        if ((m_buffer > 0) && (m_buffer != (GLuint)-1)) {
            ...
            // call NOTE: we match capacity instead of current size to reduce
            // number of re-allocations
            auto const particlecount{m_particlevertices.capacity()};
            ::glBindBuffer(GL_ARRAY_BUFFER, m_buffer);
            ::glBufferData(
                GL_ARRAY_BUFFER, particlecount * sizeof(particle_vertex),
    ...
    // ...send the data...
    if ((m_buffer > 0) && (m_buffer != (GLuint)-1)) {
        // if the buffer exists at this point it's guaranteed to be big enough
        // to hold our data
        ::glBindBuffer(GL_ARRAY_BUFFER, m_buffer);
        ::glBufferSubData(
            GL_ARRAY_BUFFER, 0,
...
std::size_t opengl_particles::render(GLint const Textureunit) {
    if (false == Global.Smoke) {
        return 0;
    }
    ...
    ::glBindBuffer(GL_ARRAY_BUFFER, m_buffer);
    ::glVertexPointer(
        3, GL_FLOAT, sizeof(particle_vertex), static_cast<char *>(nullptr));
    ::glEnableClientState(GL_VERTEX_ARRAY);
    ::glColorPointer(
        4, GL_UNSIGNED_BYTE, sizeof(particle_vertex),
        reinterpret_cast<void const *>(sizeof(float) * 3));
    ::glEnableClientState(GL_COLOR_ARRAY);
    ::glClientActiveTexture(GL_TEXTURE0 + Textureunit);
    ::glTexCoordPointer(
        2, GL_FLOAT, sizeof(particle_vertex),
        reinterpret_cast<void const *>(
            sizeof(float) * 3 + sizeof(std::uint8_t) * 4));
