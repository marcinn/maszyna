...
struct opengl_texture {
    ...
    bool create(bool const Static = false);
    // releases resources allocated on the opengl end, storing local copy if
    // requested
    void release();
    void make_stub();
    void alloc_rendertarget(
        GLint format, GLint components, int width, int height, int layers = 1,
        int samples = 1, GLint wrap = GL_CLAMP_TO_EDGE);
    void set_components_hint(GLint hint);
    static void reset_unit_cache();
    inline int width() const { return data_width; }
    inline int height() const { return data_height; }
    inline bool is_stub() const { return is_texstub; }
    ...
    bool is_ready{
        false};  // indicates the texture was processed and is ready for use
   ...
   private:
    ...
    bool is_rendertarget =
        false;  // is used as postfx rendertarget, without loaded data
    int samples = 1;
    int layers = 1;
    bool is_texstub =
        false;  // for make_from_memory internal_src: functionality
    std::vector<unsigned char>
        data;  // texture data (stored GL-style, bottom-left origin)
    resource_state data_state{
        resource_state::none};  // current state of texture data
    int data_width{0}, data_height{0}, data_mapcount{0};
    GLint data_format{0}, data_components{0};
    GLint data_type = GL_UNSIGNED_BYTE;
    GLint wrap_mode_s = GL_REPEAT;
    GLint wrap_mode_t = GL_REPEAT;
    /*
        std::atomic<bool> is_loaded{ false }; // indicates the texture data was
       loaded and can be processed std::atomic<bool> is_good{ false }; //
       indicates the texture data was retrieved without errors
    */
...
class texture_manager {
   public:
    ...
    void unit(GLint const Textureunit);
    // creates texture object out of data stored in specified file
    texture_handle create(
        std::string Filename, bool const Loadnow = true,
        GLint Formathint = GL_SRGB_ALPHA);
    // binds specified texture to specified texture unit
    void bind(std::size_t const Unit, texture_handle const Texture);
   ...
   private:
    // types:
    typedef std::pair<opengl_texture *, resource_timestamp>
        texturetimepoint_pair;
    ...
    // methods:
    // checks whether specified texture is in the texture bank. returns texture
    // id, or npos.
    texture_handle find_in_databank(std::string const &Texturename) const;
    // checks whether specified file exists. returns name of the located file,
    // or empty string.
    std::pair<std::string, std::string> find_on_disk(
        std::string const &Texturename) const;
    void delete_textures();

    // members:
    texture_handle const npos{0};  // should be -1, but the rest of the code
                                   // uses -1 for something else
    texturetimepointpair_sequence m_textures;
    index_map m_texturemappings;
    garbage_collector<texturetimepointpair_sequence> m_garbagecollector{
        m_textures, 600, 60, "texture"};
};
...
void downsample(
    ...
    */
    for (std::size_t row = 0; row < Height;
         row += 2, sampler +=
                   Width) {  // column movement advances us down another row
        for (std::size_t column = 0; column < Width;
            ...
            /*
                        ...
                        color /= 4;
            */
            // manual version of the above, but drops colour resolution to 6
            // bits
            accumulator = *sampler;
            accumulator /= 4;
            color = accumulator;
