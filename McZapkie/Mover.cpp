...
std::vector<std::string> const TMoverParameters::eimv_labels = {
    "Fkrt:", "Fmax:", "ks:  ", "df:  ", "fp:  ", "Us:  ", "pole:",
    "Ic:  ", "If:  ", "M:   ", "Fr:  ", "Ipoj:", "Pm:  ", "Pe:  ",
    "eta: ", "fkr: ", "Uzsm:", "Pmax:", "Fzad:", "Imax:", "Fful:"};
...
double ComputeCollision(
    double &v1, double &v2, double m1, double m2, double beta,
    ...
    if ((v1 < v2) && (vc == true))
        ...
        v1 = w1 *
             std::sqrt(
                 1.0 - beta);  // niejawna zmiana predkosci wskutek zderzenia
        v2 = w2 * std::sqrt(1.0 - beta);
        return m1 * (w2 - w1) * (1 - beta);
    }
}

int DirPatch(int Coupler1, int Coupler2) {  // poprawka dla liczenia sil przy
                                            // ustawieniu przeciwnym obiektow
...
void TSecuritySystem::set_enabled(bool e) {
    if (vigilance_enabled || cabsignal_enabled) enabled = e;
}
...
void TSecuritySystem::acknowledge_release() {
    ...
    if (press_timer > MaxHoldTime) alert_timer = 0.0;
    press_timer = 0.0;
}
...
void TSecuritySystem::update(double dt, double vel, bool pwr) {
    ...
    if (vigilance_enabled && velocity > AwareMinSpeed) vigilance_timer += dt;

    if (pressed && (!is_sifa || velocity > AwareMinSpeed)) press_timer += dt;
...
void TSecuritySystem::set_cabsignal() {
    if (cabsignal_enabled && power) cabsignal_active = true;
}
...
bool TSecuritySystem::is_beeping() const {
    ...
    return alert_timer > SoundSignalDelay &&
           (!separate_acknowledge || is_vigilance_blinking());
}
...
bool TSecuritySystem::radiostop_available() const { return radiostop_enabled; }

void TSecuritySystem::set_cabsignal_lock(bool v) { cabsignal_lock = v; }

bool TSecuritySystem::is_engine_blocked() const {
    if (!is_sifa) return false;
...
void TSecuritySystem::load(std::string const &line, double Vmax) {
    ...
    if (awaresystem.find("Sifa") != std::string::npos) is_sifa = true;
...
double TableInterpolation(std::map<double, double> &Map, double Parameter) {
    ...
    double ratio =
        (upper->second - lower->second) / (upper->first - lower->first);
...
double TMoverParameters::Current(double n, double U) {
    // wazna funkcja - liczy prad plynacy przez silniki polaczone szeregowo lub
    // rownolegle w zaleznosci od polozenia nastawnikow MainCtrl i ScndCtrl oraz
    // predkosci obrotowej n a takze wywala bezpiecznik nadmiarowy gdy za duzy
    // prad lub za male napiecie jest takze mozliwosc uszkodzenia silnika
    // wskutek nietypowych parametrow
    ...
    if ((DynamicBrakeType == dbrake_automatic) && (TrainType == dt_EZT)) {
        DynamicBrakeFlag =
            (Power110vIsAvailable && (TUHEX_Active || (Vadd > TUHEX_MinIw)) &&
             DynamicBrakeEMUStatus);
    }

    // wylacznik cisnieniowy yBARC - to jest chyba niepotrzebne tutaj   Q: no to
    // usuwam...
    ...
    if (DynamicBrakeFlag) {
        Hamulec->SetED(abs(
            Im /
            350));  // hamulec ED na EP09 dziala az do zatrzymania lokomotywy
        //-     WriteLog("A");
    } else {
        Hamulec->SetED(0);
        //-     WriteLog("B");
    ...
    ResistorsFlag =
        (RList[MainCtrlActualPos].R > 0.01);  // and (!DelayCtrlFlag)
    ResistorsFlag =
        (ResistorsFlag || ((DynamicBrakeFlag == true) &&
    ...
    if (DynamicBrakeFlag && (!FuseFlag) &&
        (DynamicBrakeType == dbrake_automatic) && Power110vIsAvailable &&
        Mains)  // hamowanie EP09   //TUHEX
    {
        ...
        MotorCurrent =
            -Max0R(
                MotorParam[0].fi *
                    (Vadd / (Vadd + MotorParam[0].Isat) - MotorParam[0].fi0),
                0) *
        ...
        if (ScndCtrlActualPos < 255)  // tak smiesznie bede wylaczal
        {
            ...
            if (DynamicBrakeFlag)  // hamowanie
            {
                if (DynamicBrakeType > 1) {
                    ...
                    // MotorCurrent:=-fi*n/Rz  {hamowanie silnikiem na oporach
                    // rozruchowych}
                    /*               begin
                    U:=0;
                    Isf:=Isat;
                ...
                //          writepaslog("Isat           ",
                //          FloatToStr(MotorParam[SP].Isat)); writepaslog("fi ",
                //          FloatToStr(MotorParam[SP].fi));
                Isf = Sign(U1) * MotorParam[SP].Isat;
                //          writepaslog("Isf            ", FloatToStr(Isf));
                Delta = square(Isf * Rz + Mn * MotorParam[SP].fi * n - U1) +
                        4.0 * U1 * Isf *
                            Rz;  // 105 * 1.67 + Mn * 140.9 * 20.532 - U1
                //          DeltaQ = Isf * Rz + Mn * MotorParam[SP].fi * n - U1
                //          + 4 * U1 * Isf * Rz; writepaslog("Delta          ",
                ...
                if (Mains) {
                    if (U > 0)
                        MotorCurrent =
                            (U1 - Isf * Rz - Mn * MotorParam[SP].fi * n +
                             std::sqrt(Delta)) /
                            (2.0 * Rz);
                    else
                        MotorCurrent =
    ...
    EnginePower =
        abs(Itot) * (1 + RList[MainCtrlActualPos].Mn) * abs(U) / 1000.0;
    ...
    if (MotorCurrent > 0) {
        if (FuzzyLogic(abs(n), nmax * 1.1, p_elengproblem))
            if (MainSwitch(false))
                EventFlag = true; /*zbyt duze obroty - wywalanie wskutek ognia
                                     okreznego*/
        ...
        if ((FuzzyLogic(abs(Im), Imax * 2, p_elengproblem) ||
            ...
            /*       or FuzzyLogic(Abs(U/Mn),2*NominalVoltage,1)) then */ /*poprawic
                                                                             potem*/
            if ((SetFlag(DamageFlag, dtrain_engine))) EventFlag = true;
...
//  główny konstruktor
// *************************************************************************************************
TMoverParameters::TMoverParameters(
    double VelInitial, std::string TypeNameInit, std::string NameInit, int Cab)
    : TypeName(TypeNameInit), Name(NameInit), CabOccupied(Cab) {
    WriteLog("------------------------------------------------------");
    WriteLog(
        "init default physic values for " + NameInit + ", [" + TypeNameInit +
        "]");
    Dim = TDimension();

    // BrakeLevelSet(-2); //Pascal ustawia na 0, przestawimy na odcięcie (CHK
    // jest jeszcze nie wczytane!)
    iLights[0] = 0;
    ...
    for (int b = 0; b < 2; ++b)
        for (int k = 0; k < 17; ++k) Lights[b][k] = 0;

    for (int b = 0; b < 4; ++b)
        for (int k = 1; k < 9; ++k) CompressorList[b][k] = 0;
    ...
    for (int b = 0; b < 2; ++b) {
        Couplers[b].AllowedFlag =
            3;  // domyślnie hak i hamulec, inne trzeba włączyć jawnie w FIZ
        Couplers[b].CouplingFlag = 0;
        Couplers[b].Connected = NULL;
        Couplers[b].ConnectedNr =
            0;  // Ra: to nie ma znaczenia jak nie podłączony
...
double TMoverParameters::Distance(
    const TLocation &Loc1, const TLocation &Loc2, const TDimension &Dim1,
    const TDimension &Dim2) {  // zwraca odległość pomiędzy pojazdami (Loc1) i
                               // (Loc2) z uwzględnieneim ich długości (kule!)
    return hypot(Loc2.X - Loc1.X, Loc1.Y - Loc2.Y) - 0.5 * (Dim2.L + Dim1.L);
};

double TMoverParameters::CouplerDist(
    TMoverParameters const *Left,
    TMoverParameters const
        *Right) {  // obliczenie odległości pomiędzy sprzęgami (kula!)
    return Distance(
        Left->Loc, Right->Loc, Left->Dim,
        Right->Dim);  // odległość pomiędzy sprzęgami (kula!)
};

bool TMoverParameters::Attach(
    int ConnectNo, int ConnectToNr, TMoverParameters *ConnectTo,
    int CouplingType, bool Enforce,
    bool Audible) {  //łączenie do swojego sprzęgu (ConnectNo) pojazdu
                     //(ConnectTo) stroną (ConnectToNr)
    // Ra: zwykle wykonywane dwukrotnie, dla każdego pojazdu oddzielnie
    // Ra: trzeba by odróżnić wymóg dociśnięcia od uszkodzenia sprzęgu przy
    // podczepianiu AI do składu
    ...
    auto &othercoupler =
        ConnectTo
            ->Couplers[(ConnectToNr != 2 ? ConnectToNr : coupler.ConnectedNr)];
    auto const distance{
        CouplerDist(this, ConnectTo) -
        (coupler.adapter_length + othercoupler.adapter_length)};

    auto const couplercheck{
        (Enforce) || ((distance <= dEpsilon) &&
    ...
    // stykaja sie zderzaki i kompatybilne typy sprzegow, chyba że łączenie na
    // starcie
    if (coupler.CouplingFlag == coupling::faux) {
        // jeśli wcześniej nie było połączone, ustalenie z której strony rysować
        // sprzęg
...
int TMoverParameters::DettachStatus(
    int ConnectNo) {  // Ra: sprawdzenie, czy odległość jest dobra do
                      // rozłączania
    // powinny być 3 informacje: =0 sprzęg już rozłączony, <0 da się rozłączyć.
    // >0 nie da się rozłączyć
    ...
    if ((Couplers[ConnectNo].CouplingFlag & ctrain_coupler) == 0)
        return -Couplers[ConnectNo]
                    .CouplingFlag;  // hak nie połączony - rozłączanie jest OK
    if (TestFlag(DamageFlag, dtrain_coupling))
        return -Couplers[ConnectNo]
                    .CouplingFlag;  // hak urwany - rozłączanie jest OK
                                    //    CouplerDist(ConnectNo);
    if ((Couplers[ConnectNo].type() != TCouplerType::Screw) ||
        (Neighbours[ConnectNo].distance < 0.01))
        return -Couplers[ConnectNo]
                    .CouplingFlag;  // można rozłączać, jeśli dociśnięty
    return (Neighbours[ConnectNo].distance > 0.2)
...
bool TMoverParameters::Dettach(int ConnectNo) {  // rozlaczanie
    ...
    auto couplingchange{
        coupler.CouplingFlag};  // presume we'll uncouple all active flags
    auto const couplingstate{DettachStatus(ConnectNo)};  // stan sprzęgu
    if (couplingstate < 0) {
        // gdy scisniete zderzaki, chyba ze zerwany sprzeg (wirtualnego nie
        // odpinamy z drugiej strony)
        std::tie(coupler.Connected, coupler.ConnectedNr, coupler.CouplingFlag) =
            std::tie(
                othercoupler.Connected, othercoupler.ConnectedNr,
                othercoupler.CouplingFlag) =
                std::make_tuple(nullptr, -1, coupling::faux);
    } else if (couplingstate > 0) {  // odłączamy węże i resztę, pozostaje
                                     // sprzęg fizyczny, który wymaga
    ...
    // set sound event flag
    couplingchange ^=
        coupler.CouplingFlag;  // remaining bits were removed from coupling
    if (couplingchange != 0) {
        int soundflag{sound::detach};  // HACK: use detach flag to indicate
                                       // removal of listed coupling
        std::vector<std::pair<coupling, sound>> const soundmappings = {
            {coupling::coupler, sound::attachcoupler},
            {coupling::brakehose, sound::attachbrakehose},
            {coupling::mainhose, sound::attachmainhose},
...
bool TMoverParameters::DirectionForward() {
    ...
    if ((MainCtrlPosNo > 0) && (DirActive < 1) &&
        ((CabActive != 0) ||
         ((InactiveCabFlag & activation::neutraldirection) == 0))) {
        ++DirActive;
        DirAbsolute = DirActive * CabActive;
        SendCtrlToNext("Direction", DirActive, CabActive);
        return true;
    } else if (
        (DirActive == 1) && (IsMainCtrlNoPowerPos()) && (TrainType == dt_EZT) &&
...
void TMoverParameters::BrakeLevelSet(
    ...
    if (fBrakeCtrlPos == b) return;  // nie przeliczać, jak nie ma zmiany
    fBrakeCtrlPos = b;
    ...
    // TODO: verify whether BrakeCtrlPosR and fBrakeCtrlPos can be rolled into
    // single variable
    BrakeCtrlPosR = fBrakeCtrlPos;
    int x = static_cast<int>(
        std::floor(fBrakeCtrlPos));  // jeśli odwołujemy się do BrakeCtrlPos w
                                     // pośrednich, to musi być
    // obcięte a nie zaokrągone
    while ((x > BrakeCtrlPos) &&
           (BrakeCtrlPos < BrakeCtrlPosNo))  // jeśli zwiększyło się o 1
        if (!IncBrakeLevelOld())             // T_MoverParameters::
            break;                           // wyjście awaryjne
    while ((x < BrakeCtrlPos) &&
           (BrakeCtrlPos >= -1))  // jeśli zmniejszyło się o 1
        if (!DecBrakeLevelOld())  // T_MoverParameters::
            break;
    BrakePressureActual =
        BrakePressureTable[BrakeCtrlPos];  // skopiowanie pozycji
    /*
    //youBy: obawiam sie, ze tutaj to nie dziala :P
    //Ra 2014-03: było tak zrobione, że działało - po każdej zmianie pozycji
    była wywoływana ta funkcja
    // if (BrakeSystem==Pneumatic?BrakeSubsystem==Oerlikon:false) //tylko
    Oerlikon akceptuje ułamki if(false) if (fBrakeCtrlPos>0.0)
...
bool TMoverParameters::BrakeLevelAdd(
...
bool TMoverParameters::IncBrakeLevel() {  // nowa wersja na użytek AI, false gdy
                                          // osiągnięto pozycję BrakeCtrlPosNo
...
bool TMoverParameters::ChangeCab(
    int direction) {  // zmiana kabiny i resetowanie ustawien
...
bool TMoverParameters::CurrentSwitch(bool const State) {
    ...
    // TBD, TODO: split off shunt mode toggle into a separate command? It
    // doesn't make much sense to have these two together like that dla 2Ls150
    if ((EngineType == TEngineType::DieselEngine) && (true == ShuntModeAllow) &&
        (DirActive == 0)) {
    ...
    if ((EngineType == TEngineType::DieselElectric) &&
        (true == ShuntModeAllow) && (IsMainCtrlNoPowerPos())) {
        ShuntMode = State;
        return true;
    }
...
// KURS90 - sprężarka pantografów; Ra 2014-07: teraz jest to zbiornik rozrządu,
// chociaż to jeszcze nie tak
void TMoverParameters::UpdatePantVolume(double dt) {
    ...
    PantCompFlag |=
        ((PantPress < 4.2) &&
         (true == (Pantographs[end::front].is_active |
                   Pantographs[end::rear]
                       .is_active))  // TODO: any_pantograph_is_active method
    ...
    if ((EnginePowerSource.SourceType ==
         TPowerSource::CurrentCollector)  // tylko jeśli pantografujący
        && (EnginePowerSource.CollectorParameters.CollectorsNo > 0)) {
        // Ra 2014-07: zasadniczo, to istnieje zbiornik rozrządu i zbiornik
        // pantografów - na razie mamy razem Ra 2014-07: kurek trójdrogowy łączy
        // spr.pom. z pantografami i wyłącznikiem ciśnieniowym WS Ra 2014-07:
        // zbiornika rozrządu nie pompuje się tu, tylko pantografy; potem można
        // zamknąć WS i odpalić resztę
        if (PantAutoValve ? (PantPress < ScndPipePress)
                          : bPantKurek3)  // kurek zamyka połączenie z ZG
        {  // zbiornik pantografu połączony ze zbiornikiem głównym - małą
           // sprężarką się tego nie napompuje
            // Ra 2013-12: Niebugocław mówi, że w EZT nie ma potrzeby odcinać
            // kurkiem
            PantPress = ScndPipePress;
            // ograniczenie ciśnienia do MaxPress (tylko w pantografach!)
            PantPress = clamp(
                ScndPipePress, 0.0,
                EnginePowerSource.CollectorParameters.MaxPress);
            PantVolume = (PantPress + 1.0) *
                         0.1;  // objętość, na wypadek odcięcia kurkiem
        } else {  // zbiornik główny odcięty, można pompować pantografy
            if (PantCompFlag) {
                // włączona mała sprężarka
                PantVolume += dt
                              // Ra 2013-12: Niebugocław mówi, że w EZT
                              // nabija 1.5 raz wolniej niż jak było 0.005
                              * (TrainType == dt_EZT ? 0.003 : 0.005) /
                              std::max(1.0, PantPress) *
                              (0.45 - ((0.1 / PantVolume / 10) - 0.1)) / 0.45;
            }
            PantPress = clamp(
                (10.0 * PantVolume) - 1.0, 0.0,
        ...
        if (!PantCompFlag && (PantVolume > 0.1))
            PantVolume -=
                dt * 0.0003 *
                std::max(
                    1.0, PantPress * 0.5);  // nieszczelności: 0.0003=0.3l/s
        ...
        if (PantPress < EnginePowerSource.CollectorParameters.MinPress) {
            ...
            if (true == PantPressSwitchActive) {
                ...
                if (TrainType != dt_EZT) {
                    ...
                    // unless there's alternate source of traction voltage
                    if (GetTrainsetHighVoltage() <
                        EnginePowerSource.CollectorParameters.MinV) {
                        // TODO: check whether line breaker should be open
                        // EMU-wide
                        MainSwitch(
                            false, (TrainType == dt_EZT ? range_t::unit
                                                        : range_t::local));
                    }
                } else {
                    // specialized variant for EMU -- pwr system disables
                    // converter and heating, and prevents their activation
                    // until pressure switch is set again
                    PantPressLockActive = true;
                    // TODO: separate 'heating allowed' from actual heating
                    // flag, so we can disable it here without messing up
                    // heating toggle
            ...
            if (PantPress >= 4.6) {
                // NOTE: we require active low power source to prime the
                // pressure switch this is a work-around for potential isssues
                // caused by the switch activating on otherwise idle vehicles,
                // but should check whether it's accurate
                ...
                if (PantPress >= 4.8) {
                    ...
                    // TODO: governor lock, disables usage until pressure drop
                    // below 3.8 (should really make compressor object we could
                    // reuse)
                    PantCompFlag = false;
                }
            }
...
void TMoverParameters::UpdateBatteryVoltage(
    double dt) {  // przeliczenie obciążenia baterii
    ...
    if ((BatteryVoltage > 0) && (EngineType != TEngineType::DieselEngine) &&
        ...
        // HACK: allow to draw power also from adjacent converter, applicable
        // for EMUs
        // TODO: expand power cables system to include low voltage power
        // transfers HACK: emulate low voltage generator powered directly by the
        // diesel engine
        ...
        if ((NominalBatteryVoltage / BatteryVoltage < 1.22) &&
            ...
            if (Mains)
                ...
                sn3 = 0;
            if (iLights[0] & 63)  // 64=blachy, nie ciągną prądu //rozpisać na
                                  // poszczególne żarówki...
                sn4 = dt * 0.003;
            else
                sn4 = 0;
        ...
        if (!Battery) {
            ...
            sn4 = dt * 0.000001;
            sn5 = dt *
                  0.000001;  // bardzo powolny spadek przy wyłączonych bateriach
        };
        ...
        if (NominalBatteryVoltage / BatteryVoltage > 1.57)
            if (MainSwitch(false) &&
                ...
                (EngineType != TEngineType::WheelsDriven))
                EventFlag = true;  // wywalanie szybkiego z powodu zbyt niskiego
                                   // napiecia
        if (BatteryVoltage > NominalBatteryVoltage)
            BatteryVoltage =
                NominalBatteryVoltage;  // wstrzymanie ładowania pow. 110V
        if (BatteryVoltage < 0.01) BatteryVoltage = 0.01;
    } else {
        // TODO: check and implement proper way to handle this for diesel
        // engines
...
// *****************************************************************************
// Q: 20160713
// Zwraca iloraz różnicy między przewodem kontrolnym i głównym oraz
// DeltaPipePress
// *****************************************************************************
...
double TMoverParameters::PipeRatio(void) {
    ...
    if (DeltaPipePress > 0)
        ...
        {
            if ((3.0 * PipePress) >
                (HighPipePress + LowPipePress + LowPipePress))
                pr = (HighPipePress - Min0R(HighPipePress, PipePress)) /
                     (DeltaPipePress * 4.0 / 3.0);
            else
                pr = (HighPipePress - 1.0 / 3.0 * DeltaPipePress -
                      Max0R(LowPipePress, PipePress)) /
                     (DeltaPipePress * 2.0 / 3.0);
            // if (not TestFlag(BrakeStatus, b_Ractive))
            //     and(BrakeMethod and 1 = 0) and TestFlag(BrakeDelays,
            //     bdelay_R) and (Power < 1) and
            //         (BrakeCtrlPos < 1) then pr : = Min0R(0.5, pr);
            // if (Compressor > 0.5)
            //     then pr : = pr * 1.333; // dziwny rapid wywalamy
        } else
            pr = (HighPipePress -
                  Max0R(LowPipePress, Min0R(HighPipePress, PipePress))) /
                 DeltaPipePress;
    else
        pr = 0;
    return pr;
...
double TMoverParameters::EngineIdleRPM() const {
    return (
        ...
        : EngineType == TEngineType::DieselElectric
            ...
            : std::numeric_limits<double>::max());  // shouldn't ever get here
                                                    // but, eh
}
...
void TMoverParameters::CollisionDetect(int const End, double const dt) {
    ...
    switch (End) {
        case 0: {
            CCF =
                ComputeCollision(
                    velocity, othervehiclevelocity, TotalMass,
                    othervehicle->TotalMass,
                    (coupler.beta + othercoupler.beta) / 2.0, VirtualCoupling) /
                (dt);
            break;  // yB: ej ej ej, a po
        }
        case 1: {
            CCF = ComputeCollision(
                      othervehiclevelocity, velocity, othervehicle->TotalMass,
    ...
    if (Global.crash_damage) {
        if ((-coupler.Dist >= coupler.DmaxB) &&
            (FuzzyLogic(
                std::abs(CCF), 5.0 * (coupler.FmaxC + 1.0), p_coupldmg))) {
            // small chance to smash the coupler if it's hit with excessive
            // force
            damage_coupler(End);
        }
        ...
        if ((coupler.CouplingFlag == coupling::faux ||
             (true ==
              TestFlag(
                  othervehicle->DamageFlag,
                  dtrain_out)))) {  // HACK: limit excessive speed derailment
                                    // checks to vehicles which aren't part of
                                    // the same consist
            ...
            if (velocitydifference > safevelocitylimit) {
                // HACK: crude estimation for potential derail, will take place
                // with velocity difference > 15 km/h adjusted for vehicle mass
                // ratio
                if ((false == TestFlag(DamageFlag, dtrain_out)) ||
                    (false == TestFlag(othervehicle->DamageFlag, dtrain_out))) {
                    WriteLog(
                        "Bad driving: " + Name + " and " + othervehicle->Name +
                        " collided with velocity " +
                        to_string(velocitydifference, 0) + " km/h");
                }
    ...
    if (false == TestFlag(othervehicle->DamageFlag, dtrain_out)) {
        auto const othervehicleaccelerationchange{
            (othervehiclevelocity - othervehicle->V) / dt};
...
void TMoverParameters::damage_coupler(int const End) {
    ...
    if (SetFlag(DamageFlag, dtrain_coupling)) EventFlag = true;
    ...
    if (coupler.Connected != nullptr) {
        switch (End) {
            ...
            case 0: {
                coupler.Connected->Couplers[end::rear].CouplingFlag =
                    coupling::faux;
                break;
            }
            case 1: {
                coupler.Connected->Couplers[end::front].CouplingFlag =
                    coupling::faux;
                break;
            }
            default: {
...
void TMoverParameters::derail(int const Reason) {
    if (SetFlag(DamageFlag, dtrain_out)) {
        ...
        V *= 0.65;
        if (Vel < 5.0) {
            // HACK: prevent permanent axle spin in static vehicle after a
            // collision
            nrot = 0.0;
...
// *************************************************************************************************
double TMoverParameters::ComputeMovement(
    ...
    if (!TestFlag(
            DamageFlag,
            dtrain_out)) {  // Ra: to przepisywanie tu jest bez sensu
        RunningShape = Shape;
        RunningTrack = Track;
        ...
        //             0)*/; // zasadniczo ED oporowe nie zmienia napięcia w
        //             sieci
    }
    ...
    if (CategoryFlag == 4)
        ...
        TestFlag(CategoryFlag, 1) && TestFlag(RunningTrack.CategoryFlag, 1))
        if (TestFlag(DamageFlag, dtrain_out)) {
            OffsetTrackV = -0.2;
            OffsetTrackH = Sign(RunningShape.R) * 0.2;
        }

    // TODO: investigate, seems supplied NewRot is always 0 although the code
    // here suggests some actual values are expected
    Loc = NewLoc;
    Rot = NewRot;
    ...
    if (dL == 0)  // oblicz przesuniecie}
    {
        ...
        // clamp( dt * 3.0, 0.0, 1.0 ) ); // prawo Newtona ale z wygladzaniem
        // (średnia z poprzednim)
        ...
        if (abs(Shape.R) > 0.01)
            AccN = square(V) / Shape.R +
                   g * Shape.dHrail / TrackW;  // Q: zamieniam SQR() na sqr()
        else
        ...
        AccSVBased = interpolate(
            AccSVBased, (V - Vprev) / dt, clamp(dt * 3.0, 0.0, 1.0));
        ...
        /*
                ...
                if( FuzzyLogic( ( 10.0 + Track.DamageFlag ) * Mass * Vel / Vmax,
        500000.0, p_accn ) ) {
                    // Ra: czemu tu masa bez ładunku?
                    AccV /= ( 2.0 * 0.95 + 2.0 * Random() * 0.1 ); // 95-105% of
        base modifier (2.0)
                }
                else
        #endif
                ...
                if (AccV > 1.0)
                    AccN += (7.0 - Random(5)) * (100.0 + Track.DamageFlag / 2.0)
        * AccV / 2000.0;
        */
        ...
        if (TestFlag(CategoryFlag, 1)) {
            if (FuzzyLogic(
                    (AccN / g) *
                        (1.0 + 0.1 * (Track.DamageFlag && dtrack_freerail)),
                    TrackW / Dim.H, 1) ||
                ...
                if (SetFlag(DamageFlag, dtrain_out)) {
                    ...
                    if (TestFlag(Track.DamageFlag, dtrack_norail))
                        ...
                        DerailReason =
                            2;  // Ra: powód wykolejenia: przewrócony na łuku
                }
        ...
        if (!TestFlag(RunningTrack.CategoryFlag, CategoryFlag))
            if (SetFlag(DamageFlag, dtrain_out)) {
                ...
                MainSwitch(false, range_t::local);
                DerailReason =
                    4;  // Ra: powód wykolejenia: nieodpowiednia trajektoria
            }
    ...
    auto const d{
        (EngineType == TEngineType::WheelsDriven ? dL * CabActive
                                                 :  // na chwile dla testu
             dL)};
...
double TMoverParameters::FastComputeMovement(
    double dt, const TTrackShape &Shape, TTrackParam &Track,
...
void TMoverParameters::compute_movement_(double const Deltatime) {
    ...
    if (EngineType == TEngineType::ElectricSeriesMotor) {
        ...
        if (ImaxHi > ImaxLo) {
            if (MotorOverloadRelayHighThreshold) {  // set high threshold
                if ((TrainType != dt_ET42) ? (RList[MainCtrlPos].Bn < 2)
                                           : (MainCtrlPos == 0)) {
                    Imax = ImaxHi;
                }
    ...
    if (VeselVolume > 0.0) {
        // sprężarka musi mieć jakąś niezerową wydajność żeby rozważać jej
        // załączenie i pracę
        CompressorCheck(Deltatime);
    }
    if (Power > 1.0) {
        // w rozrządczym nie (jest błąd w FIZ!) - Ra 2014-07: teraz we
        // wszystkich
        UpdatePantVolume(Deltatime);  // Ra 2014-07: obsługa zbiornika rozrządu
                                      // oraz pantografów
    }
    // heating
    ...
    if (((DCEMUED_CC & 1) != 0) &&
        ((Couplers[end::front].CouplingFlag & coupling::control) != 0)) {
        DynamicBrakeEMUStatus &=
            Couplers[end::front].Connected->DynamicBrakeEMUStatus;
    }
    if (((DCEMUED_CC & 2) != 0) &&
        ((Couplers[end::rear].CouplingFlag & coupling::control) != 0)) {
        DynamicBrakeEMUStatus &=
            Couplers[end::rear].Connected->DynamicBrakeEMUStatus;
    }
    ...
    Power24vVoltage = std::max(
        PowerCircuits[0].first, GetTrainsetVoltage(coupling::power24v));
    Power24vIsAvailable = (Power24vVoltage > 0);
    Power110vIsAvailable =
        ((PowerCircuits[1].first > 0) ||
         (GetTrainsetVoltage(coupling::power110v) > 0));
}

void TMoverParameters::MainsCheck(double const Deltatime) {
    //    if( MainsInitTime == 0.0 ) { return; }

    // TBD, TODO: move voltage calculation to separate method and use also in
    // power coupler state calculation?
    ...
    auto const maincircuitpowersupply{
        (std::abs(localvoltage) > 0.1) || (GetTrainsetHighVoltage() > 0.1)};

    if (true == maincircuitpowersupply) {
        // all is well
        if (MainsInitTimeCountdown >= 0.0) {
            // NOTE: we ensure main circuit readiness meets condition
            // MainsInitTimeCountdown < 0 this allows for simpler rejection of
            // cases where MainsInitTime == 0
    ...
    switch (EngineType) {
        case TEngineType::ElectricSeriesMotor: {
            ...
            if (GroundRelayStart != start_t::manual) {
                // NOTE: we're ignoring intricaties of battery and converter
                // types as they're unlikely to be used
...
void TMoverParameters::PowerCouplersCheck(
    double const Deltatime, coupling const Coupling) {
    auto localvoltage{0.0};

    // local power sources
    // TODO: make local voltage calculations a separate method, store results in
    // PowerCircuit fields
    switch (Coupling) {
            ...
            if (Heating) {
                switch (HeatingPowerSource.SourceType) {
                    case TPowerSource::Generator: {
                        localvoltage =
                            HeatingPowerSource.EngineGenerator.voltage -
                            TotalCurrent * 0.02;
                        break;
                    }
                    case TPowerSource::CurrentCollector: {
                    ...
                    case TPowerSource::Main: {
                        // HACK: main circuit can be fed through couplers, so we
                        // explicitly check pantograph supply here
                        localvoltage =
                            (true == Mains ? PantographVoltage : 0.0);
                        break;
                    }
                    default: {
                        break;
    ...
    for (auto side = 0; side < 2; ++side) {
        ...
        // NOTE: in the loop we actually update the state of the coupler on the
        // opposite end of the vehicle
        auto &oppositecoupler{
            Couplers[(side == end::front ? end::rear : end::front)]};
        ...
        switch (Coupling) {
            case coupling::highvoltage: {
                auto const oppositehighvoltagecoupling{
                    (oppositecoupler.CouplingFlag & coupling::highvoltage) !=
                    0};
                auto const oppositeheatingcoupling{
                    (oppositecoupler.CouplingFlag & coupling::heating) != 0};

                oppositecouplingispresent =
                    (oppositehighvoltagecoupling || oppositeheatingcoupling);
                localpowerexportisenabled =
                    (oppositehighvoltagecoupling ||
            ...
            case coupling::power110v: {
                oppositecouplingispresent =
                    (TestFlag(
                        oppositecoupler.CouplingFlag,
                        oppositecoupler.PowerCoupling)) &&
        ...
        if (coupler.Connected != nullptr) {
            auto const &connectedcoupler{
                coupler.Connected->Couplers[coupler.ConnectedNr]};
            auto const *connectedcoupling =
                (Coupling == coupling::highvoltage
                     ? &connectedcoupler.power_high
                 : Coupling == coupling::power110v
                     ? &connectedcoupler.power_110v
                 : Coupling == coupling::power24v ? &connectedcoupler.power_24v
                                                  : nullptr);
            auto const connectedvoltage{(
                connectedcoupling->is_live ? connectedcoupling->voltage : 0.0)};
            oppositecoupling->voltage = std::max(
                oppositecoupling->voltage,
                connectedvoltage - coupling->current * 0.02);
            oppositecoupling->is_live =
                (connectedvoltage > 0.1) && (oppositecouplingispresent);
        }
        ...
        if (localpowersource) {
            ...
            oppositecoupling->is_live |=
                (abslocalvoltage > 0.1) && (localpowerexportisenabled);
        }
    }
    ...
    switch (Coupling) {
        case coupling::highvoltage: {
            couplervoltage = Couplers[end::front].power_high.voltage +
                             Couplers[end::rear].power_high.voltage;
            break;
        }
        case coupling::power110v: {
            couplervoltage = Couplers[end::front].power_110v.voltage +
                             Couplers[end::rear].power_110v.voltage;
            break;
        }
        case coupling::power24v: {
            couplervoltage = Couplers[end::front].power_24v.voltage +
                             Couplers[end::rear].power_24v.voltage;
            break;
        }
        default: {
            break;
    ...
    for (auto side = 0; side < 2; ++side) {
        ...
        auto const &connectedothercoupler{coupler.Connected->Couplers[(
            coupler.ConnectedNr == end::front ? end::rear : end::front)]};
        auto const *connectedothercoupling =
            (Coupling == coupling::highvoltage
                 ? &connectedothercoupler.power_high
             : Coupling == coupling::power110v
                 ? &connectedothercoupler.power_110v
             : Coupling == coupling::power24v ? &connectedothercoupler.power_24v
                                              : nullptr);
        auto const extracurrent =
            (Coupling == coupling::highvoltage
                 ? std::abs(Itot) * IsVehicleEIMBrakingFactor()
                 : 0.0);
        ...
        if (false == localpowersource) {
            ...
            if (couplervoltage != 0.0) {
                ...
                coupling->current = (*totalcurrent + extracurrent) *
                                    coupling->voltage /
                                    couplervoltage;  // obciążenie rozkladane
                                                     // stosownie do napiec
...
double TMoverParameters::ShowEngineRotation(
    int VehN) {  // Zwraca wartość prędkości obrotowej silnika wybranego
                 // pojazdu. Do 3 pojazdów (3×SN61).
    int b;
        ...
        case 3:  // to nie uwzględnia ewentualnego odwrócenia pojazdu w środku
            for (b = 0; b <= 1; ++b)
                if (TestFlag(Couplers[b].CouplingFlag, coupling::control))
                    if (Couplers[b].Connected->Power > 0.01)
                        if (TestFlag(
                                Couplers[b].Connected->Couplers[b].CouplingFlag,
                                coupling::control))
                            if (Couplers[b]
                                    .Connected->Couplers[b]
                                    .Connected->Power > 0.01)
                                return fabs(Couplers[b]
                                                .Connected->Couplers[b]
                                                .Connected->enrot);
            break;
    };
...
void TMoverParameters::ConverterCheck(double const Timestep) {
    // TODO: move other converter checks here, to have it all in one place for
    // potential device object
    ...
    switch (ConverterStart) {
        case start_t::disabled: {
            ...
            // NOTE: if there's no converter in vehicle we can end the check
            // here
            return;
        }
    ...
    if ((ConverterAllow) && (ConverterAllowLocal) && (false == ConvOvldFlag) &&
        ...
        // HACK: allow carriages to operate converter without (missing) fuse
        // prerequisite
        && ((Power > 1.0 ? Mains : GetTrainsetHighVoltage() > 0.0))) {
        // delay timer can be optionally configured, and is set anew whenever
        // converter goes off
    ...
    if ((ConverterOverloadRelayStart == start_t::converter) &&
        ...
        // reset converter overload relay if the converter was switched off,
        // unless it's damaged
        ConvOvldFlag = false;
    }
};
...
void TMoverParameters::HeatingCheck(double const Timestep) {
    ...
    switch (HeatingPowerSource.SourceType) {
        case TPowerSource::Generator: {
            if ((HeatingPowerSource.EngineGenerator.engine_revolutions !=
                 nullptr) &&
                ...
                generator.voltage =
                    (false == HeatingAllow
                         ...
                         ? generator.voltage_min * absrevolutions /
                               generator.revolutions_min
                         :
                         //                    absrevolutions >
                         //                    generator.revolutions_max ?
                         //                    generator.voltage_max *
                         //                    absrevolutions /
                         //                    generator.revolutions_max :
                         interpolate(
    ...
    if (Heating) {
        TotalCurrent +=
            1000 * HeatingPower /
            voltage;  // heater power cost presumably specified in kilowatts
    }
}

// water pump status check
void TMoverParameters::WaterPumpCheck(double const Timestep) {
    // NOTE: breaker override with start type is sm42 specific hack, replace
    // with ability to define the presence of the breaker
    WaterPump.is_active =
        ((true == (Power24vIsAvailable || Power110vIsAvailable)) &&
         (true == WaterPump.breaker) && (false == WaterPump.is_disabled) &&
        ...
        ((true == (Power24vIsAvailable || Power110vIsAvailable)) &&
         ...
         ((FuelPump.is_active) ||
          (FuelPump.start_type == start_t::manual ? (FuelPump.is_enabled)
           : FuelPump.start_type == start_t::automatic
               ? (dizel_startup || Mains)
...
void TMoverParameters::OilPumpCheck(double const Timestep) {
    OilPump.is_active =
        ((true == (Power24vIsAvailable || Power110vIsAvailable)) &&
         (false == Mains) && (false == OilPump.is_disabled) &&
         ((OilPump.is_active) ||
    ...
    auto const minpressure{
        OilPump.pressure_minimum > 0.f ? OilPump.pressure_minimum
                                       : 0.15f};  // arbitrary fallback value

    OilPump.pressure_target =
        (enrot > 0.1 ? interpolate(
                           minpressure, OilPump.pressure_maximum,
                           static_cast<float>(EngineRPMRatio())) *
                           OilPump.resource_amount
         : true == OilPump.is_active
             ? std::min(minpressure + 0.1f, OilPump.pressure_maximum)
             :  // slight pressure margin to give time to switch off the pump
                // and start the engine
             0.f);

    if (OilPump.pressure < OilPump.pressure_target) {
        // TODO: scale change rate from 0.01-0.05 with oil/engine
        // temperature/idle time
        OilPump.pressure = std::min<float>(
            OilPump.pressure_target,
            OilPump.pressure + (enrot > 5.0 ? 0.05 : 0.035) * Timestep);
    }
    if (OilPump.pressure > OilPump.pressure_target) {
        OilPump.pressure = std::max<float>(
            OilPump.pressure_target,
...
void TMoverParameters::MotorBlowersCheck(double const Timestep) {
    ...
    for (auto &blower : MotorBlowers) {
        ...
        auto const start{
            (Vel >= blower.min_start_velocity && std::abs(Im) > 0.5)};
        ...
        blower.is_active = (
            ...
            (blower.start_type == start_t::battery     ? Power24vIsAvailable
             : blower.start_type == start_t::converter ? Power110vIsAvailable
                                                       : Mains)  // power source
            // breaker condition disabled until it's implemented in the class
            // data
            //         && ( true == blower.breaker )
            && (false == disable) &&
            ((true == blower.is_active) ||
             ((blower.stop_timer ==
               0.f)  // HACK: will be true for blower with exceeded
                     // start_velocity, and for one without start_velocity
              && (blower.start_type == start_t::manual ? blower.is_enabled
                                                       : true))));
    }
    // update
    for (auto &fan : MotorBlowers) {
        auto const revolutionstarget{
            (fan.is_active
                 ? (fan.speed > 0.f ? fan.speed * static_cast<float>(enrot) * 60
                                    : fan.speed * -1)
                 : 0.f)};
        ...
        if (revolutionstarget > 0.f) {
            auto const speedincreasecap{std::max(
                50.f, fan.speed * 0.05f *
                          -1)};  // 5% of fixed revolution speed, or 50
            fan.revolutions += clamp(
                                   revolutionstarget - fan.revolutions,
                                   speedincreasecap * -2, speedincreasecap) *
                               Timestep;
        } else {
...
void TMoverParameters::PantographsCheck(double const Timestep) {
    {
        ...
        auto const lowvoltagepower{
            valve.solenoid ? (Power24vIsAvailable || Power110vIsAvailable)
                           : true};
        auto const autostart{
            valve.start_type == start_t::automatic ||
            valve.start_type == start_t::manualwithautofallback};
        auto const manualcontrol{
            valve.start_type == start_t::manual ||
            valve.start_type == start_t::manualwithautofallback};

        PantsValve.is_active =
            (((valve.spring ? lowvoltagepower
                            : true))  // spring actuator needs power to maintain
                                      // non-default state
    ...
    for (auto &pantograph : Pantographs) {
        ...
        valve.is_active =
            (((valve.spring ? lowvoltagepower
             ...
             && (((manualcontrol && lowvoltagepower)
                      ? false == valve.is_disabled
                      : true))  // needs power to change state, without it just
                                // pass through
             && (((manualcontrol && lowvoltagepower) ? false == PantAllDown
                                                     : true)) &&
             ((valve.is_active) ||
              (manualcontrol && lowvoltagepower && valve.is_enabled) ||
              (autostart &&
               lowvoltagepower)));  // shouldn't ever get this far but, eh

        auto const pantographexists{
            (EnginePowerSource.SourceType == TPowerSource::CurrentCollector) &&
            (EnginePowerSource.CollectorParameters.PhysicalLayout &
             (1 << pant_id))};

        pantograph.is_active =
            ((valve.is_active) && (PantsValve.is_active) && (pantographexists)
...
void TMoverParameters::LightsCheck(double const Timestep) {
    ...
    light.is_active = (
        ...
        ((true == light.is_active) ||
         (light.start_type == start_t::manual ? light.is_enabled : true)));
...
double TMoverParameters::ShowCurrent(
    int AmpN) const {  // Odczyt poboru prądu na podanym amperomierzu
    switch (EngineType) {
...
bool TMoverParameters::IncMainCtrl(int CtrlSpeed) {
                ...
                if (CtrlSpeed > 1) {
                    OK =
                        (IncMainCtrl(1) &&
                         IncMainCtrl(
                             CtrlSpeed -
                             1));  // a fail will propagate up the recursion
            ...
            case TEngineType::ElectricSeriesMotor: {
                ...
                if (CtrlSpeed > 1) {
                    ...
                    if (TrainType == dt_ET40) {
                        break;  // this means ET40 won't react at all to fast
                                // acceleration command. should it issue just
                                // IncMainCtrl(1) instead?
                    }
                    ...
                    if (Imax == ImaxHi) {
                        if (RList[MainCtrlPos].Bn > 1) {
                            /* NOTE: disabled, relay configuration was moved to
                               compute_movement_ if( true == MaxCurrentSwitch(
                               false )) {
                    ...
                    //  if (Abs(Im) > IminHi)
                    //   {
                    //    MainCtrlPos--; //Blokada nastawnika po przekroczeniu
                    //    minimalnego pradu OK = false;
                    //   }
                    //}
                }
    ...
    // styczniki tkwily na tej samej pozycji (LastRelayTime byl caly czas 0 i
    // rosl po puszczeniu plusa)
...
bool TMoverParameters::DecMainCtrl(int CtrlSpeed) {
    ...
    if ((MainCtrlPosNo <= 0) || (CabActive == 0)) {
                    ...
                    switch (EngineType) {
                        ...
                        case TEngineType::ElectricSeriesMotor: {
                            if (CtrlSpeed == 1) /*and (ScndCtrlPos=0)*/
                            {
                                ...
                                //                 MainCtrlActualPos:=0;
                                //                 //yBARC: co to tutaj robi? ;)
                                OK = true;
                            } else if (CtrlSpeed > 1) /*and (ScndCtrlPos=0)*/
                            {
                                OK = true;
                                if (RList[MainCtrlPos].R ==
                                    0)  // Q: tu zrobilem = ;]
                                    DecMainCtrl(1);
                                while ((RList[MainCtrlPos].R > 0) &&
                                       DecMainCtrl(1))
        ...
        if (OK) {
            /*OK:=*/SendCtrlToNext(
                "MainCtrl", MainCtrlPos, CabActive);  // hmmmm...???!!!
    ...
    if (OK) {
        if (DelayCtrlFlag) {
            if (LastRelayTime >= InitialCtrlDelay) LastRelayTime = 0;
...
bool TMoverParameters::IsMainCtrlActualNoPowerPos() const {
    // TODO: wrap controller pieces into a class for potential specializations,
    // similar to brake subsystems
...
bool TMoverParameters::IncScndCtrl(int CtrlSpeed) {
    ...
    if ((DynamicBrakeFlag) && (TrainType == dt_ET42) && (CabActive != 0) &&
        (IsMainCtrlNoPowerPos()) && (ScndCtrlPos == 0)) {
        OK = DynamicBrakeSwitch(false);
    } else if (
        (ScndCtrlPosNo > 0) && (CabActive != 0) &&
        !((TrainType == dt_ET42) &&
          ((Imax == ImaxHi) ||
           ((DynamicBrakeFlag) && (MainCtrlPowerPos() > 0))))) {
        //     if (RList[MainCtrlPos].R=0) and (MainCtrlPos>0) and
        //     (ScndCtrlPos<ScndCtrlPosNo) and (not CoupledCtrl) then
        if ((ScndCtrlPos < ScndCtrlPosNo) && (!CoupledCtrl)) {
            // TBD, TODO: refactor this validation as part of relay check
            // routine (currently in tractionforce() TBD, TODO: diesel electric
            // engine utilize scndctrlactualpos like the other types?
            if ((EngineType == TEngineType::DieselElectric) &&
                ((IsMainCtrlNoPowerPos()) || (AutoRelayFlag) || (ShuntMode) ||
                 (false == Mains))) {
                OK = false;
            } else {
                if (CtrlSpeed == 1) {
                    ...
                    ScndCtrlPos =
                        ScndCtrlPosNo;  // takie chamskie, potem poprawie
                }
                OK = true;
            }
    ...
    // hunter-101012: poprawka
    if (OK)
        if (LastRelayTime > CtrlDelay) LastRelayTime = 0;

    if ((OK) && (EngineType == TEngineType::ElectricInductionMotor) &&
        (ScndCtrlPosNo == 1) && (MainCtrlPos > 0)) {
        SpeedCtrlValue = Vel;
    ...
    if ((OK) && (SpeedCtrl) && (ScndCtrlPos == 1) &&
        (EngineType == TEngineType::DieselEngine)) {
        // NOTE: round() already adds 0.5, are the ones added here as well
        // correct?
...
bool TMoverParameters::DecScndCtrl(int CtrlSpeed) {
    ...
    if ((IsMainCtrlNoPowerPos()) && (CabActive != 0) &&
        (TrainType == dt_ET42) && (ScndCtrlPos == 0) && !(DynamicBrakeFlag) &&
        (CtrlSpeed == 1)) {
    ...
    // hunter-101012: poprawka
    if (OK)
        if (LastRelayTime > CtrlDownDelay) LastRelayTime = 0;
...
int TMoverParameters::GetVirtualScndPos() {
    if (TrainType == dt_ET42) {
        if (DynamicBrakeFlag && !ScndCtrlPos) return -1;
    }
    return ScndCtrlPos;
}

bool TMoverParameters::IsScndCtrlNoPowerPos() const {
    // TODO: refine the check on account of potential electric series vehicles
    // with speed control
...
bool TMoverParameters::CabActivisation(bool const Enforce) {
    ...
    if (OK) {
        ...
        SecuritySystem.set_enabled(
            true);  // activate the alerter TODO: make it part of control based
                    // cab selection
...
bool TMoverParameters::CabDeactivisation(bool const Enforce) {
    ...
    OK =
        Enforce || IsCabMaster();  // o ile obsada jest w kabinie ze sterowaniem
    if (OK) {
        ...
        SecuritySystem.set_enabled(
            false);  // deactivate alerter TODO: make it part of control based
                     // cab selection
...
bool TMoverParameters::Sandbox(bool const State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        ...
        auto const couplingtype =
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control);
...
bool TMoverParameters::SandboxAutoAllow(bool State) {
    // SendCtrlToNext("SandboxAutoAllow", int(State), CabActive,
    // ctrain_controll);
...
bool TMoverParameters::BatterySwitch(bool State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            "BatterySwitch", (State ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::SpringBrakeActivate(bool State) {
    if (Power24vIsAvailable || Power110vIsAvailable) {
        SendCtrlToNext(
            "SpringBrakeActivate", int(State), CabActive,
            SpringBrake.MultiTractionCoupler);
...
bool TMoverParameters::SpringBrakeRelease() {
    if (SpringBrake.IsReady &&
        SpringBrake.Cylinder->P() < SpringBrake.MinForcePressure) {
...
bool TMoverParameters::DirectionBackward(void) {
    ...
    if ((MainCtrlPosNo > 0) && (DirActive > -1) &&
        ((CabActive != 0) ||
         ((InactiveCabFlag & activation::neutraldirection) == 0))) {
        if (EngineType == TEngineType::WheelsDriven) --CabActive;
        //    else
        --DirActive;
        DirAbsolute = DirActive * CabActive;
        SendCtrlToNext("Direction", DirActive, CabActive);
...
bool TMoverParameters::EIMDirectionChangeAllow(void) const {
    bool OK = false;
    /*
        // NOTE: disabled while eimic variables aren't immediately synced with
       master controller changes inside ai module OK = (EngineType !=
       TEngineType::ElectricInductionMotor || ((eimic <= 0) && (eimic_real <= 0)
       && (Vel < 0.1)));
    */
...
bool TMoverParameters::AntiSlippingButton(void) {
    // NOTE: disabled the sandbox part, it's already controlled by another part
    // of the AI routine
    return (AntiSlippingBrake() /*|| Sandbox(true)*/);
}

// water pump breaker state toggle
bool TMoverParameters::WaterPumpBreakerSwitch(
    ...
    */
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            "WaterPumpBreakerSwitch", (WaterPump.breaker ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::WaterPumpSwitch(bool State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            "WaterPumpSwitch", (WaterPump.is_enabled ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::WaterPumpSwitchOff(bool State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            "WaterPumpSwitchOff", (WaterPump.is_disabled ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::WaterHeaterBreakerSwitch(
    bool State, range_t const Notify) {
    /*
        if( FuelPump.start_type == start::automatic ) {
            // automatic fuel pump ignores 'manual' state commands
    ...
    */
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            "WaterHeaterBreakerSwitch", (WaterHeater.breaker ? 1 : 0),
            CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::WaterHeaterSwitch(bool State, range_t const Notify) {
    ...
    */
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            "WaterHeaterSwitch", (WaterHeater.is_enabled ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::WaterCircuitsLinkSwitch(
    bool State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            "WaterCircuitsLinkSwitch", (WaterCircuitsLink ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::FuelPumpSwitch(bool State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            "FuelPumpSwitch", (FuelPump.is_enabled ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::FuelPumpSwitchOff(bool State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            "FuelPumpSwitchOff", (FuelPump.is_disabled ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::OilPumpSwitch(bool State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            "OilPumpSwitch", (OilPump.is_enabled ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::OilPumpSwitchOff(bool State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            "OilPumpSwitchOff", (OilPump.is_disabled ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::MotorBlowersSwitch(
    bool State, end const Side, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            (Side == end::front ? "MotorBlowersFrontSwitch"
                                : "MotorBlowersRearSwitch"),
...
bool TMoverParameters::MotorBlowersSwitchOff(
    bool State, end const Side, range_t const Notify) {
    auto &fan{MotorBlowers[Side]};
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            (Side == end::front ? "MotorBlowersFrontSwitchOff"
                                : "MotorBlowersRearSwitchOff"),
...
bool TMoverParameters::CompartmentLightsSwitch(
    bool State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            "CompartmentLightsSwitch", (CompartmentLights.is_enabled ? 1 : 0),
            CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
// water pump state toggle
bool TMoverParameters::CompartmentLightsSwitchOff(
    bool State, range_t const Notify) {
    if (CompartmentLights.start_type == start_t::automatic) {
        // automatic lights ignore 'manual' state commands
        return false;
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            ...
            (CompartmentLights.is_disabled ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::MainSwitch(bool const State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        ...
        SendCtrlToNext(
            "MainSwitch", (State ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
    ...
    if ((false == State) || (true == MainSwitchCheck())) {
        if (true == State) {
            ...
            if ((EngineType == TEngineType::DieselEngine) ||
                ...
                // additional check, as vehicles without pantographs won't fail
                // relay checks earlier
                Mains = true;
            }
        } else {
            Mains = false;
            // potentially knock out the pumps if their switch doesn't force
            // them on
...
bool TMoverParameters::MainSwitchCheck() const {
    ...
    switch (EngineType) {
        ...
        case TEngineType::ElectricSeriesMotor:
        case TEngineType::ElectricInductionMotor: {
            // TODO: check whether we can simplify this check and skip the outer
            // EngineType switch
            powerisavailable =
                (EnginePowerSourceVoltage() >
                 0.5 * EnginePowerSource.MaxVoltage);
            break;
        }
        default: {
            break;
    ...
    return (
        ...
        (true == GroundRelay) && (true == NoVoltRelay) &&
        (true == OvervoltageRelay) && (LastSwitchingTime > CtrlDelay) &&
        (HasCamshaft
...
bool TMoverParameters::ConverterSwitch(bool State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            "ConverterSwitch", (State ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::CompressorSwitch(bool State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            "CompressorSwitch", (State ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::ChangeCompressorPreset(
    int const State, range_t const Notify) {
...
bool TMoverParameters::HeatingSwitch(bool const State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        ...
        // TBD: pass the requested state, or the actual state?
        SendCtrlToNext(
            "HeatingSwitch", (State ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
double TMoverParameters::EnginePowerSourceVoltage() const {
    return (
        EnginePowerSource.SourceType == TPowerSource::CurrentCollector
            ? std::max(GetTrainsetHighVoltage(), PantographVoltage)
        : EnginePowerSource.SourceType == TPowerSource::Accumulator
            ? Power24vVoltage
            : 0.0);
}
...
bool TMoverParameters::DecBrakeLevelOld(void) {
    ...
    if (BrakeCtrlPosNo > 0) {
        if (BrakeCtrlPos > ((BrakeHandle == TBrakeHandle::FV4a) ? -2 : -1)) {
            ...
            DBLO = true;
            //            if
            //            ((BrakePressureTable[BrakeCtrlPos].PipePressureVal<0.0)
            //            &&
            //                (BrakePressureTable[BrakeCtrlPos+1].PipePressureVal
            //                > 0)) LimPipePress=PipePress;
        } else
            DBLO = false;
    }
...
bool TMoverParameters::IncLocalBrakeLevel(float const CtrlSpeed) {
    ...
    if ((LocalBrakePosA < 1.0) /*and (BrakeCtrlPos<1)*/) {
        LocalBrakePosA =
            std::min(1.0, LocalBrakePosA + CtrlSpeed / LocalBrakePosNo);
        IBL = true;
    } else
        IBL = false;
...
bool TMoverParameters::DecLocalBrakeLevel(float const CtrlSpeed) {
    ...
    if (LocalBrakePosA > 0) {
        LocalBrakePosA =
            std::max(0.0, LocalBrakePosA - CtrlSpeed / LocalBrakePosNo);
        DBL = true;
    } else
        DBL = false;
...
bool TMoverParameters::BrakeReleaser(int state) {
    ...
    if (state != 0) {
        ...
        if ((false == ControlPressureSwitch) ||
            (false == ReleaserEnabledOnlyAtNoPowerPos) ||
...
bool TMoverParameters::UniversalBrakeButton(int button, int state) {
    ...
    if (Power24vIsAvailable || Power110vIsAvailable) {
        for (int i = 0; i < 3; i++) {
            flag = flag |
                   (UniversalBrakeButtonActive[i] ? UniversalBrakeButtonFlag[i]
                                                  : 0);
        }
    }
    ...
    if (TestFlag(
            UniversalBrakeButtonFlag[0] & UniversalBrakeButtonFlag[1] &
                UniversalBrakeButtonFlag[2],
...
// *************************************************************************************************
bool TMoverParameters::IncBrakePress(
    double &brake, double PressLimit, double dp) {
    bool IBP;
    //  if (DynamicBrakeType<>dbrake_switch) and (DynamicBrakeType<>dbrake_none)
    //  and
    //  ((BrakePress>2.0) or (PipePress<3.7{(LowPipePress+0.5)})) then
    if ((DynamicBrakeType != dbrake_switch) &&
        (DynamicBrakeType != dbrake_none) && (BrakePress > 2.0) &&
        (TrainType != dt_EZT))  // yB radzi nie sprawdzać ciśnienia w przewodzie
    // hunter-301211: dla EN57 silnikow nie odlaczamy
    {
        DynamicBrakeFlag =
            true;  // uruchamianie hamulca ED albo odlaczanie silnikow
...
// *************************************************************************************************
bool TMoverParameters::DecBrakePress(
    double &brake, double PressLimit, double dp) {
    bool DBP;
    ...
    //  if ((DynamicBrakeType != dbrake_switch) && ((BrakePress < 0.1) &&
    //  (PipePress > 0.45
    //  /*(LowPipePress+0.06)*/ )))
    if ((DynamicBrakeType != dbrake_switch) &&
        (BrakePress < 0.1))  // yB radzi nie sprawdzać ciśnienia w przewodzie
        DynamicBrakeFlag =
            false;  // wylaczanie hamulca ED i/albo zalaczanie silnikow
...
void TMoverParameters::UpdateBrakePressure(double dt) {
    ...
    Hamulec->ForceLeak(
        dt * AirLeakRate *
        0.25);  // fake air leaks from brake system reservoirs
...
void TMoverParameters::CompressorCheck(double dt) {
    ...
    EmergencyValveOpen =
        (Compressor >
         (EmergencyValveOpen ? EmergencyValveOff : EmergencyValveOn));
    if (EmergencyValveOpen) {
        float dV = PF(0, Compressor, EmergencyValveArea) * dt;
        CompressedVolume -= dV;
    }

    CompressedVolume = std::max(
        0.0, CompressedVolume -
                 dt * AirLeakRate * 0.1);  // nieszczelności: 0.001=1l/s

    Compressor = CompressedVolume / VeselVolume;

    // assorted operational logic
    auto const MaxCompressorF{
        CompressorList[TCompressorList::cl_MaxFactor][CompressorListPos] *
        MaxCompressor};
    auto const MinCompressorF{
        CompressorList[TCompressorList::cl_MinFactor][CompressorListPos] *
        MinCompressor};
    auto const CompressorSpeedF{
        CompressorList[TCompressorList::cl_SpeedFactor][CompressorListPos] *
        CompressorSpeed};
    auto const AllowFactor{
        CompressorList[TCompressorList::cl_Allow][CompressorListPos]};
    ...
    switch (CompressorPower) {
        ...
        case 3: {
            // HACK: make sure compressor coupled with diesel engine is always
            // ready for work
    ...
    auto const compressorpower{
        (CompressorPower == 0   ? Mains
         : CompressorPower == 3 ? Mains
                                : Power110vIsAvailable)};
    // TBD: split CompressorAllow into separate enable/disable flags, inherit
    // compressor from basic_device
    ...
    CompressorGovernorLock =
        ...
        (false ==
         pressureistoolow)  // unlock if pressure drops below minimal threshold
        && (pressureistoohigh ||
            CompressorGovernorLock);  // lock if pressure goes above maximum
                                      // threshold
    ...
    auto const coupledgovernorlock{
        ((Couplers[end::rear].Connected != nullptr) &&
         (true ==
          TestFlag(Couplers[end::rear].CouplingFlag, coupling::permanent)) &&
         (Couplers[end::rear].Connected->CompressorGovernorLock)) ||
        ((Couplers[end::front].Connected != nullptr) &&
         (true ==
          TestFlag(Couplers[end::front].CouplingFlag, coupling::permanent)) &&
    ...
    switch (CompressorPower) {
        case 3: {
            // the compressor is coupled with the diesel engine, engine
            // revolutions affect the output
            CompressedVolume +=
                CompressorSpeedF * (2.0 * MaxCompressorF - Compressor) /
                MaxCompressorF * EngineRPMRatio() * dt *
                (CompressorGovernorLock
                     ? 0.0
                     : 1.0);  // with the lock active air is vented out
            break;
        }
        default: {
            // the compressor is a stand-alone device, working at steady pace
            CompressedVolume += CompressorSpeedF *
                                (2.0 * MaxCompressorF - Compressor) /
                                MaxCompressorF * dt;
            break;
        }
    }
    ...
    if ((pressureistoohigh) &&
        ...
        // vent some air out if there's no governor lock to stop the compressor
        // from exceeding acceptable pressure level
        ...
        CompressedVolume *=
            (false == governorlockispresent ? 0.80 :  // arbitrary amount
                 ...
                 0.999);  // HACK: drop a tiny bit so the sound doesn't trigger
                          // repeatedly
        ...
        default: {
            ...
            /*
                        if( compressorowner != nullptr ) {
                            compressorowner->TotalCurrent += 0.0015 *
               compressorowner->PantographVoltage;
                        }
            */
            break;
        }
...
void TMoverParameters::UpdatePipePressure(double dt) {
    ...
    if (BrakeCtrlPosNo > 1) {
        if ((EngineType != TEngineType::ElectricInductionMotor)) {
            ...
            dpLocalValve = LocHandle->GetPF(
                std::max(lbpa, LocalBrakePosAEIM), Hamulec->GetBCP(),
                ScndPipePress, dt, 0);
        } else
            dpLocalValve = LocHandle->GetPF(
                LocalBrakePosAEIM, Hamulec->GetBCP(), ScndPipePress, dt, 0);

        LockPipe = PipePress < (LockPipe ? LockPipeOff : LockPipeOn);
        bool lock_new =
            (LockPipe && !UnlockPipe && (BrakeCtrlPosR > HandleUnlock)) ||
            ((EmergencyCutsOffHandle) &&
             (EmergencyValveFlow > 0));  // new simple codition based on .fiz
        bool lock_old =
            ((BrakeHandle ==
              TBrakeHandle::FV4a)  // old complex condition based on assumptions
             && ((PipePress < 2.75) && ((Hamulec->GetStatus() & b_rls) == 0)) &&
             (BrakeSubsystem == TBrakeSubSystem::ss_LSt) &&
             (TrainType != dt_EZT) && (!UnlockPipe));
        ...
        if (((BrakeOpModes & bom_PS) == 0) ||
            ((CabOccupied != 0) && (BrakeOpModeFlag != bom_PS))) {
            if ((BrakeOpModeFlag < bom_EP) ||
                ...
                dpMainValve =
                    Handle->GetPF(pos, PipePress, temp, dt, EqvtPipePress);
            } else {
                dpMainValve =
                    Handle->GetPF(0, PipePress, temp, dt, EqvtPipePress);
            }
        } else if (BrakeCtrlPos == Handle->GetPos(bh_EB)) {
            dpMainValve = Handle->GetPF(
                BrakeCtrlPosR, PipePress, temp, dt, EqvtPipePress);
        }
    ...
    if (((true == RadioStopFlag) || (true == AlarmChainFlag) ||
         ((true == EIMCtrlEmergency) && (LocalBrakePosA >= 1.0)) ||
         SecuritySystem.is_braking()) ||
        ((SpringBrakeDriveEmergencyVel >= 0) &&
         (Vel > SpringBrakeDriveEmergencyVel) && (SpringBrake.IsActive))
        /*
            // NOTE: disabled because 32 is 'load destroyed' flag, what does
           this have to do with emergency brake?
            // (if it's supposed to be broken coupler, such event sets
           alarmchainflag instead when appropriate)
    ...
    CntrlPipePress =
        Hamulec->GetVRP();  // ciśnienie komory wstępnej rozdzielacza
    ...
    switch (BrakeValve) {
        ...
        case TBrakeValve::EStED: {
            ...
            for (int b = 0; b < 2; b++)
                if (((TrainType & (dt_ET41 | dt_ET42)) != 0) &&
                    (Couplers[b].Connected !=
                     NULL))  // nie podoba mi się to rozwiązanie, chyba trzeba
                    // dodać jakiś wpis do fizyki na to
                    if (((Couplers[b].Connected->TrainType &
                          (dt_ET41 | dt_ET42)) != 0) &&
                        ((Couplers[b].CouplingFlag & 36) == 36))
                        LocBrakePress = std::max(
                            Couplers[b].Connected->LocHandle->GetCP(),
                            LocBrakePress);
        ...
        default: {
            ...
            //            ErrorLog( "Unsupported brake valve type (" +
            //            std::to_string( BrakeValve ) + ") in " + TypeName );
            //            ::PostQuitMessage( 0 );
            break;
        }
    }  // switch

    if (((BrakeHandle == TBrakeHandle::FVel6) ||
         (BrakeHandle == TBrakeHandle::FVE408)) &&
        (CabOccupied != 0)) {
        if ((Power24vIsAvailable) && (DirActive != 0) &&
        ...
        if ((DCEMUED_EP_max_Vel > 0.001) && (Vel > DCEMUED_EP_max_Vel) &&
            (DynamicBrakeEMUStatus))
            temp1 = 0;
        if ((DCEMUED_EP_min_Im > 0.001) && (abs(Im) > DCEMUED_EP_min_Im) &&
            (DynamicBrakeEMUStatus))
            temp1 = 0;
    ...
    Pipe->Act();
    PipePress = Pipe->P();
    if ((Hamulec->GetBrakeStatus() & b_dmg) ==
        b_dmg)       // jesli hamulec wyłączony
        temp = 0.0;  // odetnij
    else
        temp = 1.0;  // połącz
    ...
    if (ASBType == 128)
        Hamulec->ASB(
            int(SlippingWheels && (Vel > 1)) * (1 + 2 * int(nrot_eps < -0.01)));
    ...
    dpMainValve =
        dpMainValve / (100.0 * dt);  // normalizacja po czasie do syczenia;
    ...
    if (CompressedVolume < 0.0) CompressedVolume = 0.0;
}
...
void TMoverParameters::UpdateScndPipePressure(double dt) {
    ...
    if (Couplers[0].Connected != NULL)
        if (TestFlag(Couplers[0].CouplingFlag, ctrain_scndpneumatic)) {
            ...
            if (dv1 * dv1 > 0.00000000000001) c->switch_physics(true);
            c->Pipe2->Flow(-dv1);
        }
    // sprzeg 2
    if (Couplers[1].Connected != NULL)
        if (TestFlag(Couplers[1].CouplingFlag, ctrain_scndpneumatic)) {
            ...
            if (dv2 * dv2 > 0.00000000000001) c->switch_physics(true);
            c->Pipe2->Flow(-dv2);
        }
    if ((Couplers[1].Connected != NULL) && (Couplers[0].Connected != NULL))
        if ((TestFlag(Couplers[0].CouplingFlag, ctrain_scndpneumatic)) &&
            ...
            dV = 0.00025 * dt *
                 PF(Couplers[0].Connected->ScndPipePress,
    ...
    // NOTE: condition disabled to allow the air flow from the main hose to the
    // main tank as well
...
void TMoverParameters::UpdateSpringBrake(double dt) {
    ...
    if (!SpringBrake.Activate) {
        ...
        double dv =
            PF(desired_press, SpringBrake.SBP, SpringBrake.ValveOffArea);
    ...
    if (SpringBrake.SBP > SpringBrake.ResetPressure) SpringBrake.IsReady = true;

    SpringBrake.IsActive =
        SpringBrake.SBP < (SpringBrake.IsActive ? SpringBrake.PressureOff
                                                : SpringBrake.PressureOn);
...
double TMoverParameters::GetDVc(double dt) {
    ...
    if (Couplers[0].Connected != NULL)
        if (TestFlag(Couplers[0].CouplingFlag, ctrain_pneumatic)) {  //*0.85
            c = Couplers[0].Connected;  // skrot           //0.08 //e/D * L/D =
                                        // e/D^2 * L
            dv1 = 0.5 * dt *
                  PF(PipePress, c->PipePress,
                     (Spg) / (1.0 + 0.015 / Spg * Dim.L));
            if (dv1 * dv1 > 0.00000000000001) c->switch_physics(true);
            c->Pipe->Flow(-dv1);
        }
    // sprzeg 2
    ...
    //              0; // ktoś mi powie jaki jest sens tego bloku jeśli przepływ
    //              mnożony przez zero?
...
void TMoverParameters::ComputeConstans(void) {
    ...
    HideModifier =
        0;  // int(Couplers[0].CouplingFlag>0)+int(Couplers[1].CouplingFlag>0);
    ...
    // drag calculation
    {
        // NOTE: draft effect of previous vehicle is simplified and doesn't have
        // much to do with reality
        auto const *previousvehicle{
            Couplers[(V >= 0.0 ? end::front : end::rear)].Connected};
        auto dragarea{Dim.W * Dim.H};
        if (previousvehicle) {
            dragarea = std::max(
                0.0, dragarea - (0.85 * previousvehicle->Dim.W *
                                 previousvehicle->Dim.H));
        }
    ...
    if (CategoryFlag & 1) {
        ...
        // vehicles other than trains don't experience friction against the rail
        // on curves
        Curvature = 0.0;
    }
    // opór składu na łuku (youBy): +(500*TrackW/R)*TotalMassxg*0.001 do
    // FrictConst2s/d
    FrictConst2s =
        (TotalMassxg * ((500.0 * TrackW * Curvature) + 2.5 - HideModifier +
                        2 * BearingF / dtrain_bearing)) *
        0.001;
    FrictConst2d = (TotalMassxg * ((500.0 * TrackW * Curvature) + 2.0 -
                                   HideModifier + BearingF / dtrain_bearing)) *
                   0.001;
}
...
// *************************************************************************************************
// TBD, TODO: move some of the calculations out of the method, they're relevant
// to more than just force calculations
void TMoverParameters::ComputeTotalForce(double dt) {
    ...
    // ABu 300105: cos tu mieszalem , dziala teraz troche lepiej, wiec zostawiam
    {
        auto const vehicleisactive{
            (CabActive != 0) || (Vel > 0.0001) || (std::abs(AccS) > 0.0001) ||
            (LastSwitchingTime < 5) || (TrainType == dt_EZT) ||
            (TrainType == dt_DMU)};

        auto const movingvehicleahead{
            (Neighbours[end::front].vehicle != nullptr) &&
            ((Neighbours[end::front].vehicle->MoverParameters->Vel > 0.0001) ||
             (std::abs(Neighbours[end::front].vehicle->MoverParameters->AccS) >
              0.0001))};

        auto const movingvehiclebehind{
            (Neighbours[end::rear].vehicle != nullptr) &&
            ((Neighbours[end::rear].vehicle->MoverParameters->Vel > 0.0001) ||
             (std::abs(Neighbours[end::rear].vehicle->MoverParameters->AccS) >
              0.0001))};

        auto const calculatephysics{
            vehicleisactive || movingvehicleahead || movingvehiclebehind};
    ...
    if (EngineType ==
        TEngineType::ElectricSeriesMotor)  // potem ulepszyc! pantogtrafy!
    {  // Ra 2014-03: uwzględnienie kierunku jazdy w napięciu na silnikach, a
       // powinien być zdefiniowany nawrotnik
        ...
        EngineVoltage =
            (Power > 1.0 ? std::max(GetTrainsetHighVoltage(), PantographVoltage)
                         : 0.0);
    }
    ...
    if ((Vel > 0.1)  // crude trap, to prevent braked stationary vehicles from
                     // passing fb > mass * adhesive test
        &&
    ...
    if (true == SlippingWheels) {
        temp_nrot = ComputeRotatingWheel(
            Fwheels - Sign(nrot * M_PI * WheelDiameter - V) *
                          Adhesive(RunningTrack.friction) * TotalMassxg,
            dt, nrot);
        if (Sign(nrot * M_PI * WheelDiameter - V) *
                Sign(temp_nrot * M_PI * WheelDiameter - V) <
            0) {
            SlippingWheels = false;
    ...
    if (true == SlippingWheels) {
        Fwheels = Sign(temp_nrot * M_PI * WheelDiameter - V) * TotalMassxg *
                  Adhesive(RunningTrack.friction);
        if (Fwheels * Sign(V) > 0) {
            ...
            double factor =
                (FTrain - Fb * Sign(V) != 0 ? Fwheels / (FTrain - Fb * Sign(V))
                                            : 1.0);
            Fb *= factor;
            FTrain *= factor;
        }
        if (std::abs(nrot) < 0.1) {
            WheelFlat = sqrt(
                square(WheelFlat) + abs(Fwheels) / NAxles * Vel * 0.000002);
        }
...
double TMoverParameters::BrakeForceR(double ratio, double velocity) {
    ...
    if (MBPM > 2) {
        press = MaxBrakePress[1] +
                (MaxBrakePress[3] - MaxBrakePress[1]) *
                    std::min(1.0, (TotalMass - Mass) / (MBPM - Mass));
    } else {
        if (MaxBrakePress[1] > 0.1) {
            ...
            if ((BrakeDelayFlag & bdelay_R) &&
                (BrakeMethod % 128 != bp_Cosid) &&
                (BrakeMethod % 128 != bp_D1) && (BrakeMethod % 128 != bp_D2) &&
                (Power < 1) && (velocity < 40))
                ratio = ratio / 2;
...
double TMoverParameters::BrakeForceP(double press, double velocity) {
    double BFP = 0;
    double K = (((press * P2FTrans) - BrakeCylSpring) * BrakeCylMult[0] -
                BrakeSlckAdj) *
               BrakeRigEff;
...
double TMoverParameters::BrakeForce(TTrackParam const &Track) {
    ...
    if (SpringBrake.IsReady)
        K +=
            std::max(
                0.0, SpringBrake.MinForcePressure - SpringBrake.Cylinder->P()) *
            SpringBrake.MaxBrakeForce;

    u = ((BrakePress * P2FTrans) - BrakeCylSpring) * BrakeCylMult[0] -
        BrakeSlckAdj;
    ...
    if (NBrakeAxles * NBpA > 0) {
        if (Ntotal > 0)   // nie luz
            K += Ntotal;  // w kN
        K *= static_cast<double>(BrakeCylNo) /
             (NBrakeAxles * static_cast<double>(NBpA));  // w kN na os
    }
    ...
    //     else  //yB: to nie do konca ma sens, ponieważ ręczny w wagonie działa
    //     na jeden cylinder hamulcowy/wózek, dlatego potrzebne są oddzielnie
    //     liczone osie
...
double TMoverParameters::FrictionForce() const {
    double FF = 0;
    // ABu 240205: chyba juz ekstremalnie zoptymalizowana funkcja liczaca sily
    // tarcia
    if (abs(V) > 0.01)
...
double TMoverParameters::Adhesive(double staticfriction) const {
    double adhesion = 0.0;
    const double adh_factor = 0.25;  // współczynnik określający, jak bardzo
                                     // spada tarcie przy poślizgu
    const double slipfactor = 0.33;  // współczynnik określający, jak szybko
                                     // spada tarcie przy poślizgu
    const double sandfactor =
        1.25;  // współczynnik określający, jak mocno pomaga piasek
    /*
        ...
        {
            if (SandDose)
                adhesion = (Max0R(staticfriction * (100.0 + Vel) / ((50.0 + Vel)
    * 11.0), 0.048)) * (11.0 - 2.0 * Random(0.0, 1.0)); else adhesion =
    (staticfriction * (100.0 + Vel) / ((50.0 + Vel) * 10.0)) * (11.0 - 2.0 *
    Random(0.0, 1.0));
        }
        else
        {
            if (SandDose)
                ...
                adhesion = (staticfriction * 0.02) * (11.0 - 2.0 *
    Random(0.0, 1.0));
        }
        //  WriteLog(FloatToStr(adhesive));      // tutaj jest na poziomie 0.2 -
    0.3 return adhesion;
        ...
        if( true == SlippingWheels ) {
            ...
            if( true == SandDose ) { adhesion = std::max( staticfriction * (
    100.0 + Vel ) / ( 50.0 + Vel ) * 1.1, 0.48 ); } else                   {
    adhesion = staticfriction * ( 100.0 + Vel ) / ( 50.0 + Vel ); }
        }
    //    adhesion *= ( 0.9 + 0.2 * Random() );
    */
    // wersja3 by youBy - uwzględnia naturalne mikropoślizgi i wpływ
    // piasecznicy, usuwa losowość z pojazdu
    double Vwheels =
        nrot * M_PI *
        WheelDiameter;  // predkosc liniowa koła wynikająca z obrotowej
    double deltaV =
        V -
        Vwheels;  // poślizg - różnica prędkości w punkcie styku koła i szyny
    deltaV = std::max(
        0.0,
        std::abs(deltaV) -
            0.25);  // mikropoślizgi do ok. 0,25 m/s nie zrywają przyczepności
    Vwheels = std::abs(Vwheels);
    adhesion = staticfriction * (28 + Vwheels) / (14 + Vwheels) *
               ((SandDose ? sandfactor : 1) -
                (1 - adh_factor) * (deltaV / (deltaV + slipfactor)));
...
double TMoverParameters::CouplerForce(int const End, double dt) {
    ...
    auto const othervehiclemove{
        (othervehicle->dMoveLen * DirPatch(End, otherend))};
    auto const initialdistance{
        Neighbours[End].distance};  // odległość od sprzęgu sąsiada
    auto const distancedelta{
        (End == end::front ? othervehiclemove - dMoveLen
    ...
    if ((newdistance < 0.0) && (coupler.Dist > newdistance) && (dV < -0.1) &&
        (false == coupler.has_adapter())) {  // HACK: with adapter present we
                                             // presume buffers won't clash
        // 090503: dzwieki pracy zderzakow
        SetFlag(
            coupler.sounds, (absdV > 5.0 ? (sound::bufferclash | sound::loud)
                                         : sound::bufferclash));
    } else if (
        (coupler.CouplingFlag != coupling::faux) && (newdistance > 0.001) &&
        (coupler.Dist <= 0.001) && (absdV > 0.005) && (Vel > 1.0)) {
        // 090503: dzwieki pracy sprzegu
        SetFlag(
            coupler.sounds,
            (absdV > 0.035 ? (sound::couplerstretch | sound::loud)
    ...
    if ((coupler.CouplingFlag != coupling::faux) || (initialdistance < 0)) {
        coupler.Dist = clamp(
            newdistance, (coupler.has_adapter() ? 0 : -coupler.DmaxB),
            coupler.DmaxC);
        ...
        if (coupler.CouplingFlag == coupling::faux) {
            ...
            Fmax = 0.5 *
                   (coupler.FmaxC + coupler.FmaxB + othercoupler.FmaxC +
                    othercoupler.FmaxB) *
                   CouplerTune;
        }
        auto const distDelta{
            std::abs(newdistance) -
            std::abs(coupler.Dist)};  // McZapkie-191103: poprawka na histereze

        if (newdistance > 0) {
            if (distDelta > 0) {
                CF = (-(coupler.SpringKC + othercoupler.SpringKC) *
                      coupler.Dist / 2.0) *
                         DirF(End) -
                     Fmax * dV * BetaAvg;
            } else {
            ...
            if (std::abs(CF) > coupler.FmaxC) {
                ...
                // give coupler 1 sec of leeway to account for simulation
                // glitches, before checking whether it breaks (arbitrary)
                // chance to break grows from 10-100% over 10 sec period
        ...
        if (newdistance < 0) {
            if (distDelta > 0) {
                CF = (-(coupler.SpringKB + othercoupler.SpringKB) *
                      coupler.Dist / 2.0) *
                         DirF(End) -
                     Fmax * dV * BetaAvg;
            } else {
            ...
            auto const collisiondistance{
                ((coupler.has_adapter() || othercoupler.has_adapter())
                     ...
                     :  // HACK: only take into account buffering ability of
                        // automatic coupler
            ...
            if (-newdistance >= std::min(collisiondistance, dEpsilon)) {
                if ((coupler.type() == TCouplerType::Automatic) &&
                    ...
                    if (Attach(End, otherend, othervehicle, couplingtype)) {
                        // HACK: we're reusing sound enum to mark whether
                        // vehicle was connected to another
                    ...
                    /*
                       ...
                       coupler.AutomaticCouplingFlag &
                       othercoupler.AutomaticCouplingFlag ); SetFlag(
                       coupler.sounds, sound::attachcoupler );
                    */
                }
            }
        }
...
double TMoverParameters::TractionForce(double dt) {
    ...
    switch (EngineType) {
        case TEngineType::DieselElectric: {
            if ((true == Mains) && (true == FuelPump.is_active)) {
                if (EIMCtrlType > 0)  // sterowanie cyfrowe
                    tmp = (DElist[0].RPM +
                           ((DElist[MainCtrlPosNo].RPM - DElist[0].RPM) *
                            std::max(0.0, eimic_real))) /
                          60.0;
                else
                    tmp = DElist[(ControlPressureSwitch ? MainCtrlNoPowerPos()
                                                        : MainCtrlPos)]
                              .RPM /
                          60.0;

                if ((true == HeatingAllow) && (HeatingPower > 0) &&
                    (EngineHeatingRPM > 0)) {
                    // bump engine revolutions up if needed, when heating is on
                    tmp = std::max(
                        tmp, std::min(EngineMaxRPM(), EngineHeatingRPM) / 60.0);
                }
                // NOTE: fake dizel_fill calculation for the sake of smoke
                // emitter which uses this parameter to determine smoke opacity
                dizel_fill = clamp(
                    0.2 + 0.35 * (tmp - enrot) +
                        0.5 * (std::abs(Im) / DElist[MainCtrlPosNo].Imax),
                    0.05, 1.0);
            } else {
                tmp = 0.0;
                dizel_fill = 0.0;
                    ...
                    enrot +
                        (dt / dizel_AIM) *
                            (enrot < tmp
                                 ? 1.0
                                 : -1.0 *
                                       dizel_RevolutionsDecreaseRate),  // NOTE:
                                                                        // revolutions
                                                                        // typically
                                                                        // drop
                                                                        // faster
        ...
        case TEngineType::DieselEngine: {
            if (ShuntMode)  // dodatkowa przekładnia np. dla 2Ls150
                dtrans = AnPos * Transmision.Ratio *
                         MotorParam[ScndCtrlActualPos].mIsat;
            else
                dtrans =
                    Transmision.Ratio * MotorParam[ScndCtrlActualPos].mIsat;

            dmoment = dizel_Momentum(
                dizel_fill, dtrans * nrot * DirActive,
                dt);  // oblicza tez enrot
            break;
        }
        default: {
    ...
    eAngle += enrot * dt;
    if (eAngle > M_PI * 2.0) eAngle = std::fmod(eAngle, M_PI * 2.0);
    /*
        while (eAngle > M_PI * 2.0)
            // eAngle = Pirazy2 - eAngle; <- ABu: a nie czasem tak, jak nizej?
            eAngle -= M_PI * 2.0;
    */
    // hunter-091012: przeniesione z if DirActive<>0 (zeby po zejsciu z kierunku
    // dalej spadala predkosc wentylatorow) wentylatory rozruchowe TBD, TODO:
    // move this to update, it doesn't exactly have much to do with traction
    switch (EngineType) {
        case TEngineType::ElectricSeriesMotor: {
            if (true == Mains) {
                switch (RVentType) {
                    case 1: {  // manual
                        if ((true == RVentForceOn) ||
                            ...
                            RventRot +=
                                (RVentnmax - RventRot) * RVentSpeed * dt;
                        } else {
                    ...
                    case 2: {  // automatic
                        auto const motorcurrent{
                            std::min<double>(ImaxHi, std::abs(Im))};
                        if ((std::abs(Itot) > RVentMinI) &&
                            (RList[MainCtrlActualPos].R > RVentCutOff)) {
                            RventRot +=
                                (RVentnmax *
                                     std::min(
                                         1.0, ((motorcurrent / NPoweredAxles) /
                                               RVentMinI)) *
                                     motorcurrent / ImaxLo -
                                 RventRot) *
                                RVentSpeed * dt;
                            ...
                            RventRot +=
                                (RVentnmax * motorcurrent / ImaxLo - RventRot) *
                                RVentSpeed * dt;
        ...
        case TEngineType::DieselElectric: {
            // NOTE: for this type RventRot is the speed of motor blowers; we
            // also update radiator fans while at it
            if (true == Mains) {
                // TBD, TODO: currently ignores RVentType, fix this?
                RventRot += clamp(enrot - RventRot, -100.0, 50.0) * dt;
                dizel_heat.rpmw +=
                    clamp(dizel_heat.rpmwz - dizel_heat.rpmw, -100.f, 50.f) *
                    dt;
                dizel_heat.rpmw2 +=
                    clamp(dizel_heat.rpmwz2 - dizel_heat.rpmw2, -100.f, 50.f) *
                    dt;
            } else {
                RventRot *= std::max(0.0, 1.0 - RVentSpeed * dt);
        ...
        case TEngineType::DieselEngine: {
            // NOTE: we update only radiator fans, as vehicles with diesel
            // engine don't have other ventilators
            if (true == Mains) {
                dizel_heat.rpmw +=
                    clamp(dizel_heat.rpmwz - dizel_heat.rpmw, -100.f, 50.f) *
                    dt;
    ...
    switch (EngineType) {
        case TEngineType::Dumb: {
            PosRatio = (MainCtrlPos + ScndCtrlPos) /
                       (MainCtrlPosNo + ScndCtrlPosNo + 0.01);
        ...
        case TEngineType::DieselEngine: {
            ...
            if (MainCtrlPowerPos() > 1) {
                ...
                //                dmoment -= dizel_Mstand * ( 0.2 * enrot /
                //                dizel_nmax ); //yB: skąd to w ogóle się
                //                bierze?!
            }
            break;
        }
        case TEngineType::DieselElectric: {
            EnginePower = 0;  // the actual calculation is done in two steps
                              // later in the method
            break;
        }
        default: {
    ...
    switch (EngineType) {
        ...
        case TEngineType::ElectricInductionMotor: {
            // TODO: check if we can use instead the code for
            // electricseriesmotor
            if ((Mains)) {
        ...
        case TEngineType::DieselElectric: {
            // TODO: move this to the auto relay check when the electric engine
            // code paths are unified
            StLinFlag |=
                ((Mains) && (false == StLinFlag) && (MainCtrlPowerPos() == 1));
    ...
    if (DirActive != 0) switch (EngineType) {
            case TEngineType::Dumb: {
                if (Mains && (CabActive != 0)) {
                    if (Vel > 0.1) {
                        Ft = std::min(1000.0 * Power / std::abs(V), Ftmax) *
                             PosRatio;
                    } else {
                        Ft = Ftmax * PosRatio;
                    }
            ...
            case TEngineType::WheelsDriven: {
                ...
                if (PulseForceTimer > CtrlDelay) {
                    PulseForce = 0;
                    if (PulseForceCount > 0) PulseForceCount--;
                }
            ...
            case TEngineType::ElectricSeriesMotor: {
                ...
                Mm = Momentum(Current(
                    enrot, EngineVoltage));  // oblicza tez prad p/slinik

                if (TrainType == dt_ET42) {
                    if (Imax == ImaxHi) EngineVoltage = EngineVoltage * 2;
                    if ((DynamicBrakeFlag) &&
                        (abs(Im) > 300))  // przeiesione do mover.cpp
                        FuseOff();
                }
                if ((DynamicBrakeType == dbrake_automatic) &&
                    (DynamicBrakeFlag)) {
                    if (TUHEX_Stages > 0)  // hamowanie wielostopniowe,
                                           // nadpisuje wartości domyślne
                    {
                        if (Vel > 100)
                    ...
                    if (((Vadd + abs(Im)) > TUHEX_Sum + TUHEX_Diff) ||
                        ...
                        (DynamicBrakeFlag) &&
                        ((Vadd + abs(Im)) < TUHEX_Sum - TUHEX_Diff)) {
                        Vadd += 70.0 * dt;
                ...
                if ((TrainType == dt_ET22) &&
                    (DelayCtrlFlag))  // szarpanie przy zmianie układu w byku
                    Mm = Mm * RList[MainCtrlActualPos].Bn /
                         (RList[MainCtrlActualPos].Bn +
                          1);  // zrobione w momencie, żeby nie dawac elektryki
                               // w przeliczaniu sił
                ...
                if (abs(Im) > Imax)
                    ...
                    Vhyp = 0;
                if (Vhyp >
                    CtrlDelay / 2)  // jesli czas oddzialywania przekroczony
                    FuseOff();  // wywalanie bezpiecznika z powodu przetezenia
                                // silnikow
                ...
                if (((DynamicBrakeType == dbrake_automatic) ||
                    ...
                    (TrainType == dt_EZT) && (Imin == IminLo) &&
                    (ScndS))  // yBARC - boczniki na szeregu poprawnie
                    Itot = Im;
                else
                    Itot = Im * RList[MainCtrlActualPos]
                                    .Bn;  // prad silnika * ilosc galezi
            ...
            case TEngineType::DieselEngine: {
                Mw = dmoment * dtrans *
                     Transmision.Efficiency;  // dmoment i dtrans policzone przy
                                              // okazji enginerotation
                if ((hydro_R) && (hydro_R_Placement == 0))
                    Mw -= dizel_MomentumRetarder(nrot * Transmision.Ratio, dt) *
                          Transmision.Ratio * Transmision.Efficiency;
            ...
            case TEngineType::DieselElectric:  // youBy
            {
                ...
                auto const tmpV{
                    nrot * Pirazy2 * 0.5 * WheelDiameter *
                    DirAbsolute};  //*CabActive*DirActive;
                auto tempUmax = 0.0;
                auto tempImax = 0.0;
                auto tempPmax = 0.0;
                // jazda manewrowa
                if (EIMCtrlType > 0)  // sterowanie cyfrowe
                {
                    ...
                    tempUmax = DElist[MainCtrlPosNo].Umax *
                               std::min(eimic_positive, rpmratio);
                    tempPmax = DElist[MainCtrlPosNo].GenPower *
                               std::min(eimic_positive, rpmratio);
                    tmp = tempPmax;
                    if (true == StLinFlag) {
                        if (tmpV < (Vhyp * tempPmax /
                                    DElist[MainCtrlPosNo].GenPower)) {
                            ...
                            Ft = (Ftmax -
                                 ...
                                 eimic_positive;  // posratio - bo sila jakos
                                                  // tam sie rozklada
                        } else {
                            // na hiperboli
                            // 1.107 - wspolczynnik sredniej nadwyzki Ft w symku
                            // nad charakterystyka
                            Ft =
                                1000.0 * tempPmax / (tmpV + Vadd) /
                    ...
                    if (true == StLinFlag) {
                        ...
                        // NOTE: very crude way to approximate power generated
                        // at current rpm instead of instant top output NOTE,
                        // TODO: doesn't take into account potentially increased
                        // revolutions if heating is on, fix it
                        tmp =
                            EngineRPMRatio() * (SST[MainCtrlPos].Pmax * AnPos) +
                ...
                } else  // jazda ciapongowa
                {
                    ...
                    // NOTE: very crude way to approximate power generated at
                    // current rpm instead of instant top output NOTE, TODO:
                    // doesn't take into account potentially increased
                    // revolutions if heating is on, fix it
                    ...
                    if (true == StLinFlag) {
                        if (tmpV <
                            (Vhyp * power / DElist[MainCtrlPosNo].GenPower)) {
                            // czy na czesci prostej, czy na hiperboli
                            Ft = (Ftmax -
                                  ((Ftmax - 1000.0 *
                                                DElist[MainCtrlPosNo].GenPower /
                                                (Vhyp + Vadd)) *
                                   (tmpV / Vhyp) / PowerCorRatio)) *
                                 PosRatio;  // posratio - bo sila jakos tam sie
                                            // rozklada
                        } else {
                            // na hiperboli
                            // 1.107 - wspolczynnik sredniej nadwyzki Ft w symku
                ...
                if (FuseFlag)
                    ...
                    Ft = Ft * DirAbsolute;  // DirActive * CabActive; //zwrot
                                            // sily i jej wartosc
                ...
                if (abs(Mm) > MotorParam[ScndCtrlPos].fi)
                    Im = NPoweredAxles *
                         abs(abs(Mm) / MotorParam[ScndCtrlPos].mfi +
                             MotorParam[ScndCtrlPos].mIsat);
                else
                    Im = NPoweredAxles *
                         sqrt(abs(Mm * MotorParam[ScndCtrlPos].Isat));
                ...
                if (ShuntMode) {
                    ...
                    if (abs(Im) > tempImax) {  // nie ma nadmiarowego, tylko
                                               // Imax i zwarcie na pradnicy
                        Ft = Ft / Im * tempImax;
                        Im = tempImax;
                    }
                    ...
                    if (Im > 0) {
                        // jak pod obciazeniem
                        if (true == Flat) {
                            // ograniczenie napiecia w pradnicy - plaszczak u
                            // gory
                            EngineVoltage = 1000.0 * tmp / std::abs(Im);
                        } else {
                            auto tempMCP =
                                EIMCtrlType > 0
                                    ? 1 + 99 * std::max(1.0, eimic_real)
                                    : MainCtrlPos;
                            auto tempMCPN =
                                EIMCtrlType > 0 ? 100 : MainCtrlPosNo;
                            // charakterystyka pradnicy obcowzbudnej (elipsa) -
                            ...
                            EngineVoltage =
                                std::sqrt(std::abs(
                                    ...
                                    (tempMCP - 1) +
                                (1.0 - Im / tempImax) * tempUmax *
                                    (tempMCPN - tempMCP);
                            EngineVoltage /= (tempMCPN - 1);
                            EngineVoltage = clamp(
                                EngineVoltage, Im * 0.05,
                                (1000.0 * tmp / std::abs(Im)));
                        }
                    }
                    ...
                    /*
                                    // power curve drop
                                    // NOTE: disabled for the time being due to
                       side-effects if( ( tmpV > 1 ) && ( EnginePower < tmp ) )
                       { Ft = interpolate( Ft, EnginePower / tmp, clamp( tmpV
                       - 1.0, 0.0, 1.0 ) );
                                    }
                    */
                }

                if ((Imax > 1) && (Im > Imax)) FuseOff();
                if (FuseFlag) EngineVoltage = 0;
                ...
                if ((false == StLinFlag) || (ShuntMode)) {
                    ...
                    if (AutoRelayFlag) {
                        ...
                        switch (RelayType) {
                            case 0: {
                                if ((ScndCtrlPos < ScndCtrlPosNo) &&
                                    (Im <=
                                     (MPTRelay[ScndCtrlPos].Iup * PosRatio))) {
                                    ++ScndCtrlPos;
                                }
                                if ((ScndCtrlPos > 0) &&
                            ...
                            case 45: {
                                if ((ScndCtrlPos < ScndCtrlPosNo) &&
                                    ...
                                    // check for cases where the speed drops
                                    // below threshold for level 2 or 3
                                    if ((ScndCtrlPos > 1) &&
                                        (Vel <
                                ...
                                // 3rd level drops with master controller at
                                // position lower than 10...
                                if (MainCtrlPos < 11) {
                                    ScndCtrlPos = std::min(2, ScndCtrlPos);
                                }
                                // ...and below position 7 field shunt drops
                                // altogether
                                if (MainCtrlPos < 8) {
                                    ScndCtrlPos = 0;
                                }
                                /*
                                                            // crude woodward
                                   approximation; difference between rpm for
                                   consecutive positions is ~5%
                                                            // so we get full
                                   throttle until ~half way between desired and
                                   previous position, or zero on rpm reduction
                                                            auto const woodward
                                   { clamp( ( DElist[ MainCtrlPos ].RPM / (
                                   enrot * 60.0 ) - 1.0 ) * 50.0, 0.0, 1.0 ) };
                                */
                                break;
                            }
                            case 46: {
                        ...
                        if (ScndCtrlPos != shuntfieldstate) {
                            SetFlag(
                                SoundFlag, (sound::relay | sound::shuntfield));
                        }
                    }
                }
                break;
            ...
            case TEngineType::ElectricInductionMotor: {
                if (true == Mains && !SecuritySystem.is_engine_blocked()) {
                    ...
                    for (auto &inv : Inverters) {
                        if (inv.IsActive) ActiveInverters += 1.0;
                    }
                    ...
                    if (ScndCtrlPosNo == 4 && SpeedCtrlTypeTime) {
                        ...
                        if (ScndCtrlPos != NewSpeed) {
                            ...
                            SpeedCtrlTimer += dt;
                            if (SpeedCtrlTimer > SpeedCtrlDelay) {
                                int NewSCAP = (float)ScndCtrlPos /
                                              (float)ScndCtrlPosNo * Vmax;
                    ...
                    auto const localbrakeactive{
                        (CabOccupied != 0) && (LocHandle->GetCP() > 0.25)};
                    if ((false == Doors.instances[side::left].is_closed) ||
                        (false == Doors.instances[side::right].is_closed) ||
                        (Doors.permit_needed &&
                         (Doors.instances[side::left].open_permit ||
                          Doors.instances[side::right].open_permit))) {
                        DynamicBrakeFlag = true;
                    } else if (
                        ((edBCP < 0.25) && (false == localbrakeactive) &&
                         (AnPos < 0.01)) ||
                        ((edBCP < 0.25) && (ShuntModeAllow) &&
                         (LocalBrakePosA < 0.01)))
                        DynamicBrakeFlag = false;
                    else if (
                        (((BrakePress > 0.25) && (edBCP > 0.25) ||
                          localbrakeactive)) ||
                        (AnPos > 0.02))
                        DynamicBrakeFlag = true;
                    edBCP = Hamulec->GetEDBCP() *
                            eimc[eimc_p_abed];  // stala napedu
                    if ((DynamicBrakeFlag)) {
                        // ustalanie współczynnika blendingu do luzowania
                        // hamulca PN
                        if (eimv[eimv_Fmax] * Sign(V) * DirAbsolute < -1) {
                            PosRatio =
                                -Sign(V) * DirAbsolute * eimv[eimv_Fr] /
                                (eimc[eimc_p_Fh] *
                                 Max0R(
                                     edBCP, Max0R(0.01, Hamulec->GetEDBCP())) /
                                 MaxBrakePress[0]);
                            PosRatio = clamp(PosRatio, 0.0, 1.0);
                        } else {
                            PosRatio = 0;
                        }
                        PosRatio = Round(20.0 * PosRatio) /
                                   20.0;  // stopniowanie PN/ED
                        if (PosRatio < 19.5 / 20.0) PosRatio *= 0.9;
                        Hamulec->SetED(Max0R(
                            0.0,
                            std::min(
                                PosRatio, 1.0)));  // ustalenie stopnia
                                                   // zmniejszenia ciśnienia
                        // ustalanie siły hamowania ED
                        if ((Hamulec->GetEDBCP() > 0.25) &&
                                (eimc[eimc_p_abed] < 0.001) ||
                            (ActiveInverters <
                             InvertersNo))  // jeśli PN wyłącza ED
                        {
                            PosRatio = 0;
                            eimv[eimv_Fzad] = 0;
                        } else {
                            PosRatio =
                                -std::max(
                                    std::min(
                                        edBCP * 1.0 / MaxBrakePress[0], 1.0),
                                    AnPos) *
                                std::max(
                                    0.0, std::min(
                                             1.0, (Vel - eimc[eimc_p_Vh0]) /
                                                      (eimc[eimc_p_Vh1] -
                                                       eimc[eimc_p_Vh0])));
                            eimv[eimv_Fzad] = -std::min(
                                1.0, std::max(
                                         LocalBrakeRatio(),
                                         edBCP / MaxBrakePress[0]));
                        }
                        tmp = 5;
                    } else {
                        ...
                        if ((Flat) && (eimc[eimc_p_F0] * eimv[eimv_Fful] > 0))
                            PosRatio = Min0R(
                                PosRatio * eimc[eimc_p_F0] / eimv[eimv_Fful],
                                1);
                        /*                    if (ScndCtrlActualPos > 0) //speed
                           control if (Vmax < 250) PosRatio = Min0R(PosRatio,
                           Max0R(-1, 0.5 * (ScndCtrlActualPos - Vel))); else
                                                    PosRatio =
                                                        Min0R(PosRatio,
                           Max0R(-1, 0.5 * (ScndCtrlActualPos * 2 - Vel))); */
                        // PosRatio = 1.0 * (PosRatio * 0 + 1) * PosRatio; // 1
                        // * 1 * PosRatio = PosRatio
                        Hamulec->SetED(0);
                        //           (Hamulec as TLSt).SetLBP(LocBrakePress);
                        if ((PosRatio > eimv_pr))
                    ...
                    // NOTE: the commands to operate the sandbox are likely to
                    // conflict with other similar ai decisions
                    // TODO: gather these in single place so they can be
                    // resolved together
                    if ((SlippingWheels)) {
                        PosRatio = 0;
                        tmp = 10;
                    ...
                    eimv_pr += Max0R(Min0R(PosRatio - eimv_pr, 0.02), -0.02) *
                               12 * (tmp /*2{+4*byte(PosRatio<eimv_pr)*/) *
                    ...
                    auto f_cfu{
                        DynamicBrakeFlag ? eimc[eimc_f_cfuH]
                                         : eimc[eimc_f_cfu]};

                    eimv[eimv_Uzsmax] =
                        Min0R(EngineVoltage - eimc[eimc_f_DU], tmp);
                    eimv[eimv_fkr] = eimv[eimv_Uzsmax] / f_cfu;
                    if ((eimv_pr < 0)) {
                        eimv[eimv_Pmax] = eimc[eimc_p_Ph];
                    } else {
                        eimv[eimv_Pmax] = std::min(
                            eimc[eimc_p_Pmax],
                            0.001 * EngineVoltage *
                                (eimc[eimc_p_Imax] - eimc[eimc_f_I0]) *
                                Pirazy2 * eimc[eimc_s_cim] / eimc[eimc_s_p] /
                                eimc[eimc_s_cfu]);
                    }
                    eimv[eimv_FMAXMAX] =
                        0.001 *
                        ...
                        eimv[eimv_Fmax] =
                            -Sign(V) * (DirAbsolute)*std::min(
                                           eimc[eimc_p_Ph] * 3.6 /
                                               (Vel != 0.0 ? Vel : 0.001),
                                           std::min(
                                               -eimc[eimc_p_Fh] * eimv_pr,
                                               eimv[eimv_FMAXMAX]));
                        double pr = eimv_pr;
                        if (EIMCLogForce) pr = -log(1 - 4 * pr) / log(5);
                        eimv[eimv_Fr] =
                            -Sign(V) *
                            (DirAbsolute)*std::min(
                                eimc[eimc_p_Ph] * 3.6 /
                                    (Vel != 0.0 ? Vel : 0.001),
                                std::min(
                                    -eimc[eimc_p_Fh] * pr, eimv[eimv_FMAXMAX]));
                        if (InvertersRatio < 1.0) eimv[eimv_Fful] = 0;
                        //*Min0R(1,(Vel-eimc[eimc_p_Vh0])/(eimc[eimc_p_Vh1]-eimc[eimc_p_Vh0]))
                    } else {
                        eimv[eimv_Fful] = Min0R(
                            Min0R(
                                3.6 * eimv[eimv_Pmax] / Max0R(Vel, 1),
                    ...
                    for (auto &inv : Inverters) {
                        ...
                        inv.Error = inv.Failure_Const ||
                                    (inv.Failure_Drive && inv.Request != 0);
                    ...
                    if ((abs(eimv[eimv_fp]) <= eimv[eimv_fkr]))
                        ...
                        eimv[eimv_pole] = eimv[eimv_Uzsmax] / eimc[eimc_s_cfu] /
                                          abs(eimv[eimv_fp]);
                    eimv[eimv_U] =
                        eimv[eimv_pole] * eimv[eimv_fp] * eimc[eimc_s_cfu];
                    eimv[eimv_Ic] =
                        (eimv[eimv_fp] - DirAbsolute * enrot * eimc[eimc_s_p]) *
                        eimc[eimc_s_dfic] * eimv[eimv_pole];
                    eimv[eimv_If] = eimv[eimv_Ic] * eimc[eimc_s_icif];
                    eimv[eimv_M] =
                        eimv[eimv_pole] * eimv[eimv_Ic] * eimc[eimc_s_cim];
                    eimv[eimv_Ipoj] = (eimv[eimv_Ic] * NPoweredAxles *
                                       InvertersRatio * eimv[eimv_U]) /
                                          (EngineVoltage - eimc[eimc_f_DU]) +
                                      eimc[eimc_f_I0];
                    eimv[eimv_Pm] = DirActive * eimv[eimv_M] * NPoweredAxles *
                                    InvertersRatio * enrot * Pirazy2 / 1000;
                    ...
                    if ((eimv[eimv_Ipoj] >= 0))
                        ...
                        Vadd = std::max(
                            Vadd * (1.0 - 0.2 * dt),
                            0.007 *
                                (std::abs(EngineVoltage) -
                                 (EnginePowerSource.CollectorParameters.MaxV -
                                  100)));
                    Itot =
                        eimv[eimv_Ipoj] * (0.01 + std::min(0.99, 0.99 - Vadd));

                    EnginePower =
                        abs(eimv[eimv_Ic] * eimv[eimv_U] * NPoweredAxles) /
                        1000;
                    // power inverters
                    auto const tmpV{std::abs(eimv[eimv_fp])};

                    if ((RlistSize > 0) &&
                        ((std::abs(eimv[eimv_If]) > 1.0) && (tmpV > 0.0001))) {
                        int i = 0;
                        while ((i < RlistSize - 1) &&
                               (DElist[i + 1].RPM < tmpV)) {
                            ++i;
                        ...
                        InverterFrequency =
                            (tmpV - DElist[i].RPM) /
                                std::max(
                                    1.0, (DElist[i + 1].RPM - DElist[i].RPM)) *
                    ...
                    EnginePower = 0.0;
                    {
                        for (int i = 0; i < 21; ++i) eimv[i] = 0.0;
                    }
                    Hamulec->SetED(0.0);
                    InverterFrequency =
                        0.0;  //(Hamulec as TLSt).SetLBP(LocBrakePress);
                }
                break;
    ...
    switch (EngineType) {
        case TEngineType::DieselElectric: {
            ...
            EnginePower +=
                EngineRPMRatio() * 0.15 * DElist[MainCtrlPosNo].GenPower;
            break;
        }
        default: {
            break;
...
// *************************************************************************************************
double TMoverParameters::ComputeRotatingWheel(
    double WForce, double dt, double n) const {
    double newn = 0, eps = 0;
...
bool TMoverParameters::UniversalResetButton(
    int const Button, range_t const Notify) {
    auto const lowvoltagepower{Power24vIsAvailable || Power110vIsAvailable};
...
bool TMoverParameters::RelayReset(int const Relays, range_t const Notify) {
    ...
    if (TestFlag(Relays, relay_t::maincircuitground)) {
        if (((EngineType == TEngineType::ElectricSeriesMotor) ||
            ...
            (IsMainCtrlNoPowerPos()) && (ScndCtrlPos == 0) &&
            (DirActive != 0) && (!TestFlag(EngDmgFlag, 1))) {
    ...
    if (TestFlag(Relays, relay_t::tractionnmotoroverload)) {
        if (((EngineType == TEngineType::ElectricSeriesMotor) ||
            ...
            // TODO: check whether the power is required, TBD, TODO: make it
            // configurable?
...
double TMoverParameters::v2n(void) {
    ...
    n = V / (M_PI *
             WheelDiameter);  // predkosc obrotowa wynikajaca z liniowej [obr/s]
    ...
    {                    // McZapkie-221103: uszkodzenia kol podczas poslizgu
        ...
        if (deltan < -dmgn)
            if (FuzzyLogic(-deltan, dmgn, p_slippdmg))
                if (SetFlag(
                        DamageFlag,
                        dtrain_thinwheel))  // wycieranie sie obreczy
                    EventFlag = true;
...
double TMoverParameters::MomentumF(double I, double Iw, int SCP) {
    ...
    return (
        MotorParam[SCP].mfi * I *
        Max0R(
            abs(Iw) / (abs(Iw) + MotorParam[SCP].mIsat) - MotorParam[SCP].mfi0,
            0));
}
...
bool TMoverParameters::CutOffEngine(void) {
    ...
    if ((NPoweredAxles > 0) && (CabActive == 0) &&
        (EngineType == TEngineType::ElectricSeriesMotor)) {
...
bool TMoverParameters::MaxCurrentSwitch(bool State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            "MaxCurrentSwitch", (State ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::AutoRelayCheck(void) {
    ...
    //                         chodzi w tym wszystkim o to, żeby można było
    //                         zatrzymać rozruch na jakiejś pozycji wpisując
    //                         Autoswitch=0 i wymuszać przejście dalej przez
    //                         danie nastawnika na dalszą pozycję - tak to do
    //                         tej pory działało i na tym się opiera fizyka
    //                         ET22-2k
    {
        if (StLinFlag) {
            if ((RList[MainCtrlActualPos].R == 0) &&
                ...
                (!(CoupledCtrl) ||
                 (RList[MainCtrlActualPos].Relay ==
                  MainCtrlPos))) {  // zmieniaj scndctrlactualpos
                ...
                if ((DirActive < 0) && (TrainType != dt_PseudoDiesel)) {
                    if (RList[MainCtrlActualPos + 1].Bn >
                        BackwardsBranchesAllowed) {
                        return false;  // nie poprawiamy przy konwersji
                        // return ARC;// bbylo exit; //Ra: to powoduje, że EN57
                        // nie wyłącza się przy IminLo
                    }
                }
                // main bez samoczynnego rozruchu
                if ((MainCtrlActualPos < (sizeof(RList) / sizeof(TScheme) -
                                          1))  // crude guard against running
                                               // out of current fixed table
                    && ((RList[MainCtrlActualPos].Relay < MainCtrlPos) ||
                        ((RList[MainCtrlActualPos + 1].Relay == MainCtrlPos) &&
                         (MainCtrlActualPos < RlistSize)) ||
                        ((TrainType == dt_ET22) && (DelayCtrlFlag)))) {
                    // prevent switch to parallel mode if motor overload relay
                    // is set to high threshold mode
                    ...
                    if ((RList[MainCtrlPos].R == 0) && (MainCtrlPos > 0) &&
                        ...
                        if (MainCtrlPos - MainCtrlActualPos == 1) {
                            // HACK: ensure we play only single sound of basic
                            // relays for entire trasition; return false for all
                            // but last step despite configuration change, to
                            // prevent playback of the basic relay sound TBD,
                            // TODO: move the basic sound event here and enable
                            // it with call parameter
                            OK = true;
                        }
                        if (RList[MainCtrlActualPos].R == 0) {
                        ...
                        if ((TrainType == dt_ET22) && (MainCtrlPos > 1) &&
                            ...
                            if (!DelayCtrlFlag)  // najpierw przejscie
                            {
                                ...
                                LastRelayTime > 4 * CtrlDelay)  // przejscie
                            {
                                DelayCtrlFlag = false;
                                OK = true;
                            }
                        ...
                        if (MainCtrlActualPos > 0) {
                            if ((RList[MainCtrlActualPos].R == 0) &&
                                ...
                                SetFlag(
                                    SoundFlag, sound::parallel | sound::loud);
                            } else if (
                                (RList[MainCtrlActualPos].R > 0) &&
                    ...
                    if ((RList[MainCtrlPos].R == 0) && (MainCtrlPos > 0) &&
                        ...
                        if (TrainType !=
                            dt_EZT)  // tutaj powinien być tryb sterowania wałem
                        {
                            --MainCtrlActualPos;
                            OK = true;
                        }
                    ...
                    (ScndCtrlActualPos > 0)) {
                    if (LastRelayTime > CtrlDownDelay) {
                        --ScndCtrlActualPos;  // boczniki nie dzialaja na poz.
                                              // oporowych
                        SetFlag(SoundFlag, sound::shuntfield);
                        OK = true;
                    }
        ...
        } else  // not StLinFlag
        {
            ...
            if ((false == StLinFlag) &&
                ((MainCtrlActualPos > 0) || (ScndCtrlActualPos > 0))) {
                if (CoupledCtrl) {
                    if (TrainType == dt_EZT) {
...
bool TMoverParameters::MotorConnectorsCheck() {
    ...
    auto const connectorsoff{
        (false == Mains) || (true == FuseFlag) || (true == StLinSwitchOff) ||
        (DirActive == 0)};

    return (false == connectorsoff);
}

bool TMoverParameters::OperatePantographsValve(
    operation_t const State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            "PantsValve", static_cast<double>(State), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::OperatePantographValve(
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            "PantValve",
            // HACK: pack the state, pantograph index and sender cab into 8-bit
            // value with high bit storing front/rear pantograph, and 7th bit
            // storing sender cab
            static_cast<double>(
...
bool TMoverParameters::DropAllPantographs(
    ...
    if (Notify != range_t::local) {
        SendCtrlToNext(
            "PantAllDown", (State ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
void TMoverParameters::CheckEIMIC(double dt) {
    ...
    switch (EIMCtrlType) {
        case 0:
            eimic =
                (LocalBrakeRatio() > 0.01 ? -LocalBrakeRatio()
                 : eimic_analog > 0.01
                     ? eimic_analog
            ...
            if (EIMCtrlAdditionalZeros || EIMCtrlEmergency) {
                if (eimic > 0.001)
                    eimic = std::max(
                        0.002, eimic * (double)MainCtrlPosNo /
                                       ((double)MainCtrlPosNo - offset) -
                                   offset / ((double)MainCtrlPosNo - offset));
                if ((eimic < -0.001) && (BrakeHandle != TBrakeHandle::MHZ_EN57))
                    eimic = std::min(
                        -0.002,
                        eimic * (double)LocalBrakePosNo /
        ...
        case 1:
            switch (MainCtrlPos) {
                case 0:                                          // B+
                    eimic -= clamp(1.0 + eimic, 0.0, dt * 0.3);  // odejmuj do
                                                                 // -1
                    break;
                case 1:                                          // B
                ...
                case 7:  // TMax
                    eimic +=
                        clamp(1.0 - eimic, 0.0, dt * 0.3);  // dodawaj do 1, max
                    break;
            }
        ...
        case 2:
            if ((MainCtrlActualPos != MainCtrlPos) ||
                ...
                double delta =
                    (MainCtrlActualPos == MainCtrlPos ? dt * CtrlDelay : 0.01);
                switch (MainCtrlPos) {
                    case 0:
                    case 1:
                        eimic -=
                            clamp(1.0 + eimic, 0.0, delta);  // odejmuj do -1
                        if (eimic > 0) eimic = 0;
                        break;
                    case 2:
                        eimic -= clamp(0.0 + eimic, 0.0, delta);  // odejmuj do
                                                                  // 0
                        break;
                    case 3:
                        eimic += clamp(0.0 - eimic, 0.0, delta);  // dodawaj do
                                                                  // 0
                        break;
                    case 4:
                        eimic += clamp(1.0 - eimic, 0.0, delta);  // dodawaj do
                                                                  // 1
                        if (eimic < 0) eimic = 0;
                        break;
                }
        ...
        case 3:
            if (UniCtrlIntegratedBrakePNCtrl) {
                if ((UniCtrlList[MainCtrlPos].mode != BrakeCtrlPos) &&
                    (MainCtrlActualPos ==
                     MainCtrlPos))  // there was no move of controller, but
                                    // brake only
                {
                    if (BrakeCtrlPos < UniCtrlList[MainCtrlPosNo].mode)
                        BrakeLevelSet(UniCtrlList[MainCtrlPosNo]
                                          .mode);  // bottom clamping
            ...
            if ((MainCtrlActualPos != MainCtrlPos) ||
                (LastRelayTime > InitialCtrlDelay)) {
                eimic -= clamp(
                    -UniCtrlList[MainCtrlPos].SetCtrlVal + eimic, 0.0,
                    (MainCtrlActualPos == MainCtrlPos
                         ? dt * UniCtrlList[MainCtrlPos].SpeedDown
                         : sign(UniCtrlList[MainCtrlPos].SpeedDown) *
                               0.01));  // odejmuj do X
                eimic += clamp(
                    UniCtrlList[MainCtrlPos].SetCtrlVal - eimic, 0.0,
                    (MainCtrlActualPos == MainCtrlPos
                         ? dt * UniCtrlList[MainCtrlPos].SpeedUp
                         : sign(UniCtrlList[MainCtrlPos].SpeedUp) *
                               0.01));  // dodawaj do X
                eimic = clamp(
                    eimic, UniCtrlList[MainCtrlPos].MinCtrlVal,
    ...
    if (LocHandleTimeTraxx) {
        ...
        if (LocalBrakeRatio() > 0.15)  // pozycja 2
        {
            ...
            eim_localbrake =
                std::max(eim_localbrake, BrakePress / MaxBrakePress[0]);
        } else {
...
void TMoverParameters::CheckSpeedCtrl(double dt) {
    ...
    if (EIMCtrlType >= 2) {
        ...
        if (!SpeedCtrlUnit.BrakeIntervention) {
            if ((Hamulec->GetEDBCP() > 0.4) ||
                (PipePress < (HighPipePress - 0.2)))
                SpeedCtrlUnit.Standby = true;
        }
    ...
    if (SpeedCtrlUnit.IsActive) {  // speed control
        if (true) {
            if ((!SpeedCtrlUnit.Standby)) {
                ...
                double error =
                    (std::max(SpeedCtrlValue + SpeedCtrlUnit.Offset, 0.0) -
                     Vel);
                double factorP = error > 0 ? SpeedCtrlUnit.FactorPpos
                                           : SpeedCtrlUnit.FactorPneg;
                double eSCP = clamp(factorP * error, -1.2, 1.0);  // P module
                bool retarder_not_work =
                    (EngineType != TEngineType::DieselEngine) ||
                ...
                SpeedCtrlUnit.BrakeInterventionUnbraking =
                    (eSCP > 0.0) || (Vel == 0.0);
                if (abs(eSCP) < 0.999) {
                    // TODO: check how to disable integral part when braking in
                    // smart way double factorI = eimicSpeedCtrlIntegral >= 0 ?
                    // SpeedCtrlUnit.FactorIpos : SpeedCtrlUnit.FactorIneg;
                    double factorI = eimicSpeedCtrlIntegral >= 0
                                         ? SpeedCtrlUnit.FactorIpos
                                         : SpeedCtrlUnit.FactorIneg;
                    eimicSpeedCtrlIntegral = clamp(
                        eimicSpeedCtrlIntegral + factorI * eSCP * dt,
                        -1.0 + eSCP, 1.0 - eSCP);
                } else {
                    eimicSpeedCtrlIntegral = 0;
                }
                auto const DesiredeimicSpeedCtrl{clamp(
                    eimicSpeedCtrlIntegral + eSCP, -SpeedCtrlUnit.DesiredPower,
                    accfactor)};
                ...
                if (Vel < SpeedCtrlUnit.FullPowerVelocity) {
                    eimicSpeedCtrl =
                        std::min(eimicSpeedCtrl, SpeedCtrlUnit.InitialPower);
                }
                if ((Vel < SpeedCtrlUnit.StartVelocity) &&
                    (MainCtrlPos < MainCtrlPosNo)) {
                    eimicSpeedCtrl = 0;
                    eimic = 0;
                }
            ...
            SpeedCtrlUnit.Parking =
                ...
                (EngineType != TEngineType::ElectricInductionMotor);
            SendCtrlToNext(
                "SpeedCtrlUnit.Parking", SpeedCtrlUnit.Parking, CabActive);
            ...
            if (Vmax < 250)
                ...
                eimicSpeedCtrl =
                    clamp(0.5 * (SpeedCtrlValue * 2 - Vel), -1.0, 1.0);
        }
        if (((SpeedCtrlAutoTurnOffFlag & 2) == 2) &&
            (Hamulec->GetEDBCP() > 0.25)) {
            DecScndCtrl(2);
            SpeedCtrlUnit.IsActive = false;
        }
    } else {
...
void TMoverParameters::SpeedCtrlInc() {
    if ((SpeedCtrl) && (ScndCtrlPos > 0)) {
        ...
        SpeedCtrlValue =
            std::min(x * SpeedCtrlUnit.VelocityStep, SpeedCtrlUnit.MaxVelocity);
    }
}
...
void TMoverParameters::SpeedCtrlDec() {
    if ((SpeedCtrl) && (ScndCtrlPos > 0)) {
        ...
        SpeedCtrlValue =
            std::max(x * SpeedCtrlUnit.VelocityStep, SpeedCtrlUnit.MinVelocity);
    }
}

bool TMoverParameters::SpeedCtrlPowerInc() {
    if (!(SpeedCtrl && ScndCtrlPos > 0)) return false;

    if (SpeedCtrlUnit.DesiredPower == SpeedCtrlUnit.MaxPower) return false;

    SpeedCtrlUnit.DesiredPower = std::min(
        SpeedCtrlUnit.DesiredPower + SpeedCtrlUnit.PowerStep,
...
bool TMoverParameters::SpeedCtrlPowerDec() {
    ...
    if (SpeedCtrlUnit.DesiredPower == SpeedCtrlUnit.MinPower) return false;

    SpeedCtrlUnit.DesiredPower = std::max(
        SpeedCtrlUnit.DesiredPower - SpeedCtrlUnit.PowerStep,
        SpeedCtrlUnit.MinPower);
    return true;
}
...
bool TMoverParameters::dizel_EngageSwitch(double state) {
    if ((EngineType == TEngineType::DieselEngine) && (state <= 1) &&
        (state >= 0) && (state != dizel_engagestate)) {
...
bool TMoverParameters::dizel_EngageChange(double dt) {
    ...
    if (abs(dizel_engage - dizel_engagestate) < 0.11) {
        ...
        dizel_engage = dizel_engage +
                       engagespeed * dt * (dizel_engagestate - dizel_engage);
        // OK:=false;
    }
...
bool TMoverParameters::dizel_AutoGearCheck(void) {
    ...
    auto const VelUp{
        (MotorParam[ScndCtrlActualPos].mfi0 != 0.0
    ...
    auto const VelDown{
        ((MotorParam[ScndCtrlActualPos].fi0 != 0.0) && (eimic_real <= 0.0)
    ...
    if (MotorParam[ScndCtrlActualPos].AutoSwitch && Mains) {
        if ((RList[MainCtrlPos].Mn == 0) && (!hydro_TC)) {
            if (dizel_engagestate > 0) dizel_EngageSwitch(0);
    ...
    if (Mains) {
        if (EIMCtrlType > 0)  // sterowanie komputerowe
        {
            if (dizel_automaticgearstatus == 0) {
                if ((hydro_TC && hydro_TC_Fill > 0.01) || (eimic_real > 0.005))
                    ...
                    Vel > hydro_R_EngageVel && hydro_R && hydro_R_Fill > 0.01)
                    dizel_EngageSwitch(0.5);
                else
                    dizel_EngageSwitch(0.0);
            } else
                dizel_EngageSwitch(0.0);
        } else if (dizel_automaticgearstatus == 0)  // ustaw cisnienie w
                                                    // silowniku sprzegla}
        ...
        if (!(MotorParam[ScndCtrlActualPos].mIsat > 0))
            dizel_EngageSwitch(
                0.0);  // wylacz sprzeglo na pozycjach neutralnych
        if (!AutoRelayFlag) ScndCtrlActualPos = ScndCtrlPos;
    }
    return OK;
}

// performs diesel engine startup procedure; potentially clears startup switch;
// returns: true if the engine can be started, false otherwise
bool TMoverParameters::dizel_StartupCheck() {
    auto engineisready{true};  // make inital optimistic presumption, then watch
                               // the reality crush it

    // test the fuel pump
    // TODO: add fuel pressure check
    if ((false == FuelPump.is_active) ||
        ((EngineType == TEngineType::DieselEngine) &&
         (RList[MainCtrlPos].R == 0.0))) {
        engineisready = false;
        //        if( FuelPump.start_type == start_t::manual ) {
        // with manual pump control startup procedure is done only once per
    ...
    if (true == dizel_heat.PA) {
        engineisready = false;
        // TBD, TODO: reset startup procedure depending on pump and heater
        // control mode
        dizel_startup = false;
    }
...
bool TMoverParameters::dizel_Update(double dt) {
    ...
    if ((true == dizel_ignition) && (LastSwitchingTime >= InitialCtrlDelay)) {
        ...
        // TODO: split engine and main circuit state indicator in two separate
        // flags
        LastSwitchingTime = 0;
        Mains = true;
        dizel_spinup = true;
    ...
    dizel_spinup =
        (dizel_spinup && Mains &&
         (enrot < 0.95 * (EngineType == TEngineType::DieselEngine
                              ? dizel_nmin
                              : DElist[0].RPM / 60.0)));

    if ((true == Mains) && (false == FuelPump.is_active)) {
        // knock out the engine if the fuel pump isn't feeding it
        // TBD, TODO: grace period before the engine is starved for fuel and
        // knocked out
        MainSwitch(false);
    }
    ...
    if (EngineType == TEngineType::DieselEngine) {
        ...
        dizel_fill =
            dizel_fill +
            fillspeed * dt * (dizel_fillcheck(MainCtrlPos, dt) - dizel_fill);
    }
    ...
    if ((true == Mains) && (MainCtrlPosNo > 0) &&
        ...
        if ((true == dizel_ignition) &&
            ...
            if (EIMCtrlType > 0) {
                realfill = std::max(
                    0.0,
                    std::min(
                        eimic_real, 1 - MotorParam[ScndCtrlActualPos].Isat));
                if (eimic_real > 0.005 && !hydro_TC_Lockup) {
                    dizel_nreg_min = std::min(
                        dizel_nreg_min + 2.5 * dt,
                        dizel_nmin_hdrive +
                            eimic_real * dizel_nmin_hdrive_factor);
                } else {
                    if (Vel < hydro_R_EngageVel && hydro_R &&
                        hydro_R_Fill > 0.01)
                        dizel_nreg_min = std::min(
                            dizel_nreg_min + 5.0 * dt, dizel_nmin_retarder);
                    else
        ...
        if (dizel_nmax_cutoff > 0) {
            ...
            if ((enrot < dizel_nreg_min) &&
                (RList[mcp].R > 0.001))  // jesli ponizej biegu jalowego i
                                         // niezerowa dawka, to dawaj pelna
                realfill = 1;
        }
    }
...
// *************************************************************************************************
double TMoverParameters::dizel_Momentum(
    double dizel_fill, double n,
    double dt) {  // liczy moment sily wytwarzany przez silnik spalinowy}
    double Moment = 0, enMoment = 0, gearMoment = 0, eps = 0, newn = 0,
           friction = 0, neps = 0;
    ...
    neps = (n - dizel_n_old) /
           dt;  // przyspieszenie katowe walu wejsciowego skrzyni biegow

    if (enrot > 0) {
        if (dizel_Momentum_Table.size() > 1) {
            Moment =
                TableInterpolation(dizel_Momentum_Table, enrot) * dizel_fill -
                dizel_Mstand;
        } else {
            Moment = (dizel_Mmax - (dizel_Mmax - dizel_Mnmax) *
                                       square(
                                           (enrot - dizel_nMmax) /
                                           (dizel_nMmax - dizel_nmax))) *
                         dizel_fill -
                     dizel_Mstand;
        }
        Mm = Moment;
        dizel_FuelConsumptionActual =
            dizel_FuelConsumption * enrot * dizel_fill;
    ...
    if (true == dizel_spinup)
        Moment += dizel_Mstand /
                  (0.3 + std::max(0.0, enrot / dizel_nmin));  // rozrusznik
    ...
    if (hydro_TC)  // jesli przetwornik momentu
    {
        ...
        if ((Vel > hydro_TC_LockupSpeed) && (Mains) &&
            (enrot > 0.9 * dizel_nmin) && (IsPower)) {
            hydro_TC_Lockup = true;
            hydro_TC_LockupRate += hydro_TC_FillRateInc * dt;
        }
        // luzowanie sprzegla blokujacego
        if ((Vel < (IsPower ? hydro_TC_LockupSpeed : hydro_TC_UnlockSpeed)) ||
            (!Mains) || (enrot < 0.8 * dizel_nmin)) {
    ...
    if (hydro_TC)  // jesli hydro
    {
        ...
        if (hydro_TC_Table.size() > 1) {
            ...
            hydro_TC_TorqueIn =
                HydroTorque * hydro_TC_Fill * std::min(1.0, hydro_TC_TMRatio);
        } else {
            if (nOut2In < hydro_TC_CouplingPoint) {
                hydro_TC_TMRatio =
                    1 + (hydro_TC_TMMax - 1) *
                            square(1 - nOut2In / hydro_TC_CouplingPoint);
                hydro_TC_TorqueIn = HydroTorque * hydro_TC_Fill;
                hydro_TC_TorqueOut =
                    HydroTorque * hydro_TC_Fill * hydro_TC_TMRatio;
            } else {
    ...
    if (abs(Moment) > Min0R(TorqueC, TorqueL + abs(hydro_TC_TorqueIn)) ||
        (abs(dizel_n_old - enrot) >
         0.1))  // slizga sie z powodu roznic predkosci albo przekroczenia
                // momentu
    {
        ...
        if (TorqueC > TorqueL) {
            if (TorqueC > TorqueL + abs(TorqueH)) {
                hydro_TC_nOut = n;
                gearMoment =
                    TorqueL + abs(TorqueH) * sign(dizel_engagedeltaomega);
                enMoment = Moment - (TorqueL + abs(hydro_TC_TorqueIn)) *
                                        sign(dizel_engagedeltaomega);
            } else {
                hydro_TC_nOut =
                    enrot - (n - enrot) * (TorqueC - TorqueL) /
                                TorqueH;  // slizganie proporcjonalne, zeby
                                          // przetwornik nadrabial
        ...
        if (((newn - n) * (enrot - dizel_n_old) < 0) &&
            ...
            newn = n;
        if ((newn * enrot <= 0) &&
            (eps * enrot < 0))  // przejscie przez zero obrotow
            newn = 0;
        enrot = newn;
    } else  // nie slizga sie (jeszcze)
    {
        ...
        double enrot_min =
            enrot -
            (Min0R(TorqueC, TorqueL + abs(hydro_TC_TorqueIn)) - Moment) /
                dizel_AIM * dt;
        double enrot_max =
            enrot +
            (Min0R(TorqueC, TorqueL + abs(hydro_TC_TorqueIn)) + Moment) /
...
double TMoverParameters::dizel_MomentumRetarder(double n, double dt) {
    ...
    if (Vel < hydro_R_MinVel) RetarderRequest = 0;
    ...
    if ((!hydro_R_Clutch) ||
        ...
        hydro_R_n =
            sign(n) * std::min(
                          std::abs(hydro_R_n + hydro_R_ClutchSpeed * dt),
                          std::abs(n * 60));
    } else {
        hydro_R_n = 0;
    }
    ...
    {
        hydro_R_Fill =
            std::min(hydro_R_Fill + hydro_R_FillRateInc * dt, RetarderRequest);
    } else {
        hydro_R_Fill =
            std::max(hydro_R_Fill - hydro_R_FillRateDec * dt, RetarderRequest);
    }

    double Moment = hydro_R_MaxTorque;
    double pwr = Moment * std::abs(n) * M_PI * 2 * 0.001;
    if (pwr > hydro_R_MaxPower) Moment = Moment * hydro_R_MaxPower / pwr;
...
void TMoverParameters::dizel_HeatSet(float const Value) {
    dizel_heat.Te =  // TODO: don't include ambient temperature, pull it from
                     // environment data instead
        dizel_heat.Ts = dizel_heat.To = dizel_heat.Tsr = dizel_heat.Twy =
            dizel_heat.Tsr2 = dizel_heat.Twy2 = dizel_heat.temperatura1 =
...
void TMoverParameters::dizel_Heat(double const dt) {
    ...
    // TODO: calculate this once and cache for further use, instead of doing it
    ...
    auto const gw = engineon * interpolate(gwmin, gwmax, revolutionsfactor) +
                    waterpump * 1000 + engineoff * 200;
    auto const gw2 = engineon * interpolate(gwmin2, gwmax2, revolutionsfactor) +
                     waterpump * 1000 + engineoff * 200;
    ...
    dizel_heat.water.is_cold =
        ((dizel_heat.water.config.temp_min > 0) &&
         (dizel_heat.temperatura1 <
          dizel_heat.water.config.temp_min - (Mains ? 5 : 0)));
    dizel_heat.water.is_hot =
        ((dizel_heat.water.config.temp_max > 0) &&
         (dizel_heat.temperatura1 > dizel_heat.water.config.temp_max -
                                        (dizel_heat.water.is_hot ? 8 : 0)));
    dizel_heat.water_aux.is_cold =
        ((dizel_heat.water_aux.config.temp_min > 0) &&
         (dizel_heat.temperatura2 <
          dizel_heat.water_aux.config.temp_min - (Mains ? 5 : 0)));
    dizel_heat.water_aux.is_hot =
        ((dizel_heat.water_aux.config.temp_max > 0) &&
         (dizel_heat.temperatura2 > dizel_heat.water_aux.config.temp_max -
                                        (dizel_heat.water_aux.is_hot ? 8 : 0)));
    ...
    dizel_heat.oil.is_hot =
        ((dizel_heat.oil.config.temp_max > 0) &&
         (dizel_heat.To >
          dizel_heat.oil.config.temp_max - (dizel_heat.oil.is_hot ? 8 : 0)));
    ...
    auto const PT =
        ((false == dizel_heat.water.is_cold) &&
         ...
         (false ==
          dizel_heat.oil
              .is_hot) /* && ( false == awaria_termostatow ) */) /* || PTp */;
    ...
    auto const obciazenie{
        engineon * ((EnginePower / 950) + (Heating ? HeatingPower : 0) + 70)};
    auto const Qd{qs * Ge - obciazenie};
    // silnik oddaje czesc ciepla do wody chlodzacej, a takze pewna niewielka
    // czesc do otoczenia, modyfikowane przez okienko
    auto const Qs{
        (Qd - (dizel_heat.kfs * (dizel_heat.Ts - dizel_heat.Tsr)) -
         (dizel_heat.kfe *
          /* ( 0.3 + 0.7 * ( dizel_heat.okienko ? 1 : 0 ) ) * */ (
              dizel_heat.Ts - dizel_heat.Te)))};
    auto const dTss{Qs / Cs};
    dizel_heat.Ts += (dTss * dt);

    // oil heat transfers
    // olej oddaje cieplo do wody gdy krazy przez wymiennik ciepla == wlaczona
    // pompka lub silnik
    auto const dTo{
    ...
    /*
        if( typ == "SP45" )
            Qp = (float)( podgrzewacz and ( true == WaterPump.is_active ) and (
       Twy < 55 ) and ( Twy2 < 55 ) ) * 1000; else
    */
    auto const Qp =
        (((true == WaterHeater.is_active) && (true == WaterPump.is_active) &&
          (dizel_heat.Twy < 60) && (dizel_heat.Twy2 < 60))
             ? 1
             : 0) *
        1000;

    auto const kurek07{
        1};  // unknown/unimplemented device TBD, TODO: identify and implement?
    ...
    if (true == dizel_heat.auxiliary_water_circuit) {
        ...
        dizel_heat.water_aux.is_warm =
            ((true == dizel_heat.cooling) ||
             ((true == Mains) &&
              (BatteryVoltage >
               (0.75 * NominalBatteryVoltage)) /* && !bezpompy &&
                                                  !awaria_chlodzenia && !WS10 */
              && (dizel_heat.water_aux.config.temp_cooling > 0) &&
              (dizel_heat.temperatura2 >
               dizel_heat.water_aux.config.temp_cooling -
                   (dizel_heat.water_aux.is_warm ? 8 : 0))));
        auto const PTC2{(dizel_heat.water_aux.is_warm /*or PTC2p*/ ? 1 : 0)};
        dizel_heat.rpmwz2 =
            PTC2 * (dizel_heat.fan_speed >= 0 ? (rpm * dizel_heat.fan_speed)
                                              : (dizel_heat.fan_speed * -1));
        dizel_heat.zaluzje2 =
            (dizel_heat.water_aux.config.shutters
                 ? (PTC2 == 1)
        ...
        auto const kf2{
            kurek07 *
                ((dizel_heat.kw * (0.3 + 0.7 * zaluzje2)) * dizel_heat.rpmw2 +
                 (dizel_heat.kv * (0.3 + 0.7 * zaluzje2) * Vel / 3.6)) +
            2};
        auto const dTs2{
            ((dizel_heat.kfo2 * (dizel_heat.To - dizel_heat.Tsr2))) /
            (gw2 * Cw)};
        // przy otwartym kurku B ma³y obieg jest dogrzewany przez du¿y -
        // stosujemy przy korzystaniu z podgrzewacza oraz w zimie
        auto const Qch2{
            -kf2 * (dizel_heat.Tsr2 - dizel_heat.Te) +
            (80 * (true == WaterCircuitsLink ? 1 : 0) *
             (dizel_heat.Twy - dizel_heat.Tsr2))};
        auto const dTch2{Qch2 / (gw2 * Cw)};
        // auxiliary water circuit heat transfers finalization
        // NOTE: since primary circuit doesn't read data from the auxiliary one,
        // we can pretty safely finalize auxiliary updates before touching the
        // primary circuit
    ...
    dizel_heat.water.is_flowing =
        ((dizel_heat.water.config.temp_flow < 0) ||
         (dizel_heat.temperatura1 > dizel_heat.water.config.temp_flow -
                                        (dizel_heat.water.is_flowing ? 5 : 0)));
    auto const obieg{(dizel_heat.water.is_flowing ? 1 : 0)};
    dizel_heat.water.is_warm =
        ((true == dizel_heat.cooling) ||
         ((true == Mains) &&
          (BatteryVoltage >
           (0.75 * NominalBatteryVoltage)) /* && !bezpompy && !awaria_chlodzenia
                                              && !WS10 */
          && (dizel_heat.water.config.temp_cooling > 0) &&
          (dizel_heat.temperatura1 > dizel_heat.water.config.temp_cooling -
                                         (dizel_heat.water.is_warm ? 8 : 0))));
    auto const PTC1{(dizel_heat.water.is_warm /*or PTC1p*/ ? 1 : 0)};
    dizel_heat.rpmwz =
        PTC1 * (dizel_heat.fan_speed >= 0 ? (rpm * dizel_heat.fan_speed)
                                          : (dizel_heat.fan_speed * -1));
    dizel_heat.zaluzje1 =
        (dizel_heat.water.config.shutters
             ? (PTC1 == 1)
             : true);  // no shutters is an equivalent to having them open
...
bool TMoverParameters::AssignLoad(std::string const &Name, float const Amount) {
    if (Name == "pantstate") {
        if (EnginePowerSource.SourceType == TPowerSource::CurrentCollector) {
            // wartość niby "pantstate" - nazwa dla formalności, ważna jest
            // ilość
            ...
            if (pantographsetup & (1 << 0)) {
                if (DoubleTr == 1) {
                    OperatePantographValve(
                        end::front, operation_t::enable, range_t::local);
                } else {
                    OperatePantographValve(
                        end::rear, operation_t::enable, range_t::local);
                }
            }
...
// *************************************************************************************************
bool TMoverParameters::LoadingDone(
    double const LSpeed, std::string const &Loadname) {
    if (LSpeed == 0.0) {
    ...
    auto const loadchange{static_cast<float>(
        std::abs(LSpeed * LastLoadChangeTime) *
        (LoadAmount > MaxLoad ? 0.5 : 1.0))};
    ...
    if (LSpeed < 0) {
        ...
        if (loadchange > 0)  // jeśli coś przeładowano
        {
            ...
            if ((LoadAmount >= MaxLoad * (1.0 + OverLoadFactor)) ||
                (CommandIn.Value1 <= 0)) {
                LoadStatus = 4;  // skończony załadunek
                LoadAmount = std::min<float>(
                    MaxLoad * (1.0 + OverLoadFactor), LoadAmount);
            }
            ComputeMass();
        }
    }
...
bool TMoverParameters::PermitDoorStep(bool const State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        ...
        SendCtrlToNext(
            "DoorStep", (State == true ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::PermitDoors(
    side const Door, bool const State, range_t const Notify) {
...
bool TMoverParameters::ChangeDoorControlMode(
    bool const State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        // wysłanie wyłączenia do pozostałych?
        SendCtrlToNext(
            "DoorMode", (State == true ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::OperateDoors(
    side const Door, bool const State, range_t const Notify) {
    auto &door{Doors.instances[Door]};
    /*
        if( ( State == true ? door.is_open : door.is_closed ) ) {
            // TBD: should the command be passed to other vehicles regardless of
       whether it affected the primary target?
            // (for the time being no, methods are often invoked blindly which
       would lead to commands spam) return false;
        }
    */
    bool result{false};
...
bool TMoverParameters::LockDoors(bool const State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        // wysłanie wyłączenia do pozostałych?
        SendCtrlToNext(
            "DoorLock", (State == true ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
bool TMoverParameters::signal_departure(
    bool const State, range_t const Notify) {
    ...
    if (Notify != range_t::local) {
        // wysłanie wyłączenia do pozostałych?
        SendCtrlToNext(
            "DepartureSignal", (State == true ? 1 : 0), CabActive,
            (Notify == range_t::unit ? coupling::control | coupling::permanent
                                     : coupling::control));
    }
...
void TMoverParameters::update_doors(double const Deltatime) {
    ...
    Doors.is_locked = (true == Doors.has_lock) &&
                      (true == Doors.lock_enabled) && (Vel >= 10.0);
    ...
    for (auto &door : Doors.instances) {
        ...
        door.open_permit =
            ...
            &&
            ((false ==
              Doors.permit_presets.empty())  // ...there's no preset switch
                                             // controlling permit state...
             ||
             ((false == Doors.is_locked)  // ...and the door lock is engaged...
              && (false ==
                  door.remote_close)));  // ...or the door is about to be closed
        ...
        door.remote_open = (door.remote_open || Doors.remote_only) &&
                           (false == door.is_open) &&
                           ((false == Doors.permit_needed) || door.open_permit);
        door.local_close =
            door.local_close && (false == door.is_closed) &&
            ((false == remoteopencontrol) || (false == door.remote_open));
        door.remote_close =
            door.remote_close && (false == door.is_closed) &&
        ...
        door.is_opening =
            (false == door.is_open) && (true == ispowered) &&
            (false == closerequest) &&
            ((true == door.is_opening) ||
             ((true == openrequest) && (false == Doors.is_locked)));
        door.is_closing = (false == door.is_closed) && (true == ispowered) &&
                          (false == openrequest) &&
                          (door.is_closing || closerequest);
        door.step_unfolding =
            ((Doors.step_range != 0.f) && (Doors.step_enabled) &&
             (false == Doors.is_locked) && (door.step_position < 1.f) &&
             (door.is_opening));
        door.step_folding =
            ((door.step_position > 0.f)  // is unfolded
             &&
             ((false == Doors.step_enabled)  // we lost permission to stay open
                                             // or our door is calling the shots
              || (Doors.permit_needed ? (false == door.open_permit)
                                      : door.is_closing)) &&
             ((door.close_delay > Doors.close_delay) ||
              door.position <=
                  0.f));  // door is about to close, or already done

        if (true == door.is_opening) {
            door.auto_timer =
                ((localopencontrol && door.local_open) ? Doors.auto_duration
                 : (remoteopencontrol && door.remote_open &&
                    Doors.auto_include_remote)
                     ? Doors.auto_duration
                     : -1.f);
        }
        // doors
        ...
        if (true == door.is_closing) {
            ...
            if (door.close_delay > Doors.close_delay) {
                door.position = std::max<float>(
                    0.f, door.position - Doors.close_rate * Deltatime);
            }
            door.open_delay = 0.f;
        }
        // doorsteps
        if (door.step_unfolding) {
            ...
            door.step_position = std::min<float>(
                1.f, door.step_position + Doors.step_rate * Deltatime);
        }
        if (door.step_folding) {
            // fold left doorstep
            if ((TrainType == dt_EZT) || (TrainType == dt_DMU)) {
                // multi-unit vehicles typically fold the doorstep only after
                // closing the door
                if (door.position <= 0.f) {
                    door.step_position = std::max<float>(
                        0.f, door.step_position - Doors.step_rate * Deltatime);
                }
            } else {
                door.step_position = std::max<float>(
                    0.f, door.step_position - Doors.step_rate * Deltatime);
    ...
    if ((false == Doors.instances[side::right].is_open) &&
        (false == Doors.instances[side::left].is_open)) {
        return;
    }
    ...
    if (Doors.auto_duration > 0.f) {
        ...
        for (auto &door : Doors.instances) {
            ...
            // if there's load exchange in progress, reset the timer(s) for
            // already open doors
    ...
    /*
        // the door are closed if their timer goes below 0, or if the vehicle is
       moving faster than defined threshold std::array<side, 2> const doorids {
       side::right, side::left }; for( auto const doorid : doorids ) { auto
       const &door { Doors.instances[ doorid ] }; if( true == door.is_open ) {
                if( ( ( Doors.auto_velocity != -1.f ) && ( Vel >
       Doors.auto_velocity ) )
                 || ( ( door.auto_timer != -1.f ) && ( door.auto_timer <= 0.f )
       ) ) {
                    // close the door and set the timer to expired state
       (closing may happen sooner if vehicle starts moving) OperateDoors(
       doorid, false, range_t::local );
                }
            }
        }
    */
...
bool TMoverParameters::ChangeOffsetH(double DeltaOffset) {
    ...
    if (TestFlag(CategoryFlag, 2) && TestFlag(RunningTrack.CategoryFlag, 2)) {
        ...
        //     if (abs(OffsetTrackH) > (RunningTrack.Width / 1.95 - TrackW
        //     / 2.0))
        if (abs(OffsetTrackH) > (0.5 * (RunningTrack.Width - Dim.W) -
                                 0.05))  // Ra: może pół pojazdu od brzegu?
...
// *************************************************************************************************
// Q: 20160713
// Testuje zmienną (narazie tylko 0) i na podstawie uszkodzenia zwraca
// informację tekstową
// *************************************************************************************************
...
double TMoverParameters::GetTrainsetVoltage(
    int const Coupling) const {  // ABu: funkcja zwracajaca napiecie dla calego
                                 // skladu, przydatna dla EZT
    ...
    for (int end = end::front; end <= end::rear; ++end) {
        ...
        auto *connectedpowercoupling =
            ((Coupling & (coupling::highvoltage | coupling::heating)) != 0
...
// *************************************************************************************************
bool TMoverParameters::switch_physics(
    bool const State)  // DO PRZETLUMACZENIA NA KONCU
{
...
int s2NPW(std::string s) {  // wylicza ilosc osi napednych z opisu ukladu osi
    ...
    for (std::size_t k = 0; k < s.size(); ++k) {
        if (s[k] >= (char)65 && s[k] <= (char)90) NPW += s[k] - A;
    }
    return NPW;
}

int s2NNW(
    std::string s) {  // wylicza ilosc osi nienapedzanych z opisu ukladu osi
    const char Zero = 48;
    int NNW = 0;
    for (std::size_t k = 0; k < s.size(); ++k) {
        if (s[k] >= (char)49 && s[k] <= (char)57) NNW += s[k] - Zero;
    }
    return NNW;
}
...
bool TMoverParameters::readMPT0(std::string const &line) {
    // TBD, TODO: split into separate functions similar to readMPT if more
    // varied schemes appear?
    cParser parser(line);
    if (false == parser.getTokens(7, false)) {
        WriteLog(
            "Read MPT0: arguments missing in line " + std::to_string(LISTLINE));
        return false;
    }
    ...
    switch (EngineType) {
        case TEngineType::DieselEngine: {
            parser >> MotorParam[idx].mIsat >> MotorParam[idx].fi0 >>
                MotorParam[idx].fi >> MotorParam[idx].mfi0 >>
                MotorParam[idx].mfi >> MotorParam[idx].Isat;
            break;
        }
        default: {
            parser >> MotorParam[idx].mfi >> MotorParam[idx].mIsat >>
                MotorParam[idx].mfi0 >> MotorParam[idx].fi >>
                MotorParam[idx].Isat >> MotorParam[idx].fi0;
            break;
        }
    }
...
bool TMoverParameters::readMPTElectricSeries(std::string const &line) {
    ...
    if (false == parser.getTokens(5, false)) {
        WriteLog(
            "Read MPT: arguments missing in line " + std::to_string(LISTLINE));
        return false;
    }
    int idx = 0;  // numer pozycji
    parser >> idx;
    parser >> MotorParam[idx].mfi >> MotorParam[idx].mIsat >>
        MotorParam[idx].fi >> MotorParam[idx].Isat;
    if (true == parser.getTokens(1, false)) {
        int autoswitch;
        parser >> autoswitch;
        MotorParam[idx].AutoSwitch = (autoswitch == 1);
...
bool TMoverParameters::readMPTDieselElectric(std::string const &line) {
    ...
    parser >> idx;
    parser >> MotorParam[idx].mfi >> MotorParam[idx].mIsat >>
        MotorParam[idx].fi >> MotorParam[idx].Isat >> MPTRelay[idx].Iup >>
        MPTRelay[idx].Idown;
...
bool TMoverParameters::readMPTDieselEngine(std::string const &line) {
    ...
    int idx = 0;  // numer pozycji
    parser >> idx;
    parser >> MotorParam[idx].mIsat >> MotorParam[idx].fi >>
        MotorParam[idx].mfi;
    if (true == parser.getTokens(1, false)) {
        int autoswitch;
        parser >> autoswitch;
...
bool TMoverParameters::readBPT(std::string const &line) {
    ...
    if (false == parser.getTokens(5, false)) {
        WriteLog(
            "Read BPT: arguments missing in line " +
            std::to_string(LISTLINE + 1));
        return false;
    }
    ++LISTLINE;
    ...
    int idx = 0;
    parser >> idx;
    parser >> BrakePressureTable[idx].PipePressureVal >>
...
bool TMoverParameters::readRList(std::string const &Input) {
    ...
    if (false == parser.getTokens(5, false)) {
        WriteLog(
            "Read RList: arguments missing in line " +
            std::to_string(LISTLINE + 1));
        return false;
    }
    auto idx = LISTLINE++;
    if (idx >= sizeof(RList) / sizeof(TScheme)) {
        WriteLog(
            "Read RList: number of entries exceeded capacity of the data "
            "table");
        return false;
    }
    parser >> RList[idx].Relay >> RList[idx].R >> RList[idx].Bn >>
        RList[idx].Mn >> RList[idx].AutoSwitch;
...
bool TMoverParameters::readUCList(std::string const &line) {
    ...
    if (idx >= sizeof(UniCtrlList) / sizeof(TUniversalCtrl)) {
        WriteLog(
            "Read UCList: number of entries exceeded capacity of the data "
            "table");
        return false;
    }
    int i = 0;
    parser >> i >> UniCtrlList[idx].mode >> UniCtrlList[idx].MinCtrlVal >>
        UniCtrlList[idx].MaxCtrlVal >> UniCtrlList[idx].SetCtrlVal >>
        UniCtrlList[idx].SpeedUp >> UniCtrlList[idx].SpeedDown >>
        UniCtrlList[idx].ReturnPosition >> UniCtrlList[idx].NextPosFastInc >>
...
bool TMoverParameters::readDList(std::string const &line) {
    ...
    if (idx >= sizeof(RList) / sizeof(TScheme)) {
        WriteLog(
            "Read DList: number of entries exceeded capacity of the data "
            "table");
        return false;
    }
...
bool TMoverParameters::readDMList(std::string const &line) {
    ...
    if (false == parser.getTokens(2, false)) {
        WriteLog(
            "Read DMList: arguments missing in line " +
            std::to_string(LISTLINE + 1));
        return false;
    }
    auto idx = LISTLINE++;
...
bool TMoverParameters::readHTCList(std::string const &line) {
    ...
    if (false == parser.getTokens(2, false)) {
        WriteLog(
            "Read HTCList: arguments missing in line " +
            std::to_string(LISTLINE + 1));
        return false;
    }
    auto idx = LISTLINE++;
...
bool TMoverParameters::readFFList(std::string const &line) {
    ...
    if (false == parser.getTokens(2, false)) {
        WriteLog(
            "Read FList: arguments missing in line " +
            std::to_string(LISTLINE + 1));
        return false;
    }
    int idx = LISTLINE++;
    if (idx >= sizeof(DElist) / sizeof(TDEScheme)) {
        WriteLog(
            "Read FList: number of entries exceeded capacity of the data "
            "table");
        return false;
    }
...
bool TMoverParameters::readWWList(std::string const &line) {
    ...
    if (false == parser.getTokens(4, false)) {
        WriteLog(
            "Read WWList: arguments missing in line " +
            std::to_string(LISTLINE + 1));
        return false;
    }
    int idx = LISTLINE++;
    if (idx >= sizeof(DElist) / sizeof(TDEScheme)) {
        WriteLog(
            "Read WWList: number of entries exceeded capacity of the data "
            "table");
        return false;
    }
    parser >> DElist[idx].RPM >> DElist[idx].GenPower >> DElist[idx].Umax >>
        DElist[idx].Imax;
    ...
    if (true == parser.getTokens(3, false)) {
        ...
        SST[idx].Pmax =
            std::min(SST[idx].Pmax, std::pow(SST[idx].Umax, 2) / 47.6);
    }
...
bool TMoverParameters::readLightsList(std::string const &Input) {
    ...
    if (false == parser.getTokens(2, false)) {
        WriteLog(
            "Read LightsList: arguments missing in line " +
            std::to_string(LISTLINE + 1));
        return false;
    }
    int idx = LISTLINE++;
    if (idx > 16) {
        WriteLog(
            "Read LightsList: number of entries exceeded capacity of the data "
            "table");
        return false;
    }
...
bool TMoverParameters::readCompressorList(std::string const &Input) {
    ...
    if (false == parser.getTokens(4, false)) {
        WriteLog(
            "Read CompressorList: arguments missing in line " +
            std::to_string(LISTLINE + 1));
        return false;
    }
    int idx = LISTLINE++;
    if (idx > 8 - 1) {
        WriteLog(
            "Read CompressorList: number of entries exceeded capacity of the "
            "data table");
        return false;
    }
    // NOTE: content of slot [x][0] is hardcoded elsewhere
    parser >> CompressorList[0][idx + 1] >> CompressorList[1][idx + 1] >>
        CompressorList[2][idx + 1] >> CompressorList[3][idx + 1];
...
void TMoverParameters::BrakeValveDecode(std::string const &Valve) {
    ...
    BrakeValve =
        lookup != valvetypes.end() ? lookup->second : TBrakeValve::Other;
...
// TDynamicObject::Init()
// Po niej wykonywana jest CreateBrakeSys(), ktora jest odpowiednikiem
// CheckLocomotiveParameters()
// *************************************************************************************************
bool TMoverParameters::LoadFIZ(std::string chkpath) {
    ...
    */
    while (fizparser.ok()) {
        ...
        bool comment =
            ((contains(inputline, '#')) || (starts_with(inputline, "//")));
        if (true == comment) {
        ...
        if (issection("Security:", inputline)) {
            ...
            extract_value(
                EmergencyBrakeWarningSignal, "EmergencyBrakeWarningSignal",
                inputline, "");
            continue;
        }
        ...
        // NOTE: once table parsing is enabled it lasts until switched off, when
        // another section is recognized
        if (true == startBPT) {
            readBPT(inputline);
            continue;
        }
       ...
       /*
           in.close();
       */
    // Operacje na zebranych parametrach - przypisywanie do wlasciwych zmiennych
    // i ustawianie zaleznosci
    ...
    WriteLog(
        "CERROR: " + to_string(ConversionError) +
        ", SUCCES: " + to_string(result));
    return result;
}
...
void TMoverParameters::LoadFIZ_Load(std::string const &line) {
    ...
    // NOTE: last (if any) offset parameter retrieved from the list applies to
    // the remainder of the list TBD, TODO: include other load parameters in
    // this system
...
void TMoverParameters::LoadFIZ_Wheels(std::string const &line) {
    ...
    WheelDiameterL =
        WheelDiameter;  // gdyby nie było parametru, lepsze to niż zero
...
void TMoverParameters::LoadFIZ_Brake(std::string const &line) {
    ...
    if (MaxBrakePress[3] > 0.0) {
        ...
        if (BrakeCylNo > 0) {
            ...
            BrakeVolume =
                M_PI * std::pow(BrakeCylRadius, 2) * BrakeCylDist * BrakeCylNo;
            extract_value(BrakeVVolume, "BVV", line, "");

            {
                std::map<std::string, int> brakemethods{
                    {"P10-Bg", bp_P10Bg},         {"P10-Bgu", bp_P10Bgu},
                    {"FR513", bp_FR513},          {"FR510", bp_FR510},
                    {"Cosid", bp_Cosid},          {"P10yBg", bp_P10yBg},
                    {"P10yBgu", bp_P10yBgu},      {"Disk1", bp_D1},
                    {"Disk1+Mg", bp_D1 + bp_MHS}, {"Disk2", bp_D2}};
    ...
    CntrlPipePress =
        5 +
        0.001 * (Random(10) - Random(10));  // Ra 2014-07: trochę niedokładności
...
void TMoverParameters::LoadFIZ_Doors(std::string const &line) {
    ...
    // opening method
    {
        ...
        Doors.open_control = lookup != doorcontrols.end()
                                 ? lookup->second
                                 : control_t::passenger;
    }
    // closing method
    {
        ...
        Doors.close_control = lookup != doorcontrols.end()
                                  ? lookup->second
                                  : control_t::passenger;
    }
    ...
    extract_value(Doors.permit_needed, "DoorNeedPermit", line, "");
    {
        ...
        if (false == Doors.permit_presets.empty()) {
            // HACK: legacy position indices start from 1, so we deduct 1 to
            // arrive at proper index into the array
            extract_value(
                Doors.permit_preset, "DoorPermitListDefault", line, "1");
            Doors.permit_preset =
                std::min<int>(
                    Doors.permit_presets.size(), Doors.permit_preset) -
                1;
        }
    }
    ...
    // opening method
    {
        ...
        Doors.type = lookup != doortypes.end()
                         ? lookup->second
...

    {
        ...
        extract_value(
            Doors.voltage, "DoorVoltage", line,
            (remotedoorcontrol ? "24" : "0"));
    }
    ...
    extract_value(
        DoorsPermitLightBlinking, "DoorsPermitLightBlinking", line, "");
}

void TMoverParameters::LoadFIZ_BuffCoupl(
    std::string const &line, int const Index) {
    TCoupling *coupler;
    if (Index == 2) {
        coupler = &Couplers[1];
    } else {
    ...
    coupler->CouplerType =
        (lookup != couplertypes.end() ? lookup->second
...
void TMoverParameters::LoadFIZ_Cntrl(std::string const &line) {
    {
        ...
        BrakeSystem = lookup != brakesystems.end() ? lookup->second
    ...
    if (BrakeSystem != TBrakeSystem::Individual) {
        ...
        for (int idx = 0; idx < 4; ++idx) {
            extract_value(
                BrakeDelay[idx], "BDelay" + std::to_string(idx + 1), line, "");
        }
        // brakedelays, brakedelayflag
        {
            ...
            std::map<std::string, int> brakedelayflags{
                {"R", bdelay_R}, {"P", bdelay_P}, {"G", bdelay_G}};
            ...
            BrakeDelayFlag =
                lookup != brakedelayflags.end() ? lookup->second : 0;
        }
        // brakeopmode
        {
            ...
            auto lookup =
                brakeopmodes.find(extract_value("BrakeOpModes", line));
        ...
        // brakehandle
        {
            ...
            BrakeHandle = lookup != brakehandles.end() ? lookup->second
        ...
        // brakelochandle
        {
            std::map<std::string, TBrakeHandle> locbrakehandles{
                ...
                {"Westinghouse", TBrakeHandle::West}};
            auto lookup =
                locbrakehandles.find(extract_value("LocBrakeHandle", line));
            BrakeLocHandle = lookup != locbrakehandles.end()
                                 ? lookup->second
                                 : TBrakeHandle::NoHandle;
        }
        ...
        if (true == extract_value(MBPM, "MaxBPMass", line, "")) {
            // NOTE: only convert the value from tons to kilograms if the entry
    ...
    // localbrake
    {
        ...
        LocalBrake =
            lookup != localbrakes.end() ? lookup->second : TLocalBrake::NoBrake;
    }
    // mbrake
    ...
    // dynamicbrake
    {
        ...
        DynamicBrakeType =
            lookup != dynamicbrakes.end() ? lookup->second : dbrake_none;
    ...
    extract_value(
        ScndS, "ScndS", line,
        "");  // brak pozycji rownoleglej przy niskiej nastawie PSR
    ...
    FastSerialCircuit =
        (ToLower(extract_value("FSCircuit", line)) == "yes") ? 1 : 0;
    extract_value(
        BackwardsBranchesAllowed, "BackwardsBranchesAllowed", line, "");
    ...
    extract_value(
        ReleaseParkingBySpringBrake, "ReleaseParkingBySpringBrake", line, "");
    extract_value(
        ReleaseParkingBySpringBrakeWhenDoorIsOpen,
        "ReleaseParkingBySpringBrakeWhenDoorIsOpen", line, "");
    extract_value(SpringBrakeCutsOffDrive, "SpringBrakeCutsOffDrive", line, "");
    extract_value(
        SpringBrakeDriveEmergencyVel, "SpringBrakeDriveEmergencyVel", line, "");
    ...
    // battery
    {
        ...
        BatteryStart =
            lookup != starts.end() ? lookup->second : start_t::manual;
    }
    // converter
    {
    ...
    extract_value(
        ConverterOverloadRelayOffWhenMainIsOff,
        "ConverterOverloadWhenMainIsOff", line,
        (TrainType == dt_EZT ? "yes" : "no"));
    // compressor
    {
    ...
    PantAutoValve =
        (TrainType == dt_EZT);  // legacy code behaviour, automatic valve was
                                // initially installed in all EMUs
    ...
    // pantographs valve
    {
        ...
        PantsValve.start_type =
            lookup != starts.end()
                ? lookup->second
                : start_t::automatic;  // legacy code behaviour, there was no
                                       // pantographs valve
    ...
    {
        auto lookup =
            starts.find(extract_value("CompartmentLightsStart", line));
        CompartmentLights.start_type =
            lookup != starts.end() ? lookup->second
    ...
    // ground relay
    {
        ...
        GroundRelayStart =
            (lookup != starts.end() ? lookup->second
                                    : (TrainType == dt_EZT ? start_t::automatic
                                                           : start_t::manual));
    }
    // converter overload relay
    {
        auto lookup =
            starts.find(extract_value("ConverterOverloadRelayStart", line));
        ConverterOverloadRelayStart =
            (lookup != starts.end()
                 ? lookup->second
                 : (TrainType == dt_EZT
                        ? start_t::converter
                        :  // relay activates when converter is switched off
                        start_t::manual));
    }
}
...
void TMoverParameters::LoadFIZ_Light(std::string const &line) {
    LightPowerSource.SourceType =
        LoadFIZ_SourceDecode(extract_value("Light", line));
    LoadFIZ_PowerParamsDecode(LightPowerSource, "L", line);

    AlterLightPowerSource.SourceType =
        LoadFIZ_SourceDecode(extract_value("AlterLight", line));
...
void TMoverParameters::LoadFIZ_Clima(std::string const &line) {
    HeatingPowerSource.SourceType =
        LoadFIZ_SourceDecode(extract_value("Heating", line));
    LoadFIZ_PowerParamsDecode(HeatingPowerSource, "H", line);
    AlterHeatPowerSource.SourceType =
        LoadFIZ_SourceDecode(extract_value("AlterHeating", line));
    LoadFIZ_PowerParamsDecode(AlterHeatPowerSource, "AlterH", line);
}

void TMoverParameters::LoadFIZ_Power(std::string const &Line) {
    EnginePowerSource.SourceType =
        LoadFIZ_SourceDecode(extract_value("EnginePower", Line));
    ...
    */
    ...
    SystemPowerSource.SourceType =
        LoadFIZ_SourceDecode(extract_value("SystemPower", Line));
...
void TMoverParameters::LoadFIZ_SpeedControl(std::string const &Line) {
    ...
    if ((!SpeedCtrl) && (EngineType == TEngineType::ElectricInductionMotor) &&
        (ScndCtrlPosNo > 0))  // backward compatibility
        SpeedCtrl = true;
    extract_value(SpeedCtrlDelay, "SpeedCtrlDelay", Line, "");
    SpeedCtrlTypeTime =
        (extract_value("SpeedCtrlType", Line) == "Time") ? true : false;
    ...
    extract_value(
        SpeedCtrlUnit.ManualStateOverride, "OverrideManual", Line, "");
    extract_value(
        SpeedCtrlUnit.BrakeIntervention, "BrakeIntervention", Line, "");
...
void TMoverParameters::LoadFIZ_Engine(std::string const &Input) {
    ...
    if (false == transmission.empty()) {
        // transmission type. moved here because more than one engine type has
        // this entry
        ...
        Transmision.NToothM =
            std::atoi(ratios[0].c_str());  // ToothM to pierwszy czyli 18
        Transmision.NToothW =
            std::atoi(ratios[1].c_str());  // ToothW to drugi parametr czyli 79

        if (Transmision.NToothM > 0)
            Transmision.Ratio =
                static_cast<double>(Transmision.NToothW) / Transmision.NToothM;
        else
    ...
    switch (EngineType) {
        ...
        case TEngineType::DieselEngine: {
            ...
            extract_value(
                dizel_nmin_hdrive_factor, "nmin_hdrive_factor", Input, "");
            ...
            // TODO: unify naming scheme and sort out which diesel engine params
            ...
            extract_value(
                dizel_RevolutionsDecreaseRate, "RPMDecRate", Input, "");
            ...
            if (true == extract_value(AnPos, "ShuntMode", Input, "")) {
                ...
                if (AnPos < 1.0) {
                    ...
                    AnPos =
                        1.0 / AnPos;  // im większa liczba, tym wolniej jedzie
                }
            }
            ...
            if (true == hydro_TC) {
                ...
                if (true == hydro_R) {
                    ...
                    extract_value(
                        hydro_R_ClutchSpeed, "R_ClutchSpeed", Input, "");
                    extract_value(
                        hydro_R_WithIndividual, "R_WithIndividual", Input, "");
                }
            }
            break;
        }
        ...
        case TEngineType::ElectricInductionMotor: {
            ...
            extract_value(
                eimc[eimc_f_cfuH], "fcfuH", Input, to_string(eimc[eimc_f_cfu]));
            ...
            extract_value(
                InverterControlCouplerFlag, "InvCtrCplFlag", Input, "");
    ...
    if ((EngineType == TEngineType::DieselEngine) ||
        ...
        extract_value(
            dizel_heat.water.config.temp_min, "WaterMinTemperature", Input, "");
        extract_value(
            dizel_heat.water.config.temp_max, "WaterMaxTemperature", Input, "");
        extract_value(
            dizel_heat.water.config.temp_flow, "WaterFlowTemperature", Input,
            "");
        extract_value(
            dizel_heat.water.config.temp_cooling, "WaterCoolingTemperature",
            Input, "");
        extract_value(
            dizel_heat.water.config.shutters, "WaterShutters", Input, "");
        extract_value(
            dizel_heat.auxiliary_water_circuit, "WaterAuxCircuit", Input, "");
        extract_value(
            dizel_heat.water_aux.config.temp_min, "WaterAuxMinTemperature",
            Input, "");
        extract_value(
            dizel_heat.water_aux.config.temp_max, "WaterAuxMaxTemperature",
            Input, "");
        extract_value(
            dizel_heat.water_aux.config.temp_cooling,
            "WaterAuxCoolingTemperature", Input, "");
        extract_value(
            dizel_heat.water_aux.config.shutters, "WaterAuxShutters", Input,
            "");
        extract_value(
            dizel_heat.oil.config.temp_min, "OilMinTemperature", Input, "");
        extract_value(
            dizel_heat.oil.config.temp_max, "OilMaxTemperature", Input, "");
        extract_value(dizel_heat.fan_speed, "WaterCoolingFanSpeed", Input, "");
        // water heater
        extract_value(
            WaterHeater.config.temp_min, "HeaterMinTemperature", Input, "");
        extract_value(
            WaterHeater.config.temp_max, "HeaterMaxTemperature", Input, "");
    }

    // traction motors
    extract_value(
        MotorBlowers[end::front].speed, "MotorBlowersSpeed", Input, "");
    extract_value(
        MotorBlowers[end::front].sustain_time, "MotorBlowersSustainTime", Input,
        "");
    extract_value(
        MotorBlowers[end::front].min_start_velocity,
        "MotorBlowersStartVelocity", Input, "");
    MotorBlowers[end::rear] = MotorBlowers[end::front];
    // pressure switch
    extract_value(
        HasControlPressureSwitch, "PressureSwitch", Input,
        (TrainType != dt_EZT ? "yes" : "no"));
}
...
void TMoverParameters::LoadFIZ_Switches(std::string const &Input) {
    ...
    // because people can't make up their minds whether it's "impulse" or
    // "Impulse"...
...
void TMoverParameters::LoadFIZ_MotorParamTable(std::string const &Input) {
    switch (EngineType) {
        case TEngineType::DieselEngine: {
            extract_value(
                dizel_minVelfullengage, "minVelfullengage", Input, "");
...
void TMoverParameters::LoadFIZ_UCList(std::string const &Input) {
    ...
    extract_value(
        UniCtrlIntegratedLocalBrakeCtrl, "IntegratedLocBrake", Input, "");
...
void TMoverParameters::LoadFIZ_DList(std::string const &Input) {
    ...
    extract_value(
        dizel_NominalFuelConsumptionRate, "NomFuelConsRate", Input, "");

    if (dizel_nMmax == dizel_nmax) {
        // HACK: guard against cases where nMmax == nmax, leading to division by
        // 0 in momentum calculation
        dizel_nMmax = dizel_nmax - 1.0 / 60.0;
    }

    // Calculation of fuel consumption coefficient for futher calculation
    double dizel_max_power = dizel_nmax * (dizel_Mnmax - dizel_Mstand) * M_PI *
                             2 * 0.001;  // power in kW
    double dizel_max_energy =
        dizel_max_power;  // energy per one hour in kWh is equal to power in kW
                          // times 1 h
    double fuel_density = 850;  // g/l
    dizel_FuelConsumption = dizel_NominalFuelConsumptionRate *
                            dizel_max_energy / fuel_density / dizel_nmax;
}
...
void TMoverParameters::LoadFIZ_PowerParamsDecode(
    TPowerParameters &Powerparameters, std::string const Prefix,
    std::string const &Line) {
    switch (Powerparameters.SourceType) {
        case TPowerSource::NotDefined:
        case TPowerSource::InternalSource: {
            Powerparameters.PowerType =
                LoadFIZ_PowerDecode(extract_value(Prefix + "PowerType", Line));
            break;
        }
        case TPowerSource::Transducer: {
            extract_value(
                Powerparameters.Transducer.InputVoltage,
                Prefix + "TransducerInputV", Line, "");
            break;
        }
        case TPowerSource::Generator: {
            ...
            auto const enginetype{LoadFIZ_EngineDecode(
                extract_value(Prefix + "GeneratorEngine", Line))};
            if (enginetype == TEngineType::Main) {
                generatorparameters.engine_revolutions = &enrot;
            } else {
                // TODO: for engine types other than Main create requested
                // engine object and link to its revolutions
            ...
            // config
            extract_value(
                generatorparameters.voltage_min, Prefix + "GeneratorMinVoltage",
                Line, "0");
            extract_value(
                generatorparameters.voltage_max, Prefix + "GeneratorMaxVoltage",
                Line, "0");
            // NOTE: for consistency the fiz file specifies  revolutions per
            // minute
            extract_value(
                generatorparameters.revolutions_min, Prefix + "GeneratorMinRPM",
                Line, "0");
            extract_value(
                generatorparameters.revolutions_max, Prefix + "GeneratorMaxRPM",
                Line, "0");
        ...
        case TPowerSource::Accumulator: {
            extract_value(
                Powerparameters.RAccumulator.MaxCapacity, Prefix + "Cap", Line,
                "");
            Powerparameters.RAccumulator.RechargeSource =
                LoadFIZ_SourceDecode(extract_value(Prefix + "RS", Line));
            break;
        }
        case TPowerSource::CurrentCollector: {
            ...
            collectorparameters =
                TCurrentCollector{0, 0, 0, 0, 0, 0, false, 0, 0, 0, false, 0};

            extract_value(
                collectorparameters.CollectorsNo, "CollectorsNo", Line, "");
            extract_value(collectorparameters.MinH, "MinH", Line, "");
            extract_value(collectorparameters.MaxH, "MaxH", Line, "");
            extract_value(
                collectorparameters.CSW, "CSW", Line,
                "");  // szerokość części roboczej
            extract_value(collectorparameters.MaxV, "MaxVoltage", Line, "");
            extract_value(
                collectorparameters.OVP, "OverVoltProt", Line,
                "");  // przekaźnik nadnapięciowy
            // napięcie rozłączające WS
            collectorparameters.MinV =
                0.5 * collectorparameters.MaxV;  // gdyby parametr nie podany
            extract_value(collectorparameters.MinV, "MinV", Line, "");
            // napięcie wymagane do załączenia WS
            collectorparameters.InsetV =
                0.6 * collectorparameters.MaxV;  // gdyby parametr nie podany
            extract_value(collectorparameters.InsetV, "InsetV", Line, "");
            // ciśnienie rozłączające WS
            extract_value(
                collectorparameters.MinPress, "MinPress", Line,
                "3.5");  // domyślnie 2 bary do załączenia WS
            // maksymalne ciśnienie za reduktorem
            //            collectorparameters.MaxPress = 5.0 + 0.001 * ( Random(
            //            50 ) - Random( 50 ) );
            extract_value(
                collectorparameters.MaxPress, "MaxPress", Line, "5.0");
            extract_value(collectorparameters.FakePower, "FakePower", Line, "");
            if (extract_value(
                    collectorparameters.PhysicalLayout, "PhysicalLayout", Line,
                    "3"))
                collectorparameters.CollectorsNo =
                    std::min(collectorparameters.PhysicalLayout, 2);
            break;
        }
        case TPowerSource::PowerCable: {
            Powerparameters.RPowerCable.PowerTrans =
                LoadFIZ_PowerDecode(extract_value(Prefix + "PowerTrans", Line));
            if (Powerparameters.RPowerCable.PowerTrans ==
                TPowerType::SteamPower) {
                extract_value(
                    Powerparameters.RPowerCable.SteamPressure,
                    Prefix + "SteamPress", Line, "");
            }
            break;
        }
    ...
    if ((Powerparameters.SourceType != TPowerSource::Heater) &&
        ...
        extract_value(
            Powerparameters.MaxVoltage, Prefix + "MaxVoltage", Line, "");
        extract_value(
            Powerparameters.MaxCurrent, Prefix + "MaxCurrent", Line, "");
...
TPowerSource TMoverParameters::LoadFIZ_SourceDecode(std::string const &Source) {
    std::map<std::string, TPowerSource> powersources{
        ...
        {"Accu",
         TPowerSource::Accumulator},  // legacy compatibility leftover. TODO:
    ...
    return lookup != powersources.end() ? lookup->second
...
bool TMoverParameters::CheckLocomotiveParameters(bool ReadyFlag, int Dir) {
    ...
    // NOTE: for diesel-powered vehicles we automatically convert legacy "main"
    // power source to more accurate "engine"
    ...
    // WriteLog("aa = " + AxleArangement + " " + std::string( Pos("o",
    // AxleArangement)) );

    if ((contains(AxleArangement, "o")) &&
        (EngineType == TEngineType::ElectricSeriesMotor)) {
    ...
    if (BrakeSystem == TBrakeSystem::Individual)
        if (BrakeSubsystem != TBrakeSubSystem::ss_None) OK = false;  //!

    if ((BrakeVVolume == 0) && (MaxBrakePress[3] > 0) &&
        (BrakeSystem != TBrakeSystem::Individual)) {
        BrakeVVolume = MaxBrakePress[3] / (5.0 - MaxBrakePress[3]) *
                       (BrakeCylRadius * BrakeCylRadius * BrakeCylDist *
                        BrakeCylNo * M_PI) *
                       1000;
    }
    if (BrakeVVolume == 0.0) {
    ...
    switch (BrakeValve) {
        ...
        case TBrakeValve::K: {
            WriteLog("XBT W, K");
            Hamulec = std::make_shared<TWest>(
                MaxBrakePress[3], BrakeCylRadius, BrakeCylDist, BrakeVVolume,
        ...
        case TBrakeValve::ESt4: {
            ...
            Hamulec = std::make_shared<TNESt3>(
                MaxBrakePress[3], BrakeCylRadius, BrakeCylDist, BrakeVVolume,
                BrakeCylNo, BrakeDelays, BrakeMethod, NAxles, NBpA);
            static_cast<TNESt3 *>(Hamulec.get())
                ->SetSize(BrakeValveSize, BrakeValveParams);
            if (MBPM < 2)  // jesli przystawka wazaca
                Hamulec->SetLP(0, MaxBrakePress[3], 0);
            else
                Hamulec->SetLP(Mass, MBPM, MaxBrakePress[1]);
        ...
        case TBrakeValve::EStED: {
            WriteLog("XBT EStED");
            Hamulec = std::make_shared<TEStED>(
                MaxBrakePress[3], BrakeCylRadius, BrakeCylDist, BrakeVVolume,
                BrakeCylNo, BrakeDelays, BrakeMethod, NAxles, NBpA);
            Hamulec->SetRM(RapidMult);
            if (MBPM < 2) {
        ...
        case TBrakeValve::EP2: {
            WriteLog("XBT EP2");
            Hamulec = std::make_shared<TEStEP2>(
                MaxBrakePress[3], BrakeCylRadius, BrakeCylDist, BrakeVVolume,
                BrakeCylNo, BrakeDelays, BrakeMethod, NAxles, NBpA);
            Hamulec->SetLP(Mass, MBPM, MaxBrakePress[1]);
            break;
        ...
        case TBrakeValve::EP1: {
            WriteLog("XBT EP1");
            Hamulec = std::make_shared<TEStEP1>(
                MaxBrakePress[3], BrakeCylRadius, BrakeCylDist, BrakeVVolume,
                BrakeCylNo, BrakeDelays, BrakeMethod, NAxles, NBpA);
            Hamulec->SetLP(Mass, MBPM, MaxBrakePress[1]);
            Hamulec->SetRM(RapidMult);
        ...
        case TBrakeValve::CV1_L_TR: {
            ...
            Hamulec = std::make_shared<TCV1L_TR>(
                MaxBrakePress[3], BrakeCylRadius, BrakeCylDist, BrakeVVolume,
                BrakeCylNo, BrakeDelays, BrakeMethod, NAxles, NBpA);
            break;
        }
        default:
            Hamulec = std::make_shared<TBrake>(
                MaxBrakePress[3], BrakeCylRadius, BrakeCylDist, BrakeVVolume,
                BrakeCylNo, BrakeDelays, BrakeMethod, NAxles, NBpA);
    }
    ...
    Handle->SetParams(
        Handle_AutomaticOverload, Handle_ManualOverload,
        Handle_GenericDoubleParameter1, Handle_GenericDoubleParameter2,
        Handle_OverloadMaxPressure, Handle_OverloadPressureDecrease);
    ...
    if ((true == TestFlag(BrakeDelays, bdelay_G)) &&
        ((false == TestFlag(BrakeDelays, bdelay_R)) ||
         (Power > 1.0)))  // ustalanie srednicy przewodu glownego (lokomotywa
                          // lub napędowy
        Spg = 0.792;
    else
        Spg = 0.507;
    ...
    Pipe2 = std::make_shared<TReservoir>();  // zabezpieczenie, bo sie PG
                                             // wywala... :(
    Pipe->CreateCap(
        (std::max(Dim.L, 14.0) + 0.5) * Spg *
        1);  // dlugosc x przekroj x odejscia i takie tam
    ...
    if (CompressorListPosNo > 0) CompressorListPos = CompressorListDefPos;

    // NOTE: legacy compatibility behaviour for vehicles without defined heating
    // power source
    if ((EnginePowerSource.SourceType == TPowerSource::CurrentCollector) &&
    ...
    if (ReadyFlag)  // gotowy do drogi
    {
        ...
        ScndPipePress =
            (VeselVolume > 0.0 ? CompressedVolume / VeselVolume
             : (Couplers[end::front].AllowedFlag & coupling::mainhose) != 0
                 ? 5.0
             : (Couplers[end::rear].AllowedFlag & coupling::mainhose) != 0
                 ? 5.0
                 : 0.0);
        ...
        */
        DirActive =
            0;  // Dir; //nastawnik kierunkowy - musi być ustawiane osobno!
        DirAbsolute =
            DirActive * CabActive;  // kierunek jazdy względem sprzęgów
    ...
    if (BrakeLocHandle == TBrakeHandle::Knorr) LocalBrakePosA = 0.5;
    ...
    if (LoadFlag > 0) {
        if (LoadAmount < MaxLoad * 0.45) {
            ...
            if (LoadAmount < MaxLoad * 0.35) DecBrakeMult();
        } else {
            IncBrakeMult();  // TODO: przeinesiono do mover.cpp
            if (LoadAmount >= MaxLoad * 0.55) IncBrakeMult();
        }
    }
    ...
    /*
    // disabled, as test mode is used in specific situations and not really a
    default if (BrakeOpModes & bom_PS) BrakeOpModeFlag = bom_PS; else
    */
    BrakeOpModeFlag = bom_PN;

    // yB: jesli pojazdy nie maja zadeklarowanych czasow, to wsadz z przepisow
    // +-16,(6)%
    ...
    */
    ...
    SecuritySystem.MagnetLocation =
        clamp(SecuritySystem.MagnetLocation, 0.0, Dim.L);
...
// *************************************************************************************************
void TMoverParameters::PutCommand(
...
// *************************************************************************************************
bool TMoverParameters::SetInternalCommand(
    std::string NewCommand, double NewValue1, double NewValue2,
    int const Couplertype) {
    bool SIC;
    if ((CommandIn.Command == NewCommand) && (CommandIn.Value1 == NewValue1) &&
        (CommandIn.Value2 == NewValue2) && (CommandIn.Coupling == Couplertype))
        SIC = false;
    else {
        CommandIn.Command = NewCommand;
        CommandIn.Value1 = NewValue1;
...
// *************************************************************************************************
// Q: 20160714
// wysyłanie komendy w kierunku dir (1=przód, -1=tył) do kolejnego pojazdu
// (jednego)
// *************************************************************************************************
bool TMoverParameters::SendCtrlToNext(
    std::string const CtrlCommand, double const ctrlvalue, double const dir,
    int const Couplertype) {
    bool OK;
    int d;  // numer sprzęgu w kierunku którego wysyłamy

    // Ra: był problem z propagacją, jeśli w składzie jest pojazd wstawiony
    // odwrotnie Ra: problem jest również, jeśli AI będzie na końcu składu
    OK = true;  // ( dir != 0 ); // experimentally disabled
    d = (1 + static_cast<int>(Sign(dir))) /
        2;  // dir=-1=>d=0, dir=1=>d=1 - wysyłanie tylko w tył
    if (OK) {
        ...
        if ((Couplers[d].Connected != nullptr) &&
            ...
            if (Couplers[d].ConnectedNr != d) {
                ...
                if (Couplers[d].Connected->SetInternalCommand(
                        CtrlCommand, ctrlvalue, dir, Couplertype))
                    OK =
                        (Couplers[d].Connected->RunInternalCommand() &&
                         OK);  // tu jest rekurencja
            } else {
...
// Q: 20160723
// *************************************************************************************************
// wysłanie komendy otrzymanej z kierunku CValue2 (względem sprzęgów:
// 1=przod,-1=tył) Ra: Jest tu problem z rekurencją. Trzeba by oddzielić
// wykonywanie komend od mechanizmu ich propagacji w składzie. Osobnym problemem
// może być propagacja tylko w jedną stronę. Jeśli jakiś człon jest wstawiony
// odwrotnie, to również odwrotnie musi wykonywać komendy związane z kierunkami
// (PantFront, PantRear, DoorLeft, DoorRight). Komenda musi być zdefiniowana
// tutaj, a jeśli się wywołuje funkcję, to ona nie może sama przesyłać do
// kolejnych pojazdów. Należy też się zastanowić, czy dla uzyskania jakiejś
// zmiany (np. IncMainCtrl) lepiej wywołać funkcję, czy od razu wysłać komendę.
bool TMoverParameters::RunCommand(
    std::string Command, double CValue1, double CValue2,
    int const Couplertype) {
    bool OK{false};
    ...
    else if (Command == "EIMIC")  // ElectricInductionMotor Integrated Control -
                                  // propulsion and brakes in relative values
    {
        eimic_real = CValue1;
        OK = SendCtrlToNext(Command, CValue1, CValue2, Couplertype);
    } else if (Command == "Brake")  // youBy - jak sie EP hamuje, to trza sygnal
                                    // wyslac...
    {
        if (CValue1 < 0.001) DynamicBrakeEMUStatus = true;
        ...
        TUHEX_Active = TUHEX_StageActual > 0;
        if (CValue1 < 0.001) DynamicBrakeEMUStatus = true;
        // fBrakeCtrlPos:=BrakeCtrlPos; //to powinnno być w jednym miejscu,
        // aktualnie w C++!!!
        ...
        */
        ...
        if ((MotorBlowers[end::front].start_type != start_t::manual) &&
            (MotorBlowers[end::front].start_type !=
             start_t::manualwithautofallback)) {
            // automatic device ignores 'manual' state commands
    ...
    else if (Command == "CompressorSwitch") /*NBMX*/
    {
        ...
        CompressorListPos =
            clamp(static_cast<int>(CValue1), 0, CompressorListPosNo);
        OK = SendCtrlToNext(Command, CValue1, CValue2, Couplertype);
    ...
    {  // Ra: uwzględnić trzeba jeszcze zgodność sprzęgów
        ...
        auto swap{
            (false == noswap) &&
            (TestFlag(
                Couplers[(CValue2 == -1 ? end::rear : end::front)].CouplingFlag,
                coupling::control))};
        ...
        OperatePantographsValve(
            static_cast<operation_t>(static_cast<int>(CValue1)),
            range_t::local);
        OK = SendCtrlToNext(Command, CValue1, CValue2, Couplertype);
    ...
    // TODO: CValue1 defines amount to load/unload
        ...
        if ((Vel < 0.1)  // tolerance margin for small vehicle movements in the
                         // consist
            && (MaxLoad > 0) &&
        ...
        if ((Vel < 0.1)  // tolerance margin for small vehicle movements in the
            ...
            && (Distance(Loc, CommandIn.Location, Dim, Dim) <
                (CValue2 > 1.0 ? CValue2 : 10.0))) {  // ten peron
            /*mozna to rozladowac*/
            OK = LoadingDone(
                -1.f * LoadSpeed, ToLower(extract_value("UnLoad", Command)));
        } else {
            // no loading can be done if conditions aren't met
            LastLoadChangeTime = 0.0;
        }
...
// *************************************************************************************************
// Q: 20160714
// Uruchamia funkcję RunCommand aż do skutku. Jeśli będzie pozytywny to kasuje
// komendę.
// *************************************************************************************************
bool TMoverParameters::RunInternalCommand() {
    ...
    if (!CommandIn.Command.empty()) {
        OK = RunCommand(
            CommandIn.Command, CommandIn.Value1, CommandIn.Value2,
            CommandIn.Coupling);
        if (OK) {
...
// *************************************************************************************************
// Q: 20160714
// Zwraca wartość natężenia prądu na wybranym amperomierzu. Podfunkcja do
// ShowCurrent.
// *************************************************************************************************
double TMoverParameters::ShowCurrentP(int AmpN) const {
    ...
    Grupowy =
        ((DelayCtrlFlag) &&
         (TrainType == dt_ET22));      // przerzucanie walu grupowego w ET22;
            ...
            if (TestFlag(Couplers[b].CouplingFlag, coupling::control))
                if (Couplers[b].Connected->Power > 0.01)
                    current = static_cast<int>(
                        Couplers[b].Connected->ShowCurrent(AmpN));
        return current;
    }
}
