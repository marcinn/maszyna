...
3. docisk klockow hamulcowych -hamulec reczny oraz pomocniczy hamulec
pneumatyczny
...
7. hamulec zasadniczy - elektropneumatyczny (impulsowanie), pneumatyczny -
zmiany cisnienia zwiekszenie nacisku przy duzych predkosciach w hamulcach
Oerlikona
...
13. informacja na temat zrodla mocy dla silnikow trakcyjnych, ogrzewania,
oswietlenia
14. dumb - uproszczony model lokomotywy (tylko dla AI)
15. ladunki - ilosc, typ, szybkosc na- i rozladunku, funkcje przetestowac!
16. ulepszony model fizyczny silnika elektrycznego (hamowanie silnikiem, ujemne
napiecia)
...
23. poprawiona szybkosc hamowania pomocniczego i przeciwposlizgowego, odlaczanie
silnikow, odlaczanie bocznikow
24. wprowadzone systemy zabezpieczenia: SHP, Czuwak, sygnalizacja kabinowa (nie
dzialaja w DebugMode).
25. poprawiona predkosc propagacji fali cisnienia (normalizacja przez kwadrat
dlugosci) //to jest zdeka liniowe
...
// static int const LocalBrakePosNo = 10;         /*ilosc nastaw hamulca
// pomocniczego*/ static int const MainBrakeMaxPos = 10;          /*max. ilosc
// nastaw hamulca zasadniczego*/
static int const ManualBrakePosNo = 20; /*ilosc nastaw hamulca recznego*/
static int const LightsSwitchPosNo = 16;
static int const UniversalCtrlArraySize =
    32; /*max liczba pozycji uniwersalnego nastawnika*/
...
/*typ sprzegu*/
static int const ctrain_virtual =
    0;  // gdy pojazdy na tym samym torze się widzą wzajemnie
...
static int const ctrain_scndpneumatic =
    32;  // przewody 8 atm (żółte; zasilanie powietrzem)
static int const ctrain_heating = 64;  // przewody ogrzewania WN
static int const ctrain_depot =
    128;  // nie rozłączalny podczas zwykłych manewrów (międzyczłonowy), we
          // wpisie wartość ujemna
...
static int const dt_SN61 = 0x20;  // nie używane w warunkach, ale ustawiane z
                                  // CHK
...
struct TCommand {
    ...
    double Value2 = 0.0;
    int Coupling{coupling::control};  // coupler flag used to determine command
                                      // propagation
    TLocation Location;
};
...
struct TTrackParam { /*parametry odcinka - szerokosc, tarcie statyczne,
                        kategoria,  obciazalnosc w t/os, uszkodzenia*/
    double Width = 0.0;
    double friction = 0.0;
    int CategoryFlag = 0;
    int QualityFlag = 0;
...
struct TScheme {
    ...
    int ScndAct =
        0; /*jesli ma bocznik w nastawniku, to ktory bocznik na ktorej pozycji*/
};
...
struct TMotorParameters {
    ...
    double mIsat;
    double
        mfi0;  // aproksymacja M(I) silnika} {dla dizla mIsat=przekladnia biegu
    double fi;
    double Isat;
    double fi0;  // aproksymacja E(n)=fi*n}    {dla dizla fi, mfi: predkosci
                 // przelozenia biegu <->
    bool AutoSwitch;
    TMotorParameters() {
        mfi = 0.0;
...
struct TUniversalCtrl {
    ...
    int PrevPosFastDec =
        0; /*poprzednia duza pozycja przy przechodzeniu szybkim*/
};
using TUniversalCtrlTable =
    std::array<TUniversalCtrl, UniversalCtrlArraySize + 1>;
...
class TSecuritySystem {
    ...
    bool pressed = false;
    bool enabled = false;
    bool is_sifa = false;  // Sifa-like pedal device, with inverted input for
                           // convenient keyboard usage
    bool separate_acknowledge =
        false;  // cabsignal reset button is separate from vigilance
...
struct power_coupling {
    ...
    double voltage{0.0};
    bool is_local{
        false};  // whether the power comes from external or onboard source
...
struct TCoupling {
    /*parametry*/
    double SpringKB =
        1.0;            /*stala sprezystosci zderzaka/sprzegu, %tlumiennosci */
    ...
    int AllowedFlag =
        (coupling::coupler | coupling::brakehose);  // Ra: maska dostępnych
    int PowerFlag = (coupling::power110v | coupling::power24v);
    int PowerCoupling =
        coupling::permanent;  // type of coupling required for power transfer
    /*zmienne*/
    bool AutomaticCouplingAllowed{
        true};  // whether automatic coupling can be currently performed
    int CouplingFlag = 0; /*0 - wirtualnie, 1 - sprzegi, 2 - pneumatycznie, 4 -
                             sterowanie, 8 - kabel mocy*/
    class TMoverParameters *Connected = nullptr; /*co jest podlaczone*/
    int ConnectedNr =
        0;  // Ra: od której strony podłączony do (Connected): 0=przód, 1=tył
    ...
    float stretch_duration{0.f};  // seconds, elapsed time with excessive force
                                  // applied to the coupler
    // optional adapter piece
    double adapter_length{0.0};  // meters, value added on the given end to
                                 // standard vehicle (half)length
    double adapter_height{0.0};  // meters, distance from rail level
    TCouplerType adapter_type =
        TCouplerType::NoCoupler;  // CouplerType override if other than
                                  // NoCoupler
    ...
    std::string control_type;  // abstraction of control coupling interface and
    ...
    inline TCouplerType const type() const {
        return (
            adapter_type == TCouplerType::NoCoupler ? CouplerType
                                                    : adapter_type);
    }
};
...
struct neighbour_data {
    ...
    float distance{10000.f};  // distance to the obstacle // NOTE: legacy value.
                              // TBD, TODO: use standard -1 instead?
};
...
class TMoverParameters {  // Ra: wrapper na kod pascalowy, przejmujący jego
                          // funkcje  Q: 20160824 - juz nie wrapper a klasa
                          // bazowa :)
   private:
    // types
    /* TODO: implement
        ...
        // members
            ...
            // boolean pairs, exchanged data is swapped when connected to a
       matching end (front to front or back to back)
            // TBD, TODO: convert to regular bool array for efficiency once it's
       working? std::array<flag_pair, 1> flag_pairs {};
            // integers
    ...
    */
    ...
    struct cooling_fan : public basic_device {
        // config
        float speed{0.f};  // cooling fan rpm; either fraction of parent rpm, or
                           // absolute value if negative
        float sustain_time{
            0.f};  // time of sustaining work of cooling fans after stop
        float min_start_velocity{
            -1.f};  // minimal velocity of vehicle, when cooling fans activate
        // ld outputs
        float revolutions{0.f};  // current fan rpm
        float stop_timer{
            0.f};  // current time, when shut off condition is active
    };

    // basic approximation of a fuel pump
    struct fuel_pump : public basic_device {
        // TODO: fuel consumption, optional automatic engine start after
        // activation
    };
    ...
    struct oil_pump : public basic_device {
        ...
        float pressure_maximum{
            0.65f};  // oil pressure at maximum engine revolutions
        // ld inputs
        float resource_amount{
            1.f};  // amount of affected resource, compared to nominal value
        // internal data
    ...
    struct basic_pantograph {
        ...
        bool is_active{false};  // device is working
        bool sound_event{
            false};  // indicates last state which generated sound event
        double voltage{0.0};
    };
    ...
    struct basic_door {
        ...
        // internal data
        float auto_timer{-1.f};  // delay between activation of open state and
                                 // closing state for automatic doors
        float close_delay{0.f};  // delay between activation of closing state
                                 // and actual closing
        float open_delay{0.f};  // delay between activation of opening state and
                                // actual opening
        float position{
            0.f};  // current shift of the door from the closed position
        float step_position{0.f};  // current shift of the movable step from the
                                   // retracted position
        // ld outputs
        bool is_closed{true};  // the door is fully closed
        bool is_door_closed{
            true};  // the door is fully closed, step doesn't matter
    ...
    struct door_data {
        ...
        float range_out{
            0.f};  // extent of shift outward, applicable for plug doors
        int step_type{2};
        ...
        bool auto_include_remote{
            false};  // automatic door closure applies also to remote control
        bool permit_needed{false};
        std::vector<int>
            permit_presets;  // permit presets selectable with preset switch
        ...
        // vehicle parts
        std::array<basic_door, 2>
            instances;  // door on the right and left side of the vehicle
        // ld outputs
    ...
    struct heat_data {
        ...
        bool auxiliary_water_circuit{
            false};               // cooling system has an extra water circuit
        double fan_speed{0.075};  // cooling fan rpm; either fraction of engine
                                  // rpm, or absolute value if negative
        ...
        struct fluid_circuit_t {
            struct circuit_config_t {
                ...
                bool shutters{
                    false};  // the radiator has shutters to assist the cooling
            } config;
            ...
            bool is_flowing{
                false};  // fluid is being pushed through the circuit
        } water, water_aux, oil;
        ...
        // output, temperatures
        float Te{
            15.0};  // ambient temperature TODO: get it from environment data
        // NOTE: by default the engine is initialized in warm, startup-ready
        // state
    ...
    struct spring_brake {
        ...
        bool PNBrakeConnection{
            false};  // Conf: connection to pneumatic brake cylinders
        double MaxSetPressure{
            0.0};  // Conf: Maximal pressure for switched off brake
        ...
        int MultiTractionCoupler{
            127};  // Conf: Coupling flag necessary for transmitting the command
    };
   ...
   public:
    ...
    int TrainType = 0;    /*typ: EZT/elektrowoz - Winger 040304 Ra: powinno być
                             szybciej niż string*/
    TEngineType EngineType = TEngineType::None; /*typ napedu*/
    TPowerParameters EnginePowerSource;         /*zrodlo mocy dla silnikow*/
    TPowerParameters SystemPowerSource;         /*zrodlo mocy dla systemow
                                                   sterowania/przetwornic/sprezarek*/
    TPowerParameters HeatingPowerSource;        /*zrodlo mocy dla ogrzewania*/
    TPowerParameters
        AlterHeatPowerSource; /*alternatywne zrodlo mocy dla ogrzewania*/
    TPowerParameters LightPowerSource; /*zrodlo mocy dla oswietlenia*/
    TPowerParameters
        AlterLightPowerSource; /*alternatywne mocy dla oswietlenia*/
    double Vmax = -1.0;
    double Mass = 0.0;
    double Power = 0.0; /*max. predkosc kontrukcyjna, masa wlasna, moc*/
    double Mred =
        0.0; /*Ra: zredukowane masy wirujące; potrzebne do obliczeń hamowania*/
    double TotalMass = 0.0; /*wyliczane przez ComputeMass*/
    double HeatingPower = 0.0;
    double EngineHeatingRPM{
        0.0};  // guaranteed engine revolutions with heating enabled
    ...
    bool Signalling =
        false; /*Czy jest zalaczona sygnalizacja hamowania ostatniego wagonu*/
    ...
    TBrakeSystem BrakeSystem =
        TBrakeSystem::Individual; /*rodzaj hamulca zespolonego*/
    ...
    TLocalBrake LocalBrake =
        TLocalBrake::NoBrake;               /*rodzaj hamulca indywidualnego*/
    TBrakePressureTable BrakePressureTable; /*wyszczegolnienie cisnien w rurze*/
    TBrakePressure
        BrakePressureActual;  // wartości ważone dla aktualnej pozycji kranu
    int ASBType =
        0; /*0: brak hamulca przeciwposlizgowego, 1: reczny, 2: automat*/
    int UniversalBrakeButtonFlag[3] = {
        0, 0, 0}; /* mozliwe działania przycisków hamulcowych */
    int UniversalResetButtonFlag[3] = {
        0, 0, 0};  // customizable reset buttons assignments
    int TurboTest = 0;
    ...
    bool Handle_AutomaticOverload =
        false;  // automatyczna asymilacja na pozycji napelniania
    ...
    double Handle_OverloadMaxPressure =
        1.0;  // maksymalne zwiekszenie cisnienia przy asymilacji
    double Handle_OverloadPressureDecrease =
        0.002;  // predkosc spadku cisnienia przy asymilacji
    ...
    /*max. i min. robocze cisnienie w przewodzie glownym oraz roznica miedzy
     * nimi*/
    ...
    /*pojemnosc powietrza w ukladzie hamulcowym, w ukladzie glownej sprezarki
     * [m^3] */
    ...
    double AirLeakRate{0.01};    // base rate of air leak from brake system
                                 // components ( 0.001 = 1 l/sec )
    ...
    int EmergencyBrakeWarningSignal{
        0};  // combined with basic WarningSignal when manual emergency brake is
             // active
    ...
    bool UniCtrlIntegratedBrakePNCtrl =
        false; /*zintegrowany nastawnik JH obsluguje hamulec PN*/
    bool UniCtrlIntegratedBrakeCtrl =
        false; /*zintegrowany nastawnik JH obsluguje hamowanie*/
    bool UniCtrlIntegratedLocalBrakeCtrl =
        false; /*zintegrowany nastawnik JH obsluguje hamowanie hamulcem
                  pomocniczym*/
    int UniCtrlNoPowerPos{
        0};  // cached highesr position not generating traction force
    std::pair<std::string, std::array<int, 2>> PantsPreset{
        "0132",
        {0, 0}};  // pantograph preset switches; .first holds possible setups as
                  // chars, .second holds currently selected preset in each cab

    /*-sekcja parametrow dla lokomotywy elektrycznej*/
    TSchemeTable RList; /*lista rezystorow rozruchowych i polaczen silnikow, dla
                           dizla: napelnienia*/
    int RlistSize = 0;
    ...
    double u =
        0.0;  // wspolczynnik tarcia yB
              // wywalic!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    double CircuitRes = 0.0; /*rezystancje silnika i obwodu*/
    int IminLo = 0;
    int IminHi = 0;  /*prady przelacznika automatycznego rozruchu, uzywane tez
                        przez ai_driver*/
    ...
    double DynamicBrakeRes1 =
        5.8; /*rezystancja oporników przy hamowaniu ED - 1 tryb*/
    double DynamicBrakeRes2 =
        5.8; /*rezystancja oporników przy hamowaniu ED - 2 tryb*/
    ...
    double TUHEX_MinIw = 60;  /*minimalny prąd wzbudzenia przy hamowaniu ED -
                                 wynika z chk silnika*/
    double TUHEX_MaxIw = 400; /*maksymalny prąd wzbudzenia przy hamowaniu ED -
                                 ograniczenie max siły*/
    ...
    double RVentCutOff =
        0.0; /*rezystancja wylaczania wentylatorow dla RVentType=2*/
    ...
    int CompressorPower =
        1;  // 0: main circuit, 1: z przetwornicy, reczne, 2: w przetwornicy,
            // stale, 3: diesel engine, 4: converter of unit in front, 5:
            // converter of unit behind
    ...
    /*                dizel_auto_min, dizel_auto_max: real; {predkosc obrotowa
     * przelaczania automatycznej skrzyni biegow*/
    double dizel_nmax_cutoff =
        0.0; /*predkosc obrotowa zadzialania ogranicznika predkosci*/
    double dizel_nmin = 0.0; /*najmniejsza dopuszczalna predkosc obrotowa*/
    double dizel_nmin_hdrive = 0.0; /*najmniejsza dopuszczalna predkosc obrotowa
                                       w czasie jazdy na hydro */
    double dizel_nmin_hdrive_factor =
        0.0; /*wspolczynnik wzrostu obrotow minimalnych hydro zaleznosci od
                zadanego procentu*/
    double dizel_nmin_retarder =
        0.0; /*obroty pracy podczas hamowania retarderem*/
    double dizel_minVelfullengage =
        0.0; /*najmniejsza predkosc przy jezdzie ze sprzeglem bez poslizgu*/
    double dizel_maxVelANS =
        3.0; /*predkosc progowa rozlaczenia przetwornika momentu*/
    ...
    double dizel_NominalFuelConsumptionRate =
        250.0; /*jednostkowe zużycie paliwa przy mocy nominalnej, wczytywane z
                  fiz, g/kWh*/
    double dizel_FuelConsumption =
        0.0; /*współczynnik zużycia paliwa przeliczony do jednostek maszynowych,
                l/obrót*/
    double dizel_FuelConsumptionActual = 0.0; /*chwilowe spalanie paliwa w l/h*/
    double dizel_FuelConsumptedTotal =
        0.0; /*ilość paliwa zużyta od początku symulacji, l*/
    ...
    double hydro_TC_LockupTorque =
        3000.0; /*moment graniczny sprzegla blokujacego*/
    double hydro_TC_LockupRate =
        1.0; /*szybkosc zalaczania sprzegla blokujacego*/
    double hydro_TC_UnlockRate =
        1.0; /*szybkosc rozlaczania sprzegla blokujacego*/
    ...
    double hydro_TC_TorqueInIn =
        4.5; /*stala momentu proporcjonalnego do kwadratu obrotow wejsciowych*/
    double hydro_TC_TorqueInOut =
        0.0; /*stala momentu proporcjonalnego do roznica obrotow wejsciowych i
                wyjsciowych*/
    double hydro_TC_TorqueOutOut =
        0.0; /*stala momentu proporcjonalnego do kwadratu obrotow wyjsciowych*/
    double hydro_TC_LockupSpeed =
        1.0; /*prog predkosci zalaczania sprzegla blokujacego*/
    double hydro_TC_UnlockSpeed =
        1.0; /*prog predkosci rozlaczania sprzegla blokujacego*/
    std::map<double, double> hydro_TC_Table; /*tablica przetwornika momentu*/
    /*parametry retardera*/
    bool hydro_R = false; /*obecnosc retardera*/
    int hydro_R_Placement =
        0; /*umiejscowienie retardera: 0 - za skrzynia biegow, 1 - miedzy
              przetwornikiem a biegami, 2 - przed skrzynia biegow */
    double hydro_R_TorqueInIn =
        1.0; /*stala momentu proporcjonalnego do kwadratu obrotow wejsciowych*/
    double hydro_R_MaxTorque = 1.0; /*maksymalny moment retardera*/
    double hydro_R_MaxPower =
        1.0; /*maksymalna moc retardera - ogranicza moment*/
    double hydro_R_FillRateInc = 1.0; /*szybkosc napelniania sprzegla*/
    double hydro_R_FillRateDec = 1.0; /*szybkosc oprozniania sprzegla*/
    double hydro_R_MinVel =
        1.0; /*minimalna predkosc, przy ktorej retarder dziala*/
    double hydro_R_EngageVel = 1.0; /*minimalna predkosc hamowania, przy ktorej
                                       sprzeglo jest wciaz wlaczone*/
    bool hydro_R_Clutch = false;    /*czy retarder ma rozłączalne sprzęgło*/
    double hydro_R_ClutchSpeed =
        10.0; /*szybkość narastania obrotów po włączeniu sprzęgła retardera*/
    ...
    int InverterControlCouplerFlag =
        4;  // which coupling flag is necessary to controll inverters
    /* -dla pojazdów z blendingiem EP/ED (MED) */
    double MED_Vmax = 0;  // predkosc maksymalna dla obliczen chwilowej sily
                          // hamowania EP w MED
    double MED_Vmin =
        0;  // predkosc minimalna dla obliczen chwilowej sily hamowania EP w MED
    double MED_Vref = 0;    // predkosc referencyjna dla obliczen dostepnej sily
                            // hamowania EP w MED
    double MED_amax{9.81};  // maksymalne opoznienie hamowania sluzbowego MED
    bool MED_EPVC =
        0;  // czy korekcja sily hamowania EP, gdy nie ma dostepnego ED
    double MED_EPVC_Time =
        7;  // czas korekcji sily hamowania EP, gdy nie ma dostepnego ED
    bool MED_Ncor = 0;  // czy korekcja sily hamowania z uwzglednieniem nacisku
    double MED_MinBrakeReqED = 0;  // minimalne zadanie sily hamowania
                                   // uruchamiajace ED - ponizej tylko EP
    double MED_FrED_factor =
        1;  // mnoznik sily hamowania ED do korekty blendingu
    double MED_ED_Delay1 =
        0;  // opoznienie wdrazania hamowania ED (pierwszy raz)
    double MED_ED_Delay2 =
        0;  // opoznienie zwiekszania sily hamowania ED (kolejne razy)

    int DCEMUED_CC{0};  // na którym sprzęgu sprawdzać działanie ED
    double DCEMUED_EP_max_Vel{
        0.0};  // maksymalna prędkość, przy której działa EP przy włączonym ED w
               // jednostce (dla tocznych)
    double DCEMUED_EP_min_Im{
        0.0};  // minimalny prąd, przy którym EP nie działa przy włączonym ED w
               // członie (dla silnikowych)
    double DCEMUED_EP_delay{0.0};  // opóźnienie włączenia hamulca EP przy
                                   // hamowaniu ED - zwłoka wstępna

    /*-dla wagonow*/
    struct load_attributes {
        std::string name;  // name of the cargo
        float offset_min{
            0.f};  // offset applied to cargo model when load amount is 0
    ...
    int DoorOpenCtrl = 0;
    int DoorCloseCtrl =
        0; /*0: przez pasazera, 1: przez maszyniste, 2: samoczynne (zamykanie)*/
    ...
    bool DoorClosureWarningAuto =
        false;  // departure signal plays automatically while door closing
                // button is held down
    ...
    int DoorOpenMethod =
        2; /*sposob otwarcia - 1: przesuwne, 2: obrotowe, 3: trójelementowe*/
    float DoorCloseDelay{0.f};    // delay (in seconds) before the door begin
                                  // closing, once conditions to close are met
    ...
    bool MirrorForbidden{
        false}; /*czy jest pozwolenie na otworzenie lusterek (przycisk)*/
    ...
    double SpeedCtrlButtons[10]{30, 40, 50,  60,  70,
                                80, 90, 100, 110, 120}; /*przyciski prędkości*/
    double SpeedCtrlDelay =
        2; /*opoznienie dzialania tempomatu z wybieralna predkoscia*/
    double SpeedCtrlValue = 0; /*wybrana predkosc jazdy na tempomacie*/
    /*--sekcja zmiennych*/
    /*--opis konkretnego egzemplarza taboru*/
    TLocation Loc{
        0.0, 0.0,
        0.0};  // pozycja pojazdów do wyznaczenia odległości pomiędzy sprzęgami
    TRotation Rot{0.0, 0.0, 0.0};
    glm::vec3 Front{};
    std::string Name;       /*nazwa wlasna*/
    TCoupling Couplers[2];  // urzadzenia zderzno-sprzegowe, polaczenia miedzy
                            // wagonami
    ...
    std::pair<double, double>
        EnergyMeter;  // energy <drawn, returned> from grid [kWh]
    double V = 0.0;  // predkosc w [m/s] względem sprzęgów (dodania gdy jedzie w
                     // stronę 0)
    double Vel = 0.0;  // moduł prędkości w [km/h], używany przez AI
    double AccS =
        0.0;  // efektywne przyspieszenie styczne w [m/s^2] (wszystkie siły)
    double AccSVBased{};   // tangential acceleration calculated from velocity
                           // change
    ...
    double UnitBrakeForce =
        0.0;             /*!s siła hamowania przypadająca na jeden element*/
    ...
    double BrakeSlippingTimer =
        0.0; /*pomocnicza zmienna do wylaczania przeciwposlizgu*/
    double dpBrake = 0.0;
    double dpPipe = 0.0;
    double dpMainValve = 0.0;
    double dpLocalValve = 0.0;
    double EmergencyValveFlow =
        0.0;  // air flow through alerter valve during last simulation step
    ...
    double LocBrakePress =
        0.0; /*!o cisnienie w cylindrach hamulcowych z pomocniczego*/
    double PipeBrakePress =
        0.0;                /*!o cisnienie w cylindrach hamulcowych z przewodu*/
    ...
    bool CompressorAllow =
        false; /*! zezwolenie na uruchomienie sprezarki  NBMX*/
    bool CompressorAllowLocal{
        true};  // local device state override (most units don't have this
                // fitted so it's set to true not to intefere)
    bool CompressorGovernorLock{
        false};  // indicates whether compressor pressure switch was activated
                 // due to reaching cut-out pressure
    bool CompressorTankValve{
        false};  // indicates excessive pressure is vented from compressor tank
                 // directly and instantly
    start_t CompressorStart{
        start_t::manual};  // whether the compressor is started manually, or
                           // another way
    ...
    start_t ConverterStart{start_t::manual};  // whether converter is started
                                              // manually, or by other means
    float ConverterStartDelay{
        0.0f};  // delay (in seconds) before the converter is started, once its
                // activation conditions are met
    double ConverterStartDelayTimer{
        0.0};  // helper, for tracking whether converter start delay passed
    bool ConverterAllow = false; /*zezwolenie na prace przetwornicy NBMX*/
    bool ConverterAllowLocal{
        true};  // local device state override (most units don't have this
                // fitted so it's set to true not to intefere)
    bool ConverterFlag = false; /*!  czy wlaczona przetwornica NBMX*/
    start_t ConverterOverloadRelayStart{
        start_t::manual};  // whether overload relay reset responds to dedicated
                           // button
    ...
    bool WaterCircuitsLink{
        false};  // optional connection between water circuits
    heat_data dizel_heat;
    std::array<cooling_fan, 2> MotorBlowers;
    door_data Doors;
    float DoorsOpenWithPermitAfter{
        -1.f};  // remote open if permit button is held for specified time.
                // NOTE: separate from door data as its cab control thing
    int DoorsPermitLightBlinking{
        0};  // when the doors permit signal light is blinking

    int BrakeCtrlPos = -2; /*nastawa hamulca zespolonego*/
    double BrakeCtrlPosR =
        0.0; /*nastawa hamulca zespolonego - plynna dla FV4a*/
    double BrakeCtrlPos2 =
        0.0;                /*nastawa hamulca zespolonego - kapturek dla FV4a*/
    int ManualBrakePos = 0; /*nastawa hamulca recznego*/
    double LocalBrakePosA = 0.0; /*nastawa hamulca pomocniczego*/
    double LocalBrakePosAEIM =
        0.0; /*pozycja hamulca pomocniczego ep dla asynchronicznych ezt*/
    bool UniversalBrakeButtonActive[3] = {
        false, false, false};     /* brake button pressed */
                                  /*
                                          int BrakeStatus = b_off; //0 - odham, 1 - ham., 2 - uszk., 4 -
                                     odluzniacz, 8 - antyposlizg, 16 - uzyte EP, 32 - pozycja R, 64 - powrot z
                                     R
                                  */
    ...
    int BrakeDelayFlag =
        0;               /*nastawa opoznienia ham. osob/towar/posp/exp 0/1/2/4*/
    ...
    bool TUHEX_ResChange =
        false; /*czy zmiana rezystancji hamowania ED - odwzbudzanie*/
    ...
    // EndSignalsFlag: byte;  {ABu 060205: zmiany - koncowki: 1/16 - swiatla
    // prz/tyl, 2/31 - blachy prz/tyl} HeadSignalsFlag: byte; {ABu 060205:
    // zmiany - swiatla: 1/2/4 - przod, 16/32/63 - tyl}
    TCommand CommandIn;
    ...
    double ValueOut =
        0.0; /*argument komendy która ma byc przekazana na zewnatrz*/
    ...
    double MainsInitTime{
        0.0};  // config, initialization time (in seconds) of the main circuit
               // after it receives power, before it can be closed
    double MainsInitTimeCountdown{
        0.0};  // current state of main circuit initialization, remaining time
               // (in seconds) until it's ready
    start_t MainsStart{start_t::manual};
    bool LineBreakerClosesOnlyAtNoPowerPos{false};
    bool ControlPressureSwitch{
        false};  // activates if the main pipe and/or brake cylinder pressure
                 // aren't within operational levels
    ...
    int CompressorListPos =
        0; /*polozenie przelacznika wielopozycyjnego sprezarek*/
    int DirActive =
        0;  // czy lok. jest wlaczona i w ktorym kierunku: względem wybranej
            // kabiny: -1 - do tylu, +1 - do przodu, 0 - wylaczona
    int DirAbsolute = 0;  // zadany kierunek jazdy względem sprzęgów (1=w strone
                          // 0,-1=w stronę 1)
    int MainCtrlMaxDirChangePos{0};  // can't change reverser state with master
                                     // controller set above this position
    int CabActive = 0;  // numer kabiny, z której jest sterowanie: 1 lub -1; w
                        // przeciwnym razie brak sterowania - rozrzad
    int CabOccupied = 0;  // numer kabiny, w ktorej jest obsada (zwykle jedna na
                          // skład) // TODO: move to TController
    ...
    bool AutomaticCabActivation =
        true;  // czy zmostkowany rozrzad przelacza sie sam przy zmianie kabiny
    int InactiveCabFlag = 0;  // co sie dzieje przy dezaktywacji kabiny
    bool InactiveCabPantsCheck =
        false;                      // niech DynamicObject sprawdzi pantografy
    ...
    TTractionParam
        RunningTraction; /*parametry sieci trakcyjnej najblizej lokomotywy*/
    ...
    int Imin = 0;
    int Imax =
        0; /*prad przelaczania automatycznego rozruchu, prad bezpiecznika*/
    ...
    bool GroundRelay{
        true};  // switches off to protect against damage from earths
    start_t GroundRelayStart{start_t::manual};  // relay activation method
    bool StLinFlag = false;                     /*!o styczniki liniowe*/
    bool StLinSwitchOff{
        false};  // state of the button forcing motor connectors open
    ...
    bool PantPressSwitchActive{
        false};  // state of the pantograph pressure switch. gets primed at
                 // defined pressure level in pantograph air system
    bool PantPressLockActive{
        false};  // pwr system state flag. fires when pressure switch activates
                 // by pantograph pressure dropping below defined level
    bool NoVoltRelay{
        true};  // switches off if the power level drops below threshold
    bool OvervoltageRelay{
        true};  // switches off if the power level goes above threshold
    bool s_CAtestebrake = false;  // hunter-091012: zmienna dla testu ca
    std::array<std::pair<double, double>, 4>
        PowerCircuits;  // 24v, 110v, 3x400v and 3000v power circuits, voltage
                        // from local sources and current draw pairs

    /*-zmienne dla lokomotywy spalinowej z przekladnia mechaniczna*/
    double dizel_fill = 0.0; /*napelnienie*/
    double dizel_engagestate =
        0.0; /*sprzeglo skrzyni biegow: 0 - luz, 1 - wlaczone, 0.5 - wlaczone
                50% (z poslizgiem)*/
    double dizel_engage = 0.0; /*sprzeglo skrzyni biegow: aktualny docisk*/
    double dizel_automaticgearstatus =
        0.0; /*0 - bez zmiany, -1 zmiana na nizszy +1 zmiana na wyzszy*/
    ...
    double dizel_engagedeltaomega =
        0.0;                   /*roznica predkosci katowych tarcz sprzegla*/
    ...
    double dizel_nreg_min =
        0.0; /*predkosc regulatora minimalna, zmienna w hydro*/
    ...
    double eimic =
        0; /*aktualna pozycja zintegrowanego sterowania jazda i hamowaniem*/
    double eimic_analog = 0; /*pozycja zadajnika analogowa*/
    double eimic_real = 0;   /*faktycznie uzywana pozycja zintegrowanego
                                sterowania jazda i hamowaniem*/
    double eim_localbrake =
        0; /*nastawa hamowania dodatkowego pneumatycznego lokomotywy*/
    ...
    bool EIMCtrlAdditionalZeros =
        false; /*czy ma dodatkowe zero jazdy i zero hamowania */
    bool EIMCtrlEmergency =
        false; /*czy ma dodatkowe zero jazdy i zero hamowania */
    double eimv_pr =
        0; /*realizowany procent dostepnej sily rozruchu/hamowania*/
    double eimv[21];
    static std::vector<std::string> const eimv_labels;
    double SpeedCtrlTimer =
        0; /*zegar dzialania tempomatu z wybieralna predkoscia*/
    ...
    double MED_EPVC_CurrentTime =
        0; /*aktualny czas licznika czasu korekcji siły EP*/
    double MED_ED_DelayTimer =
        0; /*aktualny czas licznika opoznienia hamowania ED*/
    ...
    double DoorLeftOpenTimer{
        -1.0};  // left door closing timer for automatic door type
    bool DoorRightOpened = false;
    double DoorRightOpenTimer{
        -1.0};  // right door closing timer for automatic door type
#endif
    ...
    bool HeatingAllow{
        false};        // heating switch // TODO: wrap heating in a basic device
    ...
    bool bPantKurek3 =
        true;  // kurek trójdrogowy (pantografu): true=połączenie z ZG,
               // false=połączenie z małą sprężarką // domyślnie zbiornik
               // pantografu połączony jest ze zbiornikiem głównym
    bool PantAutoValve{false};  // type of installed pantograph compressor valve
    int iProblem = 0;  // flagi problemów z taborem, aby AI nie musiało
                       // porównywać; 0=może jechać
    int iLights[2];  // bity zapalonych świateł tutaj, żeby dało się liczyć
                     // pobór prądu

    plc::basic_controller m_plc;

    int AIHintPantstate{0};  // suggested pantograph setup
    bool AIHintPantUpIfIdle{
        true};  // whether raise both pantographs if idling for a while
    double AIHintLocalBrakeAccFactor{
        1.05};  // suggested acceleration weight for local brake operation

   public:
    TMoverParameters(
        double VelInitial, std::string TypeNameInit, std::string NameInit,
        int Cab);
    // obsługa sprzęgów
    static double CouplerDist(
        TMoverParameters const *Left, TMoverParameters const *Right);
    ...
    bool Attach(
        ...
        int CouplingType, bool Enforce = false, bool Audible = true);
    ...
    double FastComputeMovement(
        ...
        TLocation const &NewLoc,
        TRotation const
            &NewRot);  // oblicza przesuniecie pojazdu - wersja zoptymalizowana
    void compute_movement_(double const Deltatime);
    double ShowEngineRotation(int VehN);

    // Q
    // *******************************************************************************************
    double GetTrainsetVoltage(
        int const Coupling = (coupling::heating | coupling::highvoltage)) const;
    ...
    double EngineRPMRatio()
        const;  // returns current engine revolutions as percentage of max
                // engine revolutions, in range 0-1
    ...
    bool IsMainCtrlActualNoPowerPos()
        const;  // whether the master controller is actually set to position
                // which won't generate any extra power
    bool IsMainCtrlNoPowerPos()
        const;  // whether the master controller is set to position which won't
                // generate any extra power
    bool IsMainCtrlMaxPowerPos() const;
    int MainCtrlNoPowerPos()
        const;  // highest setting of master controller which won't cause engine
                // to generate extra power
    int MainCtrlActualPowerPos()
        const;  // current actual setting of master controller, relative to the
                // highest setting not generating extra power
    int MainCtrlPowerPos()
        const;  // current setting of master controller, relative to the highest
    ...
    bool SandboxManual(
        bool const State,
        range_t const Notify =
            range_t::consist); /*wlacza/wylacza reczne sypanie piasku*/
    bool SandboxAuto(
        bool const State,
        range_t const Notify =
            range_t::consist); /*wlacza/wylacza automatyczne sypanie piasku*/
    bool Sandbox(
        bool const State,
        range_t const Notify =
            range_t::consist);               /*wlacza/wylacza sypanie piasku*/
    bool SandboxAutoAllow(bool const State); /*wlacza/wylacza zezwolenie na
                                                automatyczne sypanie piasku*/
    ...
    bool UniversalBrakeButton(
        int button, int state); /*uniwersalny przycisk hamulca*/
    bool SwitchEPBrake(int state);
    bool AntiSlippingButton(
        void); /*! reczny wlacznik urzadzen antyposlizgowych*/
    ...
    void CompressorCheck(
        double dt); /*wlacza, wylacza kompresor, laduje zbiornik*/
    ...
    void ComputeConstans(
        void);  // ABu: wczesniejsze wyznaczenie stalych dla liczenia sil
    ...
    bool WaterPumpBreakerSwitch(
        bool State, range_t const Notify =
                        range_t::consist);  // water pump breaker state toggle
    bool WaterPumpSwitch(
        bool State,
        range_t const Notify = range_t::consist);  // water pump state toggle
    bool WaterPumpSwitchOff(
        bool State,
        range_t const Notify = range_t::consist);  // water pump state toggle
    bool WaterHeaterBreakerSwitch(
        bool State, range_t const Notify =
                        range_t::consist);  // water heater breaker state toggle
    bool WaterHeaterSwitch(
        bool State,
        range_t const Notify = range_t::consist);  // water heater state toggle
    bool WaterCircuitsLinkSwitch(
        bool State, range_t const Notify =
                        range_t::consist);  // water circuits link state toggle
    bool FuelPumpSwitch(
        bool State,
        range_t const Notify = range_t::consist);  // fuel pump state toggle
    bool FuelPumpSwitchOff(
        bool State,
        range_t const Notify = range_t::consist);  // fuel pump state toggle
    bool OilPumpSwitch(
        bool State,
        range_t const Notify = range_t::consist);  // oil pump state toggle
    bool OilPumpSwitchOff(
        bool State,
        range_t const Notify = range_t::consist);  // oil pump state toggle
    bool MotorBlowersSwitch(
        bool State, end const Side,
        range_t const Notify =
            range_t::consist);  // traction motor fan state toggle
    ...
    bool CompartmentLightsSwitch(
        bool State, range_t const Notify =
                        range_t::consist);  // compartment lights state toggle
    bool CompartmentLightsSwitchOff(
        bool State, range_t const Notify =
                        range_t::consist);  // compartment lights state toggle
    bool MainSwitch(
        bool const State,
        range_t const Notify = range_t::consist); /*! wylacznik glowny*/
    void MainSwitch_(bool const State);
    bool MainSwitchCheck()
        const;  // checks conditions for closing the line breaker
    bool ConverterSwitch(
        bool State,
        range_t const Notify = range_t::consist); /*! wl/wyl przetwornicy*/
    bool CompressorSwitch(
        bool State,
        range_t const Notify = range_t::consist); /*! wl/wyl sprezarki*/
    bool ChangeCompressorPreset(
        int const Change, range_t const Notify = range_t::consist);
    bool HeatingSwitch(
        bool const State, range_t const Notify = range_t::consist);
    void HeatingSwitch_(bool const State);
    double EnginePowerSourceVoltage()
        const;  // returns voltage of defined main engine power source
    ...
    bool FuseOn(
        range_t const Notify = range_t::consist);  // bezpiecznik nadamiary
    bool FuseFlagCheck(void) const;  // sprawdzanie flagi nadmiarowego
    void FuseOff(void);              // wylaczenie nadmiarowego
    bool UniversalResetButton(
        int const Button, range_t const Notify = range_t::consist);
    bool RelayReset(
        int const Relays,
        range_t const Notify = range_t::consist);  // resets specified relays
    double ShowCurrent(int AmpN)
        const;  // pokazuje bezwgl. wartosc pradu na wybranym amperomierzu
    double ShowCurrentP(int AmpN) const;  // pokazuje bezwgl. wartosc pradu w
                                          // wybranym pojezdzie //Q 20160722

    /*!o pokazuje bezwgl. wartosc obrotow na obrotomierzu jednego z 3 pojazdow*/
    /*function ShowEngineRotation(VehN:int): integer; //Ra 2014-06: przeniesione
     * do C++*/
    /*funkcje uzalezniajace sile pociagowa od predkosci: v2n, n2r, current,
     * momentum*/
    double v2n(void);
    ...
    bool MaxCurrentSwitch(
        bool State,
        range_t const Notify =
            range_t::consist);  // przelacznik pradu wysokiego rozruchu
    bool MinCurrentSwitch(
        bool State);  // przelacznik pradu automatycznego rozruchu
    ...
    bool OperatePantographsValve(
        operation_t const State, range_t const Notify = range_t::consist);
    bool OperatePantographValve(
        end const End, operation_t const State,
        range_t const Notify = range_t::consist);
    bool DropAllPantographs(
        bool const State, range_t const Notify = range_t::consist);

    void CheckEIMIC(double dt);  // sprawdzenie i zmiana nastawy zintegrowanego
                                 // nastawnika jazdy/hamowania
    ...
    double dizel_MomentumRetarder(
        double n, double dt);  // moment hamowania retardera
    ...
    bool PermitDoors(
        side const Door, bool const State = true,
        range_t const Notify = range_t::consist);
    void PermitDoors_(side const Door, bool const State = true);
    bool ChangeDoorPermitPreset(
        int const Change, range_t const Notify = range_t::consist);
    bool PermitDoorStep(
        bool const State, range_t const Notify = range_t::consist);
    bool ChangeDoorControlMode(
        bool const State, range_t const Notify = range_t::consist);
    bool OperateDoors(
        side const Door, bool const State,
        range_t const Notify = range_t::consist);
    ...
    bool LoadFIZ(std::string chkpath);  // Q 20160717    bool
                                        // LoadChkFile(std::string chkpath);
...
// double Distance(TLocation Loc1, TLocation Loc2, TDimension Dim1, TDimension
// Dim2);
