...
static int const LocalBrakePosNo =
    10; /*ilosc nastaw hamulca recznego lub pomocniczego*/
static int const MainBrakeMaxPos =
    10; /*max. ilosc nastaw hamulca zasadniczego*/
...
static int const bh_MB =
    5;  // odciecie - utrzymanie stopnia hamowania/pierwszy 1 stopien hamowania
...
static int const bh_EPN =
    9;  // ep - utrzymanie //jesli rowne luzowaniu, wtedy sterowanie przyciskiem
static int const bh_EPB =
    10;  // ep - hamowanie  //pelne hamowanie dla ep kątowego

static double const SpgD = 0.7917;
static double const SpO = 0.5067;  // przekroj przewodu 1" w l/m
                                   // wyj: jednostka dosyc dziwna, ale wszystkie
                                   // obliczenia i pojemnosci sa podane w
                                   // litrach (rozsadne wielkosci) zas dlugosc
                                   // pojazdow jest podana w metrach a predkosc
                                   // przeplywu w m/s //3.5 7//1.5
//   BPT: array[-2..6] of array [0..1] of real= ((0, 5.0), (14, 5.4), (9, 5.0),
//   (6, 4.6), (9, 4.5), (9, 4.0), (9, 3.5), (9, 2.8), (34, 2.8)); BPT:
//   array[-2..6] of array [0..1] of real= ((0, 5.0), (7, 5.0), (2.0, 5.0),
//   (4.5, 4.6), (4.5, 4.2), (4.5, 3.8), (4.5, 3.4), (4.5, 2.8), (8, 2.8));
static double const BPT[9][2] = {{0, 5.0},   {7, 5.0},   {2.0, 5.0},
                                 {4.5, 4.6}, {4.5, 4.2}, {4.5, 3.8},
                                 {4.5, 3.4}, {4.5, 2.8}, {8, 2.8}};
static double const BPT_394[7][2] = {{13, 10.0}, {5, 5.0}, {0, -1},  {5, -1},
                                     {5, 0.0},   {5, 0.0}, {18, 0.0}};
// double *BPT = zero_based_BPT[2]; //tablica pozycji hamulca dla zakresu -2..6
// double *BPT_394 = zero_based_BPT_394[1]; //tablica pozycji hamulca dla
// zakresu -1..5
//    BPT: array[-2..6] of array [0..1] of real= ((0, 5.0), (12, 5.4), (9, 5.0),
//    (9, 4.6), (9, 4.2), (9, 3.8), (9, 3.4), (9, 2.8), (34, 2.8));
//       BPT: array[-2..6] of array [0..1] of real= ((0, 0),(0, 0),(0, 0),(0,
...
{                       // kolejne flagi
    ...
    ub_UnlockPipe =
        0x02,  // odblok PG / mostkowanie hamulca bezpieczeństwa - POJAZD
    ub_HighPressure = 0x04,  // impuls wysokiego ciśnienia - ZM
    ub_Overload =
        0x08,  // przycisk asymilacji / kontrolowanego przeładowania - ZM
...
class TBrake {
   ...
   public:
    TBrake(
        ...
        int i_BD, int i_mat, int i_ba, int i_nbpa);
    // maksymalne cisnienie, promien, skok roboczy, pojemnosc ZP, ilosc
    // cylindrow, opoznienia hamulca, material klockow, osie hamowane, klocki na
    // os;
    virtual void Init(
        double const PP, double const HPP, double const LPP, double const BP,
        int const BDF);  // inicjalizacja hamulca

    double GetFC(
        double const Vel,
        double const N);  // wspolczynnik tarcia - hamulec wie lepiej
    virtual double GetPF(
        double const PP, double const dt,
        double const Vel);  // przeplyw miedzy komora wstepna i PG
    ...
    virtual void PLC(
        double const mass){};  // wspolczynnik cisnienia przystawki wazacej
...
class TEStED : public TLSt {  // zawor z EP09 - Est4 z oddzielnym
                              // przekladnikiem, kontrola rapidu i takie tam

   private:
    std::shared_ptr<TReservoir>
        Miedzypoj;       // pojemnosc posrednia (urojona) do napelniania ZP i ZS
...
class TEStEP1 : public TEStEP2 {
   public:
    ...
    inline TEStEP1(
        ...
        int i_BD, int i_mat, int i_ba, int i_nbpa)
        : TEStEP2(
              i_mbp, i_bcr, i_bcd, i_brc, i_bcn, i_BD, i_mat, i_ba, i_nbpa) {}
};
...
class TDriverHandle {
   ...
   public:
    ...
    virtual void SetReductor(
        double nAdj);                // korekcja pozycji reduktora cisnienia
    virtual double GetSound(int i);  // pobranie glosnosci wybranego dzwieku
    virtual double GetPos(
        int i);  // pobranie numeru pozycji o zadanym kodzie (funkcji)
    virtual double GetEP(double pos);  // pobranie sily hamulca ep
    virtual void SetParams(
        bool AO, bool MO, double, double, double OMP,
        double OPD){};  // ustawianie jakichs parametrow dla zaworu
    virtual void OvrldButton(
        bool Active);  // przycisk recznego przeladowania/asymilacji
...
class TFV4aM : public TDriverHandle {
   private:
    ...
    double TP = 0.0;  // zbiornik czasowy
    double RP = 0.0;  // zbiornik redukcyjny
    double XP = 0.0;  // komora powietrzna w reduktorze — jest potrzebna do
                      // odwzorowania fali
    double RedAdj =
        0.0;  // dostosowanie reduktora cisnienia (krecenie kapturkiem)
    //        Sounds: array[0..4] of real;       //wielkosci przeplywow dla
    //        dzwiekow
    bool Fala = false;
    static double const
        pos_table[11];  // = { -2, 6, -1, 0, -2, 1, 4, 6, 0, 0, 0 };
...
class TMHZ_EN57 : public TDriverHandle {
   private:
    ...
    double OverloadMaxPressure =
        1.0;  // maksymalne zwiekszenie cisnienia przy asymilacji
    double OverloadPressureDecrease =
        0.045;  // predkosc spadku cisnienia przy asymilacji
    static double const
        pos_table[11];  //= { -2, 10, -1, 0, 0, 2, 9, 10, 0, 0, 0 };
...
class TMHZ_K5P : public TDriverHandle {
   private:
    ...
    double FillingStrokeFactor =
        1.0;  // mnożnik otwarcia zaworu przy uderzeniowym (bez fali)
    static double const
        pos_table[11];  //= { -2, 10, -1, 0, 0, 2, 9, 10, 0, 0, 0 };
...
/*    FBS2= class(TTDriverHandle)
          private
                        CP, TP, RP: real;      //zbiornik sterujący, czasowy,
redukcyjny XP: real;              //komora powietrzna w reduktorze — jest
potrzebna do odwzorowania fali RedAdj: real;          //dostosowanie reduktora
cisnienia (krecenie kapturkiem)
//        Sounds: array[0..4] of real;       //wielkosci przeplywow dla dzwiekow
                Fala: boolean;
          public
                function GetPF(i_bcp:real; pp, hp, dt, ep: real): real;
override; procedure Init(press: real); override; procedure SetReductor(nAdj:
real); override; function GetSound(i: int): real; override; function GetPos(i:
int): real; override; end;                    */

/*    TD2= class(TTDriverHandle)
              private
                                  CP, TP, RP: real;      //zbiornik sterujący,
czasowy, redukcyjny XP: real;              //komora powietrzna w reduktorze —
jest potrzebna do odwzorowania fali RedAdj: real;          //dostosowanie
reduktora cisnienia (krecenie kapturkiem)
//        Sounds: array[0..4] of real;       //wielkosci przeplywow dla dzwiekow
                Fala: boolean;
              public
                function GetPF(i_bcp:real; pp, hp, dt, ep: real): real;
...
class TM394 : public TDriverHandle {
   private:
    ...
    double RedAdj =
        0.0;  // dostosowanie reduktora cisnienia (krecenie kapturkiem)
    static double const
        pos_table[11];  // = { -1, 5, -1, 0, 1, 2, 4, 5, 0, 0, 0 };
...
class TH14K1 : public TDriverHandle {
   private:
    ...
    static double const
        pos_table[11];  // = {-1, 4, -1, 0, 1, 2, 3, 4, 0, 0, 0};
...
class TFVel6 : public TDriverHandle {
   private:
    double EPS = 0.0;
    static double const
        pos_table[11];  // = {-1, 6, -1, 0, 6, 4, 4.7, 5, -1, 0, 1};
    double CP = 0.0;
...
extern double PF(
    double const P1, double const P2, double const S, double const DP = 0.25);
extern double PF1(double const P1, double const P2, double const S);

extern double PFVa(
    double PH, double PL, double const S, double LIM,
    double const DP = 0.1);  // zawor napelniajacy z PH do PL, PL do LIM
extern double PFVd(
    double PH, double PL, double const S, double LIM,
    double const DP = 0.1);  // zawor wypuszczajacy z PH do PL, PH do LIM
