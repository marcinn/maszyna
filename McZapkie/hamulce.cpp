...
double const TH14K1::BPT_K[6][2] = {{10, 0}, {4, 1},  {0, 1},
                                    {4, 0},  {4, -1}, {15, -1}};
double const TH14K1::pos_table[11] = {-1, 4, -1, 0, 1, 2, 3, 4, 0, 0, 0};
double const TSt113::BPT_K[6][2] = {{10, 0}, {4, 1},  {0, 1},
                                    {4, 0},  {4, -1}, {15, -1}};
...
double PF_old(double P1, double P2, double S) {
    ...
    if (PH - PL < 0.0001)
        ...
        return 20 * (PH - PL) * (PH + 1) * 222 * S * (P2 - P1) /
               (1.13 * PH - PL);
    else
...
double PF(double const P1, double const P2, double const S, double const DP) {
    ...
    double const FM =
        PH * 197.0 * S *
        Sign(P2 - P1);       // najwyzszy mozliwy przeplyw, wraz z kierunkiem
...
// zawor napelniajacy z PH do PL, PL do LIM
{
    if (LIM > PL) {
        ...
        double sg = std::min(
            1.0, PL / PH);  // bezwymiarowy stosunek cisnien. NOTE: sg is capped
                            // at 1 to prevent calculations from going awry.
                            // TODO, TBD: log these as errors?
        double FM =
            PH * 197 * S;  // najwyzszy mozliwy przeplyw, wraz z kierunkiem
        if ((LIM - PL) < DP)
            FM = FM * (LIM - PL) /
                 DP;  // jesli jestesmy przy nastawieniu, to zawor sie przymyka
        if ((sg > 0.5))           // jesli ponizej stosunku krytycznego
            if ((PH - PL) < DPL)  // niewielka roznica cisnien
                return (PH - PL) / DPL * FM * 2 *
                       std::sqrt((sg) * (1 - sg));  // BUG: (1-sg) can be < 0,
                                                    // leading to sqrt(-x)
            else
                return FM * 2 *
                       std::sqrt((sg) * (1 - sg));  // BUG: (1-sg) can be < 0,
...
// zawor wypuszczajacy z PH do PL, PH do LIM
{
    if (LIM < PH) {
        ...
        double FM =
            PH * 197.0 * S;  // najwyzszy mozliwy przeplyw, wraz z kierunkiem
        if ((PH - LIM) < 0.1)
            FM = FM * (PH - LIM) /
                 DP;  // jesli jestesmy przy nastawieniu, to zawor sie przymyka
        if ((sg > 0.5))           // jesli ponizej stosunku krytycznego
            if ((PH - PL) < DPL)  // niewielka roznica cisnien
                return (PH - PL) / DPL * FM * 2.0 *
                       std::sqrt((sg) * (1.0 - sg));
            else
                return FM * 2.0 * std::sqrt((sg) * (1.0 - sg));
        else  // powyzej stosunku krytycznego
...
void TReservoir::Flow(double dv) { dVol = dVol + dv; }
...
void TReservoir::CreateCap(double Capacity) { Cap = Capacity; }
...
double TBrakeCyl::P() {
    ...
    double VtoC =
        (Cap > 0.0 ? Vol / Cap : 0.0);  // stosunek cisnienia do objetosci.
                                        // Added div/0 trap for vehicles with
                                        // incomplete definitions (cars etc)
    //  P:=VtoC;
    if (VtoC < VS)
...
//---HAMULEC---
/*
constructor TBrake.Create(i_mbp, i_bcr, i_bcd, i_brc: real; i_bcn, i_BD, i_mat,
i_ba, i_nbpa: int); begin inherited Create; MaxBP:=i_mbp; BCN:=i_bcn;
...
TBrake::TBrake(
    double i_mbp, double i_bcr, double i_bcd, double i_brc, int i_bcn, int i_BD,
    ...
    // 210.88
    //  SizeBR:=i_bcn*i_bcr*i_bcr*i_bcd*40.17*MaxBP/(5-MaxBP);  //objetosc ZP w
    //  stosunku do cylindra 14" i cisnienia 4.2 atm
    SizeBR = i_brc * 0.0128;
...
double TBrake::GetBCF() { return BCA * 100 * BrakeCyl->P(); }
...
void TBrake::ASB(int const state) {  // 255-b_asb(32)
    BrakeStatus = (BrakeStatus & ~b_asb) | ((state / 2) * b_asb);
    BrakeStatus =
        (BrakeStatus & ~b_asb_unbrake) | ((state % 2) * b_asb_unbrake);
}

int TBrake::GetStatus() { return BrakeStatus; }
...
void TBrake::ForceLeak(double const Amount) {
    ...
    ValveRes->Flow(
        -Amount * ValveRes->P() * 0.01);  // this reservoir has hard coded, tiny
                                          // capacity compared to other parts
...
double TWest::GetPF(double const PP, double const dt, double const Vel) {
    ...
    temp = BVP * int(EPS > 0);
    dv =
        PF(temp, LBP, 0.0015) * dt * EPS * EPS * int(LBP * EPS < MaxBP * LoadC);
    LBP = LBP - dv;
    dv = 0;

    // przeplyw ZP <-> silowniki
    if (((BrakeStatus & b_on) == b_on) &&
        ((TareBP < 0.1) || (BCP < MaxBP * LoadC)))
        if ((BVP > LBP)) {
            DCV = false;
...
void TWest::PLC(double const mass) {
    LoadC =
        1 + int(mass < LoadM) * ((TareBP + (MaxBP - TareBP) * (mass - TareM) /
                                               (LoadM - TareM)) /
                                     MaxBP -
                                 1);
}
...
double TEStEP2::GetPF(double const PP, double const dt, double const Vel) {
    ...
    else if ((VVP + 0.10 < CVP - 0.12) && (BCP < 0.25))  // poczatek hamowania
    {
        // if ((BrakeStatus & 1) == 0)
        //{
        //    //       ValveRes.CreatePress(0.5*VVP);  //110115 - konsultacje
        //    warszawa1
        //    //       SoundFlag:=SoundFlag or sf_Acc;
        //    //       ValveRes.Act;
        //}
        BrakeStatus |= (b_on | b_hld);
    } else if ((VVP + BCP / BVM < CVP - 0.12) && (BCP > 0.25))  // zatrzymanie
                                                                // luzowanie
        BrakeStatus |= b_hld;
    ...
    if ((ImplsRes->P() > LBP + 0.01)) LBP = 0;

    // luzowanie CH
    if ((BrakeCyl->P() > temp + 0.005) ||
        (Max0R(ImplsRes->P(), 8 * LBP) < 0.05))
        dv = PF(0, BrakeCyl->P(),
                0.25 * SizeBC * (0.01 + (BrakeCyl->P() - temp))) *
             dt;
    else
        dv = 0;
    BrakeCyl->Flow(-dv);
    // przeplyw ZP <-> CH
    if ((BrakeCyl->P() < temp - 0.005) &&
        (Max0R(ImplsRes->P(), 8 * LBP) > 0.10) &&
        (Max0R(BCP, LBP) < MaxBP * LoadC))
        dv = PF(BVP, BrakeCyl->P(),
                0.35 * SizeBC * (0.01 - (BrakeCyl->P() - temp))) *
             dt;
    else
        dv = 0;
    BrakeRes->Flow(dv);
...
void TEStEP2::SetEPS(double const nEPS) {
    EPS = nEPS;
    if ((EPS > 0) && (LBP + 0.01 < BrakeCyl->P())) LBP = BrakeCyl->P();
}
...
void TEStEP2::EPCalc(double dt) {
    ...
    double dv = PF(temp, LBP, 0.00053 + 0.00060 * int(EPS < 0)) * dt * EPS *
                EPS * int(LBP * EPS < MaxBP * LoadC);
    LBP = LBP - dv;
}
...
void TEStEP1::EPCalc(double dt) {
    ...
    double LBPLim =
        Min0R(MaxBP * LoadC * temp, BrakeRes->P());  // do czego dążymy
    double S = 10 * clamp(LBPLim - LBP, -0.1, 0.1);  // przymykanie zaworku
    double dv = PF((S > 0 ? BrakeRes->P() : 0), LBP,
                   abs(S) * (0.00053 + 0.00060 * int(S < 0))) *
                dt;  // przepływ
    LBP = LBP - dv;
}
...
double TESt3::GetPF(double const PP, double const dt, double const Vel) {
    ...
    if ((BrakeStatus & b_hld) == b_off)
        dv = PF(0, BCP,
                0.0042 * (1.37 - (BrakeDelayFlag == bdelay_G ? 1.0 : 0.0)) *
                    SizeBC) *
             dt;
    else
        dv = 0;
    ...
    if ((BrakeStatus & b_on) == b_on)
        dv = PF(BVP, BCP,
                0.017 *
                    (1.00 + (((BCP < 0.58) && (BrakeDelayFlag == bdelay_G))
                                 ? 1.0
                                 : 0.0)) *
                    (1.13 - (((BCP > 0.60) && (BrakeDelayFlag == bdelay_G))
                                 ? 1.0
                                 : 0.0)) *
                    SizeBC) *
             dt;
...
double TESt4R::GetPF(double const PP, double const dt, double const Vel) {
    ...
    RapidStatus = (BrakeDelayFlag == bdelay_R) &&
                  (((Vel > 55) && (RapidStatus == true)) || (Vel > 70));

    RapidTemp = RapidTemp + (0.9 * int(RapidStatus) - RapidTemp) * dt / 2;
    temp = 1.9 - RapidTemp;
    if (((BrakeStatus & b_asb) == b_asb)) temp = 1000;
    // luzowanie CH
    if ((BrakeCyl->P() * temp > ImplsRes->P() + 0.005) ||
        (ImplsRes->P() < 0.25))
        if (((BrakeStatus & b_asb) == b_asb))
            dv = PFVd(
                     BrakeCyl->P(), 0, 0.115 * SizeBC * 4,
                     ImplsRes->P() / temp) *
                 dt;
        else
            dv = PFVd(BrakeCyl->P(), 0, 0.115 * SizeBC, ImplsRes->P() / temp) *
                 dt;
...
double TESt3AL2::GetPF(double const PP, double const dt, double const Vel) {
    ...
    if ((BrakeStatus & b_hld) == b_off)
        dv =
            PF(0, BCP, 0.00017 * (1.37 - int(BrakeDelayFlag == bdelay_G))) * dt;
    else
        dv = 0;
    ImplsRes->Flow(-dv);
    // przeplyw ZP <-> KI
    if (((BrakeStatus & b_on) == b_on) && (BCP < MaxBP))
        dv = PF(BVP, BCP,
                0.0008 *
                    (1 + int((BCP < 0.58) && (BrakeDelayFlag == bdelay_G))) *
    ...
    // luzowanie CH
    if ((BrakeCyl->P() > ImplsRes->P() * LoadC + 0.005) ||
        (ImplsRes->P() < 0.15))
...
double TLSt::GetPF(double const PP, double const dt, double const Vel) {
    ...
    // TODO: clean inheritance for checkstate() and checkreleaser() and reuse
    // these instead of manual copypaste
    if (((BrakeStatus & b_hld) == b_hld) && (BCP > 0.25)) {
        if ((VVP + 0.003 + BCP / BVM < CVP)) {
            // hamowanie stopniowe
            BrakeStatus |= b_on;
        ...
        (VVP + (BCP - 0.1) / BVM < CVP) && ((CVP - VVP) * BVM > 0.25) &&
        (BCP > 0.25)) {
        // zatrzymanie luzowanie
        BrakeStatus |= b_hld;
    }
    ...
    if (VVP > BCP) {
        dV = PF(VVP, BCP,
                0.00043 * (1.5 - (true == (((CVP - BCP) * BVM > 1.0) &&
        ...
        dV = PF(VVP, BCP,
                0.001472 * (1.36 - (true == (((CVP - BCP) * BVM > 1.0) &&
                                             (BrakeDelayFlag == bdelay_G))
                                        ? 1.0
                                        : 0.0)),
                0.1) *
...
double TEStED::GetPF(double const PP, double const dt, double const Vel) {
    ...
    if ((VVP + 0.002 + BCP / BVM < CVP - 0.05) && (Przys_blok))
        ...
        (VVP + (BCP - 0.1) / BVM < CVP - 0.05) &&
        (BCP > 0.25))  // zatrzymanie luzowania
        BrakeStatus |= b_hld;
    ...
    if ((MPP - 0.05 > BVP))
        dv = dv + PF(MPP - 0.05, BVP,
                     Nozzles[10] * nastG + (1 - nastG) * Nozzles[9]);
...
void TCV1::CheckState(double const BCP, double &dV1) {
    ...
    else if ((VVP + 0.10 < CVP) && (BCP < 0.1))  // poczatek hamowania
    {
        BrakeStatus |= (b_on | b_hld);
        dV1 = 1.25;
    } else if ((VVP + BCP / BVM < CVP) && (BCP > 0.25))  // zatrzymanie
                                                         // luzowanie
        BrakeStatus |= b_hld;
}
...
double TCV1::GetPF(double const PP, double const dt, double const Vel) {
    ...
    if ((BrakeStatus & b_hld) == b_off)
        dv = PF(0, BCP,
                0.0042 * (1.37 - int(BrakeDelayFlag == bdelay_G)) * SizeBC) *
             dt;
    else
        dv = 0;
    BrakeCyl->Flow(-dv);
...
double TCV1L_TR::GetPF(double const PP, double const dt, double const Vel) {
    ...
    // luzowanie KI
    if ((BrakeStatus & b_hld) == b_off)
        dv = PF(0, BCP, 0.000425 * (1.37 - int(BrakeDelayFlag == bdelay_G))) *
             dt;
    else
        dv = 0;
    ImplsRes->Flow(-dv);
    ...
    // przeplyw ZP <-> CH
    if ((BrakeCyl->P() < temp - 0.005) &&
        (Max0R(ImplsRes->P(), 8 * LBP) > 0.3) && (Max0R(BCP, LBP) < MaxBP))
        dv = PF(BVP, BrakeCyl->P(), 0.020 * SizeBC) * dt;
    else
        dv = 0;
    BrakeRes->Flow(dv);
...
double TKE::GetPF(double const PP, double const dt, double const Vel) {
    ...
    if (((BrakeStatus & b_on) == b_on) && (IMP < MaxBP)) {
        temp = 0.113;
        if (((BrakeDelayFlag & bdelay_G) == 0)) temp = temp + 0.636;
        if ((BCP < 0.5)) temp = temp + 0.785;
    ...
    //  temp:=Max0R(BCP,LBP);
    IMP = Max0R(
        IMP / temp, Max0R(LBP, ASBP * int((BrakeStatus & b_asb) == b_asb)));
    if ((ASBP < 0.1) && ((BrakeStatus & b_asb) == b_asb)) IMP = 0;
    // luzowanie CH
...
double TDriverHandle::GetPF(
    double const i_bcp, double PP, double HP, double dt, double ep) {
    return 0;
}
...
void TDriverHandle::OvrldButton(bool Active) { ManualOvrldActive = Active; }

void TDriverHandle::SetUniversalFlag(int flag) { UniversalFlag = flag; }
//---FV4a---
...
double TFV4a::GetPF(double i_bcp, double PP, double HP, double dt, double ep) {
    ...
    CP =
        CP + 20 * std::min(std::abs(LimPP - CP), 0.05) * PR(CP, LimPP) * dt / 1;
    ...
    if ((CP > RP + 0.05))
        dpMainValve =
            PF(std::min(CP + 0.1, HP), PP, 1.1 * ActFlowSpeed / LBDelay) * dt;
    if ((CP < RP - 0.05))
        dpMainValve = PF(CP - 0.1, PP, 1.1 * ActFlowSpeed / LBDelay) * dt;

    if (lround(i_bcp) == -1) {
        CP = CP +
             5 * std::min(std::abs(LimPP - CP), 0.2) * PR(CP, LimPP) * dt / 2;
        if ((CP < RP + 0.03))
            if ((TP < 5)) TP = TP + dt;
        //            if(cp+0.03<5.4)then
        if ((RP + 0.03 < 5.4) || (CP + 0.03 < 5.4))  // fala
            dpMainValve =
                PF(std::min(HP, 17.1), PP, ActFlowSpeed / LBDelay) * dt;
        //              dpMainValve:=20*Min0R(abs(ep-7.1),0.05)*PF(HP,pp,ActFlowSpeed/LBDelay)*dt;
        else {
            RP = 5.45;
            if ((CP < PP - 0.01))  //: /34*9
                dpMainValve =
                    PF(dpPipe, PP, ActFlowSpeed / 34 * 9 / LBDelay) * dt;
            else
    ...
    if ((lround(i_bcp) == 0)) {
        ...
        if ((CP > RP + 0.1) && (CP <= 5))
            dpMainValve =
                PF(std::min(CP + 0.25, HP), PP, 2 * ActFlowSpeed / LBDelay) *
                dt;
        else if (CP > 5)
            dpMainValve =
                PF(std::min(CP, HP), PP, 2 * ActFlowSpeed / LBDelay) * dt;
        else
            dpMainValve = PF(dpPipe, PP, ActFlowSpeed / LBDelay) * dt;
    }
...
double TFV4aM::GetPF(double i_bcp, double PP, double HP, double dt, double ep) {
    ...
    if (LimPP > CP) {
        ...
        CP += 5.0 * 60.0 * std::min(std::abs(LimPP - CP), 0.05) *
              PR(CP, LimPP) * dt;
    } else {
    ...
    double dpMainValve =
        (dpPipe > PP ? -PFVa(HP, PP, ActFlowSpeed / LBDelay, dpPipe, 0.4)
    ...
    if ((RP < ep) && (RP < BPT[std::lround(i_bcpno) + 2][1])) {
        ...
        // przypisz cisnienie w PG - wydluzanie napelniania o czas potrzebny do
        // napelnienia PG
...
void TFV4aM::SetReductor(double nAdj) { RedAdj = nAdj; }
...
double TFV4aM::GetPos(int i) { return pos_table[i]; }
...
double TFV4aM::LPP_RP(double pos)  // cisnienie z zaokraglonej pozycji;
{
    ...
    return BPT[i_pos][1] + (BPT[i_pos + 1][1] - BPT[i_pos][1]) *
                               ((pos + 2) - i_pos);  // interpolacja liniowa
}
...
double TMHZ_EN57::GetPF(
    double i_bcp, double PP, double HP, double dt, double ep) {
    static int const LBDelay = 100;
    ...
    i_bcp = Max0R(
        Min0R(i_bcp, 9.999),
        -0.999);  // na wszelki wypadek, zeby nie wyszlo poza zakres

    if ((TP > 0) && (CP > 4.9)) {
        DP = OverloadPressureDecrease;
        if (EQ(i_bcp, 0)) TP = TP - DP * dt;
        Sounds[s_fv4a_t] = DP;
    } else {
        TP = 0;
    }
    ...
    if ((LimPP > CP))  // podwyzszanie szybkie
        CP = CP + 60 * Min0R(abs(LimPP - CP), 0.05) * PR(CP, LimPP) *
                      dt;  // zbiornik sterujacy;
    else
        CP = CP + 13 * Min0R(abs(LimPP - CP), 0.05) * PR(CP, LimPP) *
                      dt;  // zbiornik sterujacy
    ...
    if ((EQ(i_bcp, -1) && (AutoOvrld)) ||
        ...
        if ((TP < OverloadMaxPressure)) TP = TP - 0.5 * dt;  // 5/10
    }
...
void TMHZ_EN57::Init(double Press) { CP = Press; }

void TMHZ_EN57::SetReductor(double nAdj) { RedAdj = nAdj; }
...
double TMHZ_EN57::GetPos(int i) { return pos_table[i]; }
...
double TMHZ_K5P::GetPF(
    ...
    if ((LimCP > CP))  // podwyzszanie szybkie
        CP = CP + 9 * Min0R(abs(LimCP - CP), 0.05) * PR(CP, LimCP) *
                      dt;  // zbiornik sterujacy;
    else
        CP = CP + 9 * Min0R(abs(LimCP - CP), 0.05) * PR(CP, LimCP) *
                      dt;  // zbiornik sterujacy
    ...
    if ((EQ(i_bcp, -1) && (AutoOvrld)) ||
        ((i_bcp < 0.5) && (UniversalFlag & TUniversalBrake::ub_Overload))) {
        if ((TP < OverloadMaxPressure)) TP = TP + 0.03 * dt;
    }
...
void TMHZ_K5P::SetReductor(double nAdj) { RedAdj = nAdj; }
...
double TMHZ_K5P::GetPos(int i) { return pos_table[i]; }

double TMHZ_K5P::GetCP() { return CP; }

double TMHZ_K5P::GetRP() { return 5.0 + TP + RedAdj; }

void TMHZ_K5P::SetParams(
    bool AO, bool MO, double OverP, double FSF, double OMP, double OPD) {
    AutoOvrld = AO;
    ManualOvrld = MO;
    UnbrakeOverPressure = std::max(0.0, OverP);
    Fala = (OverP > 0.01);
...
void TMHZ_6P::SetReductor(double nAdj) { RedAdj = nAdj; }
...
double TM394::GetPF(double i_bcp, double PP, double HP, double dt, double ep) {
    ...
    if ((BCP == 1) || (BCP == i_bcpno)) LimPP = PP;
    if ((BCP == 0)) LimPP = LimPP + RedAdj;
    if ((BCP != 2))
        if (CP < LimPP)
        ...
        //      //zbiornik sterujacy;
            ...
            CP = CP + 4 * (1 + int(BCP != 3) + int(BCP > 4)) *
                          Min0R(abs(LimPP - CP), 0.05) * PR(CP, LimPP) *
                          dt;  // zbiornik sterujacy
    ...
    if (BCP == -1) dpMainValve = PF(HP, PP, ActFlowSpeed / LBDelay) * dt;

    if (BCP == i_bcpno) dpMainValve = PF(0, PP, ActFlowSpeed / LBDelay) * dt;
...
void TM394::SetReductor(double nAdj) { RedAdj = nAdj; }

double TM394::GetCP() { return CP; }

double TM394::GetRP() { return std::max(5.0, CP) + RedAdj; }

double TM394::GetPos(int i) { return pos_table[i]; }
...
double TH14K1::GetPF(double i_bcp, double PP, double HP, double dt, double ep) {
    ...
    CP = CP + 6 * std::min(std::abs(LimPP - CP), 0.05) * PR(CP, LimPP) *
                  dt;  // zbiornik sterujacy
    ...
    if ((BCP == 0))
        dpMainValve =
            -PFVa(HP, PP, ActFlowSpeed / LBDelay, NomPress + RedAdj) * dt;
    if ((BCP > 1) && (PP > CP))
...
void TH14K1::SetReductor(double nAdj) { RedAdj = nAdj; }

double TH14K1::GetCP() { return CP; }

double TH14K1::GetRP() { return 5.0 + RedAdj; }

double TH14K1::GetPos(int i) { return pos_table[i]; }
...
double TSt113::GetPF(double i_bcp, double PP, double HP, double dt, double ep) {
    ...
    CP = CP + 6 * Min0R(abs(LimPP - CP), 0.05) * PR(CP, LimPP) *
                  dt;  // zbiornik sterujacy
...
double TSt113::GetPos(int i) { return pos_table[i]; }
...
void Ttest::Init(double Press) { CP = Press; }
...
double TFD1::GetPF(double i_bcp, double PP, double HP, double dt, double ep) {
    ...
    DP = 10.0 * std::min(std::abs(temp - BP), 0.1) *
         PF(temp, BP, 0.0006 * (temp > BP ? 3.0 : 2.0)) * dt * Speed;
    BP = BP - DP;
    return -DP;
}
...
void TFD1::SetSpeed(double nSpeed) { Speed = nSpeed; }
...
double TFVel6::GetPos(int i) { return pos_table[i]; }
...
double TFVE408::GetPos(int i) { return pos_table[i]; }
