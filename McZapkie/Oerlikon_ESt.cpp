...
void TPrzeciwposlizg::SetPoslizg(bool flag) { Poslizg = flag; }
...
void TRapid::SetRapidStatus(bool rs) { RapidStatus = rs; }
...
void TRapid::Update(double dt) {
    ...
    Next->Flow(dV);
    if (dV > 0.0) BrakeRes->Flow(-dV);
}

// ------ PRZEKŁADNIK CIĄGŁY ------

void TPrzekCiagly::SetMult(double m) { Mult = m; }
...
void TPrzek_PZZ::SetLBP(double P) { LBP = P; }
...
void TPrzekED::SetP(double P) { MaxP = P; }
    ...
    double const Vel)  // przeplyw miedzy komora wstepna i PG
{
    ...
    if (((BrakeStatus & b_on) == b_on) &&
        (Przekladniki[1]->P() * HBG300 < MaxBP))
        dV = PF(BVP, BCP,
                Nozzles[dTN] * (nastG + 2.0 * (BCP < Podskok ? 1.0 : 0.0)) +
                    Nozzles[dON] * (1.0 - nastG)) *
        ...
        if (typeid(*Przekladniki[i]) == typeid(TRapid)) {
            RapidStatus =
                (((BrakeDelayFlag & bdelay_R) == bdelay_R) &&
                 ((std::abs(Vel) > 70.0) ||
                  ((std::abs(Vel) > 50.0) && (RapidStatus)) || (RapidStaly)));
    ...
    if ((MPP - 0.05) > BVP)
        dV +=
            PF(MPP - 0.05, BVP,
               Nozzles[dPT] * nastG + (1.0 - nastG) * Nozzles[dPO]);
    if (MPP > VVP) dV += PF(MPP, VVP, d2A(5.0));
...
void TNESt3::EStParams(
    double const i_crc)  // parametry charakterystyczne dla ESt
{}
...
void TNESt3::CheckState(
    double const BCP, double &dV1)  // glowny przyrzad rozrzadczy
{
    ...
    if ((BrakeStatus & b_hld) == 0) SoundFlag |= sf_CylU;
...
void TNESt3::CheckReleaser(double const dt)  // odluzniacz
{
    ...
    if ((BrakeStatus & b_rls) == b_rls) {
        ...
        if ((CVP < (VVP + 0.3)) || (false == autom)) BrakeStatus &= ~b_rls;
    }
}
...
void TNESt3::PLC(double const mass) {
    LoadC = 1.0 + (mass < LoadM ? ((TareBP + (MaxBP - TareBP) * (mass - TareM) /
                                                 (LoadM - TareM)) /
                                       MaxBP -
                                   1.0)
                                : 0.0);
...
void TNESt3::SetSize(
    int const size, std::string const &params)  // ustawianie dysz (rozmiaru ZR)
{
    ...
    switch (size) {
        ...
        case 200: {  // dON,dOO,dTN,dTO,dP,dS
            Nozzles[dON] = dNO1l;
        ...
        case 375: {  // dON,dOO,dTN,dTO,dP,dS
            Nozzles[dON] = dNO1l;
            Nozzles[dOO] = dOO1l / 1.15;
            Nozzles[dTN] = dNT1l;
            Nozzles[dTO] = dOT1l;
        ...
        case 150: {  // dON,dOO,dTN,dTO,dP,dS
            Nozzles[dON] = dNO1l;
            Nozzles[dOO] = dOO1l;
            Nozzles[dTN] = dNT1l;
            Nozzles[dTO] = dOT1l;
        ...
        case 100: {  // dON,dOO,dTN,dTO,dP,dS
            Nozzles[dON] = dNO1l;
            Nozzles[dOO] = dOO1l;
            Nozzles[dTN] = dNT1l;
            Nozzles[dTO] = dOT1l;
