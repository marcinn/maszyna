...
void trainingcard_panel::save_thread_func() {
    ...
    std::fstream temp(
        "reports/" + recording_timestamp + ".html",
        std::ios_base::out | std::ios_base::binary);
    std::fstream input(
        "report_template.html", std::ios_base::in | std::ios_base::binary);

    std::string in_line;
    while (std::getline(input, in_line)) {
        const std::string magic("{{CONTENT}}");
        if (in_line.compare(0, magic.size(), magic) == 0) {
            temp << "<div><b>Miejsce: </b>" << (std::string(place.c_str()))
                 << "</div><br />" << std::endl;
            temp << "<div><b>Data: </b>" << (date) << "</div><br />"
                 << std::endl;
            temp << "<div><b>Czas: </b>" << (from) << " - " << (to)
                 << "</div><br />" << std::endl;
            temp << "<div><b>Imię (imiona) i nazwisko szkolonego: </b>"
                 << (trainee_name) << "</div><br />" << std::endl;
            temp << "<div><b>Data urodzenia: </b>" << (trainee_birthdate)
                 << "</div><br />" << std::endl;
            temp << "<div><b>Firma: </b>" << (trainee_company) << "</div><br />"
                 << std::endl;
            temp << "<div><b>Imię i nazwisko instruktora: </b>"
                 << (instructor_name) << "</div><br />" << std::endl;
            temp << "<div><b>Odcinek trasy: </b>" << (track_segment)
                 << "</div><br />" << std::endl;
            if (distance > 0.0f)
                temp << "<div><b>Przebyta odległość: </b>"
                     << std::round(distance) << " km</div><br />" << std::endl;
            temp << "<div><b>Uwagi: </b><br />" << (remarks) << "</div>"
                 << std::endl;
        } else {
            temp << in_line;
        }
...
void trainingcard_panel::render_contents() {
    if (ImGui::BeginPopupModal("Zapisywanie danych")) {
        ...
        if (s < 1) {
            if (s == 0)
                ImGui::TextUnformatted(
                    "Error occured please contact with administrator!");
            if (s == -1)
                ImGui::TextUnformatted(
                    "The recording of the training has not been archived, "
                    "please do not start the next training before manually "
                    "archiving the file!");
        ...
        if (s == 2)
            ImGui::TextUnformatted(
                u8"Proszę czekać, trwa archiwizacja nagrania...");
    ...
    if (start_time_wall) {
        std::tm *tm = std::localtime(&(*start_time_wall));
        std::string rep =
            u8"Czas rozpoczęcia: " + std::to_string(tm->tm_year + 1900) + "-" +
    ...
    ImGui::SameLine();
    ImGui::InputText(
        "##birthdate", &trainee_birthdate[0], trainee_birthdate.size());
    ...
    ImGui::SameLine();
    ImGui::InputText(
        "##instructor", &instructor_name[0], instructor_name.size());
    ...
    ImGui::InputTextMultiline(
        "##remarks", &remarks[0], remarks.size(), ImVec2(-1.0f, 200.0f));
    ...
    if (!start_time_wall) {
        if (ImGui::Button("Rozpocznij szkolenie")) {
            ...
            recording_timestamp =
                ...
                std::to_string(tm->tm_mon + 1) + std::to_string(tm->tm_mday) +
                std::to_string(tm->tm_hour) + std::to_string(tm->tm_min) + "_" +
                std::string(trainee_name.c_str()) + "_" +
                std::string(instructor_name.c_str());
        ...
        if (ImGui::Button(u8"Zakończ szkolenie")) {
            ...
            if (simulation::Trains.sequence().size() > 0)
                distance = simulation::Trains.sequence()[0]
                               ->Dynamic()
                               ->MoverParameters->DistCounter;

            if (save_thread.joinable()) save_thread.join();
            save_thread =
                std::thread(&trainingcard_panel::save_thread_func, this);
            ImGui::OpenPopup("Zapisywanie danych");
        }
    }
}
...
const std::string *trainingcard_panel::is_recording() {
    if (!start_time_wall) return nullptr;
