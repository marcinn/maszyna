...
void cameraview_window_callback(ImGuiSizeCallbackData *data) {
    data->DesiredSize.y = data->DesiredSize.x * (float)Global.extcam_res.y /
                          (float)Global.extcam_res.x;
}

ui::cameraview_panel::~cameraview_panel() {
    record_state = STOPPING;
    if (record_workthread.joinable()) record_workthread.join();

    capture_state = STOPPING;
    if (capture_workthread.joinable()) capture_workthread.join();
}

void ui::cameraview_panel::render() {
    if (Global.extcam_cmd.empty()) return;
    ...
    if (!recording &&
        !(record_state == STOPPED || record_state == STOPPING_DONE)) {
        record_state = STOPPING;
    }
...
void ui::cameraview_panel::render_contents() {
...

    {
        ...
        if (image_ptr) {
            ...
            glTexImage2D(
                GL_TEXTURE_2D, 0, GL_SRGB8_ALPHA8, Global.extcam_res.x,
                Global.extcam_res.y, 0, GL_RGB, GL_UNSIGNED_BYTE, image_ptr);
            image_ptr = nullptr;
...
void ui::cameraview_panel::record_func() {
    ...
    if (!rec_name.empty()) {
        ...
        if (pos != -1) cmdline.replace(pos, magic.size(), rec_name);
    }
    ...
    while (record_state == RUNNING) {
        if (bufpos == frame_size) {
            ...
            auto r = notify_var.wait_for(
                lock, std::chrono::milliseconds(50),
                [this, last_cnt] { return last_cnt != frame_cnt; });
            last_cnt = frame_cnt;
            if (!image_ptr || !r) continue;
