...
ui::map_panel::map_panel() : ui_panel(STR_C("Map"), false) {
    ...
    opengl33_renderer *gl33 =
        dynamic_cast<opengl33_renderer *>(GfxRenderer.get());
    if (!gl33) {
    ...
    m_track_shader =
        std::unique_ptr<gl::program>(new gl::program({vert, frag}));
    if (!Global.gfx_usegles || GLAD_GL_EXT_geometry_shader) {
        gl::shader poi_frag("map_poi.frag");
        gl::shader poi_geom("map_poi.geom");
        m_poi_shader = std::unique_ptr<gl::program>(
            new gl::program({vert, poi_frag, poi_geom}));
    ...
    glGetError();
    glLineWidth(2.0f);
    if (!glGetError()) m_widelines_supported = true;
...
float ui::map_panel::get_vehicle_rotation() {
    ...
    glm::vec3 front = glm::dvec3(vehicle->VectorFront()) *
                      (vehicle->DirectionGet() > 0 ? 1.0 : -1.0);
    glm::vec2 north_ptr(0.0f, 1.0f);
    return glm::atan(front.z, front.x) - glm::atan(north_ptr.y, north_ptr.x);
}

void ui::map_panel::render_map_texture(
    glm::mat4 transform, glm::vec2 surface_size) {
    cFrustum frustum;
    ...
    for (int row = 0; row < scene::EU07_REGIONSIDESECTIONCOUNT; row++) {
        for (int column = 0; column < scene::EU07_REGIONSIDESECTIONCOUNT;
             column++) {
            scene::basic_section *section = simulation::Region->get_section(
                row * scene::EU07_REGIONSIDESECTIONCOUNT + column);
            if (section &&
                frustum.sphere_inside(
                    section->area().center, section->area().radius) > 0.f) {
                const gfx::geometrybank_handle handle =
                    section->get_map_geometry();
    ...
    m_track_shader->bind();
    if (m_widelines_supported) glLineWidth(1.5f);
    ...
    if (m_widelines_supported) glLineWidth(4.5f);
    scene_ubs.cascade_end = glm::vec4(0.7f, 0.7f, 0.0f, 0.0f);
    scene_ubo->update(scene_ubs);
    gl33->Draw_Geometry(
        m_colored_paths.future.begin(), m_colored_paths.future.end());

    if (m_widelines_supported) glLineWidth(3.0f);
    scene_ubs.cascade_end = glm::vec4(0.0f, 1.0f, 0.0f, 0.0f);
    scene_ubo->update(scene_ubs);
    gl33->Draw_Geometry(
        m_colored_paths.switches.begin(), m_colored_paths.switches.end());

    if (m_widelines_supported) glLineWidth(1.5f);
    scene_ubs.cascade_end = glm::vec4(1.0f, 0.0f, 0.0f, 0.0f);
    scene_ubo->update(scene_ubs);
    gl33->Draw_Geometry(
        m_colored_paths.occupied.begin(), m_colored_paths.occupied.end());

    if (m_widelines_supported) glLineWidth(4.0f);
    scene_ubs.cascade_end = glm::vec4(0.3f, 0.3f, 1.0f, 0.0f);
    scene_ubo->update(scene_ubs);
    gl33->Draw_Geometry(
        m_colored_paths.highlighted.begin(), m_colored_paths.highlighted.end());

    if (!Global.gfx_usegles || GLAD_GL_EXT_geometry_shader) {
        gl33->Bind_Texture(0, m_icon_atlas);
        m_poi_shader->bind();
        scene_ubs.cascade_end =
            glm::vec4(1.0f / (surface_size / 200.0f), 1.0f, 0.0f);
    }
    ...
    if (Global.iMultisampling)
        m_fb->blit_from(
            m_msaa_fb.get(), surface_size.x, surface_size.y,
...
void ui::map_panel::render_labels(
    glm::mat4 transform, ImVec2 origin, glm::vec2 surface_size) {
    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.7f, 1.0f, 0.7f, 1.0f));
    for (TDynamicObject *vehicle : simulation::Vehicles.sequence()) {
        if (vehicle->Prev() || !vehicle->Mechanik) continue;

        std::string label = vehicle->Mechanik->TrainName();
        if (label.empty() || label == "none") label = ToUpper(vehicle->name());

        glm::vec4 ndc_pos =
            transform * glm::vec4(glm::vec3(vehicle->GetPosition()), 1.0f);
        if (glm::abs(ndc_pos.x) > 1.0f || glm::abs(ndc_pos.z) > 1.0f) continue;

        glm::vec2 gui_pos = (glm::vec2(ndc_pos.x, -ndc_pos.z) / 2.0f + 0.5f) *
                            glm::vec2(surface_size.x, surface_size.y);

        TDynamicObject *veh = vehicle;

        ImVec2 textsize = ImGui::CalcTextSize(label.c_str());
        ImGui::SetCursorPos(ImVec2(
            origin.x + gui_pos.x - textsize.x / 2.0f,
...
void ui::map_panel::render_contents() {
    ...
    if (aspect > 1.0f / aspect)
        aspect_transform =
            glm::scale(glm::mat4(), glm::vec3(aspect, 1.0f, 1.0f));
    else
        aspect_transform =
            glm::scale(glm::mat4(), glm::vec3(1.0f, 1.0f, 1.0f / aspect));
    ...
    if (mode == MODE_VEHICLE && simulation::Train) {
        ...
        transform =
            glm::rotate(transform, rotation, glm::vec3(0.0f, 1.0f, 0.0f));
    ...
    transform =
        glm::translate(transform, glm::vec3(translate.x, 0.0f, translate.y));
    ...
    ImGui::ImageButton(
        reinterpret_cast<void *>(m_tex->id), surface_size_im,
        ImVec2(0, surface_size.y / fb_size),
        ImVec2(surface_size.x / fb_size, 0), 0);

    if (ImGui::IsItemHovered()) {
        const ImVec2 screen_pos = ImGui::GetMousePos();
        const glm::vec2 surface_pos(
            screen_pos.x - screen_origin.x, screen_pos.y - screen_origin.y);
        ...
        if (io.MouseWheel != 0.0f) {
            ...
            glm::vec4 corrected = glm::inverse(aspect_transform) *
                                  glm::vec4(ndc_pos.x, 0.0f, ndc_pos.y, 1.0f);
            translate += glm::vec2(corrected.x, corrected.z);
            translate *= x;

            glm::vec2 surface_screen_center =
                glm::vec2(screen_origin.x, screen_origin.y) +
                surface_size / 2.0f;
            Application.set_cursor_pos(
                surface_screen_center.x, surface_screen_center.y);
        }

        if (FreeFlyModeFlag && ImGui::IsMouseClicked(2)) {
            glm::vec3 world_pos = glm::inverse(transform) *
                                  glm::vec4(ndc_pos.x, 0.0f, -ndc_pos.y, 1.0f);
        ...
        if (mode == 0 && ImGui::IsMouseDragging(0)) {
            ...
            map::sorted_object_list objects = map::Objects.find_in_range(
                glm::vec3(world_pos.x, NAN, world_pos.z), 0.03f / zoom);

            if (ImGui::IsMouseClicked(1)) {
                if (objects.size() > 1)
                    register_popup(std::make_unique<ui::disambiguation_popup>(
                        *this, std::move(objects)));
                else if (objects.size() == 1)
                    handle_map_object_click(*this, objects.begin()->second);
                else {
                    glm::vec3 nearest =
                        simulation::Region->find_nearest_track_point(world_pos);
                    if (!glm::isnan(nearest.x) &&
                        glm::distance(
                            glm::vec2(world_pos.x, world_pos.z),
                            glm::vec2(nearest.x, nearest.z)) < 60.0f)
                        register_popup(std::make_unique<obstacle_insert_window>(
                            *this, std::move(nearest)));
                }
...
void ui::handle_map_object_click(
    ui_panel &parent, std::shared_ptr<map::map_object> &obj) {
    if (auto sem = std::dynamic_pointer_cast<map::semaphore>(obj)) {
        parent.register_popup(
            std::make_unique<semaphore_window>(parent, std::move(sem)));
    } else if (auto track = std::dynamic_pointer_cast<map::launcher>(obj)) {
        parent.register_popup(
            std::make_unique<launcher_window>(parent, std::move(track)));
    } else if (
        auto track_switch = std::dynamic_pointer_cast<map::track_switch>(obj)) {
        parent.register_popup(std::make_unique<track_switch_window>(
            parent, std::move(track_switch)));
    } else if (auto obstacle = std::dynamic_pointer_cast<map::obstacle>(obj)) {
        parent.register_popup(std::make_unique<obstacle_remove_window>(
            parent, std::move(obstacle)));
    } else if (auto obstacle = std::dynamic_pointer_cast<map::vehicle>(obj)) {
        parent.register_popup(std::make_unique<vehicle_click_window>(
            parent, std::move(obstacle)));
    }
}
...
#ifdef WITH_OPENGL_MODERN
                dynamic_cast<opengl33_renderer *>(GfxRenderer.get())
                    ->Update_AnimModel(model);  // update lamp opacities
#endif
                auto state = model->LightGet(i);
                if (!state) continue;

                glm::vec3 current_color =
                    std::get<2>(*state).value_or(glm::vec3(0.5f)) *
                    std::get<1>(*state);
                if (std::get<0>(*state) < 0.0f) continue;

                ImGui::PushStyleColor(
                    ImGuiCol_Button, ImVec4(
                                         current_color.x, current_color.y,
                                         current_color.z, 1.0f));
...
ui::disambiguation_popup::disambiguation_popup(
    ui_panel &panel, map::sorted_object_list &&list)
...
ui::semaphore_window::semaphore_window(
    ui_panel &panel, std::shared_ptr<map::semaphore> &&sem)
...
void ui::semaphore_window::render_content() {
    for (auto &model : m_sem->models) {
        ...
        for (int i = 0; i < iMaxNumLights; i++) {
            ...
            ImGui::PushStyleColor(
                ...
                ImVec4(lamp_color.x, lamp_color.y, lamp_color.z, 1.0f));
            ImGui::PushStyleColor(
                ImGuiCol_Button,
                ImVec4(
                    current_color.x, current_color.y, current_color.z, 1.0f));

            std::string res = "  ##" + std::to_string(i);
            if (ImGui::Button(res.c_str())) {
                level += 1.0f;
                if (level >= 3.0f) level = 0.0f;

                m_relay.post(
                    user_command::setlight, (double)i, level, GLFW_PRESS, 0,
                    glm::vec3(0.0f), &model->name());
            }
    ...
    for (auto &item : m_sem->events) {
        ...
        if (displayname.size() < 2) continue;
        ...
        if (ImGui::Button(displayname.c_str())) {
            m_relay.post(
                ...
                glm::vec3(0.0f), &item->name());
        }
    }
}

ui::launcher_window::launcher_window(
    ui_panel &panel, std::shared_ptr<map::launcher> &&sw)
...
void ui::launcher_window::render_content() {
    ...
    const std::string &open_label =
        m_switch->type == map::launcher::track_switch ? STR("Straight |")
                                                      : STR("Open |");

    const std::string &close_label =
        m_switch->type == map::launcher::track_switch ? STR("Divert /")
                                                      : STR("Close -");

    if (ImGui::Button(open_label.c_str())) {
        m_relay.post(
            user_command::queueevent, 0.0, 0.0, GLFW_PRESS, 0, glm::vec3(0.0f),
            &m_switch->first_event->name());
        ImGui::CloseCurrentPopup();
    }
    ...
    if (ImGui::Button(close_label.c_str())) {
        m_relay.post(
            user_command::queueevent, 0.0, 0.0, GLFW_PRESS, 0, glm::vec3(0.0f),
            &m_switch->second_event->name());
        ImGui::CloseCurrentPopup();
    }
}

ui::track_switch_window::track_switch_window(
    ui_panel &panel, std::shared_ptr<map::track_switch> &&sw)
    : popup(panel), m_switch(sw) {}
    ...
    for (size_t i = 0; i < 4; i++) {
        if (m_switch->action[i]) {
            if (ImGui::Button(names[i].c_str()))
                m_relay.post(
                    user_command::queueevent, 0.0, 0.0, GLFW_PRESS, 0,
                    glm::vec3(0.0f), &m_switch->action[i]->name());
            if (ImGui::IsItemHovered()) {
                for (size_t j = 0; j < 4; j++) {
                    if ((names[i][0] - 'a') != j && (names[i][1] - 'a') != j)
                        continue;
                    highlight.push_back(std::make_pair(
                        m_switch->track[j],
                        m_switch->preview[i][j] == '1' ? 1 : 0));
                }
            }
            ImGui::SameLine();
        }
...
ui::obstacle_insert_window::obstacle_insert_window(
    ui_panel &panel, glm::dvec3 const &pos)
...
void ui::obstacle_insert_window::render_content() {
    ...
    for (auto const &entry : m_obstacles) {
        if (ImGui::Button(entry.first.c_str())) {
            std::string name(
                "obstacle_" + std::to_string(LocalRandom(0.0, 100000.0)));

            std::string payload(name + ':' + entry.second);
            m_relay.post(
                user_command::insertmodel, 0.0, 0.0, GLFW_PRESS, 0, m_position,
                &payload);
            ...
            GfxRenderer->Append(
                vertices, simulation::Region->get_map_poi_geometry(),
                GL_POINTS);
...
ui::obstacle_remove_window::obstacle_remove_window(
    ui_panel &panel, std::shared_ptr<map::obstacle> &&obstacle)
    : popup(panel), m_obstacle(obstacle) {}

void ui::obstacle_remove_window::render_content() {
    if (ImGui::Button(STR_C("Delete obstacle"))) {
        m_relay.post(
            user_command::deletemodel, 0.0, 0.0, GLFW_PRESS, 0, glm::vec3(),
            &m_obstacle->model_name);
...
ui::vehicle_click_window::vehicle_click_window(
    ui_panel &panel, std::shared_ptr<map::vehicle> &&obstacle)
...
void ui::vehicle_click_window::render_content() {
    ...
    if (ImGui::Button(STR_C("none"))) {
        ...
        m_relay.post(
            user_command::sendaicommand, 0.0, 0.0, GLFW_PRESS, 0, glm::vec3(),
            &name);
    ...
    if (ImGui::Button(STR_C("Pc6"))) {
        std::string name = m_vehicle->name + "%SetSignal";
        m_relay.post(
            user_command::sendaicommand, Signal_Pc6, 1.0, GLFW_PRESS, 0,
            glm::vec3(), &name);
    ...
    if (ImGui::Button(STR_C("A1"))) {
        std::string name = m_vehicle->name + "%SetSignal";
        m_relay.post(
            user_command::sendaicommand, Signal_A1, 1.0, GLFW_PRESS, 0,
            glm::vec3(), &name);
    ...
    if (DebugModeFlag) {
        ...
        ImGui::InputText(
            STR_C("Command"), command_buf.data(), command_buf.size());
        ...
        if (ImGui::BeginCombo(
            ...
            for (size_t i = 0; i < possible_commands.size(); i++)
                if (ImGui::Selectable(possible_commands[i].c_str(), false))
                    memcpy(
                        command_buf.data(), possible_commands[i].c_str(),
                        possible_commands[i].size() + 1);
            ImGui::EndCombo();
        }
        ...
        if (ImGui::Button(STR_C("Send"))) {
            std::string name =
                m_vehicle->name + "%" + std::string(command_buf.data());
            m_relay.post(
                user_command::sendaicommand, command_param1, command_param2,
                GLFW_PRESS, 0, glm::vec3(), &name);
