...
void basic_event::event_conditions::bind(basic_event::node_sequence *Nodes) {
    memcompare_cells = Nodes;
}

void basic_event::event_conditions::init() {
    tracks.clear();

    if (flags & (flags::track_busy | flags::track_free)) {
        for (auto &target : *memcompare_cells) {
            tracks.emplace_back(
                simulation::Paths.find(std::get<std::string>(target)));
            if (tracks.back() == nullptr) {
                // legacy compatibility behaviour, instead of disabling the
                // event we disable the memory cell comparison test
                //                m_ignored = true; // deaktywacja
                //                ErrorLog( "Bad event: track \"" +
                //                std::get<std::string>( target ) + "\"
                //                referenced in event \"" + asName + "\" doesn't
                //                exist" );
                flags &=
                    ~(flags::track_busy | flags::track_free);  // zerowanie flag
            }
        }
    }
}

bool basic_event::event_conditions::test() const {
    if (flags == 0) {
    ...
    if (flags & flags::probability) {
        ...
        WriteLog(
            "Test: Random integer - [" + std::to_string(randomroll) + "] / [" +
    ...
    if (flags & flags::track_busy) {
        ...
        for (auto *track : tracks) {
            if (true == track->IsEmpty()) {
                ...
                trackbusylog += "[" + vehicles.front()->asName + "] @ [" +
                                track->name() + "]";
            }
        }

        WriteLog(trackbusylog + " - " + (trackbusyresult ? "Pass" : "Fail"));
    ...
    if (flags & flags::track_free) {
        ...
        for (auto *track : tracks) {
            if (false == track->IsEmpty()) {
                auto const &vehicles{track->Dynamics};
                trackfreelog += "[" + vehicles.front()->asName + "] @ [" +
                                track->name() + "] - ";
        ...
        WriteLog(trackfreelog + (trackfreeresult ? "Pass" : "Fail"));
    ...
    if (flags & (flags::text | flags::value1 | flags::value2)) {
        ...
        for (auto &cellwrapper : *memcompare_cells) {
            auto *cell{static_cast<TMemCell *>(
                std::get<scene::basic_node *>(cellwrapper))};
            if (cell == nullptr) {
                //                ErrorLog( "Event " + asName + " trying
                //                conditional_memcompare with nonexistent
                //                memcell" );
                continue;  // though this is technically error, we treat it as a
                           // success to maintain backward compatibility
            }
            auto const comparisonresult = cell->Compare(
                memcompare_text, memcompare_value1, memcompare_value2, flags,
                memcompare_text_operator, memcompare_value1_operator,
                memcompare_value2_operator, memcompare_pass);
            ...
            std::string comparisonlog =
                "Test: MemCompare - " + cell->name() + " - ";

            comparisonlog += (TestFlag(flags, flags::text)
                                  ? "[" + cell->Text() + "] " +
                                        to_string(memcompare_text_operator) +
                                        " [" + memcompare_text + "]"
                                  : "[*]") +
                             combiner;
            ...
            comparisonlog +=
                (TestFlag(flags, flags::value1)
                     ? "[" + to_string(cell->Value1(), 2) + "] " +
                           to_string(memcompare_value1_operator) + " [" +
                           to_string(memcompare_value1, 2) + "]"
                     : "[*]") +
                combiner;
            ...
            comparisonlog +=
                (TestFlag(flags, flags::value2)
                     ? "[" + to_string(cell->Value2(), 2) + "] " +
                           to_string(memcompare_value2_operator) + " [" +
                           to_string(memcompare_value2, 2) + "]"
                     : "[*]") +
                " - ";
...
void basic_event::event_conditions::deserialize(cParser &Input) {
    ...
    while ((true == Input.getTokens()) &&
        ...
        if (token == "trackoccupied") {
            ...
            (token == "propability") ||
            (token ==
             "probability")) {  // remove propability in few years after
                                // changing old scenery scripts 01.2021
            ...
            memcompare_pass =
                comparison_pass_from_string(Input.getToken<std::string>());
            Input.getTokens();
            if (Input.peek() != "*")  //"*" - nie brac pod uwage
            {  // two tokens, operator followed by comparison value
                std::string operatorstring;
                Input >> operatorstring;
                memcompare_text_operator =
                    comparison_operator_from_string(operatorstring);
                Input.getTokens(1, false);  // case sensitive
                Input >> memcompare_text;
                flags |= flags::text;
            }
...
void basic_event::event_conditions::export_as_text(std::ostream &Output) const {
    if (flags != 0) {
        Output << "condition ";
        ...
        if ((flags & (flags::text | flags::value1 | flags::value2)) != 0) {
            // NOTE: export doesn't preserve original memcompare condition,
            // these are all upgraded to memcompareex format for simplicity
            Output << "memcompareex "
                   << ((flags & flags::text) == 0
                           ? "*"
                           : memcompare_text + ' ' +
                                 to_string(memcompare_text_operator))
                   << ' '
                   << ((flags & flags::value1) == 0
                           ? "*"
                           : to_string(memcompare_value1) + ' ' +
                                 to_string(memcompare_value1_operator))
                   << ' '
                   << ((flags & flags::value2) == 0
                           ? "*"
                           : to_string(memcompare_value2) + ' ' +
                                 to_string(memcompare_value2_operator))
                   << ' ';
        }
    }
...
void basic_event::deserialize(cParser &Input, scene::scratch_data &Scratchpad) {
    std::string token;
    ...
    // subclass method is expected to leave next token past its own data
    // preloaded on its exit
    while ((false == (token = Input.peek()).empty()) && (token != "endevent")) {
...
void basic_event::deserialize_targets(std::string const &Input) {
    cParser targetparser{Input};
    std::string target;
    while (false ==
           (target = targetparser.getToken<std::string>(true, "|,")).empty()) {
        // actual bindings to targets of proper type are created during scenario
        // initialization
        if (target != "none") {
...
// sends basic content of the class in legacy (text) format to provided stream
void basic_event::export_as_text(std::ostream &Output) const {
    // header
    Output << "event ";
    // name
    Output << m_name << ' ';
    ...
    if (m_targets.empty()) {
        ...
        for (auto &target : m_targets) {
            ...
            Output << (targetnode != nullptr ? targetnode->name()
                                             : std::get<std::string>(target))
    ...
    if (m_delayrandom != 0.0) {
        Output << "randomdelay " << m_delayrandom << ' ';
    }
    if (false == std::isnan(m_delayrandom)) {
        Output << "departuredelay " << m_delaydeparture << ' ';
    }
    // footer
    Output << "endevent"
           << "\n";
...
void basic_event::append(basic_event *Event) {
    // doczepienie kolejnych z tą samą nazwą
    // TODO: remove recursion
    if (m_sibling)
        m_sibling->append(
            Event);  // rekurencja! - góra kilkanaście eventów będzie potrzebne
    else {
        m_sibling = Event;
...
bool basic_event::is_instant() const { return false; }

// sends content of associated data cell to specified vehicle controller
void basic_event::send_command(TController &Controller) { return; }

// returns: true if associated data cell contains a command for vehicle
// controller
bool basic_event::is_command() const { return false; };

std::string basic_event::input_text() const {
    // odczytanie komendy z eventu
    return "";
};

TCommandType basic_event::input_command() const {
    // odczytanie komendy z eventu
    return TCommandType::cm_Unknown;
};

double basic_event::input_value(int Index) const {
    // odczytanie komendy z eventu
    return 0.0;
};

glm::dvec3 basic_event::input_location() const {
    // pobranie współrzędnych eventu
    return glm::dvec3(0, 0, 0);
};

bool basic_event::is_keyword(std::string const &Token) {
    // TODO: convert to array lookup if keyword list gets longer
    return (Token == "endevent") || (Token == "randomdelay") ||
           (Token == "departuredelay");
}

TMemCell const *input_event::input_data::data_cell() const {
    return static_cast<TMemCell const *>(
        std::get<scene::basic_node *>(data_source));
}
TMemCell *input_event::input_data::data_cell() {
    return static_cast<TMemCell *>(std::get<scene::basic_node *>(data_source));
}

// prepares event for use
void updatevalues_event::init() {
...
// event type string
std::string updatevalues_event::type() const {
    return (
        (m_input.flags & flags::mode_add) == 0 ? "updatevalues" : "addvalues");
}

// deserialize() subclass details
void updatevalues_event::deserialize_(
...
// TODO: update and copy values run_ methods are largely identical, refactor to
// a single helper
void updatevalues_event::run_() {
    if (false == m_conditions.test()) {
        return;
    }
    ...
    WriteLog(
        "Type: " +
        std::string(
            (m_input.flags & flags::mode_add) ? "AddValues" : "UpdateValues") +
        " & Track command - [" +
        ((m_input.flags & flags::text) ? m_input.data_text : "X") + "] [" +
        ((m_input.flags & flags::value1) ? to_string(m_input.data_value_1, 2)
                                         : "X") +
        "] [" +
        ((m_input.flags & flags::value2) ? to_string(m_input.data_value_2, 2)
                                         : "X") +
        "]");
    for (auto &target : m_targets) {
        auto *targetcell{
            static_cast<TMemCell *>(std::get<scene::basic_node *>(target))};
        if (targetcell == nullptr) {
            continue;
        }
        targetcell->UpdateValues(
            m_input.data_text, m_input.data_value_1, m_input.data_value_2,
            m_input.flags);
        WriteLog(
            " Memcell: " + targetcell->name() + " - " + targetcell->Values());
        //        targetcell->LogValues();
        if (targetcell->Track == nullptr) {
            continue;
        }
        // McZapkie-100302 - updatevalues oprocz zmiany wartosci robi putcommand
        // dla wszystkich 'dynamic' na danym torze
        auto const location{targetcell->location()};
        for (auto vehicle : targetcell->Track->Dynamics) {
            if (vehicle->Mechanik) {
                WriteLog(" Vehicle: [" + vehicle->name() + "]");
                targetcell->PutCommand(vehicle->Mechanik, &location);
            }
        }
    }
...
void updatevalues_event::export_as_text_(std::ostream &Output) const {
    Output << ((m_input.flags & flags::text) == 0 ? "*" : m_input.data_text)
           << ' '
           << ((m_input.flags & flags::value1) == 0
                   ? "*"
                   : to_string(m_input.data_value_1))
           << ' '
           << ((m_input.flags & flags::value2) == 0
                   ? "*"
                   : to_string(m_input.data_value_2))
           << ' ';
...
// returns: true if the event should be executed immediately
bool updatevalues_event::is_instant() const {
    return (((m_input.flags & flags::mode_add) != 0) && (m_delay == 0.0));
}

// prepares event for use
void getvalues_event::init() {
    init_targets(simulation::Memory, "memory cell");
    ...
    for (auto &target : m_targets) {
        ...
        if (targetcell->IsVelocity()) {
            // jeśli odczyt komórki a komórka zawiera komendę SetVelocity albo
            // ShuntVelocity to event nie będzie dodawany do kolejki
            m_passive = true;
        }
    }
    if (m_targets.empty()) {
...
// event type string
std::string getvalues_event::type() const { return "getvalues"; }

// deserialize() subclass details
void getvalues_event::deserialize_(
    cParser &Input, scene::scratch_data &Scratchpad) {
...
// run() subclass details
void getvalues_event::run_() {
    auto const *cell{m_input.data_cell()};

    WriteLog(
        "Type: GetValues - " + cell->name() + " - [" + cell->Text() + "] [" +
        to_string(cell->Value1(), 2) + "] [" + to_string(cell->Value2(), 2) +
        "]");
    ...
    cell->PutCommand(m_activator->Mechanik, &(cell->location()));
...
// export_as_text() subclass details
void getvalues_event::export_as_text_(std::ostream &Output) const {
    // nothing to do here
}

// sends content of associated data cell to specified vehicle controller
void getvalues_event::send_command(TController &Controller) {
    Controller.PutCommand(
        input_text(), input_value(1), input_value(2), nullptr);
    m_input.data_cell()
        ->StopCommandSent();  // komenda z komórki została wysłana
}
// returns: true if associated data cell contains a command for vehicle
// controller
bool getvalues_event::is_command() const {
    // info o komendzie z komórki
    return m_input.data_cell()->StopCommand();
}

// input data access
std::string getvalues_event::input_text() const {
    return m_input.data_cell()->Text();
}

TCommandType getvalues_event::input_command() const {
    return m_input.data_cell()->Command();
}

double getvalues_event::input_value(int Index) const {
    Index &= 1;  // tylko 1 albo 2 jest prawidłowy
    return (
        Index == 1 ? m_input.data_cell()->Value1()
                   : m_input.data_cell()->Value2());
}

glm::dvec3 getvalues_event::input_location() const {
    return m_input.data_cell()
        ->location();  // współrzędne podłączonej komórki pamięci
}

// prepares event for use
void putvalues_event::init() {
    // nothing to do here
}

// event type string
std::string putvalues_event::type() const { return "putvalues"; }

// deserialize() subclass details
void putvalues_event::deserialize_(
    cParser &Input, scene::scratch_data &Scratchpad) {
    Input.getTokens(3);
    // location, previously held in param 3, 4, 5
    Input >> m_input.location.x >> m_input.location.y >> m_input.location.z;
    // przesunięcie
    // tmp->pCenter.RotateY(aRotate.y/180.0*M_PI); //Ra 2014-11: uwzględnienie
    // rotacji TBD, TODO: take into account rotation as well?
...
// run() subclass details
void putvalues_event::run_() {
    WriteLog(
        "Type: PutValues - [" + m_input.data_text + "] [" +
    ...
    TLocation const loc{
        -m_input.location.x, m_input.location.z, m_input.location.y};

    if (m_activator->Mechanik) {
        // przekazanie rozkazu do AI
        WriteLog(
            " Vehicle: [" + m_activator->Mechanik->Vehicle()->name() + "]");
        m_activator->Mechanik->PutCommand(
            m_input.data_text, m_input.data_value_1, m_input.data_value_2, loc);
    } else if ((m_activator->ctOwner) && (is_command_for_owner(m_input))) {
        // send the command to consist owner,
        // we're acting on presumption there's hardly ever need to issue command
        // to unmanned vehicle and the intended recipient moved between vehicles
...
// export_as_text() subclass details
void putvalues_event::export_as_text_(std::ostream &Output) const {
    Output
        // location
        << m_input.location.x << ' ' << m_input.location.y << ' '
        << m_input.location.z
        << ' '
        // command
        << m_input.data_text << ' ' << m_input.data_value_1 << ' '
        << m_input.data_value_2 << ' ';
}

// determines whether provided input should be passed to consist owner
bool putvalues_event::is_command_for_owner(input_data const &Input) const {
    if (starts_with(Input.data_text, "Load=")) {
        return false;
    }
    if (starts_with(Input.data_text, "UnLoad=")) {
        return false;
    }
...
// input data access
std::string putvalues_event::input_text() const { return m_input.data_text; }

TCommandType putvalues_event::input_command() const {
    return m_input.command_type;  // komenda zakodowana binarnie
}

double putvalues_event::input_value(int Index) const {
    Index &= 1;  // tylko 1 albo 2 jest prawidłowy
    return (Index == 1 ? m_input.data_value_1 : m_input.data_value_2);
}

glm::dvec3 putvalues_event::input_location() const { return m_input.location; }

// prepares event for use
void copyvalues_event::init() {
    // skopiowanie komórki do innej
    init_targets(simulation::Memory, "memory cell");
    // source cell
    std::get<scene::basic_node *>(m_input.data_source) =
        simulation::Memory.find(std::get<std::string>(m_input.data_source));
    if (std::get<scene::basic_node *>(m_input.data_source) == nullptr) {
        m_ignored = true;  // deaktywacja
        ErrorLog(
            "Bad event: \"" + m_name + "\" (type: " + type() +
            ") can't find memory cell \"" +
            std::get<std::string>(m_input.data_source) + "\"");
    }
}

// event type string
std::string copyvalues_event::type() const { return "copyvalues"; }

// deserialize() subclass details
void copyvalues_event::deserialize_(
    cParser &Input, scene::scratch_data &Scratchpad) {
    m_input.flags =
        (flags::text | flags::value1 | flags::value2);  // normalnie trzy
    ...
    while ((true == Input.getTokens()) &&
        ...
        Input >> token;
        switch (++paramidx) {
            case 1: {  // nazwa drugiej komórki (źródłowej) // previously stored
                       // in param 9
                std::get<std::string>(m_input.data_source) = token;
                break;
            }
            case 2: {  // maska wartości
                m_input.flags = stol_def(
                    token, (flags::text | flags::value1 | flags::value2));
                break;
            }
            default: {
                break;
...
// TODO: update and copy values run_ methods are largely identical, refactor to
// a single helper
void copyvalues_event::run_() {
    // skopiowanie wartości z innej komórki
    auto const *datasource{static_cast<TMemCell *>(
        std::get<scene::basic_node *>(m_input.data_source))};
...
// export_as_text() subclass details
void copyvalues_event::export_as_text_(std::ostream &Output) const {
    auto const *datasource{static_cast<TMemCell *>(
        std::get<scene::basic_node *>(m_input.data_source))};
    Output << (datasource != nullptr
                   ? datasource->name()
                   : std::get<std::string>(m_input.data_source))
           << ' '
...
// prepares event for use
void whois_event::init() { init_targets(simulation::Memory, "memory cell"); }

// event type string
std::string whois_event::type() const { return "whois"; }

// deserialize() subclass details
void whois_event::deserialize_(
    cParser &Input, scene::scratch_data &Scratchpad) {
    m_input.flags =
        (flags::text | flags::value1 | flags::value2);  // normalnie trzy
...
void whois_event::run_() {
    for (auto &target : m_targets) {
        ...
        // event effect code
        // +40: next station name, unused, stop at next station (duplicate of +0
        // 2nd numeric value) +32: vehicle name +24: vehicle type, consist brake
        // level, obstacle distance +16: load type, load amount, max load amount
        ...
        if (m_input.flags & flags::whois_name) {
            ...
            if (m_input.flags & flags::mode_alt) {
                auto const *owner{
                    (((m_activator->Mechanik != nullptr) &&
                      (m_activator->Mechanik->primary()))
                         ? m_activator->Mechanik
                         : m_activator->ctOwner)};
                auto const nextstop{
                    (owner != nullptr ? owner->TrainTimetable().NextStop()
                                      : "none")};
                auto const isstop{
                ...
                targetcell->UpdateValues(
                    ...
                    m_input.flags &
                        (flags::text | flags::value1 | flags::value2));

                WriteLog(
                    "Type: WhoIs (" + to_string(m_input.flags) + ") - " +
                    "[next station: " + nextstop + "], " + "[X], " +
            ...
            if (m_input.flags & flags::mode_alt) {
                ...
                auto const consistbrakelevel{
                    (owner != nullptr ? owner->fReady : -1.0)};
                auto const collisiondistance{
                    (owner != nullptr ? owner->TrackObstacle() : -1.0)};

                targetcell->UpdateValues(
                    m_activator->MoverParameters->TypeName,  // typ pojazdu
                    consistbrakelevel, collisiondistance,
                    m_input.flags &
                        (flags::text | flags::value1 | flags::value2));
                ...
                WriteLog(
                    ...
                    "], " + "[obstacle distance: " +
                    to_string(collisiondistance, 2) + " m]");
            } else {
                // jeśli parametry ładunku
                targetcell->UpdateValues(
                    m_activator->MoverParameters->LoadType
                        .name,                                 // nazwa ładunku
                ...
                WriteLog(
                    "Type: WhoIs (" + to_string(m_input.flags) + ") - " +
                    "[load type: " +
                    m_activator->MoverParameters->LoadType.name + "], " +
                    "[current load: " +
                    to_string(m_activator->MoverParameters->LoadAmount, 2) +
                    "], " + "[max load: " +
                    to_string(m_activator->MoverParameters->MaxLoad, 2) + "]");
            }
        }
        // +8
        else if (m_input.flags & flags::mode_alt) {  // jeśli miejsce docelowe
                                                     // pojazdu
            targetcell->UpdateValues(
                m_activator->asDestination,  // adres docelowy
                m_activator->DirectionGet(),  // kierunek pojazdu względem czoła
                                              // składu (1=zgodny,-1=przeciwny)
                m_activator->MoverParameters
            ...
            WriteLog(
                ...
                "], " + "[engine power: " +
                to_string(m_activator->MoverParameters->Power, 2) + "]");
        }
        // +0
        else if (m_activator->Mechanik) {
            if (m_activator->Mechanik
                    ->primary()) {  // tylko jeśli ktoś tam siedzi - nie powinno
                                    // dotyczyć pasażera!
                targetcell->UpdateValues(
                    m_activator->Mechanik->TrainName(),
                    m_activator->Mechanik->StationCount() -
                        m_activator->Mechanik
                ...
                WriteLog(
                    ...
                    to_string(
                        m_activator->Mechanik->StationCount() -
                        m_activator->Mechanik->StationIndex()) +
                    "], " + "[stop at next station: " +
                    (m_activator->Mechanik->IsStop() ? "yes" : "no") + "]");
            }
        }
    }
}
...
// export_as_text() subclass details
void whois_event::export_as_text_(std::ostream &Output) const {
    Output << (m_input.flags & (flags::text | flags::value1 | flags::value2))
           << ' ';
}

// prepares event for use
void logvalues_event::init() {
    init_targets(simulation::Memory, "memory cell");
}

// event type string
std::string logvalues_event::type() const { return "logvalues"; }

// deserialize() subclass details
void logvalues_event::deserialize_(
    cParser &Input, scene::scratch_data &Scratchpad) {
    // nothing to do here, just preload next token
    Input.getTokens();
}
...
// export_as_text() subclass details
void logvalues_event::export_as_text_(std::ostream &Output) const {}

// prepares event for use
void multi_event::init() {
    auto const conditiontchecksmemcell{
        (m_conditions.flags & (flags::text | flags::value1 | flags::value2)) !=
        0};
    // not all multi-events have memory cell checks, for the ones which don't we
    // can keep quiet about it
    init_targets(simulation::Memory, "memory cell", conditiontchecksmemcell);
    if (m_ignored) {
        // legacy compatibility behaviour, instead of disabling the event we
        // disable the memory cell comparison test
        m_conditions.flags &= ~(flags::text | flags::value1 | flags::value2);
        m_ignored = false;
    }
    // conditional data
    ...
    for (auto &childevent : m_children) {
        std::get<basic_event *>(childevent) =
            simulation::Events.FindEvent(std::get<std::string>(childevent));
        if (std::get<basic_event *>(childevent) == nullptr) {
            ErrorLog(
                "Bad event: \"" + m_name + "\" (type: " + type() +
                ") can't find event \"" + std::get<std::string>(childevent) +
                "\"");
        }
    }
}
...
// event type string
std::string multi_event::type() const { return "multiple"; }

// deserialize() subclass details
void multi_event::deserialize_(
    cParser &Input, scene::scratch_data &Scratchpad) {
    ...
    while ((true == Input.getTokens()) &&
        ...
        if (token == "condition") {
            ...
            if (starts_with(token, "none_")) {
                ...
                WriteLog(
                    "Multi-event \"" + m_name + "\" ignored link to event \"" +
                    token + "\"");
            } else {
                m_children.emplace_back(
                    token, nullptr, (m_conditions.has_else == false));
            }
        }
    }
}
...
// run() subclass details
void multi_event::run_() {
    auto const conditiontest{m_conditions.test()};
    if (conditiontest || m_conditions.has_else) {
        ...
        for (auto &childwrapper : m_children) {
            ...
            if (std::get<bool>(childwrapper) != conditiontest) {
                continue;
            }
            ...
            if (childevent != this) {
                ...
                // jeśli ma być rekurencja to musi mieć sensowny okres
                // powtarzania
                if (m_delay >= 5.0) {
...
// export_as_text() subclass details
void multi_event::export_as_text_(std::ostream &Output) const {
    for (auto const &childevent : m_children) {
        if (true == std::get<bool>(childevent)) {
            auto *childeventdata{std::get<basic_event *>(childevent)};
            Output << (childeventdata != nullptr
...
// prepares event for use
void sound_event::init() {
    // odtworzenie dźwięku
    for (auto &target : m_sounds) {
        std::get<sound_source *>(target) =
            simulation::Sounds.find(std::get<std::string>(target));
        if (std::get<sound_source *>(target) == nullptr) {
            m_ignored = true;  // deaktywacja
            ErrorLog(
                "Bad event: \"" + m_name + "\" (type: " + type() +
                ") can't find static sound \"" + std::get<std::string>(target) +
                "\"");
        }
    }
}
...
// event type string
std::string sound_event::type() const { return "sound"; }

// deserialize() subclass details
void sound_event::deserialize_(
    cParser &Input, scene::scratch_data &Scratchpad) {
    Input.getTokens();
    // playback mode, previously held in param 0 // 0: wylaczyc, 1: wlaczyc; -1:
    // wlaczyc zapetlone
    Input >> m_soundmode;
    Input.getTokens();
...
void sound_event::run_() {
    WriteLog(
        ...
        (m_soundradiochannel > 0
             ? " [channel " + to_string(m_soundradiochannel) + "]"
             : ""));
    for (auto &target : m_sounds) {
        ...
        switch (m_soundmode) {
            ...
            case 1: {
                if (m_soundradiochannel > 0) {
                    simulation::radio_message(targetsound, m_soundradiochannel);
                } else {
                    targetsound->play(
                        sound_flags::exclusive | sound_flags::event);
                }
                break;
            }
            case -1: {
...
// export_as_text() subclass details
void sound_event::export_as_text_(std::ostream &Output) const {
    // playback mode
    Output << m_soundmode << ' ';
...
void sound_event::deserialize_targets(std::string const &Input) {
    // sound objects don't inherit from scene node, so we can't use the same
    // container
    // TODO: fix this, having sounds as scene nodes (or a node wrapper for one)
    // might have benefits elsewhere
    cParser targetparser{Input};
    std::string target;
    while (false ==
           (target = targetparser.getToken<std::string>(true, "|,")).empty()) {
...
TMemCell const *texture_event::input_data::data_cell() const {
    return static_cast<TMemCell const *>(
        std::get<scene::basic_node *>(data_source));
}
TMemCell *texture_event::input_data::data_cell() {
    return static_cast<TMemCell *>(std::get<scene::basic_node *>(data_source));
}

// prepares event for use
void texture_event::init() {
    // target models
    ...
    if (inputcellname != "none") {
        ...
        if (inputcell == nullptr) {
            ErrorLog(
                "Bad event: \"" + m_name + "\" (type: " + type() +
                ") can't find memory cell \"" + inputcellname + "\"");
        }
    }
...
// event type string
std::string texture_event::type() const { return "texture"; }

// deserialize() subclass details
void texture_event::deserialize_(
    cParser &Input, scene::scratch_data &Scratchpad) {
    Input.getTokens(3);
    Input >> m_skinindex >> m_skin >>
        std::get<std::string>(m_input.data_source);

    // validate input
    if (m_skinindex < 0) {
        // intercept potential error, index specified using .t3d format
        // convention
        m_skinindex *= -1;
    }
    m_skinindex = clamp(
        m_skinindex, 1,
        4);  // TODO: define-based upper bound in case of future extension

    Input.getTokens();  // preload next token
}

// run() subclass details
void texture_event::run_() {
    material_handle material{null_handle};

    if (m_input.data_cell() == nullptr) {
        // straightforward variant without parameters, we can pass expression
        // directly
        material = GfxRenderer->Fetch_Material(m_skin);
    } else {
        // variant with memory cell: pass the cell content as parameters and
        // generate filename
        cParser expression(
            m_skin, cParser::buffer_TEXT, "", true,
            {m_input.data_cell()->Text(),
             to_string(
                 m_input.data_cell()->Value1(),
                 0),  // memory cell values are passed as ints
             to_string(m_input.data_cell()->Value2(), 0)});
        material = GfxRenderer->Fetch_Material(
            expression.getToken<std::string>(false, "\n\r"));
    }
    // assign received material to target models
    for (auto &target : m_targets) {
        auto *targetmodel =
            static_cast<TAnimModel *>(std::get<scene::basic_node *>(target));
...
// export_as_text() subclass details
void texture_event::export_as_text_(std::ostream &Output) const {
    // playback mode
    Output << m_skinindex << ' ' << m_skin << ' '
           << std::get<std::string>(m_input.data_source) << ' ';
}
...
void animation_event::init() {
    ...
    for (auto &target : m_targets) {
        ...
        if (!targetcontainer) {
            ...
            ErrorLog(
                "Bad event: \"" + m_name + "\" (type: " + type() +
                ") can't find submodel " + m_animationsubmodel +
                " in model instance \"" + targetmodel->name() + "\"");
            break;
        }
        targetcontainer->WillBeAnimated();  // oflagowanie animacji
        if (targetcontainer->Event() == nullptr) {
            // nie szukać, gdy znaleziony
            targetcontainer->EventAssign(simulation::Events.FindEvent(
...
// event type string
std::string animation_event::type() const { return "animation"; }

// deserialize() subclass details
void animation_event::deserialize_(
    cParser &Input, scene::scratch_data &Scratchpad) {
    std::string token;
    ...
    if (token == "rotate") {  // obrót względem osi
        ...
        Input >> m_animationparams[0] >> m_animationparams[1] >>
            m_animationparams[2] >> m_animationparams[3];
    ...
    {                                     // animacja z pliku VMD
        {
            ...
            std::ifstream file(
                szModelPath + m_animationfilename,
                std::ios::binary | std::ios::ate);
            file.unsetf(std::ios::skipws);
            auto size = file.tellg();  // ios::ate already positioned us at the
                                       // end of the file
...
// export_as_text() subclass details
void animation_event::export_as_text_(std::ostream &Output) const {
    // animation type
    Output << (m_animationtype == 1   ? "rotate"
    ...
    Output << m_animationparams[0] << ' ' << m_animationparams[1] << ' '
           << m_animationparams[2] << ' ' << m_animationparams[3] << ' ';
}

// prepares event for use
void lights_event::init() {
    // zmiana świeteł modelu
    init_targets(simulation::Instances, "model instance");
}

// event type string
std::string lights_event::type() const { return "lights"; }

// deserialize() subclass details
void lights_event::deserialize_(
    cParser &Input, scene::scratch_data &Scratchpad) {
    ...
    while ((true == Input.getTokens()) &&
        ...
        if (lightidx < lightcountlimit) {
            ...
            ErrorLog(
                "Bad event: \"" + m_name + "\" (type: " + type() +
                ") with more than " + to_string(lightcountlimit) +
                " parameters");
        }
    }
    ...
    for (auto &target : m_targets) {
        ...
        for (auto lightidx = 0; lightidx < iMaxNumLights; ++lightidx) {
            ...
            if (m_lights[lightidx] != -1.f) {
                ...
                targetmodel->LightSet(lightidx, m_lights[lightidx]);
            }
        }
    }
}
...
// export_as_text() subclass details
void lights_event::export_as_text_(std::ostream &Output) const {
    auto lightidx{0};
...
// prepares event for use
void switch_event::init() {
    // przełożenie zwrotnicy albo zmiana stanu obrotnicy
    init_targets(simulation::Paths, "track");
    // custom target initialization code
    for (auto &target : m_targets) {
        auto *targettrack =
            static_cast<TTrack *>(std::get<scene::basic_node *>(target));
        if (targettrack == nullptr) {
            continue;
        }
        // dowiązanie toru
        if (targettrack->iAction == 0) {
            // jeśli nie jest zwrotnicą ani obrotnicą to będzie się zmieniał
            // stan uszkodzenia
            targettrack->iAction |= 0x100;
        }
        if ((m_switchstate == 0) && (m_switchmovedelay >= 0.0)) {
            // jeśli przełącza do stanu 0 & jeśli jest zdefiniowany dodatkowy
            // ruch iglic przesłanie parametrów
            targettrack->Switch(
                m_switchstate, m_switchmoverate, m_switchmovedelay);
        }
    }
}

// event type string
std::string switch_event::type() const { return "switch"; }

// deserialize() subclass details
void switch_event::deserialize_(
    cParser &Input, scene::scratch_data &Scratchpad) {
    Input.getTokens();
...
void switch_event::run_() {
    for (auto &target : m_targets) {
        ...
        // event effect code
        targettrack->Switch(m_switchstate, m_switchmoverate, m_switchmovedelay);
    }
    if (Global.iMultiplayer) {
        // dajemy znać do serwera o przełożeniu
        multiplayer::WyslijEvent(
            m_name, "");  // wysłanie nazwy eventu przełączajacego
    }
    // Ra: bardziej by się przydała nazwa toru, ale nie ma do niej stąd dostępu
}

// export_as_text() subclass details
void switch_event::export_as_text_(std::ostream &Output) const {
    Output << m_switchstate << ' ';
    if ((m_switchmoverate < 0.f) || (m_switchmovedelay < 0.f)) {
...
// event type string
std::string track_event::type() const { return "trackvel"; }

// deserialize() subclass details
void track_event::deserialize_(
    cParser &Input, scene::scratch_data &Scratchpad) {
    Input.getTokens();
    Input >> m_velocity;
    // preload next token
...
void track_event::run_() {
    ...
    for (auto &target : m_targets) {
        ...
        if (DebugModeFlag) {
            WriteLog(
                "actual track velocity for " + targettrack->name() + " [" +
                to_string(targettrack->VelocityGet(), 2) + "]");
        }
    }
}

// export_as_text() subclass details
void track_event::export_as_text_(std::ostream &Output) const {
    Output << m_velocity << ' ';
}

// prepares event for use
void voltage_event::init() {
    // zmiana napięcia w zasilaczu (TractionPowerSource)
    init_targets(simulation::Powergrid, "power source");
}

// event type string
std::string voltage_event::type() const { return "voltage"; }

// deserialize() subclass details
void voltage_event::deserialize_(
    cParser &Input, scene::scratch_data &Scratchpad) {
    // zmiana napięcia w zasilaczu (TractionPowerSource)
    Input.getTokens();
    Input >> m_voltage;
...
// run() subclass details
void voltage_event::run_() {
    // zmiana napięcia w zasilaczu (TractionPowerSource)
    WriteLog("Type: Voltage [" + to_string(m_voltage, 2) + "]");
    for (auto &target : m_targets) {
        auto *targetpowersource = static_cast<TTractionPowerSource *>(
            std::get<scene::basic_node *>(target));
        if (targetpowersource == nullptr) {
            continue;
        }
...
// export_as_text() subclass details
void voltage_event::export_as_text_(std::ostream &Output) const {
    Output << m_voltage << ' ';
}

// prepares event for use
void visible_event::init() {
    ...
    for (auto &target : m_targets) {
        ...
        if (targetnode == nullptr) {
            ...
            ErrorLog(
                "Bad event: \"" + m_name + "\" (type: " + type() +
                ") can't find item \"" + std::get<std::string>(target) + "\"");
        }
    }
}

// event type string
std::string visible_event::type() const { return "visible"; }

// deserialize() subclass details
void visible_event::deserialize_(
    cParser &Input, scene::scratch_data &Scratchpad) {
...
// export_as_text() subclass details
void visible_event::export_as_text_(std::ostream &Output) const {
    Output << (m_visible ? 1 : 0) << ' ';
}

// prepares event for use
void friction_event::init() {
    // nothing to do here
}

// event type string
std::string friction_event::type() const { return "friction"; }

// deserialize() subclass details
void friction_event::deserialize_(
    cParser &Input, scene::scratch_data &Scratchpad) {
...
// export_as_text() subclass details
void friction_event::export_as_text_(std::ostream &Output) const {
    Output << m_friction << ' ';
}

#ifdef WITH_LUA
lua_event::lua_event(lua::eventhandler_t func) { lua_func = func; }
...
// event type string
std::string lua_event::type() const { return "lua"; }
...
void lua_event::run_() {
    try {
        if (lua_func) lua_func(this, m_activator);
    } catch (...) {
...
// prepares event for use
void message_event::init() {
    // nothing to do here
}

// event type string
std::string message_event::type() const { return "message"; }

// deserialize() subclass details
void message_event::deserialize_(
    cParser &Input, scene::scratch_data &Scratchpad) {
...
// run() subclass details
void message_event::run_() {
    // TODO: implement
}

// export_as_text() subclass details
void message_event::export_as_text_(std::ostream &Output) const {
    Output << '\"' << m_message << '\"' << ' ';
}
...
basic_event *make_event(cParser &Input, scene::scratch_data &Scratchpad) {
    ...
    if (type == "addvalues") {
        event = new updatevalues_event();
    } else if (type == "updatevalues") {
        event = new updatevalues_event();
    } else if (type == "copyvalues") {
        event = new copyvalues_event();
    } else if (type == "getvalues") {
        event = new getvalues_event();
    } else if (type == "putvalues") {
        event = new putvalues_event();
    } else if (type == "whois") {
        event = new whois_event();
    } else if (type == "logvalues") {
        event = new logvalues_event();
    } else if (type == "multiple") {
        event = new multi_event();
    } else if (type == "switch") {
        event = new switch_event();
    } else if (type == "trackvel") {
        event = new track_event();
    } else if (type == "sound") {
        event = new sound_event();
    } else if (type == "texture") {
        event = new texture_event();
    } else if (type == "animation") {
        event = new animation_event();
    } else if (type == "lights") {
        event = new lights_event();
    } else if (type == "voltage") {
        event = new voltage_event();
    } else if (type == "visible") {
        event = new visible_event();
    } else if (type == "friction") {
        event = new friction_event();
    } else if (type == "message") {
        event = new message_event();
    }

    if (event == nullptr) {
        ErrorLog(
            "Bad event: unrecognized type \"" + type +
            "\" specified for event \"" + name + "\".");
        return event;
    }

    event->m_name = name;
    if (type == "addvalues") {
        static_cast<updatevalues_event *>(event)->m_input.flags =
            basic_event::flags::mode_add;
    }
    return event;
}
...
void event_manager::queue(TEventLauncher *Launcher) {
...
void event_manager::queue_receivers(
    ...
    for (auto *launcher : m_radiodrivenlaunchers.sequence()) {
        if ((launcher->key() == Message) &&
            ((launcher->dRadius < 0) ||
             (glm::length2(launcher->location() - Location) <
              launcher->dRadius)) &&
...
bool event_manager::insert(basic_event *Event) {
    ...
    if (lookup != m_eventmap.end()) {
        ...
        if (Global.bJoinEvents) {
            ...
            // NOTE: somewhat convoluted way to deal with 'replacing' events
            // without leaving dangling pointers can be cleaned up if pointers
            // to events were replaced with handles
            ErrorLog(
                "Bad scenario: duplicate event name \"" + Event->m_name + "\"");
            duplicate->append(
                Event);  // doczepka (taki wirtualny multiple bez warunków)
            duplicate->m_ignored = true;  // dezaktywacja pierwotnego - taka
                                          // proteza na wsteczną zgodność
        }
    }

    m_events.emplace_back(Event);
    if (lookup == m_eventmap.end()) {
        // if it's first event with such name, it's potential candidate for the
        // execution queue
...
basic_event *event_manager::FindEvent(std::string const &Name) {
    return FindEventById(GetEventId(Name));
}

// legacy method, inserts specified event in the event query
bool event_manager::AddToQuery(
    basic_event *Event, TDynamicObject const *Owner, double delay) {
    if (Event->m_passive) {
        return false;
    }  // jeśli może być dodany do kolejki (nie używany w skanowaniu)
    if (Event->m_inqueue > 0) {
        return false;
    ...
    if (Event->is_instant()) {
        // eventy AddValues trzeba wykonywać natychmiastowo, inaczej kolejka
        // może zgubić jakieś dodawanie
        if (false == Event->m_ignored) {
            Event->run();
        }
        // jeśli jest kolejny o takiej samej nazwie, to idzie do kolejki (and if
        // there's no joint event it'll be set to null and processing will end
        // here)
        do {
            Event = Event->m_sibling;
            // NOTE: we could've received a new event from joint event above, so
            // we need to check conditions just in case and discard the bad
            // events
            // TODO: refactor this arrangement, it's hardly optimal
        } while ((Event != nullptr) &&
                 ((Event->m_passive) || (Event->m_inqueue > 0)));
    }
    if ((Event != nullptr) && (false == Event->m_ignored)) {
        // standardowe dodanie do kolejki
        ++(Event->m_inqueue);  // zabezpieczenie przed podwójnym dodaniem do
                               // kolejki
        WriteLog(
            "EVENT ADDED TO QUEUE" + (Owner ? (" by " + Owner->asName) : "") +
            ": " + Event->m_name);
        Event->m_launchtime =
            delay + std::abs(Event->m_delay) +
            Timer::GetTime();  // czas od uruchomienia scenerii
        ...
        if ((Owner != nullptr) &&
            ...
            if (timetableowner != nullptr) {
                ...
                Event->m_launchtime +=
                    timetable.seconds_until_departure(
                        time.wHour, time.wMinute + time.wSecond * 0.0167) +
                    Event->m_delaydeparture;
            }
        }
        // NOTE: sanity check, as departure-based delay math can potentially
        // produce negative overall delay
        ...
        if (QueryRootEvent != nullptr) {
            ...
            if (Event->m_launchtime >= target->m_launchtime) {
                ...
                target->m_next = Event;
                // if we have resurrected event land at the end of list, the
                // link from previous run could potentially "add" unwanted
                // events to the queue
...
bool event_manager::CheckQuery() {
    while ((QueryRootEvent != nullptr) &&
           (QueryRootEvent->m_launchtime <
            Timer::GetTime())) {  // eventy są posortowana wg czasu wykonania
        m_workevent = QueryRootEvent;  // wyjęcie eventu z kolejki
        if (QueryRootEvent
                ->m_sibling)  // jeśli jest kolejny o takiej samej nazwie
        {                     // to teraz on będzie następny do wykonania
            QueryRootEvent =
                QueryRootEvent->m_sibling;  // następny będzie ten doczepiony
            QueryRootEvent->m_next =
                m_workevent->m_next;  // pamiętając o następnym z kolejki
            QueryRootEvent->m_launchtime =
                m_workevent->m_launchtime;  // czas musi być ten sam, bo nie
                                            // jest aktualizowany
            QueryRootEvent->m_activator =
                m_workevent->m_activator;  // pojazd aktywujący
            QueryRootEvent->m_inqueue = 1;
            // w sumie można by go dodać normalnie do kolejki, ale trzeba te
            // połączone posortować wg czasu wykonania
        } else  // a jak nazwa jest unikalna, to kolejka idzie dalej
            QueryRootEvent =
                QueryRootEvent->m_next;  // NULL w skrajnym przypadku
        if ((false == m_workevent->m_ignored) &&
            (false == m_workevent->m_passive)) {
            // w zasadzie te wyłączone są skanowane i nie powinny się nigdy w
            // kolejce znaleźć
            --(m_workevent
                   ->m_inqueue);  // teraz moze być ponownie dodany do kolejki
            m_workevent->run();
...
// scenario file
void event_manager::InitLaunchers() {
    std::vector<basic_table<TEventLauncher> *> launchertables{
        &m_inputdrivenlaunchers, &m_radiodrivenlaunchers};
        ...
        for (auto *launcher : launchertable->sequence()) {
            if (launcher->iCheckMask != 0) {
                if (launcher->asMemCellName != "none") {
                    ...
                    launcher->MemCell =
                        simulation::Memory.find(launcher->asMemCellName);
                    if (launcher->MemCell == nullptr) {
                        ErrorLog(
                            "Bad scenario: event launcher \"" +
                            launcher->name() + "\" can't find memcell \"" +
                            launcher->asMemCellName + "\"");
                    }
                } else {
            ...
            if (launcher->asEvent1Name != "none") {
                ...
                if (launcher->Event1 == nullptr) {
                    ErrorLog(
                        "Bad scenario: event launcher \"" + launcher->name() +
                        "\" can't find event \"" + launcher->asEvent1Name +
                        "\"");
                }
            }
            ...
            if (launcher->asEvent2Name != "none") {
                ...
                if (launcher->Event2 == nullptr) {
                    ErrorLog(
                        "Bad scenario: event launcher \"" + launcher->name() +
                        "\" can't find event \"" + launcher->asEvent2Name +
                        "\"");
                }
            }
        }
...
// sends basic content of the class in legacy (text) format to provided stream
void event_manager::export_as_text(std::ostream &Output) const {
    Output << "// events\n";
...
std::vector<TEventLauncher *> event_manager::find_eventlaunchers(
