...
gl::postfx::postfx(const std::string &s)
    : postfx(shader("postfx_" + s + ".frag")) {}

gl::postfx::postfx(const shader &s) {
    if (!vertex) vertex = std::make_shared<gl::shader>("quad.vert");
    if (!vao) vao = std::make_shared<gl::vao>();
...
void gl::postfx::apply(std::vector<opengl_texture *> src, framebuffer *dst) {
    ...
    size_t unit = 0;
    for (opengl_texture *tex : src) tex->bind(unit++);
