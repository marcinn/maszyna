...
void gl::buffer::bind(targets target) {
    if (binding_points[target] == *this) return;
...
gl::buffer::buffer() { glGenBuffers(1, *this); }

gl::buffer::~buffer() { glDeleteBuffers(1, *this); }
...
void gl::buffer::upload(
    targets target, const void *data, int offset, int size) {
    bind(target);
...
void gl::buffer::download(targets target, void *data, int offset, int size) {
    ...
    if (GLAD_GL_VERSION_3_3) {
        ...
        void *glbuf = glMapBufferRange(
            glenum_target(target), offset, size, GL_MAP_READ_BIT);
