...
std::string gl::shader::read_file(const std::string &filename) {
    ...
    if (str.empty()) throw shader_exception("cannot read shader: " + filename);
...
void gl::shader::expand_includes(std::string &str, const std::string &basedir) {
    ...
    while ((start_pos = str.find(magic, start_pos)) != str.npos) {
        ...
        if (fp == str.npos || fe == str.npos) return;
...
std::unordered_map<std::string, gl::shader::components_e>
    gl::shader::components_mapping = {
        {"R", components_e::R},       {"RG", components_e::RG},
        {"RGB", components_e::RGB},   {"RGBA", components_e::RGBA},
        {"sRGB", components_e::sRGB}, {"sRGB_A", components_e::sRGB_A}};

std::unordered_map<std::string, gl::shader::defaultparam_e>
    gl::shader::defaultparams_mapping = {
...
std::pair<GLuint, std::string> gl::shader::process_source(
    ...
    str += "vec4 FBOUT(vec4 x) { return " +
           (Global.gfx_shadergamma
                ? std::string("vec4(pow(x.rgb, vec3(1.0 / 2.2)), x.a)")
                : std::string("x")) +
           "; }\n";
    ...
    while ((start_pos = str.find(magic, start_pos)) != str.npos) {
        ...
        if (arg == 3) {
            if (name.empty())
                ...
                log_error(
                    "invalid texture binding: " + std::to_string(conf.id));
            else
    ...
    while ((start_pos = str.find(magic, start_pos)) != str.npos) {
        ...
        while (std::getline(ss, token, ',')) {
            ...
            if (arg == 0)
                ...
                std::string tok;
                token_ss >> tok;
                if (defaultparams_mapping.find(tok) ==
                    defaultparams_mapping.end())
        ...
        if (arg == 5) {
            if (name.empty())
                ...
                log_error(
                    "invalid param binding: " + std::to_string(conf.location));
...
gl::shader::shader(const std::string &filename) {
    ...
    std::pair<GLuint, std::string> source =
        process_source(filename, "shaders/");
    ...
    **this = glCreateShader(source.first);
    ...
    if (!status) {
        ...
        throw shader_exception(
            "failed to compile " + filename + ": " + std::string(info));
    }
}
...
void gl::program::init() {
    ...
    if ((index = glGetUniformBlockIndex(*this, "scene_ubo")) !=
        GL_INVALID_INDEX)
        glUniformBlockBinding(*this, index, 0);

    if ((index = glGetUniformBlockIndex(*this, "model_ubo")) !=
        GL_INVALID_INDEX)
        glUniformBlockBinding(*this, index, 1);

    if ((index = glGetUniformBlockIndex(*this, "light_ubo")) !=
        GL_INVALID_INDEX)
        glUniformBlockBinding(*this, index, 2);
}

gl::program::program() { **this = glCreateProgram(); }

gl::program::program(
    std::vector<std::reference_wrapper<const gl::shader>> shaders)
    : program() {
    for (const gl::shader &s : shaders) attach(s);
    link();
}
...
gl::program::~program() { glDeleteProgram(*this); }

void gl::program::bind(GLuint i) { glUseProgram(i); }
