...
void gl::vao::setup_attrib(
    gl::buffer &buffer, int attrib, int size, int type, int stride,
    int offset) {
    if (use_vao) {
        bind();
        buffer.bind(buffer::ARRAY_BUFFER);
        glVertexAttribPointer(
            attrib, size, type, GL_FALSE, stride,
            reinterpret_cast<void *>(offset));
        glEnableVertexAttribArray(attrib);
    } else {
        if (attrib == 0) params.clear();
...
void gl::vao::setup_ebo(buffer &e) {
    if (use_vao) {
        bind();
        // ELEMENT_ARRAY_BUFFER is part of VAO and therefore shouldn't be
        // managed by gl::buffer global tracker! but it is needed by it for
        // buffer data management, so eh
...
void gl::vao::bind() {
    ...
    if (use_vao) {
        ...
        for (attrib_params &param : params) {
            ...
            glVertexAttribPointer(
                param.attrib, param.size, param.type, GL_FALSE, param.stride,
...
void gl::vao::unbind() {
    ...
    if (use_vao) {
        ...
        for (size_t i = 0; i < 4; i++) glDisableVertexAttribArray(i);
    }
}
