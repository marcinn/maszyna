...
// -- there's no need to sort the particles, can be drawn in any order with
// depth write turned off
// -- the colour remains consistent throughout, only opacity changes
// -- randomized particle rotation
// -- initial velocity reduced over time to slow drift upwards (drift speed
// depends on particle and air temperature difference)
...
enum value_limit { min = 0, max = 1 };

// helper, adjusts provided variable by fixed amount, keeping resulting value
// between limits
...
class fixedstep_modifier {
   public:
    ...
    void update(Type_ &Variable, double const Timedelta) const;
    void bind(Type_ const *Modifier) { m_valuechangemodifier = Modifier; }
    Type_ value_change() const {
        return (
   ...
   private:
    ...
    //  methods
    //  members
    //     Type_ m_intialvalue { Type_( 0 ) }; // meters per second; velocity
    //     applied to freshly spawned particles
    Type_ m_valuechange{
        Type_(0)};  // meters per second; change applied to initial velocity
    Type_ m_valuelimits[2]{
        Type_(std::numeric_limits<Type_>::lowest()),
        Type_(std::numeric_limits<Type_>::max())};
    Type_ const *m_valuechangemodifier{
        nullptr};  // optional modifier applied to value change
};
...
class smoke_source {
    ...
    // existing particles are updated until dead no matter the range (presumed
    // to have certain lifespan) during update pass dead particle slots are
    // filled with new instances, if there's no particles queued the slot is
    // swapped with the last particle in the list bounding box/sphere calculated
    // based on position of all owned particles, used by the renderer to
    // include/discard data in a draw pass
   ...
   public:
    ...
    void initialize();
    void bind(TDynamicObject const *Vehicle);
    void bind(TAnimModel const *Node);
    // updates state of owned particles
    void update(double const Timedelta, bool const Onlydespawn);
    glm::vec3 const &color() const { return m_emitter.color; }
    glm::dvec3 location() const;
    // provides access to bounding area data
    scene::bounding_area const &area() const { return m_area; }
    particle_sequence const &sequence() const { return m_particles; }

   private:
    // types
    enum class owner_type { none = 0, vehicle, node };
    ...
    glm::dvec3 m_offset;  // meters, 3d space; relative position of the source,
                          // either from the owner or the region centre
    float m_spawnrate{0.f};  // number of particles to spawn per second
    particle_emitter m_emitter;
    //    bool m_inheritvelocity { false }; // whether spawned particle should
    //    receive velocity of its owner
    // TODO: replace modifiers with configurable interpolator item allowing
    // keyframe-based changes over time
    fixedstep_modifier<float> m_sizemodifier;  // particle billboard size
    //    fixedstep_modifier<glm::vec3> m_colormodifier; // particle billboard
    //    color and opacity
    fixedstep_modifier<float> m_opacitymodifier;
    //    texture_handle m_texture { -1 }; // texture assigned to particle
    //    billboards
    // current state
...
class particle_manager {
   ...
   public:
    ...
    //    ~particle_manager();
    // methods
    // adds a new particle source of specified type, placing it in specified
    // world location. returns: true on success, false if the specified type
    // definition couldn't be located
    bool insert(std::string const &Sourcetemplate, glm::dvec3 const Location);
    bool insert(
    ...
    void update();
    // data access
    source_sequence &sequence() { return m_sources; }
   ...
   private:
    ...
    // methods
    smoke_source *find(std::string const &Template);
    // members
...
template <typename Type_>
void fixedstep_modifier<Type_>::update(
    Type_ &Variable, double const Timedelta) const {
...
template <typename Type_>
void fixedstep_modifier<Type_>::deserialize(cParser &Input) {
