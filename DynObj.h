...
int const ANIM_LEVERS = 2;   // elementy obracane (wycieraczki, koła skrętne,
...
class TAnim;
// typedef void(__closure *TUpdate)(TAnim *pAnim); // typ funkcji aktualizującej
// położenie submodeli
typedef std::function<void(TAnim *)>
    TUpdate;  // __closure is Borland-specific extension
...
/*
Ra: Utworzyć klasę wyposażenia opcjonalnego, z której będą dziedziczyć klasy
drzwi, pantografów, napędu parowozu i innych ruchomych części pojazdów. Klasy
powinny być pseudo-wirtualne, bo wirtualne mogą obniżać wydajnosść. Przy
wczytywaniu MMD utworzyć tabelę wskaźnikow na te dodatki. Przy wyświetlaniu
...
*/
class TAnimValveGear {  // współczynniki do animacji parowozu (wartości
                        // przykładowe dla Pt47)
    ...
    // albo nawet zrobić dynamiczną tablicę float[] i w nią pakować wszelkie
    // współczynniki, potem używać indeksów współczynniki mogą być wspólne dla
    // 2-4 tłoków, albo każdy tłok może mieć odrębne
};

class TAnimPant {  // współczynniki do animacji pantografu
   public:
    Math3D::vector3 vPos;  // Ra: współrzędne punktu zerowego pantografu (X
                           // dodatnie dla przedniego)
    ...
    double fHoriz;  // przesunięcie ślizgu w długości pojazdu względem osi
                    // obrotu dolnego ramienia
    double fHeight;  // wysokość ślizgu ponad oś obrotu, odejmowana od wysokości
                     // drutu
    double fWidth;    // połowa szerokości roboczej ślizgu, do wykrycia
                      // ześlizgnięcia się drutu
    double fAngleL0;  // Ra: początkowy kąt dolnego ramienia (odejmowany przy
                      // animacji)
    double fAngleU0;  // Ra: początkowy kąt górnego ramienia (odejmowany przy
                      // animacji)
    double PantTraction;  // Winger 170204: wysokość drutu ponad punktem na
                          // wysokości vPos.y p.g.s.
    double PantWys;  // Ra: aktualna wysokość uniesienia ślizgu do porównania z
                     // wysokością drutu
...
class TAnim {  // klasa animowanej części pojazdu (koła, drzwi, pantografy,
               // burty, napęd parowozu, siłowniki itd.)
   public:
    // constructor
    TAnim() = default;
    // destructor
    ...
    union {
        TSubModel
            *smAnimated;  // animowany submodel (jeśli tylko jeden, np. oś)
        TSubModel **smElement;  // jeśli animowanych elementów jest więcej
                                // (pantograf, napęd
        // parowozu)
        int iShift;  // przesunięcie przed przydzieleniem wskaźnika
    };
    union {                          // parametry animacji
    ...
    union {                   // wskaźnik na obiekt odniesienia
    ...
    float fMaxDist;  // do jakiej odległości wykonywana jest animacja NOTE:
...
// parameters for the material object, as currently used by various simulator
// models
struct material_data {
    int textures_alpha{
        0x30300030};  // maska przezroczystości tekstur. default: tekstury
                      // wymienne nie mają przezroczystości
    material_handle replacable_skins[5] = {
        null_handle, null_handle, null_handle, null_handle,
        null_handle};  // McZapkie:zmienialne nadwozie
    int multi_textures{
        0};  //<0 tekstury wskazane wpisem, >0 tekstury z przecinkami, =0 jedna

    // assigns specified texture or a group of textures to replacable texture
    // slots
...
class TDynamicObject {  // klasa pojazdu
   ...
   private:  // położenie pojazdu w świecie oraz parametry ruchu
    Math3D::vector3
        vPosition;  // Ra: pozycja pojazdu liczona zaraz po przesunięciu
    Math3D::vector3
        vCoulpler[2];  // współrzędne sprzęgów do liczenia zderzeń czołowych
    Math3D::vector3 vUp, vFront,
        vLeft;             // wektory jednostkowe ustawienia pojazdu
    int iDirection;        // kierunek pojazdu względem czoła składu
    ...
    int iAxleFirst;  // numer pierwszej osi w kierunku ruchu (oś wiążąca pojazd
                     // z torem i wyzwalająca eventy)
    float
        fAxleDist;  // rozstaw wózków albo osi do liczenia proporcji zacienienia
    Math3D::vector3 modelRot;  // obrot pudła względem świata - do
                               // przeanalizowania, czy potrzebne!!!
    TDynamicObject *ABuFindNearestObject(
   ...
   public:
    ...
    std::string
        asDestination;  // dokąd pojazd ma być kierowany "(stacja):(tor)"
    Math3D::matrix4x4
        mMatrix;  // macierz przekształcenia do renderowania modeli
    TMoverParameters
        *MoverParameters;  // parametry fizyki ruchu oraz przeliczanie
    inline TDynamicObject *NextConnected() {
        return MoverParameters->Neighbours[end::rear].vehicle;
    };  // pojazd podłączony od strony sprzęgu 1 (kabina -1)
    inline TDynamicObject *PrevConnected() {
        return MoverParameters->Neighbours[end::front].vehicle;
    };  // pojazd podłączony od strony sprzęgu 0 (kabina 1)
    inline TDynamicObject *NextConnected() const {
        return MoverParameters->Neighbours[end::rear].vehicle;
    };  // pojazd podłączony od strony sprzęgu 1 (kabina -1)
    inline TDynamicObject *PrevConnected() const {
        return MoverParameters->Neighbours[end::front].vehicle;
    };  // pojazd podłączony od strony sprzęgu 0 (kabina 1)
    inline int NextConnectedNo() const {
        return MoverParameters->Neighbours[end::rear].vehicle_end;
    }
    inline int PrevConnectedNo() const {
        return MoverParameters->Neighbours[end::front].vehicle_end;
    }
    //    double fTrackBlock; // odległość do przeszkody do dalszego ruchu
    //    (wykrywanie kolizji z innym pojazdem)

    // modele składowe pojazdu
    TModel3d *mdModel;   // model pudła
    TModel3d *mdLoad;    // model zmiennego ładunku
    TModel3d *mdKabina;  // model kabiny dla użytkownika; McZapkie-030303: to z
                         // train.h
    TModel3d *mdLowPolyInt;  // ABu 010305: wnetrze lowpoly
    std::array<TSubModel *, 3>
        LowPolyIntCabs{};   // pointers to low fidelity version of individual
                            // driver cabs
    bool JointCabs{false};  // flag for vehicles with multiple virtual 'cabs'
                            // sharing location and 3d model(s)
    std::vector<TModel3d *>
        mdAttachments;  // additional models attached to main body
    struct destination_data {
        TSubModel *sign{nullptr};  // submodel mapped with replacable texture -4
        bool has_light{false};  // the submodel was originally configured with
                                // self-illumination attribute
        material_handle destination{
            null_handle};  // most recently assigned non-blank destination
                           // texture
        material_handle destination_off{null_handle};  // blank destination sign
        std::string
            background;  // potential default background texture override
        std::string
            script;  // potential python script used to generate texture data
        int update_rate{
            0};  // -1: per stop, 0: none, >0: fps // TBD, TODO: implement?
        std::string instancing;  // potential method to generate more than one
                                 // texture per timetable
        std::string
            parameters;  // potential extra parameters supplied by mmd file
                         // methods
    ...
    float fShade;  // zacienienie: 0:normalnie, -1:w ciemności, +1:dodatkowe
                   // światło (brak koloru?)
    float LoadOffset{0.f};
    std::unordered_map<std::string, std::string>
        LoadModelOverrides;  // potential overrides of default load
                             // visualization models
    glm::vec3 InteriorLight{
        0.9f * 255.f / 255.f, 0.9f * 216.f / 255.f,
        0.9f * 176.f /
            255.f};  // tungsten light. TODO: allow definition of light type?
    ...
    std::vector<vehicle_section>
        Sections;  // table of recognized vehicle sections
    bool SectionLightsActive{
        false};  // flag indicating whether section lights were set.
    std::vector<vehicle_section *>
        SectionLoadOrder;  // helper, activation/deactivation load chunk
                           // sequence
   ...
   public:
    inline material_data const *Material() const { return &m_materialdata; }
    // tymczasowo udostępnione do wyszukiwania drutu
    std::array<int, ANIM_TYPES> iAnimType{
        0};  // 0-osie,1-drzwi,2-obracane,3-zderzaki,4-wózki,5-pantografy,6-tłoki
   private:
    int iAnimations;  // liczba obiektów animujących
    std::vector<TAnim> pAnimations;
    TSubModel **pAnimated;  // lista animowanych submodeli (może być ich więcej
                            // niż obiektów animujących)
    double dWheelAngle[3];  // kąty obrotu kół: 0=przednie toczne, 1=napędzające
                            // i wiązary, 2=tylne toczne
    /*
    ...
    */
    ...
    void UpdateMirror(TAnim *pAnim);             // mirror animation
                                                 /*
                                                     void UpdateLeverDouble(TAnim *pAnim); // animacja gałki zależna od
                                                    double                                              void UpdateLeverFloat(TAnim *pAnim); // animacja gałki zależna od
                                                    float                                              void UpdateLeverInt(TAnim *pAnim); // animacja gałki zależna od int
                                                    (wartość)                                              void UpdateLeverEnum(TAnim *pAnim); // animacja gałki zależna
                                                    od int (lista kątów)
                                                 */
    void
    toggle_lights();  // switch light levels for registered interior sections
   ...
   private:
    ...
    struct coupleradapter_data {
        glm::vec2
            position;  // adapter placement; offset from vehicle end and height
        std::string model;  // 3d model of the adapter
    };

    struct coupler_sounds {
        // TBD: change to an array with index-based access for easier
        // initialization?
        ...
        sound_source dsbCouplerStretch{
            sound_placement::external};  // moved from cab
    ...
    struct pantograph_sounds {
        // TODO: split pantograph sound into one for contact of arm with the
        // wire, and electric arc sound
        sound_source sPantUp{sound_placement::external};
        sound_source sPantDown{sound_placement::external};
    };

    struct door_sounds {
        sound_source rsDoorOpen{
            sound_placement::general};  // Ra: przeniesione z kabiny
    ...
    struct powertrain_sounds {
        ...
        std::vector<sound_source>
            acmotors;  // inverter-specific traction motor sounds
        ...
        sound_source dsbWejscie_na_bezoporow{
            sound_placement::engine};                          // moved from cab
        ...
        sound_source engine{
            sound_placement::engine};  // generally diesel engine
        sound_source fake_engine{sound_placement::engine};
        sound_source engine_ignition{
            sound_placement::engine};  // moved from cab
        sound_source engine_shutdown{sound_placement::engine};
        bool engine_state_last{
            false};  // helper, cached previous state of the engine
        double engine_volume{
            0.0};  // MC: pomocnicze zeby gladziej silnik buczal
    ...
    struct pasystem_sounds {
        std::array<sound_source, static_cast<int>(announcement_t::end)>
            announcements;
    ...
    AirCoupler btCPneumatic1;  // sprzegi powietrzne //yB - zmienione z Button
                               // na AirCoupler - krzyzyki
    AirCoupler btCPneumatic2;
    AirCoupler btCPneumatic1r;  // ABu: to zeby nie bylo problemow przy laczeniu
                                // wagonow,
    AirCoupler
        btCPneumatic2r;  //     jesli beda polaczone sprzegami 1<->1 lub 0<->0
    ...
    sound_source rsBrake{
        ...
        EU07_SOUND_BRAKINGCUTOFFRANGE};  // moved from cab
    sound_source sBrakeAcc{sound_placement::external};
    bool bBrakeAcc{false};
    sound_source rsPisk{
        sound_placement::external,
        EU07_SOUND_BRAKINGCUTOFFRANGE};                 // McZapkie-260302
    ...
    float m_lastbrakepressure{
        -1.f};  // helper, cached level of pressure in the brake cylinder
    float m_brakepressurechange{
        0.f};  // recent change of pressure in the brake cylinder
    ...
    float m_lastepbrakepressure{
        -1.f};  // helper, cached level of pressure in the brake cylinder
    float m_epbrakepressurechange{
        0.f};  // recent change of pressure in the brake cylinder
    float m_epbrakepressurechangeinctimer{
        0.f};  // last time of change of pressure in the brake cylinder -
               // increase
    float m_epbrakepressurechangedectimer{
        0.f};  // last time of change of pressure in the brake cylinder -
               // decrease
    ...
    sound_source m_startjolt{
        sound_placement::general};  // movement start jolt, played once on
                                    // initial acceleration at slow enough speed
    bool m_startjoltplayed{false};
    std::array<coupler_sounds, 2> m_couplersounds;  // always front and rear
    std::vector<pantograph_sounds>
        m_pantographsounds;  // typically 2 but can be less (or more?)
    std::vector<door_sounds>
        m_doorsounds;  // can expect symmetrical arrangement, but don't count on
                       // it
    bool m_doorlocks{false};  // sound helper, current state of door locks
    sound_source sHorn1{
        sound_placement::external, 5 * EU07_SOUND_RUNNINGNOISECUTOFFRANGE};
    sound_source sHorn2{
        sound_placement::external, 5 * EU07_SOUND_RUNNINGNOISECUTOFFRANGE};
    sound_source sHorn3{
        sound_placement::external, 5 * EU07_SOUND_RUNNINGNOISECUTOFFRANGE};
    ...
    std::vector<sound_source>
        m_bogiesounds;  // TBD, TODO: wrapper for all bogie-related sounds
                        // (noise, brakes, squeal etc)
#else
    sound_source m_outernoise{
        sound_placement::external, EU07_SOUND_RUNNINGNOISECUTOFFRANGE};
#endif
    sound_source m_wheelflat{
        sound_placement::external, EU07_SOUND_RUNNINGNOISECUTOFFRANGE};
    sound_source rscurve{
        sound_placement::external,
        EU07_SOUND_RUNNINGNOISECUTOFFRANGE};  // youBy
    sound_source rsDerailment{
        sound_placement::external,
        2 * EU07_SOUND_RUNNINGNOISECUTOFFRANGE};  // McZapkie-051202

    exchange_data
        m_exchange;  // state of active load exchange procedure, if any
    exchange_sounds
        m_exchangesounds;  // sounds associated with the load exchange

    std::vector<doorspeaker_sounds> m_doorspeakers;
    pasystem_sounds m_pasystem;

    std::array<
        std::array<float, 6>  // listener: rear cab, engine, front cab, window,
                              // attached camera, free camera
        ,
        5>
        m_soundproofing = {{
            {{EU07_SOUNDPROOFING_NONE, EU07_SOUNDPROOFING_STRONG,
              ...
              EU07_SOUNDPROOFING_STRONG}},  // internal sounds
            {{EU07_SOUNDPROOFING_STRONG, EU07_SOUNDPROOFING_NONE,
              EU07_SOUNDPROOFING_STRONG, EU07_SOUNDPROOFING_SOME,
              EU07_SOUNDPROOFING_SOME,
              EU07_SOUNDPROOFING_SOME}},  // engine sounds
            {{EU07_SOUNDPROOFING_STRONG, EU07_SOUNDPROOFING_STRONG,
              EU07_SOUNDPROOFING_STRONG, EU07_SOUNDPROOFING_SOME,
              EU07_SOUNDPROOFING_SOME,
              EU07_SOUNDPROOFING_NONE}},  // external sound
            {{EU07_SOUNDPROOFING_VERYSTRONG, EU07_SOUNDPROOFING_VERYSTRONG,
              EU07_SOUNDPROOFING_VERYSTRONG, EU07_SOUNDPROOFING_STRONG,
              EU07_SOUNDPROOFING_STRONG,
              EU07_SOUNDPROOFING_NONE}},  // external ambient sound
            {{EU07_SOUNDPROOFING_NONE, EU07_SOUNDPROOFING_NONE,
              EU07_SOUNDPROOFING_NONE, EU07_SOUNDPROOFING_NONE,
              EU07_SOUNDPROOFING_NONE,
              EU07_SOUNDPROOFING_NONE}},  // custom sounds
        }};
    ...
    float ModCamRot;
    int iInventory[2]{
        0, 0};   // flagi bitowe posiadanych submodeli (np. świateł)
    bool btnOn;  // ABu: czy byly uzywane buttony, jesli tak, to po renderingu
                 // wylacz bo ten sam model moze byc jeszcze wykorzystany przez
                 // inny obiekt!
   ...
   private:
    TDynamicObject *ABuFindObject(
        int &Foundcoupler, double &Distance, TTrack const *Track,
        int const Direction, int const Mycoupler) const;
    void ABuCheckMyTrack();

   public:
    bool DimHeadlights{
        false};  // status of the headlight dimming toggle. NOTE: single toggle
                 // for all lights is a simplification. TODO: separate per-light
                 // switches
    // checks whether there's unbroken connection of specified type to specified
    // vehicle
    bool is_connected(
        TDynamicObject const *Vehicle,
        coupling const Coupling = coupling::coupler) const;
    ...
    std::string asName;
    const std::string &name() const { return asName; }
    ...
    int iOverheadMask;  // maska przydzielana przez AI pojazdom posiadającym
                        // pantograf, aby wymuszały jazdę bezprądową
    TTractionParam tmpTraction;
    double fAdjustment;  // korekcja - docelowo przenieść do TrkFoll.cpp wraz z
                         // odległością od poprzedniego
    ...
    double Init(
        std::string Name, std::string BaseDir, std::string asReplacableSkin,
        std::string Type_Name, TTrack *Track, double fDist,
        std::string DriverType, double fVel, std::string TrainName, float Load,
        std::string LoadType, bool Reversed, std::string);
    int init_sections(
    ...
    // initiates load change by specified amounts, with a platform on specified
    // side
    void LoadExchange(
        int const Disembark, int const Embark, int const Platforms);
    // calculates time needed to complete current load change, using specified
    // platforms
    float LoadExchangeTime(int const Platforms);
    // calculates time needed to complete current load change, using previously
    // specified platforms
    float LoadExchangeTime() const;
    // calculates current load exchange factor, where 1 = nominal rate, higher =
    // faster
    ...
    void RenderSounds();
    inline Math3D::vector3 GetPosition() const { return vPosition; };
    // converts location from vehicle coordinates frame to world frame
    inline Math3D::vector3 GetWorldPosition(
        Math3D::vector3 const &Location) const {
    ...
    inline double Roll() { return ((Axle1.GetRoll() + Axle0.GetRoll())); }
    /*
        // TODO: check if scanning takes into account direction when selecting
       axle
    ...
    */
    inline Math3D::vector3 VectorFront() const { return vFront; };
    inline Math3D::vector3 VectorUp() const { return vUp; };
    inline Math3D::vector3 VectorLeft() const { return vLeft; };
    inline double const *Matrix() const { return mMatrix.readArray(); };
    inline double *Matrix() { return mMatrix.getArray(); };
    inline double GetVelocity() const { return MoverParameters->Vel; };
    inline double GetLength() const { return MoverParameters->Dim.L; };
    inline double GetWidth() const { return MoverParameters->Dim.W; };
    double radius() const;
    ...
    inline double ABuGetDirection() const {  // ABu.
        return (
            Axle1.GetTrack() == MyTrack ? Axle1.GetDirection()
                                        : Axle0.GetDirection());
    };
    ...
    int DirectionGet() const { return iDirection + iDirection - 1; };
    int DettachStatus(int dir);
    int Dettach(int dir);
    TDynamicObject *Neighbour(int &dir);
    // updates potential collision sources
    void update_neighbours();
    // locates potential collision source within specified range, scanning its
    // route in specified direction
    auto find_vehicle(int const Direction, double const Range) const
        -> std::tuple<TDynamicObject *, int, double, bool>;
    // locates potential vehicle connected with specific coupling type and
    // satisfying supplied predicate
    template <typename Predicate_>
    auto find_vehicle(coupling const Coupling, Predicate_ const Predicate)
        -> TDynamicObject *;
   ...
   public:
    ...
    float HuntingAngle{0.f};  // crude approximation of hunting oscillation;
                              // current angle of sine wave
    bool IsHunting{false};
    TSpring ShakeSpring;
    struct shake_state {
        Math3D::vector3 velocity{};  // current shaking vector
        Math3D::vector3
            offset{};  // overall shake-driven offset from base position
    } ShakeState;
...
class vehicle_table : public basic_table<TDynamicObject> {
   public:
    ...
    void update(double dt, int iter);
    // legacy method, checks for presence and height of traction wire for
    // specified vehicle
    void update_traction(TDynamicObject *Vehicle);
    // legacy method, sends list of vehicles over network
    void DynamicList(bool const Onlycontrolled = false) const;

   private:
    // maintenance; removes from tracks consists with vehicles marked as
    // disabled
    bool erase_disabled();
};

template <typename Predicate_>
auto TDynamicObject::find_vehicle(
    coupling const Coupling, Predicate_ const Predicate) -> TDynamicObject * {
    if (Predicate(this)) {
        return this;
    }
...
template <typename UnaryFunction_>
void TDynamicObject::for_each(
    coupling const Coupling, UnaryFunction_ const Function) {
    Function(this);
