...
int TSubModel::iAlpha =
    0x30300030;  // maska do testowania flag tekstur wymiennych
TModel3d
    *TSubModel::pRoot;  // Ra: tymczasowo wskaźnik na model widoczny z submodelu
...
TSubModel::~TSubModel() {
    if (iFlags & 0x0200) {  // wczytany z pliku tekstowego musi sam posprzątać
        SafeDelete(Next);
        SafeDelete(Child);
...
void TSubModel::Name_Material(
    std::string const &Name) {  // ustawienie nazwy submodelu, o
                                // ile nie jest wczytany z E3D
    if (iFlags & 0x0200) {  // tylko jeżeli submodel zosta utworzony przez new
        m_materialname = Name;
    }
};

void TSubModel::Name(
    std::string const &Name) {  // ustawienie nazwy submodelu, o ile
                                // nie jest wczytany z E3D
    if (iFlags & 0x0200) pName = Name;
};

// sets rgb components of diffuse color override to specified value
void TSubModel::SetDiffuseOverride(
    glm::vec3 const &Color, bool const Includechildren,
    bool const Includesiblings) {
    ...
    if (true == Includesiblings) {
        ...
        while ((sibling = sibling->Next) != nullptr) {
            sibling->SetDiffuseOverride(
                Color, Includechildren,
                false);  // no need for all siblings to duplicate the work
        }
    }
    if ((true == Includechildren) && (Child != nullptr)) {
        Child->SetDiffuseOverride(
            Color, Includechildren,
            true);  // node's children include child's siblings and children
    }
}
...
std::optional<glm::vec3> TSubModel::GetDiffuse(float Includesiblings) {
    ...
    if (Child) return Child->GetDiffuse(true);

    return std::nullopt;
}

// sets visibility level (alpha component) to specified value
void TSubModel::SetVisibilityLevel(
    float const Level, bool const Includechildren, bool const Includesiblings) {
    fVisible = Level;
    if (true == Includesiblings) {
        auto sibling{this};
        while ((sibling = sibling->Next) != nullptr) {
            sibling->SetVisibilityLevel(
                Level, Includechildren,
                false);  // no need for all siblings to duplicate the work
        }
    }
    if ((true == Includechildren) && (Child != nullptr)) {
        Child->SetVisibilityLevel(
            Level, Includechildren,
            true);  // node's children include child's siblings and children
    }
}
...
void TSubModel::SetLightLevel(
    glm::vec4 const &Level, bool const Includechildren,
    bool const Includesiblings) {
    /*
    f4Emision = Level;
    */
...
void TSubModel::SetSelfIllum(
    float const Threshold, bool const Includechildren,
    bool const Includesiblings) {
    fLight = Threshold;
    if (true == Includesiblings) {
        auto sibling{this};
        while ((sibling = sibling->Next) != nullptr) {
            sibling->SetSelfIllum(
                Threshold, Includechildren,
                false);  // no need for all siblings to duplicate the work
        }
    }
    if ((true == Includechildren) && (Child != nullptr)) {
...
inline void readMatrix(
    cParser &parser, float4x4 &matrix) {  // Ra: wczytanie transforma
...
std::pair<int, int> TSubModel::Load(
    cParser &parser,
    bool dynamic) {  // Ra: VBO tworzone na poziomie modelu, a nie submodeli
    auto token{parser.getToken<std::string>()};
    if (token != "type:") {
        std::string errormessage{
            "Bad model: expected submodel type definition not found while "
            "loading model \"" +
        ...
        while ((true == parser.getTokens()) &&
               ...
               (token != "parent:")) {
            // skip data until next submodel, dump first few tokens in the error
            // message
            if (--count > 0) {
                errormessage += token + " ";
            }
    ...
    if (dynamic) {
        ...
        if ((token.size() >= 3) && (token.find("_on") + 3 == token.length())) {
            // jeśli nazwa kończy się na "_on" to domyślnie wyłączyć, żeby się
            // nie nakładało z obiektem "_off"
            iVisible = 0;
        }
    } else {
        // dla pozostałych modeli blokujemy zapalone światła, które mogą być
        // nieobsługiwane
        if (token.compare(0, 8, "Light_On") == 0) {
            // jeśli nazwa zaczyna się od "Light_On" to domyślnie wyłączyć, żeby
            // się nie nakładało z obiektem "Light_Off"
            iVisible = 0;
        }
    }
    ...
    {                                 // rodzaj animacji
        ...
        if (type != "false") {
            iFlags |=
                0x4000;  // jak animacja, to trzeba przechowywać macierz zawsze
            if (type == "seconds_jump")
                b_Anim = b_aAnim =
                    TAnimType::at_SecondsJump;  // sekundy z przeskokiem
            else if (type == "minutes_jump")
                b_Anim = b_aAnim =
                    TAnimType::at_MinutesJump;  // minuty z przeskokiem
            else if (type == "hours_jump")
                b_Anim = b_aAnim =
                    TAnimType::at_HoursJump;  // godziny z przeskokiem
            else if (type == "hours24_jump")
                b_Anim = b_aAnim =
                    TAnimType::at_Hours24Jump;  // godziny z przeskokiem
                ...
                b_Anim = b_aAnim =
                    TAnimType::at_Billboard;  // obrót w pionie do kamery
            else if (type == "wind")
                b_Anim = b_aAnim =
                    TAnimType::at_Wind;  // ruch pod wpływem wiatru
            else if (type == "sky")
                b_Anim = b_aAnim = TAnimType::at_Sky;  // aniamacja nieba
            else if (type == "digital")
                b_Anim = b_aAnim =
                    TAnimType::at_Digital;  // licznik mechaniczny
            else if (type == "digiclk")
                b_Anim = b_aAnim = TAnimType::at_DigiClk;  // zegar cyfrowy
            else
                b_Anim = b_aAnim =
                    TAnimType::at_Undefined;  // nieznana forma animacji
        }
    }
    if (eType < TP_ROTATOR)
    ...
    if (eType < TP_ROTATOR) {
        ...
        if (pName == "cien") {
            // crude workaround to kill specular on shadow geometry of legacy
            // models
    ...
    if (eType == TP_FREESPOTLIGHT) {
        ...
        parser >> fNearAttenStart >> discard >> fNearAttenEnd >> discard >>
            bUseNearAtten >> discard >> iFarAttenDecay >> discard >>
            fFarDecayRadius >> discard >>
        ...
        iFlags |= 0x4030;  // drawn both in solid (light point) and transparent
                           // (light glare) phases
        ...
        if (material == "none") {  // rysowanie podanym kolorem
            ...
            /*
                        if( material.find_first_of( "/" ) == material.npos ) {
                            // jeśli tylko nazwa pliku, to dawać bieżącą ścieżkę
               do tekstur material.insert( 0, Global.asCurrentTexturePath );
                        }
            */
    ...
    if (m_material > 0) {
        ...
        /*
                // if material does have opacity set, replace submodel opacity
           with it if (mat.opacity)
                {
                    iFlags &= ~0x30;
                    if (*mat.opacity == 0.0f)
        ...
        */
        ...
        if (mat.selfillum) fLight = *mat.selfillum;
    }
    ...
    if (Parent != nullptr) transformscalestack = Parent->transformscalestack;
    if (!fMatrix->IdentityIs()) {
        ...
        if ((std::abs(scale.x - 1.0f) > 0.01) ||
            ...
            ErrorLog(
                "Bad model: transformation matrix for sub-model \"" + pName +
                    "\" imposes geometry scaling (factors: " +
                    to_string(scale) + ")",
                logtype::model);
            m_normalizenormals =
                (((std::abs(scale.x - scale.y) < 0.01f) &&
                  (std::abs(scale.y - scale.z) < 0.01f))
                     ? rescale
                     : normalize);
        }
        transformscalestack *= (scale.x + scale.y + scale.z) / 3.0f;
    }
    if (eType < TP_ROTATOR) {  // wczytywanie wierzchołków
        token = parser.getToken<std::string>();
        if (token == "numindices:")  // optional block, potentially preceeding
                                     // vertex list
        {
        ...
        if ((token == "numverts:") ||
            (token == "numverts")) {  // normalna lista wierzchołków
                                      /*
                                                  // Ra 15-01: to wczytać jako tekst - jeśli pierwszy znak
                                         zawiera "*", to
                                                  // dalej będzie nazwa wcześniejszego submodelu, z
                                         którego należy wziąć wierzchołki
                                                  // zapewni to jakąś zgodność wstecz, bo zamiast liczby
                                         będzie ciąg, którego
                                                  // wartość powinna być uznana jako zerowa
                                                  token = parser.getToken<std::string>();
                                                  if( token.front() == '*' ) {
                                                  // jeśli pierwszy znak jest gwiazdką, poszukać submodelu
                                         o nazwie bez tej gwiazdki i wziąć z niego wierzchołki                           Error(
                                         "Vertices reference not yet supported!" );
                                                  }
                                      */
            ...
            if ((m_geometry.index_count <= 0) &&
                ...
                Error(
                    "Bad model: incomplete triangle encountered in submodel "
                    "\"" +
                    pName + "\"");
                return {0, 0};
            }
            if (m_geometry.vertex_count) {
                ...
                if (m_geometry.index_count) {
                    // indexed geometry chunks are expected to come with
                    // pre-generated normals and tangents, to avoid hassle
                    // required to generate them manually
                    ...
                    for (auto idx = 0; idx < m_geometry.vertex_count; ++idx) {
                        ...
                        parser >> vertex->position.x >> vertex->position.y >>
                            vertex->position.z >> vertex->normal.x >>
                            vertex->normal.y >> vertex->normal.z >>
                            vertex->texture.s >> vertex->texture.t >>
                            vertex->tangent.x >> vertex->tangent.y >>
                            vertex->tangent.z >> vertex->tangent.w;
                    }
                } else {
                    // legacy geometry, more or less incomplete
                    // to stay on the safe side we'll potentially need to
                    // convert indexed geometry to raw triangle form, cal
                    auto vertices{std::begin(Vertices)};
                    int facecount = m_geometry.vertex_count / 3;
                    std::vector<unsigned int> sg;
                    sg.resize(facecount);  // maski przynależności trójkątów do
                                           // powierzchni
                    std::vector<int> wsp;
                    wsp.resize(
                        m_geometry.vertex_count);  // z którego wierzchołka
                                                   // kopiować wektor normalny
                    int maska = 0;
                    int vertexidx = 0;  // used to keep track of vertex indices
                                        // in source file
                    for (auto idx = 0; idx < m_geometry.vertex_count; ++idx) {
                        ++vertexidx;
                        // Ra: z konwersją na układ scenerii - będzie
                        // wydajniejsze wyświetlanie
                        wsp[idx] = -1;  // wektory normalne nie są policzone dla
                                        // tego wierzchołka
                        if ((idx % 3) == 0) {
                            // jeśli będzie maska -1, to dalej będą wierzchołki
                            // z wektorami normalnymi, podanymi jawnie
                            maska = parser.getToken<int>(
                                false);  // maska powierzchni trójkąta
                            // dla maski -1 będzie 0, czyli nie ma wspólnych
                            // wektorów normalnych
                        ...
                        parser >> vertex->position.x >> vertex->position.y >>
                            vertex->position.z;
                        if (maska ==
                            -1) {  // jeśli wektory normalne podane jawnie
                            parser.getTokens(3, false);
                            parser >> vertex->normal.x >> vertex->normal.y >>
                                vertex->normal.z;
                            if (glm::length2(vertex->normal) > 0.0f) {
                                ...
                                WriteLog(
                                    ...
                                    "specified in submodel \"" +
                                        pName + "\", vertex " +
                                        std::to_string(idx),
                                    logtype::model);
                            }
                            wsp[idx] =
                                idx;  // wektory normalne "są już policzone"
                        }
                        ...
                        if (idx % 3 == 2) {
                            ...
                            if (true == degenerate(
                                ...
                                m_geometry.vertex_count -=
                                    3;     // czyli o 3 wierzchołki
                                idx -= 3;  // wczytanie kolejnego w to miejsce
                                WriteLog(
                                    "Bad model: degenerated triangle ignored "
                                    "in: \"" +
                                        pName + "\", vertices " +
                                        std::to_string(vertexidx - 2) + "-" +
                                        std::to_string(vertexidx),
                                    logtype::model);
                            }
                            if (idx > 0) {
                                // jeśli pierwszy trójkąt będzie zdegenerowany,
                                // to zostanie usunięty i nie ma co sprawdzać
                                if ((glm::length(
                                         (vertex)->position -
                                         (vertex - 1)->position) > 1000.0) ||
                                    (glm::length(
                                         (vertex - 1)->position -
                                         (vertex - 2)->position) > 1000.0) ||
                                    (glm::length(
                                         (vertex - 2)->position -
                                         (vertex)->position) > 1000.0)) {
                                    // jeżeli są dalej niż 2km od siebie //Ra
                                    // 15-01: obiekt wstawiany nie powinien być
                                    // większy niż 300m (trójkąty terenu w E3D
                                    // mogą mieć 1.5km)
                                    --facecount;  // o jeden trójkąt mniej
                                    m_geometry.vertex_count -=
                                        3;  // czyli o 3 wierzchołki
                                    idx -=
                                        3;  // wczytanie kolejnego w to miejsce
                                    WriteLog(
                                        "Bad model: too large triangle ignored "
                                        "in: \"" +
                                            pName + "\"",
                                        logtype::model);
                                }
                    ...
                    for (int i = 0; i < facecount; ++i) {
                        // pętla po trójkątach - będzie szybciej, jak wstępnie
                        // przeliczymy normalne trójkątów
                    ...
                    for (int vertexidx = 0; vertexidx < m_geometry.vertex_count;
                         ++vertexidx) {
                        // pętla po wierzchołkach trójkątów
                        auto vertex{vertices + vertexidx};
                        if (wsp[vertexidx] >= 0) {
                            // jeśli już był liczony wektor normalny z użyciem
                            // tego wierzchołka to wystarczy skopiować policzony
                            // wcześniej
                            vertex->normal =
                                (vertices + wsp[vertexidx])->normal;
                        } else {
                            // inaczej musimy dopiero policzyć
                            auto const faceidx =
                                vertexidx / 3;  // numer trójkąta
                            vertexnormal =
                                glm::vec3();  // liczenie zaczynamy od zera
                            auto adjacenvertextidx =
                                vertexidx;  // zaczynamy dodawanie wektorów
                                            // normalnych od własnego
                            while (adjacenvertextidx >= 0) {
                                // sumowanie z wektorem normalnym sąsiada
                                // (włącznie ze sobą)
                                if (glm::dot(
                                        vertexnormal,
                                        facenormals[adjacenvertextidx / 3]) >
                                    -0.99f) {
                                    wsp[adjacenvertextidx] =
                                        vertexidx;  // informacja, że w tym
                                                    // wierzchołku jest już
                                                    // policzony wektor normalny
                                    vertexnormal +=
                                        facenormals[adjacenvertextidx / 3];
                                }
                                /*
                                else {
                                    ErrorLog( "Bad model: opposite normals in
                                the same smoothing group, check sub-model \"" +
                                pName + "\" for two-sided faces and/or scaling",
                                logtype::model );
                                }
                                */
                                // i szukanie od kolejnego trójkąta
                                adjacenvertextidx = SeekFaceNormal(
                                    sg, adjacenvertextidx / 3 + 1, sg[faceidx],
                                    vertex->position, Vertices);
                            }
                            // Ra 15-01: należało by jeszcze uwzględnić
                            // skalowanie wprowadzane przez transformy, aby
                            // normalne po przeskalowaniu były jednostkowe
                            if (glm::length2(vertexnormal) == 0.0f) {
                                WriteLog(
                                    "Bad model: zero length normal vector "
                                    "generated for sub-model \"" +
                                        pName + "\"",
                                    logtype::model);
                            }
                            vertex->normal =
                                (glm::length2(vertexnormal) > 0.0f
                                     ...
                                     : facenormals[vertexidx / 3]);  // przepisanie
                                                                     // do
                                                                     // wierzchołka
                                                                     // trójkąta
                        }
                    ...
                    Vertices.resize(
                        m_geometry
                            .vertex_count);  // in case we had some degenerate
                                             // triangles along the way
                    gfx::calculate_indices(
                        Indices, Vertices, transformscalestack);
            ...
            {
                eType = TP_ROTATOR;  // submodel pomocniczy, ma tylko macierz
                                     // przekształcenia
                // dla formalności
    ...
    } else if (eType == TP_STARS) {  // punkty świecące dookólnie - składnia jak
                                     // dla smt_Mesh
        std::string discard;
        parser.getTokens(2, false);
        ...
        for (idx = 0; idx < m_geometry.vertex_count; ++idx) {
            ...
            parser.getTokens(5, false);
            parser >> vertex->position.x >> vertex->position.y >>
                vertex->position.z >> color  // zakodowany kolor
                >> discard;
            vertex->normal = {
                ((color)&0xff) / 255.0f,           // R
...
*/
/*
basic_vertex *TSubModel::TrianglePtr(int tex, int pos, glm::vec3 const &Ambient,
glm::vec3 const &Diffuse, glm::vec3 const &Specular ) { // zwraca wskaźnik do
wypełnienia tabeli wierzchołków, używane przy tworzeniu E3D terenu
        ...
        { // utworznie tabeli trójkątów
                ...
                s->iVboPtr = iInstance; // pozycja submodelu w tabeli
wierzchołków iInstance += s->iNumVerts; // pozycja dla następnego
        }
        s->ColorsSet(Ambient, Diffuse, Specular); // ustawienie kolorów świateł
        return s->Vertices + pos; // wskaźnik na wolne miejsce w tabeli
wierzchołków
};
*/

void TSubModel::InitialRotate(
    bool doit) {  // konwersja układu współrzędnych na zgodny ze scenerią
    ...
    {  // niejednostkowy transform jest mnożony i wystarczy zabawy
        if (doit) {
            ...
            if (!fMatrix) {
                ...
                fMatrix =
                    new float4x4();  // tworzy macierz o przypadkowej zawartości
        ...
        if (Child) {
            ...
            if (((iFlags & 0xC000) == 0x8000)  // o ile nie ma animacji
                && (false == is_emitter()))    // don't optimize smoke emitter
                                               // attachment points
            {  // jak nie ma potomnych, można wymnożyć przez transform i
               // wyjedynkować go
                ...
                if (false == Vertices.empty()) {
                    ...
                    if (eType != TP_STARS) {
                        // gwiazdki mają kolory zamiast normalnych, to ich wtedy
                        // nie ruszamy
                        for (auto &vertex : Vertices) {
                            vertex.normal =
                ...
                mat->Identity();    // jedynkowanie transformu po przeliczeniu
                                    // wierzchołków
                iFlags &= ~0x8000;  // transform jedynkowy
            }
        }
    } else           // jak jest jednostkowy i nie ma animacji
        if (doit) {  // jeśli jest jednostkowy transform, to przeliczamy
            ...
            for (auto &vertex : Vertices) {
                ...
                // wektory normalne również trzeba przekształcić, bo się źle
                // oświetlają
                if (eType != TP_STARS) {
                    // gwiazdki mają kolory zamiast normalnych, to // ich wtedy
                    // nie ruszamy
...
void TSubModel::ChildAdd(
    TSubModel *SubModel) {  // dodanie submodelu potemnego (uzależnionego)
                            // Ra: zmiana kolejności, żeby kolejne móc
                            // renderować po aktualnym (było przed)
    if (SubModel)
        SubModel->NextAdd(Child);  // Ra: zmiana kolejności renderowania
    Child = SubModel;
};

void TSubModel::NextAdd(
    TSubModel *SubModel) {  // dodanie submodelu kolejnego (wspólny przodek)
    if (Next)
...
uint32_t
TSubModel::FlagsCheck() {  // analiza koniecznych zmian pomiędzy submodelami
                           // samo pomijanie glBindTexture() nie poprawi
                           // wydajności ale można sprawdzić, czy można w ogóle
                           // pominąć kod do tekstur (sprawdzanie replaceskin)
    m_rotation_init_done = true;

    uint32_t i = 0;
    if (Child) {                // Child jest renderowany po danym submodelu
        if (Child->m_material)  // o ile ma teksturę
            if (Child->m_material != m_material)  // i jest ona inna niż rodzica
                Child->iFlags |=
                    0x80;  // to trzeba sprawdzać, jak z teksturami jest
        i = Child->FlagsCheck();
        iFlags |= 0x00FF0000 &
                  ((i << 16) | (i) | (i >> 8));  // potomny, rodzeństwo i dzieci
        if (eType == TP_TEXT) {  // wyłączenie renderowania Next dla znaków
    ...
    if (Next) {  // Next jest renderowany po danym submodelu (kolejność
                 // odwrócona po wczytaniu T3D)
        ...
        iFlags |= 0xFF000000 &
                  ((i << 24) | (i << 8) |
                   (i));  // następny, kolejne i ich dzieci
                          // tekstury nie ustawiamy tylko wtedy, gdy jest taka
                          // sama jak Next i jego dzieci nie zmieniają
    }
    return iFlags;
};

void TSubModel::SetRotate(
    float3 vNewRotateAxis, float fNewAngle) {  // obrócenie submodelu wg podanej
...
void TSubModel::SetRotateXYZ(float3 vNewAngles) {  // obrócenie submodelu o
...
void TSubModel::SetRotateXYZ(
    Math3D::vector3 vNewAngles) {  // obrócenie submodelu o
                                   // podane kąty wokół osi
                                   // lokalnego układu
...
void TSubModel::SetTranslate(
    float3 vNewTransVector) {  // przesunięcie submodelu (np. w kabinie)
...
void TSubModel::SetTranslate(
    Math3D::vector3
        vNewTransVector) {  // przesunięcie submodelu (np. w kabinie)
...
void TSubModel::SetRotateIK1(float3 vNewAngles) {  // obrócenie submodelu o
                                                   // podane kąty wokół osi
                                                   // lokalnego układu
    v_Angles = vNewAngles;
    iAnimOwner = iInstance;  // zapamiętanie czyja jest animacja
...
TSubModel *TSubModel::GetFromName(std::string const &search, bool i) {
    ...
    if (i)
        std::transform(
            search_lc.begin(), search_lc.end(), search_lc.begin(), ::tolower);
    std::string pName_lc = pName;
    if (i)
        std::transform(
            pName_lc.begin(), pName_lc.end(), pName_lc.begin(), ::tolower);
    if (pName.size() && search.size())
        if (pName_lc == search_lc) return this;
    if (Next) {
...
void TSubModel::RaAnimation(
    glm::mat4 &m,
    TAnimType a) {  // wykonanie animacji niezależnie od renderowania
    switch (a) {    // korekcja położenia, jeśli submodel jest animowany
        case TAnimType::at_Translate:            // Ra: było "true"
            if (iAnimOwner != iInstance) break;  // cudza animacja
            m = glm::translate(
                m,
                glm::vec3(v_TransVector.x, v_TransVector.y, v_TransVector.z));
            break;
        case TAnimType::at_Rotate:               // Ra: było "true"
            if (iAnimOwner != iInstance) break;  // cudza animacja
            m = glm::rotate(
                m, glm::radians(f_Angle),
                glm::vec3(v_RotateAxis.x, v_RotateAxis.y, v_RotateAxis.z));
            break;
        case TAnimType::at_RotateXYZ:
            ...
            m = glm::translate(
                ...
                glm::vec3(v_TransVector.x, v_TransVector.y, v_TransVector.z));
            m = glm::rotate(
                m, glm::radians(v_Angles.x), glm::vec3(1.0f, 0.0f, 0.0f));
            m = glm::rotate(
                m, glm::radians(v_Angles.y), glm::vec3(0.0f, 1.0f, 0.0f));
            m = glm::rotate(
                m, glm::radians(v_Angles.z), glm::vec3(0.0f, 0.0f, 1.0f));
            break;
        case TAnimType::at_SecondsJump:  // sekundy z przeskokiem
            m = glm::rotate(
                m, glm::radians(simulation::Time.data().wSecond * 6.0f),
                glm::vec3(0.0f, 1.0f, 0.0f));
            break;
        case TAnimType::at_MinutesJump:  // minuty z przeskokiem
            m = glm::rotate(
                m, glm::radians(simulation::Time.data().wMinute * 6.0f),
                glm::vec3(0.0f, 1.0f, 0.0f));
            break;
        case TAnimType::at_HoursJump:  // godziny skokowo 12h/360°
            m = glm::rotate(
                m, glm::radians(simulation::Time.data().wHour * 30.0f * 0.5f),
                glm::vec3(0.0f, 1.0f, 0.0f));
            break;
        case TAnimType::at_Hours24Jump:  // godziny skokowo 24h/360°
            m = glm::rotate(
                m, glm::radians(simulation::Time.data().wHour * 15.0f * 0.25f),
                glm::vec3(0.0f, 1.0f, 0.0f));
            break;
        case TAnimType::at_Seconds:  // sekundy płynnie
            m = glm::rotate(
                m, glm::radians((float)simulation::Time.second() * 6.0f),
                glm::vec3(0.0f, 1.0f, 0.0f));
            break;
        case TAnimType::at_Minutes:  // minuty płynnie
            m = glm::rotate(
                m,
                glm::radians(
                    simulation::Time.data().wMinute * 6.0f +
                    (float)simulation::Time.second() * 0.1f),
                glm::vec3(0.0f, 1.0f, 0.0f));
            break;
        case TAnimType::
            ...
            m = glm::rotate(
                m, glm::radians(2.0f * (float)Global.fTimeAngleDeg),
                glm::vec3(0.0f, 1.0f, 0.0f));
            break;
        case TAnimType::
            ...
            m = glm::rotate(
                m, glm::radians((float)Global.fTimeAngleDeg),
                glm::vec3(0.0f, 1.0f, 0.0f));
            break;
        case TAnimType::at_Billboard:  // obrót w pionie do kamery
        {
            ...
            float3 gdzie = float3(
                mat[3][0], mat[3][1],
                mat[3][2]);  // początek układu współrzędnych submodelu względem
                             // kamery
            m = glm::mat4(1.0f);
            m = glm::translate(
                m, glm::vec3(
                       gdzie.x, gdzie.y,
                       gdzie.z));  // początek układu zostaje bez zmian
            m = glm::rotate(
                m, (float)atan2(gdzie.x, gdzie.z),
                glm::vec3(
                    0.0f, 1.0f, 0.0f));  // jedynie obracamy w pionie o kąt
        } break;
        case TAnimType::at_Wind:  // ruch pod wpływem wiatru (wiatr będziemy
                                  // liczyć potem...)
            m = glm::rotate(
                m,
                glm::radians(
                    1.5f * (float)sin(M_PI * simulation::Time.second() / 6.0)),
                glm::vec3(0.0f, 1.0f, 0.0f));
            break;
        case TAnimType::at_Sky:  // animacja nieba
            m = glm::rotate(
                m, glm::radians((float)Global.fLatitudeDeg),
                glm::vec3(0.0f, 1.0f, 0.0f));  // ustawienie osi OY na północ
            m = glm::rotate(
                m, glm::radians((float)-fmod(Global.fTimeAngleDeg, 360.0)),
                glm::vec3(0.0f, 1.0f, 0.0f));
            break;
        case TAnimType::at_DigiClk:  // animacja zegara cyfrowego
        {  // ustawienie animacji w submodelach potomnych
            TSubModel *sm = ChildGet();
            do {  // pętla po submodelach potomnych i obracanie ich o kąt zależy
                  // od czasu
                if (sm->pName.size()) {
    ...
    if (mAnimMatrix)  // można by to dać np. do at_Translate
    {
        ...
        mAnimMatrix
            .reset();  // jak animator będzie potrzebował, to ustawi ponownie
    }
};

//---------------------------------------------------------------------------

void TSubModel::serialize_geometry(
        ...
        switch (Size) {
            case 1: {
                for (auto const &index :
                     GfxRenderer->Indices(m_geometry.handle)) {
...
void TSubModel::create_geometry(
    std::size_t &Indexoffset, std::size_t &Vertexoffset,
    gfx::geometrybank_handle const &Bank) {
    // data offset is used to determine data offset of each submodel into single
    // shared geometry bank (the offsets are part of legacy system which we now
    // need to work around for backward compatibility)
    if (Child) {
    ...
    if (false == Vertices.empty()) {
        ...
        // conveniently all relevant custom node types use GL_POINTS, or we'd
        // have to determine the type on individual basis
    ...
    if (m_geometry.handle != 0) {
        ...
        float squaredradius{};
        // if this happens to be root node it may already have non-squared
        // radius of the largest child since we're comparing squared radii, we
        // need to square it back for correct results
        ...
        root->m_boundingradius =
            std::max(root->m_boundingradius, m_boundingradius);
    }
...
void TSubModel::ColorsSet(
    glm::vec3 const &Ambient, glm::vec3 const &Diffuse,
    glm::vec3 const &Specular) {  // ustawienie kolorów dla modelu terenu
...
bool TSubModel::is_emitter() const {
    return (
        (eType == TP_ROTATOR) && (ToLower(pName).find("smokesource_") == 0));
}
...
void TSubModel::ParentMatrix(float4x4 *m) const {
    ...
    auto *submodel = this;
    do {
        // for given step in hierarchy there can be custom transformation
        // matrix, or no transformation retrieve it...
        submodelmatrix.Identity();
        if (submodel->GetMatrix()) {
            submodelmatrix = float4x4(*submodel->GetMatrix());
        }
        // ...potentially adjust transformations of the root matrix if the model
        // wasn't yet initialized...
        if ((submodel->Parent == nullptr) &&
            (false == submodel->m_rotation_init_done)) {
            // dla ostatniego może być potrzebny dodatkowy obrót, jeśli wczytano
            // z T3D, a nie obrócono jeszcze
...
float TSubModel::MaxY(float4x4 const &m) {
    ...
    auto maxy{0.0f};
    // binary and text models invoke this function at different stages, either
    // after or before geometry data was sent to the geometry manager
    if (m_geometry.handle != null_handle) {
        for (auto const &vertex : GfxRenderer->Vertices(m_geometry.handle)) {
            maxy = std::max(
                maxy, m[0][1] * vertex.position.x +
                          m[1][1] * vertex.position.y +
                          m[2][1] * vertex.position.z + m[3][1]);
        }
...
TSubModel *TModel3d::AddToNamed(const char *Name, TSubModel *SubModel) {
    ...
    if ((sm == nullptr) && (Name != nullptr) &&
        ...
        ErrorLog(
            "Bad model: parent for sub-model \"" + SubModel->pName +
                "\" doesn't exist or is located later in the model data",
            logtype::model);
    }
...
// jedyny poprawny sposób dodawania submodeli, inaczej mogą zginąć przy zapisie
// E3D
void TModel3d::AddTo(TSubModel *tmp, TSubModel *SubModel) {
    if (tmp) {
        // jeśli znaleziony, podłączamy mu jako potomny
        tmp->ChildAdd(SubModel);
    } else {  // jeśli nie znaleziony, podczepiamy do łańcucha głównego
        SubModel->NextAdd(
            Root);  // Ra: zmiana kolejności renderowania wymusza zmianę tu
        Root = SubModel;
    }
...
TSubModel *TModel3d::GetFromName(
    std::string const &Name) const {  // wyszukanie submodelu po nazwie
    if (Name.empty()) return Root;    // potrzebne do terenu z E3D
    if (iFlags &
        0x0200)  // wczytany z pliku tekstowego, wyszukiwanie rekurencyjne
...
glm::vec3 TSubModel::offset(float const Geometrytestoffsetthreshold) const {
    ...
    auto offset{glm::vec3{
        glm::make_mat4(parentmatrix.readArray()) * glm::vec4{0, 0, 0, 1}}};

    if (glm::length(offset) < Geometrytestoffsetthreshold) {
        // offset of zero generally means the submodel has optimized identity
        // matrix for such cases we resort to an estimate from submodel geometry
        // TODO: do proper bounding area calculation for submodel when loading
        // mesh and grab the centre point from it here
        auto const &vertices{
            (m_geometry.handle != null_handle
                 ? GfxRenderer->Vertices(m_geometry.handle)
                 : Vertices)};
        if (false == vertices.empty()) {
            // transformation matrix for the submodel can still contain rotation
            // and/or scaling, so we pass the vertex positions through it rather
            // than just grab them directly
            offset = glm::vec3();
            auto const vertexfactor{1.f / vertices.size()};
            auto const transformationmatrix{
                glm::make_mat4(parentmatrix.readArray())};
            for (auto const &vertex : vertices) {
                offset +=
                    glm::vec3{
                        transformationmatrix * glm::vec4{vertex.position, 1}} *
                    vertexfactor;
            }
        }
    }
...
bool TModel3d::LoadFromFile(std::string const &FileName, bool dynamic) {
    ...
    */
    ...
    bool const result = Root ? (iSubModelsCount > 0)
                             : false;  // brak pliku albo problem z wczytaniem
    if (false == result) {
...
void TSubModel::serialize(
    std::ostream &s, std::vector<TSubModel *> &models,
    std::vector<std::string> &names, std::vector<std::string> &textures,
    ...
    if (m_material > 0 && (Global.iConvertModels & 16))
        flags &= ~0x30;  // don't save phase information, will be guessed on
                         // binary load from material
    ...
    if (m_material <= 0)
        ...
        sn_utils::ls_int32(
            s, (int32_t)get_container_pos(textures, m_materialname));
    ...
    for (size_t i = 0; i < fill; i++) s.put(0);
}
...
void TModel3d::SaveToBinFile(std::string const &FileName) {
    ...
    if (names.size()) {
        ...
        sn_utils::ls_uint32(s, 0);
        for (size_t i = 0; i < names.size(); i++) sn_utils::s_str(s, names[i]);
        auto const pos = s.tellp();
        s.seekp(nam_spos);
...
void TModel3d::deserialize(std::istream &s, size_t size, bool dynamic) {
    ...
    while (s.tellg() < end) {
        ...
        if ((type & 0x00FFFFFF) == MAKE_ID4('S', 'U', 'B', 0))  // submodels
        {
            ...
            (type & 0x00FFFFFF) ==
            MAKE_ID4('V', 'N', 'T', 0))  // geometry vertices
        {
            // we rely on the SUB chunk coming before the vertex data, and on
            // the overall vertex count matching the size of data in the chunk.
            // geometry associated with chunks isn't stored in the same order as
            // the chunks themselves, so we need to sort that out first
            if (Root == nullptr)
                throw std::runtime_error(
                    "e3d: VNT chunk encountered before SUB chunk");
            std::vector<std::pair<int, int> >
                submodeloffsets;  // vertex data offset, submodel index
            submodeloffsets.reserve(iSubModelsCount);
            for (auto submodelindex = 0; submodelindex < iSubModelsCount;
                 ++submodelindex) {
                auto const &submodelgeometry{Root[submodelindex].m_geometry};
                if (submodelgeometry.vertex_count <= 0) {
                    continue;
                }
                submodeloffsets.emplace_back(
                    submodelgeometry.vertex_offset, submodelindex);
            }
            std::sort(
                std::begin(submodeloffsets), std::end(submodeloffsets),
                [](std::pair<int, int> const &Left,
                   std::pair<int, int> const &Right) {
                    return (Left.first) < (Right.first);
                });
            // once sorted we can grab geometry as it comes, and assign it to
            // the chunks it belongs to
                        ...
                        for (auto &vertex : submodel.Vertices) {
                            ...
                            if (submodel.eType < TP_ROTATOR) {
                                ...
                                if ((false == submodel.m_normalizenormals) &&
                                    (std::abs(
                                         glm::length2(vertex.normal) - 1.0f) >
                                     0.01f)) {
                                    submodel.m_normalizenormals = TSubModel::
                                        normalize;  // we don't know if uniform
                                                    // scaling would suffice
                                    WriteLog(
                                        "Bad model: non-unit normal vector(s) "
                                        "encountered during sub-model geometry "
                                        "deserialization",
                                        logtype::model);
                                }
                            }
            ...
            (type & 0x00FFFFFF) ==
            MAKE_ID4('I', 'D', 'X', 0))  // geometry indices
        {
            ...
            if (Root == nullptr)
                throw std::runtime_error(
                    "e3d: IDX chunk encountered before SUB chunk");
            std::vector<std::pair<int, int> >
                submodeloffsets;  // index data offset, submodel index
            submodeloffsets.reserve(iSubModelsCount);
            for (auto submodelindex = 0; submodelindex < iSubModelsCount;
                 ++submodelindex) {
                auto const &submodelgeometry{Root[submodelindex].m_geometry};
                ...
                submodeloffsets.emplace_back(
                    submodelgeometry.index_offset, submodelindex);
            }
            std::sort(
                std::begin(submodeloffsets), std::end(submodeloffsets),
                [](std::pair<int, int> const &Left,
            ...
            // once sorted we can grab indices in a continuous read, and assign
            // them to the chunks they belong to
            ...
            while (s.tellg() < end) Names.push_back(sn_utils::d_str(s));
        }

        s.seekg(end);
    }

    if (!Root) throw std::runtime_error("e3d: no submodels");

    for (size_t i = 0; (int)i < iSubModelsCount; ++i) {
        Root[i].BinInit(Root, Matrices.data(), &Textures, &Names, dynamic);

        if (Root[i].ChildGet()) Root[i].ChildGet()->Parent = &Root[i];
        if (Root[i].NextGet()) Root[i].NextGet()->Parent = Root[i].Parent;
        ...
        Root[i].m_geometry.handle = GfxRenderer->Insert(
            Root[i].Indices, Root[i].Vertices, m_geometrybank, type);
    }
}

void TSubModel::BinInit(
    TSubModel *s, float4x4 *m, std::vector<std::string> *t,
    std::vector<std::string> *n,
    bool dynamic) {  // ustawienie wskaźników w submodelu
    // m7todo: brzydko
    iVisible = 1;  // tymczasowo używane
    Child =
        (iChild > 0) ? s + iChild : nullptr;  // zerowy nie może być potomnym
    Next = (iNext > 0) ? s + iNext : nullptr;  // zerowy nie może być następnym
    fMatrix = ((iMatrix >= 0) && m) ? m + iMatrix : nullptr;

    if (n->size() && (iName >= 0)) {
        pName = n->at(iName);
        if (!pName.empty()) {  // jeśli dany submodel jest zgaszonym światłem,
                               // to domyślnie go ukrywamy
            if (starts_with(
                    pName, "Light_On")) {  // jeśli jest światłem numerowanym
                iVisible = 0;  // to domyślnie wyłączyć, żeby się nie nakładało
                               // z obiektem "Light_Off"
            } else if (dynamic) {  // inaczej wyłączało smugę w latarniach
                if (ends_with(pName, "_on")) {
                    // jeśli jest kontrolką w stanie zapalonym to domyślnie
                    // wyłączyć, żeby się nie nakładało z obiektem "_off"
                    iVisible = 0;
                }
            }
        ...
        pName = "";
    if (iTexture > 0) {  // obsługa stałej tekstury
        auto const materialindex = static_cast<std::size_t>(iTexture);
        if (materialindex < t->size()) {
            m_materialname = t->at(materialindex);
            /*
                        if( m_materialname.find_last_of( "/" ) ==
               std::string::npos ) { m_materialname =
               Global.asCurrentTexturePath + m_materialname;
                        }
            */
            m_material = GfxRenderer->Fetch_Material(m_materialname);
            ...
            if (m_material != null_handle) {
                ...
                /*
                                // if material does have opacity set, replace
                   submodel opacity with it if (mat.opacity)
                                {
                                    iFlags &= ~0x30;
                                    if (*mat.opacity == 0.0f)
                                        iFlags |= 0x20; // translucent
            ...
            ErrorLog(
                "Bad model: reference to nonexistent texture index in "
                "sub-model" +
                    (pName.empty() ? "" : " \"" + pName + "\""),
                logtype::model);
    ...
    if (eType == TP_STARS) {
        ...
        // we've added light glare which needs to be rendered during transparent
        // phase, but models converted to e3d before addition won't have the
        // render flag set correctly for this so as a workaround we're doing it
        // here manually
        iFlags |= 0x20;
    }
    ...
    f4Specular = glm::vec4{
        clamp(f4Specular.r, 0.0f, 1.0f), clamp(f4Specular.g, 0.0f, 1.0f),
        clamp(f4Specular.b, 0.0f, 1.0f), clamp(f4Specular.a, 0.0f, 1.0f)};

    iFlags &=
        ~0x0200;  // wczytano z pliku binarnego (nie jest właścicielem tablic)
...
void TModel3d::LoadFromBinFile(
    std::string const &FileName,
    bool dynamic) {  // wczytanie modelu z pliku binarnego
    WriteLog(
        "Loading binary format 3d model data from \"" + FileName + "\"...",
        logtype::model);
    ...
    WriteLog(
        "Finished loading 3d model data from \"" + FileName + "\"",
        logtype::model);
};

TSubModel *TModel3d::AppendChildFromGeometry(
    ...
    sm->iFlags |= 0x8000;
    sm->WillBeAnimated();
    if (vertices.empty()) sm->iFlags &= ~0x3F;
...
void TModel3d::LoadFromTextFile(
    std::string const &FileName,
    bool dynamic) {  // wczytanie submodelu z pliku tekstowego
    WriteLog(
        "Loading text format 3d model data from \"" + FileName + "\"...",
        logtype::model);
    iFlags |= 0x0200;  // wczytano z pliku tekstowego (właścicielami tablic są
                       // submodle)
    cParser parser(
        FileName, cParser::buffer_FILE);  // Ra: tu powinno być "models/"...
    TSubModel *SubModel;
    ...
    while (token != "" || parser.eof()) {
        ...
        parser.getTokens(
            1, false);  // nazwa submodelu nadrzędnego bez zmieny na małe
        parser >> parent;
        if (parent == "") {
            break;
        }
    ...
    // submodel ale bujanie kabiny nadal używa bananów :( od 393 przywrócone,
    // ale z dodatkowym warunkiem
    if (Global.iConvertModels &
        4) {  // automatyczne banany czasem psuły przechylanie kabin...
        if (dynamic && Root) {
            if (Root->NextGet())  // jeśli ma jakiekolwiek kolejne
            {  // dynamic musi mieć "banana", bo tylko pierwszy obiekt jest
               // animowany, a następne nie
...
void TModel3d::Init() {  // obrócenie początkowe układu współrzędnych, dla
                         // pojazdów wykonywane po analizie animacji
    if (iFlags & 0x8000) return;  // operacje zostały już wykonane
    if (Root) {
        if (iFlags & 0x0200) {
        ...
        while ((root = root->Next) != nullptr) {
            Root->m_boundingradius =
                std::max(Root->m_boundingradius, root->m_boundingradius);
        }
...
int TModel3d::TerrainCount()
    const {  // zliczanie kwadratów kilometrowych (główna
...
TSubModel *TModel3d::TerrainSquare(
    int n) {  // pobieranie wskaźnika do submodelu (n)
    int i = 0;
    TSubModel *r = Root;
    while (i < n) {
        r = r->NextGet();
