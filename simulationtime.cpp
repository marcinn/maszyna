...
void scenario_time::init(std::time_t timestamp) {
    ...
    auto const requestedtime{clamp_circular<int>(
        m_time.wHour * 60 + m_time.wMinute + Global.ScenarioTimeOffset * 60,
        24 * 60)};
    ...
    if (Global.fMoveLight > 0.0) {
        ...
        daymonth(
            ...
            static_cast<WORD>(Global.fMoveLight));
    }
    ...
    // if the time is taken from the local clock leave the seconds intact,
    ...
    // retrieve relevant time zone info from system registry (or fall back on
    // supplied default)
    ...
    if (m_yearday < year_day(
                        timezoneinfo.DaylightDate.wDay,
                        timezoneinfo.DaylightDate.wMonth, m_time.wYear)) {
        zonebias += timezoneinfo.StandardBias;
    } else if (
        m_yearday < year_day(
                        timezoneinfo.StandardDate.wDay,
                        timezoneinfo.StandardDate.wMonth, m_time.wYear)) {
...
void scenario_time::update(double const Deltatime) {
...
int scenario_time::year_day(int Day, const int Month, const int Year) const {
...
void scenario_time::daymonth(
    WORD &Day, WORD &Month, WORD const Year, WORD const Yearday) {
    WORD daytab[2][13] = {
...
int scenario_time::julian_day() const {
    int yy = (m_time.wYear < 0 ? m_time.wYear + 1 : m_time.wYear) -
             std::floor((12 - m_time.wMonth) / 10.f);
...
int scenario_time::day_of_week(
    int const Day, int const Month, int const Year) const {
    ...
    int const h =
        (q + (26 * (m + 1) / 10) + y + (y / 4) + 6 * (y / 100) + (y / 400)) % 7;

    /*	return ( (h + 5) % 7 ) + 1; // iso week standard, with monday = 1
     */ return ((h + 6) % 7) +
        1;  // sunday = 1 numbering method, used in north america, japan
}

// calculates day of month for specified weekday of specified month of the year
int scenario_time::day_of_month(
    int const Week, int const Weekday, int const Month, int const Year) const {
    int day = 0;
    ...
    if (Week == 5) {
        // 5th week potentially indicates last week in the month, not
        // necessarily actual 5th
        char const daytab[2][13] = {
            {0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31},
            {0, 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31}};
        int const leap{is_leap(Year)};
...
int scenario_time::weekdays(int const First, int const Second) const {
    if (Second >= First) {
        return Second - First;
    } else {
...
void scenario_time::convert_transition_time(SYSTEMTIME &Time) const {
    // NOTE: windows uses 0-6 range for days of week numbering, our methods use
    // 1-7
    Time.wDay =
        day_of_month(Time.wDay, Time.wDayOfWeek + 1, Time.wMonth, m_time.wYear);
}

bool scenario_time::is_leap(int const Year) const {
