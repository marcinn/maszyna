...
void TController::hint(
    driver_hint const Value, hintpredicate const Predicate,
    float const Predicateparameter) {
    if (Predicate(Predicateparameter)) {
        return;
    }
    // check already recorded hints, if there's a match update it instead of
    // adding a new one
...
void TController::update_hints() {
    m_hints.remove_if([](auto const &Hint) {
        return (std::get<hintpredicate>(Hint)(std::get<float>(Hint)));
    });
}

void TController::remove_hint(driver_hint const Value) {
    m_hints.remove_if([=](auto const &Hint) {
        return (std::get<driver_hint>(Hint) == Value);
    });
}

void TController::remove_train_brake_hints() {
...
void TController::remove_master_controller_hints() {
...
void TController::remove_reverser_hints() {
...
void TController::cue_action(
    driver_hint const Action, float const Actionparameter) {
    switch (Action) {
        // battery
        case driver_hint::batteryon: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    (mvOccupied->BatteryStart != start_t::manual) ||
                    (mvOccupied->Power24vIsAvailable == true));
            });
            break;
        }
        case driver_hint::batteryoff: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    (mvOccupied->BatteryStart != start_t::manual) ||
                    (mvOccupied->Power24vIsAvailable == false));
            });
            break;
        }
        // battery
        ...
        case driver_hint::cabdeactivation: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    (mvOccupied->AutomaticCabActivation) ||
                    ((mvOccupied->CabMaster == false) &&
                     (mvOccupied->CabActive == 0)));
            });
            break;
        }
        // radio
        ...
        case driver_hint::oilpumpon: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                ...
                return (
                    (device.start_type != start_t::manual) ||
                    (device.is_enabled == true) || (device.is_active == true) ||
                    (mvOccupied->Mains));
            });
            break;
        }
        ...
        case driver_hint::frontpantographvalveon: {
            ...
            hint(
                ...
                [this](float const Parameter) -> bool {
                    return (
                        (mvPantographUnit->Pantographs[end::front]
                             .valve.is_active == true) ||
                        ((Parameter > 0) && (mvOccupied->Vel > Parameter)));
                },
                Actionparameter);
            break;
        }
        case driver_hint::frontpantographvalveoff: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    mvPantographUnit->Pantographs[end::front].valve.is_active ==
                    false);
            });
            break;
        }
        case driver_hint::rearpantographvalveon: {
            ...
            hint(
                ...
                [this](float const Parameter) -> bool {
                    return (
                        (mvPantographUnit->Pantographs[end::rear]
                             .valve.is_active == true) ||
                        ((Parameter > 0) && (mvOccupied->Vel > Parameter)));
                },
                Actionparameter);
        ...
        case driver_hint::rearpantographvalveoff: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    mvPantographUnit->Pantographs[end::rear].valve.is_active ==
                    false);
            });
            break;
        }
        ...
        case driver_hint::primaryconverteroverloadreset: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    (mvOccupied->ConverterOverloadRelayStart !=
                     start_t::manual) ||
                    (mvOccupied->ConvOvldFlag == false));
            });
            break;
        }
        case driver_hint::maincircuitgroundreset: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    (mvOccupied->GroundRelayStart != start_t::manual) ||
                    (mvOccupied->GroundRelay == true));
            });
            break;
        }
        ...
        case driver_hint::linebreakeropen: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                // TBD, TODO: replace with consist-wide flag set true if any
                // line breaker is closed?
        ...
        case driver_hint::manualbrakon: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    ...
                    (mvOccupied->MBrake == false) ||
                    (mvOccupied->ManualBrakePos == ManualBrakePosNo));
            });
            break;
        }
        ...
        case driver_hint::mastercontrollersetidle: {
            if (AIControllFlag) {
                while ((mvControlling->RList[mvControlling->MainCtrlPos].Mn ==
                        0) &&
        ...
        case driver_hint::mastercontrollersetseriesmode: {
            if (AIControllFlag) {
                if (mvControlling->RList[mvControlling->MainCtrlPos].Bn > 1) {
                    ...
                    while (
                        (mvControlling->RList[mvControlling->MainCtrlPos].Bn >
                         1) &&
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    mvControlling->RList[mvControlling->MainCtrlPos].Bn < 2);
            });
            break;
        }
        case driver_hint::mastercontrollersetzerospeed: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    ...
                    (mvControlling->IsScndCtrlNoPowerPos()));
            });
            break;
        }
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    ...
                    (mvOccupied->EIMCtrlType > 0
                         ...
                         : ((mvControlling->IsScndCtrlMaxPowerPos()) &&
                            (mvControlling->IsMainCtrlMaxPowerPos()))));
            });
            break;
        }
        ...
        case driver_hint::watercircuitslinkon: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    (mvControlling->dizel_heat.auxiliary_water_circuit ==
                     false) ||
                    (mvControlling->WaterCircuitsLink == true));
            });
            break;
        }
        case driver_hint::watercircuitslinkoff: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    (mvControlling->dizel_heat.auxiliary_water_circuit ==
                     false) ||
                    (mvControlling->WaterCircuitsLink == false));
            });
            break;
        }
        ...
        case driver_hint::waitpantographpressuretoolow: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    ...
                    (is_emu()
                         ? (mvPantographUnit->PantPressLockActive ? 4.6 : 2.6)
                         : 4.2));
            });
            break;
        }
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    is_equal(
                        ...
                        mvOccupied->Handle->GetPos(bh_RP), 0.2) ||
        ...
        case driver_hint::trainbrakeapply: {
            if (AIControllFlag) {
                // za radą yB ustawiamy pozycję 3 kranu (ruszanie kranem w
                // innych miejscach powino zostać wyłączone)
        ...
        case driver_hint::brakingforcedecrease: {
            ...
            hint(
                ...
                std::min(
                    0.0,
                    0.95 * std::abs(mvControlling
                                        ->Fb)));  // keep hint until 5% decrease
            break;
        }
        case driver_hint::brakingforceincrease: {
            ...
            hint(
                ...
                [this](float const Parameter) -> bool {
                    return (
                        (std::abs(mvControlling->Fb) > Parameter) ||
                        (is_equal(
                            mvOccupied->fBrakeCtrlPos,
                            mvOccupied->Handle->GetPos(bh_EB), 0.2)));
                },
                std::max(
                    0.0, std::max(
        ...
        case driver_hint::brakingforcesetzero: {  // releases both train and
                                                  // independent brake
            if (AIControllFlag) {
                while (true == DecBrake()) {
                    ;
        ...
        case driver_hint::independentbrakeapply: {
            ...
            hint(
                ...
                (LocalBrakePosNo - (mvOccupied->EIMCtrlEmergency ? 1 : 0)) /
                    LocalBrakePosNo);
            break;
        }
        ...
        case driver_hint::sandingon: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    (mvControlling->Sand == 0) ||
                    (mvControlling->SandDose == true) ||
                    (mvControlling->SlippingWheels == false));
            });
            break;
        }
        ...
        case driver_hint::antislip: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    (mvControlling->ASBType != 1) ||
                    ((mvControlling->Hamulec->GetBrakeStatus() & b_asb) != 0) ||
                    (mvControlling->SlippingWheels == false));
            });
            break;
        }
        ...
        case driver_hint::hornon: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                // NOTE: we provide slightly larger horn activation window for
                // human driver
                return (
                    (fWarningDuration + 5.0 < 0.05) ||
        ...
        case driver_hint::hornoff: {
            ...
            /*
                        // NOTE: for human driver the hint to switch horn on
               will disappear automatically remove_hint( driver_hint::hornon );
                        hint(
                            Action,
                            [this](float const Parameter) -> bool {
        ...
        case driver_hint::consistdoorlockson: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    (mvOccupied->Doors.has_lock == false) ||
                    (mvOccupied->Doors.lock_enabled == true));
            });
            break;
        }
        // departure signal
        case driver_hint::departuresignalon: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                return (
                    (mvOccupied->Doors.has_warning == false) ||
                    (mvOccupied->Doors.has_autowarning == true) ||
                    (mvOccupied->DepartureSignal == true) ||
                    mvOccupied->Vel > 5.0);
            });
            break;
        }
        ...
        case driver_hint::doorrightopen: {
            if ((AIControllFlag) ||
                (pVehicle->MoverParameters->Doors.open_control ==
                 control_t::conductor)) {
        ...
        case driver_hint::consistlightson: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                ...
                return (
                    (Global.fLuminance * ConsistShade > 0.40) ||
                    (device.start_type != start_t::manual) ||
                    ((device.is_enabled == true) &&
                     (device.is_disabled == false)) ||
                    (device.is_active == true));
        ...
        case driver_hint::consistlightsoff: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                auto const &device{mvOccupied->CompartmentLights};
                return (
                    (Global.fLuminance * ConsistShade < 0.35) ||
                    (device.start_type != start_t::manual) ||
                    ((device.is_enabled == false) &&
                     (device.is_active == false)));
            });
        ...
        case driver_hint::couplingadapterattach: {
            ...
            hint(Action, [this](float const Parameter) -> bool {
                auto const &device{pVehicles[end::front]
                                       ->MoverParameters
                                       ->Couplers[static_cast<int>(Parameter)]};
        ...
        case driver_hint::headcodetb1: {
            // HACK: the 'front' and 'rear' of the consist is determined by
            // current consist direction since direction shouldn't affect Tb1
            // light configuration, we 'counter' this behaviour by virtually
            // swapping end vehicles
            if (AIControllFlag) {
                if (mvOccupied->DirActive >= 0) {
                    Lights(light::headlight_right, light::headlight_left);
                } else {
                    Lights(light::headlight_left, light::headlight_right);
                }
            }
            remove_hint(driver_hint::headcodepc1);
            remove_hint(driver_hint::headcodepc2);
            ...
            remove_hint(driver_hint::lightsoff);
            hint(Action, [this](float const Parameter) -> bool {
                auto const activeend{
                    mvOccupied->CabActive >= 0 ? end::front : end::rear};
                auto const consistfront{
                    mvOccupied->DirActive >= 0 ? end::front : end::rear};
                return (
                    pVehicles[consistfront]->has_signal_on(
                        activeend, light::headlight_right) &&
                    pVehicles[1 - consistfront]->has_signal_on(
                        1 - activeend, light::headlight_left));
            });
            break;
        }
        case driver_hint::headcodepc5: {
            if ((AIControllFlag) ||
                (Global.AITrainman &&
                 (false == pVehicles[end::rear]->is_connected(
                               pVehicle, coupling::control)))) {
                pVehicles[end::rear]->RaLightsSet(
                    -1, light::redmarker_left | light::redmarker_right |
