...
void TTranscripts::AddLine(
    std::string const &txt, float show, float hide, bool it) {
    if (show == hide) {
        return;
    }  // komentarz jest ignorowany

    // TODO: replace the timeangledeg mess with regular time points math
    show = Global.fTimeAngleDeg +
           show / 240.0;  // jeśli doba to 360, to 1s będzie równe 1/240
    hide = Global.fTimeAngleDeg + hide / 240.0;

    TTranscript transcript;
    transcript.asText = ExchangeCharInString(
        txt, '|', ' ');  // NOTE: legacy transcript lines use | as new line mark
    ...
    // TODO, TBD: sort the transcript lines? in theory, they should be coming
    // arranged in the right order anyway short of cases with multiple sounds
    // overleaping
...
void TTranscripts::Add(std::string const &txt, bool backgorund) {
    ...
    std::string asciitext{txt};
    win1250_to_ascii(
        asciitext);  // TODO: launch relevant conversion table based on language
    cParser parser(asciitext);
    while (true == parser.getTokens(3, false, "[]\n")) {
        float begin, end;
        std::string transcript;
        parser >> begin >> end >> transcript;
    ...
    while (false == transcript.empty()) {
        //        WriteLog( "Transcript text with no display/hide times: \"" +
        //        transcript + "\"" );
...
void TTranscripts::Update() {
    ...
    while (false == aLines.empty()) {
        ...
        aLines.pop_front();  // this line expired, discard it and start anew
                             // with the next one
    }
    // update next refresh time
    if (false == aLines.empty()) {
        fRefreshTime = aLines.front().fHide;
    } else {
        fRefreshTime = 360.0f;
    }
