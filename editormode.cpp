...
void editor_mode::editormode_input::poll() { keyboard.poll(); }

editor_mode::editor_mode() { m_userinterface = std::make_shared<editor_ui>(); }

// initializes internal data structures of the mode. returns: true on success,
// false otherwise
bool editor_mode::init() {
    Camera.Init(
        {0, 15, 0}, {glm::radians(-30.0), glm::radians(180.0), 0}, nullptr);
...
void editor_mode::enter() {
    ...
    // NOTE: camera placement is effectively a copy of drivermode DistantView(
    // true ) TBD, TODO: refactor into a common vehicle method?
...
void editor_mode::exit() {
    ...
    Application.set_cursor(
        (Global.ControlPicking ? GLFW_CURSOR_NORMAL : GLFW_CURSOR_DISABLED));
...
void editor_mode::on_cursor_pos(
    double const Horizontal, double const Vertical) {
    auto const mousemove{
        glm::dvec2{Horizontal, Vertical} - m_input.mouse.position()};
    ...
    if (mode_translation()) {
        ...
        if (mode_translation_vertical()) {
            ...
            auto const mouseworldposition{
                Camera.Pos + GfxRenderer->Mouse_Position()};
        ...
        auto const quantization{
            (mode_snap() ? 15.f :  // TODO: put quantization value in a variable
                 0.f)};
        ...
        if( Action == GLFW_PRESS ) {
            ...
            GfxRenderer->Pick_Node_Callback(
                [ this ]( scene::basic_node *node ) {
                    ...
                    dynamic_cast<editor_ui*>( m_userinterface.get() )->set_node(
m_node ); } );
        }
        else {
            // left button release
...
*/
void editor_mode::on_mouse_button(
    ...
    if (Button == GLFW_MOUSE_BUTTON_LEFT) {
        if (Action == GLFW_PRESS) {
            // left button press
            auto const mode =
                static_cast<editor_ui *>(m_userinterface.get())->mode();

            m_node = nullptr;

            GfxRenderer->Pick_Node_Callback([this,
                                             mode](scene::basic_node *node) {
                ...
                if (mode == nodebank_panel::MODIFY) {
                    ...
                    std::string name =
                        "editor_" + std::to_string(LocalRandom(0.0, 100000.0));

                    if (!src) return;

                    TAnimModel *cloned = simulation::State.create_model(
                        *src, name, Camera.Pos + GfxRenderer->Mouse_Position());
            ...
            // left button release
            if (m_node) Application.set_cursor(GLFW_CURSOR_NORMAL);
            m_dragging = false;
            // prime history stack for another snapshot
            m_takesnapshot = true;
        }
...
void editor_mode::on_event_poll() { m_input.poll(); }

bool editor_mode::is_command_processor() const { return false; }
...
bool editor_mode::mode_rotation() const { return (true == Global.altState); }

bool editor_mode::mode_snap() const { return (true == Global.ctrlState); }
