...
driver_ui::driver_ui() {
    ...
    if (Global.gui_showtranscripts) add_external_panel(&m_transcriptspanel);
    ...
    m_scenariopanel.size_max = {
        Global.fb_size.x * 0.95f, Global.fb_size.y * 0.95};
    ...
    m_transcriptspanel.size_max = {
        Global.fb_size.x * 0.95, Global.fb_size.y * 0.95};
...
void driver_ui::render_menu_contents() {
    ...
    if (ImGui::BeginMenu(STR_C("Mode windows"))) {
        ...
        ImGui::MenuItem(
            m_scenariopanel.title.c_str(), "F1", &m_aidpanel.is_open);
        ImGui::MenuItem(STR_C("Timetable"), "F2", &m_timetablepanel.is_open);
        ImGui::MenuItem(
            m_debugpanel.name().c_str(), "F12", &m_debugpanel.is_open);
        ImGui::MenuItem(m_mappanel.name().c_str(), "Tab", &m_mappanel.is_open);
        ImGui::MenuItem(
            m_vehiclelist.name().c_str(), nullptr, &m_vehiclelist.is_open);
        ImGui::MenuItem(
            m_trainingcardpanel.name().c_str(), nullptr,
            &m_trainingcardpanel.is_open);
        ImGui::MenuItem(
            m_cameraviewpanel.name().c_str(), nullptr,
            &m_cameraviewpanel.is_open);
        if (DebugModeFlag)
            ImGui::MenuItem(
                m_perfgraphpanel.name().c_str(), nullptr,
                &m_perfgraphpanel.is_open);

        if (ImGui::MenuItem(m_timepanel.name().c_str())) m_timepanel.open();

        ImGui::EndMenu();
    }
}

// potentially processes provided input key. returns: true if key was processed,
// false otherwise
bool driver_ui::on_key(int const Key, int const Action) {
    if (ui_layer::on_key(Key, Action)) return true;
...
// potentially processes provided mouse movement. returns: true if the input was
...
// potentially processes provided mouse button. returns: true if the input was
// processed, false otherwise
...
void driver_ui::update() {
    ...
    auto ispaused{
        (false == DebugModeFlag) && ((Global.iPause & pausemask) != 0)};
...
void driver_ui::set_cursor(bool const Visible) {
    if (Visible) {
        ...
        Application.set_cursor_pos(
            Global.window_size.x / 2, Global.window_size.y / 2);
    } else {
