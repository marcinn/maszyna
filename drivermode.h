...
class driver_mode : public application_mode {
   public:
    ...
    // methods
    // initializes internal data structures of the mode. returns: true on
    ...
    // input handlers
    void on_key(
        int const Key, int const Scancode, int const Action,
        int const Mods) override;
    void on_cursor_pos(double const Horizontal, double const Vertical) override;
    void on_mouse_button(
        int const Button, int const Action, int const Mods) override;
   ...
   private:
    // types
    enum view { consistfront, consistrear, bogie, driveby, count_ };
    ...
    struct drivermode_input {
        ...
        glm::dvec2 mouse_pickmodepos;  // stores last mouse position in control
                                       // picking mode
        ...
        bool init();
        void poll();
        std::string binding_hints(
            std::pair<user_command, user_command> const &Commands) const;
        std::pair<user_command, user_command> command_fallback(
    ...
    std::array<basic_event *, 10> KeyEvents{
        nullptr};  // eventy wyzwalane z klawiaury
    TCamera Camera;
    TCamera DebugCamera;
    ...
    TDynamicObject *pDynamicNearest{
        nullptr};  // vehicle nearest to the active camera. TODO: move to camera
    ...
    double m_primaryupdateaccumulator{
        m_secondaryupdaterate};  // keeps track of elapsed simulation time, for
                                 // core fixed step routines
    double m_secondaryupdateaccumulator{
        m_secondaryupdaterate};  // keeps track of elapsed simulation time, for
                                 // less important fixed step routines
