...
bool TButton::Init(
    std::string const &asName, TModel3d const *pModel, bool bNewOn) {
    if (pModel == nullptr) {
        return false;
    }
...
void TButton::Load(cParser &Parser, TDynamicObject const *Owner) {
    ...
    std::array<TModel3d *, 3> sources{
        Owner->mdKabina, Owner->mdLowPolyInt, Owner->mdModel};
...
bool TButton::Load_mapping(cParser &Input) {
...
glm::vec3 TButton::model_offset() const {
    auto const submodel{
        (pModelOn    ? pModelOn
         : pModelOff ? pModelOff
                     : nullptr)};
...
void TButton::Turn(bool const State) {
    if (State != m_state) {
        m_state = State;
        play();
        Update();
...
void TButton::Update(bool const Power) {
    ...
    if (pModelOn != nullptr) {
        pModelOn->iVisible = m_state;
    }
    if (pModelOff != nullptr) {
        pModelOff->iVisible = (!m_state);
    }
...
void TButton::AssignBool(bool const *bValue) { bData = bValue; }
...
void TButton::gain(float new_volume) {
