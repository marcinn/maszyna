...
void opengl_camera::update_frustum(
    ...
    m_inversetransformation =
        glm::inverse(Projection * glm::mat4{glm::mat3{Modelview}});
...
bool opengl_camera::visible(TDynamicObject const *Dynamic) const {
    ...
    // we're giving vehicles some extra padding, to allow for things like shared
    // bogeys extending past the main body
    return (
        m_frustum.sphere_inside(
            Dynamic->GetPosition(), Dynamic->radius() * 1.25) > 0.0f);
}
...
void opengl_camera::draw(glm::vec3 const &Offset) const {
    ...
    for (auto const pointindex : frustumshapepoinstorder) {
        ::glVertex3fv(
            glm::value_ptr(glm::vec3{m_frustumpoints[pointindex]} - Offset));
    }
    ::glEnd();
}
