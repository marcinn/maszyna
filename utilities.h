...
/*rozne takie duperele do operacji na stringach w paszczalu, pewnie w delfi sa
 * lepsze*/
/*konwersja zmiennych na stringi, funkcje matematyczne, logiczne, lancuchowe,
 * I/O etc*/

#define sign(x) ((x) < 0 ? -1 : ((x) > 0 ? 1 : 0))

#define DegToRad(a) \
    ((M_PI / 180.0) * (a))  //(a) w nawiasie, bo może być dodawaniem
...
#define MAKE_ID4(a, b, c, d)                                   \
    (((std::uint32_t)(d) << 24) | ((std::uint32_t)(c) << 16) | \
...
inline double Random() { return Random(0.0, 1.0); }

inline double Random(double b) { return Random(0.0, b); }

inline double LocalRandom() { return LocalRandom(0.0, 1.0); }

inline double LocalRandom(double b) { return LocalRandom(0.0, b); }
...
inline double BorlandTime() {
    ...
    /*
        // std alternative
        auto timesinceepoch =
       std::chrono::system_clock::now().time_since_epoch(); return
...
std::string ExchangeCharInString(
    std::string const &Source, char const From,
...
std::string to_minutes_str(
    float const Minutes, bool const Leadingzero, int const Width);
...
std::string to_string(glm::tvec3<Type_, Precision_> const &Value) {
    return to_string(Value.x, 2) + ", " + to_string(Value.y, 2) + ", " +
           to_string(Value.z, 2);
}

template <typename Type_, glm::precision Precision_ = glm::defaultp>
std::string to_string(
    glm::tvec4<Type_, Precision_> const &Value, int const Width = 2) {
    return to_string(Value.x, Width) + ", " + to_string(Value.y, Width) + ", " +
           to_string(Value.z, Width) + ", " + to_string(Value.w, Width);
}
...
std::string Bezogonkow(
    std::string Input, bool const Underscorestospaces = false);

std::string win1250_to_utf8(const std::string &input);

inline std::string extract_value(
    std::string const &Key, std::string const &Input) {
    // NOTE, HACK: the leading space allows to uniformly look for " variable="
    // substring
    ...
    if (lookup != std::string::npos) {
        value =
            input.substr(input.find_first_not_of(' ', lookup + Key.size() + 2));
        lookup = value.find(' ');
        if (lookup != std::string::npos) {
...
bool extract_value(
    Type_ &Variable, std::string const &Key, std::string const &Input,
...
template <>
bool extract_value(
    bool &Variable, std::string const &Key, std::string const &Input,
    std::string const &Default);

bool FileExists(std::string const &Filename);

std::pair<std::string, std::string> FileExists(
...
// potentially erases file extension from provided file name. returns: true if
// extension was removed, false otherwise
...
// potentially replaces backward slashes in provided file path with
// unix-compatible forward slashes
...
std::ptrdiff_t len_common_prefix(
...
template <typename Type_>
Type_ is_equal(
    Type_ const &Left, Type_ const &Right, Type_ const Epsilon = 1e-5) {
    if (Epsilon != 0) {
...
// keeps the provided value in specified range 0-Range, as if the range was
// circular buffer
template <typename Type_>
...
template <typename Type_>
Type_ clamp_power_of_two(
    Type_ Value, Type_ const Min = static_cast<Type_>(1),
...
Type_ min_speed(Type_ const Left, Type_ const Right) {
    ...
    return std::min(
        (Left != -1 ? Left : std::numeric_limits<Type_>::max()),
...
template <typename Type_>
Type_ interpolate(
    Type_ const &First, Type_ const &Second, double const Factor) {
...
bool degenerate(
    VecType_ const &Vertex1, VecType_ const &Vertex2, VecType_ const &Vertex3) {
    //  degenerate( A, B, C, minarea ) = ( ( B - A ).cross( C - A )
    //  ).lengthSquared() < ( 4.0f * minarea * minarea );
    return (
        glm::length2(glm::cross(Vertex2 - Vertex1, Vertex3 - Vertex1)) == 0.0);
}

// calculates bounding box for provided set of points
template <class Iterator_, class VecType_>
void bounding_box(
    VecType_ &Mincorner, VecType_ &Maxcorner, Iterator_ First, Iterator_ Last) {
    Mincorner =
        VecType_(std::numeric_limits<typename VecType_::value_type>::max());
    Maxcorner =
        VecType_(std::numeric_limits<typename VecType_::value_type>::lowest());
...
// finds point on specified segment closest to specified point in 3d space.
// returns: point on segment as value in range 0-1 where 0 = start and 1 = end
// of the segment
template <typename VecType_>
typename VecType_::value_type nearest_segment_point(
    VecType_ const &Segmentstart, VecType_ const &Segmentend,
...
std::string deserialize_random_set(
...
void deserialize_map(MapType_ &Map, cParser &Input) {
    while (Input.ok() && !Input.eof()) {
        auto const key{Input.getToken<typename MapType_::key_type>(false)};
        auto const value{
            Input.getToken<typename MapType_::mapped_type>(false, "\n")};
...
// NOTE: doesn't do any locking itself, it's merely for cleaner argument
// arrangement and passing
template <typename Type_>
struct lockable {
    Type_ data;
    std::mutex mutex;
...
// basic wrapper simplifying use of std::condition_variable for most typical
// cases. has its own mutex and secondary variable to ignore spurious wakeups
class condition_variable {
   public:
    ...
    void wait_for(const std::chrono::duration<Rep_, Period_> &Time) {
        std::unique_lock<std::mutex> lock(m_mutex);
        m_condition.wait_for(
            lock, Time, [this]() { return m_spurious == false; });
    }
    void notify_one() {
        spurious(false);
