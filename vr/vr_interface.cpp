...
bool vr_interface_factory::register_backend(
    const std::string &backend, vr_interface_factory::create_method func) {
    backends[backend] = func;
    return true;
}

std::unique_ptr<vr_interface> vr_interface_factory::create(
...
vr_interface_factory *vr_interface_factory::get_instance() {
    if (!instance) instance = new vr_interface_factory();
