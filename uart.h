...
class uart_input {
   public:
    ...
    uart_input();
    ~uart_input();
    bool init() { return recall_bindings(); }
   ...
   private:
    ...
    using input_pin_t =
        std::tuple<std::size_t, input_type_t, user_command, user_command>;
    ...
    std::array<std::uint8_t, 16>
        old_packet;  // TBD, TODO: replace with vector of configurable size?
    ...
    std::uint8_t m_trainstatecab{0};  // helper, keeps track of last active cab.
                                      // 0: front cab, 1: rear cab
};
