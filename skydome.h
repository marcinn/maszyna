...
class CSkyDome {
   public:
    ...
    std::vector<glm::vec3> const &vertices() const { return m_vertices; }
    std::vector<glm::vec3> &colors() { return m_colours; }
    std::vector<std::uint16_t> const &indices() const { return m_indices; }
   ...
   private:
    ...
    bool m_dirty{
        true};  // indicates sync state between simulation and gpu sides
