...
ui_panel::ui_panel(std::string const &Identifier, bool const Isopen)
    : m_name(Identifier), is_open(Isopen) {}
...
void ui_panel::render() {
    ...
    if (ImGui::Begin(panelname.c_str(), &is_open, flags)) {
        ...
        popups.remove_if(
            [](std::unique_ptr<ui::popup> &popup) { return popup->render(); });
    }
...
ui_layer::ui_layer() {
    if (Global.loading_log) add_external_panel(&m_logpanel);
...
void ::ui_layer::load_random_background() {
    ...
    for (auto &f : std::filesystem::directory_iterator("textures/logo"))
        if (f.is_regular_file())
            images.emplace_back(
                std::filesystem::relative(f.path(), "textures/").string());

    if (!images.empty()) {
        std::string &selected =
            images[std::lround(LocalRandom(images.size() - 1))];
...
void ui_layer::imgui_style() {
    ...
    colors[ImGuiCol_Tab] = imvec_lerp(
        colors[ImGuiCol_Header], colors[ImGuiCol_TitleBgActive], 0.80f);
    colors[ImGuiCol_TabHovered] = colors[ImGuiCol_HeaderHovered];
    colors[ImGuiCol_TabActive] = imvec_lerp(
        colors[ImGuiCol_HeaderActive], colors[ImGuiCol_TitleBgActive], 0.60f);
    colors[ImGuiCol_TabUnfocused] =
        imvec_lerp(colors[ImGuiCol_Tab], colors[ImGuiCol_TitleBg], 0.80f);
    colors[ImGuiCol_TabUnfocusedActive] =
        imvec_lerp(colors[ImGuiCol_TabActive], colors[ImGuiCol_TitleBg], 0.40f);
...
bool ui_layer::init(GLFWwindow *Window) {
    ...
    if (FileExists("fonts/dejavusans.ttf"))
        font_default = m_imguiio->Fonts->AddFontFromFileTTF(
            "fonts/dejavusans.ttf", Global.ui_fontsize, nullptr, &ranges[0]);
    if (FileExists("fonts/dejavusansmono.ttf"))
        font_mono = m_imguiio->Fonts->AddFontFromFileTTF(
            "fonts/dejavusansmono.ttf", Global.ui_fontsize, nullptr,
            &ranges[0]);
...
void ui_layer::update() {
    for (auto *panel : m_panels) panel->update();

    for (auto it = m_ownedpanels.rbegin(); it != m_ownedpanels.rend(); it++) {
        (*it)->update();
        if (!(*it)->is_open) m_ownedpanels.erase(std::next(it).base());
    }
}
...
void ui_layer::begin_ui_frame() { begin_ui_frame_internal(); }
...
void ui_layer::render_quit_widget() {
    if (!m_quit_active) return;

    ImGui::SetNextWindowSize(ImVec2(0, 0));
    ImGui::Begin(STR_C("Quit"), &m_quit_active, ImGuiWindowFlags_NoResize);
    ImGui::TextUnformatted(STR_C("Quit simulation?"));
    if (ImGui::Button(STR_C("Yes"))) Application.queue_quit(false);

    ImGui::SameLine();
    if (ImGui::Button(STR_C("No"))) m_quit_active = false;
    ImGui::End();
}
...
void ui_layer::render_panels() {
    ...
    for (auto &panel : m_ownedpanels) panel->render();

    if (m_imgui_demo) ImGui::ShowDemoWindow(&m_imgui_demo);
}
...
void ui_layer::render_menu_contents() {
    ...
    if (ImGui::BeginMenu(STR_C("Windows"))) {
        ...
        if (DebugModeFlag) {
            ...
            bool ret = ImGui::MenuItem(
                STR_C("Headlight config"), nullptr,
                GfxRenderer->Debug_Ui_State(std::nullopt));
...
void ui_layer::render_menu() {
    ...
    if (!((Global.ControlPicking && mousepos.y < 50.0f) ||
          m_imguiio->WantCaptureMouse) ||
        m_progress != 0.0f)
        return;
...
void ui_layer::render_background() {
    ...
    ImGui::Image(
        reinterpret_cast<void *>(tex.id), size, ImVec2(0, 1), ImVec2(1, 0));
    ImGui::End();
}
