...
double GetRenderTime() { return fRenderTime; }

double GetDeltaTime() {  // czas symulacji (stoi gdy pauza)
    if (override_delta != -1.0f) return override_delta;
    return DeltaTime;
}
...
void set_delta_override(double t) { override_delta = t; }
...
void UpdateTimers(bool pause) {
    ...
    if (!pause) {
        ...
        if (fSoundTimer > 0.1) fSoundTimer = 0.0;
