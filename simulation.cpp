...
namespace simulation {
    ...
    std::shared_ptr<deserializer_state> state_manager::deserialize_begin(
            std::string const &Scenariofile) {
        return m_serializer.deserialize_begin(Scenariofile);
    }

    bool state_manager::deserialize_continue(
            std::shared_ptr<deserializer_state> state) {
        return m_serializer.deserialize_continue(state);
    }

    // stores class data in specified file, in legacy (text) format
    void state_manager::export_as_text(std::string const &Scenariofile) const {
        return m_serializer.export_as_text(Scenariofile);
    }

    void state_manager::init_scripting_interface() {
        // create scenario data memory cells
        {
            auto *memorycell =
                new TMemCell({0, -1, "__simulation.weather", "memcell"});
            simulation::Memory.insert(memorycell);
            simulation::Region->insert(memorycell);
        }
        {
            auto *memorycell =
                new TMemCell({0, -1, "__simulation.time", "memcell"});
            simulation::Memory.insert(memorycell);
            simulation::Region->insert(memorycell);
        }
        {
            auto *memorycell =
                new TMemCell({0, -1, "__simulation.date", "memcell"});
            simulation::Memory.insert(memorycell);
            simulation::Region->insert(memorycell);
        }
    }
    ...
    void state_manager::update(double const Deltatime, int Iterationcount) {
        // aktualizacja animacji krokiem FPS: dt=krok czasu [s], dt*iter=czas od
        // ostatnich przeliczeń
        if (Deltatime == 0.0) {
            return;
        }

        auto const totaltime{Deltatime * Iterationcount};
        // NOTE: we perform animations first, as they can determine factors like
        // contact with powergrid
    ...
    void state_manager::update_clocks() {
        // Ra 2014-07: przeliczenie kąta czasu (do animacji zależnych od czasu)
        auto const &time = simulation::Time.data();
        Global.fTimeAngleDeg =
            time.wHour * 15.0 + time.wMinute * 0.25 +
            ((time.wSecond + 0.001 * time.wMilliseconds) / 240.0);
    ...
    void state_manager::update_scripting_interface() {
        ...
        if (simulation::is_ready) {
            ...
            m_scriptinginterface.weather =
                std::make_shared<TMemCell>(scene::node_data());
            m_scriptinginterface.date =
                std::make_shared<TMemCell>(scene::node_data());
            m_scriptinginterface.time =
                std::make_shared<TMemCell>(scene::node_data());
        }

        // update scripting interface
        weather->UpdateValues(
                Global.Weather, Global.Overcast, Global.fFogEnd,
                basic_event::flags::text | basic_event::flags::value1 |
                basic_event::flags::value2);

        time->UpdateValues(
                Global.Period, Time.data().wHour, Time.data().wMinute,
                basic_event::flags::text | basic_event::flags::value1 |
                basic_event::flags::value2);

        date->UpdateValues(
                Global.Season, Time.year_day(), 0,
                basic_event::flags::text | basic_event::flags::value1);
    ...
    void state_manager::process_commands() {
        ...
        while (Commands.pop(commanddata, (uint32_t)command_target::simulation)) {
            if (commanddata.command == user_command::consistreleaser) {
                TDynamicObject *found_vehicle =
                    simulation::Vehicles.find(commanddata.payload);
                TDynamicObject *vehicle = found_vehicle;

                while (vehicle) {
                    vehicle->MoverParameters->Hamulec->Releaser(
                            commanddata.action != GLFW_RELEASE ? 1 : 0);
            ...
            if (commanddata.action == GLFW_RELEASE) continue;
            ...
            if (commanddata.command == user_command::focuspauseset) {
                if (commanddata.param1 == 1.0)
                    Global.iPause &=
                        ~4;  // odpauzowanie, gdy jest na pierwszym planie
                else
            ...
            if (commanddata.command == user_command::entervehicle) {
                ...
                // NOTE: because malformed scenario can have vehicle name duplicates
                // we first try to locate vehicle in world, with name search as
                // fallback
                TDynamicObject *targetvehicle =
                    std::get<TDynamicObject *>(simulation::Region->find_vehicle(
                                commanddata.location, 50, false, false));
                if ((targetvehicle == nullptr) ||
                        (targetvehicle->name() != commanddata.payload)) {
                    targetvehicle = simulation::Vehicles.find(commanddata.payload);
                }

                if (!targetvehicle) continue;

                auto *senderlocaltrain{simulation::Trains.find_id(
                        static_cast<std::uint16_t>(commanddata.param2))};
                if (senderlocaltrain) {
                    auto *currentvehicle{senderlocaltrain->Dynamic()};
                    auto const samevehicle{currentvehicle == targetvehicle};

                    if (samevehicle) {
                        // we already control desired vehicle so don't
                    ...
                    auto const aidriveractive{
                        currentvehicle->Mechanik->AIControllFlag};
                    // TODO: support for primary mode request passed as
                    // commanddata.param1
                    ...
                    if (sameconsist && !aidriveractive) {
                        // since we're consist owner we can simply move to the
                        // destination vehicle
                        senderlocaltrain->MoveToVehicle(targetvehicle);
                        if (senderlocaltrain->Dynamic()->Mechanik != nullptr)
                            senderlocaltrain->Dynamic()->Mechanik->TakeControl(
                                    false, true);
                    }
                }
                ...
                if (train->Init(targetvehicle)) {
                    ...
                    if (targetvehicle->name() == Global.local_start_vehicle) {
                        ErrorLog(
                                "Failed to initialize player train, \"" +
            ...
            if (commanddata.command == user_command::queueevent) {
                ...
                if (ev) Events.AddToQuery(ev, vehicle);
            }
            ...
            if (commanddata.command == user_command::setlight) {
                ...
                if (model) model->LightSet(light, state);
            }
            ...
            if (commanddata.command == user_command::insertmodel) {
                ...
                TAnimModel *model = simulation::State.create_model(
                        data, name, commanddata.location);
                simulation::State.create_eventlauncher(
                        "node -1 0 launcher eventlauncher 0 0 0 0.8 none -10000.0 "
                        "obstacle_collision traintriggered end",
                        name + "_snd", commanddata.location);
            }

            if (commanddata.command == user_command::deletemodel) {
                simulation::State.delete_model(
                        simulation::Instances.find(commanddata.payload));
                simulation::State.delete_eventlauncher(
                        simulation::Events.FindEventlauncher(
                            commanddata.payload + "_snd"));
            }
            ...
            if (commanddata.command == user_command::resetconsist) {
                ...
                while (vehicle) {
                    ...
                    vehicle->MoverParameters->V =
                        0.000001;  // HACK: force vehicle position re-calculation
                    ...
                    // pantographs
                    for (auto idx = 0; idx < vehicle->iAnimType[ANIM_PANTS];
                            ++idx) {
                        auto &pantograph{*(vehicle->pants[idx].fParamPants)};
                        if (pantograph.PantWys >=
                                0.0)  // negative value means pantograph is broken
                            continue;
            ...
            if (commanddata.command == user_command::fillcompressor) {
                TDynamicObject *vehicle =
                    simulation::Vehicles.find(commanddata.payload);
                vehicle->MoverParameters->CompressedVolume =
                    8.0f * vehicle->MoverParameters->VeselVolume;
            }

            if (commanddata.command == user_command::dynamicmove) {
                TDynamicObject *vehicle =
                    simulation::Vehicles.find(commanddata.payload);
                if (vehicle) vehicle->move_set(commanddata.param1);
            }
            ...
            if (commanddata.command == user_command::pullalarmchain) {
                TDynamicObject *vehicle =
                    simulation::Vehicles.find(commanddata.payload);
                if (vehicle) vehicle->MoverParameters->AlarmChainSwitch(true);
            }
            ...
            if (commanddata.command == user_command::sendaicommand) {
                ...
                if (vehicle && vehicle->Mechanik)
                    vehicle->Mechanik->PutCommand(
                            command, commanddata.param1, commanddata.param2, &location);
            }
    ...
    TTrain* get_train_controlled_by_player() {
        return Train;
    }
