...
void Navigate(
    std::string const &ClassName, UINT Msg, WPARAM wParam, LPARAM lParam) {
#ifdef _WIN32
    // wysłanie komunikatu do sterującego
    HWND h = FindWindow(ClassName.c_str(), 0);  // można by to zapamiętać
    if (h == 0) h = FindWindow(0, ClassName.c_str());  // można by to zapamiętać
    SendMessage(h, Msg, wParam, lParam);
#endif
}

void OnCommandGet(
    multiplayer::DaneRozkaz *pRozkaz) {  // odebranie komunikatu z serwera
    if (pRozkaz->iSygn == EU07_MESSAGEHEADER) switch (pRozkaz->iComm) {
            case 0:  // odesłanie identyfikatora wersji
                CommLog(
                    Now() + " " + std::to_string(pRozkaz->iComm) + " version" +
                    " rcvd");
                WyslijString(Global.asVersion, 0);  // przedsatwienie się
                break;
            case 1:  // odesłanie identyfikatora wersji
                CommLog(
                    Now() + " " + std::to_string(pRozkaz->iComm) + " scenery" +
                    " rcvd");
            ...
            case 2: {
                ...
                CommLog(
                    ...
                    std::string(
                        pRozkaz->cString + 1, (unsigned)(pRozkaz->cString[0])) +
                    " rcvd");

                if (Global.iMultiplayer) {
                    auto *event = simulation::Events.FindEvent(std::string(
                        pRozkaz->cString + 1, (unsigned)(pRozkaz->cString[0])));
                    if (event != nullptr) {
                        if ((typeid(*event) == typeid(multi_event)) ||
                            ...
                            command_relay relay;
                            relay.post(
                                user_command::queueevent, 0.0, 0.0, GLFW_PRESS,
                                0, glm::vec3(0.0f), &event->name());
                        }
                    }
                }
                break;
            ...
            case 3:  // rozkaz dla AI
                if (Global.iMultiplayer) {
                    int i = int(
                        pRozkaz->cString[8]);  // długość pierwszego łańcucha (z
                                               // przodu dwa floaty)
                    CommLog(
                        Now() + " " + to_string(pRozkaz->iComm) + " " +
                        std::string(
                            pRozkaz->cString + 11 + i,
                            (unsigned)(pRozkaz->cString[10 + i])) +
                        " rcvd");
                    // nazwa pojazdu jest druga
                    auto *vehicle = simulation::Vehicles.find(
                        {pRozkaz->cString + 11 + i,
                         (unsigned)pRozkaz->cString[10 + i]});
                    if ((vehicle != nullptr) &&
                        ...
                        vehicle->Mechanik->PutCommand(
                            ...
                            pRozkaz->fPar[0], pRozkaz->fPar[1], nullptr,
                            stopExt);  // floaty są z przodu
                        WriteLog(
                            "AI command: " +
                            std::string(pRozkaz->cString + 9, i));
                    }
                }
                break;
            case 4:  // badanie zajętości toru
            {
                ...
                auto *track = simulation::Paths.find(std::string(
                    pRozkaz->cString + 1, (unsigned)(pRozkaz->cString[0])));
            ...
            {
                CommLog(
                    Now() + " " + to_string(pRozkaz->iComm) + " params " +
                    to_string(*pRozkaz->iPar) + " rcvd");
                if (*pRozkaz->iPar == 0)     // sprawdzenie czasu
                    if (*pRozkaz->iPar & 1)  // ustawienie czasu
                    {
                        auto t = pRozkaz->fPar[1];
                        simulation::Time.data().wDay =
                            std::floor(t);  // niby nie powinno być dnia, ale...
                        if (Global.fMoveLight >= 0)
                            Global.fMoveLight =
                                t;  // trzeba by deklinację Słońca przeliczyć
                        simulation::Time.data().wHour =
                            std::floor(24 * t) -
                            24.0 * simulation::Time.data().wDay;
                        simulation::Time.data().wMinute =
                            std::floor(60 * 24 * t) -
                            60.0 * (24.0 * simulation::Time.data().wDay +
                                    simulation::Time.data().wHour);
                        simulation::Time.data().wSecond =
                            std::floor(60 * 60 * 24 * t) -
                            60.0 *
                                (60.0 * (24.0 * simulation::Time.data().wDay +
                                         simulation::Time.data().wHour) +
                                 simulation::Time.data().wMinute);
                ...
                if (*pRozkaz->iPar & 2) {  // ustawienie flag zapauzowania
                    Global.iPause =
                        pRozkaz
                            ->fPar[2];  // zakładamy, że wysyłający wie, co robi
                }
            } break;
            case 6:  // pobranie parametrów ruchu pojazdu
                if (Global.iMultiplayer) {
                    ...
                    CommLog(
                        ...
                        std::string{
                            pRozkaz->cString + 1,
                            (unsigned)(pRozkaz->cString[0])} +
                        " rcvd");
                    if (pRozkaz->cString[0]) {
                        // jeśli długość nazwy jest niezerowa szukamy pierwszego
                        // pojazdu o takiej nazwie i odsyłamy parametry ramką #7
                        auto *vehicle =
                            (pRozkaz->cString[1] == '*'
                                 ? simulation::Train->Dynamic()
                                 : simulation::Vehicles.find(std::string{
                                       pRozkaz->cString + 1,
                                       (unsigned)pRozkaz->cString[0]}));
                        if (vehicle != nullptr) {
                            WyslijNamiary(
                                vehicle);  // wysłanie informacji o pojeździe
                        }
                    } else {
                        // dla pustego wysyłamy ramki 6 z nazwami pojazdów AI
                        // (jeśli potrzebne wszystkie, to rozpoznać np. "*")
            ...
            case 8:  // ponowne wysłanie informacji o zajętych odcinkach toru
                ...
                break;
            case 9:  // ponowne wysłanie informacji o zajętych odcinkach
                     // izolowanych
                CommLog(
                    Now() + " " + to_string(pRozkaz->iComm) +
                    " all busy isolated" + " rcvd");
            ...
            case 10:  // badanie zajętości jednego odcinka izolowanego
                CommLog(
                    ...
                    std::string(
                        pRozkaz->cString + 1, (unsigned)(pRozkaz->cString[0])) +
                    " rcvd");
                simulation::Paths.IsolatedBusy(std::string(
                    pRozkaz->cString + 1, (unsigned)(pRozkaz->cString[0])));
                break;
            ...
            case 12:  // skrocona ramka parametrow pojazdow AI (wszystkich!!)
                CommLog(
                    Now() + " " + to_string(pRozkaz->iComm) + " obsadzone" +
                    " rcvd");
                WyslijObsadzone();
                //    Ground.IsolatedBusy(AnsiString(pRozkaz->cString+1,(unsigned)(pRozkaz->cString[0])));
                break;
            case 13:  // ramka uszkodzenia i innych stanow pojazdu, np.
                      // wylaczenie CA, wlaczenie recznego itd.
                CommLog(
                    Now() + " " + to_string(pRozkaz->iComm) + " " +
                    std::string(
                        pRozkaz->cString + 1, (unsigned)(pRozkaz->cString[0])) +
                ...
                {  // szukamy pierwszego pojazdu o takiej nazwie i odsyłamy
                   // parametry ramką #13
                    auto *lookup =
                        (pRozkaz->cString[2] == '*'
                             ? simulation::Train->Dynamic()
                             :  // nazwa pojazdu użytkownika
                             simulation::Vehicles.find(std::string(
                                 pRozkaz->cString + 2,
                                 (unsigned)
                                     pRozkaz->cString[1])));  // nazwa pojazdu
                    if (lookup == nullptr) {
                        break;
...
void WyslijEvent(
    const std::string &e,
    const std::string &d) {  // Ra: jeszcze do wyczyszczenia
#ifdef _WIN32
    DaneRozkaz r;
    ...
    cData.lpData = &r;
    Navigate(
        "TEU07SRK", WM_COPYDATA,
        (WPARAM)glfwGetWin32Window(Application.window()), (LPARAM)&cData);
    CommLog(Now() + " " + std::to_string(r.iComm) + " " + e + " sent");
#endif
}

void WyslijUszkodzenia(
    const std::string &t,
    char fl) {  // wysłanie informacji w postaci pojedynczego tekstu
#ifdef _WIN32
    DaneRozkaz r;
    r.iSygn = EU07_MESSAGEHEADER;
...
void WyslijString(
    const std::string &t,
    int n) {  // wysłanie informacji w postaci pojedynczego tekstu
#ifdef _WIN32
    DaneRozkaz r;
    r.iSygn = EU07_MESSAGEHEADER;
...
void WyslijWolny(const std::string &t) {  // Ra: jeszcze do wyczyszczenia
    WyslijString(t, 4);                   // tor wolny
}

void WyslijNamiary(
    TDynamicObject const
        *Vehicle) {  // wysłanie informacji o pojeździe - (float), długość ramki
                     // będzie zwiększana w miarę potrzeby
#ifdef _WIN32
    DaneRozkaz r;
    r.iSygn = EU07_MESSAGEHEADER;
    ...
    r.fPar[6] =
        Vehicle->MoverParameters->nrot * M_PI *
    ...
    r.fPar[9] =
        Vehicle->MoverParameters->AccN;  // przyspieszenie Y //na razie nie
    r.fPar[10] = Vehicle->MoverParameters->AccVert;  // przyspieszenie Z
    r.fPar[11] =
        Vehicle->MoverParameters->DistCounter;  // przejechana odległość w km
    ...
    r.iPar[20] =
        Vehicle->MoverParameters->ScndCtrlActualPos;  // Pozycja bocznikowania
    r.iPar[21] =
        Vehicle->MoverParameters->ScndCtrlActualPos;  // Pozycja bocznikowania
...
#else
                 +
                 (false == Vehicle->MoverParameters->Doors.instances[side::left]
                               .is_closed) *
                     16 +
                 (false ==
                  Vehicle->MoverParameters->Doors.instances[side::right]
                      .is_closed) *
                     32
#endif
    ...
    // "+IntToStr(t->DynamicObject->iAnimType[ANIM_PANTS]));
    for (int p = 0; p < 4; p++) {
        //   WriteLog("Probuje pant "+IntToStr(p));
        if (p < Vehicle->iAnimType[ANIM_PANTS]) {
            r.fPar[23 + p] =
                Vehicle->pants[p].fParamPants->PantWys;  // stan pantografów 4
    ...
    for (int p = 0; p < 3; p++)
        r.fPar[27 + p] = Vehicle->MoverParameters->ShowCurrent(
            p + 1);  // amperomierze kolejnych grup
    ...
    CommLog(
        Now() + " " + std::to_string(r.iComm) + " " + Vehicle->asName +
        " sent");
#endif
}

void WyslijObsadzone() {  // wysłanie informacji o pojeździe
#ifdef _WIN32
    DaneRozkaz2 r;
    r.iSygn = EU07_MESSAGEHEADER;
    r.iComm = 12;  // kod 12
    ...
    for (auto *vehicle : vehiclelist) {
        if (vehicle->Mechanik) {
            ...
            strcpy(
                r.cString + 64 * i + 32,
                vehicle->GetTrack()->IsolatedName().c_str());
            strcpy(
                r.cString + 64 * i + 48,
                vehicle->Mechanik->TrainName().c_str());
            i++;
            if (i > 30) break;
        }
    }
...
void WyslijParam(
    int nr,
    int fl) {  // wysłanie parametrów symulacji w ramce (nr) z flagami (fl)
#ifdef _WIN32
    DaneRozkaz r;
    r.iSygn = EU07_MESSAGEHEADER;
    r.iComm = nr;    // zwykle 5
    ...
    switch (nr) {    // można tym przesyłać różne zestawy parametrów
        case 5:      // czas i pauza
            r.fPar[1] =
                Global.fTimeAngleDeg / 360.0;  // aktualny czas (1.0=doba)
