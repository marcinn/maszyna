...
void basic_precipitation::update() {
    auto const timedelta{static_cast<float>(
        (DebugModeFlag ? Timer::GetDeltaTime() : Timer::GetDeltaTime()))};
    ...
    if (m_freeflymode != FreeFlyModeFlag) {
        ...
        if (true == m_freeflymode) {
            ...
            // don't carry previous precipitation vector to a new unrelated
            // location
    ...
    m_cameramove = m_cameramove * std::max(0.0, 1.0 - 5.0 * timedelta) +
                   cameramove * (30.0 * timedelta);
    if (std::abs(m_cameramove.x) < 0.001) {
        m_cameramove.x = 0.0;
    }
    if (std::abs(m_cameramove.y) < 0.001) {
        m_cameramove.y = 0.0;
    }
    if (std::abs(m_cameramove.z) < 0.001) {
        m_cameramove.z = 0.0;
    }
}

float basic_precipitation::get_textureoffset() const { return m_textureoffset; }
