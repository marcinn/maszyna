...
class gamepad_input {
    ...
    */
    enum class input_type {
        threestate,  // two commands, mapped to positive and negative axis value
                     // respectively; press and release events on state change
        impulse,     // one command; press event when set, release when cleared
        value,       // one command; press event, axis value passed as specified
                     // param (stored as second 'user_command')
    ...
    struct input_axis {
        ...
        std::unordered_map<
            int, std::tuple<input_type, user_command, user_command> >
            bindings;  // associated commands for respective modes
    };
    ...
    void bind(
        std::vector<std::reference_wrapper<user_command> > &Targets,
        cParser &Input,
    ...
    //    double m_modeaccumulator{ 0.0 }; // used to throttle command input
    //    rate for vehicle controls
    std::unordered_map<user_command, std::tuple<double, double> >
        m_lastcommandparams;  // cached parameters for last issued command of
                              // given type
};
