...
void texture_window_resize(GLFWwindow *win, int w, int h) {
    python_screen_viewer *texwindow =
        (python_screen_viewer *)glfwGetWindowUserPointer(win);
...
void texture_window_mouse_button(
...
python_screen_viewer::python_screen_viewer(
    ...
    std::shared_ptr<std::vector<glm::vec2>> touchlist,
    ...
    for (const auto &viewport : Global.python_viewports) {
        if (viewport.surface == surfacename) {
            ...
            if (!monitor && viewport.monitor != "window") continue;

            conf->window = Application.window(
                -1, true, conf->window_size.x, conf->window_size.y, monitor,
                false, Global.python_sharectx);
            ...
            glfwSetFramebufferSizeCallback(
                conf->window, texture_window_fb_resize);
            glfwSetMouseButtonCallback(
                conf->window, texture_window_mouse_button);
    ...
    if (!m_windows.empty())
        m_renderthread = std::make_unique<std::thread>(
            &python_screen_viewer::threadfunc, this);
}

python_screen_viewer::~python_screen_viewer() {
    m_exit = true;
    if (m_renderthread) m_renderthread->join();
}
...
void python_screen_viewer::threadfunc() {
    for (auto &window : m_windows) {
        ...
        window->ubo =
            std::make_unique<gl::ubo>(sizeof(gl::scene_ubs), 0, GL_STREAM_DRAW);

        gl::shader vert("texturewindow.vert");
        gl::shader frag("texturewindow.frag");
        window->shader = std::make_unique<gl::program>(
            std::vector<std::reference_wrapper<const gl::shader>>(
                {vert, frag}));
    }
    ...
    while (!m_exit) {
        ...
        for (auto &window : m_windows) {
            ...
            if (!Global.python_sharectx) {
                ...
                if (window->timestamp == m_rt->timestamp) continue;

                window->timestamp = m_rt->timestamp;

                if (!m_rt->image) continue;
            ...
            m_ubs.projection = glm::mat4(glm::mat3(glm::translate(
                glm::scale(glm::mat3(), 1.0f / window->scale),
                window->offset)));
            window->ubo->update(m_ubs);

            if (image) {
                glTexImage2D(
                    GL_TEXTURE_2D, 0, format, width, height, 0, components,
...
void python_screen_viewer::notify_window_fb_size(
...
void python_screen_viewer::notify_window_size(
    GLFWwindow *window, int w, int h) {
    for (auto &conf : m_windows) {
        if (conf->window == window) {
...
void python_screen_viewer::notify_cursor_pos(
...
void python_screen_viewer::notify_click(
    ...
    for (auto &conf : m_windows) {
        if (conf->window == window) {
            auto pos =
                glm::vec2(conf->cursor_pos) / glm::vec2(conf->window_size);
            pos.y = 1.0f - pos.y;
