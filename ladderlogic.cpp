...
auto basic_element::output() const -> int {
    switch ((basic_element::type_e)data.index()) {
        case basic_element::type_e::variable: {
            return std::get<variable>(data).value;
        }
        case basic_element::type_e::timer: {
            return (
                std::get<timer>(data).time_elapsed >=
                        std::get<timer>(data).time_preset
                    ? std::get<timer>(data).value
                    : 0);
        }
        case basic_element::type_e::counter: {
            return (
                std::get<counter>(data).count_value >=
                        std::get<counter>(data).count_limit
                    ? 1
                    : 0);
        }
    }
...
auto basic_controller::input(element_handle const Element) -> int & {
    return m_elements[Element - 1].input();
}

auto basic_controller::output(element_handle const Element) const -> int {
    return m_elements[Element - 1].output();
}

auto basic_controller::load(std::string const &Filename) -> bool {
    m_program.clear();
...
auto basic_controller::update(double const Timestep) -> int {
    if (false == m_timerhandles.empty()) {
        ...
        if (updatecount > 0) {
            auto const updateamount =
                static_cast<short>(updatecount * 1000 * m_updaterate);
            for (auto const timerhandle : m_timerhandles) {
                ...
                if (timer.input() > 0) {
                    timerdata.time_elapsed = std::min<short>(
...
std::map<std::string, basic_controller::opcode_e> const
    basic_controller::m_operationcodemap = {
        {"ld", opcode_e::op_ld},   {"ldi", opcode_e::op_ldi},
        {"and", opcode_e::op_and}, {"ani", opcode_e::op_ani},
        {"anb", opcode_e::op_anb}, {"or", opcode_e::op_or},
        {"ori", opcode_e::op_ori}, {"orb", opcode_e::op_orb},
        {"out", opcode_e::op_out}, {"set", opcode_e::op_set},
        {"rst", opcode_e::op_rst}, {"end", opcode_e::op_nop}};

auto basic_controller::deserialize_operation(cParser &Input) -> bool {
    auto operationdata{Input.getToken<std::string>(true, "\n\r")};
    if (true == operationdata.empty()) {
        return false;
    }
    ...
    // HACK: operation potentially contains 1-2 parameters so we try to grab the
    // whole set
    operationparser.getTokens(3, "\t ");
    std::string operationname, operationelement, operationparameter;
    operationparser >> operationname >> operationelement >> operationparameter;

    auto const lookup{m_operationcodemap.find(operationname)};
    operation.code =
        (lookup != m_operationcodemap.end() ? lookup->second
                                            : opcode_e::op_nop);
    if (lookup == m_operationcodemap.end()) {
        log_error(
            "contains unknown command \"" + operationname + "\"",
            Input.Line() - 1);
    }

    if (operation.code == opcode_e::op_nop) {
        return true;
    }

    if (false == operationelement.empty()) {
        operation.element = find_or_insert(
            operationelement, guess_element_type_from_name(operationelement));
    }
...
auto basic_controller::insert(std::string const Name, basic_element Element)
    -> element_handle {
    ...
    if ((basic_element::type_e)Element.data.index() ==
        basic_element::type_e::timer) {
...
auto basic_controller::run() -> int {
    m_accumulator.clear();
    m_popstack = false;
    auto programline{1};

    for (auto const &operation : m_program) {
        // TBD: replace switch with function table for better
        ...
        switch (operation.code) {
            ...
            case opcode_e::op_and: {
                if (m_accumulator.empty()) {
                    log_error(
                        "attempted AND with empty accumulator", programline);
                    break;
                }
            ...
            case opcode_e::op_ani: {
                if (m_accumulator.empty()) {
                    log_error(
                        "attempted ANI with empty accumulator", programline);
                    break;
                }
            ...
            case opcode_e::op_or: {
                if (m_accumulator.empty()) {
                    log_error(
                        "attempted OR with empty accumulator", programline);
                    break;
                }
                m_accumulator.back() |= output(operation.element);
                break;
            ...
            case opcode_e::op_ori: {
                if (m_accumulator.empty()) {
                    log_error(
                        "attempted ORI with empty accumulator", programline);
                    break;
                }
                m_accumulator.back() |= inverse(output(operation.element));
                break;
            ...
            case opcode_e::op_out: {
                if (m_accumulator.empty()) {
                    log_error(
                        "attempted OUT with empty accumulator", programline);
                    break;
                }
                ...
                switch ((basic_element::type_e)target.data.index()) {
                    case basic_element::type_e::timer: {
                        std::get<basic_element::timer>(target.data)
                            .time_preset = operation.parameter1;
                        break;
                    }
                    case basic_element::type_e::counter: {
                        std::get<basic_element::counter>(target.data)
                            .count_limit = operation.parameter1;
                        // increase counter value on input activation
                        if ((initialstate == 0) && (target.input() != 0)) {
                            /*
                                                        // TBD: use
                               overflow-prone version instead of safe one?
                                                        target.data.counter.count_value
                               += 1;
                            */
                            std::get<basic_element::counter>(target.data)
                                .count_value = std::min<short>(
                                std::get<basic_element::counter>(target.data)
                                    .count_limit,
                                std::get<basic_element::counter>(target.data)
                                        .count_value +
                ...
                // accumulator was published at least once, next ld(i) operation
                // will start a new rung
                m_popstack = true;
                break;
            }
            ...
            case opcode_e::op_set: {
                if (m_accumulator.empty()) {
                    log_error(
                        "attempted SET with empty accumulator", programline);
                    break;
                }
                ...
                switch ((basic_element::type_e)target.data.index()) {
                    case basic_element::type_e::counter: {
                        ...
                        /*
                           ...
                           target.input() != 0 ) ) {
                                                    target.data.counter.count_value
                           = std::min<short>( target.data.counter.count_limit,
                                                            target.data.counter.count_value
                           + 1 );
                                                }
                        */
            ...
            case opcode_e::op_rst: {
                if (m_accumulator.empty()) {
                    log_error(
                        "attempted RST with empty accumulator", programline);
                    break;
                }
                if (m_accumulator.back() == 0) {
                    break;
...
void basic_controller::log_error(
    std::string const &Error, int const Line) const {
    ErrorLog(
        "Bad plc program: \"" + m_programfilename + "\" " + Error +
        (Line > 0 ? " (line " + to_string(Line) + ")" : ""));
}

auto basic_controller::guess_element_type_from_name(
    std::string const &Name) const -> basic_element::type_e {
    auto const name{split_string_and_number(Name)};

    if ((name.first == "t") || (name.first == "ton") ||
        (name.first.find("timer.") == 0)) {
