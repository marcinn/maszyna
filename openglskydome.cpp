...
opengl_skydome::~opengl_skydome() {
    ...
    if (m_coloursbuffer != -1) {
        ::glDeleteBuffers(1, &m_coloursbuffer);
    }
}
...
void opengl_skydome::update() {
    ...
    if (m_coloursbuffer == (GLuint)-1) {
        ...
        if ((m_coloursbuffer > 0) && (m_coloursbuffer != (GLuint)-1)) {
            ...
            ::glBufferData(
                GL_ARRAY_BUFFER, colors.size() * sizeof(glm::vec3),
    ...
    if (true == skydome.is_dirty()) {
        if ((m_coloursbuffer > 0) && (m_coloursbuffer != (GLuint)-1)) {
            ...
            /*
            float twilightfactor = clamp(
            -simulation::Environment.sun().getAngle(), 0.0f, 18.0f ) / 18.0f;
            auto gamma = interpolate( glm::vec3( 0.45f ), glm::vec3( 1.0f ),
            twilightfactor ); for( auto & color : colors ) { color = glm::pow(
            color, gamma );
            }
            */
            ::glBufferSubData(
                GL_ARRAY_BUFFER, 0, colors.size() * sizeof(glm::vec3),
                colors.data());
...
void opengl_skydome::render() {
    if ((m_indexbuffer == (GLuint)-1) || (m_vertexbuffer == (GLuint)-1) ||
    ...
    ::glDrawElements(
        GL_TRIANGLES, static_cast<GLsizei>(m_indexcount), GL_UNSIGNED_SHORT,
