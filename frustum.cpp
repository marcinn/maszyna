...
bool cFrustum::point_inside(float const X, float const Y, float const Z) const {
    // cycle through the sides of the frustum, checking if the point is behind
    // them
    for (int idx = 0; idx < 6; ++idx) {
        if (m_frustum[idx][plane_A] * X + m_frustum[idx][plane_B] * Y +
                m_frustum[idx][plane_C] * Z + m_frustum[idx][plane_D] <=
            0)
            return false;
    }
...
float cFrustum::sphere_inside(
    ...
    for (int idx = 0; idx < 6; ++idx) {
        distance = m_frustum[idx][plane_A] * X + m_frustum[idx][plane_B] * Y +
                   m_frustum[idx][plane_C] * Z + m_frustum[idx][plane_D];
...
bool cFrustum::cube_inside(
    ...
    for (int idx = 0; idx < 6; ++idx) {
        if (m_frustum[idx][plane_A] * (X - Size) +
                ...
                m_frustum[idx][plane_C] * (Z - Size) + m_frustum[idx][plane_D] >
            0)
            continue;
        if (m_frustum[idx][plane_A] * (X + Size) +
                m_frustum[idx][plane_B] * (Y - Size) +
