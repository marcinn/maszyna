...
class basic_table {
   public:
    ...
    bool insert(Type_ *Item, std::string itemname) {
        ...
        // item with this name already exists; update mapping to point to the
        // new one, for backward compatibility
        mapping.first->second = itemhandle;
        return false;
    }
    bool insert(Type_ *Item) { return insert(Item, Item->name()); }
    void purge(std::string const &Name) {
        auto lookup = m_itemmap.find(Name);
        if (lookup == m_itemmap.end()) return;
    ...
    Type_ *find(std::string const &Name) const {
        auto lookup = m_itemmap.find(Name);
   ...
   public:
    // data access
    type_sequence &sequence() { return m_items; }
    type_sequence const &sequence() const { return m_items; }
};
