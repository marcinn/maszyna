...
class null_renderer : public gfx_renderer {
   public:
    ...
    inline float Framerate() override { return 10.0f; }
    ...
    gfx::geometrybank_handle Create_Bank() override {
        return m_geometry.register_bank(std::make_unique<null_geometrybank>());
    }
    ...
    opengl_material const &Material(
        ...
        return *m_materials.at(Material - 1);
    }
    // shader methods
    ...
    // debug methods
    std::string const &info_times() const override { return empty_str; }
    std::string const &info_stats() const override { return empty_str; }
