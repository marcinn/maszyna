...
class cParser  //: public std::stringstream
{
   public:
    ...
    // constructors:
    cParser(
        std::string const &Stream, buffertype const Type = buffer_TEXT,
        std::string Path = "", bool const Loadtraction = true,
    ...
    Output_ getToken(
        bool const ToLower = true, char const *Break = "\n\r\t ;") {
    ...
    inline bool eof() { return mStream->eof(); };
    inline bool ok() { return (!mStream->fail()); };
    cParser &autoclear(bool const Autoclear);
    inline bool autoclear() const { return m_autoclear; }
    bool getTokens(
        unsigned int Count = 1, bool ToLower = true,
        char const *Break = "\n\r\t ;");
    ...
    //
    static std::size_t countTokens(
        std::string const &Stream, std::string Path = "");
    // add custom definition of text which should be ignored when retrieving
    // tokens
    void addCommentStyle(
    ...
    // returns number of currently processed line in main file, -1 if inside
    // include
    int LineMain() const;
   ...
   private:
    ...
    std::size_t count();
    // members:
    bool m_autoclear{true};   // unretrieved tokens are discarded when another
                              // read command is issued (legacy behaviour)
    bool LoadTraction{true};  // load traction?
    std::shared_ptr<std::istream>
        mStream;        // relevant kind of buffer is attached on creation.
    ...
    bool mFirstToken{true};   // processing first token in the current file;
                              // helper used when checking for utf bom
    typedef std::map<std::string, std::string> commentmap;
    commentmap mComments{
        commentmap::value_type("/*", "*/"), commentmap::value_type("//", "\n")};
    std::shared_ptr<cParser>
        mIncludeParser;  // child class to handle include directives.
    std::vector<std::string> parameters;  // parameter list for included file.
    std::deque<std::string> tokens;
};

template <>
glm::vec3 cParser::getToken(bool const ToLower, const char *Break);

template <typename Type_>
cParser &cParser::operator>>(Type_ &Right) {
