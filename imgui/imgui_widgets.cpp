...
static bool InputTextFilterCharacter(
    unsigned int* p_char, ImGuiInputTextFlags flags,
...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args) {
    ...
    const char* value_text_end =
        ...
        ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), fmt, args);
    RenderTextClipped(
        value_bb.Min, value_bb.Max, value_text_begin, value_text_end, NULL,
        ImVec2(0.0f, 0.5f));
...
void ImGui::BulletTextV(const char* fmt, va_list args) {
    ...
    RenderBullet(
        ...
        bb.Min +
            ImVec2(
                style.FramePadding.x + g.FontSize * 0.5f, line_height * 0.5f),
        text_col);
    RenderText(
        bb.Min +
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos) {
    ...
    if (hovered || held)
        window->DrawList->AddCircleFilled(
            center /*+ ImVec2(0.0f, -0.5f)*/, g.FontSize * 0.5f + 1.0f, bg_col,
            12);
    RenderArrow(
...
bool ImGui::ScrollbarEx(
    const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v,
    ...
    IM_ASSERT(
        ...
        0.0f);  // Adding this assert to check if the ImMax(XXX,1.0f) is still
                // needed. PLEASE CONTACT ME if this triggers.
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f) {
        ...
        // after the calculation of ContentSize and before setting up our
        // starting position
    ...
    window->DrawList->AddRectFilled(
        bb_frame.Min, bb_frame.Max, GetColorU32(ImGuiCol_ScrollbarBg),
...
void ImGui::Image(
    ...
    if (border_col.w > 0.0f) {
        ...
        window->DrawList->AddImage(
            user_texture_id, bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), uv0,
...
bool ImGui::ImageButton(
    ...
    window->DrawList->AddImage(
        user_texture_id, image_bb.Min, image_bb.Max, uv0, uv1,
...
bool ImGui::Checkbox(const char* label, bool* v) {
    ...
    if (window->DC.ItemFlags & ImGuiItemFlags_MixedValue) {
        ...
        ImVec2 pad(
            ...
            ImMax(1.0f, (float)(int)(square_sz / 3.6f)));
        window->DrawList->AddRectFilled(
            check_bb.Min + pad, check_bb.Max - pad, check_col,
            style.FrameRounding);
    } else if (*v) {
        const float pad = ImMax(1.0f, (float)(int)(square_sz / 6.0f));
        RenderCheckMark(
            check_bb.Min + ImVec2(pad, pad), check_col, square_sz - pad * 2.0f);
    }
...
bool ImGui::RadioButton(const char* label, bool active) {
    ...
    if (style.FrameBorderSize > 0.0f) {
        window->DrawList->AddCircle(
            center + ImVec2(1, 1), radius, GetColorU32(ImGuiCol_BorderShadow),
            16, style.FrameBorderSize);
        window->DrawList->AddCircle(
            center, radius, GetColorU32(ImGuiCol_Border), 16,
...
void ImGui::ProgressBar(
    ...
    RenderFrame(
        bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg), true,
        style.FrameRounding);
    bb.Expand(ImVec2(-style.FrameBorderSize, -style.FrameBorderSize));
    const ImVec2 fill_br =
        ImVec2(ImLerp(bb.Min.x, bb.Max.x, fraction), bb.Max.y);
    RenderRectFilledRangeH(
        window->DrawList, bb, GetColorU32(ImGuiCol_PlotHistogram), 0.0f,
    ...
    if (!overlay) {
        ImFormatString(
            overlay_buf, IM_ARRAYSIZE(overlay_buf), "%.0f%%",
            fraction * 100 + 0.01f);
        overlay = overlay_buf;
    }
...
void ImGui::ShrinkWidths(
    ...
    ImQsort(
        items, (size_t)count, sizeof(ImGuiShrinkWidthItem),
...
bool ImGui::BeginCombo(
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(
            frame_bb.Min + style.FramePadding, ImVec2(value_x2, frame_bb.Max.y),
    ...
    if (has_window_size_constraint) {
        ...
        SetNextWindowSizeConstraints(
            ImVec2(w, 0.0f), ImVec2(
                                 FLT_MAX, CalcMaxPopupHeightFromItemCount(
...
bool ImGui::Combo(
    ...
    const bool value_changed = Combo(
        label, current_item, Items_ArrayGetter, (void*)items, items_count,
        height_in_items);
    return value_changed;
}
...
int ImGui::DataTypeFormatString(
    char* buf, int buf_size, ImGuiDataType data_type, const void* data_ptr,
    const char* format) {
...
void ImGui::DataTypeApplyOp(
    ImGuiDataType data_type, int op, void* output, void* arg1,
    const void* arg2) {
    IM_ASSERT(op == '+' || op == '-');
    switch (data_type) {
...
bool ImGui::DataTypeApplyOpFromText(
    const char* buf, const char* initial_value_buf, ImGuiDataType data_type,
...
bool ImGui::DragBehaviorT(
    ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min,
    ...
    const bool is_power =
        (power != 1.0f && is_decimal && is_clamped &&
         (v_max - v_min < FLT_MAX));
...
bool ImGui::DragBehavior(
    ImGuiID id, ImGuiDataType data_type, void* v, float v_speed,
...
bool ImGui::DragScalar(
    const char* label, ImGuiDataType data_type, void* v, float v_speed,
    const void* v_min, const void* v_max, const char* format, float power) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    if (power != 1.0f)
        IM_ASSERT(
            v_min != NULL && v_max != NULL);  // When using a power curve the
...
bool ImGui::DragFloat2(
    ...
    const char* format, float power) {
    return DragScalarN(
        label, ImGuiDataType_Float, v, 2, v_speed, &v_min, &v_max, format,
        power);
}
...
bool ImGui::DragFloat3(
    ...
    const char* format, float power) {
    return DragScalarN(
        label, ImGuiDataType_Float, v, 3, v_speed, &v_min, &v_max, format,
        power);
}
...
bool ImGui::DragFloat4(
    ...
    const char* format, float power) {
    return DragScalarN(
        label, ImGuiDataType_Float, v, 4, v_speed, &v_min, &v_max, format,
        power);
}
...
bool ImGui::DragInt2(
    const char* label, int v[2], float v_speed, int v_min, int v_max,
    const char* format) {
    return DragScalarN(
...
bool ImGui::DragInt3(
    const char* label, int v[3], float v_speed, int v_min, int v_max,
    const char* format) {
    return DragScalarN(
...
bool ImGui::DragInt4(
    const char* label, int v[4], float v_speed, int v_min, int v_max,
    const char* format) {
    return DragScalarN(
...
bool ImGui::DragIntRange2(
    const char* label, int* v_current_min, int* v_current_max, float v_speed,
...
float ImGui::SliderCalcRatioFromValueT(
    ImGuiDataType data_type, TYPE v, TYPE v_min, TYPE v_max, float power,
...
bool ImGui::SliderBehaviorT(
    const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v,
...
bool ImGui::SliderBehavior(
    const ImRect& bb, ImGuiID id, ImGuiDataType data_type, void* v,
    const void* v_min, const void* v_max, const char* format, float power,
    ImGuiSliderFlags flags, ImRect* out_grab_bb) {
    switch (data_type) {
        case ImGuiDataType_S8: {
        ...
        case ImGuiDataType_S32:
            IM_ASSERT(
                *(const ImS32*)v_min >= IM_S32_MIN / 2 &&
        ...
        case ImGuiDataType_S64:
            IM_ASSERT(
                *(const ImS64*)v_min >= IM_S64_MIN / 2 &&
...
bool ImGui::SliderScalar(
    const char* label, ImGuiDataType data_type, void* v, const void* v_min,
    ...
    const bool value_changed = SliderBehavior(
        frame_bb, id, data_type, v, v_min, v_max, format, power,
...
bool ImGui::VSliderScalar(
    const char* label, const ImVec2& size, ImGuiDataType data_type, void* v,
    const void* v_min, const void* v_max, const char* format, float power) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
...
bool ImGui::TempInputTextScalar(
    ...
    bool value_changed = InputTextEx(
        label, NULL, data_buf, IM_ARRAYSIZE(data_buf), bb.GetSize(), flags);
    if (init) {
    ...
    if (value_changed) {
        value_changed = DataTypeApplyOpFromText(
            data_buf, g.InputTextState.InitialTextA.Data, data_type, data_ptr,
            NULL);
        if (value_changed) MarkItemEdited(id);
    }
    return value_changed;
...
bool ImGui::InputScalar(
    const char* label, ImGuiDataType data_type, void* data_ptr,
    ...
    flags |=
        ImGuiInputTextFlags_NoMarkEdited;  // We call MarkItemEdited() ourselve
    ...
    if (step != NULL) {
        ...
        if (InputText(
            ...
            value_changed = DataTypeApplyOpFromText(
                buf, g.InputTextState.InitialTextA.Data, data_type, data_ptr,
                format);
...
bool ImGui::InputFloat3(
    const char* label, float v[3], const char* format,
    ImGuiInputTextFlags flags) {
    return InputScalarN(
...
bool ImGui::InputFloat4(
    const char* label, float v[4], const char* format,
    ImGuiInputTextFlags flags) {
    return InputScalarN(
...
bool ImGui::InputFloat2(
    const char* label, float v[2], int decimal_precision,
    ImGuiInputTextFlags flags) {
    char format[16] = "%f";
    if (decimal_precision >= 0)
        ImFormatString(
...
bool ImGui::InputFloat3(
    const char* label, float v[3], int decimal_precision,
    ImGuiInputTextFlags flags) {
    char format[16] = "%f";
    if (decimal_precision >= 0)
        ImFormatString(
...
bool ImGui::InputFloat4(
    const char* label, float v[4], int decimal_precision,
    ImGuiInputTextFlags flags) {
    char format[16] = "%f";
    if (decimal_precision >= 0)
        ImFormatString(
...
bool ImGui::InputDouble(
    ...
    flags |= ImGuiInputTextFlags_CharsScientific;
    return InputScalar(
        label, ImGuiDataType_Double, (void*)v,
...
bool ImGui::InputText(
    ...
    return InputTextEx(
        label, NULL, buf, (int)buf_size, ImVec2(0, 0), flags, callback,
        user_data);
}
...
bool ImGui::InputTextMultiline(
    ...
    void* user_data) {
    return InputTextEx(
        label, NULL, buf, (int)buf_size, size,
...
bool ImGui::InputTextWithHint(
    ...
    IM_ASSERT(
        !(flags & ImGuiInputTextFlags_Multiline));  // call InputTextMultiline()
    return InputTextEx(
        label, hint, buf, (int)buf_size, ImVec2(0, 0), flags, callback,
        user_data);
}
...
static void STB_TEXTEDIT_LAYOUTROW(
    ...
    const ImVec2 size = InputTextCalcTextSizeW(
        text + line_start_idx, text + obj->CurLenW, &text_remaining, NULL,
        true);
    r->x0 = 0.0f;
    r->x1 = size.x;
...
bool ImGui::InputTextEx(
    ...
    if (is_resizable)
        IM_ASSERT(
            callback != NULL);  // Must provide a callback if you set the
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
        ...
        if (IsKeyPressedMap(ImGuiKey_LeftArrow)) {
            ...
            IsKeyPressedMap(ImGuiKey_Enter) ||
            ...
            if (const char* clipboard = GetClipboardText()) {
                ...
                {
                    stb_textedit_paste(
                        state, &state->Stb, clipboard_filtered,
    ...
    if (g.ActiveId == id) {
        ...
        if (apply_edit_back_to_user_buffer) {
            ...
            if (!is_readonly) {
                ...
                ImTextStrToUtf8(
                    state->TextA.Data, state->TextA.Size, state->TextW.Data,
                    NULL);
            }
    ...
    if (render_cursor || render_selection) {
        ...
        if (is_multiline ||
            ...
            ImU32 col = GetColorU32(
                is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            draw_window->DrawList->AddText(
                g.Font, g.FontSize, draw_pos, col, buf_display, buf_display_end,
...
bool ImGui::ColorEdit4(
    ...
    if ((flags & ImGuiColorEditFlags_InputHSV) &&
        ...
        if (f[1] == 0 &&
            memcmp(g.ColorEditLastColor, col, sizeof(float) * 3) == 0)
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB |
        ...
        const float w_item_one = ImMax(
            1.0f,
            (float)(int)((w_inputs - (style.ItemInnerSpacing.x) * (components - 1)) / (float)components));
        const float w_item_last = ImMax(
            1.0f,
            (float)(int)(w_inputs - (w_item_one + style.ItemInnerSpacing.x) * (components - 1)));
        ...
        for (int n = 0; n < components; n++) {
            ...
            const bool disable_hue_edit =
                (n == 0 && (flags & ImGuiColorEditFlags_DisplayHSV) &&
                 i[1] == 0);
            if (flags & ImGuiColorEditFlags_Float) {
                value_changed |= DragFloat(
                    ids[n], &f[n], 1.0f / 255.0f,
                ...
                value_changed |= DragInt(
                    ids[n], &i[n], 1.0f, disable_hue_edit ? INT_MAX : 0,
        ...
        if (InputText(
            ...
            if (alpha)
                ...
                sscanf(
                    p, "%02X%02X%02X", (unsigned int*)&i[0],
                    (unsigned int*)&i[1], (unsigned int*)&i[2]);
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
            OpenPopupOnItemClick("context");
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel)) {
        window->DC.CursorPos = ImVec2(
            pos.x + w_full + style.ItemInnerSpacing.x,
...
static void RenderArrowsForVerticalBar(
    ImDrawList* draw_list, ImVec2 pos, ImVec2 half_sz, float bar_w,
    float alpha) {
...
bool ImGui::ColorPicker4(
    ...
    if (value_changed_fix_hue_wrap && (flags & ImGuiColorEditFlags_InputRGB)) {
        ...
        if (new_H <= 0 && H > 0) {
            if (new_V <= 0 && V != new_V)
                ColorConvertHSVtoRGB(
                    H, S, new_V <= 0 ? V * 0.5f : new_V, col[0], col[1],
                    col[2]);
            else if (new_S <= 0)
                ColorConvertHSVtoRGB(
                    H, new_S <= 0 ? S * 0.5f : new_S, new_V, col[0], col[1],
                    col[2]);
        }
    }
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel) {
        ...
        draw_list->AddCircle(
            hue_cursor_pos, hue_cursor_rad + 1, col_midgrey,
            hue_cursor_segments);
        draw_list->AddCircle(
    ...
    if (alpha_bar) {
        ...
        RenderColorRectWithAlphaCheckerboard(
            bar1_bb.Min, bar1_bb.Max, 0, bar1_bb.GetWidth() / 2.0f,
            ImVec2(0.0f, 0.0f));
        draw_list->AddRectFilledMultiColor(
...
void ImGui::ColorTooltip(
    ...
    if ((flags & ImGuiColorEditFlags_InputRGB) ||
        ...
        if (flags & ImGuiColorEditFlags_NoAlpha)
            ...
            Text(
                "H: %.3f, S: %.3f, V: %.3f, A: %.3f", col[0], col[1], col[2],
                col[3]);
    }
    EndTooltip();
}
...
void ImGui::ColorPickerOptionsPopup(
    ...
    if (allow_opt_alpha_bar) {
        ...
        CheckboxFlags(
            "Alpha Bar", (unsigned int*)&g.ColorEditOptions,
    ...
    if (!is_leaf) {
        if (pressed) {
            ...
            if (flags & ImGuiTreeNodeFlags_OpenOnArrow)
                toggled |=
                    IsMouseHoveringRect(
                        interact_bb.Min, ImVec2(
                                             interact_bb.Min.x + text_offset_x,
                                             interact_bb.Max.y)) &&
...
void ImGui::TreePop() {
    ...
    IM_ASSERT(
        window->IDStack.Size >
        1);  // There should always be 1 element in the IDStack (pushed during
             // window creation). If this triggers you called TreePop/PopID too
             // much.
    PopID();
}
...
bool ImGui::CollapsingHeader(const char* label, ImGuiTreeNodeFlags flags) {
    ...
    return TreeNodeBehavior(
        window->GetID(label), flags | ImGuiTreeNodeFlags_CollapsingHeader,
        label);
}
...
bool ImGui::CollapsingHeader(
    ...
    if (p_open) {
        ...
        float button_x = ImMax(
            window->DC.LastItemRect.Min.x, window->DC.LastItemRect.Max.x -
...
bool ImGui::Selectable(
    ...
    RenderTextClipped(
        bb_inner.Min, bb_inner.Max, label, NULL, &label_size,
...
void ImGui::PlotEx(
    ...
    if (values_count >= values_count_min) {
        ...
        ImVec2 tp0 = ImVec2(
            t0, 1.0f - ImSaturate(
                           (v0 - scale_min) *
                           inv_scale));  // Point in the normalized space of our
                                         // target rectangle
...
void ImGui::PlotLines(
    ...
    PlotEx(
        ImGuiPlotType_Lines, label, &Plot_ArrayGetter, (void*)&data,
...
void ImGui::PlotLines(
    ...
    PlotEx(
        ImGuiPlotType_Lines, label, values_getter, data, values_count,
...
void ImGui::PlotHistogram(
    ...
    ImGuiPlotArrayGetterData data(values, stride);
    PlotEx(
        ImGuiPlotType_Histogram, label, &Plot_ArrayGetter, (void*)&data,
        values_count, values_offset, overlay_text, scale_min, scale_max,
        graph_size);
}
...
void ImGui::PlotHistogram(
    ...
    float scale_min, float scale_max, ImVec2 graph_size) {
    PlotEx(
        ImGuiPlotType_Histogram, label, values_getter, data, values_count,
        values_offset, overlay_text, scale_min, scale_max, graph_size);
}
...
void ImGui::EndMenuBar() {
    ...
    if (NavMoveRequestButNoResultYet() &&
        ...
        if (nav_earliest_child->ParentWindow == window &&
            ...
            FocusWindow(window);
            SetNavIDWithRectRel(
                window->NavLastIds[layer], layer, window->NavRectRel[layer]);
            g.NavLayer = layer;
...
bool ImGui::BeginMenu(const char* label, bool enabled) {
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        PopStyleVar();
        window->DC.CursorPos.x +=
            (float)(int)(style.ItemSpacing.x * (-1.0f + 0.5f));  // -1 spacing
                                                                 // to
                                                                 // compensate
                                                                 // the spacing
                                                                 // added when
...
bool ImGui::MenuItem(
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        float extra_w = ImMax(0.0f, GetContentRegionAvail().x - w);
        pressed = Selectable(
            label, false, flags | ImGuiSelectableFlags_DrawFillAvailWidth,
            ImVec2(w, 0.0f));
        ...
        if (selected)
            RenderCheckMark(
                pos + ImVec2(
                          window->MenuColumns.Pos[2] + extra_w +
                              g.FontSize * 0.40f,
...
bool ImGui::TabItemEx(
    ImGuiTabBar* tab_bar, const char* label, bool* p_open,
...
bool ImGui::TabItemLabelAndCloseButton(
    ...
    if (flags & ImGuiTabItemFlags_UnsavedDocument) {
        ...
        ImVec2 unsaved_marker_pos(
            ImMin(
                bb.Min.x + frame_padding.x + label_size.x + 2,
                text_pixel_clip_bb.Max.x),
            bb.Min.y + frame_padding.y + (float)(int)(-g.FontSize * 0.25f));
        RenderTextClippedEx(
            draw_list, unsaved_marker_pos, bb.Max - frame_padding,
...
static float GetDraggedColumnOffset(ImGuiColumns* columns, int column_index) {
    ...
    if ((columns->Flags & ImGuiColumnsFlags_NoPreserveWidths))
        x = ImMin(
            x, ImGui::GetColumnOffset(column_index + 1) -
...
void ImGui::PushColumnsBackground() {
    ...
    IM_ASSERT(
        cmd_size ==
        window->DrawList->CmdBuffer.Size);  // Being in channel 0 this should
                                            // not have created an ImDrawCmd
}
...
bool ImGui::BufferingBar(
    const char* label, float value, const ImVec2& size_arg, const ImU32& bg_col,
    const ImU32& fg_col) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
...
bool ImGui::Spinner(
    ...
    for (int i = 0; i < num_segments; i++) {
        ...
        window->DrawList->PathLineTo(ImVec2(
            centre.x + ImCos(a + progress * 8) * radius,
