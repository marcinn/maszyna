...
IMGUI_API void ProgressBar(
    float fraction, const ImVec2& size_arg = ImVec2(-1, 0),
...
IMGUI_API bool DragInt2(
    const char* label, int v[2], float v_speed = 1.0f, int v_min = 0,
    int v_max = 0, const char* format = "%d");
IMGUI_API bool DragInt3(
    const char* label, int v[3], float v_speed = 1.0f, int v_min = 0,
    int v_max = 0, const char* format = "%d");
IMGUI_API bool DragInt4(
    const char* label, int v[4], float v_speed = 1.0f, int v_min = 0,
    int v_max = 0, const char* format = "%d");
...
enum ImGuiBackendFlags_ {
    ...
    ImGuiBackendFlags_HasGamepad = 1
                                   << 0,  // Back-end Platform supports gamepad
                                          // and currently has one connected.
    ImGuiBackendFlags_HasMouseCursors =
        1 << 1,  // Back-end Platform supports honoring GetMouseCursor() value
...
struct ImGuiStyle {
    ...
    ImVec2 SelectableTextAlign;  // Alignment of selectable text when selectable
                                 // is larger than text. Defaults to (0.0f,
...
struct ImGuiIO {
    ...
    ImGuiBackendFlags
        BackendFlags;  // = 0              // See ImGuiBackendFlags_ enum. Set
                       // by back-end (imgui_impl_xxx files or custom back-end)
                       // to communicate features supported by the back-end.
    ...
    bool ConfigWindowsResizeFromEdges;  // = true           // Enable resizing
                                        ...
                                        // flag)
    bool
        ConfigWindowsMoveFromTitleBarOnly;  // = false       // [BETA] Set to
                                            // true to only allow moving windows
                                            // when clicked+dragged from the
                                            // title bar. Windows without a
                                            // title bar are not affected.
    float ConfigWindowsMemoryCompactTimer;  // = 60.0f          // [BETA]
                                            // Compact window memory usage when
    ...
    bool MouseDownOwned[5];  // Track if button was clicked inside a dear imgui
                             ...
                             // application if click started outside ImGui
                             // bounds.
...
struct ImDrawCmd {
    ...
    unsigned int
        VtxOffset;  // Start offset in vertex buffer. Pre-1.71 or without
                    ...
                    // ImGuiBackendFlags_RendererHasVtxOffset: may be >0 to
                    // support meshes larger than 64K vertices with 16-bits
                    // indices.
    unsigned int IdxOffset;  // Start offset in index buffer. Always equal to
                             // sum of ElemCount drawn so far.
...
// the renderer back-end) (to use 32-bits indices: override with '#define
...
struct ImDrawList {
    ...
    IMGUI_API void AddPolyline(
        const ImVec2* points, int num_points, ImU32 col, bool closed,
        float thickness);
    ...
    IMGUI_API void AddBezierCurve(
        const ImVec2& pos0, const ImVec2& cp0, const ImVec2& cp1,
        const ImVec2& pos1, ImU32 col, float thickness, int num_segments = 0);
    ...
    IMGUI_API void PathArcTo(
        const ImVec2& center, float radius, float a_min, float a_max,
        int num_segments = 10);
...
struct ImFontAtlas {
    ...
    IMGUI_API void CalcCustomRectUV(
        const ImFontAtlasCustomRect* rect, ImVec2* out_uv_min,
        ImVec2* out_uv_max);
...
struct ImFont {
    ...
    IMGUI_API const char* CalcWordWrapPositionA(
        float scale, const char* text, const char* text_end,
        float wrap_width) const;
    IMGUI_API void RenderChar(
        ImDrawList* draw_list, float size, ImVec2 pos, ImU32 col,
        ImWchar c) const;
    ...
    IMGUI_API void AddGlyph(
        ImWchar c, float x0, float y0, float x1, float y1, float u0, float v0,
