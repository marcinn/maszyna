...
//  Renderer: Desktop GL only: Support for large meshes (64k+ vertices) with
//  16-bits indices.
...
static void ImGui_ImplOpenGL3_SetupRenderState(
    ...
    glVertexAttribPointer(
        g_AttribLocationVtxColor, 4, GL_UNSIGNED_BYTE, GL_TRUE,
...
bool ImGui_ImplOpenGL3_CreateDeviceObjects() {
    ...
    const GLchar* fbout_gamma =
        "vec4 FBOUT(vec4 x) { return vec4(pow(x.rgb, vec3(1.0 / 2.2)), x.a); "
        "}\n";

    const GLchar* fragment_shader_with_version[3] = {
        g_GlslVersionString, Global.gfx_shadergamma ? fbout_gamma : fbout_copy,
        fragment_shader};
