...
// screenshots/video there!): https://github.com/ocornut/imgui/issues/2529
...
std::vector)?
  - How can I use the drawing facilities without a Dear ImGui window? (using
ImDrawList API)
...
/*
 ...
 the inputs to Dear ImGui. Refer to the FAQ for more information.
 ...
 A: Short explanation:
    - Please read Wiki entry for examples:
 ...
 ImGui_ImplOpenGL3_RenderDrawData() function in imgui_impl_opengl3.cpp)
 ...
 documentation and tutorials for the graphics API you are using.
    ...
    Refer to the Wiki to find simplified examples for loading textures with
 OpenGL, DirectX9 and DirectX11:
 ...
 for more details about icons font loading.) With some extra effort, you may use
 colorful icon by registering custom rectangle space inside the font atlas, and
 copying your own graphics data into it. See misc/fonts/README.txt about using
 the AddCustomRectFontGlyph API.
...
static ImGuiWindow* CreateNewWindow(
    const char* name, ImVec2 size, ImGuiWindowFlags flags);
...
// Settings
static void* SettingsHandlerWindow_ReadOpen(
...
static void SettingsHandlerWindow_WriteAll(
    ImGuiContext* imgui_ctx, ImGuiSettingsHandler* handler,
    ImGuiTextBuffer* buf);
...
static bool UpdateManualResize(
...
static void RenderWindowDecorations(
    ImGuiWindow* window, const ImRect& title_bar_rect,
...
static void RenderWindowTitleBarContents(
    ImGuiWindow* window, const ImRect& title_bar_rect, const char* name,
    bool* p_open);
...
void* ImFileLoadToMemory(
    const char* filename, const char* file_open_mode, size_t* out_file_size,
    int padding_bytes) {
...
void ImGuiStorage::BuildSortByKey() {
    struct StaticFunc {
        ...
        PairCompareByID(const void* lhs, const void* rhs) {
...
void ImGui::RenderText(
    ...
    if (text != text_display_end) {
        window->DrawList->AddText(
            g.Font, g.FontSize, pos, GetColorU32(ImGuiCol_Text), text,
            text_display_end);
...
void ImGui::RenderTextClippedEx(
    ...
    if (need_clipping) {
        ...
        draw_list->AddText(
            NULL, 0.0f, pos, GetColorU32(ImGuiCol_Text), text, text_display_end,
            0.0f, &fine_clip_rect);
    } else {
        draw_list->AddText(
            NULL, 0.0f, pos, GetColorU32(ImGuiCol_Text), text, text_display_end,
...
void ImGui::RenderTextClipped(
    ...
    ImGuiWindow* window = g.CurrentWindow;
    RenderTextClippedEx(
        window->DrawList, pos_min, pos_max, text, text_display_end,
...
void ImGui::RenderTextEllipsis(
    ...
    if (text_size.x > pos_max.x - pos_min.x) {
        ...
        float text_size_clipped_x =
            font->CalcTextSizeA(
                    font_size, text_avail_width, 0.0f, text, text_end_full,
                    &text_end_ellipsis)
                .x;
        ...
        while (text_end_ellipsis > text &&
            ...
            text_size_clipped_x -=
                font->CalcTextSizeA(
                        font_size, FLT_MAX, 0.0f, text_end_ellipsis,
                        text_end_ellipsis + 1)
...
void ImGui::RenderNavHighlight(
    ...
    if (flags & ImGuiNavHighlightFlags_TypeThin) {
        window->DrawList->AddRect(
            display_rect.Min, display_rect.Max,
...
void ImGui::SetNavIDWithRectRel(
    ImGuiID id, int nav_layer, const ImRect& rect_rel) {
    ImGuiContext& g = *GImGui;
...
bool ImGui::DebugCheckVersionAndDataLayout(
    const char* version, size_t sz_io, size_t sz_style, size_t sz_vec2,
...
static void ImGui::UpdateMouseInputs() {
    ...
    for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++) {
        ...
        if (g.IO.MouseClicked[i])  // Clicking any mouse button reactivate mouse
                                   // hovering which may have been deactivated
...
void ImGui::NewFrame() {
    ...
    memcpy(
        g.IO.KeysDownDurationPrev, g.IO.KeysDownDuration,
    ...
    for (int i = 0; i != g.Windows.Size; i++) {
        ...
        // Garbage collect (this is totally functional but we may need decide if
        // the side-effects are desirable)
...
static void AddWindowToSortBuffer(
    ...
    if (window->Active) {
        ...
        if (count > 1)
            ImQsort(
                window->DC.ChildWindows.Data, (size_t)count,
...
void ImGui::Render() {
    ...
    if (g.IO.MouseDrawCursor)
        RenderMouseCursor(
            &g.ForegroundDrawList, g.IO.MousePos, g.Style.MouseCursorScale,
...
static bool ImGui::BeginChildEx(
    ...
    if (name)
        ImFormatString(
            title, IM_ARRAYSIZE(title), "%s/%s_%08X", parent_window->Name, name,
            id);
    else
        ImFormatString(
...
bool ImGui::BeginChild(
    const char* str_id, const ImVec2& size_arg, bool border,
...
bool ImGui::BeginChild(
    ImGuiID id, const ImVec2& size_arg, bool border,
    ImGuiWindowFlags extra_flags) {
    IM_ASSERT(id != 0);
...
static void CheckStacksSize(ImGuiWindow* window, bool write) {
    ...
    IM_ASSERT(
        p_backup == window->DC.StackSizesBackup +
...
static void CalcResizePosSizeFromAnyCorner(
    ...
    ImVec2 pos_max = ImLerp(
        window->Pos + window->Size, corner_target,
...
static ImRect GetResizeBorderRect(
    ...
    if (border_n == 0)
        return ImRect(
            rect.Min.x + perp_padding, rect.Min.y - thickness,
            rect.Max.x - perp_padding, rect.Min.y + thickness);  // Top
    if (border_n == 1)
        return ImRect(
            rect.Max.x - thickness, rect.Min.y + perp_padding,
            rect.Max.x + thickness, rect.Max.y - perp_padding);  // Right
    if (border_n == 2)
        return ImRect(
            rect.Min.x + perp_padding, rect.Max.y - thickness,
            rect.Max.x - perp_padding, rect.Max.y + thickness);  // Bottom
    if (border_n == 3)
        return ImRect(
            rect.Min.x - thickness, rect.Min.y + perp_padding,
...
static bool ImGui::UpdateManualResize(
    ImGuiWindow* window, const ImVec2& size_auto_fit, int* border_held,
    int resize_grip_count, ImU32 resize_grip_col[4]) {
    ImGuiContext& g = *GImGui;
...
static inline void ClampWindowRect(
    ImGuiWindow* window, const ImRect& rect, const ImVec2& padding) {
    ImGuiContext& g = *GImGui;
...
void ImGui::RenderWindowDecorations(
    ...
    if (window->Collapsed) {
        ...
        RenderFrame(
            title_bar_rect.Min, title_bar_rect.Max, title_bar_col, true,
            window_rounding);
...
void ImGui::RenderWindowTitleBarContents(
    ...
    ImRect layout_r(
        title_bar_rect.Min.x + pad_l, title_bar_rect.Min.y,
    ...
    RenderTextClipped(
        layout_r.Min, layout_r.Max, name, NULL, &text_size,
    ...
    if (flags & ImGuiWindowFlags_UnsavedDocument) {
        ...
        RenderTextClipped(
            marker_pos + off, layout_r.Max + off, UNSAVED_DOCUMENT_MARKER, NULL,
            NULL, ImVec2(0, style.WindowTitleAlign.y), &clip_r);
    }
}
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags) {
    ...
    if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasCollapsed)
        SetWindowCollapsed(
            window, g.NextWindowData.CollapsedVal,
    ...
    if (first_begin_of_the_frame) {
        ...
        if (!window_pos_set_by_api && !(flags & ImGuiWindowFlags_ChildWindow) &&
            ...
            {
                ImVec2 clamp_padding = ImMax(
                    style.DisplayWindowPadding, style.DisplaySafeAreaPadding);
        ...
        RenderWindowDecorations(
            window, title_bar_rect, title_bar_is_highlight, resize_grip_count,
        ...
        // FIXME-OBSOLETE: window->ContentsRegionRect.Max is currently very
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
bool ImGui::Begin(
    const char* name, bool* p_open, const ImVec2& size_first_use,
...
static ImVec2 CalcNextScrollFromScrollTargetAndClamp(
    ...
    if (window->ScrollTarget.x < FLT_MAX) {
        ...
        if (snap_on_edges && cr_x <= 0.0f &&
            target_x <= window->WindowPadding.x)
            target_x = 0.0f;
        else if (
            snap_on_edges && cr_x >= 1.0f &&
...
void ImGui::BeginTooltipEx(
    ImGuiWindowFlags extra_flags, bool override_previous_tooltip) {
    ImGuiContext& g = *GImGui;
    char window_name[16];
    ImFormatString(
...
ImVec2 ImGui::FindBestWindowPosForPopup(ImGuiWindow* window) {
    ...
    if (window->Flags & ImGuiWindowFlags_Popup) {
        ImRect r_avoid = ImRect(
            window->Pos.x - 1, window->Pos.y - 1, window->Pos.x + 1,
            window->Pos.y + 1);
        return FindBestWindowPosForPopupEx(
            window->Pos, window->Size, &window->AutoPosLastDirection, r_outer,
            r_avoid);
...
static bool ImGui::NavScoreItem(ImGuiNavMoveResult* result, ImRect cand) {
                              ...
                              // Press C to rotate.
    {
        ...
        if (quadrant == g.NavMoveDir) {
            ...
            draw_list->AddText(
                g.IO.FontDefault, 13.0f, cand.Min, IM_COL32(255, 255, 255, 255),
                buf);
        }
    }
#endif
...
static void ImGui::NavProcessItem(
    ...
    if ((g.NavId != id ||
        ...
        if ((g.NavMoveRequestFlags & ImGuiNavMoveFlags_AlsoScoreVisibleSet) &&
            ...
            if (ImClamp(
                    nav_bb.Max.y, window->ClipRect.Min.y,
...
void ImGui::NavMoveRequestForward(
    ImGuiDir move_dir, ImGuiDir clip_dir, const ImRect& bb_rel,
...
#undef NAV_MAP_KEY
    ...
    memcpy(
        g.IO.NavInputsDownDurationPrev, g.IO.NavInputsDownDuration,
        ...
        if (!window_rect_rel.Contains(window->NavRectRel[g.NavLayer])) {
            ...
            window_rect_rel.Expand(ImVec2(
                ...
                -ImMin(
                    ...
                    pad)));  // Terrible approximation for the intent of
                             // starting navigation from first fully visible
                             // item
            ...
            const float move_speed = ImFloor(
                ...
                ImMin(
                    ...
                    g.IO.DisplayFramebufferScale
                        .y));  // FIXME: Doesn't code variable framerate very
                               // well
            SetWindowPos(
                g.NavWindowingTarget->RootWindow,
    ...
    for (;;) {
        ...
        if (!is_last_line || (line_start != line_end)) {
            ...
            if (log_new_line || !is_first_line)
                LogText(
                    IM_NEWLINE "%*s%.*s", tree_depth * 4, "", char_count,
                    line_start);
...
static void SettingsHandlerWindow_WriteAll(
    ...
    buf->reserve(
        buf->size() + g.SettingsWindows.Size * 96);  // ballpark reserve
    for (int i = 0; i != g.SettingsWindows.Size; i++) {
...
static void SetClipboardTextFn_DefaultImpl(void*, const char* text) {
    ...
    if (cf_data) {
        PasteboardPutItemFlavor(
            main_clipboard, (PasteboardItemID)1,
    ...
    for (ItemCount i = 0; i < item_count; i++) {
        ...
        for (CFIndex j = 0, nj = CFArrayGetCount(flavor_type_array); j < nj;
            ...
            if (PasteboardCopyItemFlavorData(
                    main_clipboard, item_id, CFSTR("public.utf8-plain-text"),
                    &cf_data) == noErr) {
...
void ImGui::ShowMetricsWindow(bool* p_open) {
    ...
    ImGui::Text(
        "%d active windows (%d visible)", io.MetricsActiveWindows,
    ...
    struct Funcs {
        ...
        static void NodeDrawList(
            ...
            ImDrawList* fg_draw_list =
                GetForegroundDrawList(window);  // Render additional visuals
            ...
            if (window && !window->WasActive)
                ImGui::Text(
                    "(Note: owning Window is inactive: DrawList is not being "
                    "rendered!)");
            ...
            for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin();
                ...
                ImFormatString(
                    buf, IM_ARRAYSIZE(buf),
                    "Draw %4d triangles, tex 0x%p, clip_rect "
                    "(%4.0f,%4.0f)-(%4.0f,%4.0f)",
                ...
                ImGui::Text(
                    "ElemCount: %d, ElemCount/3: %d, VtxOffset: +%d, "
                    "IdxOffset: +%d",
        ...
        static void NodeWindow(ImGuiWindow* window, const char* label) {
            ...
            ImGui::BulletText(
                "Storage: %d bytes", window->StateStorage.Data.size_in_bytes());
            ImGui::TreePop();
        }
    ...
    if (ImGui::TreeNode("Internal state")) {
        ...
        ImGui::Text(
            "NavActivateId: 0x%08X, NavInputId: 0x%08X", g.NavActivateId,
            g.NavInputId);
        ImGui::Text(
    ...
    if (ImGui::TreeNode("Tools")) {
        ...
        show_windows_rects |= ImGui::Combo(
            "##show_windows_rect_type", &show_windows_rect_type,
