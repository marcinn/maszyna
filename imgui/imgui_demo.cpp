...
void ImGui::ShowDemoWindow(bool* p_open) {
    ...
    if (ImGui::BeginMenuBar()) {
        ...
        if (ImGui::BeginMenu("Examples")) {
            ...
            ImGui::MenuItem(
                "Constrained-resizing window", NULL,
    ...
    if (ImGui::CollapsingHeader("Configuration")) {
        ...
        if (ImGui::TreeNode("Configuration##2")) {
            ...
            HelpMarker(
                ...
                "ImGuiConfigFlags_NavEnableSetMousePos.");
            ImGui::CheckboxFlags(
                "io.ConfigFlags: NoMouse", (unsigned int*)&io.ConfigFlags,
        ...
        if (ImGui::TreeNode("Backend Flags")) {
            ...
            ImGuiBackendFlags backend_flags =
                io.BackendFlags;  // Make a local copy to avoid modifying actual
                                  // back-end flags.
            ImGui::CheckboxFlags(
                "io.BackendFlags: HasGamepad", (unsigned int*)&backend_flags,
                ImGuiBackendFlags_HasGamepad);
            ImGui::CheckboxFlags(
...
static void ShowDemoWindowWidgets() {
    ...
    if (ImGui::TreeNode("Trees")) {
        ...
        if (ImGui::TreeNode("Advanced, with Selectable nodes")) {
            HelpMarker(
                "This is a more typical looking tree with selectable "
            ...
            for (int i = 0; i < 6; i++) {
                ...
                if (i < 3) {
                    ...
                    bool node_open = ImGui::TreeNodeEx(
                        (void*)(intptr_t)i, node_flags, "Selectable Node %d",
                        i);
                    ...
                    ImGui::TreeNodeEx(
                        (void*)(intptr_t)i, node_flags, "Selectable Leaf %d",
                        i);
                    if (ImGui::IsItemClicked()) node_clicked = i;
                }
            }
            if (node_clicked != -1) {
                ...
                if (ImGui::GetIO().KeyCtrl)
                    selection_mask ^=
                      ...
                      // commented bit preserve selection when clicking on item
    ...
    if (ImGui::TreeNode("Images")) {
        ...
        ImVec2 pos = ImGui::GetCursorScreenPos();
        ImGui::Image(
            my_tex_id, ImVec2(my_tex_w, my_tex_h), ImVec2(0, 0), ImVec2(1, 1),
    ...
    if (ImGui::TreeNode("Combo")) {
        ...
        ImGui::CheckboxFlags(
            "ImGuiComboFlags_PopupAlignLeft", (unsigned int*)&flags,
            ImGuiComboFlags_PopupAlignLeft);
        ImGui::SameLine();
        HelpMarker(
            "Only makes a difference if the popup is larger than the combo");
        if (ImGui::CheckboxFlags(
                "ImGuiComboFlags_NoArrowButton", (unsigned int*)&flags,
                ImGuiComboFlags_NoArrowButton))
            flags &= ~ImGuiComboFlags_NoPreview;  // Clear the other flag, as we
                                                  // cannot combine both
        if (ImGui::CheckboxFlags(
                "ImGuiComboFlags_NoPreview", (unsigned int*)&flags,
                ImGuiComboFlags_NoPreview))
            flags &=
                ~ImGuiComboFlags_NoArrowButton;  // Clear the other flag, as we
                                                 // cannot combine both
        ...
        if (ImGui::BeginCombo(
                ...
                flags))  // The second parameter is the label previewed before
                         // opening the combo.
        {
        ...
        ImGui::Combo(
            "combo 4 (function)", &item_current_4, &FuncHolder::ItemGetter,
    ...
    if (ImGui::TreeNode("Text Input")) {
        if (ImGui::TreeNode("Multi-line Text Input")) {
            ...
            ImGui::CheckboxFlags(
                "ImGuiInputTextFlags_ReadOnly", (unsigned int*)&flags,
                ImGuiInputTextFlags_ReadOnly);
            ImGui::CheckboxFlags(
                "ImGuiInputTextFlags_AllowTabInput", (unsigned int*)&flags,
                ImGuiInputTextFlags_AllowTabInput);
            ImGui::CheckboxFlags(
        ...
        if (ImGui::TreeNode("Resize Callback")) {
            ...
            // misc/cpp/imgui_stdlib.h and .cpp for an implementation of this
            // using std::string.
            HelpMarker(
                "Demonstrate using ImGuiInputTextFlags_CallbackResize to wire "
                "your resizable string type to InputText().\n\nSee "
            ...
            struct Funcs {
                ...
                // Tip: Because ImGui:: is a namespace you would typicall add
                // your own function into the namespace in your own source
                // files. For example, you may add a function called
    ...
    if (ImGui::TreeNode("Plots Widgets")) {
        ...
        while (refresh_time < ImGui::GetTime())  // Create dummy data at fixed
                                                 // 60 hz rate for the demo
        {
        ...
        ImGui::PlotLines(
            "Lines", values, IM_ARRAYSIZE(values), values_offset, "avg 0.0",
            -1.0f, 1.0f, ImVec2(0, 80));
        ImGui::PlotHistogram(
            "Histogram", arr, IM_ARRAYSIZE(arr), 0, NULL, 0.0f, 1.0f,
            ImVec2(0, 80));
        ...
        ImGui::PlotLines(
            "Lines", func, NULL, display_count, 0, NULL, -1.0f, 1.0f,
            ImVec2(0, 80));
        ImGui::PlotHistogram(
            "Histogram", func, NULL, display_count, 0, NULL, -1.0f, 1.0f,
            ImVec2(0, 80));
        ImGui::Separator();
    ...
    if (ImGui::TreeNode("Color/Picker Widgets")) {
        ...
        HelpMarker(
            ...
            "but ImGuiColorEditFlags_InputHSV allows you to store colors as "
            "HSV and pass them to ColorEdit and ColorPicker as HSV. This comes "
            "with the added benefit that you can manipulate hue values with "
            "the picker even when saturation or value are zero.");
        ImGui::Text("Color widget with InputHSV:");
        ImGui::ColorEdit4(
            "HSV shown as RGB##1", (float*)&color_stored_as_hsv,
            ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_InputHSV |
                ImGuiColorEditFlags_Float);
        ImGui::ColorEdit4(
            "HSV shown as HSV##1", (float*)&color_stored_as_hsv,
            ImGuiColorEditFlags_DisplayHSV | ImGuiColorEditFlags_InputHSV |
                ImGuiColorEditFlags_Float);
        ImGui::DragFloat4(
            "Raw HSV values", (float*)&color_stored_as_hsv, 0.01f, 0.0f, 1.0f);
    ...
    if (ImGui::TreeNode("Range Widgets")) {
        ...
        ImGui::DragFloatRange2(
            "range", &begin, &end, 0.25f, 0.0f, 100.0f, "Min: %.1f %%",
            "Max: %.1f %%");
        ImGui::DragIntRange2(
...
// natural type maximum, hence the /2 below.
        ...
        ImGui::DragScalar(
            "drag s8", ImGuiDataType_S8, &s8_v, drag_speed,
            drag_clamp ? &s8_zero : NULL, drag_clamp ? &s8_fifty : NULL);
        ImGui::DragScalar(
            "drag u8", ImGuiDataType_U8, &u8_v, drag_speed,
        ...
        ImGui::DragScalar(
            "drag float", ImGuiDataType_Float, &f32_v, 0.005f, &f32_zero,
            &f32_one, "%f", 1.0f);
        ImGui::DragScalar(
            "drag float ^2", ImGuiDataType_Float, &f32_v, 0.005f, &f32_zero,
            &f32_one, "%f", 2.0f);
        ImGui::SameLine();
        HelpMarker(
        ...
        ImGui::DragScalar(
            "drag double", ImGuiDataType_Double, &f64_v, 0.0005f, &f64_zero,
            NULL, "%.10f grams", 1.0f);
        ImGui::DragScalar(
            "drag double ^2", ImGuiDataType_Double, &f64_v, 0.0005f, &f64_zero,
        ...
        ImGui::SliderScalar(
            ...
            "%I64u ms");
        ImGui::SliderScalar(
            "slider float low", ImGuiDataType_Float, &f32_v, &f32_zero,
            &f32_one);
        ImGui::SliderScalar(
            "slider float low^2", ImGuiDataType_Float, &f32_v, &f32_zero,
            &f32_one, "%.10f", 2.0f);
        ImGui::SliderScalar(
            "slider float high", ImGuiDataType_Float, &f32_v, &f32_lo_a,
            &f32_hi_a, "%e");
        ImGui::SliderScalar(
            "slider double low", ImGuiDataType_Double, &f64_v, &f64_zero,
            &f64_one, "%.10f grams", 1.0f);
        ImGui::SliderScalar(
            "slider double low^2", ImGuiDataType_Double, &f64_v, &f64_zero,
            &f64_one, "%.10f", 2.0f);
        ImGui::SliderScalar(
            "slider double high", ImGuiDataType_Double, &f64_v, &f64_lo_a,
        ...
        ImGui::InputScalar(
            "input s8", ImGuiDataType_S8, &s8_v, inputs_step ? &s8_one : NULL,
            NULL, "%d");
        ImGui::InputScalar(
            "input u8", ImGuiDataType_U8, &u8_v, inputs_step ? &u8_one : NULL,
            NULL, "%u");
        ImGui::InputScalar(
            "input s16", ImGuiDataType_S16, &s16_v,
            inputs_step ? &s16_one : NULL, NULL, "%d");
        ImGui::InputScalar(
            "input u16", ImGuiDataType_U16, &u16_v,
            inputs_step ? &u16_one : NULL, NULL, "%u");
        ImGui::InputScalar(
            "input s32", ImGuiDataType_S32, &s32_v,
            inputs_step ? &s32_one : NULL, NULL, "%d");
        ImGui::InputScalar(
            "input s32 hex", ImGuiDataType_S32, &s32_v,
            inputs_step ? &s32_one : NULL, NULL, "%08X",
            ImGuiInputTextFlags_CharsHexadecimal);
        ImGui::InputScalar(
            "input u32", ImGuiDataType_U32, &u32_v,
            inputs_step ? &u32_one : NULL, NULL, "%u");
        ImGui::InputScalar(
            "input u32 hex", ImGuiDataType_U32, &u32_v,
            inputs_step ? &u32_one : NULL, NULL, "%08X",
            ImGuiInputTextFlags_CharsHexadecimal);
        ImGui::InputScalar(
            "input s64", ImGuiDataType_S64, &s64_v,
            inputs_step ? &s64_one : NULL);
        ImGui::InputScalar(
            "input u64", ImGuiDataType_U64, &u64_v,
            inputs_step ? &u64_one : NULL);
        ImGui::InputScalar(
            "input float", ImGuiDataType_Float, &f32_v,
            inputs_step ? &f32_one : NULL);
        ImGui::InputScalar(
            "input double", ImGuiDataType_Double, &f64_v,
    ...
    if (ImGui::TreeNode("Querying Status (Active/Focused/Hovered etc.)")) {
        ...
        if (item_type == 9) {
            ...
            ret = ImGui::ListBox(
                "ITEM: ListBox", &current, items, IM_ARRAYSIZE(items),
                IM_ARRAYSIZE(items));
        }
        ImGui::BulletText(
            "Return value = %d\n"
            ...
            ImGui::IsItemClicked(), ImGui::GetItemRectMin().x,
        ...
        ImGui::InputText(
            "dummy", dummy_str, IM_ARRAYSIZE(dummy_str),
            ImGuiInputTextFlags_ReadOnly);
...
static void ShowDemoWindowLayout() {
    ...
    if (ImGui::TreeNode("Tabs")) {
        ...
        if (ImGui::TreeNode("Advanced & Close Button")) {
            ...
            ImGui::CheckboxFlags(
                "ImGuiTabBarFlags_Reorderable", (unsigned int*)&tab_bar_flags,
                ImGuiTabBarFlags_Reorderable);
            ImGui::CheckboxFlags(
    ...
    if (ImGui::TreeNode("Groups")) {
        ...
        ImGui::PlotHistogram(
            "##values", values, IM_ARRAYSIZE(values), 0, NULL, 0.0f, 1.0f,
            size);
    ...
    if (ImGui::TreeNode("Text Baseline Alignment")) {
        ...
        ImGui::AlignTextToFramePadding();  // If your line starts with text,
                                           // call this to align it to upcoming
                                           // widgets.
        ...
        bool node_open = ImGui::TreeNode(
            "Node##2");  // Common mistake to avoid: if we want to SameLine
                         // after TreeNode we need to do it before we add child
                         // content.
        ImGui::SameLine(0.0f, spacing);
    ...
    if (ImGui::TreeNode("Scrolling")) {
        ...
        scroll_to_off |= ImGui::DragFloat(
            "##off_y", &scroll_to_off_px, 1.00f, 0, 9999, "+%.0f px");

        bool scroll_to_pos = ImGui::Button("Scroll To Pos");
        ImGui::SameLine(140);
        scroll_to_pos |= ImGui::DragFloat(
            "##pos_y", &scroll_to_pos_px, 1.00f, 0, 9999, "Y = %.0f px");

        ImGui::PopItemWidth();
        if (scroll_to_off || scroll_to_pos) track = false;
        ...
        for (int i = 0; i < 5; i++) {
            ...
            ImGui::BeginChild(
                ImGui::GetID((void*)(intptr_t)i), ImVec2(child_w, 200.0f), true,
                child_flags);
    ...
    if (ImGui::TreeNode("Horizontal Scrolling")) {
        ...
        if (scroll_x_delta != 0.0f) {
            ImGui::BeginChild(
                "scrolling");  // Demonstrate a trick: you can use Begin to set
                               // yourself in the context of another window
        ...
        if (show_horizontal_contents_size_demo_window) {
            ...
            ImGui::Text(
                "Scroll %.1f/%.1f %.1f/%.1f", ImGui::GetScrollX(),
...
static void ShowDemoWindowColumns() {
    ...
    */
    ...
    if (ImGui::TreeNode("Horizontal Scrolling")) {
        ...
        ImGui::BeginChild(
            "##ScrollingRegion", ImVec2(0, ImGui::GetFontSize() * 20), false,
            ImGuiWindowFlags_HorizontalScrollbar);
        ImGui::Columns(10);
...
static void ShowDemoWindowMisc() {
    ...
    if (ImGui::CollapsingHeader("Inputs, Navigation & Focus")) {
        ...
        if (ImGui::TreeNode("Dragging")) {
            ...
            ImGui::Text(
                "GetMouseDragDelta(0):\n  w/ default threshold: (%.1f, "
                "%.1f),\n  w/ zero threshold: (%.1f, %.1f)\nMouseDelta: (%.1f, "
                "%.1f)",
                value_with_lock_threshold.x, value_with_lock_threshold.y,
                value_raw.x, value_raw.y, mouse_delta.x, mouse_delta.y);
            ImGui::TreePop();
        }
        ...
        if (ImGui::TreeNode("Mouse cursors")) {
            ...
            ImGui::Text(
                "Current mouse cursor = %d: %s", ImGui::GetMouseCursor(),
...
void ImGui::ShowAboutWindow(bool* p_open) {
    ...
    if (show_config_info) {
        ...
        ImGui::Separator();
        ImGui::Text(
            "style.WindowPadding: %.2f,%.2f", style.WindowPadding.x,
            style.WindowPadding.y);
        ImGui::Text("style.WindowBorderSize: %.2f", style.WindowBorderSize);
        ImGui::Text(
            "style.FramePadding: %.2f,%.2f", style.FramePadding.x,
            style.FramePadding.y);
        ImGui::Text("style.FrameRounding: %.2f", style.FrameRounding);
        ImGui::Text("style.FrameBorderSize: %.2f", style.FrameBorderSize);
        ImGui::Text(
            "style.ItemSpacing: %.2f,%.2f", style.ItemSpacing.x,
            style.ItemSpacing.y);
        ImGui::Text(
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref) {
    ...
    if (ImGui::BeginTabBar("##tabs", ImGuiTabBarFlags_None)) {
        if (ImGui::BeginTabItem("Sizes")) {
            ImGui::Text("Main");
            ImGui::SliderFloat2(
                "WindowPadding", (float*)&style.WindowPadding, 0.0f, 20.0f,
                "%.0f");
            ImGui::SliderFloat2(
                "FramePadding", (float*)&style.FramePadding, 0.0f, 20.0f,
                "%.0f");
            ImGui::SliderFloat2(
                "ItemSpacing", (float*)&style.ItemSpacing, 0.0f, 20.0f, "%.0f");
            ImGui::SliderFloat2(
                "ItemInnerSpacing", (float*)&style.ItemInnerSpacing, 0.0f,
                20.0f, "%.0f");
            ImGui::SliderFloat2(
                "TouchExtraPadding", (float*)&style.TouchExtraPadding, 0.0f,
                10.0f, "%.0f");
            ImGui::SliderFloat(
            ...
            ImGui::SliderFloat2(
                "WindowTitleAlign", (float*)&style.WindowTitleAlign, 0.0f, 1.0f,
                "%.2f");
            ImGui::Combo(
                "WindowMenuButtonPosition",
                (int*)&style.WindowMenuButtonPosition, "Left\0Right\0");
            ImGui::SliderFloat2(
                "ButtonTextAlign", (float*)&style.ButtonTextAlign, 0.0f, 1.0f,
                "%.2f");
            ImGui::SameLine();
            HelpMarker(
                ...
                "content.");
            ImGui::SliderFloat2(
                "SelectableTextAlign", (float*)&style.SelectableTextAlign, 0.0f,
                1.0f, "%.2f");
            ImGui::SameLine();
            HelpMarker(
                "Alignment applies when a selectable is larger than its text "
            ...
            ImGui::SliderFloat2(
                "DisplaySafeAreaPadding", (float*)&style.DisplaySafeAreaPadding,
                0.0f, 30.0f, "%.0f");
            ImGui::EndTabItem();
        }
        ...
        if (ImGui::BeginTabItem("Fonts")) {
            ...
            for (int i = 0; i < atlas->Fonts.Size; i++) {
                ...
                if (font_details_opened) {
                    ...
                    ImGui::Text(
                        "Ascent: %f, Descent: %f, Height: %f", font->Ascent,
                        font->Descent, font->Ascent - font->Descent);
                    ImGui::Text(
                        "Fallback character: '%c' (%d)", font->FallbackChar,
                        font->FallbackChar);
            ...
            ImGui::DragFloat(
                "global scale", &io.FontGlobalScale, 0.005f, 0.3f, 2.0f,
                "%.2f");  // scale everything
            ImGui::PopItemWidth();
        ...
        if (ImGui::BeginTabItem("Rendering")) {
            ...
            ImGui::PushItemWidth(100);
            ImGui::DragFloat(
                "Curve Tessellation Tolerance", &style.CurveTessellationTol,
                0.02f, 0.10f, FLT_MAX, "%.2f", 2.0f);
    ...
    void Draw(const char* title, bool* p_open) {
        ...
        for (int i = 0; i < Items.Size; i++) {
            ...
            if (strstr(item, "[error]")) {
                ...
                ImGui::PushStyleColor(
                    ImGuiCol_Text, ImVec4(1.0f, 0.8f, 0.6f, 1.0f));
                pop_color = true;
            }
...
static void ShowExampleAppAutoResize(bool* p_open) {
    if (!ImGui::Begin(
            "Example: Auto-resizing window", p_open,
    ...
    if (ImGui::BeginTabBar("##TabBar")) {
        ...
        if (ImGui::BeginTabItem("Primitives")) {
            ...
            for (int n = 0; n < 2; n++) {
                ...
                draw_list->AddCircle(
                    ImVec2(x + sz * 0.5f, y + sz * 0.5f), sz * 0.5f, col, 6,
                    th);
                x += sz + spacing;  // Hexagon
                draw_list->AddCircle(
                    ImVec2(x + sz * 0.5f, y + sz * 0.5f), sz * 0.5f, col, 20,
                    th);
                x += sz + spacing;  // Circle
                draw_list->AddRect(
                    ImVec2(x, y), ImVec2(x + sz, y + sz), col, 0.0f,
                    corners_none, th);
                x += sz + spacing;  // Square
                draw_list->AddRect(
                ...
                draw_list->AddTriangle(
                    ImVec2(x + sz * 0.2f, y), ImVec2(x, y + sz - 0.5f),
            ...
            x += sz + spacing;  // Hexagon
            draw_list->AddCircleFilled(
                ImVec2(x + sz * 0.5f, y + sz * 0.5f), sz * 0.5f, col, 32);
            x += sz + spacing;  // Circle
        ...
        if (ImGui::BeginTabItem("Canvas")) {
            ...
            for (int i = 0; i < points.Size - 1; i += 2)
                draw_list->AddLine(
                    ImVec2(
                        canvas_pos.x + points[i].x, canvas_pos.y + points[i].y),
                    ImVec2(
                        canvas_pos.x + points[i + 1].x,
