...
class driver_ui : public ui_layer {
   public:
    ...
    bool on_key(int const Key, int const Action) override;
    // potentially processes provided mouse movement. returns: true if the input
    // was processed, false otherwise
    bool on_cursor_pos(double const Horizontal, double const Vertical) override;
    // potentially processes provided mouse button. returns: true if the input
    // was processed, false otherwise
    bool on_mouse_button(int const Button, int const Action) override;
   ...
   private:
    ...
    void set_cursor(bool const Visible);
    ...
    transcripts_panel m_transcriptspanel{
        "Transcripts", true};  // voice transcripts
