...
void zmq_input::poll() {
    ...
    while ((ok = multipart.recv(*sock, ZMQ_DONTWAIT))) {
        if (multipart.size() < 3) continue;

        if (multipart[0].size() != 5) continue;
        ...
        if (peer_it == peers.end()) {
            ...
            peer_it->second.last_update =
                std::chrono::high_resolution_clock::now();
        }

        if (multipart[1] == zmq::message_t("REG_SOPI", 8)) {
            if (multipart.size() < 4) continue;
            ...
            for (size_t i = 3; i < multipart.size(); i++) {
                std::string chan_name(
                    (char *)multipart[i].data(), multipart[i].size());
        ...
        if (multipart[1] == zmq::message_t("REG_SIPO", 8)) {
            ...
            for (size_t i = 2; i < multipart.size(); i++) {
                ...
                user_command cmd1 = user_command::none,
                             cmd2 = user_command::none;

                auto cmd1_it = nametocommandmap.find(id1);
                if (cmd1_it != nametocommandmap.end()) cmd1 = cmd1_it->second;
                ...
                if (cmd1 == user_command::none) type = input_type::none;

                peer_it->second.sipo_list.push_back(
                    std::make_tuple(type, cmd1, cmd2, false));
            }
        }
    ...
    for (auto peer = peers.begin(); peer != peers.end();) {
        if (std::chrono::duration<float>(now - peer->second.last_update)
                .count() < peer->second.update_interval) {
            ++peer;
            continue;
        }
        ...
        uint8_t peerbuf[5] = {
            0, (uint8_t)(peer->first >> 24), (uint8_t)(peer->first >> 16),
            (uint8_t)(peer->first >> 8), (uint8_t)(peer->first)};
        zmq::multipart_t msg;
...
std::unordered_map<std::string, zmq_input::output_fields>
    zmq_input::output_fields_map = {
...
zmq::message_t zmq_input::pack_field(zmq_input::output_fields f) {
    ...
    if (f == output_fields::time_minute_of_month)
        return pack_float(
            (time.wDay - 1) * 1440 + time.wHour * 60 + time.wMinute);
    ...
    if (!train) return pack_float(0.0f);
    const TTrain::state_t state = train->get_state();

    if (f == output_fields::shp) return pack_float(state.shp);
    if (f == output_fields::alerter) return pack_float(state.alerter);
    if (f == output_fields::radio_stop) return pack_float(state.radio_stop);
    if (f == output_fields::motor_resistors)
        return pack_float(state.motor_resistors);
    if (f == output_fields::line_breaker) return pack_float(state.line_breaker);
    ...
    if (f == output_fields::wheelslip) return pack_float(state.wheelslip);
    ...
    if (f == output_fields::cab) return pack_float(state.cab);
    ...
    if (f == output_fields::velocity) return pack_float(state.velocity);
    ...
    if (f == output_fields::hv_voltage) return pack_float(state.hv_voltage);
    ...
    if (f == output_fields::lv_voltage) return pack_float(state.lv_voltage);
    if (f == output_fields::distance) return pack_float(state.distance);
