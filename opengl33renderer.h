...
class opengl33_renderer : public gfx_renderer {
   public:
    ...
    bool Debug_Ui_State(std::optional<bool>) override;
   ...
   private:
    ...
    struct viewport_config {
        ...
        bool real_window =
            true;  // whether we need to blit onto GLFWwindow surface
        bool custom_backbuffer =
            false;  // whether we want to render to our offscreen LDR backbuffer
                    // (pipeline required)
        bool shadow = false;  // include this viewport area in shadowmap

        enum vp_type { normal, custom, vr_left, vr_right } proj_type;
        ...
        // final HDR buffer (also serving as msaa resolve buffer when not using
        // motion blur)
    ...
    bool Init_caps();
    void setup_pass(
        viewport_config &Viewport, renderpass_config &Config,
        rendermode const Mode, float const Znear = 0.f, float const Zfar = 1.f,
...
*/
    ...
    double m_environmentupdatetime{
        0};  // time of the most recent environment map update
    ...
    std::array<renderpass_config, 3>
        m_shadowpass;  // parametrs of most recent shadowmap pass for each of
                       // csm stages
    ...
    glm::mat4 perspective_projection_raw(
    ...
    glm::mat4 ortho_projection(
        ...
        float z_far);
    glm::mat4 ortho_frustumtest_projection(
        float left, float right, float bottom, float top, float z_near,
        float z_far);

    std::vector<std::function<void(TSubModel const *, glm::vec2)>>
