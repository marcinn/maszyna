...
void global_settings::ConfigParse(cParser &Parser) {
    ...
    do {
        ...
        if (ConfigParse_gfx(Parser, token)) {
            ...
            Parser >> FreeCameraInit[0].x, FreeCameraInit[0].y,
                FreeCameraInit[0].z;
        } else if (token == "wireframe") {
            Parser.getTokens();
            Parser >> bWireFrame;
        } else if (token == "debugmode") {  // McZapkie! - DebugModeFlag uzywana
                                            // w mover.pas,
            // warto tez blokowac cheaty gdy false
            Parser.getTokens();
            Parser >> DebugModeFlag;
        } else if (token == "soundenabled") {  // McZapkie-040302 - blokada
                                               // dzwieku - przyda
            ...
            AudioRenderer =
                Parser.getToken<std::string>(false);  // case-sensitive
            ...
            EnvironmentPositionalVolume =
                clamp(EnvironmentPositionalVolume, 0.f, 1.f);
        } else if (token == "sound.volume.ambient") {
            Parser.getTokens();
            Parser >> EnvironmentAmbientVolume;
            EnvironmentAmbientVolume =
                clamp(EnvironmentAmbientVolume, 0.f, 1.f);
        }
        ...
        // bRenderAlpha=(GetNextSymbol().LowerCase()==AnsiString("yes"));
        else if (token == "physicslog") {  // McZapkie-030402 - logowanie
                                           // parametrow
            // fizycznych dla kazdego pojazdu z maszynista
            Parser.getTokens();
            Parser >> WriteLogFlag;
        } else if (token == "fullphysics") {  // McZapkie-291103 - usypianie
                                              // fizyki
            Parser.getTokens();
            Parser >> FullPhysics;
            ...
            // Winger 040204 - 'zywe' patyki dostosowujace sie do trakcji; Ra
            // 2014-03: teraz łamanie
            Parser.getTokens();
        ...
        } else if (token == "friction") {  // mnożnik tarcia - KURS90
            Parser.getTokens(1, false);
            Parser >> fFriction;
            ...
            if (AnisotropicFiltering < 1.0f) AnisotropicFiltering = 1.0f;
            ...
            Parser.getTokens(1, false);
            Parser >> fMoveLight;
            if (fMoveLight == 0.f) {  // pobranie daty z systemu
                std::time_t timenow = std::time(0);
                std::tm *localtime = std::localtime(&timenow);
                fMoveLight =
                    localtime->tm_yday + 1;  // numer bieżącego dnia w roku
            }
            simulation::Environment.compute_season(fMoveLight);
            ...
            // max 8 lights per opengl specs, minus one used for sun. at least
            // one light for controlled vehicle
            ...
            ScenarioTimeOverride =
                clamp(ScenarioTimeOverride, 0.f, 24 * 1439 / 1440.f);
            ...
            // whether strength of specular highlights should be adjusted
            // (generally needed for legacy 3d models)
            Parser.getTokens();
            ...
            float discard;
            Parser >> shadowtune.map_size >> discard >> shadowtune.range >>
                discard;
            shadowtune.map_size = clamp_power_of_two<unsigned int>(
                shadowtune.map_size, 512, 8192);
            // make sure we actually make effective use of all csm stages
            shadowtune.range = std::max(
            ...
            // color of the ui text. NOTE: will be obsolete once the real ui is
            // in place
            Parser.getTokens(3, false);
        ...
        if (token == "screenshotsdir") {
            ...
            Parser >> uart_conf.mainbrakemin >> uart_conf.mainbrakemax >>
                ...
                uart_conf.tankmax >> uart_conf.tankuart >> uart_conf.pipemax >>
                uart_conf.pipeuart >> uart_conf.brakemax >>
                uart_conf.brakeuart >> uart_conf.pantographmax >>
                uart_conf.pantographuart >> uart_conf.hvmax >>
                uart_conf.hvuart >> uart_conf.currentmax >>
                uart_conf.currentuart >> uart_conf.lvmax >> uart_conf.lvuart;
        } else if (token == "uarttachoscale") {
            Parser.getTokens(1);
            Parser >> uart_conf.tachoscale;
        } else if (token == "uartfeature") {
            Parser.getTokens(4);
            Parser >> uart_conf.mainenable >> uart_conf.scndenable >>
                uart_conf.trainenable >> uart_conf.localenable;
            ...
            Parser >> conf.projection.pa.x >> conf.projection.pa.y >>
                conf.projection.pa.z;
            Parser >> conf.projection.pb.x >> conf.projection.pb.y >>
                conf.projection.pb.z;
            Parser >> conf.projection.pc.x >> conf.projection.pc.y >>
                conf.projection.pc.z;
            Parser >> conf.projection.pe.x >> conf.projection.pe.y >>
                conf.projection.pe.z;
            ...
            Parser >> headtrack_conf.move_axes[0] >>
                headtrack_conf.move_axes[1] >> headtrack_conf.move_axes[2];
            Parser >> headtrack_conf.move_mul[0] >>
                headtrack_conf.move_mul[1] >> headtrack_conf.move_mul[2];
            Parser >> headtrack_conf.rot_axes[0] >>
                headtrack_conf.rot_axes[1] >> headtrack_conf.rot_axes[2];
            Parser >> headtrack_conf.rot_mul[0] >> headtrack_conf.rot_mul[1] >>
                headtrack_conf.rot_mul[2];
            ...
            Parser >> exec_on_exit;
            std::replace(
                std::begin(exec_on_exit), std::end(exec_on_exit), '_', ' ');
    ...
    {  // tutaj wyłączenie, bo mogą nie być zdefiniowane w INI
        ...
        bLiveTraction =
            false;  // false = pantografy zawsze zbierają 95% MaxVoltage
    }
    if (vr) {
    ...
    /*
        fFpsMin = fFpsAverage -
                  fFpsDeviation; // dolna granica FPS, przy której promień
       scenerii będzie zmniejszany fFpsMax = fFpsAverage + fFpsDeviation; //
       górna granica FPS, przy której promień scenerii będzie zwiększany
    */
    if (iPause) iTextMode = GLFW_KEY_F1;  // jak pauza, to pokazać zegar

#ifdef _WIN32
    Console::ModeSet(
        iFeedbackMode, iFeedbackPort);  // tryb pracy konsoli sterowniczej
#endif
}

bool global_settings::ConfigParse_gfx(
    cParser &Parser, std::string_view const Token) {
    ...
    if (Token == "gfx.shadows.cab.range") {
        ...
        // variable internally uses negative values, but is presented as
        // positive in settings so it's likely it'll be supplied as positive
        // number by external launcher
...
void global_settings::export_as_text(std::ostream &Output) const {
    ...
    // NOTE: values are changed dynamically during simulation. cache initial
    // settings and export instead
    if (FreeFlyModeFlag) {
        Output << "freefly yes " << FreeCameraInit[0].x << " "
               << FreeCameraInit[0].y << " " << FreeCameraInit[0].z << "\n";
    } else {
    ...
    export_as_text(
        Output, "sound.volume.positional", EnvironmentPositionalVolume);
    ...
    Output << "mousescale " << fMouseXScale << " " << fMouseYScale << "\n";
    ...
    Output << "shadowtune " << shadowtune.map_size << " " << 0 << " "
           << shadowtune.range << " " << 0 << "\n";
    ...
    export_as_text(
        Output, "gfx.reflections.framerate",
        1.0 / reflectiontune.update_interval);
    ...
    for (auto idx = 0; idx < 6; ++idx) {
        Output << "calibrate5din " << idx << " " << fCalibrateIn[idx][0] << " "
               << fCalibrateIn[idx][1] << " " << fCalibrateIn[idx][2] << " "
               << fCalibrateIn[idx][3] << " " << fCalibrateIn[idx][4] << " "
               << fCalibrateIn[idx][5] << "\n";
    }
    for (auto idx = 0; idx < 6; ++idx) {
        Output << "calibrate5dout " << idx << " " << fCalibrateOut[idx][0]
               << " " << fCalibrateOut[idx][1] << " " << fCalibrateOut[idx][2]
               << " " << fCalibrateOut[idx][3] << " " << fCalibrateOut[idx][4]
               << " " << fCalibrateOut[idx][5] << "\n";
    }
    Output << "calibrateoutmaxvalues " << fCalibrateOutMax[0] << " "
           << fCalibrateOutMax[1] << " " << fCalibrateOutMax[2] << " "
           << fCalibrateOutMax[3] << " " << fCalibrateOutMax[4] << " "
           << fCalibrateOutMax[5] << " " << fCalibrateOutMax[6] << "\n";
    export_as_text(Output, "calibrateoutdebuginfo", iCalibrateOutDebugInfo);
    for (auto idx = 0; idx < 7; ++idx) {
        Output << "pwm " << idx << " " << iPoKeysPWM[idx] << "\n";
    }
    ...
    Output << "uitextcolor " << UITextColor.r * 255 << " "
           << UITextColor.g * 255 << " " << UITextColor.b * 255 << "\n";
    export_as_text(Output, "ui.bg.opacity", UIBgOpacity);
    export_as_text(Output, "input.gamepad", InputGamepad);
#ifdef WITH_UART
    if (uart_conf.enable) {
        Output << "uart " << uart_conf.port << " " << uart_conf.baud << " "
               << uart_conf.updatetime << "\n";
    }
    Output << "uarttune " << uart_conf.mainbrakemin << " "
           << uart_conf.mainbrakemax << " " << uart_conf.localbrakemin << " "
           << uart_conf.localbrakemax << " " << uart_conf.tankmax << " "
           << uart_conf.tankuart << " " << uart_conf.pipemax << " "
           << uart_conf.pipeuart << " " << uart_conf.brakemax << " "
           << uart_conf.brakeuart << " " << uart_conf.pantographmax << " "
           << uart_conf.pipemax << " " << uart_conf.hvmax << " "
           << uart_conf.hvuart << " " << uart_conf.currentmax << " "
           << uart_conf.currentuart << " " << uart_conf.lvmax << " "
           << uart_conf.lvuart << "\n";
    export_as_text(Output, "uarttachoscale", uart_conf.tachoscale);
    Output << "uartfeature " << uart_conf.mainenable << " "
           << uart_conf.scndenable << " " << uart_conf.trainenable << " "
    ...
    Output << "extcam.res " << extcam_res.x << " " << extcam_res.y << "\n";
    ...
    export_as_text(
        Output, "gfx.postfx.motionblur.enabled", gfx_postfx_motionblur_enabled);
    export_as_text(
        Output, "gfx.postfx.motionblur.shutter", gfx_postfx_motionblur_shutter);
    // TODO: export gfx_postfx_motionblur_format
    export_as_text(
        Output, "gfx.postfx.chromaticaberration.enabled",
        gfx_postfx_chromaticaberration_enabled);
    ...
    if (network_client) {
        Output << "network.client " << network_client->first << " "
               << network_client->second << "\n";
    }
    export_as_text(Output, "execonexit", exec_on_exit);
}

template <>
void global_settings::export_as_text(
    std::ostream &Output, std::string const Key,
...
template <>
void global_settings::export_as_text(
    std::ostream &Output, std::string const Key, bool const &Value) const {
