...
// helper, locates submodel with specified name in specified 3d model; returns:
// pointer to the submodel, or null
TSubModel *GetSubmodelFromName(TModel3d *const Model, std::string const Name) {
...
//---------------------------------------------------------------------------
void TAnimPant::AKP_4E() {  // ustawienie wymiarów dla pantografu AKP-4E
    vPos = Math3D::vector3(
        0, 0, 0);    // przypisanie domyśnych współczynników do pantografów
    fLenL1 = 1.22;   // 1.176289 w modelach
    fLenU1 = 1.755;  // 1.724482197 w modelach
    fHoriz =
        0.535;  // 0.54555075 przesunięcie ślizgu w długości pojazdu względem
    // osi obrotu dolnego
    // ramienia
    ...
    PantWys = fLenL1 * sin(fAngleL) + fLenU1 * sin(fAngleU) +
              fHeight;  // wysokość początkowa
...
//---------------------------------------------------------------------------
int TAnim::TypeSet(int i, int fl) {  // ustawienie typu animacji i zależnej od
                                     // niego ilości animowanych submodeli
    fMaxDist = -1.0;                 // normalnie nie pokazywać
    switch (
        i) {  // maska 0x000F: ile używa wskaźników na submodele (0 gdy jeden,
        // wtedy bez tablicy)
        // maska 0x00F0:
...
TAnim::~TAnim() {  // usuwanie animacji
    switch (iFlags &
            0xF0) {  // usuwanie struktur, zależnie ile zostało stworzonych
...
void material_data::assign(std::string const &Replacableskin) {
    ...
    if (contains(Replacableskin, '|')) {
        ...
        nameparser >> texturename;
        while ((texturename != "") && (skinindex < 4)) {
            replacable_skins[skinindex + 1] =
                GfxRenderer->Fetch_Material(texturename);
            ++skinindex;
            texturename = "";
            nameparser >> texturename;
        }
        ...
        int skinindex = 0;
        do {
            // test quietly for file existence so we don't generate tons of
            // false errors in the log NOTE: this means actual missing files
            // won't get reported which is hardly ideal, but still somewhat
            // better
            auto const material{TextureTest(
                ToLower(Replacableskin + "," + std::to_string(skinindex + 1)))};
            if (true == material.empty()) {
                break;
            }

            replacable_skins[skinindex + 1] =
                GfxRenderer->Fetch_Material(material);
            ++skinindex;
        } while (skinindex < 4);
        multi_textures = skinindex;
        if (multi_textures == 0) {
    ...
    // BUGS! it's not entierly designed whether opacity is property of material
    // or submodel, and code does confusing things with this in various places
    textures_alpha =
        (GfxRenderer->Material(replacable_skins[1]).is_translucent()
             ? 0x31310031
             :  // tekstura -1 z kanałem alfa - nie renderować w cyklu
                // nieprzezroczystych
             0x30300030);  // wszystkie tekstury nieprzezroczyste - nie
                           // renderować w cyklu przezroczystych
    if (GfxRenderer->Material(replacable_skins[2]).is_translucent()) {
        // tekstura -2 z kanałem alfa - nie renderować w cyklu
        // nieprzezroczystych
        textures_alpha |= 0x02020002;
    }
    if (GfxRenderer->Material(replacable_skins[3]).is_translucent()) {
        // tekstura -3 z kanałem alfa - nie renderować w cyklu
        // nieprzezroczystych
        textures_alpha |= 0x04040004;
    }
    if (GfxRenderer->Material(replacable_skins[4]).is_translucent()) {
        // tekstura -4 z kanałem alfa - nie renderować w cyklu
        // nieprzezroczystych
...
//---------------------------------------------------------------------------
TDynamicObject *TDynamicObject::FirstFind(
    int &coupler_nr,
    int cf) {  // szukanie skrajnego połączonego pojazdu w pociagu
    // od strony sprzegu (coupler_nr) obiektu (start)
    TDynamicObject *temp = this;
    for (int i = 0; i < 300;
         i++)  // ograniczenie do 300 na wypadek zapętlenia składu
    {
        if (!temp)
            return NULL;  // Ra: zabezpieczenie przed ewentaulnymi błędami
                          // sprzęgów
        if ((temp->MoverParameters->Couplers[coupler_nr].CouplingFlag & cf) !=
            cf)
            return temp;  // nic nie ma już dalej podłączone sprzęgiem cf
        if (coupler_nr == end::front) {  // jeżeli szukamy od sprzęgu 0
            if (temp->PrevConnected())   // jeśli mamy coś z przodu
            {
                if (temp->PrevConnectedNo() ==
                    end::front)  // jeśli pojazd od strony sprzęgu 0 jest
                                 // odwrócony
                    coupler_nr =
                        1 - coupler_nr;        // to zmieniamy kierunek sprzęgu
                temp = temp->PrevConnected();  // ten jest od strony 0
            } else
                return temp;  // jeśli jednak z przodu nic nie ma
        } else {
            if (temp->NextConnected()) {
                if (temp->NextConnectedNo() ==
                    end::rear)  // jeśli pojazd od strony sprzęgu 1 jest
                                // odwrócony
                    coupler_nr =
                        1 - coupler_nr;        // to zmieniamy kierunek sprzęgu
...
//---------------------------------------------------------------------------
float TDynamicObject::GetEPP() {  // szukanie skrajnego połączonego pojazdu w
                                  // pociagu
    // od strony sprzegu (coupler_nr) obiektu (start)
    TDynamicObject *temp = this;
    int coupler_nr = 0;
         ...
         ++i)  // ograniczenie do 300 na wypadek zapętlenia składu
    {
        ...
        if ((temp->MoverParameters->Couplers[coupler_nr].CouplingFlag &
             coupling::brakehose) != coupling::brakehose)
            break;                      // nic nie ma już dalej podłączone
        if (coupler_nr == 0) {          // jeżeli szukamy od sprzęgu 0
            if (temp->PrevConnected())  // jeśli mamy coś z przodu
            {
                if (temp->PrevConnectedNo() ==
                    end::front)  // jeśli pojazd od strony sprzęgu 0 jest
...
//---------------------------------------------------------------------------
TDynamicObject *TDynamicObject::GetFirstDynamic(
    int cpl_type, int cf) {  // Szukanie skrajnego połączonego pojazdu w pociagu
...
int TDynamicObject::GetPneumatic(bool front, bool red) {
    ...
    z = 0;                           // brak węży?
    if ((x > 0) && (y > 0)) z = 3;   // dwa
    if ((x > 0) && (y == 0)) z = 1;  // lewy, brak prawego
    if ((x == 0) && (y > 0)) z = 2;  // brak lewego, prawy

    return z;
}

void TDynamicObject::SetPneumatic(bool front, bool red) {
    int x = 0, ten = 0, tamten = 0;
    ten = GetPneumatic(front, red);  // 1=lewy skos,2=prawy skos,3=dwa proste
    if (front)
        if (PrevConnected())  // pojazd od strony sprzęgu 0
            tamten = PrevConnected()->GetPneumatic(
                (PrevConnectedNo() == end::front ? true : false), red);
    if (!front)
        if (NextConnected())  // pojazd od strony sprzęgu 1
            tamten = NextConnected()->GetPneumatic(
                (NextConnectedNo() == end::front ? true : false), red);
    if (ten == tamten)  // jeśli układ jest symetryczny
        switch (ten) {
            ...
            case 3:     // wszystkie cztery na prosto
                if (MoverParameters->Couplers[front ? end::front : end::rear]
                        .Render)
                    x = 1;
                else
                    x = 4;
...
void TDynamicObject::UpdateAxle(TAnim *pAnim) {  // animacja osi
    size_t wheel_id = pAnim->dWheelAngle;
    pAnim->smAnimated->SetRotate(float3(1, 0, 0), dWheelAngle[wheel_id]);
    pAnim->smAnimated->future_transform = glm::rotate(
        (float)glm::radians(m_future_wheels_angle[wheel_id]),
        glm::vec3(1.0f, 0.0f, 0.0f));
};

// animacja drzwi - przesuw
void TDynamicObject::UpdateDoorTranslate(TAnim *pAnim) {
    if (pAnim->smAnimated == nullptr) {
        return;
    }

    auto const &door{
        MoverParameters->Doors
            .instances[((pAnim->iNumber & 1) == 0 ? side::right : side::left)]};

    pAnim->smAnimated->SetTranslate(Math3D::vector3{0.0, 0.0, door.position});
};
...
void TDynamicObject::UpdateDoorRotate(TAnim *pAnim) {
    ...
    pAnim->smAnimated->SetRotate(float3(1, 0, 0), door.position);
};
...
void TDynamicObject::UpdateDoorFold(TAnim *pAnim) {
    ...
    // skrzydło mniejsze
    pAnim->smAnimated->SetRotate(float3(0, 0, 1), door.position);
    ...
    sm->SetRotate(float3(0, 0, 1), -door.position - door.position);
    ...
    sm->SetRotate(float3(0, 1, 0), door.position);
};
...
void TDynamicObject::UpdateDoorPlug(TAnim *pAnim) {
    ...
    pAnim->smAnimated->SetTranslate(Math3D::vector3{
        std::min(door.position * 2.f, MoverParameters->Doors.range_out), 0.0,
        std::max(
            0.f, door.position - MoverParameters->Doors.range_out * 0.5f)});
}

void TDynamicObject::UpdatePant(
    TAnim *pAnim) {  // animacja pantografu - 4 obracane ramiona, ślizg piąty
    float a, b, c;
...
void TDynamicObject::UpdatePlatformRotate(TAnim *pAnim) {
    ...
    pAnim->smAnimated->SetRotate(
        float3(0, 1, 0),
        interpolate(
            0.f, MoverParameters->Doors.step_range, door.step_position));
}
...
void TDynamicObject::UpdateMirror(TAnim *pAnim) {
    ...
    // only animate the mirror if it's located on the same end of the vehicle as
    // the active cab
    auto const isactive{
        (MoverParameters->CabOccupied > 0
             ? ((pAnim->iNumber >> 4) == end::front ? 1.0 : 0.0)
         : MoverParameters->CabOccupied < 0
             ? ((pAnim->iNumber >> 4) == end::rear ? 1.0 : 0.0)
             : 0.0)};
    ...
    if (pAnim->iNumber & 1)
        pAnim->smAnimated->SetRotate(
            float3(0, 1, 0),
            interpolate(
                0.0, MoverParameters->MirrorMaxShift, dMirrorMoveR * isactive));
    else
        pAnim->smAnimated->SetRotate(
            float3(0, 1, 0),
            interpolate(
                0.0, MoverParameters->MirrorMaxShift, dMirrorMoveL * isactive));
}
...
{ // ustawienie kąta na
    ...
    pAnim->smAnimated->SetRotate(float3(1, 0, 0),
pAnim->fParam[*pAnim->iIntBase]);
};
*/
// sets light levels for registered interior sections
void TDynamicObject::toggle_lights() {
    if (true == SectionLightsActive) {
        ...
        // set lights with probability depending on the compartment type. TODO:
        // expose this in .mmd file
        for (auto &section : Sections) {
            auto const sectionname{section.compartment->pName};
...
void TDynamicObject::set_cab_lights(int const Cab, float const Level) {
    for (auto &section : Sections) {
        // cab compartments are placed at the beginning of the list, so we can
        // bail out as soon as we find different compartment type
        auto const sectionname{section.compartment->pName};
        if (sectionname.size() < 4) {
            return;
        }
        if (sectionname.find("cab") != 0) {
            return;
        }
        if (sectionname[3] != Cab + '0') {
            continue;
        }  // match the cab with correct index
...
void TDynamicObject::ABuLittleUpdate(
    double ObjSqrDist) {  // ABu290105: pozbierane i uporzadkowane powtarzajace
    ...
    if (ObjSqrDist < (400 * 400))  // gdy bliżej niż 400m
    {
        for (auto &animation : pAnimations) {
            ...
            if ((ObjSqrDist < animation.fMaxDist) && (animation.yUpdate)) {
                // jeśli zdefiniowana funkcja aktualizacja animacji (położenia
                // submodeli
                animation.yUpdate(&animation);
            }
        }

        if ((mdModel != nullptr) && (ObjSqrDist < (50 * 50))) {
            // gdy bliżej niż 50m
            ...
            // ABu-240105: Dodatkowy warunek: if (...).Render, zeby rysowal
            // tylko jeden z polaczonych sprzegow display _on if connected with
            // another vehicle and the coupling owner (render flag) display _xon
            // if connected with another vehicle and not the coupling owner
            // display _xon if not connected, but equipped with coupling adapter
            // display _off if not connected, not equipped with coupling adapter
            // or if _xon model is missing
            if (TestFlag(
                    MoverParameters->Couplers[end::front].CouplingFlag,
                    coupling::coupler)) {
                ...
                if (true ==
                    MoverParameters->Couplers[end::front].has_adapter()) {
                    btCoupler1.TurnxOnWithOffAsFallback();
                    btnOn = true;
                }
            }
            if (TestFlag(
                    MoverParameters->Couplers[end::rear].CouplingFlag,
                    coupling::coupler)) {
                ...
                if (true ==
                    MoverParameters->Couplers[end::rear].has_adapter()) {
                    btCoupler2.TurnxOnWithOffAsFallback();
                    btnOn = true;
                }
            }
            //********************************************************************************
            // przewody powietrzne j.w., ABu: decyzja czy rysowac tylko na
            // podstawie 'render' - juz nie przewody powietrzne, yB: decyzja na
            // podstawie polaczen w t3d
            if (Global.bnewAirCouplers) {
                SetPneumatic(
                    false, false);  // wczytywanie z t3d ulozenia wezykow
            ...
            else  // po staremu ABu'oewmu
            {
                ...
                // //yB - zasilajace
                if (TestFlag(
                        MoverParameters->Couplers[0].CouplingFlag,
                        ctrain_scndpneumatic)) {
                ...
                if (TestFlag(
                        MoverParameters->Couplers[1].CouplingFlag,
                        ctrain_scndpneumatic)) {
            ...
            for (int i = 0; i < 2; ++i) {
                if (MoverParameters->Couplers[i].has_adapter()) {
                    // HACK: if there's coupler adapter on this side, we presume
                    // there's additional distance put between vehicles which
                    // prevents buffers from clashing against each other (or the
                    // other vehicle doesn't have buffers to begin with)
                    continue;
                }

                auto const dist{clamp(
                    MoverParameters->Couplers[i].Dist / 2.0,
                    -MoverParameters->Couplers[i].DmaxB, 0.0)};
        ...
        if (TestFlag(
                MoverParameters->Couplers[0].CouplingFlag, coupling::control)) {
            btCCtrl1.Turn(true);
            btnOn = true;
        }
        // else btCCtrl1.TurnOff();
        if (TestFlag(
                MoverParameters->Couplers[1].CouplingFlag, coupling::control)) {
            btCCtrl2.Turn(true);
            btnOn = true;
        }
        ...
        if (TestFlag(
                MoverParameters->Couplers[0].CouplingFlag, ctrain_passenger)) {
            btCPass1.Turn(true);
            btnOn = true;
        }
        // else btCPass1.TurnOff();
        if (TestFlag(
                MoverParameters->Couplers[1].CouplingFlag, ctrain_passenger)) {
            btCPass2.Turn(true);
            btnOn = true;
        }
        // else btCPass2.TurnOff();
        if (MoverParameters->Power24vIsAvailable ||
            MoverParameters->Power110vIsAvailable) {  // sygnaly konca pociagu
            if (m_endsignals1.Active()) {
                if (TestFlag(
                        MoverParameters->iLights[end::front],
                ...
                if (TestFlag(
                        MoverParameters->iLights[end::front],
                        light::redmarker_left)) {
                    m_endsignal13.Turn(true);
                    btnOn = true;
                }
                if (TestFlag(
                        MoverParameters->iLights[end::front],
                        light::redmarker_right)) {
            ...
            if (m_endsignals2.Active()) {
                if (TestFlag(
                        MoverParameters->iLights[end::rear],
                        (light::redmarker_left | light::redmarker_right))) {
        ...
        // tablice blaszane:
        if (TestFlag(
                MoverParameters->iLights[end::front], light::rearendsignals)) {
            m_endtab1.Turn(true);
            btnOn = true;
        }
        // else btEndSignalsTab1.TurnOff();
        if (TestFlag(
                MoverParameters->iLights[end::rear], light::rearendsignals)) {
            m_endtab2.Turn(true);
            btnOn = true;
        }
        // destination signs
        ...
        if ((false == bDisplayCab)  // edge case, lowpoly may act as a stand-in
                                    // for the hi-fi cab, so make sure not to
                                    // show the driver when inside
            && (Mechanik != nullptr) &&
    ...
    if (MoverParameters->Power24vIsAvailable ||
        MoverParameters->Power110vIsAvailable) {  // sygnały czoła pociagu //Ra:
                                                  // wyświetlamy bez
        // ograniczeń odległości, by były widoczne z
        // daleka
        if (TestFlag(
                MoverParameters->iLights[end::front], light::headlight_left)) {
            if (DimHeadlights) {
        ...
        if (TestFlag(
                MoverParameters->iLights[end::front], light::headlight_upper)) {
            if (DimHeadlights) {
        ...
        if (TestFlag(
                MoverParameters->iLights[end::front], light::headlight_right)) {
            if (DimHeadlights) {
        ...
        if (TestFlag(
                MoverParameters->iLights[end::rear], light::headlight_left)) {
            if (DimHeadlights) {
        ...
        if (TestFlag(
                MoverParameters->iLights[end::rear], light::headlight_upper)) {
            if (DimHeadlights) {
        ...
        if (TestFlag(
                MoverParameters->iLights[end::rear], light::headlight_right)) {
            if (DimHeadlights) {
        ...
        // auxiliary lights
        if (TestFlag(
                MoverParameters->iLights[end::front], light::auxiliary_left)) {
            if (DimHeadlights) {
        ...
        if (TestFlag(
                MoverParameters->iLights[end::front], light::auxiliary_right)) {
            if (DimHeadlights) {
        ...
        if (TestFlag(
                MoverParameters->iLights[end::rear], light::auxiliary_left)) {
            if (DimHeadlights) {
        ...
        if (TestFlag(
                MoverParameters->iLights[end::rear], light::auxiliary_right)) {
            if (DimHeadlights) {
    ...
    for (auto const &section : Sections) {
        if (cabsection) {
            ...
            cabsection &=
                ((sectionname.size() >= 4) &&
                 (starts_with(sectionname, "cab")));
        }
        // TODO: add cablight devices
        auto const sectionlightlevel{
            section.light_level *
            (cabsection ? 1.0f : MoverParameters->CompartmentLights.intensity)};
        sectionlightcolor = glm::vec4(
            (((sectionlightlevel == 0.f) ||
              (Global.fLuminance > section.compartment->fLight))
                 ? glm::vec3(240.f / 255.f)
                 :  // TBD: save and restore initial submodel diffuse instead of
                    // enforcing one?
    ...
    for (auto const &section : Sections) {
        ...
        // if the section root isn't visible we can skip meddling with its
        // children
        if (false == section.load->iVisible) {
            continue;
        }
    ...
    for (int cabidx = 0; cabidx < LowPolyIntCabs.size(); ++cabidx) {
        if (LowPolyIntCabs[cabidx] == nullptr) {
            continue;
        }
        LowPolyIntCabs[cabidx]->iVisible =
            (mdKabina == nullptr ? true :  // there's no hi-fi cab
                 bDisplayCab == false ? true
                                      :  // we're in external view
                 simulation::Train == nullptr
                 ? true
                 :  // not a player-driven vehicle, implies external view
                 simulation::Train->Dynamic() != this
                 ? true
                 :  // not a player-driven vehicle, implies external view
                 JointCabs ? false
                           :  // internal view, all cabs share the model so hide
                              // them 'all'
                 (simulation::Train->iCabn !=
                  cabidx));  // internal view, hide occupied cab and show others
    }
}
// ABu 29.01.05 koniec przeklejenia *************************************

TDynamicObject *TDynamicObject::ABuFindNearestObject(
    glm::vec3 pos, TTrack *Track, TDynamicObject *MyPointer, int &CouplNr) {
    // zwraca wskaznik do obiektu znajdujacego sie na torze (Track), którego
    // sprzęg jest najblizszy punktowi
...
TDynamicObject *TDynamicObject::ABuScanNearestObject(
    glm::vec3 pos, TTrack *Track, double ScanDir, double ScanDist,
    int &CouplNr) {  // skanowanie toru w poszukiwaniu obiektu najblizszego
                     // punktowi
    if (ABuGetDirection() < 0) ScanDir = -ScanDir;
    TDynamicObject *FoundedObj;
    FoundedObj = ABuFindNearestObject(
        pos, Track, this,
        CouplNr);  // zwraca numer sprzęgu znalezionego pojazdu
    if (FoundedObj == NULL) {
        ...
        while (ActDist < ScanDist) {
            ...
            if (Track != NULL) {  // jesli jest kolejny odcinek toru
                CurrDist = Track->Length();
                FoundedObj = ABuFindNearestObject(pos, Track, this, CouplNr);
                if (FoundedObj != NULL) ActDist = ScanDist;
...
void TDynamicObject::ABuModelRoll() {  // ustawienie przechyłki pojazdu i jego
                                       // zawartości
                                       // Ra: przechyłkę załatwiamy na etapie
                                       // przesuwania modelu
}

// ABu 06.05.04 poczatek wyliczania obrotow wozkow **********************

void TDynamicObject::ABuBogies() {  // Obracanie wozkow na zakretach. Na razie
    // uwzględnia tylko zakręty,
    // bez zadnych gorek i innych przeszkod.
    if ((smBogie[0] != NULL) && (smBogie[1] != NULL)) {
        // modelRot.z=ABuAcos(Axle0.pPosition-Axle1.pPosition); //kąt obrotu
        // pojazdu [rad] bogieRot[0].z=ABuAcos(Axle0.pPosition-Axle3.pPosition);
        bogieRot[0].z = Axle0.vAngles.z;
        bogieRot[0] =
            RadToDeg(modelRot - bogieRot[0]);  // mnożenie wektora przez stałą
...
void TDynamicObject::ABuCheckMyTrack() {  // Funkcja przypisujaca obiekt
...
TDynamicObject *TDynamicObject::ABuFindObject(
    int &Foundcoupler, double &Distance, TTrack const *Track,
    int const Direction, int const Mycoupler)
    const {  // Zwraca wskaźnik najbliższego obiektu znajdującego się
    ...
    //    Direction - kierunek szukania na torze (+1:w stronę Point2, -1:w
    //    stronę Point1) Mycoupler - nr sprzegu obiektu szukajacego;
    // WY:
    ...
    if (true == Track->Dynamics.empty()) {
        ...
        Distance += Track->Length();  // doliczenie długości odcinka do
                                      // przeskanowanej odległości
        return nullptr;               // nie ma pojazdów na torze, to jest NULL
    }

    double distance = Track->Length();  // najmniejsza znaleziona odleglość
    ...
    if (MyTrack == Track) {
        ...
        myposition =
            RaTranslationGet();  // położenie wózka względem Point1 toru
    } else {
    ...
    double objectposition;  // robocza odległość od kolejnych pojazdów na danym
                            // odcinku
    for (auto dynamic : Track->Dynamics) {
        if (dynamic == this) {
            continue;
           ...
           /*
                   if( ( ( dynamic == PrevConnected ) && (
              MoverParameters->Couplers[ TMoverParameters::side::front
              ].CouplingFlag != coupling::faux ) )
                    || ( ( dynamic == NextConnected ) && (
              MoverParameters->Couplers[ TMoverParameters::side::rear
              ].CouplingFlag != coupling::faux ) )  ){
                       // stop-gap check to prevent 'detection' of attached
              vehicles
                       // which seems to be a side-effect of inaccurate location
              calculation in 'simple' mode?
        ...
        if (Direction > 0) {
            ...
            objectposition = (dynamic->RaTranslationGet()) -
                             myposition;  // odległogłość tamtego od szukającego
        ...
        if (Track->iCategoryFlag & 254) {
            ...
            // szynie i się mija Ra: mijanie samochodów wcale nie jest proste
            ...
            if (Foundcoupler != Mycoupler) {
                ...
                relativeoffset = std::abs(
                    MoverParameters->OffsetTrackH -
                    dynamic->MoverParameters->OffsetTrackH);
            } else {
                relativeoffset = std::abs(
                    MoverParameters->OffsetTrackH +
                    dynamic->MoverParameters->OffsetTrackH);
            }
            if (relativeoffset + relativeoffset >
                MoverParameters->Dim.W + dynamic->MoverParameters->Dim.W) {
                // odległość większa od połowy sumy szerokości - kolizji nie
                // będzie
                continue;
            }
        ...
        distance =
            objectposition;  // odleglość pomiędzy aktywnymi osiami pojazdów
    }

    Distance += distance;  // doliczenie odległości przeszkody albo długości
                           // odcinka do przeskanowanej odległości
    return foundobject;
}

int TDynamicObject::DettachStatus(int dir) {  // sprawdzenie odległości sprzęgów
    ...
    return (MoverParameters->DettachStatus(
        dir));  // czy jest w odpowiedniej odległości?
}

int TDynamicObject::Dettach(int dir) {  // rozłączenie sprzęgów rzeczywistych od
    // strony (dir): 0=przód,1=tył
    // zwraca maskę bitową aktualnych sprzegów (0 jeśli rozłączony)
    if (ctOwner) {  // jeśli pojazd ma przypisany obiekt nadzorujący skład, to
                    // póki
        // są wskaźniki
    ...
    if (MoverParameters->Couplers[dir].CouplingFlag) {
        ...
        /*
                if( true == MoverParameters->Dettach( dir ) ) {
                    ...
                    auto const othercoupler { dir ? NextConnectedNo() :
           PrevConnectedNo() }; ( othercoupler ? othervehicle->NextConnected :
           othervehicle->PrevConnected ) = nullptr; ( dir ? NextConnected :
...
void TDynamicObject::couple(int const Side) {
    ...
    auto const &othercoupler{
        othervehicleparams->Couplers[neighbour.vehicle_end]};

    if (coupler.CouplingFlag == coupling::faux) {
        // najpierw hak
        if ((coupler.AllowedFlag & othercoupler.AllowedFlag &
             coupling::coupler) == coupling::coupler) {
            if (MoverParameters->Attach(
                    Side, neighbour.vehicle_end, othervehicleparams,
                    coupling::coupler)) {
    ...
    if (false == TestFlag(
                     MoverParameters->Couplers[Side].CouplingFlag,
                     coupling::brakehose)) {
        // pneumatyka
        if ((coupler.AllowedFlag & othercoupler.AllowedFlag &
             coupling::brakehose) == coupling::brakehose) {
...
int TDynamicObject::uncouple(int const Side) {
...
bool TDynamicObject::attach_coupler_adapter(
    int const Side, bool const Enforce) {
    ...
    // TBD: empty struct instead of fallback defaults, to allow vehicles without
    // adapter?
    auto adapterdata{coupleradapter_data{{0.085f, 0.95f}, "tabor/polsprzeg"}};
    ...
    if ((MoverParameters->Neighbours[Side].distance - adapterdata.position.x <
         0.5) &&
        (false == Enforce)) {
...
bool TDynamicObject::remove_coupler_adapter(int const Side) {
    auto &coupler{MoverParameters->Couplers[Side]};
    ...
    if (coupler.Connected != nullptr) {
        // TBD: disallow instead adapter removal if it's coupled with another
        // vehicle?
        uncouple(Side);
    }
...
TDynamicObject::TDynamicObject() {
    ...
    //    NextConnectedNo = PrevConnectedNo = 2; // ABu: Numery sprzegow. 2=nie
    //    podłączony
    bEnabled = true;
    MyTrack = NULL;
    // McZapkie-260202
    ...
    // Ra: domyślne ilości animacji dla zgodności wstecz (gdy brak ilości
    // podanych w MMD) ustawienie liczby modeli animowanych podczas
    // konstruowania obiektu a nie na 0 prowadzi prosto do wysypów jeśli źle
    // zdefiniowane mmd
...
void TDynamicObject::place_on_track(
    TTrack *Track, double fDist, bool Reversed) {
    for (auto &axle : m_axlesounds) {
        // wyszukiwanie osi (0 jest na końcu, dlatego dodajemy długość?)
        axle.distance =
            (Reversed ? -axle.offset : (axle.offset + MoverParameters->Dim.L)) +
            fDist;
    }
    ...
    fDist -=
        0.5 *
        MoverParameters->Dim.L;  // dodajemy pół długości pojazdu, bo ustawiamy
                                 // jego środek (zliczanie na minus)
    switch (iNumAxles) {
        ...
        case 2:  // ustawianie osi na torze
            ...
            Axle1.Reset();
            Axle1.Move(
                (iDirection ? fDist : -fDist) - fAxleDistHalf,
                false);  // false, żeby nie generować eventów
            break;
    }
    // potrzebne do wyliczenia aktualnej pozycji; nie może być zero, bo nie
    // przeliczy pozycji teraz jeszcze trzeba przypisać pojazdy do nowego toru,
    // bo przesuwanie początkowe osi nie zrobiło tego
    Move(0.0001);
...
double TDynamicObject::Init(
    ...
    std::string
        MoreParams  // dodatkowe parametry wczytywane w postaci tekstowej
) {                 // Ustawienie początkowe pojazdu
    iDirection =
        (Reversed ? 0
                  : 1);  // Ra: 0, jeśli ma być wstawiony jako obrócony tyłem
    ...
    MoverParameters =
        new TMoverParameters(iDirection ? fVel : -fVel, Type_Name, asName, Cab);
    // McZapkie: TypeName musi byc nazwą CHK/MMD pojazdu
    if (!MoverParameters->LoadFIZ(
            asBaseDir)) {  // jak wczytanie CHK się nie uda, to błąd
        if (ConversionError == 666)
            ErrorLog(
                "Bad vehicle: failed to locate definition file \"" + BaseDir +
                "/" + Type_Name + ".fiz" + "\"");
        else {
            ErrorLog(
                "Bad vehicle: failed to load definition from file \"" +
                BaseDir + "/" + Type_Name + ".fiz\" (error " +
                to_string(ConversionError) + ")");
        }
    ...
    bool driveractive =
        (fVel != 0.0);  // jeśli prędkość niezerowa, to aktywujemy ruch
    if (!MoverParameters->CheckLocomotiveParameters(
            driveractive,
            (fVel > 0 ? 1 : -1) * Cab *
                (iDirection
                     ? 1
                     : -1)))  // jak jedzie lub obsadzony to gotowy do drogi
    {
        Error(
            "Parameters mismatch: dynamic object " + asName + " from \"" +
            BaseDir + "/" + Type_Name + "\"");
        return 0.0;  // zerowa długość to brak pojazdu
    }
    // controller position
    ...
    if (driveractive) {
        if (Cab == 0)
            MoverParameters->BrakeCtrlPos = static_cast<int>(
                std::floor(MoverParameters->Handle->GetPos(bh_NP)));
        else
            MoverParameters->BrakeCtrlPos = static_cast<int>(
                std::floor(MoverParameters->Handle->GetPos(bh_RP)));

        // engage independent brake if applicable, if the vehicle is to be on
        // standby NOTE: with more than one driver in the consist this is likely
        // to go awery, since all but one will be sent to sleep TBD, TODO: have
        // the virtual helper release independent brakes during consist check?
        if (DriverType != "") {
    ...
    MoreParams +=
        ".";  // wykonuje o jedną iterację za mało, więc trzeba mu dodać
    // kropkę na koniec
    ...
    while (kropka != std::string::npos)    // jesli sa kropki jeszcze
    {
        ...
        MoreParams = MoreParams.substr(
            kropka + 1, dlugosc - kropka);  // reszta do dalszej obrobki
        ...
        {                                // sprawdzanie kolejno nastaw
            ...
            // wylaczanie hamulca
            if (contains(
                    ActPar, "<>"))  // wylaczanie na probe hamowania naglego
            {
                MoverParameters->Hamulec->SetBrakeStatus(
                    MoverParameters->Hamulec->GetBrakeStatus() |
                    b_dmg);  // wylacz
            }
            ...
            if (contains(
                    ActPar,
                    'X'))  // agonalny wylaczanie 20%, usrednienie przekladni
            {
                    ...
                    0.01)  // jesli jest nastawiacz mechaniczny PL
                {
                    ...
                    {
                        MoverParameters->BrakeCylMult[1] =
                            MoverParameters->BrakeCylMult[1] * 0.50;
                        MoverParameters->BrakeCylMult[2] =
                            MoverParameters->BrakeCylMult[2] * 0.75;
                    } else if (rnd < 80)  // losowanie 80/100-70/100    tylko
                                          // prozny
                    {
                        MoverParameters->BrakeCylMult[2] =
                            MoverParameters->BrakeCylMult[1];
        ...
        else if ((ActPar.size() >= 3) && (ActPar.front() == 'W')) {
            // wheel
            ActPar.erase(0, 1);
            ...
            while (false == ActPar.empty()) {
                // TODO: convert this whole copy and paste mess to something
                // more elegant one day
                switch (ActPar.front()) {
                    case 'F': {
                        // fixed flat size
                        auto const indexstart{1};
                        auto const indexend{
                            ActPar.find_first_not_of("1234567890", indexstart)};
                        fixedflatsize = std::atoi(
                            ActPar.substr(indexstart, indexend).c_str());
                    ...
                    case 'H': {
                        ...
                        auto const indexend{
                            ActPar.find_first_not_of("1234567890", indexstart)};
                        auto const huntingchance{std::atoi(
                            ActPar.substr(indexstart, indexend).c_str())};
                        MoverParameters->TruckHunting =
                            (Random(0, 100) < huntingchance);
                        ActPar.erase(0, indexend);
                        break;
                    }
                    default: {
            ...
            if (Random(0, 100) <= flatchance) {
                MoverParameters->WheelFlat +=
                    fixedflatsize + Random(0, randomflatsize);
            }
        }  // wheel
        else if ((ActPar.size() >= 2) && (ActPar.front() == 'T')) {
            // temperature
            ActPar.erase(0, 1);
        ...
        else if ((ActPar.size() >= 2) && (ActPar.front() == 'L')) {
            // load
            ActPar.erase(0, 1);
            ...
            // TBD: make it instead an optional sub-parameter?
            {
                ...
                auto const indexend{
                    ActPar.find_first_not_of("1234567890", indexstart)};
                MoverParameters->MaxLoad =
                    std::atoi(ActPar.substr(indexstart, indexend).c_str());
                ActPar.erase(0, indexend);
            }
            while (false == ActPar.empty()) {
                switch (ActPar.front()) {
           ...
           /*        else if (ActPar.substr(0, 1) == "") // tu mozna wpisac inny
              prefiks i inne rzeczy
                   {
    ...
    {  // ustawianie samochodow na poboczu albo na środku drogi
        if (Track->fTrackWidth < 3.5)  // jeśli droga wąska
            MoverParameters->OffsetTrackH =
                0.0;  // to stawiamy na środku, niezależnie od stanu
        // ruchu
        else if (driveractive) {  // od 3.5m do 8.0m jedzie po środku pasa, dla
            // szerszych w odległości
            // 1.5m
            auto const lanefreespace =
                0.5 * Track->fTrackWidth - MoverParameters->Dim.W;
            MoverParameters->OffsetTrackH =
                (lanefreespace > 1.5
                     ? -0.5 * MoverParameters->Dim.W - 0.75
                     :  // wide road, keep near centre with safe margin
                     (lanefreespace > 0.1 ? -0.25 * Track->fTrackWidth
                                          :  // narrow fit, stay on the middle
                          -0.5 * MoverParameters->Dim.W -
                              0.075));  // too narrow, just keep some minimal
                                        // clearance
    ...
    */
    ...
    iInventory[end::front] |=
        m_endsignal12.Active() ? light::redmarker_right : 0;
    iInventory[end::front] |=
        m_endsignal13.Active() ? light::redmarker_left : 0;
    iInventory[end::rear] |=
        m_endsignal22.Active() ? light::redmarker_right : 0;
    iInventory[end::rear] |= m_endsignal23.Active() ? light::redmarker_left : 0;
    iInventory[end::front] |=
        m_endsignals1.Active()
            ? (light::redmarker_left | light::redmarker_right)
            : 0;
    iInventory[end::rear] |=
        m_endsignals2.Active()
            ? (light::redmarker_left | light::redmarker_right)
            : 0;
    iInventory[end::front] |=
        m_endtab1.Active() ? light::rearendsignals : 0;  // tabliczki blaszane
    iInventory[end::rear] |= m_endtab2.Active() ? light::rearendsignals : 0;
    iInventory[end::front] |=
        m_headlamp11.Active() ? light::headlight_upper : 0;
    iInventory[end::front] |=
        m_headlamp12.Active() ? light::headlight_right : 0;
    ...
    iInventory[end::front] |=
        m_headsignal12.Active() ? light::auxiliary_right : 0;
    iInventory[end::front] |=
        m_headsignal13.Active() ? light::auxiliary_left : 0;
    iInventory[end::rear] |=
        m_headsignal22.Active() ? light::auxiliary_right : 0;
    iInventory[end::rear] |=
        m_headsignal23.Active() ? light::auxiliary_left : 0;
    ...
    if (mdLowPolyInt != nullptr) {
        // check the low poly interior for potential compartments of interest,
        // ie ones which can be individually lit
        ...
        std::vector<std::string> nameprefixes = {
            "corridor", "korytarz", "compartment", "przedzial"};
        for (auto const &nameprefix : nameprefixes) {
            init_sections(
                mdLowPolyInt, nameprefix,
                MoverParameters->CompartmentLights.start_type ==
                    start_t::manual);
        }
    }
    // destination sign
    ...
    // 'external_load' is an optional special section in the main model,
    // pointing to submodel of external load
    if (mdModel) {
    ...
    if (mdModel) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (smBuforPrawy[i]) smBuforPrawy[i]->WillBeAnimated();
        }
    }
    if (Track->fSoundDistance > 0.f) {
        for (auto &axle : m_axlesounds) {
            // wyszukiwanie osi (0 jest na końcu, dlatego dodajemy długość?)
            axle.distance = clamp_circular<double>(
                (Reversed ? -axle.offset : axle.offset) -
                    0.5 * MoverParameters->Dim.L + fDist,
                Track->fSoundDistance);
        }
    }
    // McZapkie-250202 end.
    Track->AddDynamicObject(
        this);  // wstawiamy do toru na pozycję 0, a potem przesuniemy
    ...
    fAxleDist = clamp(
        ...
        MoverParameters->Dim.L -
            0.2);  // nie mogą być za daleko bo będzie "walenie w mur"
    double fAxleDistHalf = fAxleDist * 0.5;
    // przesuwanie pojazdu tak, aby jego początek był we wskazanym miejcu
    fDist -=
        0.5 *
    ...
    switch (iNumAxles) {
        ...
        case 4:
            Axle0.Init(Track, this, iDirection ? 1 : -1);
            Axle0.Move(
                (iDirection ? fDist : -fDist) +
                    (fAxleDistHalf + MoverParameters->ADist * 0.5),
                false);
            Axle1.Init(Track, this, iDirection ? 1 : -1);
            Axle1.Move(
                (iDirection ? fDist : -fDist) -
    ...
    if (mdModel)  // jeśli ma w czym szukać
    {
        ...
        if (!smBogie[0])
            smBogie[0] =
                mdModel->GetFromName("boogie01");  // Ra: alternatywna nazwa
        if (!smBogie[1])
            smBogie[1] =
                mdModel->GetFromName("boogie02");  // Ra: alternatywna nazwa
        if (smBogie[0]) smBogie[0]->WillBeAnimated();
        if (smBogie[1]) smBogie[1]->WillBeAnimated();
    }
...
int TDynamicObject::init_sections(
    TModel3d const *Model, std::string const &Nameprefix,
    bool const Overrideselfillum) {
    auto sectioncount = 0;
    auto sectionindex = 0;
    ...
    do {
        ...
        if ((sectionsubmodel == nullptr) && (sectionindex < 10)) {
            sectionsubmodel =
                Model->GetFromName(Nameprefix + "0" + sectionindexname);
        }
        if (sectionsubmodel != nullptr) {
            // HACK: disable automatic self-illumination threshold, at least
            // until 3d model update
...
bool TDynamicObject::init_destination(TModel3d *Model) {
    if (Model->GetSMRoot() == nullptr) {
        return false;
    }

    std::tie(DestinationSign.sign, DestinationSign.has_light) =
        Model->GetSMRoot()->find_replacable4();

    return DestinationSign.sign != nullptr;
}

void TDynamicObject::create_controller(
    std::string const Type, bool const Trainset) {
    if (Type == "") {
        return;
    }
    ...
    if ((Type == "1") || (Type == "2")) {
        ...
        if (false == Trainset) {
            ...
            Mechanik->PutCommand(
                "Timetable:",
                MoverParameters->V * 3.6 * (iDirection ? -1.0 : 1.0), 0,
                nullptr);
        }
    } else if (Type == "p") {
...
void TDynamicObject::Move(double fDistance) {  // przesuwanie pojazdu po
    ...
    {  // powiązanie pojazdu z osią można zmienić tylko wtedy, gdy skrajne osie
       // są
        // na tym samym torze
        if (MoverParameters->Vel >
            2)  //|[km/h]| nie ma sensu zmiana osi, jesli pojazd drga na postoju
            iAxleFirst =
                (MoverParameters->V >= 0.0)
                    ? 1
                    : 0;  //[m/s] ?1:0 - aktywna druga oś w kierunku jazdy
        // aktualnie eventy aktywuje druga oś, żeby AI nie wyłączało sobie
        // semafora za szybko
    }
    if (fDistance >
        0.0) {  // gdy ruch w stronę sprzęgu 0, doliczyć korektę do osi 1
        bEnabled &=
            Axle0.Move(fDistance, iAxleFirst == 0);  // oś z przodu pojazdu
        bEnabled &= Axle1.Move(
            fDistance /*-fAdjustment*/, iAxleFirst != 0);  // oś z tyłu pojazdu
    } else if (fDistance < 0.0) {  // gdy ruch w stronę sprzęgu 1, doliczyć
                                   // korektę do osi 0
        bEnabled &= Axle1.Move(
            fDistance,
            iAxleFirst != 0);  // oś z tyłu pojazdu prusza się pierwsza
        bEnabled &= Axle0.Move(
            fDistance /*-fAdjustment*/,
            iAxleFirst == 0);  // oś z przodu pojazdu
    ...
    {  // liczenie pozycji pojazdu tutaj, bo jest używane w wielu miejscach
        vPosition = 0.5 * (Axle1.pPosition +
                           Axle0.pPosition);  //środek między skrajnymi osiami
        vFront = Axle0.pPosition -
                 Axle1.pPosition;  // wektor pomiędzy skrajnymi osiami
        // Ra 2F1J: to nie jest stabilne (powoduje rzucanie taborem) i wymaga
        // dopracowania
        fAdjustment = vFront.Length() - fAxleDist;  // na łuku będzie ujemny
        // if (fabs(fAdjustment)>0.02) //jeśli jest zbyt dużo, to rozłożyć na
        // kilka przeliczeń (wygasza drgania?)
        ...
        // fAdjustment=0.0;
        vFront = Normalize(
            vFront);  // kierunek ustawienia pojazdu (wektor jednostkowy)
        vLeft = Normalize(
            CrossProduct(vWorldUp, vFront));  // wektor poziomy w lewo,
        // normalizacja potrzebna z powodu pochylenia (vFront)
        vUp = CrossProduct(vFront, vLeft);  // wektor w górę, będzie jednostkowy
        modelRot.z = atan2(
            -vFront.x, vFront.z);  // kąt obrotu pojazdu [rad]; z ABuBogies()
        auto const roll{Roll()};   // suma przechyłek
        if (roll !=
            0.0) {  // wyznaczanie przechylenia tylko jeśli jest przechyłka
            // można by pobrać wektory normalne z toru...
            mMatrix.Identity();  // ta macierz jest potrzebna głównie do
                                 // wyświetlania
            mMatrix.Rotation(
                roll * 0.5, vFront);  // obrót wzdłuż osi o przechyłkę
            vUp = mMatrix *
                  vUp;  // wektor w górę pojazdu (przekręcenie na przechyłce)
        ...
        mMatrix.Identity();  // to też można by od razu policzyć, ale potrzebne
                             // jest do wyświetlania
        mMatrix.BasisChange(
            vLeft, vUp,
            vFront);  // przesuwanie jest jednak rzadziej niż renderowanie
        mMatrix = Inverse(mMatrix);  // wyliczenie macierzy dla pojazdu
                                     // (potrzebna tylko do wyświetlania?)
        // if (MoverParameters->CategoryFlag&2)
        {  // przesunięcia są używane po wyrzuceniu pociągu z toru
            vPosition.x += MoverParameters->OffsetTrackH *
                           vLeft.x;  // dodanie przesunięcia w bok
            vPosition.z += MoverParameters->OffsetTrackH *
                           vLeft.z;  // vLeft jest wektorem poprzecznym
            // if () na przechyłce będzie dodatkowo zmiana wysokości samochodu
            vPosition.y +=
                MoverParameters
                    ->OffsetTrackV;  // te offsety są liczone przez moverparam
        }
        // obliczanie pozycji sprzęgów do liczenia zderzeń
        auto dir = (0.5 * MoverParameters->Dim.L) * vFront;  // wektor sprzęgu
        vCoulpler[end::front] =
            vPosition + dir;  // współrzędne sprzęgu na początku
        ...
        {                                   // przeliczenie cienia
            ...
            if ((t0->eEnvironment == e_flat) &&
                (t1->eEnvironment == e_flat))  // może być e_bridge...
                fShade = 0.0;                  // standardowe oświetlenie
            else {  // jeżeli te tory mają niestandardowy stopień zacienienia
                // (e_canyon, e_tunnel)
                if (t0->eEnvironment == t1->eEnvironment) {
                    switch (t0->eEnvironment) {  // typ zmiany oświetlenia
                        case e_canyon:
                            fShade = 0.65f;
                ...
                {       // liczymy proporcję
                    double d =
                        Axle0.GetTranslation();  // aktualne położenie na torze
                    if (Axle0.GetDirection() < 0)
                        d = t0->Length() -
                            d;       // od drugiej strony liczona długość
                    d /= fAxleDist;  // rozsataw osi procentowe znajdowanie się
                                     // na torze

                    float shadefrom = 1.0f, shadeto = 1.0f;
                    // NOTE, TODO: calculating brightness level is used enough
                    // times to warrant encapsulation into a function
                    ...
                    fShade =
                        interpolate(shadefrom, shadeto, static_cast<float>(d));
                    /*
                                        switch (t0->eEnvironment)
                                        { // typ zmiany oświetlenia - zakładam,
                       że
                                        // drugi tor ma e_flat
                                        case e_canyon:
                                            fShade = (d * 0.65) + (1.0 - d);
...
void TDynamicObject::AttachNext(
    TDynamicObject *Object, int iType) {  // Ra: doczepia Object na końcu składu
                                          // (nazwa funkcji może być myląca)
    // Ra: używane tylko przy wczytywaniu scenerii
    auto const vehicleend{iDirection};
    auto const othervehicleend{Object->iDirection ^ 1};

    MoverParameters->Attach(
        vehicleend, othervehicleend, Object->MoverParameters, iType, true,
        false);
    ...
    // HACK: we're doing it after establishin actual connection, as the method
    // needs valid neighbour data
    auto &coupler{MoverParameters->Couplers[vehicleend]};
    auto &othercoupler{Object->MoverParameters->Couplers[(
    ...
    if (coupler.type() != othercoupler.type()) {
        if (othercoupler.type() == TCouplerType::Automatic) {
            ...
            attach_coupler_adapter(vehicleend, true);
        ...
        // update distance to neighbours on account of potentially attached
        // adapter
        update_neighbours();
        Object->update_neighbours();
    }
    // potentially adjust vehicle position to avoid collision at the simulation
    // start
    if (MoverParameters->Neighbours[vehicleend].distance > -0.001) {
        return;
    }

    Object->Move(
        MoverParameters->Neighbours[vehicleend].distance *
        Object->DirectionGet());
    // HACK: manually update vehicle position as it's used by neighbour distance
    // update we do next
    Object->MoverParameters->Loc = {
        -Object->vPosition.x, Object->vPosition.z, Object->vPosition.y};
...
void TDynamicObject::LoadExchange(
    int const Disembark, int const Embark, int const Platforms) {
    /*
    ...
    */
    if (Platforms == 0) {
        return;
    }  // edge case, if there's no accessible platforms discard the request
...
float TDynamicObject::LoadExchangeTime() const {
    if ((m_exchange.unload_count < 0.01) && (m_exchange.load_count < 0.01)) {
        return 0.f;
    }

    auto const baseexchangetime{
        m_exchange.unload_count / MoverParameters->UnLoadSpeed +
        m_exchange.load_count / MoverParameters->LoadSpeed};
    auto const nominalexchangespeedfactor{
        (m_exchange.platforms == 3 ? 2.f : 1.f)};
    auto const actualexchangespeedfactor{LoadExchangeSpeed()};

    return baseexchangetime / (actualexchangespeedfactor > 0.f
                                   ? actualexchangespeedfactor
                                   : nominalexchangespeedfactor);
}
...
float TDynamicObject::LoadExchangeSpeed() const {
    ...
    if (m_exchange.platforms & lewe) {
        exchangespeedfactor +=
            (MoverParameters->Doors.instances[side::left].is_open ? 1.f : 0.f);
    }
    if (m_exchange.platforms & prawe) {
        exchangespeedfactor +=
            (MoverParameters->Doors.instances[side::right].is_open ? 1.f : 0.f);
    }
...
void TDynamicObject::update_exchange(double const Deltatime) {
    // TODO: move offset calculation after initial check, when the loading
    // system is all unified
    update_load_offset();
    ...
    if (MoverParameters->Vel < 2.0) {
        ...
        if (exchangespeed < (m_exchange.platforms == 3 ? 2.f : 1.f)) {
            // the exchange isn't performed at optimal rate, or at all. try to
            // open viable vehicle doors to speed it up
            auto const lewe{(DirectionGet() > 0) ? 1 : 2};
            auto const prawe{3 - lewe};
            if ((m_exchange.platforms & lewe) &&
                (false ==
        ...
        if (exchangespeed > 0.f) {
            ...
            while ((m_exchange.unload_count > 0.01) &&
                   (m_exchange.time >= 1.0)) {
                m_exchange.time -= 1.0;
                auto const exchangesize = std::min(
                    m_exchange.unload_count,
                    MoverParameters->UnLoadSpeed * exchangespeed);
                m_exchange.unload_count -= exchangesize;
                MoverParameters->LoadStatus = 1;
                MoverParameters->LoadAmount =
                    std::max(0.f, MoverParameters->LoadAmount - exchangesize);
                MoverParameters->ComputeMass();
                update_load_visibility();
            }
            if (m_exchange.unload_count < 0.01) {
                // finish any potential unloading operation before adding new
                // load don't load more than can fit
                m_exchange.load_count = std::min(
                    m_exchange.load_count,
                    MoverParameters->MaxLoad - MoverParameters->LoadAmount);
                while ((m_exchange.load_count > 0.01) &&
                       (m_exchange.time >= 1.0)) {
                    m_exchange.time -= 1.0;
                    auto const exchangesize = std::min(
                        m_exchange.load_count,
                        MoverParameters->LoadSpeed * exchangespeed);
                    m_exchange.load_count -= exchangesize;
                    MoverParameters->LoadStatus = 2;
                    MoverParameters->LoadAmount = std::min(
                        MoverParameters->MaxLoad,
                        MoverParameters->LoadAmount +
                            exchangesize);  // std::max not strictly needed but,
                                            // eh
                    MoverParameters->ComputeMass();
                    update_load_visibility();
                }
    ...
    if ((m_exchange.unload_count < 0.01) && (m_exchange.load_count < 0.01)) {
        ...
        // if the exchange is completed (or canceled) close the door, if
        // applicable
            ...
            if ((MoverParameters->Vel > 2.0) ||
                (Random() <
                 (
                     ...
                     MoverParameters->Doors.close_control ==
                             control_t::passenger
                         ? 0.75
                         : 0.50))) {
                MoverParameters->OperateDoors(
        ...
        // if the vehicle was emptied potentially switch load visualization
        // model
...
void TDynamicObject::LoadUpdate() {
    ...
    if (MoverParameters->LoadTypeChange) {
        ...
        // TODO: discern from vehicle component which merely uses vehicle
        // directory and has no animations, so it can be initialized outright
        // and actual vehicles which get their initialization after their
        // animations are set up
        if (mdLoad != nullptr) {
            mdLoad->Init();
        }
        // update bindings between lowpoly sections and potential load chunks
        // placed inside them
        update_load_sections();
...
void TDynamicObject::update_load_sections() {
    ...
    for (auto &section : Sections) {
        ...
        if (section.load != nullptr) {
            ...
            // TBD, TODO: store also pointer to chunk submodel and control its
            // visibility more directly, instead of per-section visibility flag?
            ...
            // HACK: disable automatic self-illumination threshold, at least
            // until 3d model update
            if (MoverParameters->CompartmentLights.start_type ==
                start_t::manual) {
...
void TDynamicObject::update_load_visibility() {
    // start with clean load chunk visibility slate
    for (auto &section : Sections) {
        section.load_chunks_visible = 0;
    }
    // each entry in load order sequence matches a single chunk of the section
    // it points to the length of load order sequence matches total number of
    // load chunks
    ...
    auto visiblechunkcount{
        (SectionLoadOrder.empty()
             ...
             : static_cast<int>(std::ceil(
                   loadpercentage * SectionLoadOrder.size() - 0.001f)))};
...
void TDynamicObject::update_load_offset() {
    if (MoverParameters->LoadType.offset_min == 0.f) {
        return;
    }
    ...
    LoadOffset = interpolate(
        MoverParameters->LoadType.offset_min, 0.f,
        clamp(0.0, 1.0, loadpercentage * 0.01));
}

void TDynamicObject::shuffle_load_order() {
    std::shuffle(
        std::begin(SectionLoadOrder), std::end(SectionLoadOrder),
        Global.random_engine);
    // shift chunks assigned to corridors to the end of the list, so they show
    // up last
    ...
    // NOTE: potentially we're left with a mix of corridor and external section
    // loads but that's not necessarily a wrong outcome, so we leave it this way
    // for the time being
}
...
void TDynamicObject::update_destinations() {
    if (DestinationSign.sign == nullptr) {
        return;
    }

    auto const lowvoltagepower{
        (MoverParameters->Power24vIsAvailable ||
         MoverParameters->Power110vIsAvailable)};

    DestinationSign.sign->fLight =
        (((DestinationSign.has_light) && (lowvoltagepower)) ? 2.0 : -1.0);

    // jak są 4 tekstury wymienne, to nie zmieniać rozkładem
    if (std::abs(m_materialdata.multi_textures) >= 4) {
        return;
    }
    // TODO: dedicated setting to discern electronic signs, instead of fallback
    // on light presence
    m_materialdata.replacable_skins[4] =
        (((DestinationSign.destination != null_handle) &&
          ((false ==
            DestinationSign.has_light)  // physical destination signs remain up
                                        // until manually changed
           || ((lowvoltagepower)        // lcd signs are off without power
               && (ctOwner != nullptr))))
             ?  // lcd signs are off for carriages without engine, potentially
                // left on a siding
...
bool TDynamicObject::Update(double dt, double dt1) {
    if (dt1 == 0.0) return true;  // Ra: pauza
    ...
    if (!MyTrack)
        return false;  // pojazdy postawione na torach portalowych mają
                       // MyTrack==NULL
    if (!bEnabled) return false;  // a normalnie powinny mieć bEnabled==false

    double dDOMoveLen;
    // McZapkie: parametry powinny byc pobierane z toru
    if (Axle0.vAngles.z !=
        Axle1.vAngles.z) {  // wyliczenie promienia z obrotów osi - modyfikację
                            // zgłosił youBy
        ts.R = Axle0.vAngles.z -
               Axle1.vAngles.z;  // różnica może dawać stałą ±M_2PI
        if ((ts.R > 15000.0) || (ts.R < -15000.0)) {
            // szkoda czasu na zbyt duże promienie, 4km to promień nie
            // wymagający przechyłki
            ts.R = 0.0;
        } else {
            // normalizacja
            if (ts.R > M_PI) {
    ...
    // (!!!odwrotny)
    ts.dHrail =
        (Axle1.GetRoll() + Axle0.GetRoll()) * 0.5;  //średnia przechyłka pudła
    // TTrackParam tp;
    tp.Width = MyTrack->fTrackWidth;
    // McZapkie-250202
    tp.friction =
        MyTrack->Friction() * Global.fFriction * Global.FrictionWeatherFactor;
    ...
    if (MoverParameters->EnginePowerSource.SourceType ==
        TPowerSource::CurrentCollector) {  // dla EZT tylko silnikowy

        tmpTraction.TractionVoltage = std::max(
            std::abs(MoverParameters->PantRearVolt),
            std::abs(MoverParameters->PantFrontVolt));
        // jeśli brak zasilania dłużej niż 0.2 sekundy (25km/h pod izolatorem
        // daje 0.15s) Ra 2F1H: prowizorka, trzeba przechować napięcie, żeby nie
        // wywalało WS pod izolatorem
        if (tmpTraction.TractionVoltage > 0.0) {
            ...
            if (NoVoltTime <= 0.2) {
                tmpTraction.TractionVoltage =
                    MoverParameters->PantographVoltage;
            } else {
                // jeśli jedzie
                if (MoverParameters->Vel > 0.5) {
                    // Ra 2014-07: doraźna blokada logowania zimnych lokomotyw -
                    // zrobić to trzeba inaczej
                    if (MoverParameters->Pantographs[end::front].is_active ||
                        MoverParameters->Pantographs[end::rear].is_active) {
                        if ((MoverParameters
                                 ->Mains)  // Ra 15-01: logować tylko, jeśli WS
                                           // załączony
                            && (MoverParameters->GetTrainsetHighVoltage() <
                                0.1f)) {  // yB 16-03: i nie jest to asynchron
                                          // zasilany z daleka
                            // Ra 15-01: bezwzględne współrzędne pantografu nie
                            // są dostępne więc lepiej się tego nie zaloguje
                            ErrorLog(
                                "Bad traction: " + MoverParameters->Name +
                                " lost power for " + to_string(NoVoltTime, 2) +
                                " sec. at " + to_string(glm::dvec3{vPosition}));
                        }
                    }
                }
            }
    ...
    tmpTraction.TractionMaxCurrent =
        7500;  // Ra: chyba za dużo? powinno wywalać przy 1500
    ...
    if (Mechanik) {  // Ra 2F3F: do Driver.cpp to przenieść?
        if (Mechanik->primary()) {
            MoverParameters->EqvtPipePress =
                GetEPP();  // srednie cisnienie w PG
        }
        if ((Mechanik->primary()) &&
            ...
            MoverParameters->eimic_real = std::min(
                MoverParameters->eimic, MoverParameters->eimicSpeedCtrl);
            MoverParameters->SendCtrlToNext(
                "EIMIC", MoverParameters->eimic_real,
                MoverParameters->CabActive);
        }
        if ((Mechanik->primary()) && (MoverParameters->EngineType ==
                                      TEngineType::ElectricInductionMotor)) {
            // jesli glowny i z asynchronami, to niech steruje hamulcem i
            // napedem lacznie dla calego pociagu/ezt
            auto const kier =
                (DirectionGet() * MoverParameters->CabOccupied > 0);
            auto FED{0.0};
            auto np{0};
            auto masa{0.0};
            auto FrED{0.0};
            ...
            if ((MoverParameters->Power < 1) && (ctOwner != nullptr)) {
                MoverParameters->MainCtrlPos =
                    ctOwner->Controlling()->MainCtrlPos *
                    MoverParameters->MainCtrlPosNo /
                    std::max(1, ctOwner->Controlling()->MainCtrlPosNo);
                MoverParameters->SpeedCtrlValue =
                    ctOwner->Controlling()->SpeedCtrlValue;
                MoverParameters->SpeedCtrlUnit.IsActive =
                    ctOwner->Controlling()->SpeedCtrlUnit.IsActive;
            }
            MoverParameters->CheckEIMIC(dt1);
            MoverParameters->CheckSpeedCtrl(dt1);

            auto eimic =
                Min0R(MoverParameters->eimic, MoverParameters->eimicSpeedCtrl);
            MoverParameters->eimic_real = eimic;
            if (MoverParameters->EIMCtrlType == 2 &&
                MoverParameters->MainCtrlPos == 0)
                eimic = -1.0;
            MoverParameters->SendCtrlToNext(
                "EIMIC", Max0R(0, eimic), MoverParameters->CabActive);
            auto LBR = Max0R(-eimic, 0);
            auto eim_lb =
                (Mechanik->AIControllFlag ||
                         !MoverParameters->LocHandleTimeTraxx
                     ? 0
                     : MoverParameters->eim_localbrake);

            // 1. ustal wymagana sile hamowania calego pociagu
            //   - opoznienie moze byc ustalane na podstawie charakterystyki
            //   - opoznienie moze byc ustalane na podstawie mas i cisnien
            //   granicznych
            //

            // 2. ustal mozliwa do realizacji sile hamowania ED
            //   - w szczegolnosci powinien brac pod uwage rozne sily hamowania
            for (TDynamicObject *p = GetFirstDynamic(
                     MoverParameters->CabOccupied < 0 ? 1 : 0, 4);
                 p; (kier ? p = p->Next(4) : p = p->Prev(4))) {
                ++np;
                ...
                auto const Nmax = ((p->MoverParameters->P2FTrans *
                ...
                FmaxED +=
                    ((p->MoverParameters->Mains) &&
                             (p->MoverParameters->DirActive != 0) &&
            ...
            if (((MoverParameters->BrakeDelays & (bdelay_P + bdelay_R)) ==
                 (bdelay_P + bdelay_R)) &&
            ...
            auto const amax =
                RapidMult *
                std::min(FmaxPN / masamax, MoverParameters->MED_amax);

            Mechanik->fMedAmax = amax;
            auto doorisopen{
                (false ==
                 MoverParameters->Doors.instances[side::left].is_closed) ||
                (false ==
            ...
            doorisopen &=
                !(MoverParameters->ReleaseParkingBySpringBrakeWhenDoorIsOpen &&
                  MoverParameters->SpringBrake.IsActive);

            if ((MoverParameters->Vel < 0.5) &&
                (eimic < 0 || doorisopen ||
                 MoverParameters->Hamulec->GetEDBCP())) {
                MoverParameters->ShuntMode = true;
            }
            if (MoverParameters->ShuntMode) {
                MoverParameters->ShuntModeAllow =
                    (false == doorisopen) && (LBR < 0.01);
            }
            if ((MoverParameters->Vel > 1) && (false == doorisopen)) {
                MoverParameters->ShuntMode = false;
                MoverParameters->ShuntModeAllow =
                    (MoverParameters->BrakePress > 0.2) && (LBR < 0.01);
            }
            auto Fzad = amax * LBR * masa;
            if (((MoverParameters->BrakeCtrlPos ==
                  MoverParameters->Handle->GetPos(bh_EB)) &&
            ...
            if ((MoverParameters->ScndS) &&
                (MoverParameters->Vel > MoverParameters->eimc[eimc_p_Vh1]) &&
                (FmaxED > 0)) {
                ...
                (MoverParameters->V * MoverParameters->DirAbsolute < -0.2)*/)
            {
                auto const sbd{
                    ((MoverParameters->SpringBrake.IsActive &&
                      MoverParameters->ReleaseParkingBySpringBrake)
                         ? 0.0
                         : MoverParameters->StopBrakeDecc)};
                Fzad = std::max(Fzad, sbd * masa);
            }
            if ((Fzad > 1) && (!MEDLogFile.is_open()) &&
                (MoverParameters->Vel > 1)) {
                MEDLogFile.open(
                    "MEDLOGS/" + MoverParameters->Name + "_" +
                        to_string(++MEDLogCount) + ".csv",
                    std::ios::in | std::ios::out | std::ios::trunc);
                MEDLogFile << "t\tVel\tMasa\tOsie\tFmaxPN\tFmaxED\tFfulED\tFrED"
                              "\tFzad\tFzadED\tFzadPN";
                for (int k = 1; k <= np; k++) {
                    MEDLogFile << "\tBP" << k;
                }
            ...
            if (FzadED > MED_oldFED) {
                if (MoverParameters->MED_ED_DelayTimer <= 0) {
                    ...
                    FzadED = std::min(FzadED, MED_oldFED);
                    MoverParameters->MED_ED_DelayTimer =
                        (FrED > 0 ? -MoverParameters->MED_ED_Delay2
            ...
            // np = 0;
            //  BUG: likely memory leak, allocation per inner loop, deleted only
            //  once outside
            ...
            for (TDynamicObject *p = GetFirstDynamic(
                     MoverParameters->CabOccupied < 0 ? 1 : 0, 4);
                 p; p = (kier == true ? p->Next(4) : p->Prev(4))) {
                auto const Nmax = ((p->MoverParameters->P2FTrans *
                                        p->MoverParameters->MaxBrakePress[0] -
                                    p->MoverParameters->BrakeCylSpring) *
                                       p->MoverParameters->BrakeCylMult[0] -
                ...
                FzEP[i] =
                    static_cast<float>(FzadPN * p->MoverParameters->NAxles) /
                    static_cast<float>(osie);
                ++i;
                p->MoverParameters->ShuntMode = MoverParameters->ShuntMode;
                p->MoverParameters->ShuntModeAllow =
                    MoverParameters->ShuntModeAllow;
            }
            while (test) {
                test = false;
                    ...
                    if ((FzEP[i] > 0.01) &&
                        (FzEP[i] >
                         p->MoverParameters->TotalMass *
                                 p->MoverParameters->eimc[eimc_p_eped] +
                             Min0R(p->MoverParameters->eimv[eimv_Fmax], 0) *
                                 1000) &&
                        (!PrzekrF[i])) {
                        float przek1 =
                            -Min0R(p->MoverParameters->eimv[eimv_Fmax], 0) *
                ...
                for (TDynamicObject *p = GetFirstDynamic(
                         MoverParameters->CabOccupied < 0 ? 1 : 0, 4);
                     p; (true == kier ? p = p->Next(4) : p = p->Prev(4))) {
                    if (!PrzekrF[i]) {
                        FzEP[i] += przek;
                    }
                    ++i;
            ...
            for (TDynamicObject *p = GetFirstDynamic(
                ...
                bool EPVC =
                    ((p->MoverParameters->MED_EPVC) &&
                     ((p->MoverParameters->MED_EPVC_Time < 0) ||
                      (p->MoverParameters->MED_EPVC_CurrentTime <
                       p->MoverParameters->MED_EPVC_Time)));
                float VelC =
                    (EPVC ? clamp(
                                p->MoverParameters->Vel,
                                p->MoverParameters->MED_Vmin,
                                p->MoverParameters->MED_Vmax)
                          : p->MoverParameters
                                ->MED_Vref);  // korekcja EP po prędkości
                float FmaxPoj = Nmax *
                                p->MoverParameters->Hamulec->GetFC(
                                    Nmax / (p->MoverParameters->NAxles *
                                            p->MoverParameters->NBpA),
                ...
                if (p->MoverParameters->LocalBrakePosAEIM > 0.009)
                    if (p->MoverParameters->P2FTrans *
                            p->MoverParameters->BrakeCylMult[0] *
                            p->MoverParameters->MaxBrakePress[0] !=
                        0) {
                        float x = (p->MoverParameters->BrakeSlckAdj /
                                       p->MoverParameters->BrakeCylMult[0] +
                                   p->MoverParameters->BrakeCylSpring) /
                                  (p->MoverParameters->P2FTrans *
                                   p->MoverParameters->MaxBrakePress[0]);
                        p->MoverParameters->LocalBrakePosAEIM =
                            x + (1 - x) * p->MoverParameters->LocalBrakePosAEIM;
                    } else
                        p->MoverParameters->LocalBrakePosAEIM =
                            p->MoverParameters->LocalBrakePosAEIM;
                else
                ...
                if (p->MoverParameters->LocHandleTimeTraxx) {
                    ...
                    p->MoverParameters->LocalBrakePosAEIM =
                        std::max(p->MoverParameters->LocalBrakePosAEIM, eim_lb);
                }
                ++i;
            }
            ...
            if (MEDLogFile.is_open()) {
                MEDLogFile << MEDLogTime << "\t" << MoverParameters->Vel << "\t"
                           << masa * 0.001 << "\t" << osie << "\t"
                           << FmaxPN * 0.001 << "\t" << FmaxED * 0.001 << "\t"
                           << FfulED * 0.001 << "\t" << FrED * 0.001 << "\t"
                           << Fzad * 0.001 << "\t" << FzadED * 0.001 << "\t"
                           << FzadPN * 0.001;
                for (TDynamicObject *p = GetFirstDynamic(
                         MoverParameters->CabOccupied < 0 ? 1 : 0, 4);
                     p; (true == kier ? p = p->Next(4) : p = p->Prev(4))) {
                ...
                if ((MoverParameters->Vel < 0.1) ||
                    (MoverParameters->MainCtrlPowerPos() > 0)) {
    ...
    if (MoverParameters->Mains) {  // nie wchodzić w funkcję bez potrzeby
        if ((false == (MoverParameters->Power24vIsAvailable ||
                       MoverParameters->Power110vIsAvailable))
            /*
                      // NOTE: disabled on account of multi-unit setups, where
               the unmanned unit wouldn't be affected
                        && ( Controller == Humandriver )
            */
            && (MoverParameters->EngineType != TEngineType::DieselEngine) &&
            (MoverParameters->EngineType !=
             TEngineType::WheelsDriven)) {  // jeśli bateria wyłączona, a nie
                                            // diesel ani drezyna reczna
            if (MoverParameters->MainSwitch(
                    false,
                    (MoverParameters->TrainType == dt_EZT ? range_t::unit
                                                          : range_t::local))) {
                // wyłączyć zasilanie
                // NOTE: we turn off entire EMU, but only the affected unit for
                // other multi-unit consists
                MoverParameters->EventFlag = true;
                // drop pantographs
                // NOTE: this isn't universal behaviour
                // TODO: have this dependant on .fiz-driven flag
                // NOTE: moved to pantspeed calculation part a little later in
                // the function. all remarks and todo still apply
                /*
                                MoverParameters->PantFront( false, (
                   MoverParameters->TrainType == dt_EZT ? range::unit :
                   range::local ) ); MoverParameters->PantRear( false, (
                   MoverParameters->TrainType == dt_EZT ? range::unit :
                   range::local ) );
                */
            }
    ...
    TLocation const l{-vPosition.x, vPosition.z, vPosition.y};
    TRotation const r{0.0, 0.0, modelRot.z};
    // McZapkie-260202 - dMoveLen przyda sie przy stukocie kol
    dDOMoveLen = GetdMoveLen() + MoverParameters->ComputeMovement(
                                     dt, dt1, ts, tp, tmpTraction, l, r);
    if (Mechanik) {
    ...
    m_future_movement =
        (glm::dvec3(vPosition) - old_pos) / dt1 * Timer::GetDeltaRenderTime();
    ...
    if (MyTrack->fSoundDistance != -1) {
        ...
        if (dRailLength > 0.0) {
            if (MoverParameters->Vel > 0) {
                // TODO: track quality and/or environment factors as separate
                // subroutine
                auto volume = interpolate(
                    0.8, 1.2, clamp(MyTrack->iQualityFlag / 20.0, 0.0, 1.0));
                ...
                if (dRailLength > 0.0) {
                    ...
                    for (auto &axle : m_axlesounds) {
                        ...
                        if ((axle.distance < 0) ||
                            ...
                            axle.distance =
                                clamp_circular(axle.distance, dRailLength);
                            if (MoverParameters->Vel > 0.1) {
                                // NOTE: for combined clatter sound we supply
                                // 1/100th of actual value, as the sound module
                                // converts does the opposite, converting
                                // received (typically) 0-1 values to 0-100
                                // range
                                auto const frequency =
                                    (true == axle.clatter.is_combined()
                                         ? MoverParameters->Vel * 0.01
                                         : 1.0);
                                axle.clatter.pitch(frequency)
                                    .gain(volume)
                                    .play();
                                // crude bump simulation, drop down on even
                                // axles, move back up on the odd ones
                                MoverParameters->AccVert +=
                                    interpolate(
                                        0.01, 0.05,
                                        clamp(
                                            GetVelocity() /
                                                (1 + MoverParameters->Vmax),
                                            0.0, 1.0)) *
                                    ((axleindex % 2) != 0 ? 1 : -1);
                            }
                        }
    ...
    if (MoverParameters->Vel != 0) {  // McZapkie-050402: krecenie kolami:
        glm::dvec3 old_wheels =
            glm::dvec3(dWheelAngle[0], dWheelAngle[1], dWheelAngle[2]);

        dWheelAngle[0] += 114.59155902616464175359630962821 *
                          MoverParameters->V * dt1 /
                          MoverParameters->WheelDiameterL;  // przednie toczne
        dWheelAngle[1] += MoverParameters->nrot * dt1 * 360.0;  // napędne
        dWheelAngle[2] += 114.59155902616464175359630962821 *
                          MoverParameters->V * dt1 /
                          MoverParameters->WheelDiameterT;  // tylne toczne

        m_future_wheels_angle =
            (glm::dvec3(dWheelAngle[0], dWheelAngle[1], dWheelAngle[2]) -
             old_wheels) /
            dt1 * Timer::GetDeltaRenderTime();
    ...
    if (pants)     // pantograf może być w wagonie kuchennym albo pojeździe
                   // rewizyjnym (np. SR61)
    {              // przeliczanie kątów dla pantografów
        ...
        double fCurrent =
            ((MoverParameters->DynamicBrakeFlag &&
              MoverParameters->ResistorsFlag)
                 ? 0
                 : std::abs(MoverParameters->Itot) *
                       MoverParameters->IsVehicleEIMBrakingFactor()) +
        ...
        // woltomierz, rozładowuje kondensator orgromowy 4µF
        /*
                double fPantCurrent = fCurrent; // normalnie cały prąd przez
           jeden pantograf if (iAnimType[ANIM_PANTS] > 1) { // a jeśli są dwa
           pantografy //Ra 1014-11: proteza, trzeba zrobić sensowniej if
           (pants[0].fParamPants->hvPowerWire &&
           pants[1].fParamPants->hvPowerWire) { // i oba podłączone do drutów
                        fPantCurrent = fCurrent * 0.5; // to dzielimy prąd równo
           na oba (trochę bez sensu, ale lepiej tak niż podwoić prąd)
                    }
                }
        */
        ...
        for (int idx = 0; idx < iAnimType[ANIM_PANTS]; ++idx) {
            ...
            if (Global.bLiveTraction == false) {
                ...
                if ((pantograph->hvPowerWire != nullptr) &&
                    (true ==
                     MoverParameters->Pantographs[end::front].is_active) &&
                    (pantograph->PantTraction - pantograph->PantWys <
                     0.01)) {  // tolerancja niedolegania
                    ++activepantographs;
                }
            }
        }
        ...
        for (int i = 0; i < iAnimType[ANIM_PANTS];
             ++i) {  // pętla po wszystkich pantografach
            p = pants[i].fParamPants;
            if (p->PantWys <
                0) {  // patograf został połamany, liczony nie będzie
                if (p->fAngleL > p->fAngleL0)
                    p->fAngleL -=
                        0.2 * dt1;  // nieco szybciej niż jak dla opuszczania
                if (p->fAngleL < p->fAngleL0)
                    p->fAngleL = p->fAngleL0;  // kąt graniczny
                if (p->fAngleU < M_PI)
                    p->fAngleU += 0.5 * dt1;  // górne się musi ruszać szybciej.
                if (p->fAngleU > M_PI) p->fAngleU = M_PI;
                if (i &
                    1)  // zgłoszono, że po połamaniu potrafi zostać zasilanie
                ...
                case 0:
                    if ((Global.bLiveTraction == false) &&
                        ...
                        MoverParameters->PantFrontVolt =
                            (p->PantWys >= 1.2)
                                ? 0.95 * MoverParameters->EnginePowerSource
                                             .MaxVoltage
                                : 0.0;
                    } else if (
                        (true ==
                    ...
                    {
                        if (p->hvPowerWire) {
                            auto const lastvoltage{
                                MoverParameters->PantFrontVolt};
                            // TODO: wyliczyć trzeba prąd przypadający na
                            // pantograf i wstawić do GetVoltage()
                            if (lastvoltage == 0.0) {
                                // HACK: retrieve the wire voltage for
                                // calculations down the road without blowing up
                                // the supply
                                MoverParameters->PantFrontVolt =
                                    p->hvPowerWire->VoltageGet(
                                        MoverParameters->PantographVoltage,
                                        0.0);
                            } else {
                                MoverParameters->PantFrontVolt =
                                ...
                                if (MoverParameters->PantFrontVolt > 0.0) {
                                    fCurrent -=
                                        fPantCurrent;  // taki prąd płynie przez
                                                       // powyższy pantograf
                                                       // (unless it doesn't)
                                }
                            }
                            // TODO: refactor reaction to voltage change to
                            // mover as sound event for specific pantograph
                            if ((lastvoltage == 0.0) &&
                                (MoverParameters->PantFrontVolt > 0.0)) {
                                for (auto &pantograph : m_pantographsounds) {
                                    if (pantograph.sPantUp.offset().z > 0) {
                                        // limit to pantographs located in the
                                        // front half of the vehicle
                                        pantograph.sPantUp.play(
                                            sound_flags::exclusive);
                                    }
                                }
                            }
                        } else
                        ...
                        MoverParameters->PantFrontVolt = 0.0;
                    ((fPantCurrent > 0.0)
                         ? MoverParameters->EnergyMeter.first
                         : MoverParameters->EnergyMeter.second) +=
                        MoverParameters->PantRearVolt * fPantCurrent * dt1 /
                        3600000.0;
                    break;
                case 1:
                    ...
                    ((fPantCurrent > 0.0)
                         ? MoverParameters->EnergyMeter.first
                         : MoverParameters->EnergyMeter.second) +=
                        MoverParameters->PantFrontVolt * fPantCurrent * dt1 /
                        3600000.0;
                    break;
            ...
            if (MoverParameters->PantPress >
                (MoverParameters->TrainType == dt_EZT
                     ? 2.45
                     :  // Ra 2013-12: Niebugocław mówi, że w EZT podnoszą się
                        // przy 2.5
                     3.45)) {
                // z EXE Kursa
            ...
            if ((pantspeedfactor > 0.0) &&
                (MoverParameters->Pantographs[i]
                     .is_active))  // jeśli ma być podniesiony
            {
                if (PantDiff > 0.001)  // jeśli nie dolega do drutu
                {  // jeśli poprzednia wysokość jest mniejsza niż pożądana,
                   // zwiększyć kąt dolnego
                    // ramienia zgodnie z ciśnieniem
                    if (pantspeedfactor >
                        0.55 * PantDiff)  // 0.55 to około pochodna kąta po
                                          // wysokości
                        k += 0.55 *
                             PantDiff;  // ograniczenie "skoku" w danej klatce
                    else
                    ...
                    // jeśli wysokość jest zbyt duża, wyznaczyć zmniejszenie
                    // kąta jeśli zmniejszenie kąta jest zbyt duże, przejść do
                    // trybu łamania pantografu if (PantFrontDiff<-0.05) //skok
                    // w dół o 5cm daje złąmanie pantografu
                    k +=
                        0.4 * PantDiff;  // mniej niż pochodna kąta po wysokości
                }     // jeśli wysokość jest dobra, nic więcej nie liczyć
            } else {  // jeśli ma być na dole
            ...
            if (k != p->fAngleL) {  //żeby nie liczyć w kilku miejscach ani gdy
                ...
                if (k + p->fAngleU <
                    ...
                    //=acos((b*cos()+c)/a)
                    // p->dPantAngleT=acos((1.22*cos(k)+0.535)/1.755); //górne
                    // ramię
                    p->fAngleU = acos(
                        (p->fLenL1 * cos(k) + p->fHoriz) /
                        p->fLenU1);  // górne ramię
                    // wyliczyć aktualną wysokość z wzoru sinusowego
                    // h=a*sin()+b*sin()
                    // wysokość całości
                    p->PantWys = p->fLenL1 * sin(k) +
                                 p->fLenU1 * sin(p->fAngleU) + p->fHeight;
                }
            }
        }  // koniec pętli po pantografach
        // TBD, TODO: generate sound event during mover update instead?
        if (MoverParameters->Pantographs[end::front].sound_event !=
            MoverParameters->Pantographs[end::front].is_active) {
            if (MoverParameters->Pantographs[end::front].is_active) {
                ...
                for (auto &pantograph : m_pantographsounds) {
                    if (pantograph.sPantDown.offset().z > 0) {
                        // limit to pantographs located in the front half of the
                        // vehicle
                        pantograph.sPantDown.play(sound_flags::exclusive);
                    }
                }
            }
            MoverParameters->Pantographs[end::front].sound_event =
                MoverParameters->Pantographs[end::front].is_active;
        }
        if (MoverParameters->Pantographs[end::rear].sound_event !=
            MoverParameters->Pantographs[end::rear].is_active) {
            ...
            MoverParameters->Pantographs[end::rear].sound_event =
                MoverParameters->Pantographs[end::rear].is_active;
        }
    } else if (
        MoverParameters->EnginePowerSource.SourceType ==
        TPowerSource::InternalSource)
        if (MoverParameters->EnginePowerSource.PowerType ==
            TPowerType::SteamPower)
        // if (smPatykird1[0])
        {  // Ra: animacja rozrządu parowozu, na razie nieoptymalizowane
            /* //Ra: tymczasowo wyłączone ze względu na porządkowanie animacji
               ...
               {//obie strony w ten sam sposób
                fi=DegToRad(dWheelAngle[1]+(i?pant2x:pant1x)); //kąt obrotu koła
               dla tłoka 1 sin_fi=sin(fi); cos_fi=cos(fi);
               ...
               //nieoptymalne
                ...
                //smPatykirg1[0]->SetRotate(float3(0,1,0),RadToDeg(fi));
               //obracamy
                //ruch drążka mimośrodkowego oraz jarzma
                //korzystałem z pliku PDF "mm.pdf" (opis czworoboku
               korbowo-wahaczowego):
                //"MECHANIKA MASZYN. Szkic wykładu i laboratorium
               komputerowego."
                //Prof. dr hab. inż. Jerzy Zajączkowski, 2007, Politechnika
               Łódzka
                ...
                //Lr - promień jarzma =1.0 (pozostałe przeliczone
               proporcjonalnie)
               ...
               jarzma, czyli
                ...
                //2) dla dWheelAngle[1]=90° korba jest do tyłu, a mimośród w
               dół, czyli fi=83° sin_fi=sin(fi); cos_fi=cos(fi);
                ...
                G3=2.0*(L1-Lc*sin_fi);
                G3_2=G3*G3;
                sin_ksi=(G1*G2-G3*_fm_sqrt(G2_2+G3_2-G1_2))/(G2_2+G3_2); //x1
               (minus delta) ksi=asin(sin_ksi); //kąt jarzma if (smPatykirg2[i])
                 smPatykirg2[i]->SetRotateXYZ(vector3(RadToDeg(ksi),0,0));
               //obrócenie jarzma
                //1) ksi=-23°, gam=
                //2) ksi=10°, gam=
               ...
               względem pionu
                gam=atan2((L1-cos(ksi)+Lc*sin_fi),(L2-sin_ksi+Lc*cos_fi)); //kąt
               od poziomu if (smPatykird2[i]) //na razie zabezpieczenie
    ...
    if ((MoverParameters->Vel > MoverParameters->MirrorVelClose) ||
        ...
        if (dMirrorMoveL > 0.0) {
            dMirrorMoveL = std::max(0.0, dMirrorMoveL - 1.0 * dt1);
        }
        if (dMirrorMoveR > 0.0) {
            dMirrorMoveR = std::max(0.0, dMirrorMoveR - 1.0 * dt1);
        }
    } else {
        ...
        if ((dMirrorMoveL < 1.0) &&
            (true ==
             MoverParameters->Doors.instances[side::left].open_permit)) {
            dMirrorMoveL = std::min(1.0, dMirrorMoveL + 1.0 * dt1);
        }
        if ((dMirrorMoveR < 1.0) &&
            (true ==
             MoverParameters->Doors.instances[side::right].open_permit)) {
            dMirrorMoveR = std::min(1.0, dMirrorMoveR + 1.0 * dt1);
        }
    }

    // compartment lights
    // if the vehicle has a controller, we base the light state on state of the
    // controller otherwise we check the vehicle itself
    if ((ctOwner != nullptr ? SectionLightsActive !=
                                  MoverParameters->CompartmentLights.is_active
         : Mechanik != nullptr
             ? SectionLightsActive !=
                   MoverParameters->CompartmentLights.is_active
             : SectionLightsActive)) {  // without controller switch the lights
                                        // off
        toggle_lights();
    }
    ...
    if (MoverParameters->InactiveCabPantsCheck) {
        ...
        if (p) {
            ...
            if (isAnyPantUp) {
                MoverParameters->OperatePantographValve(
                    end::front, operation_t::enable);
                MoverParameters->OperatePantographValve(
                    end::rear, operation_t::enable);
            }
        }
    ...
    if (MoverParameters->DerailReason > 0) {
        switch (MoverParameters->DerailReason) {
            case 1:
                ErrorLog(
                    "Bad driving: " + asName + " derailed due to end of track");
                break;
            case 2:
                ErrorLog(
                    "Bad driving: " + asName +
                    " derailed due to too high speed");
                break;
            case 3:
                ErrorLog(
                    "Bad dynamic: " + asName + " derailed due to track width");
                break;  // błąd w scenerii
            case 4:
                ErrorLog(
                    "Bad dynamic: " + asName +
                    " derailed due to wrong track type");
                break;  // błąd w scenerii
        }
...
bool TDynamicObject::FastUpdate(double dt) {
    ...
    // tp.DamageFlag=MyTrack->iDamageFlag;
    // tp.QualityFlag=MyTrack->iQualityFlag;
    dDOMoveLen = MoverParameters->FastComputeMovement(
        dt, ts, tp, l, r);  // ,ts,tp,tmpTraction);
    // Move(dDOMoveLen);
    // ResetdMoveLen();
    FastMove(dDOMoveLen);
...
void TDynamicObject::TurnOff() {  // wyłączenie rysowania submodeli zmiennych
                                  // dla
    // egemplarza pojazdu
    btnOn = false;
    btCoupler1.TurnOff();
...
void TDynamicObject::RenderSounds() {
    ...
    if ((m_startjoltplayed) &&
        ((std::abs(MoverParameters->AccSVBased) < 0.01) ||
         (GetVelocity() < 0.01))) {
        // if the vehicle comes to a stop set the movement jolt to play when it
        // starts moving again
    ...
    if (MoverParameters->ConverterFlag) {
        if (MoverParameters->EngineType == TEngineType::ElectricSeriesMotor) {
            auto const voltage{std::max(
                MoverParameters->GetTrainsetHighVoltage(),
                MoverParameters->PantographVoltage)};
            if (voltage > 0.0) {
                // NOTE: we do sound modulation here to avoid sudden jump on
                // voltage loss
                frequency =
                    (voltage /
                     (MoverParameters->NominalVoltage *
                      MoverParameters->RList[MoverParameters->RlistSize].Mn));
                frequency *=
                    sConverter.m_frequencyfactor + sConverter.m_frequencyoffset;
                sConverter.pitch(
                    clamp(frequency, 0.75, 1.25));  // arbitrary limits )
            }
        } else {
            frequency = sConverter.m_frequencyoffset +
                        sConverter.m_frequencyfactor * frequency;
            sConverter.pitch(
                clamp(frequency, 0.75, 1.25));  // arbitrary limits )
        }
    ...
    if (MoverParameters->CompressorSpeed > 0.0) {
        ...
        if (MoverParameters->CompressorFlag) {
            // for compressor coupled with the diesel engine sound pitch is
            // driven by engine revolutions
            if (MoverParameters->CompressorPower == 3) {
                // presume the compressor sound is recorded for idle revolutions
                // increase the pitch according to increase of engine
                // revolutions
                auto const enginefactor{clamp(  // try to keep the sound pitch
                                                // in semi-reasonable range
                    MoverParameters->EngineMaxRPM() /
                        MoverParameters->EngineIdleRPM() *
                        MoverParameters->EngineRPMRatio(),
                    0.5, 2.5)};
            ...
            if (sCompressorIdle.empty()) {
                ...
                // enhanced sound path, with dedicated sound for idling
                // compressor
                if (MoverParameters->CompressorGovernorLock) {
                    sCompressor.stop();
                    sCompressorIdle.play(
                        sound_flags::exclusive | sound_flags::looping);
                } else {
                    sCompressor.play(
                        sound_flags::exclusive | sound_flags::looping);
    ...
    auto const brakepressureratio{
        std::max(0.0, MoverParameters->BrakePress) /
        std::max(1.0, MoverParameters->MaxBrakePress[3])};
    if (m_lastbrakepressure != -1.f) {
        ...
        auto const lastbrakepressureratio{
            std::max(0.f, m_lastbrakepressure) /
            std::max(1.0, MoverParameters->MaxBrakePress[3])};
        auto const quantizedratiochange{
            quantizedratio - static_cast<int>(15 * lastbrakepressureratio)};
        if (quantizedratiochange > 0) {
            m_brakecylinderpistonadvance
                .pitch(
                    true == m_brakecylinderpistonadvance.is_combined()
                        ? quantizedratio * 0.01f
                        : m_brakecylinderpistonadvance.m_frequencyoffset +
                              m_brakecylinderpistonadvance.m_frequencyfactor *
                                  1.f)
                .play();
            ...
            m_brakecylinderpistonrecede
                .pitch(
                    true == m_brakecylinderpistonrecede.is_combined()
                        ? quantizedratio * 0.01f
                        : m_brakecylinderpistonrecede.m_frequencyoffset +
                              m_brakecylinderpistonrecede.m_frequencyfactor *
                                  1.f)
                .play();
        }
    }
    ...
    // epbrake - epcompact
    if ((MoverParameters->BrakeSystem == TBrakeSystem::ElectroPneumatic) &&
        (MoverParameters->LocHandle)) {
        auto const epbrakepressureratio{std::min(
            std::max(0.0, MoverParameters->LocHandle->GetCP()) /
                std::max(1.0, MoverParameters->MaxBrakePress[0]),
            m_epbrakepressurechangedectimer > -1.0f
                ? std::max(
                      MoverParameters->LocalBrakePosAEIM,
                      MoverParameters->Hamulec->GetEDBCP() /
        ...
        if (m_lastepbrakepressure != -1.f) {
            ...
            // actual sound playback
            auto const epquantizedratio{
                static_cast<int>(50 * epbrakepressureratio)};
            auto const lastepbrakepressureratio{
                std::max(0.f, m_lastepbrakepressure) /
                std::max(1.0, MoverParameters->MaxBrakePress[0])};
            auto const epquantizedratiochange{
                epquantizedratio -
                static_cast<int>(50 * lastepbrakepressureratio)};
            if (epquantizedratiochange > 0 &&
                m_epbrakepressurechangeinctimer > 0.05f) {
                m_epbrakepressureincrease
                    .pitch(
                        true == m_epbrakepressureincrease.is_combined()
                            ? epquantizedratio * 0.01f
                            : m_epbrakepressureincrease.m_frequencyoffset +
                                  m_epbrakepressureincrease.m_frequencyfactor *
                                      1.f)
                    .play();
                m_epbrakepressurechangeinctimer = 0;
            } else if (
                epquantizedratiochange < 0 &&
                m_epbrakepressurechangedectimer > 0.3f) {
                m_epbrakepressuredecrease
                    .pitch(
                        true == m_epbrakepressuredecrease.is_combined()
                            ? -epquantizedratiochange * 0.01f
                            : m_epbrakepressuredecrease.m_frequencyoffset +
                                  m_epbrakepressuredecrease.m_frequencyfactor *
                                      1.f)
                    .play();
                m_epbrakepressurechangedectimer = 0;
            }
        }
        if ((m_epbrakepressurechangeinctimer == 0) ||
            (m_epbrakepressurechangedectimer == 0))
            m_lastepbrakepressure = std::min(
                MoverParameters->LocHandle->GetCP(),
                MoverParameters->LocalBrakePosAEIM *
                    std::max(1.0, MoverParameters->MaxBrakePress[0]));
    }
    ...
    if (MoverParameters->EmergencyValveFlow > 0.025) {
        ...
        m_emergencybrakeflow =
            (m_emergencybrakeflow == 0.0
                 ...
                 : interpolate(
                       m_emergencybrakeflow,
                       MoverParameters->EmergencyValveFlow, 0.1));
        // scale volume based on the flow rate and on the pressure in the main
        // pipe
        auto const flowpressure{
            clamp(m_emergencybrakeflow, 0.0, 1.0) +
            clamp(0.1 * MoverParameters->PipePress, 0.0, 0.5)};
        m_emergencybrake
            .pitch(
                m_emergencybrake.m_frequencyoffset +
                1.0 * m_emergencybrake.m_frequencyfactor)
            .gain(
                m_emergencybrake.m_amplitudeoffset +
                clamp(flowpressure, 0.0, 1.0) *
                    m_emergencybrake.m_amplitudefactor)
            .play(sound_flags::exclusive | sound_flags::looping);
    ...
    // air release
    if (m_lastbrakepressure != -1.f) {
        // calculate rate of pressure drop in brake cylinder, once it's been
        // initialized
        auto const brakepressuredifference{
            m_lastbrakepressure - MoverParameters->BrakePress};
        m_brakepressurechange = interpolate<float>(
            m_brakepressurechange, brakepressuredifference / dt, 0.05f);
    }
    m_lastbrakepressure = MoverParameters->BrakePress;
    // ensure some basic level of volume and scale it up depending on pressure
    // in the cylinder; scale this by the air release rate
    volume = 20 * m_brakepressurechange * (0.25 + 0.75 * brakepressureratio);
    if ((m_brakepressurechange > 0.05) && (brakepressureratio > 0.05)) {
        rsUnbrake.gain(volume).play(
            sound_flags::exclusive | sound_flags::looping);
    } else {
    ...
    if (MoverParameters->SlippingWheels) {
        if ((MoverParameters->UnitBrakeForce > 100.0) &&
            ...
            auto const velocitydifference{
                GetVelocity() / MoverParameters->Vmax};
            rsSlippery
                .gain(
                    rsSlippery.m_amplitudeoffset +
                    rsSlippery.m_amplitudefactor * velocitydifference)
                .play(sound_flags::exclusive | sound_flags::looping);
        }
    } else {
        rsSlippery.stop();
    ...
    if (  //( false == mvOccupied->SlippingWheels ) &&
        ...
        brakeforceratio = clamp(
            MoverParameters->UnitBrakeForce /
                std::max(
                    1.0,
                    MoverParameters->BrakeForceR(1.0, MoverParameters->Vel) /
                        (MoverParameters->NAxles *
                         std::max(1, MoverParameters->NBpA))),
            0.0, 1.0);
        rsBrake
            .pitch(
                rsBrake.m_frequencyoffset +
                MoverParameters->Vel * rsBrake.m_frequencyfactor)
            .gain(
    ...
    if (true == bBrakeAcc) {
        if (true ==
            TestFlag(MoverParameters->Hamulec->GetSoundFlag(), sf_Acc)) {
    ...
    if (MoverParameters->Vel > 2.5) {
        volume =
            rsPisk.m_amplitudeoffset +
            interpolate(-1.0, 1.0, brakeforceratio) * rsPisk.m_amplitudefactor;
        if (volume > 0.075) {
            rsPisk
                .pitch(
                    true == rsPisk.is_combined()
                        ? MoverParameters->Vel * 0.01f
                        : rsPisk.m_frequencyoffset +
                              rsPisk.m_frequencyfactor * 1.f)
                .gain(volume)
                .play(sound_flags::exclusive | sound_flags::looping);
        }
    } else {
    ...
    if (MoverParameters->Doors.has_warning) {
        ...
        for (auto &doorspeaker : m_doorspeakers) {
            ...
            if ((MoverParameters->DepartureSignal) && (lowvoltagepower)
                /*
                             || ( ( MoverParameters->DoorCloseCtrl =
                   control::autonomous )
                               && ( ( ( false == MoverParameters->DoorLeftOpened
                   )  && ( dDoorMoveL > 0.0 ) )
                                 || ( ( false ==
                   MoverParameters->DoorRightOpened ) && ( dDoorMoveR > 0.0 ) )
                   ) )
                */
            ) {
                // for the autonomous doors play the warning automatically
                // whenever a door is closing MC: pod warunkiem ze jest
                // zdefiniowane w chk
                doorspeaker.departure_signal.play(
                    sound_flags::exclusive | sound_flags::looping);
            } else {
    ...
    // announcements
    {
        ...
        if (lowvoltagepower) {
            ...
            if ((false == m_pasystem.announcement_queue.empty()) &&
                ...
                m_pasystem.announcement.range(
                    0.5 * MoverParameters->Dim.L * -1);
                if (m_pasystem.soundproofing) {
                    if (!m_pasystem.announcement.soundproofing()) {
                        m_pasystem.announcement.soundproofing() =
                            m_pasystem.soundproofing;
                    }
                }
    ...
    for (auto const side : sides) {
        ...
        if (true == door.is_opening) {
            ...
            if ((false == door.step_unfolding)  // no wait if no doorstep
                || (MoverParameters->Doors.step_type ==
                    2)) {                    // no wait for rotating doorstep
                if (door.position < 0.5f) {  // safety measure, to keep slightly
                                             // too short sounds from repeating
        ...
        if (true == door.is_closing) {
            // door sounds can start playing before the door begins moving but
            // shouldn't cease once the door closes
        ...
        if (door.step_folding) {
            if (door.step_position <
                1.0f) {  // sanity check, the vehicles may keep the doorstep
                         // unfolded until the door close
                for (auto &doorsounds : m_doorsounds) {
                    if (doorsounds.placement == side) {
                        // determine left side doors from their offset
    ...
    // horns
    {
        // for moving vehicle combine regular horn activation flag with
        // emergency brake horn activation flag, if the brake is active
    ...
    if ((GetVelocity() > 0.5)
...
#endif
        &&
        (  // compound test whether the vehicle belongs to user-driven consist
           // (as these don't emit outer noise in cab view)
            FreeFlyModeFlag ? true
                            :  // in external view all vehicles emit outer noise
                // Global.pWorld->train() == nullptr ? true : // (can skip this
                // check, with no player train the external view is a given)
                ctOwner == nullptr
                ? true
                :  // standalone vehicle, can't be part of user-driven train
                ctOwner != simulation::Train->Dynamic()->ctOwner
                ? true
                :  // confirmed isn't a part of the user-driven train
                Global.CabWindowOpen
                ? true
                :  // sticking head out we get to hear outer noise
                false)) {
#ifdef EU07_SOUND_BOGIESOUNDS
        ...
        auto const normalizer{
            (true == bogiesound.is_combined() ? MoverParameters->Vmax * 0.01f
                                              : 1.f)};
        frequency =
        ...
        // volume calculation
        volume = bogiesound.m_amplitudeoffset +
        ...
        if (false == bogiesound.is_combined()) {
            volume *= interpolate(
                0.0, 1.0, clamp(MoverParameters->Vel / 40.0, 0.0, 1.0));
        }
        ...
        if (volume > 0.05) {
            ...
            for (auto &bogiesound : m_bogiesounds) {
                bogiesound
                    .pitch(frequency)  // arbitrary limits to prevent the pitch
                                       // going out of whack
                    .gain(volume)
                    .play(sound_flags::exclusive | sound_flags::looping);
            }
    ...
    if (MoverParameters->CategoryFlag == 1) {
        // trains only
        if ((GetVelocity() > 1.0) && (MoverParameters->WheelFlat > 5.0)) {
            m_wheelflat
                .pitch(
                    m_wheelflat.m_frequencyoffset +
                    std::abs(MoverParameters->nrot) *
                        m_wheelflat.m_frequencyfactor)
                .gain(
                    m_wheelflat.m_amplitudeoffset +
                    m_wheelflat.m_amplitudefactor *
                        ((1.0 + (MoverParameters->Vel / MoverParameters->Vmax) +
                          clamp(MoverParameters->WheelFlat / 60.0, 0.0, 1.0)) /
                         3.0))
                .play(sound_flags::exclusive | sound_flags::looping);
        } else {
            m_wheelflat.stop();
    ...
    if ((MoverParameters->Vel > 5.0) && (ts.R * ts.R > 1.0) &&
        (std::abs(ts.R) < 15000.0)) {
        // scale volume with curve radius and vehicle speed
        volume = std::abs(MoverParameters->AccN)  // * MoverParameters->AccN
                 * interpolate(
                       0.5, 1.0, clamp(MoverParameters->Vel / 40.0, 0.0, 1.0)) *
                 (((MyTrack->eType == tt_Switch) && (std::abs(ts.R) < 1500.0))
                      ? 100.0
                      : 1.0);
    } else {
        volume = 0;
        ...
        rscurve
            .pitch(
                true == rscurve.is_combined()
                    ? MoverParameters->Vel * 0.01f
                    : rscurve.m_frequencyoffset +
                          rscurve.m_frequencyfactor * 1.f)
            .gain(2.5 * volume)
            .play(sound_flags::exclusive | sound_flags::looping);
    } else {
        rscurve.stop();
    ...
    if (false == m_startjoltplayed) {
        ...
        if ((MoverParameters->V > 0.0 ? (MoverParameters->AccSVBased > 0.1)
                                      : (MoverParameters->AccSVBased < 0.1)) &&
            (velocity > 1.0) && (velocity < 15.0)) {
            m_startjolt.play(sound_flags::exclusive);
            m_startjoltplayed = true;
        }
    }

    // McZapkie! - to wazne - SoundFlag wystawiane jest przez moje moduly gdy
    // zachodza pewne wydarzenia komentowane dzwiekiem. pneumatic relay
    ...
    if (TestFlag(MoverParameters->SoundFlag, sound::doorpermit)) {
        // NOTE: current implementation doesn't discern between permit for
        // left/right side, which may be undesired in weird setups with doors
        // only on one side TBD, TODO: rework into dedicated sound event flag
        // for each door location instance?
        for (auto &door : m_doorsounds) {
        ...
        if (true == TestFlag(coupler.sounds, sound::bufferclash)) {
            ...
            if (true == TestFlag(coupler.sounds, sound::loud)) {
                ...
                if (false == couplersounds.dsbBufferClamp_loud.empty()) {
                    ...
                    couplersounds.dsbBufferClamp_loud.gain(1.f).play(
                        sound_flags::exclusive);
                } else {
                    // fallback on the standard sound
                    couplersounds.dsbBufferClamp.gain(1.f).play(
                        sound_flags::exclusive);
                }
            } else {
                // basic clash
        ...
        if (true == TestFlag(coupler.sounds, sound::couplerstretch)) {
            ...
            if (true == TestFlag(coupler.sounds, sound::loud)) {
                ...
                if (false == couplersounds.dsbCouplerStretch_loud.empty()) {
                    ...
                    couplersounds.dsbCouplerStretch_loud.gain(1.f).play(
                        sound_flags::exclusive);
                } else {
                    // fallback on the standard sound
                    couplersounds.dsbCouplerStretch.gain(1.f).play(
                        sound_flags::exclusive);
                }
            } else {
                // basic clash
...
double TDynamicObject::tracing_offset() const {
...
double TDynamicObject::radius() const {
    glm::vec3 diagonal(
...
void TDynamicObject::LoadMMediaFile(
    std::string const &TypeName, std::string const &ReplacableSkin) {
    ...
    pants = NULL;  // wskaźnik pierwszego obiektu animującego dla pantografów
    {
        ...
        if (false == parser.ok()) {
            ErrorLog(
                "Failed to load appearance data for vehicle " +
                MoverParameters->Name);
            return;
        }
    }
    ...
    cParser parser(
        "include " + TypeName + ".mmd" + " " + asName  // (p1)
            + " " + TypeName                           // (p2)
            + " " + ReplacableSkin                     // (p3)
            + " end",
        cParser::buffer_TEXT, asBaseDir);
    std::string token;
    do {
        token = "";
        parser.getTokens();
        ...
        if (token == "models:") {
            // modele i podmodele
            m_materialdata.multi_textures =
                0;  // czy jest wiele tekstur wymiennych?
            parser.getTokens();
            parser >> asModel;
            ...
            // name can contain leading slash, erase it to avoid creation of
            // double slashes when the name is combined with current directory
            erase_leading_slashes(asModel);
            /*
            // never really used, may as well get rid of it
            std::size_t i = asModel.find( ',' );
            if ( i != std::string::npos )
            { // Ra 2015-01: może szukać przecinka w nazwie modelu, a po
            przecinku była by liczba tekstur? if( i < asModel.length() ) {
                    m_materialdata.multi_textures = asModel[ i + 1 ] - '0';
                }
                m_materialdata.multi_textures = clamp(
            m_materialdata.multi_textures, 0, 1 ); // na razie ustawiamy na 1
            }
            */
            asModel =
                asBaseDir + asModel;  // McZapkie 2002-07-20: dynamics maja
                                      // swoje modele w dynamics/basedir
            Global.asCurrentTexturePath =
                asBaseDir;  // biezaca sciezka do tekstur to dynamic/...
            ...
            do {
                ...
                if (token == "animations:") {
                    // Ra: ustawienie ilości poszczególnych animacji - musi być
                    // jako pierwsze, inaczej ilości będą domyślne
                    /*
                                        if( nullptr == pAnimations )
                    */
                    if (true ==
                        pAnimations
                            .empty()) {  // jeśli nie ma jeszcze tabeli
                                         // animacji, można odczytać nowe ilości
                        int co = 0;
                        iAnimations = 0;
                        int ile;
                        do {  // kolejne liczby to ilość animacj, -1 to znacznik
                              // końca
                            ile = -1;
                            ...
                            ++co;
                                 ...
                                 (co < ANIM_TYPES));  //-1 to znacznik końca
                        ...
                        for (j = 0; j < ANIM_TYPES; ++j)
                            for (i = 0; i < iAnimType[j]; ++i) {
                                if (j ==
                                    ANIM_PANTS)  // zliczamy poprzednie animacje
                                    if (!pants)
                                        if (iAnimType
                                                [ANIM_PANTS])  // o ile jakieś
                                                               // pantografy są
                                                               // (a domyślnie
                                                               // są)
                                            pants =
                                ...
                                sm += pAnimations[k++].TypeSet(
                                    j);  // ustawienie typu animacji i zliczanie
                                         // tablicowanych submodeli
                            }
                        if (sm)  // o ile są bardziej złożone animacje
                        {
                            pAnimated = new TSubModel
                                *[sm];  // tabela na animowane submodele
                            for (k = 0; k < iAnimations; ++k)
                                pAnimations[k].smElement =
                                    pAnimated +
                                    pAnimations[k]
                                        .iShift;  // przydzielenie wskaźnika do
                                                  // tabelki
                        }
                    }
                }
                    ...
                    erase_leading_slashes(asModel);
                    asModel =
                        asBaseDir + asModel;  // McZapkie-200702 - dynamics maja
                                              // swoje modele w dynamic/basedir
                    Global.asCurrentTexturePath =
                        asBaseDir;  // biezaca sciezka do tekstur to dynamic/...
                    ...
                    // content provided as a series of values together enclosed
                    // in "{}" each value is a name of additional 3d model value
                    // can be optionally set of values enclosed in "[]" in which
                    // case one value will be picked randomly TBD: reconsider
                    // something more yaml-compliant and/or ability to define
                    // offset and rotation
                    while (((token = deserialize_random_set(parser)) != "") &&
                        ...
                        Global.asCurrentTexturePath =
                            asBaseDir;  // biezaca sciezka do tekstur to
                                        // dynamic/...
                        auto *attachmentmodel{
                            TModelsManager::GetModel(asBaseDir + token, true)};
                    ...
                    // content provided as "key: value" pairs together enclosed
                    // in "{}" value can be optionally set of values enclosed in
                    // "[]" in which case one value will be picked randomly
                    while (((token = parser.getToken<std::string>()) != "") &&
                           (token != "}")) {
                        if (token.back() == ':') {
                            auto loadmodel{deserialize_random_set(parser)};
                            replace_slashes(loadmodel);
                            LoadModelOverrides.emplace(
                                token.erase(token.size() - 1), loadmodel);
                        }
                    }
                }
                    ...
                    {  // McZapkie-050402: wyszukiwanie kol o nazwie str*
                        ...
                        pAnimations[i].smAnimated =
                            GetSubmodelFromName(mdModel, asAnimName);
                        if (pAnimations[i].smAnimated) {  //++iAnimatedAxles;
                            pAnimations[i]
                                .smAnimated
                                ->WillBeAnimated();  // wyłączenie optymalizacji
                                                     // transformu
                            pAnimations[i].yUpdate = std::bind(
                                &TDynamicObject::UpdateAxle, this,
                                std::placeholders::_1);
                            pAnimations[i].fMaxDist =
                                Global.fDistanceFactor *
                                MoverParameters->WheelDiameter * 200;
                    ...
                    for (i = 0; i < iAnimType[ANIM_WHEELS];
                        ...
                        pAnimations[i].dWheelAngle =
                            1;  // domyślnie wskaźnik na napędzające
                    i = 0;
                    j = 1;
                    k = 0;
                    m = 0;  // numer osi; kolejny znak; ile osi danego typu;
                            // która średnica
                    if ((MoverParameters->WheelDiameterL !=
                         MoverParameters->WheelDiameter) ||
                        (MoverParameters->WheelDiameterT !=
                         MoverParameters
                             ->WheelDiameter)) {  // obsługa różnych średnic, o
                                                  // ile występują
                        while (
                            (i < iAnimType[ANIM_WHEELS]) &&
                            (j <= MoverParameters->AxleArangement
                            ...
                            {
                                pAnimations[i++].dWheelAngle =
                                    1;  // obrót osi napędzających
                                --k;  // następna będzie albo taka sama, albo
                                      // bierzemy kolejny znak
                                m = 2;  // następujące toczne będą miały inną
                                        // średnicę
                            } else if ((k >= '1') && (k <= '9')) {
                                pAnimations[i++].dWheelAngle =
                                    m;  // obrót osi tocznych
                                --k;  // następna będzie albo taka sama, albo
                                      // bierzemy kolejny znak
                            } else
                                k = MoverParameters->AxleArangement
                                        [j++];  // pobranie kolejnego znaku
                        }
                    }
                }
                // else if (str==AnsiString("animrodprefix:")) //prefiks
                // wiazarow dwoch
                // {
                //  str= Parser->GetNextSymbol();
                //  for (int i=1; i<=2; i++)
                //  {//McZapkie-050402: wyszukiwanie max 2 wiazarow o nazwie
                //  str*
                //   asAnimName=str+i;
                ...
                // Pantografy - Winger 160204
                    ...
                    parser.getTokens();
                    parser >> token;
                    float4x4 m;  // macierz do wyliczenia pozycji i wektora
                                 // ruchu pantografu
                    TSubModel *sm;
                    if (pants)
                        for (int i = 0; i < iAnimType[ANIM_PANTS];
                             i++) {  // Winger 160204: wyszukiwanie max 2
                                     // patykow o nazwie str*
                            asAnimName = token + std::to_string(i + 1);
                            sm = GetSubmodelFromName(mdModel, asAnimName);
                            pants[i].smElement[0] =
                                sm;    // jak NULL, to nie będzie animowany
                            if (sm) {  // w EP09 wywalało się tu z powodu NULL
                                sm->WillBeAnimated();
                                sm->ParentMatrix(
                                    &m);  // pobranie macierzy transformacji
                                // m(3)[1]=m[3][1]+0.054; //w górę o wysokość
                                // ślizgu (na razie tak)
                                pants[i].fParamPants->vPos.z =
                                    m[3][0];  // przesunięcie w bok (asymetria)
                                pants[i].fParamPants->vPos.y =
                                    m[3][1];  // przesunięcie w górę odczytane z
                                              // modelu
                                if ((sm = pants[i].smElement[0]->ChildGet()) !=
                                    NULL) {  // jeśli ma potomny, można policzyć
                                             // długość (odległość potomnego od
                                             // osi obrotu)
                                    m = float4x4(
                                        *sm->GetMatrix());  // wystarczyłby
                                                            // wskaźnik, nie
                                                            // trzeba kopiować
                                    // może trzeba: pobrać macierz dolnego
                                    // ramienia, wyzerować przesunięcie,
                                    // przemnożyć przez macierz górnego
                                    pants[i].fParamPants->fHoriz =
                                        -fabs(m[3][1]);
                                    pants[i].fParamPants->fLenL1 = hypot(
                                        m[3][1],
                                        m[3][2]);  // po osi OX nie potrzeba
                                    pants[i].fParamPants->fAngleL0 =
                                        atan2(fabs(m[3][2]), fabs(m[3][1]));
                                    // if
                                    // (pants[i].fParamPants->fAngleL0<M_PI_2)
                                    // pants[i].fParamPants->fAngleL0+=M_PI;
                                    // //gdyby w odwrotną stronę wyszło if
                                    ...
                                    pants[i].fParamPants->fAngleL =
                                        pants[i]
                                            .fParamPants
                                            ->fAngleL0;  // początkowy kąt
                                                         // dolnego ramienia
                                    if ((sm = sm->ChildGet()) !=
                                        NULL) {  // jeśli dalej jest ślizg,
                                                 // można policzyć długość
                                                 // górnego ramienia
                                        m = float4x4(
                                            *sm->GetMatrix());  // wystarczyłby
                                                                // wskaźnik,
                                        // nie trzeba kopiować trzeba by
                                        // uwzględnić macierz dolnego ramienia,
                                        // żeby uzyskać kąt do poziomu...
                                        pants[i].fParamPants->fHoriz +=
                                            fabs(m(3)[1]);  // różnica długości
                                                            // rzutów ramion na
                                        // płaszczyznę podstawy (jedna dodatnia,
                                        // druga ujemna)
                                        pants[i].fParamPants->fLenU1 = hypot(
                                            m[3][1],
                                            m[3][2]);  // po osi OX nie potrzeba
                                        ...
                                        // //górne ramię
                                        pants[i].fParamPants->fAngleU0 = atan2(
                                            fabs(m[3][2]),
                                            fabs(m[3]
                                                  [1]));  // początkowy kąt
                                                          // górnego ramienia,
                                                          // odczytany z modelu
                                        // if
                                        // (pants[i].fParamPants->fAngleU0<M_PI_2)
                                        // pants[i].fParamPants->fAngleU0+=M_PI;
                                        // //gdyby w odwrotną stronę wyszło if
                                        // (pants[i].fParamPants->fAngleU0<0)
                                        ...
                                        pants[i].fParamPants->fAngleU =
                                            pants[i]
                                                .fParamPants
                                                ->fAngleU0;  // początkowy kąt
                                        // Ra: ze względu na to, że niektóre
                                        // modele pantografów są zrąbane, ich
                                        // mierzenie ma obecnie ograniczony sens
                                        sm->ParentMatrix(
                                            &m);  // pobranie macierzy
                                                  // transformacji pivota ślizgu
                                                  // względem wstawienia pojazdu
                                        float det = Det(m);
                                        if (std::fabs(det - 1.0) <
                                            0.001)  // dopuszczamy 1 promil
                                                    // błędu na skalowaniu
                                                    // ślizgu
                                        {  // skalowanie jest w normie, można
                                           // pobrać wymiary z modelu
                                        ...
                                        } else {  // gdy ktoś przesadził ze
                                                  // skalowaniem
                                            pants[i].fParamPants->fHeight =
                                                0.0;  // niech będzie odczyt z
                                                      // pantfactors:
                                            ErrorLog(
                                                "Bad model: " + asModel +
                                                    ", scale of " +
                                ...
                                ErrorLog(
                                    "Bad model: " + asFileName +
                                        " - missed submodel " + asAnimName,
                                    logtype::model);
                            }
                        }
                }
                    ...
                    if (pants) {
                        for (int i = 0; i < iAnimType[ANIM_PANTS]; i++) {
                            // Winger 160204: wyszukiwanie max 2 patykow o
                            // nazwie str*
                            asAnimName = token + std::to_string(i + 1);
                            sm = GetSubmodelFromName(mdModel, asAnimName);
                            pants[i].smElement[1] =
                            ...
                            if (sm) {  // w EP09 wywalało się tu z powodu NULL
                                ...
                                if (pants[i].fParamPants->vPos.y == 0.0) {
                                    ...
                                    pants[i].fParamPants->vPos.z =
                                        m[3]
                                         [0];  // przesunięcie w bok (asymetria)
                                    pants[i].fParamPants->vPos.y =
                                        m[3][1];  // przesunięcie w górę
                                                  // odczytane z modelu
                                }
                            } else
                                ErrorLog(
                    ...
                    if (pants) {
                        for (int i = 0; i < iAnimType[ANIM_PANTS]; i++) {
                            ...
                            if (pants[i].smElement[4]) {
                                ...
                                pants[i].yUpdate = std::bind(
                                    &TDynamicObject::UpdatePant, this,
                                    std::placeholders::_1);
                                pants[i].fMaxDist =
                                    300 *
                                    300;  // nie podnosić w większej odległości
                                pants[i].iNumber = i;
                            } else {
                                ErrorLog(
                                    "Bad model: " + asFileName +
                    ...
                    if ((pant1x < 0) &&
                        (pant2x >
                         0))  // pierwsza powinna być dodatnia, a druga ujemna
                    {
                        pant1x = -pant1x;
                        pant2x = -pant2x;
                    }
                    if (pants) {
                        for (int i = 0; i < iAnimType[ANIM_PANTS]; ++i) {
                            // przepisanie współczynników do pantografów (na
                            // razie nie będzie lepiej)
                            auto &pantograph{*(pants[i].fParamPants)};

                            pantograph.fAngleL =
                                pantograph.fAngleL0;  // początkowy kąt dolnego
                                                      // ramienia
                            pantograph.fAngleU =
                                pantograph.fAngleU0;  // początkowy kąt
                            ...
                            // //wysokość początkowa
                            if (pantograph.fHeight ==
                                0.0)  // gdy jest nieprawdopodobna wartość (np.
                                      // nie znaleziony ślizg)
                            {  // gdy pomiary modelu nie udały się, odczyt
                               // podanych parametrów z MMD
                                pantograph.vPos.x = (i & 1) ? pant2x : pant1x;
                                pantograph.fHeight =
                                    (i & 1) ? pant2h
                                            : pant1h;  // wysokość ślizgu jest
                                                       // zapisana w MMD
                            }
                            pantograph.PantWys =
                            ...
                            if (pantograph.vPos.y == 0.0) {
                                // crude fallback, place the pantograph(s) atop
                                // of the vehicle-sized box
                                pantograph.vPos.y = MoverParameters->Dim.H -
                                                    pantograph.fHeight -
                                                    pantograph.PantWys;
                            }
                            // pants[i].fParamPants->vPos.z=0; //niezerowe dla
                            // pantografów asymetrycznych
                            pantograph.PantTraction = pantograph.PantWys;
                            // połowa szerokości ślizgu; jest w "Power: CSW="
                            pantograph.fWidth =
                                0.5 * MoverParameters->EnginePowerSource
                                          .CollectorParameters.CSW;
                    ...
                    // silnika parowego
                    // (tłoka)
                    /* //Ra: tymczasowo wyłączone ze względu na porządkowanie
                       animacji pantografów
                             pant1x=Parser->GetNextSymbol().ToDouble(); //kąt
                       przesunięcia dla pierwszego tłoka
                             pant2x=Parser->GetNextSymbol().ToDouble(); //kąt
                       przesunięcia dla drugiego tłoka
                             panty=Parser->GetNextSymbol().ToDouble(); //długość
                       korby (r) panth=Parser->GetNextSymbol().ToDouble();
                       //długoś korbowodu (k)
                    */
                    MoverParameters->EnginePowerSource.PowerType =
                        TPowerType::SteamPower;  // Ra: po chamsku, ale z CHK
                                                 // nie działa
                }

                else if (token == "animreturnprefix:") {
                    // prefiks drążka mimośrodowego - na razie używamy modeli
                    // pantografów
                    parser.getTokens(1, false);
                    parser >> token;
                    for (int i = 1; i <= 2; i++) {
                    ...
                    asAnimName = "";
                    for (int i = 1; i <= 4; ++i) {
                        // McZapkie-050402: wyszukiwanie max 4 wahaczy o nazwie
                        // str*
                    ...
                    {  // NBMX wrzesien 2003: wyszukiwanie drzwi o nazwie str*
                       // ustalenie submodelu
                        ...
                        pAnimations[i + j].smAnimated =
                            GetSubmodelFromName(mdModel, asAnimName);
                        if (pAnimations[i + j]
                                .smAnimated) {  //++iAnimatedDoors;
                            pAnimations[i + j]
                                .smAnimated
                                ->WillBeAnimated();  // wyłączenie optymalizacji
                                                     // transformu
                            switch (MoverParameters->Doors
                                        .type) {  // od razu zapinamy potrzebny
                                                  // typ animacji
                                case 1:
                                    pAnimations[i + j].yUpdate = std::bind(
                                        &TDynamicObject::UpdateDoorTranslate,
                                        this, std::placeholders::_1);
                                    break;
                                case 2:
                                    pAnimations[i + j].yUpdate = std::bind(
                                        &TDynamicObject::UpdateDoorRotate, this,
                                        std::placeholders::_1);
                                    break;
                                case 3:
                                    pAnimations[i + j].yUpdate = std::bind(
                                        &TDynamicObject::UpdateDoorFold, this,
                                        std::placeholders::_1);
                                    break;  // obrót 3 kolejnych submodeli
                                case 4:
                                    pAnimations[i + j].yUpdate = std::bind(
                                        &TDynamicObject::UpdateDoorPlug, this,
                                        std::placeholders::_1);
                                    break;
                                default:
                                    break;
                            ...
                            pAnimations[i + j].iNumber =
                                i +
                                1;  // parzyste działają inaczej niż nieparzyste
                            pAnimations[i + j].fMaxDist =
                                300 * 300;  // drzwi to z daleka widać
                                            /*
                                                                        // NOTE: no longer used
                                                                        pAnimations[i +
                    ...
                    for (i = 0, j = 0; i < ANIM_DOORSTEPS; ++i)
                        j += iAnimType[i];  // zliczanie wcześniejszych animacji
                    for (i = 0; i < iAnimType[ANIM_DOORSTEPS];
                         ++i)  // liczba drzwi
                    {  // NBMX wrzesien 2003: wyszukiwanie drzwi o nazwie str*
                       // ustalenie submodelu
                        asAnimName = token + std::to_string(i + 1);
                        ...
                        if (pAnimations[i + j]
                            ...
                            pAnimations[i + j]
                                ...
                                ->WillBeAnimated();  // wyłączenie optymalizacji
                                                     // transformu
                            switch (MoverParameters->Doors
                                        .step_type) {  // od razu zapinamy
                                                       // potrzebny typ animacji
                                case 1:                // shift
                                    pAnimations[i + j].yUpdate = std::bind(
                                        &TDynamicObject::
                                            UpdatePlatformTranslate,
                                        this, std::placeholders::_1);
                                    break;
                                case 2:  // rotate
                                    pAnimations[i + j].yUpdate = std::bind(
                                        &TDynamicObject::UpdatePlatformRotate,
                                        this, std::placeholders::_1);
                                    break;
                                default:
                                    break;
                            ...
                            pAnimations[i + j].iNumber =
                                ...
                                1;  // parzyste działają inaczej niż nieparzyste
                            pAnimations[i + j].fMaxDist =
                                150 * 150;  // drzwi to z daleka widać
                                            /*
                                                                        // NOTE: no longer used
                                                                        pAnimations[i +
                                               j].fSpeed = Random(150); // oryginalny koncept z
                    ...
                    for (i = 0, j = 0; i < ANIM_MIRRORS; ++i)
                        j += iAnimType[i];  // zliczanie wcześniejszych animacji
                    for (i = 0; i < iAnimType[ANIM_MIRRORS];
                         ++i)  // liczba drzwi
                    {  // NBMX wrzesien 2003: wyszukiwanie drzwi o nazwie str*
                       // ustalenie submodelu
                        asAnimName = token + std::to_string(i + 1);
                        ...
                        if (pAnimations[i + j].smAnimated) {
                            ...
                            auto const offset{
                                pAnimations[i + j].smAnimated->offset()};
                            pAnimations[i + j].yUpdate = std::bind(
                                &TDynamicObject::UpdateMirror, this,
                                std::placeholders::_1);
                            // we don't expect more than 2-4 mirrors, so it
                            // should be safe to store submodel location
                            // (front/rear) in the higher bits parzyste działają
                            // inaczej niż nieparzyste
                            pAnimations[i + j].iNumber =
            ...
            do {
                ...
                if (token == "wheel_clatter:") {
                    ...
                    while (((token = parser.getToken<std::string>()) != "") &&
                        ...
                        axle_sounds axle{
                            ...
                            std::atof(token.c_str()) *
                                -1.0,  // for axle locations negative value
                                       // means ahead of centre but vehicle
                                       // faces +Z in 'its' space
                            {sound_placement::external,
                             static_cast<float>(dSDist)}};
                ...
                else if ((token == "engine:") && (MoverParameters->Power > 0)) {
                    // plik z dzwiekiem silnika, mnozniki i ofsety amp. i czest.
                    m_powertrainsounds.engine.deserialize(
                        parser, sound_type::single,
                        sound_parameters::range | sound_parameters::amplitude |
                            sound_parameters::frequency);
                    ...
                    auto const amplitudedivisor = static_cast<float>((
                        MoverParameters->EngineType == TEngineType::DieselEngine
                            ? 1
                        : MoverParameters->EngineType ==
                                TEngineType::DieselElectric
                            ? 1
                            : MoverParameters->nmax * 60 +
                                  MoverParameters->Power * 3));
                    m_powertrainsounds.engine.m_amplitudefactor /=
                        amplitudedivisor;

                } else if (token == "fakeengine:") {
                    m_powertrainsounds.fake_engine.deserialize(
                        parser, sound_type::single);
                    ...
                    m_powertrainsounds.engine_revving.deserialize(
                        parser, sound_type::single, sound_parameters::range);
                    m_powertrainsounds.engine_revving.owner(this);
                }

                else if (token == "oilpump:") {
                    m_powertrainsounds.oil_pump.deserialize(
                        parser, sound_type::single);
                    m_powertrainsounds.oil_pump.owner(this);
                }

                else if (token == "fuelpump:") {
                    m_powertrainsounds.fuel_pump.deserialize(
                        parser, sound_type::single);
                    m_powertrainsounds.fuel_pump.owner(this);
                }

                else if (token == "waterpump:") {
                    m_powertrainsounds.water_pump.deserialize(
                        parser, sound_type::single);
                    m_powertrainsounds.water_pump.owner(this);
                }

                else if (token == "waterheater:") {
                    m_powertrainsounds.water_heater.deserialize(
                        parser, sound_type::single);
                    ...
                    if (true == motors.empty()) {
                        // fallback for cases without specified motor locations,
                        // convert sound template to a single sound source
                        motors.emplace_back(motortemplate);
                    } else {
                        // apply configuration to all defined motors
                        for (auto &motor : motors) {
                            // combine potential x- and y-axis offsets of the
                            // sound template with z-axis offsets of individual
                            // motors
                            auto motoroffset{motortemplate.offset()};
                            motoroffset.z = motor.offset().z;
                            motor = motortemplate;
                            motor.offset(motoroffset);
                            // apply randomized playback start offset for each
                            // instance, to reduce potential reverb with
                    ...
                    auto const amplitudedivisor{static_cast<float>(
                        MoverParameters->MotorBlowers[end::front].speed > 0
                            ? MoverParameters->MotorBlowers[end::front].speed *
                                      MoverParameters->nmax * 60 +
                                  MoverParameters->Power * 3
                        : blowertemplate.has_bookends()
                            ? 1
                            :  // NOTE: for motorblowers with fixed speed if the
                               // sound has defined bookends we skip
                               // revolutions-based part of frequency/volume
                               // adjustments
                            MoverParameters->MotorBlowers[end::front].speed *
                                -1)};
                    ...
                    if (true == m_powertrainsounds.motors.empty()) {
                        // fallback for cases without specified motor locations,
                        // convert sound template to a single sound source
                        m_powertrainsounds.motorblowers.emplace_back(
                            blowertemplate);
                    } else {
                    ...
                    // plik z dzwiekiem wentylatora, mnozniki i ofsety amp. i
                    // czest.
                    m_powertrainsounds.inverter.deserialize(
                        parser, sound_type::single,
                        sound_parameters::range | sound_parameters::amplitude |
                            sound_parameters::frequency);
                    ...
                    // plik z dzwiekiem wentylatora, mnozniki i ofsety amp. i
                    // czest.
                    m_powertrainsounds.rsWentylator.deserialize(
                        parser, sound_type::single,
                        sound_parameters::range | sound_parameters::amplitude |
                            sound_parameters::frequency);
                    m_powertrainsounds.rsWentylator.owner(this);

                    m_powertrainsounds.rsWentylator.m_amplitudefactor /=
                        MoverParameters->RVentnmax;
                    m_powertrainsounds.rsWentylator.m_frequencyfactor /=
                        MoverParameters->RVentnmax;
                }

                else if (token == "radiatorfan1:") {
                    // primary circuit radiator fan
                    m_powertrainsounds.radiator_fan.deserialize(
                        parser, sound_type::single);
                    ...
                    rsBrake.deserialize(
                        parser, sound_type::single,
                        sound_parameters::amplitude |
                            sound_parameters::frequency);
                    rsBrake.owner(this);
                    // NOTE: can't pre-calculate amplitude normalization based
                    // on max brake force, as this varies depending on vehicle
                    // speed
                    ...
                    if (rsPisk.m_amplitudefactor > 10.f) {
                        // HACK: convert old style activation point threshold to
                        // the new, regular amplitude adjustment system
                    ...
                    m_brakecylinderpistonadvance.deserialize(
                        parser, sound_type::single);
                    ...
                    m_brakecylinderpistonrecede.deserialize(
                        parser, sound_type::single);
                    ...
                    // brake cylinder pressure increase sounds
                    m_epbrakepressureincrease.deserialize(
                        parser, sound_type::single);
                    ...
                    // brake cylinder pressure decrease sounds
                    m_epbrakepressuredecrease.deserialize(
                        parser, sound_type::single);
                ...
                // spring brake sounds
                    ...
                    // pliki z trabieniem
                    sHorn1.deserialize(
                        parser, sound_type::multipart, sound_parameters::range);
                    sHorn1.owner(this);
                }
                    ...
                    if (iHornWarning) {
                        iHornWarning = 2;  // numer syreny do użycia po
                                           // otrzymaniu sygnału do jazdy
                    }
                }
                    ...
                    m_powertrainsounds.engine_turbo.deserialize(
                        parser, sound_type::multipart, sound_parameters::range);
                    ...
                    soundtemplate.owner(this);
                    for (auto &door : m_doorsounds) {
                        // apply configuration to all defined doors, but
                        // preserve their individual offsets
                    ...
                    m_exchangesounds.unloading.range(
                        MoverParameters->Dim.L * 0.5f * -1);
                    m_exchangesounds.unloading.deserialize(
                        parser, sound_type::single);
                    m_exchangesounds.unloading.owner(this);
                }

                else if (token == "loading:") {
                    m_exchangesounds.loading.range(
                        MoverParameters->Dim.L * 0.5f * -1);
                    m_exchangesounds.loading.deserialize(
                        parser, sound_type::single);
                    ...
                    // szum podczas jazdy:
                    sound_source noisetemplate{
                        sound_placement::external,
                        EU07_SOUND_RUNNINGNOISECUTOFFRANGE};
                    noisetemplate.deserialize(
                        parser, sound_type::single,
                        sound_parameters::amplitude |
                            sound_parameters::frequency,
                    ...
                    noisetemplate.m_amplitudefactor /=
                        (1 + MoverParameters->Vmax);
                    noisetemplate.m_frequencyfactor /=
                        (1 + MoverParameters->Vmax);
#ifdef EU07_SOUND_BOGIESOUNDS
                    if (true == m_bogiesounds.empty()) {
                        // fallback for cases without specified noise locations,
                        // convert sound template to a single sound source
                    ...
                    // apply randomized playback start offset for each bogie, to
                    // reduce potential reverb with identical nearby sources
                    auto bogieidx(0);
                    for (auto &bogie : m_bogiesounds) {
                        bogie.start(
                            (bogieidx % 2 ? LocalRandom(0.0, 30.0)
                    ...
                    // apply randomized playback start offset, to reduce
                    // potential reverb with identical nearby sources
                    ...
                    // in "{}" value can be optionally set of values enclosed in
                    // "[]" in which case one value will be picked randomly
                    std::unordered_map<std::string, announcement_t> const
                        announcements = {
                    ...
                    while (((token = parser.getToken<std::string>()) != "") &&
                        ...
                        if (token.back() == ':') {
                            if (token == "soundproofing:") {
                                // custom soundproofing in format [ p1, p2, p3,
                                // p4, p5, p6 ]
                                parser.getTokens(6, false, "\n\r\t ,;[]");
                                std::array<float, 6> soundproofing;
                                parser >> soundproofing[0] >>
                                    soundproofing[1] >> soundproofing[2] >>
                                    soundproofing[3] >> soundproofing[4] >>
                                    soundproofing[5];
                                for (auto &soundproofingelement :
                                     soundproofing) {
                                    if (soundproofingelement != -1.f) {
                                        soundproofingelement = std::sqrt(clamp(
                                            soundproofingelement, 0.f, 1.f));
                                    }
                                }
                            ...
                            auto const announcementtype{
                                (lookup != announcements.end()
                                     ? lookup->second
                                     : announcement_t::idle)};
                            // NOTE: we retrieve key value for all keys, not
                            // just recognized ones
                            if (announcementtype == announcement_t::idle) {
                                token = parser.getToken<std::string>();
                                continue;
                            }
                            /*
                                                        auto announcementsound {
                               deserialize_random_set( parser ) };
                                                        replace_slashes(
                               announcementsound );
                            */
                            sound_source soundtemplate{
                                sound_placement::engine};  // NOTE: sound range
                                                           // gets filled by pa
                                                           // system
                            soundtemplate.deserialize(
                                parser, sound_type::single);
                            soundtemplate.owner(this);
                            m_pasystem.announcements[static_cast<int>(
                                announcementtype)] = soundtemplate;
                        }
                    }
                    // set provided custom soundproofing to assigned sounds (for
                    // sounds without their own custom soundproofing)
                    if (m_pasystem.soundproofing) {
                        for (auto &announcement : m_pasystem.announcements) {
                            if (!announcement.soundproofing()) {
                                announcement.soundproofing() =
                                    m_pasystem.soundproofing;
                            }
                        }
                    }
            ...
            do {
                ...
                if (token == "doors:") {
                    // a list of pairs: offset along vehicle's z-axis and sides
                    // on which the door is present; followed with "end"
                    while (((token = parser.getToken<std::string>()) != "") &&
                           (token != "end")) {
                        // vehicle faces +Z in 'its' space, for door locations
                        // negative value means ahead of centre
                        auto const offset{std::atof(token.c_str()) * -1.f};
                        // recognized side specifications are "right", "left"
                        // and "both"
                        auto const sides{parser.getToken<std::string>()};
                        // NOTE: we skip setting owner of the sounds, it'll be
                        // done during individual sound deserialization
                        door_sounds door;
                        // add entries to the list:
                        if ((sides == "both") || (sides == "left")) {
                            // left...
                            auto const location{glm::vec3{
                                MoverParameters->Dim.W * 0.5f,
                                MoverParameters->Dim.H * 0.5f, offset}};
                        ...
                        if ((sides == "both") || (sides == "right")) {
                            // ...and right
                            auto const location{glm::vec3{
                                MoverParameters->Dim.W * -0.5f, 2.f, offset}};
                        ...
                        m_doorspeakers.emplace_back(
                            doorspeaker_sounds{{0.f, 3.f, offset}, {}});
                    }
                }

                else if (token == "tractionmotors:") {
                    // a list of offsets along vehicle's z-axis; followed with
                    // "end"
                    while (((token = parser.getToken<std::string>()) != "") &&
                           (token != "end")) {
                        // vehicle faces +Z in 'its' space, for motor locations
                        // negative value means ahead of centre
                        auto const offset{std::atof(token.c_str()) * -1.f};
                        // NOTE: we skip setting owner of the sounds, it'll be
                        // done during individual sound deserialization
                        sound_source motor{
                            sound_placement::external};  // generic traction
                                                         // motor sounds
                        sound_source acmotor{
                            sound_placement::external};  // inverter-specific
                                                         // traction motor
                                                         // sounds
                        sound_source motorblower{
                            sound_placement::engine};  // associated motor
                                                       // blowers
                    ...
                    while (((token = parser.getToken<std::string>()) != "") &&
                           (token != "end")) {
                        // vehicle faces +Z in 'its' space, for bogie locations
                        // negative value means ahead of centre
                        auto const offset{std::atof(token.c_str()) * -1.f};
                        // NOTE: we skip setting owner of the sounds, it'll be
                        // done during individual sound deserialization
                        sound_source bogienoise{
                            sound_placement::external,
                            EU07_SOUND_RUNNINGNOISECUTOFFRANGE};
                        // add entry to the list
                        auto const location{glm::vec3{0.f, 0.f, offset}};
            ...
            } while ((token != "") && (token != "endlocations"));
            ...
            do {
                ...
                // coupler sounds
                    ...
                    parser.getTokens(6, false);
                    parser >> BaseShake.jolt_scale.x >>
                        BaseShake.jolt_scale.y >> BaseShake.jolt_scale.z >>
                        BaseShake.jolt_limit >> BaseShake.angle_scale.x >>
                        BaseShake.angle_scale.z;
                } else if (token == "enginespring:") {
                    parser.getTokens(5, false);
                    parser >> EngineShake.scale >> EngineShake.fadein_offset >>
                    ...
                    parser.getTokens(4, false);
                    parser >> HuntingShake.scale >> HuntingShake.frequency >>
                        HuntingShake.fadein_begin >> HuntingShake.fadein_end;
                }
                    ...
                    for (auto &soundproofingtable : m_soundproofing) {
                        for (auto &soundproofingelement : soundproofingtable) {
                            ...
                            if (value != -1.f) {
                                soundproofingelement =
                                    std::sqrt(clamp(value, 0.f, 1.f));
                            }
                        }
                    }
                }
                    ...
                    parser >> InteriorLight.r >> InteriorLight.g >>
                        InteriorLight.b;
                    InteriorLight = glm::clamp(
                        InteriorLight / 255.f, glm::vec3(0.f), glm::vec3(1.f));
                    ...
                    if ((false == DestinationSign.script.empty()) &&
                        ...
                        DestinationSign.script =
                            asBaseDir + DestinationSign.script;
                    }
                }
                // NOTE: legacy key, now expected as optional "background:"
                // parameter in pydestinationsign: { parameter block }
    ...
    if (!iAnimations) {
        // if the animations weren't defined the model is likely to be
        // non-functional. warrants a warning.
        ErrorLog(
            "Animations tag is missing from the .mmd file \"" + asFileName +
            "\"");
    }

    if (ReplacableSkin != "none") {
        // potentially set blank destination texture
        DestinationSign.destination_off = DestinationFind(
            (DestinationSign.background.empty() ? "nowhere"
                                                : DestinationSign.background));
    }

    // assign default samples to sound emitters which weren't included in the
    // config file
    if (TestFlag(MoverParameters->CategoryFlag, 1)) {
        ...
        if (MoverParameters->Power > 0) {
            if (true == m_powertrainsounds.dsbWejscie_na_bezoporow.empty()) {
                ...
                m_powertrainsounds.dsbWejscie_na_bezoporow.deserialize(
                    "wejscie_na_bezoporow", sound_type::single);
                m_powertrainsounds.dsbWejscie_na_bezoporow.owner(this);
            }
            if (true == m_powertrainsounds.motor_parallel.empty()) {
                m_powertrainsounds.motor_parallel.deserialize(
                    "wescie_na_drugi_uklad", sound_type::single);
                m_powertrainsounds.motor_parallel.owner(this);
            }
        }
        // braking sounds
        ...
        for (auto &couplersounds : m_couplersounds) {
            if (true == couplersounds.attach_coupler.empty()) {
                couplersounds.attach_coupler.deserialize(
                    "couplerattach_default", sound_type::single);
                couplersounds.attach_coupler.owner(this);
            }
            if (true == couplersounds.detach_coupler.empty()) {
                couplersounds.detach_coupler.deserialize(
                    "couplerdetach_default", sound_type::single);
                couplersounds.detach_coupler.owner(this);
            }
            if (true == couplersounds.dsbCouplerStretch.empty()) {
                couplersounds.dsbCouplerStretch.deserialize(
                    "couplerstretch_default", sound_type::single);
                couplersounds.dsbCouplerStretch.owner(this);
            }
            if (true == couplersounds.dsbBufferClamp.empty()) {
                couplersounds.dsbBufferClamp.deserialize(
                    "bufferclamp_default", sound_type::single);
        ...
        if (true == m_wheelflat.empty()) {
            m_wheelflat.deserialize(
                "lomotpodkucia 0.23 0.0", sound_type::single,
                sound_parameters::frequency);
            m_wheelflat.owner(this);
        }
    ...
    if (mdModel)
        mdModel->Init();  // obrócenie modelu oraz optymalizacja, również
                          // zapisanie binarnego
    ...
    Global.asCurrentTexturePath =
        szTexturePath;  // kiedyś uproszczone wnętrze mieszało tekstury nieba
    ...
    auto const enginelocation{
        glm::vec3{0.f, MoverParameters->Dim.H * 0.5f, 0.f}};
    ...
    std::vector<sound_source *> enginesounds = {
        &sConverter,       &sCompressor, &sCompressorIdle,
        &sSmallCompressor, &sHeater,     &m_batterysound};
    ...
    if (pants != nullptr) {
        ...
        for (auto &pantographsounds : m_pantographsounds) {
            auto const pantographoffset{glm::vec3(
                0.f, pants[pantographindex].fParamPants->vPos.y,
    ...
    // couplers
    auto const frontcoupleroffset{
        glm::vec3{0.f, 1.f, MoverParameters->Dim.L * 0.5f}};
    ...
    m_couplersounds[end::front].dsbCouplerStretch_loud.offset(
        frontcoupleroffset);
...
TModel3d *TDynamicObject::LoadMMediaFile_mdload(std::string const &Name) const {
    ...
    if (loadmodel == nullptr) {
        auto const specializedloadfilename{
            asBaseDir + MoverParameters->TypeName + "_" + loadname};
        loadmodel =
            TModelsManager::GetModel(specializedloadfilename, true, false);
    }
    ...
    if (loadmodel != nullptr) loadmodel->GetSMRoot()->WillBeAnimated();

    return loadmodel;
}

//---------------------------------------------------------------------------
void TDynamicObject::RadioStop() {  // zatrzymanie pojazdu
    if (Mechanik) {
        ...
        if ((MoverParameters->SecuritySystem.radiostop_available()) &&
            ...
            // TODO: do it selectively for the 'local' driver once the
            // multiplayer is in
...
void TDynamicObject::SetLights() {
    auto const isfrontcaboccupied{
        MoverParameters->CabOccupied * DirectionGet() >= 0};
    int const automaticmarkers{
        MoverParameters->CabActive == 0 &&
                (MoverParameters->InactiveCabFlag & activation::redmarkers)
            ? light::redmarker_left + light::redmarker_right
            : 0};
    ...
    auto const frontlights{
        automaticmarkers > 0 ? automaticmarkers
                             : MoverParameters->Lights[front][lightpos]};
    auto const rearlights{
        automaticmarkers > 0 ? automaticmarkers
                             : MoverParameters->Lights[rear][lightpos]};
    auto *vehicle{GetFirstDynamic(
        MoverParameters->CabOccupied >= 0 ? end::front : end::rear,
        coupling::control)};
    while (vehicle != nullptr) {
        // set lights on given side if there's no coupling with another vehicle,
        // turn them off otherwise
        auto const *frontvehicle{
            (isfrontcaboccupied ? vehicle->Prev(coupling::coupler)
                                : vehicle->Next(coupling::coupler))};
        auto const *rearvehicle{
            (isfrontcaboccupied ? vehicle->Next(coupling::coupler)
                                : vehicle->Prev(coupling::coupler))};
        vehicle->RaLightsSet(
            (frontvehicle == nullptr ? frontlights : 0),
            (rearvehicle == nullptr ? rearlights : 0));
        vehicle =
            (isfrontcaboccupied ? vehicle->Next(coupling::control)
                                : vehicle->Prev(coupling::control));
    }
};

void TDynamicObject::RaLightsSet(
    int head, int rear) {  // zapalenie świateł z przodu i z
    // tyłu, zależne od kierunku
    // pojazdu
    if (!MoverParameters) return;  // może tego nie być na początku
    if (rear == (light::redmarker_left | light::redmarker_right |
                 light::rearendsignals)) {  // jeśli koniec pociągu, to trzeba
                                            // ustalić, czy
        // jest tam czynna lokomotywa
        // EN57 może nie mieć końcówek od środka członu
        if (MoverParameters->Power > 1.0)  // jeśli ma moc napędową
            if (!MoverParameters
                     ->DirActive)  // jeśli nie ma ustawionego kierunku
            {  // jeśli ma zarówno światła jak i końcówki, ustalić, czy jest w
               // stanie aktywnym
        ...
        if (rear == (light::redmarker_left | light::redmarker_right |
            ...
            if (iInventory[(iDirection ? end::rear : end::front)] &
                (light::redmarker_left | light::redmarker_right)) {
                // czy ma jakieś światła czerowone od danej strony
                rear =
                    (light::redmarker_left |
                     light::redmarker_right);  // dwa światła czerwone
            } else {
                rear = light::rearendsignals;  // tablice blaszane
            }
    }
...
bool TDynamicObject::has_signal_pc1_on() const {
    ...
    auto const pattern{
        equippedlights & (light::headlight_left | light::headlight_right |
                          light::headlight_upper)};
    auto const patternfallback{
        equippedlights & (light::auxiliary_left | light::auxiliary_right |
                          light::headlight_upper)};
    auto const hasauxiliarylights{
        (equippedlights & (light::auxiliary_left | light::auxiliary_right)) !=
        0};
    auto const activelights{MoverParameters->iLights[vehicleend]};

    return (
        ((pattern != 0) && (activelights == pattern)) ||
        ((hasauxiliarylights) && (activelights == patternfallback)) ||
        ((pattern == 0) && (patternfallback == 0) &&
         (activelights == light::rearendsignals)));  // pc4
}
...
bool TDynamicObject::has_signal_pc2_on() const {
    ...
    auto const equippedlights{iInventory[vehicleend]};
    auto const pattern{
        equippedlights & (light::redmarker_left | light::headlight_right |
                          light::headlight_upper)};
    auto const patternfallback{
        equippedlights & (light::redmarker_left | light::auxiliary_right |
                          light::headlight_upper)};
    auto const hasauxiliarylights{
        (equippedlights & (light::auxiliary_left | light::auxiliary_right)) !=
        0};
...
bool TDynamicObject::has_signal_pc5_on() const {
    ...
    auto const equippedlights{iInventory[vehicleend]};
    auto const pattern{
        equippedlights & (light::redmarker_left | light::redmarker_right)};
...
int TDynamicObject::DirectionSet(
    int d) {  // ustawienie kierunku w składzie (wykonuje AI)
    auto const lastdirection{iDirection};
    iDirection =
        (d > 0)
            ? 1
            : 0;  // d:1=zgodny,-1=przeciwny; iDirection:1=zgodny,0=przeciwny;

    if (iDirection != lastdirection) {
        // direction was flipped, switch recorded servicable platform sides for
        // potentially ongoing load exchange
    ...
    if (MyTrack) {  // podczas wczytywania wstawiane jest AI, ale może jeszcze
                    // nie być toru
...
TDynamicObject *TDynamicObject::Prev(int C) const {
    return (
        (MoverParameters->Couplers[iDirection ^ 1].CouplingFlag & C)
...
TDynamicObject *TDynamicObject::Next(int C) const {
    return (
        (MoverParameters->Couplers[iDirection].CouplingFlag & C)
            ? MoverParameters->Neighbours[iDirection].vehicle
            : nullptr);  // gdy sprzęg wirtualny, to jakby nic nie było
}

// checks whether there's unbroken connection of specified type to specified
// vehicle
bool TDynamicObject::is_connected(
    TDynamicObject const *Vehicle, coupling const Coupling) const {
    auto *vehicle{this};
    if (vehicle == Vehicle) {
        // edge case, vehicle is always "connected" with itself
        return true;
    }
    // check ahead, it's more likely the "owner" using this method is located
    // there
...
// ustalenie następnego (1) albo poprzedniego (0) w składzie bez względu na
// prawidłowość iDirection
TDynamicObject *TDynamicObject::Neighbour(int &dir) {
    auto *neighbour{
...
void TDynamicObject::update_neighbours() {
    for (int end = end::front; end <= end::rear; ++end) {
        ...
        if ((coupler.Connected != nullptr) && (neighbour.vehicle != nullptr)) {
            // physical connection with another vehicle locks down collision
            // source on this end
            //            neighbour.vehicle = coupler.Connected;
            //            neighbour.vehicle_end = coupler.ConnectedNr;
            neighbour.distance = TMoverParameters::CouplerDist(
                MoverParameters, coupler.Connected);
            // take into account potential adapters attached to the couplers
            auto const &othercoupler{neighbour.vehicle->MoverParameters
                                         ->Couplers[neighbour.vehicle_end]};
            neighbour.distance -= coupler.adapter_length;
            neighbour.distance -= othercoupler.adapter_length;
        } else {
            // if there's no connected vehicle check for potential collision
            // sources in the vicinity NOTE: we perform a new scan on each
            // update to ensure we always locate the nearest potential source
            neighbour = neighbour_data();
            // 10m ~= 140 km/h at 4 fps + safety margin, potential distance
            // between outer axles of involved vehicles
            auto const scanrange{
                std::max(10.0, std::abs(MoverParameters->V)) + 40.0};
            auto const lookup{find_vehicle(end, scanrange)};

            if (false == std::get<bool>(lookup)) {
                continue;
            }
            ...
            if ((neighbour.vehicle) &&
                (neighbour.distance <
                 (neighbour.vehicle->MoverParameters->CategoryFlag == 2
                      ? 50
                      : 100))) {
                // at short distances (re)calculate range between couplers
                // directly
                neighbour.distance = TMoverParameters::CouplerDist(
                    MoverParameters, neighbour.vehicle->MoverParameters);
                // take into account potential adapters attached to the couplers
                auto const &othercoupler{neighbour.vehicle->MoverParameters
                                             ->Couplers[neighbour.vehicle_end]};
                neighbour.distance -= coupler.adapter_length;
...
// locates potential collision source within specified range, scanning track in
// specified direction. returns: true if neighbour was located, false otherwise
// NOTE: reuses legacy code. TBD, TODO: review, refactor?
std::tuple<TDynamicObject *, int, double, bool> TDynamicObject::find_vehicle(
    int const Direction, double const Distance) const {
    auto direction{(Direction == end::front ? 1 : -1)};
    auto const initialdirection{
        direction};  // zapamiętanie kierunku poszukiwań na torze początkowym,
                     // względem sprzęgów
    ...
    // (teraz względem toru)
    auto const mycoupler{
        (initialdirection < 0 ? end::rear
                              : end::front)};  // numer sprzęgu do podłączenia w
                                               // obiekcie szukajacym
    auto foundcoupler{
        -1};  // numer sprzęgu w znalezionym obiekcie (znaleziony wypełni)
    auto distance{0.0};  // przeskanowana odleglość; odległość do zawalidrogi
    auto *foundobject{ABuFindObject(
        foundcoupler, distance, track, direction,
        mycoupler)};  // zaczynamy szukać na tym samym torze

    if (foundobject == nullptr) {
        // jeśli nie ma na tym samym, szukamy po okolicy szukanie najblizszego
        // toru z jakims obiektem praktycznie przeklejone z TraceRoute()...
        if (direction >=
            0)  // uwzględniamy kawalek przeanalizowanego wcześniej toru
            distance = track->Length() -
                       RaTranslationGet();  // odległość osi od Point2 toru
        else
        ...
        while (distance < Distance) {
            if (direction > 0) {
                ...
                if (track ? track->iNextDirection : false) {
                    // jeśli następny tor jest podpięty od Point2
                    direction = -direction;  // to zmieniamy kierunek szukania
                                             // na tym torze
                }
                if (track) {
                    track = track->CurrentNext();  // potem dopiero zmieniamy
                                                   // wskaźnik
                }
            } else {
                // w kierunku Point1
                if (track ? !track->iPrevDirection : true) {
                    // jeśli poprzedni tor nie jest podpięty od Point2
                    direction = -direction;  // to zmieniamy kierunek szukania
                                             // na tym torze
                }
                if (track) {
                    track = track->CurrentPrev();  // potem dopiero zmieniamy
                                                   // wskaźnik
                }
            }
            if (track) {
                // jesli jest kolejny odcinek toru
                foundobject = ABuFindObject(
                    foundcoupler, distance, track, direction,
                    mycoupler);  // przejrzenie pojazdów tego toru
                if (foundobject) {
                    break;
                }
            } else {
    ...
    /*
       ...
       odległość między nimi nie może być liczona w linii prostej!
            // NOTE: the distance is approximated, and additionally less
       accurate for cars heading in opposite direction distance -= ( 0.5 * (
       vehicle->Dim.L + foundvehicle->Dim.L ) );
        }
        else if( distance < 100.0 ) {
            // at short distances start to calculate range between couplers
       directly
...
TDynamicObject *TDynamicObject::FindPowered() {  // taka proteza:
    // chcę podłączyć kabinę EN57 bezpośrednio z silnikowym, aby nie robić tego
    // przez ukrotnienie drugi silnikowy i tak musi być ukrotniony, podobnie jak
    // kolejna jednostka lepiej by było przesyłać komendy sterowania, co jednak
    // wymaga przebudowy transmisji komend (LD) problem się robi ze światłami,
    // które będą zapalane w silnikowym, ale muszą świecić się w rozrządczych
    // dla EZT światłą czołowe będą "zapalane w silnikowym", ale widziane z
    // rozrządczych również wczytywanie MMD powinno dotyczyć aktualnego członu
    ...
    // jeździć dobrze również hamowanie wykonuje się zaworem w członie, a nie w
    // silnikowym...
    auto const coupling{
        ((MoverParameters->TrainType == dt_EZT) ||
    ...
    auto *lookup{find_vehicle(coupling, [](TDynamicObject *vehicle) {
    ...
    return (
        lookup != nullptr
            ? lookup
            : this);  // always return valid vehicle for backward compatibility
}

TDynamicObject *TDynamicObject::FindPantographCarrier() {
    // try first within a single unit, broaden to all vehicles under our control
    // if first attempt fails
    std::array<coupling, 2> const couplings = {
        coupling::permanent, coupling::control};

    for (auto const coupling : couplings) {
        auto *result = find_vehicle(coupling, [](TDynamicObject *vehicle) {
            return (
                (vehicle->MoverParameters->EnginePowerSource.SourceType ==
                 TPowerSource::CurrentCollector) &&
                (vehicle->MoverParameters->EnginePowerSource.CollectorParameters
                     .CollectorsNo > 0));
        });
        if (result != nullptr) {
            return result;
...
void TDynamicObject::ParamSet(
    int what,
    int into) {  // ustawienie lokalnego parametru (what) na stan (into)
    switch (what & 0xFF00) {
        case 0x0100:  // to np. są drzwi, bity 0..7 określają numer 1..254 albo
                      // maskę
            // dla 8 różnych
            if (what & 1)  // na razie mamy lewe oraz prawe, czyli używamy maskę
                           // 1=lewe,
                // 2=prawe, 3=wszystkie
                if (MoverParameters->Doors.instances[side::left]
                        .is_open) {  // są otwarte
            ...
            if (what & 2)  // prawe działają niezależnie od lewych
                if (MoverParameters->Doors.instances[side::right]
                        .is_open) {  // są otwarte
                    if (!into)       // jeśli zamykanie
                    {
                        // dźwięk zamykania
...
int TDynamicObject::RouteWish(TTrack *tr) {  // zapytanie do AI, po którym
...
void TDynamicObject::DestinationSet(std::string to, std::string numer) {
    // ustawienie stacji docelowej oraz wymiennej tekstury 4, jeśli istnieje
    // plik w zasadzie, to każdy wagon mógłby mieć inną stację docelową
    // zwłaszcza w towarowych, pod kątem zautomatyzowania maewrów albo pracy
    // górki ale to jeszcze potrwa, zanim będzie możliwe, na razie można wpisać
    // stację z rozkładu
    ...
    if (DestinationSign.script.empty()) {
        return;
    }  // no script so no way to make the texture
    if (numer == "none") {
        return;
    }  // blank or incomplete/malformed timetable, don't bother

    std::string signrequest{
        "make:" + DestinationSign.script +
        "?"
        // timetable include
        + "$timetable=" +
        (ctOwner == nullptr ? MoverParameters->Name
                            :  // leading vehicle, can point to it directly
             ctOwner->Vehicle()
                 ->MoverParameters
                 ->Name)  // owned vehicle, safer to point to owner as carriages
                          // can have identical names
        // basic instancing string
        // NOTE: underscore doesn't have any magic meaning for the time being,
        // it's just less likely to conflict with regular dictionary keys
        + "&_id1=" +
        (ctOwner != nullptr    ? ctOwner->TrainName()
         : Mechanik != nullptr ? Mechanik->TrainName()
                               : "none")};  // shouldn't get here but, eh
    // TBD, TODO: replace instancing with support for variables in extra
    // parameters string?
...
material_handle TDynamicObject::DestinationFind(std::string Destination) {
    ...
    // destination textures are kept in the vehicle's directory so we point the
    // current texture path there
    ...
    if (starts_with(Destination, "make:")) {
        ...
        // regular texture
        Destination =
            Bezogonkow(Destination);  // do szukania pliku obcinamy ogonki
        // now see if we can find any version of the texture
        std::vector<std::string> const destinations{
            Destination + '@' + MoverParameters->TypeName, Destination};
...
void TDynamicObject::announce(
    announcement_t const Announcement, bool const Chime) {
    if (m_doorspeakers.empty()) {
        return;
    }

    auto const *driver{(ctOwner != nullptr ? ctOwner : Mechanik)};
    if (driver == nullptr) {
        return;
    }
    ...
    // if the announcement sound was defined queue playback
    {
        ...
        if ((true == playchime) &&
            (false ==
             announcements[static_cast<int>(announcement_t::chime)].empty())) {
            m_pasystem.announcement_queue.emplace_back(
                announcements[static_cast<int>(announcement_t::chime)]);
            playchime = false;
        }
        // ...then play the announcement itself
        m_pasystem.announcement_queue.emplace_back(
            announcements[static_cast<int>(Announcement)]);
...
void TDynamicObject::OverheadTrack(float o) {  // ewentualne wymuszanie jazdy
    ...
    {  // trzeba zaktualizować mapę flag bitowych jazdy bezprądowej
        if (o < 0.0) {  // normalna jazda po tym torze
            ctOwner->iOverheadZero &=
                ~iOverheadMask;  // zerowanie bitu - może pobierać prąd
            ctOwner->iOverheadDown &=
                ~iOverheadMask;  // zerowanie bitu - może podnieść pantograf
        } else if (o > 0.0) {    // opuszczenie pantografów
            ctOwner->iOverheadZero |=
                iOverheadMask;  // ustawienie bitu - ma jechać bez pobierania
                                // prądu
            ctOwner->iOverheadDown |=
                iOverheadMask;  // ustawienie bitu - ma opuścić pantograf
        } else {                // jazda bezprądowa z podniesionym pantografem
            ctOwner->iOverheadZero |=
                iOverheadMask;  // ustawienie bitu - ma jechać bez pobierania
                                // prądu
            ctOwner->iOverheadDown &=
...
void TDynamicObject::update_shake(double const Timedelta) {
    ...
    if (Global.iSlowMotion == 0) {  // musi być pełna prędkość
        ...
        if ((MoverParameters->EngineType == TEngineType::DieselElectric) ||
            ...
            if (std::abs(MoverParameters->enrot) > 0.0) {
                ...
                shakevector.x +=
                    ...
                    * clamp(
                          (MoverParameters->enrot - EngineShake.fadein_offset) *
                              EngineShake.fadein_factor,
                          0.0, 1.0)
        ...
        if ((HuntingShake.fadein_begin > 0.f) &&
            ...
            // hunting oscillation
            HuntingAngle = clamp_circular(
                HuntingAngle + 4.0 * HuntingShake.frequency * Timedelta *
                                   MoverParameters->Vel,
                360.0);
            auto const huntingamount = interpolate(
                0.0, 1.0,
                clamp(
                    (MoverParameters->Vel - HuntingShake.fadein_begin) /
                        (HuntingShake.fadein_end - HuntingShake.fadein_begin),
                    0.0, 1.0));
            shakevector.x += (std::sin(glm::radians(HuntingAngle)) * Timedelta *
                              HuntingShake.scale) *
                             huntingamount;
            IsHunting = (huntingamount > 0.025);
        }

        if (FreeFlyModeFlag) shakevector *= 0;

        auto const iVel{std::min(GetVelocity(), 150.0)};
        if (iVel > 0.5) {
            // acceleration-driven base shake
            shakevector += Math3D::vector3(
                -MoverParameters->AccN * Timedelta *
                    5.0,  // highlight side sway
                -MoverParameters->AccVert * Timedelta,
                -MoverParameters->AccSVBased * Timedelta *
                    1.25);  // accent acceleration/deceleration
        }

        auto shake{
            1.25 * ShakeSpring.ComputateForces(shakevector, ShakeState.offset)};

        if (LocalRandom(iVel) > 25.0) {
            // extra shake at increased velocity
            shake += ShakeSpring.ComputateForces(
                Math3D::vector3(
                    (LocalRandom(iVel * 2) - iVel) / ((iVel * 2) * 4) *
                    ...
                    //                * (( 200 -
                    //                DynamicObject->MyTrack->iQualityFlag ) *
                    //                0.0075 ) // scale to 75-150% based on
                    //                track quality
                    * 1.25,
                ShakeState.offset);
        }
        ...
        ShakeState.velocity -=
            (shake + ShakeState.velocity * 100) *
            (BaseShake.jolt_scale.x + BaseShake.jolt_scale.y +
             BaseShake.jolt_scale.z) /
            (200);
        ...
        ShakeState.offset -=
            ShakeState.offset *
            std::min(Timedelta, 1.0);  // po tym chyba potrafią zostać jakieś
                                       // ułamki, które powodują zjazd
    }
}

std::pair<double, double> TDynamicObject::shake_angles() const {
    return {
        std::atan(ShakeState.velocity.x * BaseShake.angle_scale.x),
        std::atan(ShakeState.velocity.z * BaseShake.angle_scale.z)};
}

void TDynamicObject::powertrain_sounds::position(glm::vec3 const Location) {
    auto const nullvector{glm::vec3()};
    std::vector<sound_source *> enginesounds = {
        &inverter,         &motor_relay,      &dsbWejscie_na_bezoporow,
        &motor_parallel,   &motor_shuntfield, &rsWentylator,
        &engine,           &engine_ignition,  &engine_shutdown,
        &engine_revving,   &engine_turbo,     &oil_pump,
        &fuel_pump,        &water_pump,       &water_heater,
        &radiator_fan,     &radiator_fan_aux, &transmission,
        &rsEngageSlippery, &retarder};
    for (auto sound : enginesounds) {
        if (sound->offset() == nullvector) {
...
void TDynamicObject::powertrain_sounds::render(
    TMoverParameters const &Vehicle, double const Deltatime) {
    if (Vehicle.Power == 0) {
        return;
    }
    ...
    if (true == Vehicle.OilPump.is_active) {
        oil_pump
            .pitch(
                oil_pump.m_frequencyoffset + oil_pump.m_frequencyfactor * 1.f)
    ...
    if (true == Vehicle.FuelPump.is_active) {
        fuel_pump
            .pitch(
                fuel_pump.m_frequencyoffset + fuel_pump.m_frequencyfactor * 1.f)
            .gain(
                fuel_pump.m_amplitudeoffset + fuel_pump.m_amplitudefactor * 1.f)
            .play(sound_flags::exclusive | sound_flags::looping);
    } else {
        fuel_pump.stop();
    }
    ...
    if (true == Vehicle.WaterPump.is_active) {
        water_pump
            .pitch(
                water_pump.m_frequencyoffset +
                water_pump.m_frequencyfactor * 1.f)
            .gain(
                water_pump.m_amplitudeoffset +
                water_pump.m_amplitudefactor * 1.f)
            .play(sound_flags::exclusive | sound_flags::looping);
    } else {
        water_pump.stop();
    }
    ...
    if (true == Vehicle.WaterHeater.is_active) {
        water_heater
            .pitch(
                water_heater.m_frequencyoffset +
                water_heater.m_frequencyfactor * 1.f)
            .gain(
                water_heater.m_amplitudeoffset +
                water_heater.m_amplitudefactor * 1.f)
            .play(sound_flags::exclusive | sound_flags::looping);
    } else {
        water_heater.stop();
    }
    ...
    if (engine_state_last != Vehicle.Mains) {
        if (true == Vehicle.Mains) {
            ...
            engine_ignition
                .pitch(
                    engine_ignition.m_frequencyoffset +
                    engine_ignition.m_frequencyfactor * 1.f)
                .gain(
                    engine_ignition.m_amplitudeoffset +
                    engine_ignition.m_amplitudefactor * 1.f)
                .play(sound_flags::exclusive);
            // main circuit activation
            linebreaker_close
                .pitch(
                    linebreaker_close.m_frequencyoffset +
                    linebreaker_close.m_frequencyfactor * 1.f)
                .gain(
                    linebreaker_close.m_amplitudeoffset +
                    linebreaker_close.m_amplitudefactor * 1.f)
                .play();
        } else {
            // engine deactivation
            ...
            engine_shutdown
                .pitch(
                    engine_shutdown.m_frequencyoffset +
                    engine_shutdown.m_frequencyfactor * 1.f)
                .gain(
                    engine_shutdown.m_amplitudeoffset +
                    engine_shutdown.m_amplitudefactor * 1.f)
                .play(sound_flags::exclusive);
            // main circuit deactivation
            linebreaker_open
                .pitch(
                    linebreaker_open.m_frequencyoffset +
                    linebreaker_open.m_frequencyfactor * 1.f)
                .gain(
                    linebreaker_open.m_amplitudeoffset +
                    linebreaker_open.m_amplitudefactor * 1.f)
                .play();
        }
    ...
    if (true == Vehicle.Mains) {
        if ((std::abs(Vehicle.enrot) > 0.01)
            // McZapkie-280503: zeby dla dumb dzialal silnik na jalowych
            // obrotach
            ...
            auto const normalizer{
                (true == engine.is_combined()
                     ?
                     // for combined engine sound we calculate sound point in
                     // rpm, to make .mmd files setup easier NOTE: we supply
                     // 1/100th of actual value, as the sound module converts
                     // does the opposite, converting received (typically) 0-1
                     // values to 0-100 range
                     60.f * 0.01f
                     :
                     // for legacy single-piece sounds the standard frequency
                     // calculation is left untouched
                     1.f)};
            frequency = engine.m_frequencyoffset + engine.m_frequencyfactor *
                                                       std::abs(Vehicle.enrot) *
                                                       normalizer;

            if (Vehicle.EngineType == TEngineType::Dumb) {
                frequency -=
                    0.2 * Vehicle.EnginePower / (1 + Vehicle.Power * 1000);
            }
            ...
            switch (Vehicle.EngineType) {
                ...
                case TEngineType::DieselElectric: {
                    volume = engine.m_amplitudeoffset +
            ...
            if (engine_volume >= 0.05) {
                ...
                if ((Vehicle.EngineType == TEngineType::DieselElectric) ||
                    ...
                    // diesel engine revolutions increase; it can potentially
                    // decrease volume of base engine sound
                    if (engine_revs_last != -1.f) {
                        // calculate potential recent increase of engine
                        // revolutions
                        auto const revolutionsperminute{Vehicle.enrot * 60};
                        auto const revolutionsdifference{
                            revolutionsperminute - engine_revs_last};
                        auto const idlerevolutionsthreshold{
                            1.01 * Vehicle.EngineIdleRPM()};
                        engine_revs_change =
                            std::max(0.0, engine_revs_change - 2.5 * Deltatime);
                        if ((revolutionsperminute > idlerevolutionsthreshold) &&
                            (revolutionsdifference > 1.0 * Deltatime)) {
                            engine_revs_change = clamp(
                                engine_revs_change + 5.0 * Deltatime, 0.0,
                                1.25);
                        }
                    ...
                    auto const enginerevfrequency{
                        (true == engine_revving.is_combined()
                             ?
                             // if the sound contains multiple samples we reuse
                             // rpm-based calculation from the engine
                             frequency
                             : engine_revving.m_frequencyoffset +
                                   1.f * engine_revving.m_frequencyfactor)};

                    if (enginerevvolume > 0.02) {
                        engine_revving.pitch(enginerevfrequency)
                ...
                // multi-part revving sound pieces replace base engine sound,
                // single revving simply gets mixed with the base
                auto const enginevolume{
                    (((enginerevvolume > 0.02) &&
                      (true == engine_revving.is_combined()))
                         ? std::max(0.0, engine_volume - enginerevvolume)
                         : engine_volume)};
                engine.pitch(frequency)
                    .gain(enginevolume)
                    .play(sound_flags::exclusive | sound_flags::looping);
            }  // enginevolume > 0.05
        } else {
    ...
    if (Vehicle.Mains) {
        ...
        fake_engine
            .pitch(
                ...
                power * fake_engine.m_frequencyfactor)
            .gain(
                fake_engine.m_amplitudeoffset +
                power * fake_engine.m_amplitudefactor)
            .play(sound_flags::exclusive | sound_flags::looping);
    } else {
        fake_engine.stop();
    }
    ...
    if (Vehicle.TurboTest > 0) {
        // udawanie turbo:
        auto const pitch_diesel{
            Vehicle.EngineType == TEngineType::DieselEngine
                ? Vehicle.enrot / Vehicle.dizel_nmax
                : 1};
        auto const goalpitch{std::max(
            0.025,
            (engine_volume * pitch_diesel + engine_turbo.m_frequencyoffset) *
                engine_turbo.m_frequencyfactor)};
        auto const goalvolume{
            (((Vehicle.MainCtrlPos >= Vehicle.TurboTest) &&
              (Vehicle.enrot > 0.1))
                 ? std::max(
                       0.0,
                       (engine_turbo_pitch + engine_turbo.m_amplitudeoffset) *
                           engine_turbo.m_amplitudefactor)
                 : 0.0)};
        ...
        engine_turbo.pitch(0.4 + engine_turbo_pitch * 0.4).gain(volume);
    ...
    if (Vehicle.dizel_engage > 0.1) {
        if (std::abs(Vehicle.dizel_engagedeltaomega) > 0.2) {
            frequency = rsEngageSlippery.m_frequencyoffset +
                        rsEngageSlippery.m_frequencyfactor *
                            std::fabs(Vehicle.dizel_engagedeltaomega);
            volume =
                rsEngageSlippery.m_amplitudeoffset +
                rsEngageSlippery.m_amplitudefactor * (Vehicle.dizel_engage);
        } else {
            frequency =
                1.f;  // rsEngageSlippery.FA+0.7*rsEngageSlippery.FM*(fabs(mvControlled->enrot)+mvControlled->nmax);
            volume =
                (Vehicle.dizel_engage > 0.2
                     ? rsEngageSlippery.m_amplitudeoffset +
                           0.2 * rsEngageSlippery.m_amplitudefactor *
                               (Vehicle.enrot / Vehicle.nmax)
                     : 0.f);
        }

        rsEngageSlippery.pitch(frequency).gain(volume).play(
            sound_flags::exclusive | sound_flags::looping);
    } else {
    ...
    if (Vehicle.hydro_R) {
        ...
        retarder
            .pitch(
                retarder.m_frequencyoffset + speed * retarder.m_frequencyfactor)
            .gain(
                retarder.m_amplitudeoffset +
                Vehicle.hydro_R_Fill * retarder.m_amplitudefactor);

        if ((retarder.gain() > 0.01) && (speed > 1) &&
            (Vehicle.hydro_R_ClutchActive)) {
    ...
    if (false == motors.empty()) {
        if (std::abs(Vehicle.nrot) > 0.01) {
            ...
            if (true == motor.is_combined()) {
                // for combined motor sound we calculate sound point in rpm, to
                // make .mmd files setup easier NOTE: we supply 1/100th of
                // actual value, as the sound module converts does the opposite,
                // converting received (typically) 0-1 values to 0-100 range
                normalizer = 60.f * 0.01f;
            }
            auto const motorrevolutions{
                std::abs(Vehicle.nrot) * Vehicle.Transmision.Ratio};
            frequency = motor.m_frequencyoffset +
                        motor.m_frequencyfactor * motorrevolutions * normalizer;

            // base volume calculation
            switch (Vehicle.EngineType) {
                case TEngineType::ElectricInductionMotor: {
                    volume = motor.m_amplitudeoffset +
                             motor.m_amplitudefactor *
                                 (Vehicle.EnginePower + motorrevolutions * 2);
                    break;
                }
                case TEngineType::ElectricSeriesMotor: {
                    volume =
                        ...
                        motor.m_amplitudefactor *
                            (Vehicle.EnginePower + motorrevolutions * 60.0);
                    break;
                }
                default: {
                    volume = motor.m_amplitudeoffset +
            ...
            if (Vehicle.EngineType == TEngineType::ElectricSeriesMotor) {
                // volume variation
                if ((volume < 1.0) && (Vehicle.EnginePower < 100)) {
                    auto const volumevariation{
                        LocalRandom(100) * Vehicle.enrot / (1 + Vehicle.nmax)};
                    if (volumevariation < 2) {
                        volume += volumevariation / 200;
                    }
                }

                if ((Vehicle.DynamicBrakeFlag) && (Vehicle.EnginePower > 0.1)) {
                    // Szociu - 29012012 - jeżeli uruchomiony jest hamulec
                    // elektrodynamiczny, odtwarzany jest dźwięk silnika
                    volume += 0.8;
                }
            }
            // scale motor volume based on whether they're active
            motor_momentum = clamp(
                motor_momentum - 1.0 * Deltatime  // smooth out decay
                ...
                for (auto &motor : acmotors) {
                    motor
                        .pitch(
                            ...
                            motor.m_frequencyfactor * Vehicle.InverterFrequency)
                        .gain(
                            motor.m_amplitudeoffset +
                            motor.m_amplitudefactor *
                                std::sqrt(std::abs(Vehicle.eimv_pr)))
                        .play(sound_flags::exclusive | sound_flags::looping);
                }
            } else {
                for (auto &motor : acmotors) {
    ...
    if (false == motorblowers.empty()) {
        for (auto &blowersound : motorblowers) {
            // match the motor blower and the sound source based on whether
            // they're located in the front or the back of the vehicle
            auto const &blower{Vehicle.MotorBlowers[(
                blowersound.offset().z > 0 ? end::front : end::rear)]};
            // TODO: for the sounds with provided bookends invoke stop() when
            // the stop is triggered and revolutions start dropping, instead of
            // after full stop
            if (blower.revolutions > 1) {
                // NOTE: for motorblowers with fixed speed if the sound has
                // defined bookends we skip revolutions-based part of
                // frequency/volume adjustments
                auto const revolutionmodifier{
                    (Vehicle.MotorBlowers[end::front].speed < 0.f) &&
                            (blowersound.has_bookends())
                        ? 1.f
                        : blower.revolutions};
                blowersound
                    .pitch(
                        true == blowersound.is_combined()
                            ...
                            : blowersound.m_frequencyoffset +
                                  blowersound.m_frequencyfactor *
                                      revolutionmodifier)
                    .gain(
                        blowersound.m_amplitudeoffset +
                        blowersound.m_amplitudefactor * revolutionmodifier)
                    .play(sound_flags::exclusive | sound_flags::looping);
            } else {
                blowersound.stop();
            }
    ...
    if (Vehicle.EngineType == TEngineType::ElectricInductionMotor) {
        if (Vehicle.InverterFrequency > 0.001) {
            volume = inverter.m_amplitudeoffset +
                     inverter.m_amplitudefactor *
                         std::sqrt(std::abs(Vehicle.eimv_pr));

            inverter
                .pitch(
                    inverter.m_frequencyoffset +
                    inverter.m_frequencyfactor * Vehicle.InverterFrequency)
                .gain(volume)
                .play(sound_flags::exclusive | sound_flags::looping);
        } else {
            inverter.stop();
    ...
    if (Vehicle.RventRot > 0.1) {
        rsWentylator
            .pitch(
                rsWentylator.m_frequencyoffset +
                rsWentylator.m_frequencyfactor * Vehicle.RventRot)
            .gain(
                rsWentylator.m_amplitudeoffset +
                rsWentylator.m_amplitudefactor * Vehicle.RventRot)
            .play(sound_flags::exclusive | sound_flags::looping);
    } else {
    ...
    if ((Vehicle.EngineType == TEngineType::DieselEngine) ||
        ...
        if (Vehicle.dizel_heat.rpmw > 0.1) {
            // NOTE: fan speed tends to max out at ~100 rpm; by default we try
            // to get pitch range of 0.5-1.5 and volume range of 0.5-1.0
            radiator_fan
                .pitch(
                    0.5 + radiator_fan.m_frequencyoffset +
                    radiator_fan.m_frequencyfactor * Vehicle.dizel_heat.rpmw *
                        0.01)
                .gain(
                    0.5 + radiator_fan.m_amplitudeoffset +
                    0.5 * radiator_fan.m_amplitudefactor *
                        Vehicle.dizel_heat.rpmw * 0.01)
                .play(sound_flags::exclusive | sound_flags::looping);
        } else {
            // ...otherwise shut down the sound
            radiator_fan.stop();
        ...
        if (Vehicle.dizel_heat.rpmw2 > 0.1) {
            ...
            radiator_fan_aux
                .pitch(
                    0.5 + radiator_fan_aux.m_frequencyoffset +
                    radiator_fan_aux.m_frequencyfactor *
                        Vehicle.dizel_heat.rpmw2 * 0.01)
                .gain(
                    0.5 + radiator_fan_aux.m_amplitudeoffset +
                    0.5 * radiator_fan_aux.m_amplitudefactor *
                        Vehicle.dizel_heat.rpmw2 * 0.01)
                .play(sound_flags::exclusive | sound_flags::looping);
        } else {
            // ...otherwise shut down the sound
            ...
            motor_shuntfield
                .pitch(
                    true == motor_shuntfield.is_combined()
                        ...
                        : motor_shuntfield.m_frequencyoffset +
                              motor_shuntfield.m_frequencyfactor * 1.f)
                .gain(
                    motor_shuntfield.m_amplitudeoffset +
                    (true == TestFlag(soundflags, sound::loud) ? 1.0f : 0.8f) *
            ...
            motor_relay
                .pitch(
                    true == motor_relay.is_combined()
                        ...
                        : motor_relay.m_frequencyoffset +
                              motor_relay.m_frequencyfactor * 1.f)
                .gain(
    ...
    if (Vehicle.Vel > 0.1) {
        transmission
            .pitch(
                transmission.m_frequencyoffset +
                transmission.m_frequencyfactor * Vehicle.Vel)
            .gain(
                transmission.m_amplitudeoffset +
                transmission.m_amplitudefactor * Vehicle.Vel)
            .play(sound_flags::exclusive | sound_flags::looping);
    } else {
        transmission.stop();
    }
...
void vehicle_table::update(double Deltatime, int Iterationcount) {
    // Ra: w zasadzie to trzeba by utworzyć oddzielną listę taboru do liczenia
    // fizyki
    //    na którą by się zapisywały wszystkie pojazdy będące w ruchu
    //    pojazdy stojące nie potrzebują aktualizacji, chyba że np. ktoś im
    //    zmieni nastawę hamulca oddzielną listę można by zrobić na pojazdy z
    //    napędem, najlepiej posortowaną wg typu napędu
    for (auto *vehicle : m_items) {
        if (false == vehicle->bEnabled) {
            continue;
        }
        // Ra: zmienić warunek na sprawdzanie pantografów w jednej zmiennej: czy
        // pantografy i czy podniesione
        if (vehicle->MoverParameters->EnginePowerSource.SourceType ==
            TPowerSource::CurrentCollector) {
...
// legacy method, checks for presence and height of traction wire for specified
// vehicle
void vehicle_table::update_traction(TDynamicObject *Vehicle) {
    auto const vFront = glm::make_vec3(
        Vehicle->VectorFront()
            .getArray());  // wektor normalny dla płaszczyzny ruchu pantografu
    auto const vUp = glm::make_vec3(
        Vehicle->VectorUp().getArray());  // wektor pionu pudła (pochylony od
                                          // pionu na przechyłce)
    auto const vLeft =
        glm::make_vec3(Vehicle->VectorLeft()
                           .getArray());  // wektor odległości w bok (odchylony
                                          // od poziomu na przechyłce)
    auto const position =
        glm::dvec3{Vehicle->GetPosition()};  // współrzędne środka pojazdu

    for (int pantographindex = 0;
         pantographindex < Vehicle->iAnimType[ANIM_PANTS]; ++pantographindex) {
        // pętla po pantografach
        auto *pantograph{Vehicle->pants[pantographindex].fParamPants};
        if (true ==
            Vehicle->MoverParameters->Pantographs[pantographindex].is_active) {
            // jeśli pantograf podniesiony
            auto const pant0{
                position + (vLeft * pantograph->vPos.z) +
                (vUp * pantograph->vPos.y) + (vFront * pantograph->vPos.x)};
            if (pantograph->hvPowerWire != nullptr) {
                // jeżeli znamy drut z poprzedniego przebiegu
                for (int attempts = 0; attempts < 30; ++attempts) {
                    // sanity check. shouldn't happen in theory, but did happen
                    // in practice
                    if (pantograph->hvPowerWire == nullptr) {
                        break;
                    }

                    // powtarzane aż do znalezienia odpowiedniego odcinka na
                    // liście dwukierunkowej
                    if (pantograph->hvPowerWire->iLast & 0x3) {
                        // dla ostatniego i przedostatniego przęsła wymuszamy
                        // szukanie innego nie to, że nie ma, ale trzeba
                        // sprawdzić inne
                        pantograph->hvPowerWire = nullptr;
                        break;
                    }
                    if (pantograph->hvPowerWire->hvParallel) {
                        // jeśli przęsło tworzy bieżnię wspólną, to trzeba
                        // sprawdzić pozostałe nie to, że nie ma, ale trzeba
                        // sprawdzić inne
                        pantograph->hvPowerWire = nullptr;
                        break;
                    }
                    // obliczamy wyraz wolny równania płaszczyzny (to miejsce
                    // nie jest odpowienie) podstawiamy równanie parametryczne
                    // drutu do równania płaszczyzny pantografu
                    // TODO: investigate this routine with reardriver/negative
                    // speed, does it picks the right wire?
                    auto const fRaParam =
                        -(glm::dot(pantograph->hvPowerWire->pPoint1, vFront) -
                          glm::dot(pant0, vFront)) /
                        glm::dot(pantograph->hvPowerWire->vParametric, vFront);

                    if (fRaParam < -0.001) {
                        // histereza rzędu 7cm na 70m typowego przęsła daje 1
                        // promil
                        pantograph->hvPowerWire =
                            pantograph->hvPowerWire->hvNext[0];
                        continue;
                    }
                    if (fRaParam > 1.001) {
                        pantograph->hvPowerWire =
                            pantograph->hvPowerWire->hvNext[1];
                        continue;
                    }
                    // jeśli t jest w przedziale, wyznaczyć odległość wzdłuż
                    // wektorów vUp i vLeft punkt styku płaszczyzny z drutem
                    // (dla generatora łuku el.)
                    auto const vStyk{
                        pantograph->hvPowerWire->pPoint1 +
                        fRaParam * pantograph->hvPowerWire->vParametric};
                    auto const vGdzie{vStyk - pant0};  // wektor
                    // odległość w pionie musi być w zasięgu ruchu "pionowego"
                    // pantografu musi się mieścić w przedziale ruchu pantografu
                    auto const fVertical{glm::dot(vGdzie, vUp)};
                    // odległość w bok powinna być mniejsza niż pół szerokości
                    // pantografu to się musi mieścić w przedziale zależnym od
                    // szerokości pantografu
                    auto const fHorizontal{
                        std::abs(glm::dot(vGdzie, vLeft)) - pantograph->fWidth};
                    // jeśli w pionie albo w bok jest za daleko, to dany drut
                    // jest nieużyteczny
                    if (fHorizontal <= 0.0) {
                        ...
                        // the wire is outside contact area and as of now we
                        // don't have good detection of parallel sections as
                        // such there's no guaratee there isn't parallel section
                        // present. therefore we don't bother checking if the
                        // wire is still within range of guide horns but simply
                        // force area search for potential better option
                        pantograph->hvPowerWire = nullptr;
                        break;
                    }
                }
            ...
            if (pantograph->hvPowerWire == nullptr) {
                // look in the region for a suitable traction piece if we don't
                // already have any
            ...
            if ((pantograph->hvPowerWire == nullptr) &&
                ...
                // jeśli drut nie znaleziony ale można oszukiwać to dajemy coś
                // tam dla picu
...
void vehicle_table::DynamicList(bool const Onlycontrolled) const {
    // odesłanie nazw pojazdów dostępnych na scenerii (nazwy, szczególnie
    // wagonów, mogą się powtarzać!)
    for (auto const *vehicle : m_items) {
        if ((false == Onlycontrolled) || (vehicle->Mechanik != nullptr)) {
            // same nazwy pojazdów
...
bool vehicle_table::erase_disabled() {
    if (false == TDynamicObject::bDynamicRemove) {
        return false;
    }
    ...
    for (auto *vehicle : disabledvehicles) {
        ...
        // (try to) run propagation in both directions, it's simpler than
        // branching based on direction etc
    ...
    while (vehicleiter != std::end(m_items)) {
        ...
        if (true == vehicle->bEnabled) {
            ...
            /*
                        // finally get rid of the vehicle and its record
               themselves
                        // BUG: deleting the vehicle leaves dangling pointers in
               event->Activator and potentially elsewhere
                        // TBD, TODO: either mark 'dead' vehicles with
               additional flag, or delete the references as well SafeDelete(
               vehicle ); vehicleiter = m_items.erase( vehicleiter );
            */
            ++vehicleiter;  // NOTE: instead of the erase in the disabled
                            // section
        }
    }
    // ...and call it a day
