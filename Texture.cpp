...
std::array<GLuint, gl::MAX_TEXTURES + gl::HELPER_TEXTURES>
    opengl_texture::units = {0};
...
std::unordered_map<GLint, std::unordered_map<GLint, GLint>>
    opengl_texture::mapping = {
        // image have,                         material wants, gl internalformat
        {GL_COMPRESSED_RGBA8_ETC2_EAC,
         {{GL_SRGB_ALPHA, GL_COMPRESSED_SRGB8_ALPHA8_ETC2_EAC},
        ...
        {GL_COMPRESSED_RGBA_S3TC_DXT1_EXT,
         {{GL_SRGB_ALPHA, GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT1_EXT},
        ...
        {GL_COMPRESSED_RGBA_S3TC_DXT3_EXT,
         {{GL_SRGB_ALPHA, GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT3_EXT},
        ...
        {GL_COMPRESSED_RGBA_S3TC_DXT5_EXT,
         {{GL_SRGB_ALPHA, GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT5_EXT},
...
texture_manager::texture_manager() {
    // since index 0 is used to indicate no texture, we put a blank entry in the
    // first texture slot
    m_textures.emplace_back(
        new opengl_texture(), std::chrono::steady_clock::time_point());
}
...
void opengl_texture::gles_match_internalformat(GLuint internalformat) {
    ...
    if (precompressed_formats.find(internalformat) !=
        precompressed_formats.end())
        return;

    // don't care about sRGB here
    if (internalformat == GL_SRGB8) internalformat = GL_RGB8;
    if (internalformat == GL_SRGB8_ALPHA8) internalformat = GL_RGBA8;
    ...
    if (data_format == GL_RGBA && internalformat == GL_RGBA8) return;
    if (data_format == GL_RGB && internalformat == GL_RGB8) return;
    if (data_format == GL_RG && internalformat == GL_RG8) return;
    if (data_format == GL_RED && internalformat == GL_R8) return;
    ...
    if (!in_c || !out_c) return;  // conversion not supported
...
void opengl_texture::load() {
    if (data_state == resource_state::good) return;

    if (type == "make:" || type == "internalsrc:") {
        // for generated texture we delay data creation until texture is bound
        // this ensures the script will receive all simulation data it might
        // potentially want as any binding will happen after simulation is
        // loaded, initialized and running until then we supply data for a tiny
        // 2x2 grey stub
        make_stub();
    } else {
        WriteLog(
            "Loading texture data from \"" + name + "\"", logtype::texture);
    ...
    if (data_state == resource_state::good) {
        // verify texture size
        if ((clamp_power_of_two(data_width) != data_width) ||
            (clamp_power_of_two(data_height) != data_height)) {
            if (name != "logo") {
                WriteLog(
                    "Warning: dimensions of texture \"" + name +
                        "\" aren't powers of 2",
                    logtype::texture);
            }
        }
        if ((quantize(data_width, 4) != data_width) ||
            (quantize(data_height, 4) != data_height)) {
            WriteLog(
                "Warning: dimensions of texture \"" + name +
...
void opengl_texture::load_PNG() {
    ...
    png_image_finish_read(
        &png, nullptr, (void *)&data[0],
        -data_width * PNG_IMAGE_PIXEL_SIZE(png.format), nullptr);
...
void opengl_texture::make_from_memory(
    size_t width, size_t height, const uint8_t *raw) {
    release();
...
void opengl_texture::make_request() {
...
void opengl_texture::load_DDS() {
    std::ifstream file(name + type, std::ios::binary | std::ios::ate);
    file.unsetf(std::ios::skipws);
    std::size_t filesize = static_cast<size_t>(
        file.tellg());  // ios::ate already positioned us at the end of the file
    ...
    while ((data_width > Global.CurrentMaxTextureSize) ||
           (data_height > Global.CurrentMaxTextureSize)) {
        // pomijanie zbyt dużych mipmap, jeśli wymagane jest ograniczenie
        // rozmiaru
        ...
        --data_mapcount;
        WriteLog(
            "Texture pixelcount exceeds specified limits, skipping mipmap "
            "level");
    };
    ...
    if (data_mapcount <= 0) {
        ...
        WriteLog(
            "Texture \"" + name +
            "\" has no mipmaps which can fit currently set texture pixelcount "
            "limits.");
        data_state = resource_state::failed;
        return;
    }
    ...
    if (datasize == 0) {
        ...
        WriteLog(
            "Bad texture: file \"" + name +
                "\" is malformed and holds no texture data.",
            logtype::texture);
        data_state = resource_state::failed;
        return;
    }
    ...
    if (Global.dds_upper_origin) {
        ...
        int mapcount = data_mapcount, width = data_width, height = data_height;
        while (mapcount) {
...
void opengl_texture::load_KTX() {
    ...
    if (info.format != DDSKTX_FORMAT_ETC2A) {
        ErrorLog(
            "Bad texture: currently unsupported KTX type", logtype::texture);
        data_state = resource_state::failed;
        return;
    }
    ...
    for (int level = 0; level < info.num_mips; level++) {
        ...
        ddsktx_get_sub(
            &info, &sub_data, filecontent.data(), filecontent.size(), 0, 0,
            level);
        ...
        if (!started) {
            ...
            if ((data_width > Global.CurrentMaxTextureSize) ||
                (data_height > Global.CurrentMaxTextureSize)) {
                data_mapcount--;
                continue;
            }
...
void opengl_texture::load_TEX() {
    std::ifstream file(name + type, std::ios::binary);
    file.unsetf(std::ios::skipws);
    ...
    if (std::string("RGB ") == head) {
        ...
        ErrorLog(
            "Bad texture: unrecognized TEX texture sub-format: " +
                std::string(head),
            logtype::texture);
        data_state = resource_state::failed;
        return;
...
void opengl_texture::load_TGA() {
    ...
    if (tgaheader[2] == 2) {
        ...
        if (bytesperpixel == 4) {
            ...
            unsigned char buffer[4] = {
                255, 255, 255, 255};  // alpha channel will be white
            ...
            for (int i = 0; i < pixelcount; ++i) {
                file.read(
                    (char *)&buffer[0], sizeof(unsigned char) * bytesperpixel);
                if (bytesperpixel == 1) {
        ...
        do {
            ...
            if ((chunkheader & 0x80) == 0) {
                // if the high bit is not set, it means it is the number of RAW
                // color packets, plus 1
                ...
                // rle chunk, the color supplied afterwards is reapeated header
                // + 1 times (not including the highest bit)
                chunkheader &= ~0x80;
    ...
    if ((data_width > Global.CurrentMaxTextureSize) ||
        (data_height > Global.CurrentMaxTextureSize)) {
        // for non-square textures there's currently possibility the scaling
        // routine will have to abort before it gets all work done
        data_state = resource_state::failed;
        return;
    }

    // TODO: add horizontal/vertical data flip, based on the descriptor (18th)
    // header byte

    // fill remaining data info
    data_mapcount = 1;
    data_format = GL_BGRA;
    data_components = (bytesperpixel == 4 ? GL_RGBA : GL_RGB);
    data_state = resource_state::good;

    return;
}

bool opengl_texture::bind(size_t unit) {
    if ((false == is_ready) && (false == create())) {
        return false;
    }
...
bool opengl_texture::create(bool const Static) {
    ...
    // TODO: consider creating and storing low-res version of the texture if
    // it's ever unloaded from the gfx card, as a placeholder until it can be
    // loaded again
    if (id == -1) {
        ::glGenTextures(1, &id);
        ::glBindTexture(target, id);
        ...
        // upload texture data
        int dataoffset = 0, datasize = 0, datawidth = data_width,
            dataheight = data_height;
        if (is_rendertarget) {
            if (data_components == GL_DEPTH_COMPONENT) {
                glTexParameteri(
                    target, GL_TEXTURE_COMPARE_MODE, GL_COMPARE_REF_TO_TEXTURE);
            ...
            if (Global.gfx_usegles) {
                if (target == GL_TEXTURE_2D)
                    glTexStorage2D(
                        ...
                        count_trailing_zeros(
                            std::max(data_width, data_height)) +
                            1,
                        data_format, data_width, data_height);
                else if (target == GL_TEXTURE_2D_MULTISAMPLE)
                    glTexStorage2DMultisample(
                        target, samples, data_format, data_width, data_height,
                        GL_FALSE);
                    ...
                    glTexStorage3D(
                        ...
                        count_trailing_zeros(
                            std::max(data_width, data_height)) +
                            1,
                        data_format, data_width, data_height, layers);
                ...
                if (target == GL_TEXTURE_2D)
                    glTexImage2D(
                        target, 0, data_format, data_width, data_height, 0,
                        data_components, GL_UNSIGNED_SHORT, nullptr);
                else if (target == GL_TEXTURE_2D_MULTISAMPLE)
                    ...
                    glTexImage3D(
                        target, 0, data_format, data_width, data_height, layers,
                        0, data_components, GL_UNSIGNED_SHORT, nullptr);
                else if (target == GL_TEXTURE_2D_MULTISAMPLE_ARRAY)
            ...
            // data_format and data_type specifies how image is laid out in
            // memory data_components specifies what useful channels image
            // contains components_hint specifies what format we want to load
            ...
            if (f_it != precompressed_formats.end()) components = data_format;

            if (!components_hint) components_hint = GL_SRGB_ALPHA;
            ...
            for (int maplevel = 0; maplevel < data_mapcount; ++maplevel) {
                if (blocksize_it != precompressed_formats.end()) {
                    ...
                    datasize = ((std::max(datawidth, 4) + 3) / 4) *
                               ((std::max(dataheight, 4) + 3) / 4) *
                               datablocksize;

                    ::glCompressedTexImage2D(
                        target, maplevel, internal_format, datawidth,
                        dataheight, 0, datasize, (GLubyte *)&data[dataoffset]);
                    ...
                    GLint compressed_format =
                        drivercompressed_formats[internal_format];

                    // uncompressed texture data. have the gfx card do the
                    // compression as it sees fit
                    ::glTexImage2D(
                        target, 0,
                        Global.compress_tex ? compressed_format
                                            : internal_format,
            ...
            if ((true == Global.ResourceMove) ||
                ...
                // if garbage collection is disabled we don't expect having to
                // upload the texture more than once
        ...
        if (type == "make:") {
            // for generated textures send a request to have the actual content
            // of the texture generated
            make_request();
        }
...
// releases resources allocated on the opengl end, storing local copy if
// requested
void opengl_texture::release() {
    ...
    if (true == Global.ResourceMove && !is_rendertarget) {
        // if resource move is enabled we don't keep a cpu side copy after
        // upload so need to re-acquire the data before release TBD, TODO:
        // instead of vram-ram transfer fetch the data 'normally' from the disk
        // using worker thread
        if (type == "make:") {
            ...
            ::glGetTexLevelParameteriv(
                target, 0, GL_TEXTURE_COMPRESSED, &iscompressed);
            if (iscompressed == GL_TRUE) {
                // texture is compressed on the gpu side
                // query texture details needed to perform the backup...
                ::glGetTexLevelParameteriv(
                    target, 0, GL_TEXTURE_INTERNAL_FORMAT, &data_format);
                ::glGetTexLevelParameteriv(
                    target, 0, GL_TEXTURE_COMPRESSED_IMAGE_SIZE, &datasize);
                data.resize(datasize);
                // ...fetch the data...
                ::glGetCompressedTexImage(target, 0, &data[0]);
            } else {
                // for whatever reason texture didn't get compressed during
                // upload fallback on plain rgba storage...
                data_format = GL_RGBA;
                data_type = GL_UNSIGNED_BYTE;
                data.resize(data_width * data_height * 4);
                // ...fetch the data...
                ::glGetTexImage(
                    target, 0, data_format, GL_UNSIGNED_BYTE, &data[0]);
            }
...
void opengl_texture::alloc_rendertarget(
    GLint format, GLint components, int width, int height, int l, int s,
    GLint wrap) {
    data_width = width;
    data_height = height;
    data_format = format;
    ...
    if (layers > 1) {
        target =
            (samples > 1 ? GL_TEXTURE_2D_MULTISAMPLE_ARRAY
                         : GL_TEXTURE_2D_ARRAY);
    } else {
...
void opengl_texture::set_components_hint(GLint hint) { components_hint = hint; }
...
void opengl_texture::set_filtering() const {
...
void opengl_texture::downsize(GLuint const Format) {
    while ((data_width > Global.CurrentMaxTextureSize) ||
           (data_height > Global.CurrentMaxTextureSize)) {
        // scale down the base texture, if it's larger than allowed maximum
        // NOTE: scaling is uniform along both axes, meaning non-square textures
        // can drop below the maximum
        // TODO: replace with proper scaling function once we have image
        // middleware in place
        ...
        WriteLog(
            "Texture pixelcount exceeds specified limits, downsampling data");
        // trim potential odd texture sizes
        data_width -= (data_width % 2);
        data_height -= (data_height % 2);
        switch (Format) {
            case GL_RGB: {
                downsample<glm::tvec3<std::uint8_t>>(
                    data_width, data_height, data.data());
                break;
            }
            case GL_BGRA:
            case GL_RGBA: {
                downsample<glm::tvec4<std::uint8_t>>(
                    data_width, data_height, data.data());
                break;
            }
            default: {
...
void opengl_texture::flip_vertical() {
...
void texture_manager::unit(GLint const Textureunit) {
    if (opengl_texture::m_activeunit == Textureunit) {
        return;
    }
...
texture_handle texture_manager::create(
    std::string Filename, bool const Loadnow, GLint Formathint) {
    ...
    std::pair<std::string, std::string>
        locator;  // resource name, resource type
    std::string traits;
    ...
    if (isgenerated || isinternalsrc) {
        ...
        // potentially trim file type indicator since we check for multiple
        // types
        erase_extension(Filename);
        // clean up slashes
        erase_leading_slashes(Filename);
        Filename = ToLower(Filename);
        // temporary code for legacy assets -- textures with names beginning
        // with # are to be sharpened
        if ((starts_with(Filename, "#")) || (contains(Filename, "/#"))) {
            traits += '#';
        }
    }
    // try to locate requested texture in the databank
    // TBD, TODO: include trait specification in the resource id in the
    // databank?
    ...
    if (isgenerated) {
        ...
        if (true == locator.first.empty()) {
            ...
            // failure
            ErrorLog(
                "Bad file: failed to locate texture file \"" + Filename + "\"",
                logtype::file);
            return npos;
        }
    }
    ...
    WriteLog(
        "Created texture object for \"" + locator.first + "\"",
        logtype::texture);
    ...
    if (true == Loadnow) {
        ...
        // texture creation binds a different texture, force a re-bind on next
        // use
        m_activetexture = -1;
#endif
    }
...
void texture_manager::delete_textures() {
    for (auto const &texture : m_textures) {
        // usunięcie wszyskich tekstur (bez usuwania struktury)
        if ((texture.first->id > 0) && (texture.first->id != -1)) {
...
std::string texture_manager::info() const {
    ...
    return "textures: "
           ...
           to_string(readytexturesize / 1024.0f, 2) + " mb)" + " in vram, "
#endif
           + std::to_string(totaltexturecount) + " (" +
           to_string(totaltexturesize / 1024.0f, 2) + " mb)" + " total";
}

// checks whether specified texture is in the texture bank. returns texture id,
// or npos.
texture_handle texture_manager::find_in_databank(
    std::string const &Texturename) const {
    std::vector<std::string> const filenames{
        Global.asCurrentTexturePath + Texturename, Texturename,
...
std::pair<std::string, std::string> texture_manager::find_on_disk(
    std::string const &Texturename) const {
    std::vector<std::string> const filenames{
        Global.asCurrentTexturePath + Texturename, Texturename,
        szTexturePath + Texturename};

    auto lookup = FileExists(filenames, {Global.szDefaultExt});
    ...
    // NOTE: slightly wasteful as it means preferred extension is tested twice,
    // but, eh
    return (FileExists(
        filenames, {".dds", ".tga", ".ktx", ".png", ".bmp", ".jpg", ".tex"}));
}
