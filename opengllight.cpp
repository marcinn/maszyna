...
void opengl_light::apply_intensity(float const Factor) {
    if (Factor == 1.f) {
        ...
        // temporary light scaling mechanics (ultimately this work will be left
        // to the shaders
        glm::vec4 scaledambient(
            ambient.r * factor, ambient.g * factor, ambient.b * factor,
            ambient.a);
        glm::vec4 scaleddiffuse(
            diffuse.r * factor, diffuse.g * factor, diffuse.b * factor,
            diffuse.a);
        glm::vec4 scaledspecular(
            specular.r * factor, specular.g * factor, specular.b * factor,
            specular.a);
...
void opengl_light::apply_angle() {
    ::glLightfv(
        id, GL_POSITION,
        glm::value_ptr(glm::vec4{position, (is_directional ? 0.f : 1.f)}));
