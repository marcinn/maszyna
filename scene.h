...
int const EU07_REGIONSIDESECTIONCOUNT =
    500;  // number of sections along a side of square region
...
// basic element of rudimentary partitioning scheme for the section. fixed size,
// no further subdivision TBD, TODO: replace with quadtree scheme?
class basic_cell {
   ...
   public:
    ...
    basic_cell() = default;
    // methods
    // potentially activates event handler with the same name as provided node,
    // and within handler activation range
    void on_click(TAnimModel const *Instance);
    // legacy method, finds and assigns traction piece to specified pantograph
    // of provided vehicle
    ...
    // find a vehicle located nearest to specified point, within specified
    // radius. reurns: located vehicle and distance
    std::tuple<TDynamicObject *, float> find(
    ...
    std::tuple<TTraction *, int> find(
        glm::dvec3 const &Point, TTraction const *Exclude) const;
    // finds a traction piece located nearest to specified point, sharing
    // section with specified other piece and powered in specified direction.
    // returns: located traction piece
    std::tuple<TTraction *, int, float> find(
    ...
    bounding_area const &area() const { return m_area; }
   ...
   private:
    ...
    // methods
    void launch_event(TEventLauncher *Launcher, bool local_only);
    void enclose_area(scene::basic_node *Node);
    // members
    scene::bounding_area m_area{
        glm::dvec3(), static_cast<float>(0.5 * M_SQRT2 * EU07_CELLSIZE)};
    ...
    // animation of owned items (legacy code, clean up along with track
    // refactoring)
...
// basic scene partitioning structure, holds terrain geometry and collection of
// cells
class basic_section {
    friend opengl_renderer;
    friend opengl33_renderer;
   ...
   public:
    ...
    // of provided vehicle
    void update_traction(TDynamicObject *Vehicle, int const Pantographindex);
    // legacy method, updates sounds and polls event launchers within radius
    // around specified point
    ...
    // legacy method, triggers radio-stop procedure for all vehicles in 2km
    // radius around specified location
    ...
    void insert(Type_ *Node) {
        ...
        m_area.radius = std::max(
            m_area.radius,
            static_cast<float>(
                glm::length(m_area.center - targetcell.area().center) +
                targetcell.area().radius));
    }
    ...
    // registers provided node in the lookup directory of the section enclosing
    // specified point
    template <class Type_>
   ...
   private:
    ...
    using cell_array = std::array<
        basic_cell, (EU07_SECTIONSIZE / EU07_CELLSIZE) *
                        (EU07_SECTIONSIZE / EU07_CELLSIZE)>;
    using shapenode_sequence = std::vector<shape_node>;
    // methods
    // provides access to section enclosing specified point
    basic_cell &cell(
        glm::dvec3 const &Location, const glm::ivec2 &offset = glm::ivec2(0));
    // members
    // placement and visibility

    scene::bounding_area m_area{
        glm::dvec3(), static_cast<float>(0.5 * M_SQRT2 * EU07_SECTIONSIZE)};
    // content
    cell_array m_cells;  // partitioning scheme
    shapenode_sequence
        m_shapes;  // large pieces of opaque geometry and (legacy) terrain
    // TODO: implement dedicated, higher fidelity, fixed resolution terrain mesh
    // item gfx renderer data
...
class basic_region {
   ...
   public:
    ...
    // restores content of the class from file with specified name. returns:
    ...
    // inserts provided node in the region and registers its ends in lookup
    // directory
    template <class Type_>
    ...
    // find a vehicle located nearest to specified point, within specified
    // radius. reurns: located vehicle and distance
    std::tuple<TDynamicObject *, float> find_vehicle(
        glm::dvec3 const &Point, float const Radius, bool const Onlycontrolled,
        bool const Findbycoupler);
    // finds a path with one of its ends located in specified point. returns:
    // located path and id of the matching endpoint
    std::tuple<TTrack *, int> find_path(
        glm::dvec3 const &Point, TTrack const *Exclude);
    // finds a traction piece with one of its ends located in specified point.
    // returns: located traction piece and id of the matching endpoint
    std::tuple<TTraction *, int> find_traction(
        glm::dvec3 const &Point, TTraction const *Exclude);
    // finds a traction piece located nearest to specified point, sharing
    // section with specified other piece and powered in specified direction.
    // returns: located traction piece
    ...
    std::vector<basic_section *> const &sections(
        glm::dvec3 const &Point, float const Radius);
    void create_map_geometry();
    void update_poi_geometry();
    basic_section *get_section(size_t section) { return m_sections[section]; }
    gfx::geometrybank_handle get_map_poi_geometry() { return m_map_poipoints; }
    glm::vec3 find_nearest_track_point(const glm::dvec3 &pos) {
   ...
   private:
    ...
    using section_array = std::array<
        basic_section *,
        EU07_REGIONSIDESECTIONCOUNT * EU07_REGIONSIDESECTIONCOUNT>;
    ...
    // legacy method, trims provided shape to fit into a section. adds trimmed
    // part at the end of provided list, returns true if changes were made
    static bool RaTriangleDivider(
        shape_node &Shape, std::deque<shape_node> &Shapes);
    // provides access to section enclosing specified point
    basic_section &section(glm::dvec3 const &Location);
