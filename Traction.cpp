...
void TTraction::Load(cParser *parser, glm::dvec3 const &pOrigin) {
    parser->getTokens(4);
    *parser >> asPowerSupplyName >> NominalVoltage >> MaxCurrent >>
        fResistivity;
    if (fResistivity == 0.01f) {
        // tyle jest w sceneriach [om/km]
        // taka sensowniejsza wartość za
    ...
    parser->getTokens(2);
    *parser >> WireThickness >> DamageFlag;
    ...
    fHeightDifference =
        (pPoint3.y - pPoint1.y + pPoint4.y - pPoint2.y) * 0.5 - minheight;
    auto const segmentlength{parser->getToken<double>()};
    iNumSections =
        (segmentlength ? glm::length((pPoint1 - pPoint2)) / segmentlength : 0);
    parser->getTokens(2);
    *parser >> Wires >> WireOffset;
    m_visible = (parser->getToken<std::string>() == "vis");

    std::string token{parser->getToken<std::string>()};
    if (token == "parallel") {
        // jawne wskazanie innego przęsła, na które może przestawić się
        // pantograf
        parser->getTokens();
        *parser >> asParallel;
    }
    while ((false == token.empty()) && (token != "endtraction")) {
...
std::vector<glm::dvec3> TTraction::endpoints() const {
    return {pPoint1, pPoint2};
}

std::size_t TTraction::create_geometry(gfx::geometrybank_handle const &Bank) {
    ...
    v2 = pPoint2 - pPoint1;
    float step = 0;
    if (iNumSections > 0) step = 1.0f / (float)iNumSections;
    double f = step;
    float mid = 0.5;
    float t;
    ...
    if (Wires > 1) {  // lina nośna w kawałkach
        startvertex.position = glm::vec3(
            pPoint3.x - m_origin.x, pPoint3.y - m_origin.y,
        ...
        endvertex.position = glm::vec3(
            pPoint4.x - m_origin.x, pPoint4.y - m_origin.y,
    ...
    if (Wires > 1) {
        auto const flo{
            static_cast<float>(Wires == 4 ? 0.25f * fHeightDifference : 0)};
...
int TTraction::TestPoint(
    glm::dvec3 const &Point) {  // sprawdzanie, czy przęsła można połączyć
    if ((hvNext[0] == nullptr) &&
...
void TTraction::Connect(int my, TTraction *with, int to) {
    ...
    if ((hvNext[0] != nullptr) && (hvNext[1] != nullptr)) {
    ...
    if ((with->hvNext[0] != nullptr) && (with->hvNext[1] != nullptr)) {
...
bool TTraction::WhereIs() {
    ...
    for (int endindex = 0; endindex < 2; ++endindex) {
        if (hvNext[endindex] == nullptr) {
            ...
            // otherwise if that neighbour has no further connection on the
            // opposite end then this piece is second-last
            iLast |= 2;
        }
    }
...
void TTraction::Init() {  // przeliczenie parametrów
    vParametric =
        pPoint2 -
        pPoint1;  // wektor mnożników parametru dla równania parametrycznego
};

void TTraction::ResistanceCalc(
    int d, double r,
    TTractionPowerSource
        *ps) {  //(this) jest przęsłem zasilanym, o rezystancji (r), policzyć
                //rezystancję zastępczą sąsiednich
    if (d >= 0) {  // podążanie we wskazanym kierunku
        TTraction *t = hvNext[d], *p;
        if (ps)
            psPower[d ^ 1] = ps;  // podłączenie podanego
        else
            ps = psPower[d ^ 1];  // zasilacz od przeciwnej strony niż idzie
                                  // analiza
        d = iNext[d];             // kierunek
        PowerState = 4;
        while ((t != nullptr) &&
               (t->psPower[d] == nullptr))  // jeśli jest jakiś kolejny i nie ma
                                            // ustalonego zasilacza
        {  // ustawienie zasilacza i policzenie rezystancji zastępczej
            ...
            t->psPower[d] =
                ps;  // skopiowanie wskaźnika zasilacza od danej strony
            t->fResistance[d] =
                r;  // wpisanie rezystancji w kierunku tego zasilacza
            r += t->fResistivity *
                 glm::length(
                     t->vParametric);  // doliczenie oporu kolejnego odcinka
            p = t;                     // zapamiętanie dotychczasowego
            t = p->hvNext[d ^ 1];      // podążanie w tę samą stronę
            d = p->iNext[d ^ 1];
            // w przypadku zapętlenia sieci może się zawiesić?
        }
    } else {  // podążanie w obu kierunkach, można by rekurencją, ale szkoda
              // zasobów
        r = 0.5 * fResistivity *
            glm::length(
                vParametric);  // powiedzmy, że w zasilanym przęśle jest połowa
...
void TTraction::PowerSet(
    TTractionPowerSource *ps) {  // podłączenie przęsła do zasilacza
    if (ps->bSection)
        psSection = ps;  // ustalenie sekcji zasilania
    else {  // ustalenie punktu zasilania (nie ma jeszcze połączeń między
            // przęsłami)
        psPowered = ps;  // ustawienie bezpośredniego zasilania dla przęsła
        psPower[0] = psPower[1] =
            ps;  // a to chyba nie jest dobry pomysł, bo nawet zasilane przęsło
        // powinno mieć wskazania na inne
        fResistance[0] = fResistance[1] =
            0.0;  // a liczy się tylko rezystancja zasilacza
    }
};

double TTraction::VoltageGet(
    double u, double i) {  // pobranie napięcia na przęśle po podłączeniu do
                           // niego rezystancji (res) - na razie jest to prąd
    if (!psSection)
        if (!psPowered)
            return NominalVoltage;  // jak nie ma zasilacza, to napięcie podane
                                    // w przęśle
    // na początek można założyć, że wszystkie podstacje mają to samo napięcie i
    // nie płynie prąd pomiędzy nimi dla danego przęsła mamy 3 źródła zasilania
    ...
    // 3. zasilacz psPowered z jego wewnętrzną rezystancją dla przęseł
    // zasilanych bezpośrednio
    ...
    if (psPower[0] && psPower[1]) {  // gdy przęsło jest zasilane z obu stron -
                                     // mamy trójkąt: res, r0t, r1t
        // yB: Gdy wywali podstacja, to zaczyna się robić nieciekawie - napięcie
        // w sekcji na jednym końcu jest równe zasilaniu, yB: a na drugim końcu
        // jest równe 0. Kolejna sprawa to rozróżnienie uszynienia sieci na
        // podstacji/odłączniku (czyli yB: potencjał masy na sieci) od braku
        // zasilania (czyli odłączenie źródła od sieci i brak jego wpływu na
        // napięcie).
        if ((r0t > 0.0) &&
            (r1t > 0.0)) {  // rezystancje w mianowniku nie mogą być zerowe
            r0g = res + r0t +
                  (res * r0t) / r1t;  // przeliczenie z trójkąta na gwiazdę
            r1g = res + r1t + (res * r1t) / r0t;
            // pobierane są prądy dla każdej rezystancji, a suma jest mnożona
            // przez rezystancję pojazdu w celu uzyskania napięcia
    ...
    } else if (psPower[0] && (r0t >= 0.0)) {  // jeśli odcinek podłączony jest
...
glm::vec3 TTraction::wire_color() const {
    glm::vec3 color;
    if (!DebugTractionFlag) {
        switch (Material) {  // Ra: kolory podzieliłem przez 2, bo po zmianie
                             // ambient za jasne były trzeba uwzględnić kierunek
                             // świecenia Słońca - tylko ze Słońcem widać kolor
            case 1: {
        ...
        switch (PowerState) {
            ...
            case 4: {
                ...
                color =
                    (psPowered != nullptr
                         ? glm::
                               vec3{239.f / 255.f, 239.f / 255.f, 239.f / 255.f}
                         : glm::vec3{
                               239.f / 255.f, 128.f / 255.f, 128.f / 255.f});
                break;
            }
            default: {
                break;
        ...
        if (hvParallel) {  // jeśli z bieżnią wspólną, to dodatkowo
                           // przyciemniamy
            color.r *= 0.6f;
            color.g *= 0.6f;
            color.b *= 0.6f;
...
float TTraction::radius_() {
    auto const points{endpoints()};
    auto radius{0.f};
    for (auto &point : points) {
        radius = std::max(
            radius, static_cast<float>(glm::length(m_area.center - point)));
    }
    return radius;
}
...
// stream
void TTraction::serialize_(std::ostream &Output) const {
    // TODO: implement
}
// deserialize() subclass details, restores content of the subclass from
// provided stream
void TTraction::deserialize_(std::istream &Input) {
    // TODO: implement
}

// export() subclass details, sends basic content of the class in legacy (text)
// format to provided stream
void TTraction::export_as_text_(std::ostream &Output) const {
    // header
    Output << "traction ";
    // basic attributes
    Output << asPowerSupplyName << ' ' << NominalVoltage << ' ' << MaxCurrent
           << ' ' << (fResistivity * 1000) << ' '
           << (Material == 2   ? "al"
               : Material == 0 ? "none"
                               : "cu")
           << ' ' << WireThickness << ' ' << DamageFlag << ' ';
    // path data
    Output << pPoint1.x << ' ' << pPoint1.y << ' ' << pPoint1.z << ' '
           << pPoint2.x << ' ' << pPoint2.y << ' ' << pPoint2.z << ' '
           << pPoint3.x << ' ' << pPoint3.y << ' ' << pPoint3.z << ' '
           << pPoint4.x << ' ' << pPoint4.y << ' ' << pPoint4.z << ' ';
    // minimum height
    Output << ((pPoint3.y - pPoint1.y + pPoint4.y - pPoint2.y) * 0.5 -
               fHeightDifference)
           << ' ';
    // segment length
    Output << static_cast<int>(
                  iNumSections ? glm::length(pPoint1 - pPoint2) / iNumSections
                               : 0.0)
           << ' ';
    // wire data
    Output << Wires << ' ' << WireOffset << ' ';
    // visibility
...
void traction_table::InitTraction() {
    ...
    for (auto *traction : m_items) {
        ...
        // wykonywane najpierw, żeby można było logować podłączenie 2 zasilaczy
        // do jednego drutu izolator zawieszony na przęśle jest ma być osobnym
        // odcinkiem drutu o długości ok. 1m, podłączonym do zasilacza o nazwie
        // "*" (gwiazka); "none" nie będzie odpowiednie
        auto *powersource =
            simulation::Powergrid.find(traction->asPowerSupplyName);
        if (powersource) {
            // jak zasilacz znaleziony to podłączyć do przęsła
            traction->PowerSet(powersource);
        } else {
            if ((traction->asPowerSupplyName !=
                 "*")  // gwiazdka dla przęsła z izolatorem
                && (traction->asPowerSupplyName !=
                    "none")) {  // dopuszczamy na razie brak podłączenia?
                // logowanie błędu i utworzenie zasilacza o domyślnej zawartości
                ErrorLog(
                    "Bad scenario: traction piece connected to nonexistent "
                    "power source \"" +
                    traction->asPowerSupplyName + "\"");
                ...
                powersource->Init(
                    traction->NominalVoltage, traction->MaxCurrent);
    ...
    for (auto first = std::rbegin(m_items); first != std::rend(m_items);
        ...
        if (traction->hvNext[0] == nullptr) {
            ...
            std::tie(matchingtraction, connection) =
                simulation::Region->find_traction(traction->pPoint1, traction);
            switch (connection) {
                case 0:
                case 1: {
            ...
            if (traction->hvNext[0]) {
                ...
                if ((traction->psSection != nullptr) &&
                    ...
                    // tylko przęsło z izolatorem może nie mieć zasilania, bo ma
                    // 2, trzeba sprawdzać sąsiednie
                    if (traction->psSection != matchingtraction->psSection) {
                        // połączone odcinki mają różne zasilacze
                        // to może być albo podłączenie podstacji lub kabiny
                        // sekcyjnej do sekcji, albo błąd
                        if ((true == traction->psSection->bSection) &&
                            (false == matchingtraction->psSection->bSection)) {
                            //(tmp->psSection) jest podstacją, a
                            //(Traction->psSection) nazwą sekcji
                            matchingtraction->PowerSet(
                                traction->psSection);  // zastąpienie wskazaniem
                                                       // sekcji
                        } else if (
                            (false == traction->psSection->bSection) &&
                            (true == matchingtraction->psSection->bSection)) {
                            //(Traction->psSection) jest podstacją, a
                            //(tmp->psSection) nazwą sekcji
                            traction->PowerSet(
                                matchingtraction
                                    ->psSection);  // zastąpienie wskazaniem
                                                   // sekcji
                        } else {
                            // jeśli obie to sekcje albo obie podstacje, to
                            // będzie błąd
                            ErrorLog(
                                "Bad scenario: faulty traction power "
                                "connection at location " +
                                to_string(traction->pPoint1));
                        }
                    }
                }
            }
        ...
        if (traction->hvNext[1] == nullptr) {
            // tylko jeśli jeszcze nie podłączony
            std::tie(matchingtraction, connection) =
                simulation::Region->find_traction(traction->pPoint2, traction);
            switch (connection) {
                case 0:
                case 1: {
    ...
    auto endcount{0};
    for (auto *traction : m_items) {
        // operacje mające na celu wykrywanie bieżni wspólnych i łączenie
        // przęseł naprążania
        if (traction->WhereIs()) {
    ...
    std::vector<TTraction *> ends;
    ends.reserve(endcount);
    for (auto *traction : m_items) {
        ...
        if (false == traction->asParallel.empty()) {
            ...
            if ((traction->asParallel == "none") ||
                (traction->asParallel == "*")) {
                // jeśli nieokreślone
                traction->iLast |= 2;  // jakby przedostatni - niech po prostu
                                       // szuka (iLast już przeliczone)
                ...
                if (nTemp != nullptr) {
                    ...
                    if (nTemp->hvParallel == nullptr) {
                        ...
                        traction->hvParallel =
                            nTemp;  // wpisać siebie i dalej dać mu wskaźnik
                                    // zwrotny
                    } else {
                        // a jak ma, to albo dołączyć się do kółeczka
                        traction->hvParallel =
                            nTemp->hvParallel;  // przjąć dotychczasowy wskaźnik
                                                // od niego
                    }
                    nTemp->hvParallel =
                ...
                if (traction->hvParallel == nullptr) {
                    ErrorLog(
                        "Missed overhead: " +
                        traction->asParallel);  // logowanie braku
                }
            }
        }
    ...
    do {
        ...
        // flaga podłączonych przęseł końcowych: -1=puste wskaźniki, 0=coś
        // zostało, 1=wykonano łączenie
        connected = false;
        for (auto &end : ends) {
            ...
            // każdy przebieg to próba podłączenia końca segmentu naprężania do
            // innego zasilanego przęsła
            if (end->hvNext[0] != nullptr) {
                // jeśli końcowy ma ciąg dalszy od strony 0 (Point1), szukamy
                // odcinka najbliższego do Point2
                std::tie(matchingtraction, connection) =
                    simulation::Region->find_traction(end->pPoint2, end, 0);
                if (matchingtraction != nullptr) {
                    // jak znalezione przęsło z zasilaniem, to podłączenie
                    // "równoległe"
                    end->ResistanceCalc(
                        0, matchingtraction->fResistance[connection],
                        matchingtraction->psPower[connection]);
                    // jak coś zostało podłączone, to może zasilanie gdzieś
                    // dodatkowo dotrze
                    connected = true;
                    end = nullptr;
                }
            } else if (end->hvNext[1] != nullptr) {
                // jeśli końcowy ma ciąg dalszy od strony 1 (Point2), szukamy
                // odcinka najbliższego do Point1
                std::tie(matchingtraction, connection) =
                    simulation::Region->find_traction(end->pPoint1, end, 1);
                if (matchingtraction != nullptr) {
                    // jak znalezione przęsło z zasilaniem, to podłączenie
                    // "równoległe"
                    end->ResistanceCalc(
                        1, matchingtraction->fResistance[connection],
                        matchingtraction->psPower[connection]);
                    // jak coś zostało podłączone, to może zasilanie gdzieś
                    // dodatkowo dotrze
                    connected = true;
                ...
                // gdy koniec jest samotny, to na razie nie zostanie podłączony
                // (nie powinno takich być)
                end = nullptr;
            }
        }
