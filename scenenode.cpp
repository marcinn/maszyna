...
void lighting_data::serialize(std::ostream &Output) const {
...
void lighting_data::deserialize(std::istream &Input) {
...
// stores content of the struct in provided output stream
void bounding_area::serialize(std::ostream &Output) const {
    // center
...
// restores content of the struct from provided input stream
void bounding_area::deserialize(
    std::istream &Input, bool const Preserveradius) {
    center = sn_utils::d_dvec3(Input);
    radius =
        (Preserveradius ? std::max(radius, sn_utils::ld_float32(Input))
                        : sn_utils::ld_float32(Input));
}

// sends content of the struct to provided stream
void shape_node::shapenode_data::serialize(std::ostream &Output) const {
    // bounding area
    ...
    sn_utils::s_str(
        Output,
        (material != null_handle ? GfxRenderer->Material(material).name : ""));
    ...
    for (auto const &vertex : vertices) {
        gfx::basic_vertex(
            glm::vec3{vertex.position - origin}, vertex.normal, vertex.texture)
            .serialize(Output);
    }
}

// restores content of the struct from provided input stream
void shape_node::shapenode_data::deserialize(std::istream &Input) {
    // bounding area
...
void shape_node::serialize(std::ostream &Output) const {
    // name
...
shape_node &shape_node::deserialize(std::istream &Input) {
    // name
...
// restores content of the node from provided input stream
shape_node &shape_node::import(
    cParser &Input, scene::node_data const &Nodedata) {
    // import common data
    m_name = Nodedata.name;
    m_data.rangesquared_min = Nodedata.range_min * Nodedata.range_min;
    m_data.rangesquared_max =
        (Nodedata.range_max >= 0.0 ? Nodedata.range_max * Nodedata.range_max
    ...
    if (token == "material") {
        ...
        while (token != "endmaterial") {
            if (token == "ambient:") {
                Input.getTokens(3);
                Input >> m_data.lighting.ambient.r >>
                    m_data.lighting.ambient.g >> m_data.lighting.ambient.b;
                m_data.lighting.ambient /= 255.f;
                m_data.lighting.ambient.a = 1.f;
            } else if (token == "diffuse:") {
                Input.getTokens(3);
                Input >> m_data.lighting.diffuse.r >>
                    m_data.lighting.diffuse.g >> m_data.lighting.diffuse.b;
                m_data.lighting.diffuse /= 255.f;
                m_data.lighting.diffuse.a = 1.f;
            } else if (token == "specular:") {
                Input.getTokens(3);
                Input >> m_data.lighting.specular.r >>
                    m_data.lighting.specular.g >> m_data.lighting.specular.b;
    ...
    bool const clamps = (texturehandle ? contains(texture.traits, 's') : false);
    bool const clampt = (texturehandle ? contains(texture.traits, 't') : false);

    // remainder of legacy 'problend' system -- geometry assigned a texture with
    // '@' in its name is treated as translucent, opaque otherwise
    if (texturehandle != null_handle) {
        m_data.translucent =
            (((contains(texture.name, '@')) && (true == texture.has_alpha))
                 ? true
                 : false);
    } else {
    ...
    // geometry
    enum subtype { triangles, triangle_strip, triangle_fan };
    ...
    do {
        ...
        if (true == clamps) {
            vertex.texture.s = clamp(vertex.texture.s, 0.001f, 0.999f);
        }
        if (true == clampt) {
            vertex.texture.t = clamp(vertex.texture.t, 0.001f, 0.999f);
        }
        // convert all data to gl_triangles to allow data merge for matching
        // nodes
        switch (nodetype) {
            case triangles: {
                if (vertexcount == 0) {
                    ...
                    if (false == degenerate(
                        ...
                        ErrorLog(
                            ...
                            (m_name != "" ? " in node \"" + m_name + "\""
                                          : "") +
                            " (vertices: " + to_string(vertex1.position) +
                            " + " + to_string(vertex2.position) + " + " +
                            to_string(vertex.position) + ")");
                    }
                }
                ++vertexcount;
                if (vertexcount > 2) {
                    vertexcount = 0;
                }  // start new triangle if needed
                break;
            }
            case triangle_fan: {
                if (vertexcount == 0) {
            ...
            case triangle_strip: {
                if (vertexcount == 0) {
                    ...
                    if (false == degenerate(
                                     vertex1.position, vertex2.position,
                                     vertex.position)) {
                        // swap order every other triangle, to maintain
                        // consistent winding
                        if (vertexcount % 2 == 0) {
                            m_data.vertices.emplace_back(vertex1);
                            m_data.vertices.emplace_back(vertex2);
...
shape_node &shape_node::convert(TSubModel const *Submodel) {
    ...
    m_data.translucent =
        (GfxRenderer->Material(m_data.material).get_or_guess_opacity() == 0.0f);
    // NOTE: we set unlimited view range typical for terrain, because we don't
    // expect to convert any other 3d models
    m_data.rangesquared_max = std::numeric_limits<double>::max();

    if (Submodel->m_geometry.handle == null_handle) {
        return *this;
    }
    ...
    world_vertex vertex, vertex1, vertex2;
    for (auto const &sourcevertex :
         GfxRenderer->Vertices(Submodel->m_geometry.handle)) {
        ...
        if (vertexcount == 0) {
            ...
            if (false ==
                degenerate(
                    vertex1.position, vertex2.position, vertex.position)) {
    ...
    if (true == importedvertices.empty()) {
        return *this;
    }
    ...
    for (auto const &vertex : m_data.vertices) {
        squareradius = std::max(
            squareradius, glm::length2(vertex.position - m_data.area.center));
    }
    m_data.area.radius = std::max(
        m_data.area.radius, static_cast<float>(std::sqrt(squareradius)));

    return *this;
}

// adds content of provided node to already enclosed geometry. returns: true if
// merge could be performed
bool shape_node::merge(shape_node &Shape) {
    ...
    m_data.area.center = interpolate(
        m_data.area.center, Shape.m_data.area.center,
        static_cast<double>(Shape.m_data.vertices.size()) /
            (Shape.m_data.vertices.size() + m_data.vertices.size()));
    m_data.vertices.insert(
        std::end(m_data.vertices), std::begin(Shape.m_data.vertices),
        std::end(Shape.m_data.vertices));
    invalidate_radius();

    return true;
}

// generates renderable version of held non-instanced geometry in specified
// geometry bank
void shape_node::create_geometry(gfx::geometrybank_handle const &Bank) {
    gfx::vertex_array vertices;
    vertices.reserve(m_data.vertices.size());

    for (auto const &vertex : m_data.vertices) {
        vertices.emplace_back(
            vertex.position - m_data.origin, vertex.normal, vertex.texture);
    }
...
void shape_node::compute_radius() {
    auto squaredradius{0.0};
    for (auto const &vertex : m_data.vertices) {
        squaredradius = std::max(
            squaredradius, glm::length2(vertex.position - m_data.area.center));
    }
    m_data.area.radius = static_cast<float>(std::sqrt(squaredradius));
}

void shape_node::invalidate_radius() { m_data.area.radius = -1.0f; }

float shape_node::radius() {
    if (m_data.area.radius == -1.0f) compute_radius();

    return m_data.area.radius;
}

// sends content of the struct to provided stream
void lines_node::linesnode_data::serialize(std::ostream &Output) const {
    // bounding area
    area.serialize(Output);
    // visibility
    sn_utils::ls_float64(Output, rangesquared_min);
...
// sends content of the class to provided stream
void lines_node::serialize(std::ostream &Output) const {
    // name
    sn_utils::s_str(Output, m_name);
    // node data
    m_data.serialize(Output);
...
// restores content of the node from provided input stream
lines_node &lines_node::deserialize(std::istream &Input) {
    // name
    m_name = sn_utils::d_str(Input);
    // node data
    m_data.deserialize(Input);
...
lines_node &lines_node::import(
    ...
    Input >> m_data.lighting.diffuse.r >> m_data.lighting.diffuse.g >>
        m_data.lighting.diffuse.b;
    m_data.lighting.diffuse /= 255.f;
    m_data.lighting.diffuse.a = 1.f;
    Input.getTokens(1, false);
    Input >> m_data.line_width;
    m_data.line_width =
        std::min(30.f, m_data.line_width);  // 30 pix equals rougly width of a
                                            // signal pole viewed from ~1m away

    // geometry
    enum subtype { lines, line_strip, line_loop };
    ...
    if ((nodetype == line_loop) && (vertexcount > 2)) {
        m_data.vertices.emplace_back(vertex1);
        m_data.vertices.emplace_back(vertex0);
    }
    if (m_data.vertices.size() % 2 != 0) {
        ErrorLog(
            "Lines node specified odd number of vertices, defined in file \"" +
...
// adds content of provided node to already enclosed geometry. returns: true if
// merge could be performed
bool lines_node::merge(lines_node &Lines) {
    ...
    m_data.area.center = interpolate(
        ...
        static_cast<double>(Lines.m_data.vertices.size()) /
            (Lines.m_data.vertices.size() + m_data.vertices.size()));
    m_data.vertices.insert(
        std::end(m_data.vertices), std::begin(Lines.m_data.vertices),
        std::end(Lines.m_data.vertices));
    // NOTE: we could recalculate radius with something other than brute force,
    // but it'll do
    compute_radius();
...
// generates renderable version of held non-instanced geometry in specified
// geometry bank
void lines_node::create_geometry(gfx::geometrybank_handle const &Bank) {
    gfx::vertex_array vertices;
    vertices.reserve(m_data.vertices.size());
...
void lines_node::compute_radius() {
    auto squaredradius{0.0};
    for (auto const &vertex : m_data.vertices) {
        squaredradius = std::max(
            squaredradius, glm::length2(vertex.position - m_data.area.center));
...
// sends content of the class to provided stream
void basic_node::serialize(std::ostream &Output) const {
    // bounding area
...
void basic_node::deserialize(std::istream &Input) {
    // bounding area
...
void basic_node::export_as_text(std::ostream &Output) const {
    Output
        // header
        << "node"
        // visibility
        << ' '
        << (m_rangesquaredmax < std::numeric_limits<double>::max()
                ? std::sqrt(m_rangesquaredmax)
                : -1)
        << ' '
...
void basic_node::export_as_text(std::string &Output) const {
    std::stringstream converter;
    export_as_text(converter);
    Output += converter.str();
}

float const &basic_node::radius() {
