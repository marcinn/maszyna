...
void eu07_application::queue_screenshot() { m_screenshot_queued = true; }
...
int eu07_application::run_crashgui() {
    ...
    while (!glfwWindowShouldClose(m_windows.front())) {
        ...
        if (Global.asLang == "pl") {
            ImGui::Begin(
                u8"Raportowanie błędów", nullptr,
                ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoSavedSettings |
                    ImGuiWindowFlags_NoResize);
            ImGui::TextUnformatted(
                u8"Podczas ostatniego uruchomienia symulatora wystąpił "
                u8"błąd.\nWysłać raport o błędzie do deweloperów?\n");
            ImGui::TextUnformatted((u8"Usługa udostępniana przez " +
                                    crashreport_get_provider() + "\n")
                                       .c_str());
            y = ImGui::Button(u8"Tak", ImVec2S(60, 0));
            ImGui::SameLine();
            ImGui::Checkbox(
                u8"W przyszłości przesyłaj raporty o błędach automatycznie",
                &autoup);
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::BeginTooltip();
                ImGui::TextUnformatted(
                    u8"W celu wyłączenia tej funkcji będzie trzeba skasować "
                    u8"plik crashdumps/autoupload_enabled.conf");
                ImGui::EndTooltip();
            }
            ...
            ImGui::Begin(
                "Crash reporting", nullptr,
                ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoSavedSettings |
                    ImGuiWindowFlags_NoResize);
            ImGui::TextUnformatted(
                "Crash occurred during last launch of the simulator.\nSend "
                "crash report to developers?\n");
            ImGui::TextUnformatted(
                ("Service provided by " + crashreport_get_provider() + "\n")
                    .c_str());
            y = ImGui::Button("Yes", ImVec2S(60, 0));
            ImGui::SameLine();
            ImGui::Checkbox(
                "In future send crash reports automatically", &autoup);
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::BeginTooltip();
                ImGui::TextUnformatted(
                    "To disable this feature remove file "
                    "crashdumps/autoupload_enabled.conf");
                ImGui::EndTooltip();
            }
        ...
        if (y) {
            ...
            if (autoup) crashreport_set_autoupload();
            return 0;
        }
...
int eu07_application::init(int Argc, char *Argv[]) {
    ...
    WriteLog(
        "Starting MaSzyna rail vehicle simulator (release: " +
        Global.asVersion + ")");
    WriteLog(
        "For online documentation and additional files refer to: "
        "http://eu07.pl");
    WriteLog(
        "Authors: Marcin_EU, McZapkie, ABu, Winger, Tolaris, nbmx, OLO_EU, "
        "Bart, Quark-t, "
        "ShaXbee, Oli_EU, youBy, KURS90, Ra, hunter, szociu, Stele, Q, firleju "
        "and others");

    if (!crashreport_get_provider().empty())
        WriteLog(
            "Crashdump analysis provided by " + crashreport_get_provider() +
            "\n");
    ...
    if (crashreport_is_pending()) {  // run crashgui as early as possible
        if ((result = run_crashgui()) != 0) return result;
    }
    ...
    crashreport_add_info(
        "python_enabled", Global.python_enabled ? "yes" : "no");
...
double eu07_application::generate_sync() {
    if (Timer::GetDeltaTime() == 0.0) return 0.0;
    double sync = 0.0;
...
int eu07_application::run() {
    ...
    while (!glfwWindowShouldClose(m_windows.front()) && !m_modestack.empty()) {
        ...
        if (m_headtrack) m_headtrack->update();
        ...
        if (m_modes[m_modestack.top()]->is_command_processor()) {
            ...
            while (--loop_remaining > 0) {
                ...
                command_queue::commands_map local_commands =
                    simulation::Commands.pop_intercept_queue();
                double slave_sync;

                // if we're the server
                if (m_network && m_network->servers) {
                    // fetch from network layer command requests received from
                    // clients
                    command_queue::commands_map remote_commands =
                        m_network->servers->pop_commands();
                ...
                if (m_network && m_network->client) {
                    ...
                    auto frame_info = m_network->client->get_next_delta(
                        MAX_NETWORK_PER_FRAME - loop_remaining);
                ...
                // if we're the server
                if (m_network && m_network->servers) {
                    // send delta, sync, and commands we just executed to
                    // clients
                    double delta = Timer::GetDeltaTime();
                    double render = Timer::GetDeltaRenderTime();
                    m_network->servers->push_delta(
                        render, delta, sync, commands_to_exec);
                }
                ...
                if (m_network && m_network->client) {
                    ...
                    if (sync != slave_sync) {
                        WriteLog(
                            "net: desync! calculated: " + std::to_string(sync) +
                                ", received: " + std::to_string(slave_sync),
                            logtype::net);
            ...
            if (!loop_remaining) {
                ...
                m_modes[m_modestack.top()]->set_progress(
                    100.0f * (received - awaiting) / received);
            } else {
        ...
        if (m_network) m_network->update();

        auto const frametime{Timer::subsystem.mainloop_total.stop()};
        if (Global.minframetime.count() != 0.0f &&
            (Global.minframetime - frametime).count() > 0.0f) {
...
// issues request for a worker thread to perform specified task. returns: true
...
// ensures the main thread holds the python gil and can safely execute python
// calls
void eu07_application::acquire_python_lock() { m_taskqueue.acquire_lock(); }

// frees the python gil and swaps out the main thread
void eu07_application::release_python_lock() { m_taskqueue.release_lock(); }
...
void eu07_application::exit() {
    ...
    if (!Global.exec_on_exit.empty()) system(Global.exec_on_exit.c_str());
}
...
bool eu07_application::push_mode(eu07_application::mode const Mode) {
    ...
    if (!m_modes[Mode]) {
        ...
        if (!m_modes[Mode]->init()) return false;
    }
...
void eu07_application::set_progress(
    float const Progress, float const Subtaskprogress) {
    if (m_modestack.empty()) {
        return;
    }
...
void eu07_application::set_cursor_pos(
...
void eu07_application::on_key(
    int const Key, int const Scancode, int const Action, int const Mods) {
    if (ui_layer::key_callback(Key, Scancode, Action, Mods)) return;
...
void eu07_application::on_cursor_pos(
    double const Horizontal, double const Vertical) {
    if (m_modestack.empty()) {
        return;
    }
...
void eu07_application::on_mouse_button(
    int const Button, int const Action, int const Mods) {
    if (ui_layer::mouse_button_callback(Button, Action, Mods)) return;
...
void eu07_application::on_scroll(double const Xoffset, double const Yoffset) {
    if (ui_layer::scroll_callback(Xoffset, Yoffset)) return;
...
void eu07_application::on_char(unsigned int c) {
    if (ui_layer::char_callback(c)) return;
}

void eu07_application::on_focus_change(bool focus) {
    if (Global.bInactivePause &&
        !m_network->client) {  // jeśli ma być pauzowanie okna w tle
        command_relay relay;
        relay.post(
            user_command::focuspauseset, focus ? 1.0 : 0.0, 0.0, GLFW_PRESS, 0);
    }
}

GLFWwindow *eu07_application::window(
    int const Windowindex, bool visible, int width, int height,
    GLFWmonitor *monitor, bool keep_ownership, bool share_ctx) {
    if (Windowindex >= 0) {
        return (
            Windowindex < m_windows.size() ? m_windows[Windowindex] : nullptr);
    }
    // for index -1 create a new child window

    auto const *vmode{
        glfwGetVideoMode(monitor ? monitor : glfwGetPrimaryMonitor())};
    ...
    auto *childwindow = glfwCreateWindow(
        width, height, "eu07window", monitor,
        share_ctx ? m_windows.front() : nullptr);
    if (!childwindow) return nullptr;

    if (keep_ownership) m_windows.emplace_back(childwindow);
...
GLFWmonitor *eu07_application::find_monitor(const std::string &str) const {
    ...
    for (size_t i = 0; i < monitor_count; i++) {
        if (describe_monitor(monitors[i]) == str) return monitors[i];
    }
...
int eu07_application::init_glfw() {
    ...
    if ((Global.gfx_skippipeline || Global.LegacyRenderer) &&
    ...
    auto *mainwindow = window(
        -1, true, Global.window_size.x, Global.window_size.y,
        (Global.bFullScreen ? monitor : nullptr), true, false);
...
int eu07_application::init_gfx() {
    ...
    for (const global_settings::extraviewport_config &conf :
         Global.extra_viewports)
        if (!GfxRenderer->AddViewport(conf)) return -1;

    if (!Global.headtrack_conf.joy.empty()) m_headtrack.emplace();
...
int eu07_application::init_audio() {
    ...
    // NOTE: lack of audio isn't deemed a failure serious enough to throw in the
    // towel
    return 0;
}

int eu07_application::init_data() {
    // HACK: grab content of the first {} block in load_unit_weights using
    // temporary parser, then parse it normally. on any error our weight list
    // will be empty string
    auto loadweights{cParser(
        cParser("data/load_weights.txt", cParser::buffer_FILE)
            .getToken<std::string>(true, "{}"),
        cParser::buffer_TEXT)};
    while (true == loadweights.getTokens(2)) {
        std::pair<std::string, float> weightpair;
        loadweights >> weightpair.first >> weightpair.second;
        weightpair.first.erase(
            weightpair.first.end() - 1);  // trim trailing ':' from the key
...
bool eu07_application::init_network() {
    ...
    if (Global.network_client) {
        // create client
        m_network->connect(
            Global.network_client->first, Global.network_client->second);
    } else {
        // we're simulation master
        if (!Global.random_seed) Global.random_seed = std::random_device{}();
        ...
        int64_t offset =
            (tm_local.tm_hour * 3600 + tm_local.tm_min * 60 + tm_local.tm_sec) -
