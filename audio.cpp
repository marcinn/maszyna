...
openal_buffer::openal_buffer(std::string const &Filename) : name(Filename) {
    SF_INFO si;
    si.format = 0;
    ...
    if (sf == nullptr) throw std::runtime_error("sound: sf_open failed");
...
// creates buffer object out of data stored in specified file. returns: handle
// to the buffer or null_handle if creation failed
audio::buffer_handle buffer_manager::create(std::string const &Filename) {
    ...
    // if dynamic-specific and/or direct lookups find nothing, try the default
    // sound folder
    ...
    ErrorLog(
        "Bad file: failed to locate audio file \"" + Filename + "\"",
        logtype::file);
    return null_handle;
}

// provides direct access to a specified buffer
audio::openal_buffer const &buffer_manager::buffer(
    audio::buffer_handle const Buffer) const {
    return m_buffers[Buffer];
}

// places in the bank a buffer containing data stored in specified file.
// returns: handle to the buffer
audio::buffer_handle buffer_manager::emplace(std::string Filename) {
    ...
    erase_extension(Filename);
    m_buffermappings.emplace(Filename, handle);

    return handle;
}

audio::buffer_handle buffer_manager::find_buffer(
    std::string const &Buffername) const {
...
std::string buffer_manager::find_file(std::string const &Filename) const {
    auto const lookup{FileExists({Filename}, {".ogg", ".flac", ".wav"})};
