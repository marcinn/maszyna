...
enum class sound_category : unsigned int {
    ...
    local,    // source gain is altered by positional environment sound volume
              // modifier
    ambient,  // source gain is altered by ambient environment sound volume
              // modifier
};
...
enum class sync_state { good, bad_distance, bad_resource };
...
struct openal_source {
    ...
    openal_source &bind(
        sound_source *Controller, uint32_sequence Sounds, Iterator_ First,
        Iterator_ Last);
   ...
   private:
    ...
    glm::vec3 sound_distance{
        0.f};  // cached distance between sound and the listener
    glm::vec3 sound_velocity{0.f};  // sound movement vector
    bool is_in_range{false};        // helper, indicates the source was recently
                                    // within audible range
    bool is_multipart{
        false};  // multi-part sounds are kept alive at longer ranges
};
...
class openal_renderer {
   ...
   public:
    ...
    bool init();
    // schedules playback of provided range of samples, under control of the
    // specified sound emitter
    template <class Iterator_>
    void insert(
        Iterator_ First, Iterator_ Last, sound_source *Controller,
        uint32_sequence Sounds) {
        m_sources.emplace_back(
            fetch_source().bind(Controller, Sounds, First, Last));
    }
    // removes from the queue all sounds controlled by the specified sound
    // emitter
   ...
   private:
    ...
    // TBD: list of sources as vector, sorted by distance, for openal
    // implementations with limited number of active sources?
    source_list m_sources;
    source_sequence
        m_sourcespares;  // already created and currently unused sound sources
