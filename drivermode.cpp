...
#endif
    /*
        // TBD, TODO: wrap current command in object, include other input
       sources? input::command = ( mouse.command() != user_command::none ?
                mouse.command() :
                keyboard.command() );
    */
}
...
std::string driver_mode::drivermode_input::binding_hints(
    std::pair<user_command, user_command> const &Commands) const {
    ...
    std::string inputhints =
        inputhintleft + (inputhintright.empty()  ? ""
                         : inputhintleft.empty() ? inputhintright
                                                 : "] [" + inputhintright);

    return inputhints;
}

std::unordered_map<user_command, std::pair<user_command, user_command>>
    commandfallbacks = {
        ...
        {user_command::linebreakeropen,
         {user_command::linebreakertoggle, user_command::none}},
        ...
        {user_command::batteryenable,
         {user_command::batterytoggle, user_command::none}},
        {user_command::batterydisable,
         {user_command::batterytoggle, user_command::none}},
};

std::pair<user_command, user_command>
driver_mode::drivermode_input::command_fallback(
    user_command const Command) const {
...
driver_mode::driver_mode() { m_userinterface = std::make_shared<driver_ui>(); }

// initializes internal data structures of the mode. returns: true on success,
// false otherwise
bool driver_mode::init() { return m_input.init(); }

// mode-specific update of simulation data. returns: false on error, true
// otherwise
bool driver_mode::update() {
    ...
    if (deltatime != 0.0) {
        ...
        /*
                // NOTE: until we have no physics state interpolation during
           render, we need to rely on the old code,
                // as doing fixed step calculations but flexible step render
           results in ugly mini jitter
                // core routines (physics)
                int updatecount = 0;
                while( ( m_primaryupdateaccumulator >= m_primaryupdaterate )
                         &&( updatecount < 20 ) ) {
                        // no more than 20 updates per single pass, to keep
           physics from hogging up all run time Ground.Update(
           m_primaryupdaterate, 1 );
                        ++updatecount;
        ...
        */
        ...
        if (deltatime > m_primaryupdaterate)  // normalnie 0.01s
        {
            /*
                            ...
                            auto const iterations = std::ceil(dt /
               m_primaryupdaterate); updatecount = std::min( 20,
               static_cast<int>( iterations ) );
            */
            updatecount = std::ceil(deltatime / m_primaryupdaterate);
            /*
                            // NOTE: changing dt wrecks things further down the
               code. re-acquire proper value later or cleanup here dt = dt /
               iterations; // Ra: fizykę lepiej by było przeliczać ze stałym
               krokiem
            */
        }
        auto const stepdeltatime{deltatime / updatecount};
        // NOTE: updates are limited to 20, but dt is distributed over
        // potentially many more iterations this means at count > 20 simulation
        // and render are going to desync. is that right? NOTE: experimentally
        // changing this to prevent the desync.
        // TODO: test what happens if we hit more than 20 * 0.01 sec slices,
        // i.e. less than 5 fps
        ...
        while (m_secondaryupdateaccumulator >= m_secondaryupdaterate) {
            ...
            if (Global.iMultiplayer)  // dajemy znać do serwera o wykonaniu
                if (iPause !=
                    Global.iPause) {  // przesłanie informacji o pauzie do
                                      // programu nadzorującego
                    multiplayer::WyslijParam(
                        5, 3);  // ramka 5 z czasem i stanem zapauzowania
            ...
            m_secondaryupdateaccumulator -=
                m_secondaryupdaterate;  // these should be inexpensive enough we
                                        // have no cap
        }
        ...
        if (!change_train.empty()) {
            ...
            if (train) {
                ...
                InOutKey();
                m_relay.post(
                    user_command::aidriverdisable, 0.0, 0.0, GLFW_PRESS, 0);
                change_train.clear();
            }
        }

        if ((simulation::Train == nullptr) && (false == FreeFlyModeFlag)) {
            // intercept cases when the driven train got removed after entering
            // portal
            InOutKey();
        }

        if (!FreeFlyModeFlag &&
            simulation::Train->Dynamic() != Camera.m_owner) {
        ...
        if (simulation::Train != nullptr)
            TSubModel::iInstance =
                reinterpret_cast<std::uintptr_t>(simulation::Train->Dynamic());
        else
    ...
    auto const deltarealtime =
        Timer::GetDeltaRenderTime();  // nie uwzględnia pauzowania ani mnożenia
                                      // czasu
    simulation::State.process_commands();

    // fixed step render time routines

    fTime50Hz += deltarealtime;  // w pauzie też trzeba zliczać czas, bo przy
                                 // dużym FPS będzie problem z odczytem ramek
    bool runonce{false};
...
#endif
        ui::Transcripts
            .Update();  // obiekt obsługujący stenogramy dźwięków na ekranie
        ...
        if (false == runonce) {
            ...
            if ((train != nullptr) && (false == FreeFlyModeFlag)) {
                if (false == DebugModeFlag) {
                    // in regular mode show control functions, for defined
                    // controls
                    auto const controlname{
                        train->GetLabel(GfxRenderer->Pick_Control())};
                    if (false == controlname.empty()) {
                        auto const mousecommands{
                            m_input.mouse.bindings(controlname)};
                        auto inputhints{m_input.binding_hints(mousecommands)};
                        // if the commands bound with the control don't have any
                        // assigned keys try potential fallbacks
                        if (inputhints.empty()) {
                            inputhints = m_input.binding_hints(
                                m_input.command_fallback(mousecommands.first));
                        }
                        if (inputhints.empty()) {
                            inputhints = m_input.binding_hints(
                                m_input.command_fallback(mousecommands.second));
                        }
                    ...
                    // in debug mode show names of submodels, to help with cab
                    // setup and/or debugging
            ...
            if ((true == Global.ControlPicking) && (true == FreeFlyModeFlag) &&
                ...
                set_tooltip((scenerynode ? scenerynode->name() : ""));
            }
    ...
    simulation::Environment
        .update_precipitation();  // has to be launched after camera step to
                                  // work properly
    ...
    // NOTE: particle system runs on simulation time, but needs actual camera
    // position to determine how to update each particle source
    ...
    simulation::is_ready = simulation::is_ready ||
                           ((simulation::Train != nullptr) &&
                            (simulation::Train->is_cab_initialized)) ||
...
void driver_mode::enter() {
    ...
    Camera.Init(
        Global.FreeCameraInit[0], Global.FreeCameraInitAngle[0], nullptr);
    ...
    if (nPlayerTrain) {
        WriteLog(
            "Trying to enter player train, \"" + Global.local_start_vehicle +
            "\"");

        m_relay.post(
            user_command::entervehicle, 0.0, 0.0, GLFW_PRESS, 0,
            nPlayerTrain->GetPosition(), &Global.local_start_vehicle);
        change_train = nPlayerTrain->name();
    } else if (Global.local_start_vehicle != "ghostview") {
        Global.local_start_vehicle = "ghostview";
        Error(
            "Bad scenario: failed to locate player train, \"" +
            Global.local_start_vehicle + "\"");
    }
...
void driver_mode::on_key(
    int const Key, int const Scancode, int const Action, int const Mods) {
    ...
    if ((true == Global.InputMouse) &&
        ...
        // if the alt key was pressed toggle control picking mode and set
        // matching cursor behaviour
...
void driver_mode::on_cursor_pos(
    double const Horizontal, double const Vertical) {
    // give the ui first shot at the input processing...
...
void driver_mode::on_mouse_button(
    int const Button, int const Action, int const Mods) {
    // give the ui first shot at the input processing...
...
void driver_mode::on_event_poll() { m_input.poll(); }

bool driver_mode::is_command_processor() const { return true; }

void driver_mode::update_camera(double const Deltatime) {
    auto *controlled =
        (simulation::Train ? simulation::Train->Dynamic() : nullptr);

    if (false == Global.ControlPicking) {
        if (m_input.mouse.button(GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS) {
            Camera.Reset();  // likwidacja obrotów - patrzy horyzontalnie na
                             // południe
            if (Camera.m_owner == nullptr) {
                if (controlled &&
                    LengthSquared3(controlled->GetPosition() - Camera.Pos) <
                        (1500 * 1500)) {
                    // gdy bliżej niż 1.5km
                    Camera.LookAt = controlled->GetPosition() +
                                    0.4 * controlled->VectorUp() *
                                        controlled->MoverParameters->Dim.H;
                } else {
                    TDynamicObject *d = std::get<TDynamicObject *>(
                        simulation::Region->find_vehicle(
                            Camera.Pos, 300, false, false));
                    if (!d)
                        d = std::get<TDynamicObject *>(
                            simulation::Region->find_vehicle(
                                Camera.Pos, 1000, false,
                                false));  // dalej szukanie, jesli bliżej nie ma

                    if (d && pDynamicNearest) {
                        // jeśli jakiś jest znaleziony wcześniej
                        if (100.0 *
                                LengthSquared3(d->GetPosition() - Camera.Pos) >
                            LengthSquared3(
                                pDynamicNearest->GetPosition() - Camera.Pos)) {
                            d = pDynamicNearest;  // jeśli najbliższy nie jest
                                                  // 10 razy bliżej niż
                        }
                    }
                    ...
                    if (d)
                        pDynamicNearest =
                            d;  // zmiana na nowy, jeśli coś znaleziony niepusty
                    if (pDynamicNearest)
                        Camera.LookAt =
                            pDynamicNearest->GetPosition() +
                            0.4 * pDynamicNearest->VectorUp() *
                                pDynamicNearest->MoverParameters->Dim.H;
                }
                Camera.RaLook();  // jednorazowe przestawienie kamery
            } else {
                if (false == FreeFlyModeFlag) {
                    // reset cached view angle in the cab
                    simulation::Train->pMechViewAngle = {
    ...
    if (m_input.mouse.button(GLFW_MOUSE_BUTTON_MIDDLE) == GLFW_PRESS) {
        ...
        Global.ZoomFactor = std::min(
            4.5f, Global.ZoomFactor + 15.0f * static_cast<float>(Deltatime));
        ...
        Global.ZoomFactor = std::max(
            1.0f, Global.ZoomFactor - 15.0f * static_cast<float>(Deltatime));
    }
    ...
    if (false == DebugCameraFlag) {
        ...
        if ((simulation::Train != nullptr) && (false == FreeFlyModeFlag) &&
            ...
            // if in cab potentially alter camera placement based on changes in
            // train object
        ...
        if ((simulation::Train != nullptr) && (false == FreeFlyModeFlag)) {
            ...
            Camera.m_owneroffset =
                simulation::Train->clamp_inside(Camera.m_owneroffset);
        }
    ...
    if ((simulation::Train != nullptr) && (Camera.m_owner != nullptr) &&
        (false == DebugCameraFlag)) {
        // jeśli jazda w kabinie, przeliczyć trzeba parametry kamery
        /*
                auto tempangle = controlled->VectorFront() * (
           controlled->MoverParameters->CabOccupied == -1 ? -1 : 1 ); double
           modelrotate = atan2( -tempangle.x, tempangle.z );
        */
        if ((false == FreeFlyModeFlag) && (true == Global.ctrlState) &&
            ...
            // Camera.Yaw powinno być wyzerowane, aby po powrocie patrzeć do
            // przodu
            Camera.Pos =
                controlled->GetPosition() +
                simulation::Train->MirrorPosition(lr);  // pozycja lusterka
            ...
            if (simulation::Train->Occupied()->CabOccupied == 0) {
                ...
                Camera.LookAt =
                    Camera.Pos - (lr ? -1 : 1) * controlled->VectorLeft() *
                                     simulation::Train->Occupied()->CabOccupied;
            } else {  // patrzenie w kierunku osi pojazdu, z uwzględnieniem
                      // kabiny - jakby z lusterka,
                // ale bez odbicia
                Camera.LookAt =
                    Camera.Pos -
                    simulation::Train->GetDirection() *
                        simulation::Train->Occupied()->CabOccupied;  //-1 albo 1
            }
            auto const shakeangles{
                simulation::Train->Dynamic()->shake_angles()};
            Camera.Angle.x =
                0.5 * shakeangles.second;        // hustanie kamery przod tyl
            Camera.Angle.z = shakeangles.first;  // hustanie kamery na boki
                                                 /*
                                                             Camera.Roll = std::atan( simulation::Train->pMechShake.x
                                                    * simulation::Train->BaseShake.angle_scale.x ); // hustanie
                                                    kamery na boki                                      Camera.Pitch = 0.5 * std::atan(
                                                    simulation::Train->vMechVelocity.z *
                                                    simulation::Train->BaseShake.angle_scale.z ); // hustanie kamery
                                                    przod tyl
                                                 */
            ...
            if (false == FreeFlyModeFlag) {
                // potentially restore view angle after returning from external
                // view
            ...
            Camera.Pos = (Camera.m_owner->GetWorldPosition(
                FreeFlyModeFlag ? shakencamerapos
                                :  // TODO: vehicle collision box for the
                                   // external vehicle camera
            ...
            if (!Global.iPause) {
                ...
                Camera.Angle.z = shakeangles.first;  // hustanie kamery na boki
                                                     /*
                                                                     Camera.Roll = std::atan(
                                                        simulation::Train->vMechVelocity.x *
                                                        simulation::Train->BaseShake.angle_scale.x ); // hustanie
                                                        kamery na boki                                      Camera.Pitch -= 0.5 * atan(
                                                        simulation::Train->vMechVelocity.z *
                                                        simulation::Train->BaseShake.angle_scale.z ); // hustanie
                                                        kamery przod tyl
                                                     */
            }
            /*
...
void driver_mode::OnKeyDown(int cKey) {
    ...
    if (keyinfo.empty() == false) {
        ...
        if (Global.shiftState) keymodifiers += "[Shift]+";
        if (Global.ctrlState) keymodifiers += "[Ctrl]+";
        ...
        if (Global.shiftState) {
            ...
            if ((Global.iPause == 0)  // podczas pauzy klawisze nie działają
                ...
                m_relay.post(
                    ...
                    glm::vec3(0.0f), &KeyEvents[i]->name());
            }
            ...
            if (FreeFlyModeFlag) {
                ...
                if ((Global.FreeCameraInit[i].x == 0.0) &&
                    ...
                    // jeśli kamera jest w punkcie zerowym, zapamiętanie
                    // współrzędnych i kątów
                    ...
                    WriteLog(
                        ...
                        std::to_string(
                            RadToDeg(Global.FreeCameraInitAngle[i].x)) +
                        " " +
                        std::to_string(
                            RadToDeg(Global.FreeCameraInitAngle[i].y)) +
                        " " +
                        std::to_string(
                            RadToDeg(Global.FreeCameraInitAngle[i].z)) +
                        " " + std::to_string(i) + " endcamera");
                } else  // również przeskakiwanie
                {  // Ra: to z tą kamerą (Camera.Pos i Global.pCameraPosition)
                   // jest trochę bez sensu
                    Global.pCamera.Pos =
                        Global.FreeCameraInit[i];  // nowa pozycja dla
                                                   // generowania obiektów
                    Camera.Init(
                        Global.FreeCameraInit[i], Global.FreeCameraInitAngle[i],
                        nullptr);  // przestawienie
                }
            }
        }
        return;
    ...
    switch (cKey) {
        ...
        case GLFW_KEY_F5: {
            ...
            TDynamicObject *dynamic =
                std::get<TDynamicObject *>(simulation::Region->find_vehicle(
                    Global.pCamera.Pos, 50, false, false));
            if (dynamic) {
                m_relay.post(
        ...
        case GLFW_KEY_F6: {
            ...
            if (DebugModeFlag) {
                if (Global.ctrlState) {
                    ...
                    Global.fTimeSpeed =
                        (Global.shiftState ? 5.0 : Global.default_timespeed);
                }
            }
            break;
        }
        case GLFW_KEY_F7: {
            ...
            if (DebugModeFlag) {
                if (Global.ctrlState && Global.shiftState) {
                    // shift + ctrl + f7 toggles between debug and regular
                    // camera
...
// places camera outside the controlled vehicle, or nearest if nothing is under
// control depending on provided switch the view is placed right outside, or at
// medium distance
void driver_mode::DistantView(bool const Near) {
    TDynamicObject const *vehicle = {
        ((simulation::Train != nullptr) ? simulation::Train->Dynamic()
                                        : pDynamicNearest)};
    ...
    if (true == Near) {
        Camera.Pos = Math3D::vector3(
                         Camera.Pos.x, vehicle->GetPosition().y, Camera.Pos.z) +
                     left * vehicle->GetWidth() +
                     Math3D::vector3(1.25 * left.x, 1.6, 1.25 * left.z);
    } else {
        Camera.Pos = vehicle->GetPosition() +
                     vehicle->VectorFront() *
                         vehicle->MoverParameters->CabOccupied * 50.0 +
...
void driver_mode::ExternalView() {
    ...
    if (true == m_externalview) {
        // we're already in some external view mode, so select next one on the
        // list
        m_externalviewmode = clamp_circular(
            ++m_externalviewmode, static_cast<int>(view::count_));
    }
    ...
    switch (m_externalviewmode) {
            ...
            if (owner == viewconfig.owner) {
                ...
                auto const offsetflip{
                    (vehicle->MoverParameters->CabOccupied == 0
                         ? 1
                         : vehicle->MoverParameters->CabOccupied) *
                    (vehicle->MoverParameters->DirActive == 0
                         ? 1
                         : vehicle->MoverParameters->DirActive)};
                ...
                Camera.Angle.y = glm::radians(
                    (vehicle->MoverParameters->DirActive < 0 ? 180.0 : 0.0));
            }
        ...
        case view::consistrear: {
            ...
            if (owner == viewconfig.owner) {
                ...
                Camera.Angle.y = glm::radians(
                    (vehicle->MoverParameters->DirActive < 0 ? 0.0 : 180.0));
            }
            auto const shakeangles{owner->shake_angles()};
            Camera.Angle.x -=
                0.5 * shakeangles.second;        // hustanie kamery przod tyl
        ...
        case view::bogie: {
            ...
            if (owner == viewconfig.owner) {
                ...
                Camera.m_owneroffset = {
                    -0.65 * owner->MoverParameters->Dim.W * offsetflip, 0.90,
...
void driver_mode::CabView() {
    ...
    if (train->Occupied()->CabOccupied == 0) {
        Camera.LookAt = Camera.m_owner->GetWorldPosition(Camera.m_owneroffset) +
                        Camera.m_owner->VectorFront() * 5.0;
    } else {
...
void driver_mode::InOutKey() {  // przełączenie widoku z kabiny na zewnętrzny i
                                // odwrotnie
    ...
    if (FreeFlyModeFlag) {
        ...
        // cache current cab position so there's no need to set it all over
        // again after each out-in switch
    ...
    Application.set_title(
        Global.AppName + " (" +
        (train != nullptr ? train->Occupied()->Name : "") + " @ " +
        Global.SceneryFile + ")");
}
...
void driver_mode::set_picking(bool const Picking) {
    if (Picking) {
        ...
        Application.set_cursor_pos(
            m_input.mouse_pickmodepos.x, m_input.mouse_pickmodepos.y);
