...
// used by the renderer to determine most suitable placement for actual light
// sources during render
struct light_array {
   public:
    // types
    struct light_record {
        ...
        TDynamicObject const
            *owner;           // the object in world which 'carries' the light
        ...
        glm::vec3 color{
            255.0f / 255.0f, 241.0f / 255.0f,
            224.0f / 255.0f};   // color of the light, default is halogen light
    ...
    void insert(TDynamicObject const *Owner);
    // removes records for lights of specified owner from the collection
    void remove(TDynamicObject const *Owner);
