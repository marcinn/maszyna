...
ui::keymapper_panel::keymapper_panel() : ui_panel(STR("Keymapper"), false) {
    keyboard.init();
}

bool ui::keymapper_panel::key(int key) {
    if (!is_open || bind_active == user_command::none) return false;

    auto it = keyboard.keytonamemap.find(key);
    if (it == keyboard.keytonamemap.end()) return false;

    if (Global.shiftState) key |= keyboard_input::keymodifier::shift;
    if (Global.ctrlState) key |= keyboard_input::keymodifier::control;
...
void ui::keymapper_panel::render() {
    ...
    if (ImGui::Begin(panelname.c_str(), &is_open)) {
        if (ImGui::Button(STR_C("Load"))) keyboard.init();

        ImGui::SameLine();

        if (ImGui::Button(STR_C("Save"))) keyboard.dump_bindings();

        for (const std::pair<user_command, int> &binding :
             keyboard.bindings()) {
            ...
            if (it != keyboard.keytonamemap.end()) label += it->second;
            ...
            if (styles_pushed) ImGui::PopStyleColor();
        }
    }
