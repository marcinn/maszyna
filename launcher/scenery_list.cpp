...
void ui::scenerylist_panel::draw_scenery_list() {
    ...
    for (auto &desc : scanner.scenarios) {
        ...
        if (collapse_open) {
            if (just_opened) selected_scenery = &desc;
...
void ui::scenerylist_panel::draw_scenery_image() {
    if (!selected_scenery->image_path.empty()) {
        ...
        desc->image =
            GfxRenderer->Fetch_Texture(selected_scenery->image_path, true);
    ...
    if (selected_scenery->image != null_handle) {
        ...
        if (tex.is_ready) {
            ...
            ImGui::Image(
                reinterpret_cast<void *>(tex.id), ImVec2(avail_width, height),
...
void ui::scenerylist_panel::draw_launch_box() {
    ...
    if (ImGui::Button(STR_C("Launch"), ImVec2(-1, 0))) {
        if (!launch_simulation()) ImGui::OpenPopup("missing_driver");
    }
...
bool ui::scenerylist_panel::launch_simulation() {
    ...
    for (auto &desc : selected_scenery->trainsets) add_replace_entry(desc);
...
void ui::scenerylist_panel::add_replace_entry(const trainset_desc &trainset) {
    ...
    for (const auto &veh : trainset.vehicles) {
        ...
        if (veh.params.size() > 0) set += "." + veh.params;
        set += " " + std::to_string(veh.loadcount) + " ";
        if (veh.loadcount > 0) set += veh.loadtype + " ";
        set += "enddynamic\n";
    }
    set += "endtrainset\n";
...
void ui::scenerylist_panel::render_contents() {
    ...
    if (ImGui::BeginChild("child1", ImVec2(0, -200))) draw_scenery_list();
    ImGui::EndChild();

    ImGui::NextColumn();

    if (ImGui::BeginChild("child2", ImVec2(0, -200))) draw_scenery_info();
    ImGui::EndChild();
    ...
    if (selected_scenery) {
        if (ImGui::BeginChild(
                "child3", ImVec2(0, -200), false, ImGuiWindowFlags_NoScrollbar))
            draw_scenery_image();
        ImGui::EndChild();
    }
    ImGui::Columns();
    ...
    if (ImGui::BeginChild("child4")) {
        if (selected_scenery) {
            if (selected_trainset) ImGui::Columns(2);

            if (ImGui::BeginChild(
                    "child5", ImVec2(0, 0), false,
                    ImGuiWindowFlags_HorizontalScrollbar))
                draw_trainset_box();
            ImGui::EndChild();

            if (selected_trainset) draw_launch_box();
        }
    }
    ImGui::EndChild();
}
...
void ui::scenerylist_panel::draw_summary_tooltip(const dynamic_desc &dyn_desc) {
    std::string name =
        (dyn_desc.vehicle->path.parent_path() / dyn_desc.vehicle->path.stem())
            .string();
    ...
    if (dyn_desc.loadcount > 0)
        ImGui::Text(
            STR_C("Load: %s: %d"), dyn_desc.loadtype.c_str(),
            dyn_desc.loadcount);
...
void ui::scenerylist_panel::draw_trainset(trainset_desc &trainset) {
    ...
    if (ImGui::Selectable(
            "##set", selected_trainset == &trainset, 0, ImVec2(0, 30)))
    ...
    for (auto &dyn_desc : trainset.vehicles) {
        ...
        ImGui::Image(
            reinterpret_cast<void *>(mini->get()), ImVec2(width, 30),
            ImVec2(0, 1), ImVec2(1, 0));
        ...
        if (ImGui::BeginDragDropSource(
            ...
            ImGui::SetDragDropPayload(
                "vehicle_set", &data, sizeof(vehicle_moved));
        ...
        if (ImGui::IsItemDeactivated() && ImGui::IsItemHovered()) {
            ...
            register_popup(
                std::make_unique<ui::dynamic_edit_popup>(*this, dyn_desc));
...
void ui::scenerylist_panel::draw_droptarget(
    trainset_desc &trainset, int position) {
    ImGui::Dummy(ImVec2(15, 30));
    if (ImGui::BeginDragDropTarget()) {
        const ImGuiPayload *payload =
            ImGui::AcceptDragDropPayload("vehicle_pure");
        if (payload) {
            ...
            desc.name = skin->skin + "_" +
                        std::to_string((int)LocalRandom(0.0, 10000000.0));
        ...
        if (payload) {
            vehicle_moved *moved =
                reinterpret_cast<vehicle_moved *>(payload->Data);

            dynamic_desc desc_copy = moved->dynamic;
            int offset = 0;
            if (&trainset == &moved->source) {
                trainset.vehicles.erase(
                    trainset.vehicles.begin() + moved->position);
                if (moved->position < position) offset = -1;
            } else {
                desc_copy.name =
                    desc_copy.skin->skin + "_" +
                    std::to_string((int)LocalRandom(0.0, 10000000.0));
            }

            trainset.vehicles.insert(
                trainset.vehicles.begin() + position + offset, desc_copy);
        }
        ImGui::EndDragDropTarget();
    }
}

ui::dynamic_edit_popup::dynamic_edit_popup(
    ui_panel &panel, dynamic_desc &dynamic)
...
void ui::dynamic_edit_popup::render_content() {
    static std::vector<std::string> occupancy_names = {
        STRN("headdriver"), STRN("reardriver"), STRN("passenger"),
        STRN("nobody")};

    std::string name =
        (dynamic.vehicle->path.parent_path() / dynamic.vehicle->path.stem())
            .string();
    ...
    if (ImGui::BeginCombo(
            STR_C("Skin"), dynamic.skin->skin.c_str(),
    ...
    if (ImGui::BeginCombo(
            STR_C("Occupancy"),
            Translations.lookup_c(dynamic.drivertype.c_str()))) {
        for (auto const &str : occupancy_names) {
            bool is_selected = (str == dynamic.drivertype);
            if (ImGui::Selectable(
                    Translations.lookup_c(str.c_str()), is_selected))
    ...
    if (ImGui::InputText(
            STR_C("Load type"), load_buf.data(), load_buf.size())) {
        dynamic.loadtype = load_buf.data();
        if (!load_buf[0]) dynamic.loadtype = "none";
    }
    ...
    if (dynamic.loadcount < 0) dynamic.loadcount = 0;

    if (ImGui::InputText(
            STR_C("Parameters"), param_buf.data(), param_buf.size()))
