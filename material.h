...
struct opengl_material {
    ...
    int shadow_rank{
        0};  // priority as shadow caster; higher = more likely to be skipped
...
class material_manager {
   public:
    material_manager() {
        m_materials.emplace_back(opengl_material());
    }  // empty bindings for null material

    material_handle create(std::string const &Filename, bool const Loadnow);
   ...
   private:
    ...
    // id, or npos.
    material_handle find_in_databank(std::string const &Materialname) const;
