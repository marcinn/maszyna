...
void cSun::init() { m_observer.timezone = -1.0 * simulation::Time.zone_bias(); }
...
*/
glm::vec3 cSun::getDirection() { return m_position; }

float cSun::getAngle() const { return (float)m_body.elevref; }

// return current hour angle
double cSun::getHourAngle() const { return m_body.hrang; }

float cSun::getIntensity() {
    irradiance();
    return (float)(m_body.etr / 1399.0);  // arbitrary scaling factor taken from
                                          // etrn value
}
...
void cSun::setPressure(float const Pressure) { m_observer.press = Pressure; }
...
void cSun::move() {
    ...
    // ecliptic longitude
    m_body.eclong =
        clamp_circular(m_body.tranom + m_body.phlong);  // lon = v + w
                                                        /*
                                                            ...
                                                            double const yequat = y * std::cos( degtorad * m_body.oblecl ) - 0.0 *
                                                           std::sin( degtorad * m_body.oblecl );                                                     double const zequat = y * std::sin(
                                                           degtorad * m_body.oblecl ) + 0.0 * std::cos( degtorad * m_body.oblecl );
                                                            // declination
                                                            m_body.declin = radtodeg * std::atan2( zequat, std::sqrt( xequat*xequat
                                                           + yequat*yequat ) );
                                                            // right ascension
    ...
    m_observer.gmst = clamp_circular(
        6.697375 + 0.0657098242 * daynumber + m_observer.utime, 24.0);

    // local mean sidereal time (deg)
    m_observer.lmst =
        clamp_circular(m_observer.gmst * 15.0 + m_observer.longitude);
...
void cSun::irradiance() {
    ...
    double cd = std::cos(
        glm::radians(m_body.dayang));  // cosine of the day angle or delination
