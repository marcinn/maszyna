...
inline bool operator==(lighting_data const &Left, lighting_data const &Right) {
    return (
        (Left.diffuse == Right.diffuse) && (Left.ambient == Right.ambient) &&
...
struct bounding_area {
    ...
    void serialize(std::ostream &Output) const;
    // restores content of the struct from provided input stream.
    void deserialize(std::istream &Input, bool const Preserveradius = true);
};
...
class shape_node {
    friend class basic_region;  // region might want to modify node content when
                                // it's being inserted
   ...
   public:
    ...
    struct shapenode_data {
        ...
        // geometry data
        glm::dvec3
            origin;  // world position of the relative coordinate system origin
        gfx::geometry_handle geometry{
            0, 0};  // relative origin-centered chunk of geometry held by gfx
                    // renderer
        std::vector<world_vertex>
            vertices;  // world space source data of the geometry
                       // methods:
    ...
    // adds content of provided node to already enclosed geometry. returns: true
    // if merge could be performed
    bool merge(shape_node &Shape);
    ...
    // set origin point
    void origin(glm::dvec3 Origin);
    // data access
    shapenode_data const &data() const;
    // get bounding radius
    // NOTE: use this method instead of direct access to the data member, due to
    // lazy radius evaluation
    float radius();
...
// set visibility
inline void shape_node::visible(bool State) { m_data.visible = State; }
// set origin point
inline void shape_node::origin(glm::dvec3 Origin) { m_data.origin = Origin; }
// data access
inline shape_node::shapenode_data const &shape_node::data() const {
    return m_data;
}
...
class lines_node {
   ...
   public:
    ...
    void origin(glm::dvec3 Origin);
    // data access
    linesnode_data const &data() const;
...
// set visibility
inline void lines_node::visible(bool State) { m_data.visible = State; }
// set origin point
inline void lines_node::origin(glm::dvec3 Origin) { m_data.origin = Origin; }
// data access
inline lines_node::linesnode_data const &lines_node::data() const {
    return m_data;
}
...
// types
    ...
    struct pathnode_data {
        ...
        // geometry data
        std::vector<world_vertex> vertices; // world space source data of the
geometry glm::dvec3 origin; // world position of the relative coordinate system
origin using geometryhandle_sequence = std::vector<geometry_handle>;
        geometryhandle_sequence geometry_1; // geometry chunks textured with
texture 1 geometryhandle_sequence geometry_2; // geometry chunks textured with
texture 2
    };
// methods
...
class basic_node {
   public:
    ...
    void export_as_text(std::string &Output) const;
    std::string const &name() const;
    void location(glm::dvec3 const Location);
   ...
   protected:
    ...
    scene::group_handle m_group{
        null_handle};  // group this node belongs to, if any
...
inline std::string const &basic_node::name() const { return m_name; }

inline void basic_node::location(glm::dvec3 const Location) {
    m_area.center = Location;
}

inline glm::dvec3 const &basic_node::location() const { return m_area.center; }

inline glm::dvec3 &basic_node::location() { return m_area.center; }

inline void basic_node::visible(bool const Visible) { m_visible = Visible; }

inline bool basic_node::visible() const { return m_visible; }

inline void basic_node::group(scene::group_handle Group) { m_group = Group; }

inline scene::group_handle basic_node::group() const { return m_group; }
