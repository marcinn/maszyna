...
*/
void control_mapper::clear() { *this = control_mapper(); }
...
bool TTrain::screen_entry::deserialize_mapping(cParser &Input) {
    ...
    if (key == "{") {
        ...
        // HACK: we expect this to be true only if the screen entry doesn't
        // start with a { which means legacy configuration format
        target = key;
        script = Input.getToken<std::string>();
        return false;
    }
...
void TCab::Load(cParser &Parser) {
    // NOTE: clearing control tables here is bit of a crutch, imposed by current
    // scheme of loading compartments anew on each cab change
    ggList.clear();
    btList.clear();
...
TGauge &TCab::Gauge(int n) {  // pobranie adresu obiektu aniomowanego ruchem
                              /*
                                  if (n < 0)
                                  { // rezerwacja wolnego
...
TButton &TCab::Button(
    int n) {  // pobranie adresu obiektu animowanego wyborem 1 z 2
              /*
                  if (n < 0)
                  { // rezerwacja wolnego
...
void TCab::Update(
    bool const Power) {  // odczyt parametrów i ustawienie animacji submodelom
...
// NOTE: we're currently using universal handlers and static handler map but it
// may be beneficial to have these implemented on individual class instance
// basis TBD, TODO: consider this approach if we ever want to have customized
// consist behaviour to received commands, based on the consist/vehicle type or
// whatever
TTrain::commandhandler_map const TTrain::m_commandhandlers = {
    ...
    {user_command::mastercontrollerincrease,
     &TTrain::OnCommand_mastercontrollerincrease},
    {user_command::mastercontrollerincreasefast,
     &TTrain::OnCommand_mastercontrollerincreasefast},
    {user_command::mastercontrollerdecrease,
     &TTrain::OnCommand_mastercontrollerdecrease},
    {user_command::mastercontrollerdecreasefast,
     &TTrain::OnCommand_mastercontrollerdecreasefast},
    {user_command::mastercontrollerset, &TTrain::OnCommand_mastercontrollerset},
    {user_command::secondcontrollerincrease,
     &TTrain::OnCommand_secondcontrollerincrease},
    {user_command::secondcontrollerincreasefast,
     &TTrain::OnCommand_secondcontrollerincreasefast},
    {user_command::secondcontrollerdecrease,
     &TTrain::OnCommand_secondcontrollerdecrease},
    {user_command::secondcontrollerdecreasefast,
     &TTrain::OnCommand_secondcontrollerdecreasefast},
    ...
    {user_command::mucurrentindicatorothersourceactivate,
     &TTrain::OnCommand_mucurrentindicatorothersourceactivate},
    {user_command::independentbrakeincrease,
     &TTrain::OnCommand_independentbrakeincrease},
    {user_command::independentbrakeincreasefast,
     &TTrain::OnCommand_independentbrakeincreasefast},
    {user_command::independentbrakedecrease,
     &TTrain::OnCommand_independentbrakedecrease},
    {user_command::independentbrakedecreasefast,
     &TTrain::OnCommand_independentbrakedecreasefast},
    {user_command::independentbrakeset, &TTrain::OnCommand_independentbrakeset},
    {user_command::independentbrakebailoff,
     &TTrain::OnCommand_independentbrakebailoff},
    {user_command::universalbrakebutton1,
     &TTrain::OnCommand_universalbrakebutton1},
    {user_command::universalbrakebutton2,
     &TTrain::OnCommand_universalbrakebutton2},
    {user_command::universalbrakebutton3,
     &TTrain::OnCommand_universalbrakebutton3},
    ...
    {user_command::trainbrakefirstservice,
     &TTrain::OnCommand_trainbrakefirstservice},
    {user_command::trainbrakeservice, &TTrain::OnCommand_trainbrakeservice},
    {user_command::trainbrakefullservice,
     &TTrain::OnCommand_trainbrakefullservice},
    ...
    {user_command::trainbrakebasepressureincrease,
     &TTrain::OnCommand_trainbrakebasepressureincrease},
    {user_command::trainbrakebasepressuredecrease,
     &TTrain::OnCommand_trainbrakebasepressuredecrease},
    {user_command::trainbrakebasepressurereset,
     &TTrain::OnCommand_trainbrakebasepressurereset},
    {user_command::trainbrakeoperationtoggle,
    ...
    {user_command::wheelspinbrakeactivate,
     &TTrain::OnCommand_wheelspinbrakeactivate},
    ...
    {user_command::autosandboxdeactivate,
     &TTrain::OnCommand_autosandboxdeactivate},
    {user_command::epbrakecontroltoggle,
     &TTrain::OnCommand_epbrakecontroltoggle},
    {user_command::trainbrakeoperationmodeincrease,
     &TTrain::OnCommand_trainbrakeoperationmodeincrease},
    ...
    {user_command::brakeactingspeeddecrease,
     &TTrain::OnCommand_brakeactingspeeddecrease},
    {user_command::brakeactingspeedsetcargo,
     &TTrain::OnCommand_brakeactingspeedsetcargo},
    {user_command::brakeactingspeedsetpassenger,
     &TTrain::OnCommand_brakeactingspeedsetpassenger},
    {user_command::brakeactingspeedsetrapid,
     &TTrain::OnCommand_brakeactingspeedsetrapid},
    {user_command::brakeloadcompensationincrease,
     &TTrain::OnCommand_brakeloadcompensationincrease},
    {user_command::brakeloadcompensationdecrease,
     &TTrain::OnCommand_brakeloadcompensationdecrease},
    {user_command::mubrakingindicatortoggle,
     &TTrain::OnCommand_mubrakingindicatortoggle},
    ...
    {user_command::cabsignalacknowledge,
     &TTrain::OnCommand_cabsignalacknowledge},
    ...
    {user_command::cabactivationdisable,
     &TTrain::OnCommand_cabactivationdisable},
    {user_command::pantographcompressorvalvetoggle,
     &TTrain::OnCommand_pantographcompressorvalvetoggle},
    {user_command::pantographcompressorvalveenable,
     &TTrain::OnCommand_pantographcompressorvalveenable},
    {user_command::pantographcompressorvalvedisable,
     &TTrain::OnCommand_pantographcompressorvalvedisable},
    {user_command::pantographcompressoractivate,
     &TTrain::OnCommand_pantographcompressoractivate},
    {user_command::pantographtogglefront,
     &TTrain::OnCommand_pantographtogglefront},
    {user_command::pantographtogglerear,
     &TTrain::OnCommand_pantographtogglerear},
    {user_command::pantographraisefront,
     &TTrain::OnCommand_pantographraisefront},
    {user_command::pantographraiserear, &TTrain::OnCommand_pantographraiserear},
    {user_command::pantographlowerfront,
     &TTrain::OnCommand_pantographlowerfront},
    ...
    {user_command::pantographselectnext,
     &TTrain::OnCommand_pantographselectnext},
    {user_command::pantographselectprevious,
     &TTrain::OnCommand_pantographselectprevious},
    {user_command::pantographtoggleselected,
     &TTrain::OnCommand_pantographtoggleselected},
    {user_command::pantographraiseselected,
     &TTrain::OnCommand_pantographraiseselected},
    {user_command::pantographlowerselected,
     &TTrain::OnCommand_pantographlowerselected},
    {user_command::pantographvalvesupdate,
     &TTrain::OnCommand_pantographvalvesupdate},
    ...
    {user_command::waterheaterbreakertoggle,
     &TTrain::OnCommand_waterheaterbreakertoggle},
    {user_command::waterheaterbreakerclose,
     &TTrain::OnCommand_waterheaterbreakerclose},
    {user_command::waterheaterbreakeropen,
     &TTrain::OnCommand_waterheaterbreakeropen},
    ...
    {user_command::waterpumpbreakertoggle,
     &TTrain::OnCommand_waterpumpbreakertoggle},
    {user_command::waterpumpbreakerclose,
     &TTrain::OnCommand_waterpumpbreakerclose},
    {user_command::waterpumpbreakeropen,
     &TTrain::OnCommand_waterpumpbreakeropen},
    ...
    {user_command::watercircuitslinktoggle,
     &TTrain::OnCommand_watercircuitslinktoggle},
    {user_command::watercircuitslinkenable,
     &TTrain::OnCommand_watercircuitslinkenable},
    {user_command::watercircuitslinkdisable,
     &TTrain::OnCommand_watercircuitslinkdisable},
    ...
    {user_command::convertertogglelocal,
     &TTrain::OnCommand_convertertogglelocal},
    {user_command::converteroverloadrelayreset,
     &TTrain::OnCommand_converteroverloadrelayreset},
    ...
    {user_command::compressortogglelocal,
     &TTrain::OnCommand_compressortogglelocal},
    {user_command::compressorpresetactivatenext,
     &TTrain::OnCommand_compressorpresetactivatenext},
    {user_command::compressorpresetactivateprevious,
     &TTrain::OnCommand_compressorpresetactivateprevious},
    {user_command::compressorpresetactivatedefault,
     &TTrain::OnCommand_compressorpresetactivatedefault},
    {user_command::motorblowerstogglefront,
     &TTrain::OnCommand_motorblowerstogglefront},
    {user_command::motorblowerstogglerear,
     &TTrain::OnCommand_motorblowerstogglerear},
    {user_command::motorblowersdisableall,
     &TTrain::OnCommand_motorblowersdisableall},
    ...
    {user_command::motorconnectorsclose,
     &TTrain::OnCommand_motorconnectorsclose},
    {user_command::motordisconnect, &TTrain::OnCommand_motordisconnect},
    {user_command::motoroverloadrelaythresholdtoggle,
     &TTrain::OnCommand_motoroverloadrelaythresholdtoggle},
    {user_command::motoroverloadrelaythresholdsetlow,
     &TTrain::OnCommand_motoroverloadrelaythresholdsetlow},
    {user_command::motoroverloadrelaythresholdsethigh,
     &TTrain::OnCommand_motoroverloadrelaythresholdsethigh},
    {user_command::motoroverloadrelayreset,
     &TTrain::OnCommand_motoroverloadrelayreset},
    {user_command::universalrelayreset1,
     &TTrain::OnCommand_universalrelayreset},
    ...
    {user_command::lightspresetactivatenext,
     &TTrain::OnCommand_lightspresetactivatenext},
    {user_command::lightspresetactivateprevious,
     &TTrain::OnCommand_lightspresetactivateprevious},
    ...
    {user_command::headlightdisableleft,
     &TTrain::OnCommand_headlightdisableleft},
    {user_command::headlighttoggleright,
     &TTrain::OnCommand_headlighttoggleright},
    {user_command::headlightenableright,
     &TTrain::OnCommand_headlightenableright},
    {user_command::headlightdisableright,
     &TTrain::OnCommand_headlightdisableright},
    {user_command::headlighttoggleupper,
     &TTrain::OnCommand_headlighttoggleupper},
    {user_command::headlightenableupper,
     &TTrain::OnCommand_headlightenableupper},
    {user_command::headlightdisableupper,
     &TTrain::OnCommand_headlightdisableupper},
    ...
    {user_command::redmarkerdisableleft,
     &TTrain::OnCommand_redmarkerdisableleft},
    {user_command::redmarkertoggleright,
     &TTrain::OnCommand_redmarkertoggleright},
    {user_command::redmarkerenableright,
     &TTrain::OnCommand_redmarkerenableright},
    {user_command::redmarkerdisableright,
     &TTrain::OnCommand_redmarkerdisableright},
    {user_command::headlighttogglerearleft,
     &TTrain::OnCommand_headlighttogglerearleft},
    {user_command::headlighttogglerearright,
     &TTrain::OnCommand_headlighttogglerearright},
    {user_command::headlighttogglerearupper,
     &TTrain::OnCommand_headlighttogglerearupper},
    {user_command::redmarkertogglerearleft,
     &TTrain::OnCommand_redmarkertogglerearleft},
    {user_command::redmarkertogglerearright,
     &TTrain::OnCommand_redmarkertogglerearright},
    ...
    {user_command::headlightsdimdisable,
     &TTrain::OnCommand_headlightsdimdisable},
    ...
    {user_command::interiorlightdisable,
     &TTrain::OnCommand_interiorlightdisable},
    {user_command::interiorlightdimtoggle,
     &TTrain::OnCommand_interiorlightdimtoggle},
    {user_command::interiorlightdimenable,
     &TTrain::OnCommand_interiorlightdimenable},
    {user_command::interiorlightdimdisable,
     &TTrain::OnCommand_interiorlightdimdisable},
    {user_command::compartmentlightstoggle,
     &TTrain::OnCommand_compartmentlightstoggle},
    {user_command::compartmentlightsenable,
     &TTrain::OnCommand_compartmentlightsenable},
    {user_command::compartmentlightsdisable,
     &TTrain::OnCommand_compartmentlightsdisable},
    {user_command::instrumentlighttoggle,
     &TTrain::OnCommand_instrumentlighttoggle},
    {user_command::instrumentlightenable,
     &TTrain::OnCommand_instrumentlightenable},
    {user_command::instrumentlightdisable,
     &TTrain::OnCommand_instrumentlightdisable},
    {user_command::dashboardlighttoggle,
     &TTrain::OnCommand_dashboardlighttoggle},
    {user_command::timetablelighttoggle,
     &TTrain::OnCommand_timetablelighttoggle},
    ...
    {user_command::doorpermitpresetactivatenext,
     &TTrain::OnCommand_doorpermitpresetactivatenext},
    {user_command::doorpermitpresetactivateprevious,
     &TTrain::OnCommand_doorpermitpresetactivateprevious},
    ...
    {user_command::nearestcarcouplingincrease,
     &TTrain::OnCommand_nearestcarcouplingincrease},
    {user_command::nearestcarcouplingdisconnect,
     &TTrain::OnCommand_nearestcarcouplingdisconnect},
    {user_command::nearestcarcoupleradapterattach,
     &TTrain::OnCommand_nearestcarcoupleradapterattach},
    {user_command::nearestcarcoupleradapterremove,
     &TTrain::OnCommand_nearestcarcoupleradapterremove},
    {user_command::occupiedcarcouplingdisconnect,
     &TTrain::OnCommand_occupiedcarcouplingdisconnect},
    ...
    {user_command::radiochannelincrease,
     &TTrain::OnCommand_radiochannelincrease},
    {user_command::radiochanneldecrease,
     &TTrain::OnCommand_radiochanneldecrease},
    ...
    {user_command::vehiclemovebackwards,
     &TTrain::OnCommand_vehiclemovebackwards},
    ...
    {user_command::springbrakeshutofftoggle,
     &TTrain::OnCommand_springbrakeshutofftoggle},
    {user_command::springbrakeshutoffenable,
     &TTrain::OnCommand_springbrakeshutoffenable},
    {user_command::springbrakeshutoffdisable,
     &TTrain::OnCommand_springbrakeshutoffdisable},
    {user_command::springbrakerelease, &TTrain::OnCommand_springbrakerelease},
    {user_command::distancecounteractivate,
     &TTrain::OnCommand_distancecounteractivate},
    {user_command::speedcontrolincrease,
     &TTrain::OnCommand_speedcontrolincrease},
    {user_command::speedcontroldecrease,
     &TTrain::OnCommand_speedcontroldecrease},
    {user_command::speedcontrolpowerincrease,
     &TTrain::OnCommand_speedcontrolpowerincrease},
    {user_command::speedcontrolpowerdecrease,
     &TTrain::OnCommand_speedcontrolpowerdecrease},
...
TTrain::TTrain() {
    simulation = new SimulationState(this);
    ...
    for (int i = 0; i < 20; ++i) {
        for (int j = 0; j < 6; ++j) fPress[i][j] = 0.0;
...
bool TTrain::Init(
    TDynamicObject *NewDynamicObject,
    bool e3d) {  // powiązanie ręcznego sterowania kabiną z pojazdem
    if (NewDynamicObject->Mechanik == nullptr) {
        /*
        ErrorLog( "Bad config: can't take control of inactive vehicle \"" +
        NewDynamicObject->asName + "\"" ); return false;
        */
        auto const activecab{
            (NewDynamicObject->MoverParameters->CabOccupied > 0   ? "1"
             : NewDynamicObject->MoverParameters->CabOccupied < 0 ? "2"
                                                                  : "p")};
        NewDynamicObject->create_controller(
            activecab, NewDynamicObject->ctOwner != nullptr);
    }

    DynamicSet(NewDynamicObject);
    if (!e3d)
        if (DynamicObject->Mechanik == NULL) return false;

    DynamicObject->MechInside = true;

    fMainRelayTimer =
        0;  // Hunter, do k...y nędzy, ustawiaj wartości początkowe zmiennych!
...

    {
        ...
        if (DynamicObject->Controller == Humandriver) {
            // McZapkie-030303: mozliwosc wyswietlania kabiny, w przyszlosci dac
            // opcje w mmd
...
dictionary_source *TTrain::GetTrainState(
    dictionary_source const &Extraparameters) {
    ...
    dict->insert(
        "main_init",
        (mvControlled->MainsInitTimeCountdown < mvControlled->MainsInitTime) &&
            (mvControlled->MainsInitTimeCountdown > 0.0));
    dict->insert(
        "main_ready", (false == mvControlled->Mains) && (fHVoltage > 0.0) &&
                          (mvControlled->MainsInitTimeCountdown <= 0.0));
    ...
    dict->insert(
        "lights_compartments", mvOccupied->CompartmentLights.is_active ||
    ...
    if (Dynamic()->Mechanik) {
        ...
        auto const *frontvehicle{
            controller->Vehicle(traindirection >= 0 ? end::front : end::rear)};
        auto const *rearvehicle{
            controller->Vehicle(traindirection >= 0 ? end::rear : end::front)};
        auto const frontvehicledirection{
            (frontvehicle->DirectionGet() ==
                     controller->Vehicle()->DirectionGet()
                 ? 1
                 : -1)};
        auto const rearvehicledirection{(
            rearvehicle->DirectionGet() == controller->Vehicle()->DirectionGet()
                ? 1
                : -1)};
        auto const fronttrainlights{
            frontvehicle->MoverParameters->iLights
                [frontvehicledirection * cabmodifier >= 0 ? end::front
                                                          : end::rear]};
        auto const reartrainlights{
            rearvehicle->MoverParameters->iLights
                [rearvehicledirection * cabmodifier >= 0 ? end::rear
                                                         : end::front]};
    ...
    bool const bEP =
        (mvControlled->LocHandle->GetCP() > 0.2) || (fEIMParams[0][5] > 0.01);
    ...
    for (auto idx = 0; idx < ggUniversals.size(); idx++) {
        if (idx != 3) {
            dict->insert(
                "universal" + std::to_string(idx),
                (ggUniversals[idx].GetValue() > 0.5));
        }
    }
    ...
    dict->insert(
        "door_permit_left",
        mvOccupied->Doors.instances[side::left].open_permit);
    dict->insert(
        "door_permit_right",
        mvOccupied->Doors.instances[side::right].open_permit);
    // movement data
    ...
    dict->insert(
        "traction_voltage", std::abs(mvPantographUnit->PantographVoltage));
    ...
    char const *TXTT[10] = {"fd",  "fdt",    "fdb", "pd", "pdt",
                            "pdb", "itothv", "1",   "2",  "3"};
    char const *TXTC[10] = {"fr",  "frt", "frb", "pr",  "prt",
                            "prb", "im",  "vm",  "ihv", "uhv"};
    char const *TXTD[10] = {"enrot",        "nrot",       "fill_des",
                            "fill_real",    "clutch_des", "clutch_real",
                            "water_temp",   "oil_press",  "engine_temp",
                            "retarder_fill"};
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        for (int j = 0; j < 10; ++j)
            dict->insert(
                ("eimp_c" + idx + "_" + std::string(TXTC[j])),
                fEIMParams[i + 1][j]);

        for (int j = 0; j < 10; ++j)
            dict->insert(
                ("diesel_param_" + idx + "_" + std::string(TXTD[j])),
                fDieselParams[i + 1][j]);
    ...
    for (int i = 0; i < bCompressors.size(); i++) {
        auto const idx{std::to_string(i + 1)};
        dict->insert(
            "compressors_" + idx + "_allow", std::get<0>(bCompressors[i]));
        dict->insert(
            "compressors_" + idx + "_work", std::get<1>(bCompressors[i]));
        dict->insert(
            "compressors_" + idx + "_car_no", std::get<2>(bCompressors[i]));
    }

    bool kier = (DynamicObject->DirectionGet() * mvOccupied->CabOccupied > 0);
    TDynamicObject *p = DynamicObject->GetFirstDynamic(
        mvOccupied->CabOccupied < 0 ? end::rear : end::front, 4);
    int in = 0;
    while (p && in < 8) {
        if (p->MoverParameters->eimc[eimc_p_Pmax] > 1) {
            in++;
            dict->insert(
                ("eimp_c" + std::to_string(in) + "_invno"),
                p->MoverParameters->InvertersNo);
            for (int j = 0; j < p->MoverParameters->InvertersNo; j++) {
                dict->insert(
                    ("eimp_c" + std::to_string(in) + "_inv" +
                     std::to_string(j + 1) + "_act"),
                    p->MoverParameters->Inverters[j].IsActive);
                dict->insert(
                    ("eimp_c" + std::to_string(in) + "_inv" +
                     std::to_string(j + 1) + "_error"),
                    p->MoverParameters->Inverters[j].Error);
                dict->insert(
                    ("eimp_c" + std::to_string(in) + "_inv" +
                     std::to_string(j + 1) + "_allow"),
                    p->MoverParameters->Inverters[j].Activate);
            }
        }
    ...
    for (int i = 0; i < 20; ++i) {
        for (int j = 0; j < 6; ++j) {
            dict->insert(
                ("eimp_pn" + std::to_string(i + 1) + "_" + TXTP[j]),
                fPress[i][j]);
        }
        for (int j = 0; j < 2; ++j) {
            dict->insert(
                ("brakes_" + std::to_string(i + 1) + "_" + TXTB[j]),
                bBrakes[i][j]);
        }
    }
    ...
    for (int i = 0; i < 20; i++) {
        ...
        dict->insert(
            ("code_" + caridx), (std::to_string(iUnits[i]) + cCode[i]));
        dict->insert(("car_name" + caridx), asCarName[i]);
        dict->insert(("slip_" + caridx), bSlip[i]);
    }
    // ai state data
    auto const *driver{
        (DynamicObject->ctOwner != nullptr ? DynamicObject->ctOwner
...
TTrain::state_t TTrain::get_state() const {
    return {
        ...
        fHVoltage,
        {fHCurrent[(mvControlled->TrainType & dt_EZT) ? 0 : 1], fHCurrent[2],
         fHCurrent[3]},
        ggLVoltage.GetValue(),
        ...
        mvOccupied->DirActive < 0,
        mvOccupied->Doors
            .instances[mvOccupied->CabOccupied < 0 ? side::right : side::left]
            .open_permit,
        mvOccupied->Doors
            .instances[mvOccupied->CabOccupied < 0 ? side::right : side::left]
            .is_open,
        mvOccupied->Doors
            .instances[mvOccupied->CabOccupied < 0 ? side::left : side::right]
            .open_permit,
        mvOccupied->Doors
            .instances[mvOccupied->CabOccupied < 0 ? side::left : side::right]
            .is_open,
...
// moves train brake lever to specified position, potentially emits switch sound
// if conditions are met
void TTrain::set_train_brake(double const Position) {
    auto const originalbrakeposition{
        static_cast<int>(100.0 * mvOccupied->fBrakeCtrlPos)};
    mvOccupied->BrakeLevelSet(Position);

    if (static_cast<int>(100.0 * mvOccupied->fBrakeCtrlPos) ==
        originalbrakeposition) {
        return;
    }

    if ((true == is_eztoer()) &&
        (false == (((originalbrakeposition / 100 == 0) ||
                    (originalbrakeposition / 100 >= 5)) &&
                   ((mvOccupied->BrakeCtrlPos == 0) ||
                    (mvOccupied->BrakeCtrlPos >= 5))))) {
...
void TTrain::zero_charging_train_brake() {
    if ((mvOccupied->BrakeCtrlPos == -1) &&
        (DynamicObject->Controller != AIdriver) && (Global.iFeedbackMode < 3) &&
...
void TTrain::set_train_brake_speed(TDynamicObject *Vehicle, int const Speed) {
    if (true == Vehicle->MoverParameters->BrakeDelaySwitch(Speed)) {
        // visual feedback
        // TODO: add setting indicator to vehicle class, for external
        // lever/indicator
        if (Vehicle == DynamicObject) {
            ...
            if (ggBrakeProfileG.SubModel != nullptr) {
                ggBrakeProfileG.UpdateValue(
                    (mvOccupied->BrakeDelayFlag == bdelay_G ? 1.0 : 0.0),
                    dsbSwitch);
            }
            if (ggBrakeProfileR.SubModel != nullptr) {
                ggBrakeProfileR.UpdateValue(
                    ((mvOccupied->BrakeDelayFlag & bdelay_R) != 0 ? 1.0 : 0.0),
                    dsbSwitch);
            }
        }
    }
...
void TTrain::set_paired_open_motor_connectors_button(bool const State) {
    if ((mvControlled->TrainType == dt_ET41) ||
        ...
        // crude implementation of the button affecting entire unit for
        // multi-unit engines
        // TODO: rework it into part of standard command propagation system
        if ((mvControlled->Couplers[end::front].Connected != nullptr) &&
            (true == TestFlag(
                         mvControlled->Couplers[end::front].CouplingFlag,
                         coupling::permanent))) {
            mvControlled->Couplers[end::front].Connected->StLinSwitchOff =
                State;
        }
        if ((mvControlled->Couplers[end::rear].Connected != nullptr) &&
            (true == TestFlag(
                         mvControlled->Couplers[end::rear].CouplingFlag,
                         coupling::permanent))) {
...
TDynamicObject *TTrain::find_nearest_consist_vehicle(
    ...
    if (!freefly) return DynamicObject;

    auto coupler{-2};  // scan for vehicle, not any specific coupler
    auto *vehicle{DynamicObject->ABuScanNearestObject(
        pos, DynamicObject->GetTrack(), 1, 1500, coupler)};
    if (vehicle == nullptr)
        vehicle = DynamicObject->ABuScanNearestObject(
            pos, DynamicObject->GetTrack(), -1, 1500, coupler);
...
// command handlers
void TTrain::OnCommand_aidriverenable(
    TTrain *Train, command_data const &Command) {
...
void TTrain::OnCommand_aidriverdisable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // on press
...
void TTrain::OnCommand_jointcontrollerset(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
        // on press or hold
        // value controls brake in range 0-0.5, master controller in range
        // 0.5-1.0
            ...
            Train->set_master_controller(
                ...
                (Train->mvControlled->CoupledCtrl
                     ? Train->mvControlled->MainCtrlPosNo +
                           Train->mvControlled->ScndCtrlPosNo
                     : Train->mvControlled->MainCtrlPosNo));
            Train->m_mastercontrollerinuse = true;
            Train->mvOccupied->LocalBrakePosA = 0;
        } else {
            Train->mvOccupied->LocalBrakePosA =
                (clamp(1.0 - (Command.param1 * 2), 0.0, 1.0));
            if (Train->mvControlled->MainCtrlPowerPos() > 0) {
                Train->set_master_controller(
                    Train->mvControlled->MainCtrlNoPowerPos());
            }
        }
    } else {
        // release
        Train->m_mastercontrollerinuse = false;
        Train->m_mastercontrollerreturndelay =
            EU07_CONTROLLER_BASERETURNDELAY;  // NOTE: keyboard return delay is
                                              // omitted for other input sources
    }
}

void TTrain::OnCommand_mastercontrollerincrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
        // on press or hold
        ...
        Train->m_mastercontrollerinuse = false;
        Train->m_mastercontrollerreturndelay =
            EU07_CONTROLLER_KEYBOARDETURNDELAY +
            EU07_CONTROLLER_BASERETURNDELAY;
    }
}

void TTrain::OnCommand_mastercontrollerincreasefast(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
        // on press or hold
        if ((Train->ggJointCtrl.SubModel != nullptr) &&
            ...
            Train->mvControlled->IncMainCtrl(
                Train->mvControlled->MainCtrlPosNo);
            Train->m_mastercontrollerinuse = true;
        }
    } else {
...
void TTrain::OnCommand_mastercontrollerdecrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
        // on press or hold
        if ((Train->ggJointCtrl.SubModel != nullptr) &&
...
void TTrain::OnCommand_mastercontrollerdecreasefast(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
        // on press or hold
        if ((Train->ggJointCtrl.SubModel != nullptr) &&
            ...
            Train->mvControlled->DecMainCtrl(
                Train->mvControlled->MainCtrlPowerPos());
            Train->m_mastercontrollerinuse = true;
        }
    } else {
        // release
...
void TTrain::OnCommand_mastercontrollerset(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
        // on press or hold
...
void TTrain::OnCommand_secondcontrollerincrease(
    TTrain *Train, command_data const &Command) {
    if ((Train->mvControlled->EngineType == TEngineType::DieselElectric) &&
        ...
        if (Command.action != GLFW_RELEASE) {
            Train->mvControlled->AnPos =
                clamp(Train->mvControlled->AnPos + 0.025, 0.0, 1.0);
        }
    } else {
        // regular mode
        ...
        if (Train->ggScndCtrl.is_push()) {
            auto const activeposition{
                Train->ggScndCtrl.is_toggle() ? 1.f : 1.f};
            auto const neutralposition{
                Train->ggScndCtrl.is_toggle() ? 0.5f : 0.f};
            Train->ggScndCtrl.UpdateValue(
                ((Command.action == GLFW_RELEASE) ? neutralposition
                                                  : activeposition),
                Train->dsbSwitch);
        }
...
void TTrain::OnCommand_secondcontrollerincreasefast(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
        // on press or hold
        if ((Train->mvControlled->EngineType == TEngineType::DieselElectric) &&
...
void TTrain::OnCommand_notchingrelaytoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_tempomattoggle(
    TTrain *Train, command_data const &Command) {
...
void TTrain::OnCommand_distancecounteractivate(
    TTrain *Train, command_data const &Command) {
...
void TTrain::OnCommand_mucurrentindicatorothersourceactivate(
    TTrain *Train, command_data const &Command) {
    if (Train->ggNextCurrentButton.SubModel == nullptr) {
        if (Command.action == GLFW_PRESS) {
            WriteLog(
                "Current Indicator Source switch is missing, or wasn't "
                "defined");
        }
        return;
    }
...
void TTrain::OnCommand_secondcontrollerdecrease(
    TTrain *Train, command_data const &Command) {
    if ((Train->mvControlled->EngineType == TEngineType::DieselElectric) &&
        (true == Train->mvControlled->ShuntMode)) {
        if (Command.action != GLFW_RELEASE) {
        ...
        if (Train->ggScndCtrl.is_push()) {
            auto const activeposition{
                Train->ggScndCtrl.is_toggle() ? 0.f : 1.f};
            auto const neutralposition{
                Train->ggScndCtrl.is_toggle() ? 0.5f : 0.f};
            Train->ggScndCtrl.UpdateValue(
                ((Command.action == GLFW_RELEASE) ? neutralposition
...
void TTrain::OnCommand_secondcontrollerdecreasefast(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
        // on press or hold
        if ((Train->mvControlled->EngineType == TEngineType::DieselElectric) &&
...
void TTrain::OnCommand_secondcontrollerset(
    TTrain *Train, command_data const &Command) {
    auto const targetposition{
        std::min<int>(Command.param1, Train->mvControlled->ScndCtrlPosNo)};
    // HACK: potentially animate push or pushtoggle control
    if (Train->ggScndCtrl.is_push()) {
        auto const activeposition{
            Train->ggScndCtrl.is_toggle()
                ? (targetposition < Train->mvControlled->ScndCtrlPos ? 0.f
                   : targetposition > Train->mvControlled->ScndCtrlPos
                       ? 1.f
                       : Train->ggScndCtrl.GetDesiredValue())
                :  // leave the control in its current position if it hits the
                   // limit
...
void TTrain::OnCommand_independentbrakeincrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
            ...
            if ((Train->ggJointCtrl.SubModel != nullptr) &&
                ...
                Train->mvOccupied->IncLocalBrakeLevel(
                    (Train->ggJointCtrl.SubModel == nullptr)
                        ? (Global.brake_speed * Command.time_delta *
                           LocalBrakePosNo)
                        : 1);
                if (Train->ggJointCtrl.SubModel != nullptr) {
                    Train->m_mastercontrollerinuse = true;
...
void TTrain::OnCommand_independentbrakeincreasefast(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
        if (Train->mvOccupied->LocalBrake != TLocalBrake::ManualBrake) {
            if ((Train->ggJointCtrl.SubModel != nullptr) &&
...
void TTrain::OnCommand_independentbrakedecrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
        if ((Train->mvOccupied->LocalBrake != TLocalBrake::ManualBrake)
            // Ra 1014-06: AI potrafi zahamować pomocniczym mimo jego braku -
            // odhamować jakoś trzeba
            // TODO: sort AI out so it doesn't do things it doesn't have
            // equipment for
            || (Train->mvOccupied->LocalBrakePosA > 0)) {
            if ((Train->ggJointCtrl.SubModel != nullptr) &&
                (Train->mvOccupied->LocalBrakePosA == 0.0)) {
...
void TTrain::OnCommand_independentbrakedecreasefast(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
        if ((Train->mvOccupied->LocalBrake != TLocalBrake::ManualBrake)
            // Ra 1014-06: AI potrafi zahamować pomocniczym mimo jego braku -
...
void TTrain::OnCommand_independentbrakeset(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
...
void TTrain::OnCommand_independentbrakebailoff(
    TTrain *Train, command_data const &Command) {
    if (false == Command.freefly) {
        // TODO: check if this set of conditions can be simplified.
        // it'd be more flexible to have an attribute indicating whether bail
        // off position is supported
        if ((Train->mvControlled->TrainType != dt_EZT) &&
            ((Train->mvControlled->EngineType ==
              TEngineType::ElectricSeriesMotor) ||
        ...
        auto *vehicle{Train->find_nearest_consist_vehicle(
            Command.freefly, Command.location)};
        if (vehicle != nullptr) {
            if (Command.action == GLFW_PRESS) {
                // press or hold
...
void TTrain::OnCommand_universalbrakebutton1(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // press or hold
        // visual feedback
...
void TTrain::OnCommand_universalbrakebutton2(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // press or hold
        // visual feedback
...
void TTrain::OnCommand_universalbrakebutton3(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // press or hold
        // visual feedback
...
void TTrain::OnCommand_trainbrakeincrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT &&
        Train->mvOccupied->BrakeHandle == TBrakeHandle::FV4a)
        Train->mvOccupied->BrakeLevelAdd(
            Global.brake_speed * Command.time_delta *
        ...
        Train->set_train_brake(
            Train->mvOccupied->fBrakeCtrlPos + Global.fBrakeStep);
}

void TTrain::OnCommand_trainbrakedecrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT &&
        Train->mvOccupied->BrakeHandle == TBrakeHandle::FV4a)
        Train->mvOccupied->BrakeLevelAdd(
...
void TTrain::OnCommand_trainbrakeset(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
        // press or hold
        Train->mvOccupied->BrakeLevelSet(interpolate(
            Train->mvOccupied->Handle->GetPos(bh_MIN),
            Train->mvOccupied->Handle->GetPos(bh_MAX),
            clamp(Command.param1, 0.0, 1.0)));
    } else {
        // release
        Train->zero_charging_train_brake();
    }
...
void TTrain::OnCommand_trainbrakecharging(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
        // press or hold
...
void TTrain::OnCommand_trainbrakerelease(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
...
void TTrain::OnCommand_trainbrakefirstservice(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        Train->set_train_brake(1);
    }
...
void TTrain::OnCommand_trainbrakeservice(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        Train->set_train_brake(
            (Train->mvOccupied->BrakeCtrlPosNo / 2 +
             (Train->mvOccupied->BrakeHandle == TBrakeHandle::FV4a ? 1 : 0)));
    }
}

void TTrain::OnCommand_trainbrakefullservice(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
...
void TTrain::OnCommand_trainbrakehandleoff(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
...
void TTrain::OnCommand_trainbrakeemergency(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
...
void TTrain::OnCommand_trainbrakebasepressureincrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
        switch (Train->mvOccupied->BrakeHandle) {
            case TBrakeHandle::FV4a: {
                Train->mvOccupied->BrakeCtrlPos2 =
                    clamp(Train->mvOccupied->BrakeCtrlPos2 - 0.01, -1.5, 2.0);
                break;
            }
            default: {
...
void TTrain::OnCommand_trainbrakebasepressuredecrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
        switch (Train->mvOccupied->BrakeHandle) {
            case TBrakeHandle::FV4a: {
...
void TTrain::OnCommand_trainbrakebasepressurereset(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
...
void TTrain::OnCommand_trainbrakeoperationtoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        auto *vehicle{Train->find_nearest_consist_vehicle(
            Command.freefly, Command.location)};
        ...
        vehicle->MoverParameters->Hamulec->SetBrakeStatus(
            vehicle->MoverParameters->Hamulec->GetBrakeStatus() ^ b_dmg);
    }
}

void TTrain::OnCommand_manualbrakeincrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
        auto *vehicle{Train->find_nearest_consist_vehicle(
            Command.freefly, Command.location)};
        ...
        if ((vehicle->MoverParameters->LocalBrake ==
             TLocalBrake::ManualBrake) ||
...
void TTrain::OnCommand_manualbrakedecrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_RELEASE) {
        auto *vehicle{Train->find_nearest_consist_vehicle(
            Command.freefly, Command.location)};
...
void TTrain::OnCommand_alarmchaintoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
...
void TTrain::OnCommand_wheelspinbrakeactivate(
    TTrain *Train, command_data const &Command) {
    // TODO: proper control deviced definition for the interiors, that doesn't
    // hinge of presence of 3d submodels
    ...
    if (Train->mvOccupied->BrakeSystem != TBrakeSystem::ElectroPneumatic) {
        ...
        if (Command.action == GLFW_PRESS) {
            ...
            // NOTE: system activation is (repeatedly) done in the train update
            // routine
        } else if (Command.action == GLFW_RELEASE) {
            // visual feedback
...
void TTrain::OnCommand_sandboxactivate(
    TTrain *Train, command_data const &Command) {
    // TODO: proper control deviced definition for the interiors, that doesn't
    // hinge of presence of 3d submodels
...
void TTrain::OnCommand_autosandboxtoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_autosandboxactivate(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_autosandboxdeactivate(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_epbrakecontroltoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
    ...
    auto const ispush{
        (static_cast<int>(Train->ggEPFuseButton.type()) &
         static_cast<int>(TGaugeType::push)) != 0};
    auto const istoggle{
        (static_cast<int>(Train->ggEPFuseButton.type()) &
         static_cast<int>(TGaugeType::toggle)) != 0};
...
void TTrain::OnCommand_trainbrakeoperationmodeincrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
        if (((Train->mvOccupied->BrakeOpModeFlag << 1) &
             Train->mvOccupied->BrakeOpModes) != 0) {
            // next mode
...
void TTrain::OnCommand_trainbrakeoperationmodedecrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_brakeactingspeedincrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        auto *vehicle{Train->find_nearest_consist_vehicle(
            Command.freefly, Command.location)};
...
void TTrain::OnCommand_brakeactingspeeddecrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        auto *vehicle{Train->find_nearest_consist_vehicle(
            Command.freefly, Command.location)};
...
void TTrain::OnCommand_brakeactingspeedsetcargo(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        auto *vehicle{Train->find_nearest_consist_vehicle(
            Command.freefly, Command.location)};
...
void TTrain::OnCommand_brakeactingspeedsetpassenger(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        auto *vehicle{Train->find_nearest_consist_vehicle(
            Command.freefly, Command.location)};
...
void TTrain::OnCommand_brakeactingspeedsetrapid(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        auto *vehicle{Train->find_nearest_consist_vehicle(
            Command.freefly, Command.location)};
...
void TTrain::OnCommand_brakeloadcompensationincrease(
    TTrain *Train, command_data const &Command) {
...
void TTrain::OnCommand_brakeloadcompensationdecrease(
    TTrain *Train, command_data const &Command) {
    if ((true == Command.freefly) && (Command.action == GLFW_PRESS)) {
        auto *vehicle{Train->find_nearest_consist_vehicle(
            Command.freefly, Command.location)};
...
void TTrain::OnCommand_mubrakingindicatortoggle(
    TTrain *Train, command_data const &Command) {
...
void TTrain::OnCommand_reverserincrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // HACK: master controller position isn't set in occupied vehicle in
        // E(D)MUs so we do a manual check in relevant vehicle here
...
void TTrain::OnCommand_reverserdecrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // HACK: master controller position isn't set in occupied vehicle in
        // E(D)MUs so we do a manual check in relevant vehicle here
...
void TTrain::OnCommand_reverserforwardhigh(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // HACK: master controller position isn't set in occupied vehicle in
        // E(D)MUs so we do a manual check in relevant vehicle here
        ...
        // HACK: try to move the reverser one position back, in case it's set to
        // "high forward"
...
void TTrain::OnCommand_reverserforward(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // HACK: master controller position isn't set in occupied vehicle in
        // E(D)MUs so we do a manual check in relevant vehicle here
...
void TTrain::OnCommand_reverserneutral(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // HACK: master controller position isn't set in occupied vehicle in
        // E(D)MUs so we do a manual check in relevant vehicle here
...
void TTrain::OnCommand_reverserbackward(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // HACK: master controller position isn't set in occupied vehicle in
        // E(D)MUs so we do a manual check in relevant vehicle here
...
void TTrain::OnCommand_alerteracknowledge(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // visual feedback
...
void TTrain::OnCommand_cabsignalacknowledge(
    TTrain *Train, command_data const &Command) {
...
void TTrain::OnCommand_batterytoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_REPEAT) {
...
void TTrain::OnCommand_batteryenable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // visual feedback
...
void TTrain::OnCommand_batterydisable(
    TTrain *Train, command_data const &Command) {
    // TBD, TODO: ewentualnie zablokować z FIZ, np. w samochodach się nie
    // odłącza akumulatora
    if (Command.action == GLFW_PRESS) {
        // visual feedback
...
void TTrain::OnCommand_cabactivationtoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_REPEAT) {
        // keep the switch from flipping back and forth if key is held down
...
void TTrain::OnCommand_cabactivationenable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // visual feedback
...
void TTrain::OnCommand_cabactivationdisable(
    TTrain *Train, command_data const &Command) {
    // TBD, TODO: ewentualnie zablokować z FIZ, np. w samochodach się nie
    // odłącza akumulatora
    if (Command.action == GLFW_PRESS) {
        ...
        if ((Train->mvOccupied->LightsPosNo > 0) &&
            (Train->mvOccupied->InactiveCabFlag & activation::redmarkers)) {
            Train->Dynamic()->SetLights();
        }
    } else if (Command.action == GLFW_RELEASE) {
        if (Train->ggCabActivationButton.type() == TGaugeType::push) {
...
void TTrain::OnCommand_pantographtogglefront(
    TTrain *Train, command_data const &Command) {
    // HACK: presence of pantograph selector prevents manual operation of the
    // individual valves
    ...
    if (Command.action == GLFW_PRESS) {
        ...
        auto const &pantograph{
            Train->mvPantographUnit->Pantographs[end::front]};
        auto const state{
        ...
        if (Train->mvOccupied->PantSwitchType == "impulse") {
            ...
            Train->mvOccupied->OperatePantographValve(
                end::front, operation_t::none,
                (ismanual ? range_t::local : range_t::consist));
        }
    }
}

void TTrain::OnCommand_pantographtogglerear(
    TTrain *Train, command_data const &Command) {
    // HACK: presence of pantograph selector prevents manual operation of the
    // individual valves
    if (Train->m_controlmapper.contains("pantselect_sw:")) {
    ...
    if (Command.action == GLFW_PRESS) {
        ...
        if (Train->mvOccupied->PantSwitchType == "impulse") {
            auto const ismanual{Train->iCabn == 0};
            Train->mvOccupied->OperatePantographValve(
                end::rear, operation_t::none,
                (ismanual ? range_t::local : range_t::consist));
        }
    }
}

void TTrain::OnCommand_pantographraisefront(
    TTrain *Train, command_data const &Command) {
    // HACK: presence of pantograph selector prevents manual operation of the
    // individual valves
    if (Train->m_controlmapper.contains("pantselect_sw:")) {
    ...
    if ((Train->iCabn != 0) &&
        (false == Train->m_controlmapper.contains("pantfront_sw:"))) {
        return;
    }

    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down HACK: don't propagate pantograph commands issued
        // from engine compartment, these are presumed to be manually moved
        // levers
        auto const ismanual{Train->iCabn == 0};
        Train->mvOccupied->OperatePantographValve(
            end::front,
        ...
        // NOTE: bit of a hax here, we're reusing button reset routine so we
        // don't need a copy in every branch
        OnCommand_pantographtogglefront(Train, Command);
    }
}

void TTrain::OnCommand_pantographraiserear(
    TTrain *Train, command_data const &Command) {
    // HACK: presence of pantograph selector prevents manual operation of the
    // individual valves
    if (Train->m_controlmapper.contains("pantselect_sw:")) {
    ...
    // prevent operation without submodel outside of engine compartment
    if ((Train->iCabn != 0) &&
        (false == Train->m_controlmapper.contains("pantrear_sw:"))) {
        return;
    }
...
void TTrain::OnCommand_pantographlowerfront(
    TTrain *Train, command_data const &Command) {
    // HACK: presence of pantograph selector prevents manual operation of the
    // individual valves
    if (Train->m_controlmapper.contains("pantselect_sw:")) {
...
void TTrain::OnCommand_pantographlowerrear(
    TTrain *Train, command_data const &Command) {
    // HACK: presence of pantograph selector prevents manual operation of the
    // individual valves
    if (Train->m_controlmapper.contains("pantselect_sw:")) {
...
void TTrain::OnCommand_pantographlowerall(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
    ...
    if (Train->ggPantAllDownButton.SubModel == nullptr) {
        // TODO: expand definition of cab controls so we can know if the control
        // is present without testing for presence of 3d switch
        if (Command.action == GLFW_PRESS) {
            WriteLog(
                "Lower All Pantographs switch is missing, or wasn't defined");
        }
        return;
    }
    ...
    if (Train->ggPantAllDownButton.type() == TGaugeType::toggle) {
        ...
        if (Command.action == GLFW_PRESS) {
            Train->mvPantographUnit->DropAllPantographs(
                false == Train->mvPantographUnit->PantAllDown);
            // visual feedback
            Train->ggPantAllDownButton.UpdateValue(
                (Train->mvPantographUnit->PantAllDown ? 1.0 : 0.0),
                Train->dsbSwitch);
        }
    } else {
        // impulse switch
        ...
        // visual feedback
        Train->ggPantAllDownButton.UpdateValue(
            (Command.action == GLFW_PRESS ? 1.0 : 0.0), Train->dsbSwitch);
    }
}

void TTrain::OnCommand_pantographselectnext(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_PRESS) {
        return;
    }
...
void TTrain::OnCommand_pantographselectprevious(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_PRESS) {
        return;
    }
...
void TTrain::OnCommand_pantographtoggleselected(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
    ...
    if (Command.action == GLFW_PRESS) {
        ...
        auto const state{
            ...
            Train->mvPantographUnit->PantsValve
                .is_active};  // fallback for impulse switches
        if (state) {
...
void TTrain::OnCommand_pantographraiseselected(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
...
void TTrain::OnCommand_pantographlowerselected(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
...
void TTrain::update_pantograph_valves() {
    ...
    // check desired states for both pantographs; value: whether the pantograph
    // should be raised
    auto const frontstate{preset & (swapends ? 2 : 1)};
    auto const rearstate{preset & (swapends ? 1 : 2)};
    mvOccupied->OperatePantographValve(
        end::front, (frontstate ? operation_t::enable : operation_t::disable));
    mvOccupied->OperatePantographValve(
        end::rear, (rearstate ? operation_t::enable : operation_t::disable));
}
...
void TTrain::change_pantograph_selection(int const Change) {
    ...
    selection =
        clamp<int>(selection + Change, 0, std::max<int>(presets.size() - 1, 0));
...
void TTrain::OnCommand_pantographvalvesupdate(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
    ...
    if (Command.action == GLFW_PRESS) {
        ...
        // visual feedback
        // NOTE: pantvalves_sw: is a specialized button, with no toggle behavior
        // support
        Train->ggPantValvesButton.UpdateValue(0.5, Train->dsbSwitch);
    }
}

void TTrain::OnCommand_pantographvalvesoff(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
    ...
    if (Command.action == GLFW_PRESS) {
        // implement action
        Train->mvOccupied->OperatePantographValve(
            end::front, operation_t::disable);
        Train->mvOccupied->OperatePantographValve(
            end::rear, operation_t::disable);
        // visual feedback
...
void TTrain::OnCommand_pantographcompressorvalvetoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only react to press
...
void TTrain::OnCommand_pantographcompressorvalvedisable(
    TTrain *Train, command_data const &Command) {
    auto const valveispresent{
        (Train->ggPantCompressorValve.SubModel != nullptr) ||
        ((Train->mvOccupied == Train->mvPantographUnit) &&
...
void TTrain::OnCommand_pantographcompressoractivate(
    TTrain *Train, command_data const &Command) {
    // tylko w maszynowym, unless actual device is present
...
void TTrain::OnCommand_linebreakertoggle(
    ...
    if (Command.action == GLFW_PRESS) {
        ...
        if ((Train->ggMainOnButton.SubModel != nullptr) ||
            ...
            if (Train->m_linebreakerstate == 0) {
                // ...after opening circuit, or holding for too short time to
                // close it
                OnCommand_linebreakeropen(Train, Command);
            } else {
        ...
        // HACK: ignition key ignores lack of submodel, so we can start vehicles
        // without any modeled controls
        Train->ggIgnitionKey.UpdateValue(0.0);
    }
}

void TTrain::OnCommand_linebreakeropen(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // visual feedback
        if (Train->ggMainOffButton.SubModel != nullptr) {
            ...
            // NOTE: legacy behaviour, for vehicles equipped only with impulse
            // close switch it doesn't make any real sense to animate this one,
            // but some people can't get over how there's no visual reaction to
            // their keypress
        ...
        // visual feedback
        // we don't exactly know which of the two buttons was used, so reset
        // both for setup with two separate swiches
        if (Train->ggMainOnButton.SubModel != nullptr) {
        ...
        if (Train->ggMainButton.SubModel != nullptr) {
            Train->ggMainButton.UpdateValue(
                (Train->ggMainButton.type() != TGaugeType::toggle ? 0.5 : 0.0),
                Train->dsbSwitch);
        }
    }
}

void TTrain::OnCommand_linebreakerclose(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // visual feedback
        if (Train->ggMainOnButton.SubModel != nullptr) {
            ...
            // HACK: ignition key ignores lack of submodel, so we can start
            // vehicles without any modeled controls
            Train->ggIgnitionKey.UpdateValue(1.0);
            return;
        }
        // the actual closing of the line breaker is handled in the train update
        // routine
    } else if (Command.action == GLFW_RELEASE) {
        // visual feedback
        if (Train->ggMainOnButton.SubModel != nullptr) {
        ...
        if (Train->m_linebreakerstate == 2) {
            // we don't need to start the diesel twice, but the other types
            // (with impulse switch setup) still need to be launched NOTE: this
            // behaviour should depend on MainOnButton presence and type_delayed
            // TODO: change it when/if vehicle definition files get their proper
            // switch types
            if (Train->mvControlled->EngineType ==
                TEngineType::ElectricSeriesMotor) {
                // try to finalize state change of the line breaker, set the
                // state based on the outcome
                Train->m_linebreakerstate =
                    (Train->mvControlled->MainSwitch(true) ? 1 : 0);
            }
        }
...
void TTrain::OnCommand_fuelpumptoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
...
void TTrain::OnCommand_fuelpumpenable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
...
void TTrain::OnCommand_fuelpumpdisable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
...
void TTrain::OnCommand_oilpumptoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
...
void TTrain::OnCommand_oilpumpdisable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
...
void TTrain::OnCommand_waterheaterbreakertoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_waterheaterbreakerclose(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // visual feedback
...
void TTrain::OnCommand_waterheaterbreakeropen(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // visual feedback
...
void TTrain::OnCommand_waterheatertoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_waterheaterenable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // visual feedback
...
void TTrain::OnCommand_waterheaterdisable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // visual feedback
...
void TTrain::OnCommand_waterpumpbreakertoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_waterpumpbreakeropen(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // visual feedback
...
void TTrain::OnCommand_waterpumptoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
...
void TTrain::OnCommand_waterpumpdisable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
...
void TTrain::OnCommand_watercircuitslinktoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_watercircuitslinkenable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // visual feedback
...
void TTrain::OnCommand_watercircuitslinkdisable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // visual feedback
...
void TTrain::OnCommand_convertertoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_converterenable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // visual feedback
        ...
        // NOTE: this is most likely setup wrong, but the whole thing is smoke
        // and mirrors anyway
        if ((Train->mvOccupied->ConvSwitchType != "impulse") ||
...
void TTrain::OnCommand_converterdisable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // visual feedback
        Train->ggConverterButton.UpdateValue(0.0, Train->dsbSwitch);
...
void TTrain::OnCommand_convertertogglelocal(
    TTrain *Train, command_data const &Command) {
    ...
    if (Command.action == GLFW_PRESS) {
        ...
        if ((false == Train->mvOccupied->ConverterAllowLocal) &&
            ...
            /*
               ...
               range::local ) ) {
                            // side effects
                            // control the compressor, if it's paired with the
               converter if( Train->mvControlled->CompressorPower == 2 ) {
            ...
            */
            ...
            /*
               ...
               range::local );
                            ...
                            // if there's no (low voltage) power source left,
               drop pantographs if( false == Train->mvControlled->Battery ) {
                                Train->mvControlled->PantFront( false,
               range::local ); Train->mvControlled->PantRear( false,
               range::local );
                            }
                        }
            */
...
void TTrain::OnCommand_converteroverloadrelayreset(
    TTrain *Train, command_data const &Command) {
    if (Train->ggConverterFuseButton.SubModel == nullptr) {
        if (Command.action == GLFW_PRESS) {
            WriteLog(
                "Converter Overload Relay Reset button is missing, or wasn't "
                "defined");
        }
        //        return;
    }
...
void TTrain::OnCommand_compressortoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
    ...
    /*
        // disabled because we don't have yet support for compressor switch type
       definition else if( Command.action == GLFW_RELEASE ) {
            // on button release...
...
void TTrain::OnCommand_compressorenable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // visual feedback
...
void TTrain::OnCommand_compressordisable(
    TTrain *Train, command_data const &Command) {
    ...
    if (Command.action == GLFW_PRESS) {
        ...
        /*
                if( Train->ggCompressorOffButton.SubModel != nullptr ) {
                    Train->ggCompressorOffButton.UpdateValue( 1.0,
           Train->dsbSwitch );
                }
        */
...
void TTrain::OnCommand_compressortogglelocal(
    TTrain *Train, command_data const &Command) {
...
void TTrain::OnCommand_compressorpresetactivatenext(
    TTrain *Train, command_data const &Command) {
    ...
    if (Train->ggCompressorListButton.type() == TGaugeType::push) {
        // impulse switch
        if (Train->mvOccupied->CompressorListPosNo <
        ...
        // visual feedback
        Train->ggCompressorListButton.UpdateValue(
            Train->mvOccupied->CompressorListPos - 1, Train->dsbSwitch);
    } else {
        // multi-state switch
        if (Command.action == GLFW_RELEASE) {
            return;
        ...
        if ((Train->mvOccupied->CompressorListPos <
             Train->mvOccupied->CompressorListPosNo) ||
...
void TTrain::OnCommand_compressorpresetactivateprevious(
    TTrain *Train, command_data const &Command) {
    if (Train->mvOccupied->CompressorListPosNo == 0) {
        return;
    }
    ...
    if (Train->ggCompressorListButton.type() == TGaugeType::push) {
        // impulse switch toggles only between positions 'default' and
        // 'default+1'
        return;
    }
...
void TTrain::OnCommand_compressorpresetactivatedefault(
    TTrain *Train, command_data const &Command) {
    if (Train->mvOccupied->CompressorListPosNo == 0) {
        return;
    }
...
void TTrain::OnCommand_motorblowerstogglefront(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
...
void TTrain::OnCommand_motorblowersenablefront(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
...
void TTrain::OnCommand_motorblowersdisablefront(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
...
void TTrain::OnCommand_motorblowerstogglerear(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
...
void TTrain::OnCommand_motorblowersdisablerear(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
...
void TTrain::OnCommand_motorblowersdisableall(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
    ...
    if (Train->ggMotorBlowersAllOffButton.type() == TGaugeType::push) {
        ...
        if (Command.action == GLFW_PRESS) {
            // visual feedback
            Train->ggMotorBlowersAllOffButton.UpdateValue(
                1.f, Train->dsbSwitch);
            Train->mvControlled->MotorBlowersSwitchOff(true, end::front);
            Train->mvControlled->MotorBlowersSwitchOff(true, end::rear);
        } else if (Command.action == GLFW_RELEASE) {
        ...
        if (Command.action == GLFW_PRESS) {
            if (Train->ggMotorBlowersAllOffButton.GetDesiredValue() < 0.5f) {
                ...
                Train->mvControlled->MotorBlowersSwitchOff(true, end::rear);
                // visual feedback
                Train->ggMotorBlowersRearButton.UpdateValue(
                    1.f, Train->dsbSwitch);
            } else {
                // deactivate
                Train->mvControlled->MotorBlowersSwitchOff(false, end::front);
...
void TTrain::OnCommand_coolingfanstoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_PRESS) {
        return;
    }
...
void TTrain::OnCommand_motorconnectorsopen(
    TTrain *Train, command_data const &Command) {
    // TODO: don't rely on presense of 3d model to determine presence of the
    // switch
    if (Train->ggStLinOffButton.SubModel == nullptr) {
        if (Command.action == GLFW_PRESS) {
            WriteLog(
                "Open Motor Power Connectors button is missing, or wasn't "
                "defined");
        }
        return;
    }
    // HACK: because we don't have modeled actual circuits this is a
    // simplification of the real mechanics namely, pressing the button will
    // flip it in the entire unit, which isn't exactly physically possible
    if (Command.action == GLFW_PRESS) {
...
void TTrain::OnCommand_motorconnectorsclose(
    TTrain *Train, command_data const &Command) {
    // TODO: don't rely on presense of 3d model to determine presence of the
    // switch
    if (Train->ggStLinOffButton.SubModel == nullptr) {
    ...
    if (Command.action == GLFW_PRESS) {
        if (Train->mvControlled->StLinSwitchType == "toggle") {
            // default type of button (impulse) has only one effect on press,
            // but the toggle type can toggle the state visual feedback
            Train->ggStLinOffButton.UpdateValue(0.0, Train->dsbSwitch);
        }
...
void TTrain::OnCommand_motordisconnect(
    TTrain *Train, command_data const &Command) {
    if ((Train->mvControlled->TrainType == dt_EZT
...
void TTrain::OnCommand_motoroverloadrelaythresholdtoggle(
    ...
    if (Command.action == GLFW_PRESS) {
        ...
        if ((true == Train->mvControlled->ShuntModeAllow
                 ...
                 : (false ==
                    Train->mvControlled->MotorOverloadRelayHighThreshold))) {
            // turn on
...
void TTrain::OnCommand_motoroverloadrelaythresholdsetlow(
    ...
    if (Command.action == GLFW_PRESS) {
        ...
        // visual feedback
        Train->ggMaxCurrentCtrl.UpdateValue(
            Train->mvControlled->MotorOverloadRelayHighThreshold ? 1 : 0,
            Train->dsbSwitch);
    }
}
...
void TTrain::OnCommand_motoroverloadrelayreset(
    TTrain *Train, command_data const &Command) {
    if (Train->ggFuseButton.SubModel == nullptr) {
        if (Command.action == GLFW_PRESS) {
            WriteLog(
                "Motor Overload Relay Reset button is missing, or wasn't "
                "defined");
        }
        //        return;
    }
...
void TTrain::OnCommand_universalrelayreset(
    TTrain *Train, command_data const &Command) {
    auto const itemindex = static_cast<int>(Command.command) -
                           static_cast<int>(user_command::universalrelayreset1);
...
void TTrain::OnCommand_lightspresetactivatenext(
    TTrain *Train, command_data const &Command) {
    if (Train->mvOccupied->LightsPosNo == 0) {
        // no preset selector
        return;
    ...
    if ((Train->mvOccupied->LightsPos < Train->mvOccupied->LightsPosNo) ||
        ...
        // active light preset is stored as value in range 1-LigthPosNo
        auto const restartcycle{
            Train->mvOccupied->LightsPos == Train->mvOccupied->LightsPosNo};
        Train->mvOccupied->LightsPos =
            (false == restartcycle ? Train->mvOccupied->LightsPos + 1
                                   : 1);  // wrap mode

        Train->Dynamic()->SetLights();
        // visual feedback
        if (Train->ggLightsButton.SubModel != nullptr) {
            // HACK: skip submodel animation when restarting cycle, since it
            // plays in the 'wrong' direction
            if (false == restartcycle) {
                Train->ggLightsButton.UpdateValue(
                    Train->mvOccupied->LightsPos - 1, Train->dsbSwitch);
            } else {
                Train->ggLightsButton.PutValue(
...
void TTrain::OnCommand_lightspresetactivateprevious(
    TTrain *Train, command_data const &Command) {
    if (Train->mvOccupied->LightsPosNo == 0) {
        // no preset selector
        return;
...
void TTrain::OnCommand_headlighttoggleleft(
    TTrain *Train, command_data const &Command) {
    ...
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
        if ((Train->mvOccupied->iLights[vehicleend] & light::headlight_left) ==
            0) {
            // turn on
...
void TTrain::OnCommand_headlightdisableleft(
    TTrain *Train, command_data const &Command) {
    if (Train->mvOccupied->LightsPosNo > 0) {
        // lights are controlled by preset selector
        return;
...
void TTrain::OnCommand_headlighttoggleright(
    TTrain *Train, command_data const &Command) {
    auto const vehicleend{Train->cab_to_end()};
    ...
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
        if ((Train->mvOccupied->iLights[vehicleend] & light::headlight_right) ==
            0) {
            // turn on
...
void TTrain::OnCommand_headlightenableright(
    TTrain *Train, command_data const &Command) {
    if (Train->mvOccupied->LightsPosNo > 0) {
        // lights are controlled by preset selector
        return;
...
void TTrain::OnCommand_headlighttoggleupper(
    TTrain *Train, command_data const &Command) {
    auto const vehicleend{Train->cab_to_end()};
    ...
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
        if ((Train->mvOccupied->iLights[vehicleend] & light::headlight_upper) ==
            0) {
            // turn on
...
void TTrain::OnCommand_headlightenableupper(
    TTrain *Train, command_data const &Command) {
    if (Train->mvOccupied->LightsPosNo > 0) {
        // lights are controlled by preset selector
        return;
...
void TTrain::OnCommand_headlightdisableupper(
    TTrain *Train, command_data const &Command) {
    if (Train->mvOccupied->LightsPosNo > 0) {
        // lights are controlled by preset selector
        return;
...
void TTrain::OnCommand_redmarkertoggleleft(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
        ...
        if ((Train->mvOccupied->iLights[vehicleend] & light::redmarker_left) ==
            0) {
            // turn on
...
void TTrain::OnCommand_redmarkerenableleft(
    TTrain *Train, command_data const &Command) {
    if (Train->mvOccupied->LightsPosNo > 0) {
        // lights are controlled by preset selector
        return;
    ...
    if (Command.action == GLFW_PRESS) {
        ...
        if (Train->ggLeftEndLightButton.SubModel != nullptr) {
            ...
            // we interpret lack of dedicated switch as a sign the light is
            // controlled with 3-way switch this is crude, but for now will do
            Train->ggLeftLightButton.UpdateValue(-1.0, Train->dsbSwitch);
...
void TTrain::OnCommand_redmarkerdisableleft(
    TTrain *Train, command_data const &Command) {
    if (Train->mvOccupied->LightsPosNo > 0) {
        // lights are controlled by preset selector
        return;
...
void TTrain::OnCommand_redmarkertoggleright(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_redmarkerenableright(
    TTrain *Train, command_data const &Command) {
    if (Train->mvOccupied->LightsPosNo > 0) {
        // lights are controlled by preset selector
        return;
...
void TTrain::OnCommand_redmarkerdisableright(
    TTrain *Train, command_data const &Command) {
    if (Train->mvOccupied->LightsPosNo > 0) {
        // lights are controlled by preset selector
        return;
...
void TTrain::OnCommand_headlighttogglerearleft(
    TTrain *Train, command_data const &Command) {
    if (Train->mvOccupied->LightsPosNo > 0) {
        // lights are controlled by preset selector
        return;
    ...
    if (Command.action == GLFW_PRESS) {
        // NOTE: we toggle the light on opposite side, as 'rear right' is 'front
        // left' on the rear end etc
        auto const vehicleotherend{
            (Train->cab_to_end() == end::front ? end::rear : end::front)};
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
        if ((Train->mvOccupied->iLights[vehicleotherend] &
             light::headlight_right) == 0) {
            // turn on
            Train->mvOccupied->iLights[vehicleotherend] ^=
...
void TTrain::OnCommand_headlighttogglerearright(
    TTrain *Train, command_data const &Command) {
    if (Train->mvOccupied->LightsPosNo > 0) {
        // lights are controlled by preset selector
        return;
...
void TTrain::OnCommand_redmarkertogglerearleft(
    TTrain *Train, command_data const &Command) {
    if (Train->mvOccupied->LightsPosNo > 0) {
        // lights are controlled by preset selector
        return;
...
void TTrain::OnCommand_redmarkertogglerearright(
    TTrain *Train, command_data const &Command) {
    if (Train->mvOccupied->LightsPosNo > 0) {
        // lights are controlled by preset selector
        return;
...
void TTrain::OnCommand_redmarkerstoggle(
    TTrain *Train, command_data const &Command) {
    if ((true == Command.freefly) && (Command.action == GLFW_PRESS)) {
        auto *vehicle{
            std::get<TDynamicObject *>(simulation::Region->find_vehicle(
                Command.location, 10, false, true))};

        if (vehicle == nullptr) {
            return;
        }

        int const CouplNr{clamp(
            vehicle->DirectionGet() *
                (Math3D::LengthSquared3(
                     vehicle->HeadPosition() - Command.location) >
                         Math3D::LengthSquared3(
                             vehicle->RearPosition() - Command.location)
                     ? 1
                     : -1),
        ...
        vehicle->MoverParameters->iLights[CouplNr] =
            (false == TestFlag(
                          vehicle->MoverParameters->iLights[CouplNr], lightset)
                 ? vehicle->MoverParameters->iLights[CouplNr] |= lightset
                 :  // turn signals on
                 vehicle->MoverParameters->iLights[CouplNr] ^=
                 lightset);  // turn signals off
    }
}

void TTrain::OnCommand_endsignalstoggle(
    TTrain *Train, command_data const &Command) {
    if ((true == Command.freefly) && (Command.action == GLFW_PRESS)) {
        auto *vehicle{
            std::get<TDynamicObject *>(simulation::Region->find_vehicle(
...
void TTrain::OnCommand_headlightsdimtoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_headlightsdimenable(
    ...
    if (Command.action == GLFW_PRESS) {
        ...
        if (Train->ggDimHeadlightsButton.SubModel == nullptr) {
            // TODO: proper control deviced definition for the interiors, that
            // doesn't hinge of presence of 3d submodels
...
void TTrain::OnCommand_headlightsdimdisable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_interiorlighttoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_interiorlightenable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_interiorlightdisable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_interiorlightdimtoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_interiorlightdimenable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_interiorlightdimdisable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_compartmentlightstoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
...
void TTrain::OnCommand_compartmentlightsenable(
    ...
    if (Command.action == GLFW_PRESS) {
        ...
        // visual feedback
        if (Train->m_controlmapper.contains("compartmentlights_sw:")) {
            Train->ggCompartmentLightsButton.UpdateValue(
                1.0f, Train->dsbSwitch);
        }
        if (Train->m_controlmapper.contains("compartmentlightson_sw:")) {
            Train->ggCompartmentLightsOnButton.UpdateValue(
                1.0f, Train->dsbSwitch);
        }
    } else if (Command.action == GLFW_RELEASE) {
        if (Train->m_controlmapper.contains("compartmentlights_sw:")) {
        ...
        if (Train->m_controlmapper.contains("compartmentlightson_sw:")) {
            Train->mvOccupied->CompartmentLightsSwitch(false);
            Train->ggCompartmentLightsOnButton.UpdateValue(
                0.0f, Train->dsbSwitch);
        }
    }
}

void TTrain::OnCommand_compartmentlightsdisable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        ...
        if (Train->m_controlmapper.contains("compartmentlightsoff_sw:")) {
            Train->ggCompartmentLightsOffButton.UpdateValue(
                1.0f, Train->dsbSwitch);
        }
    } else if (Command.action == GLFW_RELEASE) {
        if (Train->m_controlmapper.contains("compartmentlights_sw:")) {
...
void TTrain::OnCommand_instrumentlightdisable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_dashboardlighttoggle(
    TTrain *Train, command_data const &Command) {
    // only reacting to press, so the switch doesn't flip back and forth if key
    // is held down
    if (Command.action != GLFW_PRESS) {
        return;
    }
...
void TTrain::OnCommand_timetablelighttoggle(
    TTrain *Train, command_data const &Command) {
    // only reacting to press, so the switch doesn't flip back and forth if key
    // is held down
    if (Command.action != GLFW_PRESS) {
...
void TTrain::OnCommand_heatingtoggle(
    TTrain *Train, command_data const &Command) {
    ...
    if (Command.action == GLFW_PRESS) {
        // ignore repeats so the switch doesn't flip back and forth if key is
        // held down
...
void TTrain::OnCommand_heatingenable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
...
void TTrain::OnCommand_heatingdisable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        Train->mvOccupied->HeatingSwitch(false);
        // visual feedback
        Train->ggTrainHeatingButton.UpdateValue(
            (Train->ggTrainHeatingButton.type() == TGaugeType::push ? 1.0
                                                                    : 0.0),
            Train->dsbSwitch);
    }
}

void TTrain::OnCommand_generictoggle(
    TTrain *Train, command_data const &Command) {
    auto const itemindex = static_cast<int>(Command.command) -
                           static_cast<int>(user_command::generictoggle0);
    auto &item = Train->ggUniversals[itemindex];
    /*
        if( item.SubModel == nullptr ) {
            if( Command.action == GLFW_PRESS ) {
                WriteLog( "Train generic item " + std::to_string( itemindex ) +
       " is missing, or wasn't defined" );
            }
            return;
        }
    */
...
void TTrain::OnCommand_springbraketoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_springbrakeenable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_springbrakedisable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_springbrakeshutofftoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_springbrakeshutoffenable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_springbrakeshutoffdisable(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_springbrakerelease(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_speedcontrolincrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_speedcontroldecrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
...
void TTrain::OnCommand_speedcontrolpowerincrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
        ...
        // visual feedback
        Train->ggSpeedControlPowerIncreaseButton.UpdateValue(
            1.0, Train->dsbSwitch);
    } else if (Command.action == GLFW_RELEASE) {
        // release
        // visual feedback
...
void TTrain::OnCommand_speedcontrolpowerdecrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // only reacting to press, so the switch doesn't flip back and forth if
        // key is held down
        ...
        // visual feedback
        Train->ggSpeedControlPowerDecreaseButton.UpdateValue(
            1.0, Train->dsbSwitch);
    } else if (Command.action == GLFW_RELEASE) {
        // release
        // visual feedback
...
void TTrain::OnCommand_speedcontrolbutton(
    TTrain *Train, command_data const &Command) {
    auto const itemindex = static_cast<int>(Command.command) -
                           static_cast<int>(user_command::speedcontrolbutton0);
...
void TTrain::OnCommand_inverterenable(
    TTrain *Train, command_data const &Command) {
    auto itemindex = static_cast<int>(Command.command) -
                     static_cast<int>(user_command::inverterenable1);
    ...
    if (Command.action == GLFW_PRESS) {
        ...
        // key is held down
        bool kier =
            (Train->DynamicObject->DirectionGet() *
                 Train->mvOccupied->CabOccupied >
             0);
        int flag =
            Train->DynamicObject->MoverParameters->InverterControlCouplerFlag;
        TDynamicObject *p = Train->DynamicObject->GetFirstDynamic(
            Train->mvOccupied->CabOccupied < 0 ? end::rear : end::front, flag);
        while (p) {
            if (p->MoverParameters->eimc[eimc_p_Pmax] > 1) {
...
void TTrain::OnCommand_inverterdisable(
    TTrain *Train, command_data const &Command) {
    auto itemindex = static_cast<int>(Command.command) -
                     static_cast<int>(user_command::inverterdisable1);
...
void TTrain::OnCommand_invertertoggle(
    TTrain *Train, command_data const &Command) {
    auto itemindex = static_cast<int>(Command.command) -
                     static_cast<int>(user_command::invertertoggle1);
    ...
    if (Command.action == GLFW_PRESS) {
        ...
        while (p) {
            if (p->MoverParameters->eimc[eimc_p_Pmax] > 1) {
                if (itemindex < p->MoverParameters->InvertersNo) {
                    p->MoverParameters->Inverters[itemindex].Activate =
                        !p->MoverParameters->Inverters[itemindex].Activate;
                    // visual feedback
                    item.UpdateValue(
                        p->MoverParameters->Inverters[itemindex].Activate ? 1.0
                                                                          : 0.0,
                        Train->dsbSwitch);
                    break;
                } else {
...
void TTrain::OnCommand_doorlocktoggle(
    TTrain *Train, command_data const &Command) {
...
void TTrain::OnCommand_doortoggleleft(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // NOTE: test how the door state check works with consists where the
        // occupied vehicle doesn't have opening doors
        if (false == ((Train->ggDoorLeftButton.GetDesiredValue() > 0.5) ||
            ...
            if ((Train->ggDoorAllOffButton.SubModel != nullptr) &&
                ...
                // if two-button setup lacks dedicated closing button require
                // the user to press appropriate button manually
                return;
            } else {
        ...
        if (true == ((Train->ggDoorLeftButton.GetDesiredValue() > 0.5) ||
                     (Train->ggDoorLeftOnButton.GetDesiredValue() > 0.5))) {
            // open
            if ((Train->mvOccupied->Doors.has_autowarning) &&
                ...
                if ((Train->ggDoorAllOffButton.SubModel != nullptr) &&
                    (Train->ggDoorLeftOffButton.SubModel == nullptr)) {
                    // OnCommand_doorcloseall( Train, Command );
                    // if two-button setup lacks dedicated closing button
                    // require the user to press appropriate button manually
                    return;
                } else {
                    OnCommand_doorcloseleft(Train, Command);
                }
        ...
        // visual feedback
        // dedicated closing buttons are presumed to be impulse switches and
        // return automatically to neutral position NOTE: temporary arrangement,
        // can be removed when LD system is in place
...
void TTrain::OnCommand_doorpermitleft(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
    ...
    auto const side{
        (Train->cab_to_end() == end::front ? side::left : side::right)};
    ...
    if (Command.action == GLFW_PRESS) {
        if (Train->ggDoorLeftPermitButton.is_push()) {
            ...
            Train->m_doorpermittimers[side] =
                Train->mvOccupied->DoorsOpenWithPermitAfter;
        } else {
            // two-state switch
            auto const newstate{
                !(Train->ggDoorLeftPermitButton.GetDesiredValue() > 0.5)};

            Train->mvOccupied->PermitDoors(side, newstate);
            // visual feedback
            Train->ggDoorLeftPermitButton.UpdateValue(
                (newstate ? 1.0 : 0.0), Train->dsbSwitch);
        }
    } else if (Command.action == GLFW_RELEASE) {
        if (Train->ggDoorLeftPermitButton.is_push()) {
            // impulse switch
...
void TTrain::OnCommand_doorpermitright(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_REPEAT) {
        return;
    }
    ...
    auto const side{
        (Train->cab_to_end() == end::front ? side::right : side::left)};
    ...
    if (Command.action == GLFW_PRESS) {
        if (Train->ggDoorRightPermitButton.type() == TGaugeType::push) {
            ...
            // two-state switch
            auto const newstate{
                !(Train->ggDoorRightPermitButton.GetDesiredValue() > 0.5)};

            Train->mvOccupied->PermitDoors(side, newstate);
            // visual feedback
            Train->ggDoorRightPermitButton.UpdateValue(
                (newstate ? 1.0 : 0.0), Train->dsbSwitch);
        }
    } else if (Command.action == GLFW_RELEASE) {
        if (Train->ggDoorRightPermitButton.type() == TGaugeType::push) {
...
void TTrain::OnCommand_doorpermitpresetactivatenext(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        Train->mvOccupied->ChangeDoorPermitPreset(1);
        // visual feedback
        Train->ggDoorPermitPresetButton.UpdateValue(
            Train->mvOccupied->Doors.permit_preset, Train->dsbSwitch);
    }
}

void TTrain::OnCommand_doorpermitpresetactivateprevious(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        Train->mvOccupied->ChangeDoorPermitPreset(-1);
        // visual feedback
...
void TTrain::OnCommand_dooropenleft(
    TTrain *Train, command_data const &Command) {
...
void TTrain::OnCommand_doorcloseleft(
    TTrain *Train, command_data const &Command) {
    ...
    if (Command.action == GLFW_PRESS) {
        if (Train->mvOccupied->Doors.has_autowarning) {
            // automatic departure signal delays actual door closing until the
            // button is released
            Train->mvOccupied->signal_departure(true);
        } else {
            // TODO: move door opening/closing to the update, so the switch
            // animation doesn't hinge on door working
            Train->mvOccupied->OperateDoors(
                (Train->cab_to_end() == end::front ? side::left : side::right),
                false);
        }
        ...
        // visual feedback
        // dedicated closing buttons are presumed to be impulse switches and
        // return automatically to neutral position
        if (Train->ggDoorLeftOffButton.SubModel)
            Train->ggDoorLeftOffButton.UpdateValue(0.0, Train->dsbSwitch);
    }
}

void TTrain::OnCommand_doortoggleright(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        // NOTE: test how the door state check works with consists where the
        // occupied vehicle doesn't have opening doors
        if (false == ((Train->ggDoorRightButton.GetDesiredValue() > 0.5) ||
        ...
        if (true == ((Train->ggDoorRightButton.GetDesiredValue() > 0.5) ||
                     (Train->ggDoorRightOnButton.GetDesiredValue() > 0.5))) {
            // open
            if ((Train->mvOccupied->Doors.has_autowarning) &&
                (Train->mvOccupied->DepartureSignal)) {
...
void TTrain::OnCommand_dooropenright(
    TTrain *Train, command_data const &Command) {
    auto const remoteopencontrol{
        (Train->mvOccupied->Doors.open_control == control_t::driver) ||
        (Train->mvOccupied->Doors.open_control == control_t::mixed)};
...
void TTrain::OnCommand_doorcloseright(
    TTrain *Train, command_data const &Command) {
    auto const remoteclosecontrol{
        (Train->mvOccupied->Doors.close_control == control_t::driver) ||
        (Train->mvOccupied->Doors.close_control == control_t::mixed)};
...
void TTrain::OnCommand_doorcloseall(
    TTrain *Train, command_data const &Command) {
    auto const remoteclosecontrol{
        (Train->mvOccupied->Doors.close_control == control_t::driver) ||
        (Train->mvOccupied->Doors.close_control == control_t::mixed)};
...
void TTrain::OnCommand_doorsteptoggle(
    TTrain *Train, command_data const &Command) {
    // TODO: move logic/visualization code to the gauge, on_command() should
    // return hint whether it should invoke a reaction
    if (Command.action == GLFW_PRESS) {
        // effect
        if (false == Train->ggDoorStepButton.is_delayed()) {
            Train->mvOccupied->PermitDoorStep(
                false == Train->mvOccupied->Doors.step_enabled);
        }
        // visual feedback
        auto const isactive{
            (Train->ggDoorStepButton.is_push()  // always press push button
             || Train->mvOccupied->Doors
                    .step_enabled)};  // for toggle buttons indicate item state
...
void TTrain::OnCommand_doormodetoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        Train->mvOccupied->ChangeDoorControlMode(
            false == Train->mvOccupied->Doors.remote_only);
    }
}

void TTrain::OnCommand_mirrorstoggle(
    TTrain *Train, command_data const &Command) {
    if (Command.action != GLFW_PRESS) {
        return;
    }
...
void TTrain::OnCommand_nearestcarcouplingincrease(
    TTrain *Train, command_data const &Command) {
    if ((true == Command.freefly) && (Command.action == GLFW_PRESS)) {
        // tryb freefly, press only
        auto coupler{-1};
        auto *vehicle{Train->DynamicObject->ABuScanNearestObject(
            Command.location, Train->DynamicObject->GetTrack(), 1, 1500,
            coupler)};
        if (vehicle == nullptr)
            vehicle = Train->DynamicObject->ABuScanNearestObject(
                Command.location, Train->DynamicObject->GetTrack(), -1, 1500,
                coupler);
...
void TTrain::OnCommand_nearestcarcouplingdisconnect(
    TTrain *Train, command_data const &Command) {
    if ((true == Command.freefly) && (Command.action == GLFW_PRESS)) {
        // tryb freefly, press only
        auto coupler{-1};
...
void TTrain::OnCommand_nearestcarcoupleradapterattach(
    TTrain *Train, command_data const &Command) {
    if ((true == Command.freefly) && (Command.action == GLFW_PRESS)) {
        // tryb freefly, press only
        auto *vehicle{
            std::get<TDynamicObject *>(simulation::Region->find_vehicle(
                Command.location, 50, false, true))};
        if (vehicle == nullptr) {
            return;
        }
        ...
        auto const coupler =
            (glm::length2(
                 glm::vec3{vehicle->CouplerPosition(end::front)} -
                 Command.location) <
                     glm::length2(
                         glm::vec3{vehicle->CouplerPosition(end::rear)} -
                         Command.location)
                 ? end::front
                 : end::rear);
...
void TTrain::OnCommand_nearestcarcoupleradapterremove(
    TTrain *Train, command_data const &Command) {
    if ((true == Command.freefly) && (Command.action == GLFW_PRESS)) {
        // tryb freefly, press only
        auto *vehicle{
...
void TTrain::OnCommand_occupiedcarcouplingdisconnect(
    TTrain *Train, command_data const &Command) {
    //    if( false == Train->m_controlmapper.contains( "couplingdisconnect_sw:"
    //    ) ) { return; }
...
void TTrain::OnCommand_occupiedcarcouplingdisconnectback(
    TTrain *Train, command_data const &Command) {
    //    if( false == Train->m_controlmapper.contains( "couplingdisconnect_sw:"
    //    ) ) { return; }
...
void TTrain::OnCommand_departureannounce(
    TTrain *Train, command_data const &Command) {
...
void TTrain::OnCommand_hornlowactivate(
    TTrain *Train, command_data const &Command) {
    ...
    Train->playerCtrl->setHornLow(Command.action != GLFW_RELEASE);
}

void TTrain::OnCommand_hornhighactivate(
    TTrain *Train, command_data const &Command) {
    if ((Train->ggHornButton.SubModel == nullptr) &&
    ...
    Train->playerCtrl->setHornHigh(Command.action != GLFW_RELEASE);
}

void TTrain::OnCommand_whistleactivate(
    TTrain *Train, command_data const &Command) {
...
void TTrain::OnCommand_radiotoggle(TTrain *Train, command_data const &Command) {
    ...
    // NOTE: we ignore the lack of 3d model to allow system reset after
    // receiving radio-stop signal
    /*
...
void TTrain::OnCommand_radiochannelincrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
...
void TTrain::OnCommand_radiochanneldecrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        Train->RadioChannel() = clamp(Train->RadioChannel() - 1, 1, 10);
        // visual feedback
...
void TTrain::OnCommand_radiostopsend(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        if ((true == Train->mvOccupied->Radio) &&
            (Train->mvOccupied->Power24vIsAvailable ||
             Train->mvOccupied->Power110vIsAvailable)) {
...
void TTrain::OnCommand_radiostoptest(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
...
void TTrain::OnCommand_radiocall3send(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        if ((Train->RadioChannel() != 10) &&
            (true == Train->mvOccupied->Radio) &&
            (Train->mvOccupied->Power24vIsAvailable ||
             Train->mvOccupied->Power110vIsAvailable)) {
            simulation::Events.queue_receivers(
                radio_message::call3, Train->Dynamic()->GetPosition());
        }
        // visual feedback
...
void TTrain::OnCommand_radiovolumeincrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
...
void TTrain::OnCommand_radiovolumedecrease(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        Global.RadioVolume = clamp(Global.RadioVolume - 0.125, 0.0, 1.0);
        // visual feedback
...
void TTrain::OnCommand_cabchangeforward(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        auto const *owner{
        ...
        if (false == Train->CabChange(movedirection)) {
            auto const exitdirection{
                (movedirection > 0 ? end::front : end::rear)};
            if (TestFlag(
                    Train->mvOccupied->Couplers[exitdirection].CouplingFlag,
                    coupling::gangway)) {
        ...
        // HACK: match consist door permit state with the preset in the
        // active cab
...
void TTrain::OnCommand_cabchangebackward(
    TTrain *Train, command_data const &Command) {
    if (Command.action == GLFW_PRESS) {
        auto const *owner{
            (Train->DynamicObject->ctOwner != nullptr
        ...
        auto const movedirection{-1};
        if (false == Train->CabChange(movedirection)) {
            // current vehicle doesn't extend any farther in this direction,
            // check if we there's one connected we can move to
            auto const exitdirection{
                (movedirection > 0 ? end::front : end::rear)};
            if (TestFlag(
                    Train->mvOccupied->Couplers[exitdirection].CouplingFlag,
...
void TTrain::OnCommand_vehiclemoveforwards(
    TTrain *Train, const command_data &Command) {
    if (Command.action == GLFW_RELEASE || !DebugModeFlag) return;

    Train->DynamicObject->move_set(100.0);
}

void TTrain::OnCommand_vehiclemovebackwards(
    TTrain *Train, const command_data &Command) {
    if (Command.action == GLFW_RELEASE || !DebugModeFlag) return;
...
void TTrain::OnCommand_vehicleboost(
    TTrain *Train, const command_data &Command) {
    if (Command.action == GLFW_RELEASE || !DebugModeFlag) return;
...
void TTrain::UpdateCab() {
    ...
    if ((DynamicObject->Mechanik)  // może nie być?
        ...
        if (iCabn !=
            ...
            InitializeCab(
                mvOccupied->CabOccupied, mvOccupied->TypeName + ".mmd");
        }
    }
...
bool TTrain::Update(double const Deltatime) {
    ...
    if (m_linebreakerstate == 0) {
        if (true == mvControlled->Mains) {
            // crude way to sync state of the linebreaker with ai-issued
            // commands
    ...
    if (m_linebreakerstate == 1) {
        if (false == (mvControlled->Mains || mvControlled->dizel_startup)) {
            ...
            // because the state of the line breaker isn't changed to match,
    ...
    if (((ggMainButton.SubModel != nullptr) &&
        ...
        ((ggMainOnButton.SubModel != nullptr) &&
             (ggMainOnButton.GetDesiredValue() > 0.95) ||
         (ggIgnitionKey.GetDesiredValue() > 0.95))) {  // HACK: fallback
        // keep track of period the line breaker button is held down, to
        // determine when/if circuit closes
    ...
    if (ggMainOffButton.GetDesiredValue() > 0.95) {
        // if the button disconnecting the line breaker is down prevent the
    ...
    if (m_linebreakerstate == 0) {
        if (fMainRelayTimer > mvControlled->InitialCtrlDelay) {
            ...
            // mark the line breaker as ready to close; for electric series
            // vehicles with impulse switch the setup is completed on button
            // release
            m_linebreakerstate = 2;
        }
    }
    if (m_linebreakerstate == 2) {
        // for diesels and/or vehicles with toggle switch setup we complete
    ...
    if (DynamicObject->Mechanik != nullptr) {
        ...
        m_doorspermitleft =
            mvOccupied->Doors
                .instances[(
                    cab_to_end() == end::front ? side::left : side::right)]
                .open_permit &&
            ((simulation::Time.data().wSecond % 2 < 1) ||
             (mvOccupied->DoorsPermitLightBlinking < 1) ||
             ((mvOccupied->DoorsPermitLightBlinking < 2) &&
                  (DynamicObject->Mechanik->IsAnyDoorOpen[(
                      cab_to_end() == end::front ? side::left
                                                 : side::right)]) ||
              ((mvOccupied->DoorsPermitLightBlinking < 3) &&
               DynamicObject->Mechanik->IsAnyDoorOnlyOpen[(
                   cab_to_end() == end::front ? side::left : side::right)])));
        m_doorspermitright =
            mvOccupied->Doors
                .instances[(
                    cab_to_end() == end::front ? side::right : side::left)]
                .open_permit &&
            ((simulation::Time.data().wSecond % 2 < 1) ||
             (mvOccupied->DoorsPermitLightBlinking < 1) ||
             ((mvOccupied->DoorsPermitLightBlinking < 2) &&
                  (DynamicObject->Mechanik->IsAnyDoorOpen[(
                      cab_to_end() == end::front ? side::right
                                                 : side::left)]) ||
              ((mvOccupied->DoorsPermitLightBlinking < 3) &&
               DynamicObject->Mechanik->IsAnyDoorOnlyOpen[(
                   cab_to_end() == end::front ? side::right : side::left)])));
    ...
    // NOTE: this is a temporary arrangement, for the transition period from
    // old command setup to the new one eventually commands are going to be
    // retrieved directly by the vehicle, filtered through active control
    // stand and ultimately executed, provided the stand allows it.
    command_data commanddata;
    while (simulation::Commands.pop(
        commanddata,
        static_cast<std::size_t>(command_target::vehicle) | id())) {
        auto lookup = m_commandhandlers.find(commanddata.command);
        if (lookup != m_commandhandlers.end()) {
            // debug data
            if (commanddata.action == GLFW_PRESS) {
                WriteLog(
                    mvOccupied->Name + " received command: [" +
                    simulation::Commands_descriptions[static_cast<std::size_t>(
                                                          commanddata.command)]
                        .name +
                    "]");
            }
    ...
    if ((DynamicObject->Mechanik != nullptr) &&
        ...
        if ((mvOccupied->TrainType == dt_ET40) ||
            (mvOccupied->TrainType == dt_EP05) || (mvOccupied->HasCamshaft)) {
            // dla ET40 i EU05 automatyczne cofanie nastawnika - i tak nie
            ...
            if (false == (m_mastercontrollerinuse || Global.ctrlState)) {
                ...
                if (m_mastercontrollerreturndelay < 0.f) {
                    m_mastercontrollerreturndelay =
                        EU07_CONTROLLER_BASERETURNDELAY;
                    if (mvOccupied->MainCtrlPos >
                        mvOccupied->MainCtrlActualPos) {
                        mvOccupied->DecMainCtrl(1);
                    } else if (
                        mvOccupied->MainCtrlPos <
                        mvOccupied->MainCtrlActualPos) {
    ...
    // McZapkie: predkosc wyswietlana na tachometrze brana jest z obrotow
    // kol
    auto const maxtacho{3.0};
    fTachoVelocity = static_cast<float>(std::min(
        std::abs(11.31 * mvControlled->WheelDiameter * mvControlled->nrot),
        mvControlled->Vmax * 1.05));
    {  // skacze osobna zmienna
        float ff =
            simulation::Time.data().wSecond;  // skacze co sekunde - pol sekundy
        ...
        if (ff != fTachoTimer)  // jesli w tej sekundzie nie zmienial
        {
            if (fTachoVelocity > 1)  // jedzie
                fTachoVelocityJump =
                    fTachoVelocity +
                    (2.0 - LocalRandom(3) + LocalRandom(3)) * 0.5;
            else
    ...
    // Ra 2014-09: napięcia i prądy muszą być ustalone najpierw, bo wysyłane
    // są ewentualnie na PoKeys
    if ((mvControlled->EngineType != TEngineType::DieselElectric) &&
        (mvControlled->EngineType !=
         TEngineType::ElectricInductionMotor)) {  // Ra 2014-09: czy taki
                                                  // rozdzia? ma sens?
        fHVoltage = std::max(
            mvControlled->PantographVoltage,
            mvControlled
                ->GetTrainsetHighVoltage());  // Winger czy to nie jest zle?
    }
    // *mvControlled->Mains);
    else {
    ...
    for (int i = 0; i < 20; i++) {
        if (p) {
            ...
            fPress[i][5] =
                (p->MoverParameters->TotalMass - p->MoverParameters->Mred) *
                0.001;
            bBrakes[i][0] = p->MoverParameters->SpringBrake.IsActive;
            bBrakes[i][1] = p->MoverParameters->SpringBrake.ShuttOff;
            bDoors[i][1] =
                (p->MoverParameters->Doors.instances[side::left].position >
                 0.f);
            bDoors[i][2] =
                (p->MoverParameters->Doors.instances[side::right].position >
                 0.f);
            bDoors[i][3] =
                (p->MoverParameters->Doors.instances[side::left].step_position >
                 0.f);
            bDoors[i][4] =
                (p->MoverParameters->Doors.instances[side::right]
                     .step_position > 0.f);
            ...
            cCode[i] =
                p->MoverParameters
                    ->TypeName[p->MoverParameters->TypeName.length() - 1];
            asCarName[i] = p->name();
            if (p->MoverParameters->EnginePowerSource.SourceType ==
                TPowerSource::CurrentCollector) {
                bPants[iUnitNo - 1][end::front] =
                    (bPants[iUnitNo - 1][end::front] ||
                     p->MoverParameters->Pantographs[end::front].is_active);
                bPants[iUnitNo - 1][end::rear] =
                    (bPants[iUnitNo - 1][end::rear] ||
                     p->MoverParameters->Pantographs[end::rear].is_active);
            }
            // TBD, TODO: clean up compressor data arrangement?
            if (iUnitNo <= 8) {
                bComp[iUnitNo - 1][0] =
                    (bComp[iUnitNo - 1][0] ||
                     p->MoverParameters->CompressorAllow ||
                     (p->MoverParameters->CompressorStart ==
                      start_t::automatic));
            }
            if (p->MoverParameters->CompressorSpeed > 0.00001) {
                ...
                bCompressors.emplace_back(
                    p->MoverParameters->CompressorAllow ||
                        (p->MoverParameters->CompressorStart ==
                         start_t::automatic),
                    p->MoverParameters->CompressorFlag, i);
            }
            ...
            if ((in < 8) && (p->MoverParameters->eimc[eimc_p_Pmax] > 1)) {
                ...
                fEIMParams[1 + in][3] =
                    p->MoverParameters->eimv[eimv_Fmax] /
                ...
                fEIMParams[1 + in][8] =
                    p->MoverParameters
                        ->Itot;  // p->MoverParameters->eimv[eimv_Ipoj];
                ...
                // bComp[in][2] = (p->MoverParameters->CompressorAllow ||
                // (p->MoverParameters->CompressorStart ==
                // start_t::automatic)); bComp[in][3] =
                // (p->MoverParameters->CompressorFlag);
                in++;
                iPowerNo = in;
            }
            if ((in < 8) && ((p->MoverParameters->EngineType ==
                              TEngineType::DieselEngine) ||
                             (p->MoverParameters->EngineType ==
                              TEngineType::DieselElectric))) {
                fDieselParams[1 + in][0] = p->MoverParameters->enrot * 60;
                fDieselParams[1 + in][1] = p->MoverParameters->nrot;
                fDieselParams[1 + in][2] =
                    p->MoverParameters->RList[p->MoverParameters->MainCtrlPos]
                        .R;
                fDieselParams[1 + in][3] = p->MoverParameters->dizel_fill;
                fDieselParams[1 + in][4] =
                    p->MoverParameters->RList[p->MoverParameters->MainCtrlPos]
                        .Mn;
            ...
            if ((kier ? p->Next(coupling::permanent)
                      : p->Prev(coupling::permanent)) !=
                (kier ? p->Next(coupling::control)
                      : p->Prev(coupling::control)))
                iUnitNo++;
            p =
                (kier ? p->Next(coupling::control)
                      : p->Prev(coupling::control));
            ...
            fPress[i][0] = fPress[i][1] = fPress[i][2] = fPress[i][3] =
                fPress[i][4] = fPress[i][5] = 0;
            bDoors[i][0] = bDoors[i][1] = bDoors[i][2] = bDoors[i][3] =
                bDoors[i][4] = false;
            bBrakes[i][0] = bBrakes[i][1] = false;
            bSlip[i] = false;
            iUnits[i] = 0;
    ...
    //            fEIMParams[0][3] = mvControlled->eimv[eimv_Fzad]; //
    //            procent zadany
    //        else
    //            fEIMParams[0][3] =
    //                mvControlled->eimv[eimv_Fzad] -
    //                mvOccupied->LocalBrakeRatio(); // procent zadany
    ...
    if (Global.iFeedbackMode == 4) {
        ...
        Console::ValueSet(
            5, fHCurrent[(mvControlled->TrainType & dt_EZT) ? 0 : 1]);
        // Ra: prędkość na pin 43 - wyjście analogowe (to nie jest PWM);
        // skakanie zapewnia mechanika napędu
    ...
    if (mvControlled->ConverterFlag == true) {
        ...
        if ((mvControlled->CompressorFlag == true) &&
            (mvControlled->CompressorPower == 1) &&
            ((mvControlled->EngineType == TEngineType::ElectricSeriesMotor) ||
             (mvControlled->TrainType == dt_EZT)) &&
            (DynamicObject->Controller ==
             Humandriver)  // hunter-110212: poprawka dla EZT
            && (false ==
                DynamicObject->Mechanik
                    ->AIControllFlag)) {  // hunter-091012: poprawka (zmiana
                                          // warunku z CompressorPower
                                          // /rozne od 0/ na /rowne 1/)
            if (fConverterTimer < fConverterPrzekaznik) {
                mvControlled->ConvOvldFlag = true;
                if (mvControlled->TrainType != dt_EZT)
                    mvControlled->MainSwitch(
                        false,
                        (mvControlled->TrainType == dt_EZT ? range_t::unit
                                                           : range_t::local));
            } else if (fConverterTimer >= fConverterPrzekaznik) {
                // changed switch from always true to take into account
                // state of the compressor switch
    ...
    auto const lowvoltagepower{
        mvOccupied->Power24vIsAvailable || mvOccupied->Power110vIsAvailable};
        ...
        if (tmp) {
            if (tmp->MoverParameters->Power > 0) {
                ...
                if (ggWater1TempB.SubModel) {
                    ggWater1TempB.UpdateValue(
                        tmp->MoverParameters->dizel_heat.temperatura1);
                    ggWater1TempB.Update();
                }
                if (ggOilPressB.SubModel) {
                    ggOilPressB.UpdateValue(
                        tmp->MoverParameters->OilPump.pressure);
                    ggOilPressB.Update();
                }
            }
        }
    ...
    if (ggClockMInd.SubModel) {
        ...
        ggClockMInd.Update();
        ggClockHInd.UpdateValue(
            simulation::Time.data().wHour +
            simulation::Time.data().wMinute / 60.0);
        ggClockHInd.Update();
    }
        ...
        if (mvControlled->DynamicBrakeFlag) {
            ...
            if ((mvControlled->RList[mvControlled->MainCtrlActualPos].Mn > 0) &&
                ...
                ggEngineVoltage.UpdateValue(
                    (x *
                     (std::abs(mvControlled->EngineVoltage) -
    ...
    if (ggLVoltage.SubModel) {
        // NOTE: since we don't have functional converter object, we're
        // faking it here by simple check whether converter is on
        // TODO: implement object-based circuits and power systems model so
        // we can have this working more properly
        ggLVoltage.UpdateValue(std::max(
            (mvOccupied->Power110vIsAvailable
                 ? mvOccupied->NominalBatteryVoltage
                 : 0.0),
            (mvOccupied->Power24vIsAvailable ? mvOccupied->BatteryVoltage
                                             : 0.0)));
        ggLVoltage.Update();
    }

    if (mvControlled->EngineType ==
        TEngineType::DieselElectric) {  // ustawienie zmiennych dla silnika
                                        // spalinowego
        ...
        mvControlled->EngineType ==
        TEngineType::DieselEngine) {  // albo dla innego spalinowego
        fEngine[1] = mvControlled->ShowEngineRotation(1);
        fEngine[2] = mvControlled->ShowEngineRotation(2);
        fEngine[3] = mvControlled->ShowEngineRotation(3);
        if (ggMainGearStatus.SubModel) {
            if (mvControlled->Mains)
                ggMainGearStatus.UpdateValue(
                    1.1 - std::abs(mvControlled->dizel_automaticgearstatus));
            else
        ...
        if ((ggIgnitionKey.SubModel) &&
            ...
            ggIgnitionKey.UpdateValue(
                (mvControlled->Mains) || (mvControlled->dizel_startup) ||
                (fMainRelayTimer > 0.f) ||
                ((ggMainButton.SubModel != nullptr) &&
                 (ggMainButton.GetDesiredValue() > 0.95)) ||
                ((ggMainOnButton.SubModel != nullptr) &&
                 (ggMainOnButton.GetDesiredValue() > 0.95)));
        }
    ...
    if (mvControlled->SlippingWheels) {
        // Ra 2014-12: lokomotywy 181/182 dostają SlippingWheels po
        // zahamowaniu powyżej 2.85 bara i buczały
        double veldiff = (DynamicObject->GetVelocity() - fTachoVelocity) /
                         mvControlled->Vmax;
        if (veldiff < -0.01) {
            // 1% Vmax rezerwy, żeby 181/182 nie buczały po zahamowaniu, ale
            // to proteza
    ...
    if (true == lowvoltagepower) {
        ...
        btLampkaCzuwakaSHP.Turn(
            btLampkaSHP.GetValue() || btLampkaCzuwaka.GetValue());

        btLampkaWylSzybki.Turn(
            (((m_linebreakerstate == 2) || (true == mvControlled->Mains))
                 ? true
                 : false));
        ...
        btLampkaMainBreakerBlinkingIfReady.Turn(
            (((m_linebreakerstate == 2) || (true == mvControlled->Mains) ||
              ((mvControlled->MainsInitTimeCountdown < 0.0) &&
               (simulation::Time.data().wMilliseconds > 500)))
                 ? true
                 : false));

        btLampkaPrzetw.Turn(mvOccupied->Power110vIsAvailable);
        btLampkaPrzetwOff.Turn(false == mvOccupied->Power110vIsAvailable);
        btLampkaNadmPrzetw.Turn(
            Dynamic()->Mechanik
                ? Dynamic()->Mechanik->IsAnyConverterOverloadRelayOpen
                : mvControlled->ConvOvldFlag);

        btLampkaOpory.Turn(
            mvControlled->StLinFlag ? mvControlled->ResistorsFlagCheck()
                                    : false);
        ...
        btLampkaPrzekRozn.Turn(
            ((mvControlled->GroundRelay) ||
             (mvControlled->ControlPressureSwitch))
                ? false
                : (mvControlled->BrakePress <
                   1.0));  // relay is off and needs a reset

        btLampkaNadmSil.Turn(
            ((false == mvControlled->FuseFlagCheck()) ||
             (mvControlled->ControlPressureSwitch))
                ? false
                : (mvControlled->BrakePress <
                   1.0));  // relay is off and needs a reset

        if (((mvControlled->CabOccupied == 1) &&
             (TestFlag(
                 mvControlled->Couplers[end::rear].CouplingFlag,
                 coupling::control))) ||
            ((mvControlled->CabOccupied == -1) &&
             (TestFlag(
                 mvControlled->Couplers[end::front].CouplingFlag,
                 coupling::control)))) {
        ...
        btLampkaHamPosp.Turn((TestFlag(
            mvOccupied->Hamulec->GetBrakeStatus(),
            1)));  // lampka drugiego stopnia hamowania
        // TODO: youBy wyciągnąć flagę wysokiego stopnia

        // hunter-121211: lampka zanikowo-pradowego wentylatorow:
        btLampkaNadmWent.Turn(
            (mvControlled->RventRot < 5.0) &&
        ...
        if ((false == mvControlled->DelayCtrlFlag) &&
            ((mvControlled->ScndCtrlActualPos > 0) ||
             ((mvControlled->RList[mvControlled->MainCtrlActualPos].ScndAct !=
               0) &&
              (mvControlled->RList[mvControlled->MainCtrlActualPos].ScndAct !=
               255)))) {
        ...
        btLampkaNapNastHam.Turn(
            mvControlled->DirActive != 0);  // napiecie na nastawniku hamulcowym
        btLampkaSprezarka.Turn(
            mvControlled->CompressorFlag);  // mutopsitka dziala
        ...
        // Ra: w SU45 boczniki wchodzą na MainCtrlPos, a nie na
        // MainCtrlActualPos
        // - pokićkał ktoś?
        scp = mvControlled->RList[mvControlled->MainCtrlPos].ScndAct;
        scp = (scp == 255 ? 0 : scp);  // Ra: whatta hella is this?
        if ((mvControlled->ScndCtrlPos > 0) ||
            (mvControlled->ScndInMain != 0) &&
                (scp > 0)) {  // boczniki pojedynczo
        ...
        switch (mvControlled->TrainType) {
            ...
            case dt_EZT: {
                btLampkaHamienie.Turn(
                    (mvControlled->BrakePress >= 0.2) &&
            ...
            case dt_ET41: {
                ...
                if (mvSecond) {
                    // bo może komuś przyjść do głowy jeżdżenie jednym
                    // członem
            ...
            default: {
                ...
                btLampkaBrakingOff.Turn(
                    (mvOccupied->BrakePress < 0.1) &&
                    (false == mvControlled->DynamicBrakeFlag));
                break;
            }
        }
        // KURS90
        ...
        if (DynamicObject->Mechanik != nullptr) {
            btLampkaDoorLeft.Turn(DynamicObject->Mechanik->IsAnyDoorOpen[(
                cab_to_end() == end::front ? side::left : side::right)]);
            btLampkaDoorRight.Turn(DynamicObject->Mechanik->IsAnyDoorOpen[(
                cab_to_end() == end::front ? side::right : side::left)]);
        }
        ...
        btLampkaBrakeProfileG.Turn(
            TestFlag(mvOccupied->BrakeDelayFlag, bdelay_G));
        btLampkaBrakeProfileP.Turn(
            TestFlag(mvOccupied->BrakeDelayFlag, bdelay_P));
        btLampkaBrakeProfileR.Turn(
            TestFlag(mvOccupied->BrakeDelayFlag, bdelay_R));
        ...
        // light indicators
        // NOTE: sides are hardcoded to deal with setups where single cab is
        // equipped with all indicators
        btLampkaUpperLight.Turn(
            (mvOccupied->iLights[end::front] & light::headlight_upper) != 0);
        btLampkaLeftLight.Turn(
            (mvOccupied->iLights[end::front] & light::headlight_left) != 0);
        btLampkaRightLight.Turn(
            (mvOccupied->iLights[end::front] & light::headlight_right) != 0);
        btLampkaLeftEndLight.Turn(
            (mvOccupied->iLights[end::front] & light::redmarker_left) != 0);
        btLampkaRightEndLight.Turn(
            (mvOccupied->iLights[end::front] & light::redmarker_right) != 0);
        btLampkaRearUpperLight.Turn(
            (mvOccupied->iLights[end::rear] & light::headlight_upper) != 0);
        btLampkaRearLeftLight.Turn(
            (mvOccupied->iLights[end::rear] & light::headlight_left) != 0);
        btLampkaRearRightLight.Turn(
            (mvOccupied->iLights[end::rear] & light::headlight_right) != 0);
        btLampkaRearLeftEndLight.Turn(
            (mvOccupied->iLights[end::rear] & light::redmarker_left) != 0);
        btLampkaRearRightEndLight.Turn(
            (mvOccupied->iLights[end::rear] & light::redmarker_right) != 0);
        // others
        btLampkaMalfunction.Turn(mvControlled->dizel_heat.PA);
        btLampkaMotorBlowers.Turn(
            (mvControlled->MotorBlowers[end::front].is_active) &&
            (mvControlled->MotorBlowers[end::rear].is_active));
    ...
    {                                  // yB - wskazniki drugiego czlonu
        TDynamicObject *tmp{nullptr};  //=mvControlled->mvSecond; //Ra 2014-07:
                                       // trzeba to jeszcze wyjąć z kabiny...
        // Ra 2014-07: no nie ma potrzeby szukać tego w każdej klatce
        if ((TestFlag(
                mvControlled->Couplers[1].CouplingFlag, coupling::control)) &&
            (mvOccupied->CabOccupied > 0))
            tmp = DynamicObject->NextConnected();
        if ((TestFlag(
                mvControlled->Couplers[0].CouplingFlag, coupling::control)) &&
            (mvOccupied->CabOccupied < 0))
            tmp = DynamicObject->PrevConnected();
        ...
        if (tmp) {
            if (lowvoltagepower) {
                ...
                // hunter-271211: sygnalizacja poslizgu w pierwszym
                // pojezdzie, gdy wystapi w drugim
                if (mover->SlippingWheels) {
                    // Ra 2014-12: lokomotywy 181/182 dostają SlippingWheels
                    // po zahamowaniu powyżej 2.85 bara i buczały
                    auto const veldiff{
                        (DynamicObject->GetVelocity() - fTachoVelocity) /
                        mvControlled->Vmax};
                    if (veldiff < -0.01) {
                        // 1% Vmax rezerwy, żeby 181/182 nie buczały po
                        // zahamowaniu, ale to proteza
                        auto const lightstate{std::abs(mover->Im) > 10.0};
                        btLampkaPoslizg.Turn(
                            btLampkaPoslizg.GetValue() || lightstate);
                    }
                }

                btLampkaSprezarkaB.Turn(
                    mover->CompressorFlag);  // mutopsitka dziala
                ...
                btLampkaNadmPrzetwB.Turn(
                    mover->ConvOvldFlag);  // nadmiarowy przetwornicy?
                btLampkaPrzetwB.Turn(
                    mover->ConverterFlag);  // zalaczenie przetwornicy
                btLampkaPrzetwBOff.Turn(false == mover->ConverterFlag);
                btLampkaHVoltageB.Turn(
                    mover->NoVoltRelay && mover->OvervoltageRelay);
                btLampkaMalfunctionB.Turn(mover->dizel_heat.PA);
                // motor fuse indicator turns on if the fuse was blown in
                // any unit under control
                if (mover->Mains) {
                    btLampkaNadmSil.Turn(
                        btLampkaNadmSil.GetValue() || mover->FuseFlagCheck());
                }
            } else  // wylaczone
            {
    ...
    if (ggJointCtrl.SubModel != nullptr) {
        // joint master controller moves forward to adjust power and
        // backward to adjust brakes
        auto const brakerangemultiplier{
            /* NOTE: scaling disabled as it was conflicting with associating
            sounds with control positions ( mvControlled->CoupledCtrl ?
                mvControlled->MainCtrlPosNo + mvControlled->ScndCtrlPosNo :
                mvControlled->MainCtrlPosNo )
        ...
        ggJointCtrl.UpdateValue(
            (mvOccupied->LocalBrakePosA > 0.0
                 ? mvOccupied->LocalBrakePosA * LocalBrakePosNo * -1 *
                       brakerangemultiplier
             : mvControlled->CoupledCtrl
    ...
    if (ggMainCtrl.SubModel != nullptr) {
        ...
        if ((DynamicObject->Mechanik != nullptr) &&
            (false ==
             DynamicObject->Mechanik->AIControllFlag)  // nie blokujemy AI
        ...
        if (mvControlled->CoupledCtrl) {
            ...
            ggMainCtrl.UpdateValue(
                double(mvControlled->MainCtrlPos), dsbNastawnikJazdy);
        }
        ggMainCtrl.Update();
    }
    if (ggMainCtrlAct.SubModel != nullptr) {
        if (mvControlled->CoupledCtrl)
            ggMainCtrlAct.UpdateValue(double(
                mvControlled->MainCtrlActualPos +
                mvControlled->ScndCtrlActualPos));
        else
    ...
    if (ggScndCtrlButton.SubModel != nullptr) {
        if (ggScndCtrlButton.is_toggle()) {
            ggScndCtrlButton.UpdateValue(
                ((mvControlled->ScndCtrlPos > 0) ? 1.f : 0.f), dsbSwitch);
        }
    ...
    if (ggDirKey.SubModel != nullptr) {
        if (mvControlled->TrainType != dt_EZT) {
            ggDirKey.UpdateValue(
                double(mvControlled->DirActive), dsbReverserKey);
        } else {
            ggDirKey.UpdateValue(
    ...
    if (ggBrakeCtrl.SubModel != nullptr) {
        ...
        if (DynamicObject->Mechanik ? (
                                    ...
                                    : false && Global.fCalibrateIn[0][1] !=
                                                   0.0)  // nie blokujemy AI
        {  // Ra: nie najlepsze miejsce, ale na początek gdzieś to dać
           // trzeba
                                      ...
                                      // FV4a i FVel6?
            {
                ...
                double b = Console::AnalogCalibrateGet(0);
                b = b * (mvOccupied->Handle->GetPos(bh_MAX) -
                         mvOccupied->Handle->GetPos(bh_MIN)) +
                    mvOccupied->Handle->GetPos(bh_MIN);
                b = clamp<double>(
                    b, mvOccupied->Handle->GetPos(bh_MIN),
                    mvOccupied->Handle->GetPos(
                        bh_MAX));            // przycięcie zmiennej do granic
                ggBrakeCtrl.UpdateValue(b);  // przesów bez zaokrąglenia
                mvOccupied->BrakeLevelSet(b);
            }
        } else
...
/*         || ( Global.bMWDmasterEnable && Global.bMWDBreakEnable )*/ ) ) {
            // Ra: nie najlepsze miejsce, ale na początek gdzieś to dać
            // trzeba Firleju: dlatego kasujemy i zastepujemy funkcją w
            // Console
            auto const b =
                clamp<double>(Console::AnalogCalibrateGet(1), 0.0, 1.0);
...
#endif
        {
            ...
            ggLocalBrake.UpdateValue(
                mvOccupied->LocalBrakePosA * LocalBrakePosNo);
        }
        ggLocalBrake.Update();
    }
    ...
    ggMaxCurrentCtrl.UpdateValue(
        (true == mvControlled->ShuntModeAllow
             ? (true == mvControlled->ShuntMode ? 1.f : 0.f)
             : (mvControlled->MotorOverloadRelayHighThreshold ? 1.f : 0.f)));
    ...
    auto const lightpower{
        (InstrumentLightType == 0 ? mvOccupied->Power24vIsAvailable ||
         ...
         : InstrumentLightType == 3 ? mvOccupied->Power24vIsAvailable ||
                                          mvOccupied->Power110vIsAvailable
         : InstrumentLightType == 4 ? mvOccupied->Power24vIsAvailable ||
                                          mvOccupied->Power110vIsAvailable
                                    : false)};
    InstrumentLightActive =
        (InstrumentLightType == 3 ? true :  // TODO: link the light state with
                                            // the state of the master key
             InstrumentLightType == 4
             ? (mvOccupied->iLights[end::front] != 0) ||
                   (mvOccupied->iLights[end::rear] != 0)
             : InstrumentLightActive);
    ...
    */
    ...
    ggHornButton.UpdateValue(
        simulation->isHighHornSignalActive()
            ? 1.0
            : (simulation->isLowHornSignalActive() ? -1.0 : 0.0));
    ggHornButton.Update();
    ggHornLowButton.UpdateValue((float)simulation->isLowHornSignalActive());
    ggHornLowButton.Update();
    ggHornHighButton.UpdateValue((float)simulation->isHighHornSignalActive());
    ...
    if ((ggCabActivationButton.SubModel != nullptr) &&
        (ggCabActivationButton.type() != TGaugeType::push)) {
        ggCabActivationButton.UpdateValue(
            mvOccupied->IsCabMaster() ? 1.0 : 0.0);
    }
    ...
    btHaslerBrakes.Turn(
        mvOccupied->BrakePress > 0.4);            // ciśnienie w cylindrach
    btHaslerCurrent.Turn(mvOccupied->Im != 0.0);  // prąd na silnikach

    // calculate current level of interior illumination
    {
        // TODO: organize it along with rest of train update in a more
        // sensible arrangement Ra: uzeleżnic od napięcia w obwodzie
        // sterowania hunter-091012: uzaleznienie jasnosci od przetwornicy
        int cabidx{0};
        for (auto &cab : Cabine) {
    ...
    // anti slip system activation, maintained while the control button is
    // down
    ...
    if (!FreeFlyModeFlag &&
        simulation::Train == this)  // don't bother if we're outside
...
void TTrain::update_sounds(double const Deltatime) {
    ...
    if (m_lastlocalbrakepressure != -1.f) {
        // calculate rate of pressure drop in local brake cylinder, once
        // it's been initialized
        auto const brakepressuredifference{
            mvOccupied->LocBrakePress - m_lastlocalbrakepressure};
        m_localbrakepressurechange = interpolate<float>(
            m_localbrakepressurechange,
            10 * (brakepressuredifference / Deltatime), 0.1f);
    }
    ...
    if (rsSBHiss) {
        if ((m_localbrakepressurechange < -0.05f) &&
            ...
            rsSBHiss->gain(clamp(
                rsSBHiss->m_amplitudeoffset + rsSBHiss->m_amplitudefactor *
                                                  -m_localbrakepressurechange *
                                                  0.05,
                0.0, 1.5));
    ...
    if (rsSBHissU) {
        if (m_localbrakepressurechange > 0.05f) {
            rsSBHissU->gain(clamp(
                rsSBHissU->m_amplitudeoffset + rsSBHissU->m_amplitudefactor *
                                                   m_localbrakepressurechange *
                                                   0.05,
                0.0, 1.5));
    ...
    // TODO: softer volume reduction than plain abrupt stop, perhaps as
    // reusable wrapper?
    if ((mvOccupied->BrakeHandle == TBrakeHandle::FV4a) ||
        (mvOccupied->BrakeHandle == TBrakeHandle::FVel6)) {
        // upuszczanie z PG
        if (rsHiss) {
            fPPress = interpolate(
                fPPress,
                static_cast<float>(mvOccupied->Handle->GetSound(s_fv4a_b)),
                0.05f);
            volume =
                (fPPress > 0 ? rsHiss->m_amplitudefactor * fPPress * 0.25 +
                                   rsHiss->m_amplitudeoffset
                             : 0);
        ...
        if (rsHissU) {
            fNPress = interpolate(
                fNPress,
                static_cast<float>(mvOccupied->Handle->GetSound(s_fv4a_u)),
                0.25f);
            volume =
                (fNPress > 0 ? rsHissU->m_amplitudefactor * fNPress +
                                   rsHissU->m_amplitudeoffset
                             : 0);
            if (volume * brakevolumescale > 0.05) {
        ...
        if (rsHissE) {
            volume = mvOccupied->Handle->GetSound(s_fv4a_e) *
                         rsHissE->m_amplitudefactor +
                     rsHissE->m_amplitudeoffset;
            if (volume * brakevolumescale > 0.05) {
        ...
        if (rsHissX) {
            volume = mvOccupied->Handle->GetSound(s_fv4a_x) *
                         rsHissX->m_amplitudefactor +
                     rsHissX->m_amplitudeoffset;
            if (volume * brakevolumescale > 0.05) {
        ...
        if (rsHissT) {
            volume = mvOccupied->Handle->GetSound(s_fv4a_t) *
                         rsHissT->m_amplitudefactor +
                     +rsHissT->m_amplitudeoffset;
            if (volume * brakevolumescale > 0.05) {
        ...
        if (rsHiss) {
            fPPress =
                (4.0f * fPPress + std::max(0.0, mvOccupied->dpMainValve)) /
                (4.0f + 1.0f);
            volume =
                (fPPress > 0.0f ? 2.0 * rsHiss->m_amplitudefactor * fPPress +
                                      rsHiss->m_amplitudeoffset
                                : 0.0);
            if (volume > 0.05) {
                rsHiss->gain(volume);
        ...
        // napelnianie PG
        if (rsHissU) {
            fNPress = (4.0f * fNPress + Min0R(0.0, mvOccupied->dpMainValve)) /
                      (4.0f + 1.0f);
            volume =
                (fNPress < 0.0f ? -1.0 * rsHissU->m_amplitudefactor * fNPress +
                                      rsHissU->m_amplitudeoffset
                                : 0.0);
            if (volume > 0.01) {
                rsHissU->gain(volume);
    ...
    if (rsBrake) {
        if ((mvOccupied->UnitBrakeForce > 10.0) && (mvOccupied->Vel > 0.05)) {
            auto const brakeforceratio{clamp(
                mvOccupied->UnitBrakeForce /
                    std::max(
                        1.0, mvOccupied->BrakeForceR(1.0, mvOccupied->Vel) /
                                 (mvOccupied->NAxles *
                                  std::max(1, mvOccupied->NBpA))),
                0.0, 1.0)};
            // HACK: in external view mute the sound rather than stop it, in
            // case there's an opening bookend it'd (re)play on sound
            // restart after returning inside
            volume =
                (FreeFlyModeFlag ? 0.0
            ...
            rsBrake->pitch(
                ...
                mvOccupied->Vel * rsBrake->m_frequencyfactor);
            rsBrake->gain(volume);
    ...
    // ambient sound
    // since it's typically ticking of the clock we can center it on
    // tachometer or on middle of compartment bounding area
    if (rsFadeSound) {
    ...
    if (rsRunningNoise) {
        if ((false == FreeFlyModeFlag) && (false == Global.CabWindowOpen) &&
            ...
            // don't play the optional ending sound if the listener switches
            // views
    ...
    if (rsHuntingNoise) {
        if ((false == FreeFlyModeFlag) && (false == Global.CabWindowOpen) &&
            ...
            auto const huntingamount = interpolate(
                ...
                clamp(
                    (mvOccupied->Vel -
                     DynamicObject->HuntingShake.fadein_begin) /
                        (DynamicObject->HuntingShake.fadein_end -
                         DynamicObject->HuntingShake.fadein_begin),
                    0.0, 1.0));
    ...
    if (m_rainsound) {
        if ((false == FreeFlyModeFlag) && (false == Global.CabWindowOpen) &&
            ...
            m_rainsound->gain(
                m_rainsound->m_amplitudeoffset +
                m_rainsound->m_amplitudefactor * 1.f);
        ...
        if (fTachoCount >= 3.f) {
            auto const frequency{
                (true == dsbHasler->is_combined()
                     ? fTachoVelocity * 0.01
                     : dsbHasler->m_frequencyoffset +
                           dsbHasler->m_frequencyfactor)};
    ...
    if (mvOccupied->Power24vIsAvailable || mvOccupied->Power110vIsAvailable) {
        ...
        if (mvOccupied->SecuritySystem.is_beeping()) {
            if (dsbBuzzer && false == dsbBuzzer->is_playing()) {
                dsbBuzzer->pitch(
                    dsbBuzzer->m_frequencyoffset +
                    dsbBuzzer->m_frequencyfactor);
                dsbBuzzer->gain(
                    dsbBuzzer->m_amplitudeoffset +
                    dsbBuzzer->m_amplitudefactor);
        ...
        if (m_distancecounterclear) {
            ...
            if (m_distancecounter > owner->fLength) {
                // play assigned sound if the train travelled its full
                // length since meter activation TBD: check all combinations
                // of directions and active cab
                m_distancecounter =
                    -1.f;  // turn off the meter after its task is done
                m_distancecounterclear->pitch(
                    m_distancecounterclear->m_frequencyoffset +
                    m_distancecounterclear->m_frequencyfactor);
                m_distancecounterclear->gain(
                    m_distancecounterclear->m_amplitudeoffset +
                    m_distancecounterclear->m_amplitudefactor);
...
void TTrain::update_sounds_runningnoise(sound_source &Sound) {
    ...
    auto const normalizer{
        (true == Sound.is_combined() ? mvOccupied->Vmax * 0.01f : 1.f)};
    auto const frequency{
        Sound.m_frequencyoffset +
        Sound.m_frequencyfactor * mvOccupied->Vel * normalizer};

    // volume calculation
    auto volume = Sound.m_amplitudeoffset +
                  Sound.m_amplitudefactor *
                      interpolate(
    ...
    if (std::abs(mvOccupied->nrot) > 0.01) {
        ...
        auto const brakeforceratio{(clamp(
            mvOccupied->UnitBrakeForce /
                std::max(
                    1.0,
                    mvOccupied->BrakeForceR(1.0, mvOccupied->Vel) /
                        (mvOccupied->NAxles * std::max(1, mvOccupied->NBpA))),
            0.0, 1.0))};
    ...
    volume *= interpolate(
        0.8, 1.2, clamp(DynamicObject->MyTrack->iQualityFlag / 20.0, 0.0, 1.0));
    // for single sample sounds muffle the playback at low speeds
    if (false == Sound.is_combined()) {
        volume *=
            interpolate(0.0, 1.0, clamp(mvOccupied->Vel / 25.0, 0.0, 1.0));
    }

    if (volume > 0.05) {
        Sound.pitch(frequency).gain(volume).play(
...
void TTrain::update_sounds_radio() {
    ...
    // adjust audibility of remaining messages based on current radio
    // conditions
    auto const radioenabled{
        (true == mvOccupied->Radio) &&
        (mvOccupied->Power24vIsAvailable || mvOccupied->Power110vIsAvailable)};
    for (auto &message : m_radiomessages) {
        auto const volume{
            (true == radioenabled) && (Dynamic()->Mechanik != nullptr) &&
                    (message.first == RadioChannel())
                ? Global.RadioVolume
                : 0.0};
        message.second->gain(volume);
        radio_message_played |= (true == radioenabled) &&
...
void TTrain::update_screens(double dt) {
    for (auto &screen : m_screens) {
        if (screen.updatetimecounter >= 0) screen.updatetimecounter += dt;

        if (screen.updatetimecounter <= screen.updatetime) continue;
...
bool TTrain::CabChange(int iDirection) {  // McZapkie-090902: zmiana kabiny
                                          // 1->0->2 i z powrotem
    if ((DynamicObject->Mechanik == nullptr) ||
        ...
        if (mvOccupied->ChangeCab(iDirection)) {
            if (InitializeCab(
                ...
                mvOccupied->CabActivisationAuto();  // załączenie rozrządu
                                                    // (wirtualne kabiny)
                DynamicObject->Mechanik->DirectionChange();
                return true;  // udało się zmienić kabinę
            }
        }
        // aktywizacja poprzedniej, bo jeszcze nie wiadomo, czy jakiś pojazd
        // jest
...
bool TTrain::LoadMMediaFile(std::string const &asFileName) {
    ...
    std::unordered_map<
        std::string, std::tuple<
                         std::optional<sound_source> &, sound_placement, float,
                         sound_type, int, double>>
        internalsounds = {
            {"ctrl:",
             {dsbNastawnikJazdy, sound_placement::internal,
              EU07_SOUND_CABCONTROLSCUTOFFRANGE, sound_type::single, 0, 100.0}},
            {"ctrlscnd:",
             {dsbNastawnikBocz, sound_placement::internal,
              EU07_SOUND_CABCONTROLSCUTOFFRANGE, sound_type::single, 0, 100.0}},
            {"reverserkey:",
             {dsbReverserKey, sound_placement::internal,
              EU07_SOUND_CABCONTROLSCUTOFFRANGE, sound_type::single, 0, 100.0}},
            {"buzzer:",
             {dsbBuzzer, sound_placement::internal,
              EU07_SOUND_CABCONTROLSCUTOFFRANGE, sound_type::single, 0, 100.0}},
            {"radiostop:",
             {m_radiostop, sound_placement::internal,
              EU07_SOUND_CABCONTROLSCUTOFFRANGE, sound_type::single, 0, 100.0}},
            {"slipalarm:",
             {dsbSlipAlarm, sound_placement::internal,
              EU07_SOUND_CABCONTROLSCUTOFFRANGE, sound_type::single, 0, 100.0}},
            {"distancecounter:",
             {m_distancecounterclear, sound_placement::internal,
              EU07_SOUND_CABCONTROLSCUTOFFRANGE, sound_type::single, 0, 100.0}},
            {"tachoclock:",
             {dsbHasler, sound_placement::internal,
              EU07_SOUND_CABCONTROLSCUTOFFRANGE, sound_type::single, 0, 100.0}},
            {"switch:",
             {dsbSwitch, sound_placement::internal,
              EU07_SOUND_CABCONTROLSCUTOFFRANGE, sound_type::single, 0, 100.0}},
            {"pneumaticswitch:",
             {dsbPneumaticSwitch, sound_placement::internal,
              EU07_SOUND_CABCONTROLSCUTOFFRANGE, sound_type::single, 0, 100.0}},
            {"airsound:",
             {rsHiss, sound_placement::internal,
              EU07_SOUND_CABCONTROLSCUTOFFRANGE, sound_type::single,
              sound_parameters::amplitude, 100.0}},
            {"airsound2:",
             {rsHissU, sound_placement::internal,
              EU07_SOUND_CABCONTROLSCUTOFFRANGE, sound_type::single,
              sound_parameters::amplitude, 100.0}},
            {"airsound3:",
             {rsHissE, sound_placement::internal,
              EU07_SOUND_CABCONTROLSCUTOFFRANGE, sound_type::single,
              sound_parameters::amplitude, 100.0}},
            {"airsound4:",
             {rsHissX, sound_placement::internal,
              EU07_SOUND_CABCONTROLSCUTOFFRANGE, sound_type::single,
              sound_parameters::amplitude, 100.0}},
            {"airsound5:",
             {rsHissT, sound_placement::internal,
              EU07_SOUND_CABCONTROLSCUTOFFRANGE, sound_type::single,
              sound_parameters::amplitude, 100.0}},
            {"localbrakesound:",
             {rsSBHiss, sound_placement::internal,
              EU07_SOUND_CABCONTROLSCUTOFFRANGE, sound_type::single,
              sound_parameters::amplitude, 100.0}},
            {"localbrakesound2:",
             {rsSBHissU, sound_placement::internal,
              EU07_SOUND_CABCONTROLSCUTOFFRANGE, sound_type::single,
              sound_parameters::amplitude, 100.0}},
            {"brakesound:",
             {rsBrake, sound_placement::internal, -1, sound_type::single,
              sound_parameters::amplitude | sound_parameters::frequency,
              100.0}},
            {"fadesound:",
             {rsFadeSound, sound_placement::internal,
              EU07_SOUND_CABCONTROLSCUTOFFRANGE, sound_type::single, 0, 100.0}},
            {"runningnoise:",
             {rsRunningNoise, sound_placement::internal, EU07_SOUND_GLOBALRANGE,
              ...
              mvOccupied->Vmax}},
            {"huntingnoise:",
             {rsHuntingNoise, sound_placement::internal, EU07_SOUND_GLOBALRANGE,
              sound_type::single,
              sound_parameters::amplitude | sound_parameters::frequency,
              mvOccupied->Vmax}},
            {"rainsound:",
             {m_rainsound, sound_placement::internal, -1, sound_type::single, 0,
              100.0}},
        };
    for (auto &soundconfig : internalsounds) {
        std::get<std::optional<sound_source> &>(soundconfig.second).reset();
    }
    // NOTE: since radiosound is an incomplete template not using
    // std::optional it gets a special treatment
    ...
    // NOTE: yaml-style comments are disabled until conflict in use of # is
    // resolved parser.addCommentStyle( "#", "\n" );
    std::string token;
    do {
        token = "";
        parser.getTokens();
    ...
    if (token == "internaldata:") {
        do {
            ...
            sound.deserialize(
                parser, std::get<sound_type>(soundconfig),
                std::get<int>(soundconfig), std::get<double>(soundconfig));
            sound.owner(DynamicObject);
            std::get<std::optional<sound_source> &>(soundconfig) = sound;
        } while (token != "");

        // assign default samples to sound emitters which weren't included
        // in the config file
        if (!m_rainsound) {
...
bool TTrain::InitializeCab(int NewCabNo, std::string const &asFileName) {
    ...
    is_cab_initialized =
        true;  // the attempt may fail, but it's the attempt that counts

    bool parse = false;
    int cabindex = 0;
    DynamicObject->mdKabina =
        nullptr;         // likwidacja wskaźnika na dotychczasową kabinę
    ...
    if (token == cabstr) {
        ...
        // NOTE: the position and angle definitions depend on strict entry
        // order
        // TODO: refactor into more flexible arrangement
        parser.getTokens();
        parser >> token;
        if (token ==
            std::string("driver" + std::to_string(cabindex) + "angle:")) {
            // camera view angle
            parser.getTokens(2, false);
            // angle is specified in degrees but internally stored in
            // radians
            glm::vec2 viewangle;
        ...
        if (token ==
            std::string("driver" + std::to_string(cabindex) + "pos:")) {
        ...
        if (token ==
            std::string("driver" + std::to_string(cabindex) + "sitpos:")) {
            // ABu 180404 pozycja siedzaca maszynisty
            parser.getTokens(3, false);
            parser >> pMechSittingPosition.x >> pMechSittingPosition.y >>
        ...
        do {
            ...
            // inicjacja kabiny
            // Ra 2014-08: zmieniamy zasady - zamiast przypisywać submodel
            // do istniejących obiektów animujących będziemy teraz
            // uaktywniać obiekty animujące z tablicy i podawać im submodel
            // oraz wskaźnik na parametr
            if (token ==
                std::string("cab" + std::to_string(cabindex) + "model:")) {
                // model kabiny
                parser.getTokens();
                parser >> token;
                ...
                if (token != "none") {
                    ...
                    // name can contain leading slash, erase it to avoid
                    // creation of double slashes when the name is combined
                    ...
                    TModel3d *kabina = TModelsManager::GetModel(
                        DynamicObject->asBaseDir + token, true, true,
                        (Global.network_servers.empty() &&
                    ...
                    //(mdKabina) może zostać to samo po przejściu do innego
                    // członu bez
                    // zmiany kabiny, przy powrocie musi być wiązanie
                    // ponowne else break; //wyjście z pętli, bo model
                    // zostaje bez zmian
                } else if (cabindex == 1) {
                    // model tylko, gdy nie ma kabiny 1
                    // McZapkie-170103: szukaj elementy kabiny w glownym
                    // modelu
            ...
            /*
                        ...
                        {
                            // don't bother with other parts until the cab
               is initialised continue;
                        }
            */
            ...
            // TODO: add "pydestination:"
                ...
                if ((false == screen.script.empty()) &&
                    (substr_path(screen.script).empty())) {
                ...
                if (screen.target != "none") {
                    submodel =
                        (DynamicObject->mdKabina
                             ? DynamicObject->mdKabina->GetFromName(
                                   screen.target)
                         : DynamicObject->mdLowPolyInt
                             ? DynamicObject->mdLowPolyInt->GetFromName(
                                   screen.target)
                             : nullptr);
                    if (submodel == nullptr) {
                        WriteLog(
                            "Python Screen: submodel " + screen.target +
                            " not found - Ignoring screen");
                        continue;
                    }
                    auto const material{submodel->GetMaterial()};
                    if (material <= 0) {
                        // sub model nie posiada tekstury lub tekstura
                        // wymienna
                        // - nie obslugiwana
                        WriteLog(
                            "Python Screen: invalid texture id " +
                            std::to_string(material) + " - Ignoring screen");
                        continue;
                    }

                    tex = &GfxRenderer->Texture(
                ...
                tex->create(true);  // make the surface static so it doesn't
                                    // get destroyed by garbage collector if
                                    // the user spends long time outside cab
                // TBD, TODO: keep texture handles around, so we can undo
                // the static switch when the user changes cabs?
                auto rt = std::make_shared<python_rt>();
                rt->shared_tex = tex->id;

                // record renderer and material binding for future update
                // requests
                m_screens.emplace_back(screen);
                m_screens.back().rt = rt;

                m_screens.back().touch_list =
                    std::make_shared<std::vector<glm::vec2>>();
                if (submodel)
                    submodel->screen_touch_list = m_screens.back().touch_list;

                if (Global.python_displaywindows)
                    m_screens.back().viewer =
                        std::make_unique<python_screen_viewer>(
                            rt, m_screens.back().touch_list,
                            m_screens.back().script);
            ...
            // btLampkaUnknown.Init("unknown",mdKabina,false);
                 ...
                 // TODO: enable full per-cab deserialization when/if .mmd
                 // files get proper per-cab switch configuration
        ...
        for (auto &screen : m_screens) {
            if (screen.updatetime > 0) {
                screen.updatetime =
                    std::max(
                        (int)screen.updatetime, Global.PythonScreenUpdateRate) *
                    0.001;
            }
            if (screen.updatetime == 0) {
                screen.updatetime =
                    std::max(Global.PythonScreenUpdateRate, ScreenUpdateRate) *
                    0.001;
            }
    ...
    /*
        ...
        {
    */
    // configure placement of sound emitters which aren't bound with any
    // device model, and weren't placed manually
    auto const caboffset{
        glm::dvec3{
            (Cabine[cabindex].CabPos1 + Cabine[cabindex].CabPos2) * 0.5} +
        glm::dvec3{0, 1, 0}};
    // NOTE: since radiosound is an incomplete template not using
    // std::optional it gets a special treatment
    ...
    std::vector<std::pair<
        std::reference_wrapper<std::optional<sound_source>>, glm::vec3>>
        soundlocations = {
            {dsbReverserKey, ggDirKey.model_offset()},
            {dsbNastawnikJazdy, ggJointCtrl.model_offset()},
            {dsbNastawnikJazdy,
             ggMainCtrl.model_offset()},  // NOTE: fallback for vehicles
                                          // without universal controller
            ...
            {rsSBHiss,
             ggBrakeCtrl.model_offset()},  // NOTE: fallback if the local
                                           // brake model can't be located
    ...
    // second pass, in case some items received no positioning due to
    // missing submodels etc
    for (auto &sound : soundlocations) {
        if ((sound.first.get()) &&
            (sound.first.get()->offset() == nullvector)) {
    ...
    if (DynamicObject->mdKabina)
        DynamicObject->mdKabina
            ->Init();  // obrócenie modelu oraz optymalizacja, również
                       // zapisanie binarnego
...
Math3D::vector3 TTrain::MirrorPosition(
    bool lewe) {  // zwraca współrzędne widoku kamery z lusterka
    auto const shiftdirection{(lewe ? -1 : 1) * (iCabn == 2 ? 1 : -1)};

    return DynamicObject->mMatrix *
           Math3D::vector3(
               mvOccupied->Dim.W * (0.5 * shiftdirection) +
                   (0.2 * shiftdirection),
               1.5 + Cabine[iCabn].CabPos1.y,
               interpolate(
                   Cabine[iCabn].CabPos1.z, Cabine[iCabn].CabPos2.z, 0.5));
};

void TTrain::DynamicSet(TDynamicObject *d) {  // taka proteza: chcę podłączyć
    // kabinę EN57 bezpośrednio z
    // silnikowym, aby nie robić tego
    // przez ukrotnienie
    // drugi silnikowy i tak musi być ukrotniony, podobnie jak kolejna
    // jednostka problem się robi ze światłami, które będą zapalane w
    // silnikowym, ale muszą świecić się w rozrządczych dla EZT światła
    // czołowe będą "zapalane w silnikowym", ale widziane z rozrządczych
    ...
    // jeździć dobrze również hamowanie wykonuje się zaworem w członie, a
    // nie w silnikowym...
    DynamicObject = d;  // jedyne miejsce zmiany
    mvOccupied = mvControlled =
        (d ? DynamicObject->MoverParameters : nullptr);  // albo silnikowy w EZT
    ...
    if (mvOccupied->Power >
        1.0)  // dwuczłonowe lub ukrotnienia, żeby nie szukać każdorazowo
        if (mvOccupied->Couplers[1].Connected
                ? mvOccupied->Couplers[1].AllowedFlag & coupling::control
                : false) {  // gdy jest człon od sprzęgu 1, a sprzęg łączony
            // warsztatowo (powiedzmy)
            if (mvOccupied->Couplers[1].Connected->Power >
                1.0)  // ten drugi ma moc
                mvSecond = (TMoverParameters *)mvOccupied->Couplers[1]
                               .Connected;  // wskaźnik na drugiego
        } else if (
            mvOccupied->Couplers[0].Connected
                ? mvOccupied->Couplers[0].AllowedFlag & coupling::control
                : false) {  // gdy jest człon od sprzęgu 0, a sprzęg łączony
            // warsztatowo (powiedzmy)
            if (mvOccupied->Couplers[0].Connected->Power >
                1.0)  // ale ten drugi ma moc
                mvSecond = (TMoverParameters *)mvOccupied->Couplers[0]
                               .Connected;  // wskaźnik na drugiego
        }
    // cache nearest unit equipped with pantographs
    {
        auto *lookup{DynamicObject->FindPantographCarrier()};
        // HACK: set pointer to existing vehicle to avoid error checking all
        // over the place
        mvPantographUnit =
            (lookup != nullptr ? lookup->MoverParameters : mvControlled);
    }
};
...
void TTrain::MoveToVehicle(TDynamicObject *target) {
    ...
    if (target_train) {
        ...
        if (!Dynamic()->Mechanik || !Dynamic()->Mechanik->AIControllFlag) {
            ...
            Occupied()->BrakeLevelSet(Occupied()->Handle->GetPos(
            ...
            if (Dynamic()->Mechanik) Dynamic()->Mechanik->MoveTo(target);

            target_train->Occupied()->LimPipePress =
                target_train->Occupied()->PipePress;
            target_train->Occupied()->CabActivisationAuto(
                true);  // załączenie rozrządu (wirtualne kabiny)
            target_train->Dynamic()->MechInside = true;
            if (target_train->Dynamic()->Mechanik) {
                target_train->Dynamic()->Controller =
                    target_train->Dynamic()->Mechanik->AIControllFlag;
        ...
        if (simulation::Train == this) {
            ...
            // our local driver may potentially be in external view mode, in
            // which case we shouldn't activate cab visualization
        ...
        // remove TTrain from global list, we're going to change dynamic
        // anyway
...
bool TTrain::point_inside(Math3D::vector3 const Point) const {
    return (Point.x >= Cabine[iCabn].CabPos1.x) &&
           (Point.x <= Cabine[iCabn].CabPos2.x) &&
           (Point.y >= Cabine[iCabn].CabPos1.y + 0.5) &&
           (Point.y <= Cabine[iCabn].CabPos2.y + 1.8) &&
           (Point.z >= Cabine[iCabn].CabPos1.z) &&
           (Point.z <= Cabine[iCabn].CabPos2.z);
}

Math3D::vector3 TTrain::clamp_inside(Math3D::vector3 const &Point) const {
    if (DebugModeFlag) {
        return Point;
    }
...
void TTrain::radio_message(sound_source *Message, int const Channel) {
    ...
    if ((soundrange > 0) &&
        (glm::length2(
             Message->location() - glm::dvec3{DynamicObject->GetPosition()}) >
         (soundrange * soundrange))) {
        // skip message playback if the receiver is outside of the emitter's
        // range
        return;
    }
    // NOTE: we initiate playback of all sounds in range, in case the user
    // switches on the radio or tunes to the right channel mid-play
    m_radiomessages.emplace_back(
        Channel, std::make_shared<sound_source>(m_radiosound));
...
void TTrain::clear_cab_controls() {
    ...
    btLampkaNadmPrzetw.Clear(
        ((mvControlled->TrainType & dt_EZT) != 0)
            ? -1
            : 7);  // EN57 nie ma tej lampki
    btLampkaPrzetwOff.Clear(
        ((mvControlled->TrainType & dt_EZT) != 0) ? 7 : -1);  // za to ma tę
    btLampkaNadmSpr.Clear(8);
    btLampkaNadmWent.Clear(9);
    btLampkaWysRozr.Clear(
        ((mvControlled->TrainType & dt_ET22) != 0)
            ? -1
            : 10);  // ET22 nie ma tej lampki
    btLampkaOgrzewanieSkladu.Clear(11);
    btHaslerBrakes.Clear(
        12);  // ciśnienie w cylindrach do odbijania na haslerze
...
// NOTE: we can get rid of this function once we have per-cab persistent
// state
void TTrain::set_cab_controls(int const Cab) {
    ...
    ggCabActivationButton.PutValue(
        (ggCabActivationButton.type() == TGaugeType::push ? 0.5f
         : mvOccupied->IsCabMaster()                      ? 1.f
                                                          : 0.f));
    // line breaker
    if (ggMainButton.SubModel != nullptr) {  // instead of single main button
                                             // there can be on/off pair
    ...
    // radio
    ggRadioChannelSelector.PutValue(
        (Dynamic()->Mechanik ? Dynamic()->Mechanik->iRadioChannel : 1) - 1);
    // pantographs
    /*
       ...
       ? 1.f : 0.f ) );
            ...
            if( ggPantFrontButtonOff.SubModel ) {
                ggPantFrontButtonOff.PutValue(
                    (
       mvControlled->Pantographs[end::front].valve.is_disabled ? 1.f : 0.f )
       );
            }
        }
        if( mvOccupied->PantSwitchType != "impulse" ) {
    ...
    */
    ...
    if (ggPantSelectedButton.type() == TGaugeType::toggle) {
        ggPantSelectedButton.PutValue(
            (mvPantographUnit->PantsValve.is_enabled ? 1.f : 0.f));
    } else {
        if (false == m_controlmapper.contains("pantselectedoff_sw:")) {
            // single impulse switch arrangement, with neutral position
            // mid-way
    ...
    if (ggPantSelectedDownButton.type() == TGaugeType::toggle) {
        ggPantSelectedDownButton.PutValue(
            (mvPantographUnit->PantsValve.is_disabled ? 1.f : 0.f));
    }
    ...
    ggPantCompressorValve.PutValue(
        mvControlled->bPantKurek3
            ? 0.f
            :  // default setting is pantographs connected with primary tank
            1.f);
    ...
    if (false == ggDoorLeftPermitButton.is_push()) {
        ggDoorLeftPermitButton.PutValue(
            mvOccupied->Doors
                    .instances[(
                        cab_to_end() == end::front ? side::left : side::right)]
                    .open_permit
    ...
    if (false == ggDoorRightPermitButton.is_push()) {
        ggDoorRightPermitButton.PutValue(
            mvOccupied->Doors
                    .instances[(
                        cab_to_end() == end::front ? side::right : side::left)]
                    .open_permit
    ...
    if (ggBrakeProfileG.SubModel != nullptr) {
        ggBrakeProfileG.PutValue(
            mvOccupied->BrakeDelayFlag == bdelay_G ? 1.f : 0.f);
    }
    if (ggBrakeProfileR.SubModel != nullptr) {
        ggBrakeProfileR.PutValue(
            (mvOccupied->BrakeDelayFlag & bdelay_R) != 0 ? 1.f : 0.f);
    }
    ...
    ggWaterPumpBreakerButton.PutValue(
        mvControlled->WaterPump.breaker ? 1.f : 0.f);
    if (ggWaterPumpButton.type() != TGaugeType::push) {
        ggWaterPumpButton.PutValue(
            mvControlled->WaterPump.is_enabled ? 1.f : 0.f);
    }
    // water heater
    ggWaterHeaterBreakerButton.PutValue(
        mvControlled->WaterHeater.breaker ? 1.f : 0.f);
    ggWaterHeaterButton.PutValue(
        mvControlled->WaterHeater.is_enabled ? 1.f : 0.f);
    ggWaterCircuitsLinkButton.PutValue(
        mvControlled->WaterCircuitsLink ? 1.f : 0.f);
    // fuel pump
    if (ggFuelPumpButton.type() != TGaugeType::push) {
        ggFuelPumpButton.PutValue(
            mvControlled->FuelPump.is_enabled ? 1.f : 0.f);
    }
    // oil pump
    ...
    if (ggMotorBlowersFrontButton.type() != TGaugeType::push) {
        ggMotorBlowersFrontButton.PutValue(
            mvControlled->MotorBlowers[end::front].is_enabled ? 1.f : 0.f);
    }
    if (ggMotorBlowersRearButton.type() != TGaugeType::push) {
        ggMotorBlowersRearButton.PutValue(
            mvControlled->MotorBlowers[end::rear].is_enabled ? 1.f : 0.f);
    }
    ...
    if (ggScndCtrl.is_push()) {
        ggScndCtrl.PutValue(
            ggScndCtrl.is_toggle() ? 0.5f
                                   :  // pushtoggle is two-way control with
                                      // neutral position in the middle
                0.f);  // push is on/off control, active while held down,
                       // due to legacy use
    }
    // tempomat
    ...
    // finding each inverter - not so optimal, but action ins performed only
    // during changing cabin
    bool kier = (DynamicObject->DirectionGet() * mvOccupied->CabOccupied > 0);
    ...
    for (auto &item : ggInverterToggleButtons)  // for each button
    {
        ...
        itemstart++;
        TDynamicObject *p = DynamicObject->GetFirstDynamic(
            mvOccupied->CabOccupied < 0 ? end::rear : end::front, flag);
        while (p) {
            if (p->MoverParameters->eimc[eimc_p_Pmax] > 1) {
                if (itemindex < p->MoverParameters->InvertersNo) {
                    // visual feedback
                    ggInverterToggleButtons[itemstart - 1].PutValue(
                        p->MoverParameters->Inverters[itemindex].Activate
                            ? 1.0
                            : 0.0);
                    break;
    ...
    // TODO: when cleaning up break setting indicator state into a separate
    // function, so we can reuse it
}

// initializes a button matching provided label. returns: true if the label
// was found, false otherwise
// TODO: refactor the cabin controls into some sensible structure
bool TTrain::initialize_button(
    cParser &Parser, std::string const &Label, int const Cabindex) {
    ...
    };
    {
        ...
        if (lookup != autolights.end()) {
            auto &button =
                Cabine[Cabindex].Button(-1);  // pierwsza wolna lampka
...
// initializes a gauge matching provided label. returns: true if the label
// was found, false otherwise
// TODO: refactor the cabin controls into some sensible structure
bool TTrain::initialize_gauge(
    cParser &Parser, std::string const &Label, int const Cabindex) {
    ...
    // TODO: convert dedicated gauges to auto-allocated ones, replace
    // dedicated references in command handlers to mapper lookups
    std::unordered_map<
        std::string, std::tuple<TGauge &, bool const *>> const stategauges = {
        {"tempomat_sw:",
         {ggScndCtrlButton, &mvOccupied->SpeedCtrlUnit.IsActive}},
        {"tempomatoff_sw:",
         {ggScndCtrlOffButton, &mvOccupied->SpeedCtrlUnit.IsActive}},
        {"speedinc_bt:",
         {ggSpeedControlIncreaseButton, &mvOccupied->SpeedCtrlUnit.IsActive}},
        {"speeddec_bt:",
         {ggSpeedControlDecreaseButton, &mvOccupied->SpeedCtrlUnit.IsActive}},
        {"speedctrlpowerinc_bt:",
         {ggSpeedControlPowerIncreaseButton,
          &mvOccupied->SpeedCtrlUnit.IsActive}},
        {"speedctrlpowerdec_bt:",
         {ggSpeedControlPowerDecreaseButton,
          &mvOccupied->SpeedCtrlUnit.IsActive}},
        {"speedbutton0:",
         {ggSpeedCtrlButtons[0], &mvOccupied->SpeedCtrlUnit.IsActive}},
        {"speedbutton1:",
         {ggSpeedCtrlButtons[1], &mvOccupied->SpeedCtrlUnit.IsActive}},
        {"speedbutton2:",
         {ggSpeedCtrlButtons[2], &mvOccupied->SpeedCtrlUnit.IsActive}},
        {"speedbutton3:",
         {ggSpeedCtrlButtons[3], &mvOccupied->SpeedCtrlUnit.IsActive}},
        {"speedbutton4:",
         {ggSpeedCtrlButtons[4], &mvOccupied->SpeedCtrlUnit.IsActive}},
        {"speedbutton5:",
         {ggSpeedCtrlButtons[5], &mvOccupied->SpeedCtrlUnit.IsActive}},
        {"speedbutton6:",
         {ggSpeedCtrlButtons[6], &mvOccupied->SpeedCtrlUnit.IsActive}},
        {"speedbutton7:",
         {ggSpeedCtrlButtons[7], &mvOccupied->SpeedCtrlUnit.IsActive}},
        {"speedbutton8:",
         {ggSpeedCtrlButtons[8], &mvOccupied->SpeedCtrlUnit.IsActive}},
        {"speedbutton9:",
         {ggSpeedCtrlButtons[9], &mvOccupied->SpeedCtrlUnit.IsActive}},
    ...
    std::unordered_map<std::string, bool *> const autoboolgauges = {
        ...
        {"pantfront_sw:",
         &mvPantographUnit->Pantographs[end::front].valve.is_enabled},
        {"pantrear_sw:",
         &mvPantographUnit->Pantographs[end::rear].valve.is_enabled},
        {"pantfrontoff_sw:",
         &mvPantographUnit->Pantographs[end::front].valve.is_disabled},
        {"pantrearoff_sw:",
         &mvPantographUnit->Pantographs[end::rear].valve.is_disabled},
    ...
    //*************************************************************
        ...
        if (Parser.getToken<std::string>() == "analog") {
            if (DynamicObject->mdKabina) {
                // McZapkie-300302: zegarek
                ggClockSInd.Init(
                    DynamicObject->mdKabina->GetFromName("ClockShand"), nullptr,
                    TGaugeAnimation::gt_Rotate, 1.0 / 60.0);
                ggClockMInd.Init(
                    DynamicObject->mdKabina->GetFromName("ClockMhand"), nullptr,
                    TGaugeAnimation::gt_Rotate, 1.0 / 60.0);
                ggClockHInd.Init(
                    DynamicObject->mdKabina->GetFromName("ClockHhand"), nullptr,
                    TGaugeAnimation::gt_Rotate, 1.0 / 12.0);
            }
        }
        ...
        if (mvControlled->HeatingPowerSource.SourceType ==
            TPowerSource::Generator) {
            auto &gauge = Cabine[Cabindex].Gauge(-1);  // pierwsza wolna gałka
            gauge.Load(Parser, DynamicObject);
            gauge.AssignDouble(
                &(mvControlled->HeatingPowerSource.EngineGenerator.voltage));
        }
...
uint16_t TTrain::id() {
    if (vid == 0) {
        ...
        WriteLog(
            "net: assigning id " + std::to_string(vid) + " to vehicle " +
                Dynamic()->name(),
            logtype::net);
    }
    return vid;
...
void train_table::update(double dt) {
    for (TTrain *train : m_items) {
        ...
        if (train->pending_delete) {
            ...
            if (simulation::Train == train) simulation::Train = nullptr;
        }

        // for single-player destroy non-player trains
        if (simulation::Train != train && Global.network_servers.empty() &&
            !Global.network_client) {
            purge(train->Dynamic()->name());
        }
    }
