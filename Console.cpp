...
radia, ponieważ nie ma potrzeby ich uzależniać (tzn. badać wspólną maskę
bitową).
...
skryptu. Każdy pojazd może mieć inny schemat wejść i masek, ale w miarę
możliwości należy dążyć do unifikacji. Skrypty mogą również używać dodatkowych
masek bitowych. Maski bitowe odpowiadają stanom przełączników, czujników,
styczników itd.
...
int Console::iBits =
    0;  // zmienna statyczna - obiekt Console jest jednen wspólny
int Console::iMode = 0;
int Console::iConfig = 0;
TPoKeys55 *Console::PoKeys55[2] = {NULL, NULL};
TLPT *Console::LPT = NULL;
int Console::iSwitch[8];  // bistabilne w kabinie, załączane z [Shift],
                          // wyłączane bez
int Console::iButton[8];  // monostabilne w kabinie, załączane podczas trzymania
                          // klawisza
...
Console::~Console() { Console::Off(); };

void Console::ModeSet(int m, int h) {  // ustawienie trybu pracy
    iMode = m;
    iConfig = h;
};

int Console::On() {  // załączenie konsoli (np. nawiązanie komunikacji)
    iSwitch[0] = iSwitch[1] = iSwitch[2] = iSwitch[3] =
        0;  // bity 0..127 - bez [Ctrl]
    iSwitch[4] = iSwitch[5] = iSwitch[6] = iSwitch[7] =
        0;  // bity 128..255 - z [Ctrl]
    switch (iMode) {
        ...
        case 3:                                         // LPT
            ...
            if (LPT ? LPT->Connect(iConfig) : false) {  // wysłać 0?
                BitsUpdate(-1);  // aktualizacjia stanów, bo przy wczytywaniu
                                 // mogło być nieaktywne
...
void Console::Off() {  // wyłączenie informacji zwrotnych (reset pulpitu)
    BitsClear(-1);
    if ((iMode == 1) || (iMode == 2))
...
void Console::BitsSet(
    int mask,
    int entry) {  // ustawienie bitów o podanej masce (mask) na wejściu (entry)
    if ((iBits & mask) != mask)  // jeżeli zmiana
    {
        ...
        if (iMode == 4)
            WriteLog(
                "PoKeys::BitsSet: mask: " + std::to_string(mask) +
                " iBits: " + std::to_string(iBits));
    }
};

void Console::BitsClear(
    int mask,
    int entry) {  // zerowanie bitów o podanej masce (mask) na wejściu (entry)
...
void Console::BitsUpdate(
    int mask) {  // aktualizacja stanu interfejsu informacji zwrotnej; (mask) -
                 // zakres zmienianych bitów
    switch (iMode) {
        ...
        case 2:  // sterowanie światełkami klawiatury: CA+SHP
            ...
            break;
        case 3:  // LPT Marcela z modyfikacją (jazda na oporach zamiast
                 // brzęczyka)
            if (LPT) LPT->Out(iBits);
            break;
        case 4:  // PoKeys55 wg Marcela - wersja druga z końca 2012
            if (PoKeys55[0]) {  // pewnie trzeba będzie to dodatkowo buforować i
                ...
                if (mask & 0x0010)  // b4 Lampka przekaźnika nadmiarowego
                ...
                if (mask &
                    0x0100)  // b8 Kontrolka przekaźnika nadmiarowego sprężarki
                    PoKeys55[0]->Write(0x40, 33 - 1, (iBits & 0x0100) ? 1 : 0);
                if (mask &
                    0x0200)  // b9 Kontrolka sygnalizacji wentylatorów i oporów
                ...
                if (mask & 0x1000)  // b12 Ciśnienie w cylindrach do odbijania w
                                    // haslerze
                    PoKeys55[0]->Write(0x40, 52 - 1, (iBits & 0x1000) ? 1 : 0);
                if (mask &
                    0x2000)  // b13 Prąd na silnikach do odbijania w haslerze
...
void Console::ValueSet(
    int x, double y) {  // ustawienie wartości (y) na kanale analogowym (x)
    if (iMode != 4) {
        return;
    }
    ...
    if (PoKeys55[0]) {
        ...
        double temp =
            (((((Global.fCalibrateOut[x][5] * y) + Global.fCalibrateOut[x][4]) *
                   y +
...
void Console::Update() {  // funkcja powinna być wywoływana regularnie, np. raz
                          // w każdej ramce ekranowej
    if (iMode == 4)
        if (PoKeys55[0])
            if (PoKeys55[0]->Update(
                    (Global.iPause & 8) >
                    0)) {  // wykrycie przestawionych przełączników?
                Global.iPause &= ~8;
...
float Console::AnalogGet(int x) {  // pobranie wartości analogowej
    if (iMode == 4)
        if (PoKeys55[0]) return PoKeys55[0]->fAnalog[x];
    return -1.0;
};

float Console::AnalogCalibrateGet(
    int x) {  // pobranie i kalibracja wartości analogowej, jeśli nie ma PoKeys
              // zwraca NULL
    if (iMode == 4 && PoKeys55[0]) {
        float b = PoKeys55[0]->fAnalog[x];
        b = (((((Global.fCalibrateIn[x][5] * b) + Global.fCalibrateIn[x][4]) *
                   b +
...
unsigned char Console::DigitalGet(int x) {  // pobranie wartości cyfrowej
    if (iMode == 4)
        if (PoKeys55[0]) return PoKeys55[0]->iInputs[x];
    return 0;
};

void Console::OnKeyDown(
    int k) {          // naciśnięcie klawisza z powoduje wyłączenie, a
    if (k & 0x10000)  // jeśli [Shift]
    {                 // ustawienie bitu w tabeli przełączników bistabilnych
        if (k & 0x20000)  // jeśli [Ctrl], to zestaw dodatkowy
            iSwitch[4 + (char(k) >> 5)] |=
                1 << (k & 31);  // załącz bistabliny dodatkowy
        else {  // z [Shift] włączenie bitu bistabilnego i dodatkowego
                // monostabilnego
            iSwitch[char(k) >> 5] |=
                1 << (k & 31);  // załącz bistabliny podstawowy
            iButton[4 + (char(k) >> 5)] |=
                (1 << (k & 31));  // załącz monostabilny dodatkowy
        }
    } else {  // zerowanie bitu w tabeli przełączników bistabilnych
        if (k & 0x20000)  // jeśli [Ctrl], to zestaw dodatkowy
            iSwitch[4 + (char(k) >> 5)] &=
                ~(1 << (k & 31));  // wyłącz bistabilny dodatkowy
        else {
            iSwitch[char(k) >> 5] &=
                ~(1 << (k & 31));  // wyłącz bistabilny podstawowy
            iButton[char(k) >> 5] |=
                1 << (k & 31);  // załącz monostabilny podstawowy
        }
    }
};

void Console::OnKeyUp(int k) {  // puszczenie klawisza w zasadzie nie ma
                                // znaczenia dla iSwitch, ale zeruje iButton
    if ((k & 0x20000) == 0)     // monostabilne tylko bez [Ctrl]
        if (k & 0x10000)        // jeśli [Shift]
            iButton[4 + (char(k) >> 5)] &=
                ~(1 << (k & 31));  // wyłącz monostabilny dodatkowy
        else
            iButton[char(k) >> 5] &=
                ~(1 << (k & 31));  // wyłącz monostabilny podstawowy
};
