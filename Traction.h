...
class TTraction : public scene::basic_node {
   ...
   public:  // na razie
    TTractionPowerSource *psPower[2]{
        nullptr, nullptr};  // najbliższe zasilacze z obu kierunków
    TTractionPowerSource *psPowered{
        nullptr};  // ustawione tylko dla bezpośrednio zasilanego przęsła
    TTraction *hvNext[2]{nullptr, nullptr};  //łączenie drutów w sieć
    int iNext[2]{0, 0};                      // do którego końca się łączy
    int iLast{0};  //że niby ostatni drut // ustawiony bit 0, jeśli jest
                   //ostatnim drutem w sekcji; bit1 - przedostatni
   public:
    ...
    TTractionPowerSource *psSection{
        nullptr};  // zasilacz (opcjonalnie może to być pulpit sterujący EL2 w
                   // hali!)
    std::string asParallel;  // nazwa przęsła, z którym może być bieżnia wspólna
    TTraction *hvParallel{nullptr};  // jednokierunkowa i zapętlona lista
                                     // przęseł ewentualnej bieżni wspólnej
    float fResistance[2]{
        -1.0f, -1.0f};  // rezystancja zastępcza do punktu zasilania (0: przęsło
                        // zasilane, <0: do policzenia)
    int iTries{
        1};  // 0 is used later down the road to mark directly powered pieces
    ...
    // creates geometry data in specified geometry bank. returns: number of
    // created elements, or NULL NOTE: deleting nodes doesn't currently release
    // geometry data owned by the node. TODO: implement erasing individual
    // geometry chunks and banks
    ...
    bool WhereIs();
    void ResistanceCalc(
        int d = -1, double r = 0, TTractionPowerSource *ps = nullptr);
