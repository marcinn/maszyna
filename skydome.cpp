...
float CSkyDome::m_distributionluminance[5][2] = {
    // Perez distributions
...
CSkyDome::CSkyDome(int const Tesselation) : m_tesselation(Tesselation) {
...
void CSkyDome::Generate() {
    ...
    float const offset =
        0.1f * radius;  // horizontal offset, a cheap way to prevent a gap
    ...
    for (int i = 0; i <= latitudes; ++i) {
        float const latitude =
            M_PI * (-0.5f + (float)(i) / latitudes / 2);  // half-sphere only
        ...
        for (int j = 0; j <= longitudes; ++j) {
            ...
            /*
                        m_vertices.emplace_back( float3( x * zr, y * zr -
            offset, z ) * radius );
                        // we aren't using normals, but the code is left here in
            ...
            */
            ...
            m_vertices.emplace_back(
                glm::vec3(-x * zr, -z - offset, -y * zr) * radius);
            m_colours.emplace_back(
                glm::vec3(0.75f, 0.75f, 0.75f));  // placeholder

            if ((i == 0) || (j == 0)) {
                // initial edge of the dome, don't start indices yet
                ++index;
            } else {
                // indices for two triangles, formed between current and
                // previous latitude
...
void CSkyDome::SetExposure(bool const Linearexposure, float const Expfactor) {
    ...
    m_expfactor =
        1.0f / clamp(Expfactor, 1.0f, std::numeric_limits<float>::infinity());
}

void CSkyDome::SetOvercastFactor(float const Overcast) {
    m_overcast = clamp(Overcast, 0.0f, 1.0f) *
                 0.75f;  // going above 0.65 makes the model go pretty bad,
                         // appearance-wise
}

void CSkyDome::GetPerez(
...
float CSkyDome::GetZenith(
    float Zenithmatrix[3][4], const float Theta, const float Turbidity) {
...
float CSkyDome::PerezFunctionO1(
    float Perezcoeffs[5], const float Thetasun, const float Zenithval) {
    const float val =
        (1.0f + Perezcoeffs[0] * std::exp(Perezcoeffs[1])) *
        (1.0f + Perezcoeffs[2] * std::exp(Perezcoeffs[3] * Thetasun) +
         Perezcoeffs[4] * std::pow(std::cos(Thetasun), 2));

    return Zenithval / val;
}

float CSkyDome::PerezFunctionO2(
    float Perezcoeffs[5], const float Icostheta, const float Gamma,
    const float Cosgamma2, const float Zenithval) {
    // iCosTheta = 1.0f / cosf(theta)
    // cosGamma2 = SQR( cosf( gamma ) )
    return Zenithval *
           (1.0f + Perezcoeffs[0] * std::exp(Perezcoeffs[1] * Icostheta)) *
           (1.0f + Perezcoeffs[2] * std::exp(Perezcoeffs[3] * Gamma) +
            Perezcoeffs[4] * Cosgamma2);
}

void CSkyDome::RebuildColors() {
    float twilightfactor =
        clamp(-simulation::Environment.sun().getAngle(), 0.0f, 18.0f) / 18.0f;
    auto gammacorrection =
        interpolate(glm::vec3(0.45f), glm::vec3(1.0f), twilightfactor);

    // get zenith luminance
    float const chi =
        ((4.0f / 9.0f) - (m_turbidity / 120.0f)) * (M_PI - (2.0f * m_thetasun));
    float zenithluminance =
        ((4.0453f * m_turbidity) - 4.9710f) * std::tan(chi) -
    ...
    zenithluminance =
        PerezFunctionO1(perezluminance, m_thetasun, zenithluminance);
    ...
    for (unsigned int i = 0; i < m_vertices.size(); ++i) {
        ...
        // warning : major hack!!! .. i had to do something with values under
        // horizon
        ...
        // Compute x,y values
        float const x =
            PerezFunctionO2(perezx, icostheta, gamma, cosgamma2, zenithx);
        float const y =
            PerezFunctionO2(perezy, icostheta, gamma, cosgamma2, zenithy);

        // luminance(Y) for clear & overcast sky
        float const yclear = std::max(
            0.01f,
            PerezFunctionO2(
                perezluminance, icostheta, gamma, cosgamma2, zenithluminance));
        float const yover =
            std::max(0.01f, zenithluminance * (1.0f + 2.0f * vertex.y) / 3.0f);
        ...
        colorconverter.y = clamp(
            colorconverter.y * Global.m_skysaturationcorrection, 0.0f, 1.0f);
        // desaturate sky colour, based on overcast level
        if (colorconverter.y > 0.0f) {
            colorconverter.y *= (1.0f - 0.5f * m_overcast);
        }

        // override the hue, based on sun height above the horizon. crude way to
        // deal with model shortcomings correction begins when the sun is higher
        // than 10 degrees above the horizon, and fully in effect at 10+15
        // degrees
        float const degreesabovehorizon = 90.0f - m_thetasun * (180.0f / M_PI);
        auto const sunbasedphase =
            clamp((1.0f / 15.0f) * (degreesabovehorizon - 10.0f), 0.0f, 1.0f);
        // correction is applied in linear manner from the bottom, becomes fully
        // in effect for vertices with y = 0.50
        auto const heightbasedphase = clamp(vertex.y * 2.0f, 0.0f, 1.0f);
        // this height-based factor is reduced the farther the sun is up in the
        // sky
        float const shiftfactor = clamp(
            interpolate(heightbasedphase, sunbasedphase, sunbasedphase), 0.0f,
            1.0f);
        ...
        color = interpolate(
            color, shiftedcolor, shiftfactor * Global.m_skyhuecorrection);
        ...
        if ((color.x <= 0.05f) && (color.y <= 0.05f)) {
            // darken the sky as the sun goes deeper below the horizon
            // 15:50:75 is picture-based night sky colour. it may not be
            // accurate but looks 'right enough'
    ...
    m_averagecolour = glm::max(
        glm::vec3(), averagecolor / static_cast<float>(m_vertices.size()));
    m_averagehorizoncolour = glm::max(
        glm::vec3(),
        averagehorizoncolor / static_cast<float>(m_tesselation * 3 + 3));
