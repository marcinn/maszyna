...
void basic_vertex::serialize(std::ostream &s, bool const Tangent) const {
...
void basic_vertex::deserialize(std::istream &s, bool const Tangent) {
...
void basic_vertex::serialize_packed(std::ostream &s, bool const Tangent) const {
...
void basic_vertex::deserialize_packed(std::istream &s, bool const Tangent) {
...
void calculate_tangents(
    ...
    if (!vertex_count || vertices[0].tangent.w != 0.0f) return;
    ...
    for (size_t a = 0; a < triangle_count; a++) {
        ...
        if (ri == 0.0f) {
            // ErrorLog("Bad model: failed to generate tangent vectors for
            // vertices: " +
...
void calculate_indices(
    index_array &Indices, vertex_array &Vertices, float tolerancescale) {
    Indices.resize(Vertices.size());
    std::iota(std::begin(Indices), std::end(Indices), 0);
    // gather instances of used vertices, replace the original vertex bank with
    // it after you're done
    vertex_array indexedvertices;
    indexedvertices.reserve(std::max<size_t>(
        100, Vertices.size() / 3));  // optimistic guesstimate, but should
                                     // reduce re-allocation somewhat
    auto const matchtolerance{1e-5f * tolerancescale};
    for (auto idx = 0; idx < Indices.size(); ++idx) {
        if (Indices[idx] < idx) {
            // this index is pointing to a vertex out of linear order, i.e. it's
            // an already processed duplicate we can skip
            continue;
        }
        // due to duplicate removal our vertex will likely have different index
        // in the processed set
        Indices[idx] = indexedvertices.size();
        // see if there's any pointers in the remaining index subrange to
        // similar enough vertices if found, remap these to use our current
        // vertex instead
        auto vertex{Vertices[idx]};
        auto matchiter{std::cbegin(Vertices) + idx};
        for (auto matchidx = idx + 1;
             matchidx < Indices.size() &&
             matchidx < idx + Global.iConvertIndexRange;
             ++matchidx) {
            ++matchiter;
            if ((glm::all(glm::epsilonEqual(
                    vertex.position, matchiter->position, matchtolerance))) &&
                (glm::all(glm::epsilonEqual(
                    vertex.normal, matchiter->normal, matchtolerance))) &&
                (glm::all(glm::epsilonEqual(
                    vertex.texture, matchiter->texture, matchtolerance)))) {
...
// creates a new geometry chunk of specified type from supplied data. returns:
// handle to the chunk or NULL
gfx::geometry_handle geometry_bank::create(
    gfx::vertex_array &Vertices, unsigned int const Type) {
    if (true == Vertices.empty()) {
        return {0, 0};
    }

    m_chunks.emplace_back(Vertices, Type);
    // NOTE: handle is effectively (index into chunk array + 1) this leaves
    // value of 0 to serve as error/empty handle indication
    gfx::geometry_handle chunkhandle{
        0, static_cast<std::uint32_t>(m_chunks.size())};
...
// creates a new indexed geometry chunk of specified type from supplied data.
...
// specified offset
bool geometry_bank::replace(
    gfx::vertex_array &Vertices, gfx::geometry_handle const &Geometry,
    std::size_t const Offset) {
    if ((Geometry.chunk == 0) || (Geometry.chunk > m_chunks.size())) {
        return false;
    }

    auto &chunk = gfx::geometry_bank::chunk(Geometry);

    if ((Offset == 0) && (Vertices.size() == chunk.vertices.size())) {
        // check first if we can get away with a simple swap...
        chunk.vertices.swap(Vertices);
    } else {
        // ...otherwise we need to do some legwork
        // NOTE: if the offset is larger than existing size of the chunk, it'll
        // bridge the gap with 'blank' vertices TBD: we could bail out with an
        // error instead if such request occurs
        chunk.vertices.resize(Offset + Vertices.size(), gfx::basic_vertex());
        chunk.vertices.insert(
            std::end(chunk.vertices), std::begin(Vertices), std::end(Vertices));
    }
    // template method implementation
    replace_(Geometry);
    // all done
...
bool geometry_bank::append(
    ...
    return replace(
        Vertices, Geometry,
        gfx::geometry_bank::chunk(Geometry).vertices.size());
}

// draws geometry stored in specified chunk
std::size_t geometry_bank::draw(
...
// frees subclass-specific resources associated with the bank, typically called
// when the bank wasn't in use for a period of time
void geometry_bank::release() {
    // template method implementation
    release_();
}

// provides direct access to indexdata of specfied chunk
index_array const &geometry_bank::indices(
    gfx::geometry_handle const &Geometry) const {
    return geometry_bank::chunk(Geometry).indices;
}

// provides direct access to vertex data of specfied chunk
vertex_array const &geometry_bank::vertices(
    gfx::geometry_handle const &Geometry) const {
...
void geometrybank_manager::update() { m_garbagecollector.sweep(); }

// creates a new geometry bank. returns: handle to the bank or NULL
gfx::geometrybank_handle geometrybank_manager::register_bank(
    std::unique_ptr<geometry_bank> bank) {
    m_geometrybanks.emplace_back(
        std::move(bank), std::chrono::steady_clock::time_point());
...
gfx::geometry_handle geometrybank_manager::create_chunk(
    vertex_array &Vertices, gfx::geometrybank_handle const &Geometry,
    int const Type) {
    auto const newchunkhandle = bank(Geometry).first->create(Vertices, Type);

    if (newchunkhandle.chunk != 0) {
        return {Geometry.bank, newchunkhandle.chunk};
    } else {
        return {0, 0};
    }
}
...
gfx::geometry_handle geometrybank_manager::create_chunk(
    ...
    auto const newchunkhandle =
        bank(Geometry).first->create(Indices, Vertices, Type);
...
// replaces data of specified chunk with the supplied vertex data, starting from
// specified offset
bool geometrybank_manager::replace(
    gfx::vertex_array &Vertices, gfx::geometry_handle const &Geometry,
    std::size_t const Offset) {
...
// adds supplied vertex data at the end of specified chunk
bool geometrybank_manager::append(
    gfx::vertex_array &Vertices, gfx::geometry_handle const &Geometry) {
    return bank(Geometry).first->append(Vertices, Geometry);
}
// draws geometry stored in specified chunk
void geometrybank_manager::draw(
    gfx::geometry_handle const &Geometry, unsigned int const Streams) {
...
gfx::index_array const &geometrybank_manager::indices(
    gfx::geometry_handle const &Geometry) const {
    return bank(Geometry).first->indices(Geometry);
}

// provides direct access to vertex data of specfied chunk
gfx::vertex_array const &geometrybank_manager::vertices(
    gfx::geometry_handle const &Geometry) const {
