...
class TTrackFollower {
   public:
    ...
    inline TTrack *GetTrack() const { return pCurrentTrack; };
    // przechyłka policzona przy ustalaniu pozycji
    inline double GetRoll() { return vAngles.x; };
    //{return pCurrentSegment->GetRoll(fCurrentDistance)*fDirection;}; //zamiast
    //liczyć można pobrać
    // zwrot na torze
    inline double GetDirection() const { return fDirection; };
    // ABu-030403
    inline double GetTranslation() const { return fCurrentDistance; };
    ...
    // members
    double fOffsetH = 0.0;      // Ra: odległość środka osi od osi toru (dla
                                // samochodów) - użyć do wężykowania
    Math3D::vector3 pPosition;  // współrzędne XYZ w układzie scenerii
    Math3D::vector3 vAngles;  // x:przechyłka, y:pochylenie, z:kierunek w planie
                              // (w radianach)
   private:
    // methods
    ...
    std::shared_ptr<TSegment>
        pCurrentSegment;  // zwrotnice mog¹ mieæ dwa segmenty
    double fCurrentDistance =
        0.0;                  // przesuniêcie wzglêdem Point1 w stronê Point2
    double fDirection = 1.0;  // ustawienie wzglêdem toru: -1.0 albo 1.0,
                              // mno¿one przez dystans // jest przodem do Point2
    TDynamicObject *Owner = nullptr;  // pojazd posiadający
    int iEventFlag =
        0;  // McZapkie-020602: informacja o tym czy wyzwalac zdarzenie: 0,1,2,3
    int iEventallFlag = 0;
    int iSegment = 0;  // który segment toru jest używany (żeby nie
                       // przeskakiwało po przestawieniu zwrotnicy pod taborem)
};
//---------------------------------------------------------------------------
#endif
