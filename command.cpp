...
void command_queue::update() {
    ...
    for (auto c : m_active_continuous) {
        command_data data(
            {c.first, GLFW_REPEAT, 0.0, 0.0, delta, false,
             glm::vec3()});  // todo: improve
        auto lookup = m_commands.emplace(c.second, commanddata_sequence());
        // recipient stack was either located or created, so we can add to it
        // quite safely
...
void command_queue::push(
    command_data const &Command, uint32_t const Recipient) {
    if (is_network_target(Recipient)) {
        auto lookup =
            m_intercept_queue.emplace(Recipient, commanddata_sequence());
...
void command_queue::push_direct(
    const command_data &Command, const uint32_t Recipient) {
    auto const &desc =
        simulation::Commands_descriptions[static_cast<std::size_t>(
            Command.command)];
    if (desc.mode == command_mode::continuous) {
        if (Command.action == GLFW_PRESS)
            m_active_continuous.emplace(
                std::make_pair(Command.command, Recipient));
        else if (Command.action == GLFW_RELEASE)
            m_active_continuous.erase(
                std::make_pair(Command.command, Recipient));
    ...
    // recipient stack was either located or created, so we can add to it quite
    // safely
    lookup.first->second.emplace_back(Command);
}
...
bool command_queue::pop(command_data &Command, uint32_t const Recipient) {
    ...
    // we have command stack with command(s) on it, retrieve and pop the first
    // one
...
bool command_queue::is_network_target(uint32_t const Recipient) {
    ...
    if (target == command_target::entity) return false;
...
void command_queue::push_commands(const commands_map &commands) {
    for (auto const &kv : commands)
        for (command_data const &data : kv.second) push_direct(data, kv.first);
}

void command_relay::post(
    user_command const Command, double const Param1, double const Param2,
    int const Action, uint16_t Recipient, glm::vec3 Position,
    const std::string *Payload) const {
    auto const &command =
        simulation::Commands_descriptions[static_cast<std::size_t>(Command)];

    if (command.target == command_target::vehicle && Recipient == 0) {
        // default 0 recipient is currently controlled train
        if (simulation::Train == nullptr) return;
    ...
    if ((command.target == command_target::vehicle) &&
        ...
        // in realistic control mode don't pass vehicle commands if the user
        // isn't in one, unless we're in debug mode
        return;
    }

    if (Position == glm::vec3(0.0f)) Position = Global.pCamera.Pos;

    uint32_t combined_recipient =
        static_cast<uint32_t>(command.target) | Recipient;
    command_data commanddata(
        {Command, Action, Param1, Param2, Timer::GetDeltaTime(),
         FreeFlyModeFlag, Position});
