...
class TTractionPowerSource : public scene::basic_node {
   ...
   public:
    ...
    void VoltageSet(double const v) { NominalVoltage = v; };
    void PowerSet(TTractionPowerSource *ps);
    bool Fuse() const { return (FastFuse || SlowFuse); }
    // members
    TTractionPowerSource *psNode[2] = {
        nullptr, nullptr};  // zasilanie na końcach dla sekcji
    bool bSection = false;  // czy jest sekcją
    bool IsAutogenerated{
        false};  // indicates autogenerated source for individual traction piece
