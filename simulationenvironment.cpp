...
void world_environment::on_daylight_change() {
    if (Global.FakeLight) {
...
void world_environment::compute_season(int const Yearday) {
    ...
    // season can affect the weather so if it changes, re-calculate weather as
    // well
    compute_weather();
}

// calculates current weather
void world_environment::compute_weather() {
    Global.Weather =
        (Global.Overcast <= 0.10   ? "clear:"
         : Global.Overcast <= 0.50 ? "scattered:"
         : Global.Overcast <= 0.90 ? "broken:"
         : Global.Overcast <= 1.00
             ? "overcast:"
             : (Global.Season != "winter:" ? "rain:" : "snow:"));
}
...
void world_environment::init() {
    ...
    {
        auto const rainsoundoverride{
            simulation::Sound_overrides.find("weather.rainsound:")};
    ...
    m_wind = basic_wind{
        static_cast<float>(Random(0, 360)), static_cast<float>(Random(-5, 5)),
        static_cast<float>(Random(-2, 4)),  static_cast<float>(Random(-1, 1)),
...
void world_environment::update() {
    ...
    // diffuse (sun) intensity goes down after twilight, and reaches minimum 18
    // degrees below horizon
    float twilightfactor = clamp(-m_sun.getAngle(), 0.0f, 18.0f) / 18.0f;
    // NOTE: sun light receives extra padding to prevent moon from kicking in
    // too soon
    auto const sunlightlevel =
        m_sun.getIntensity() + 0.05f * (1.f - twilightfactor);
    auto const moonlightlevel =
        m_moon.getIntensity() *
        0.65f;  // scaled down by arbitrary factor, it's pretty bright otherwise

    // ...update skydome to match the current sun position as well...
    // twilight factor can be reset later down, so we do it here while it's
    // still reflecting state of the sun turbidity varies from 2-3 during the
    // day based on overcast, 3-4 after sunset to deal with sunlight bleeding
    // too much into the sky from below horizon
    m_skydome.SetTurbidity(
        2.25f + clamp(Global.Overcast, 0.f, 1.f) +
    ...
    if (moonlightlevel > sunlightlevel) {
        // rare situations when the moon is brighter than the sun, typically at
        // night
        ...
        twilightfactor = 0.0f;
        keylightcolor =
            glm::vec3(255.0f / 255.0f, 242.0f / 255.0f, 202.0f / 255.0f);
    } else {
        ...
        float const duskfactor =
            1.0f - clamp(Global.SunAngle, 0.0f, 18.0f) / 18.0f;
    ...
    // intensity combines intensity of the sun and the light reflected by the
    // sky dome it'd be more technically correct to have just the intensity of
    // the sun here, but whether it'd _look_ better is something to be tested
    auto const intensity =
        std::min(1.15f * (0.05f + keylightintensity + skydomehsv.z), 1.25f);
    // the impact of sun component is reduced proportionally to overcast level,
    // as overcast increases role of ambient light
    auto const diffuselevel = interpolate(
        keylightintensity, intensity * (1.0f - twilightfactor),
        1.0f - std::min(1.f, Global.Overcast) * 0.75f);
    // ...update light colours and intensity.
    keylightcolor = keylightcolor * diffuselevel;
    Global.DayLight.diffuse =
        glm::vec4(keylightcolor, Global.DayLight.diffuse.a);
    Global.DayLight.specular = glm::vec4(keylightcolor * 0.85f, diffuselevel);

    // tonal impact of skydome color is inversely proportional to how high the
    // sun is above the horizon (this is pure conjecture, aimed more to 'look
    // right' than be accurate)
    float const ambienttone =
        clamp(1.0f - (Global.SunAngle / 90.0f), 0.0f, 1.0f);
    float const ambientintensitynightfactor =
        1.f - 0.75f * clamp(-m_sun.getAngle(), 0.0f, 18.0f) / 18.0f;
    Global.DayLight.ambient[0] =
        interpolate(skydomehsv.z, skydomecolour.r, ambienttone) *
        ambientintensitynightfactor;
    Global.DayLight.ambient[1] =
        interpolate(skydomehsv.z, skydomecolour.g, ambienttone) *
        ambientintensitynightfactor;
    Global.DayLight.ambient[2] =
        interpolate(skydomehsv.z, skydomecolour.b, ambienttone) *
        ambientintensitynightfactor;

    Global.fLuminance = intensity;

    // update the fog. setting it to match the average colour of the sky dome is
    // cheap but quite effective way to make the distant items blend with
    // background better
    Global.FogColor = interpolate(
                          m_skydome.GetAverageColor(),
                          m_skydome.GetAverageHorizonColor(), 0.25f) *
...
void world_environment::update_precipitation() { m_precipitation.update(); }

void world_environment::update_moon() { m_moon.update(true); }

void world_environment::update_wind() {
    ...
    if (m_wind.change_time < 0) {
        ...
        if (Random() < 0.05) {
            // changes in wind direction should be less frequent than changes in
            // wind speed TBD, TODO: configuration-driven direction change
            // frequency
            m_wind.azimuth_change = Random(-5, 5);
        } else {
            // keep direction change periods short, to avoid too drastic changes
            // in direction
    ...
    m_wind.azimuth =
        clamp_circular(m_wind.azimuth + m_wind.azimuth_change * timedelta);
    // HACK: negative part of range allows for some quiet periods, without
    // active wind
    m_wind.velocity = clamp<float>(
        m_wind.velocity + m_wind.velocity_change * timedelta, -2, 4);
    ...
    m_wind.vector = std::max(0.f, m_wind.velocity) *
                    glm::vec3(
