...
void opengl33_precipitation::create(int const Tesselation) {
    m_vertices.clear();
    m_uvs.clear();
    m_indices.clear();
...
void opengl33_precipitation::update() {
    if (!m_shader) {
    ...
    if (!m_vertexbuffer) {
        ...
        m_vertexbuffer->allocate(
            gl::buffer::ARRAY_BUFFER, m_vertices.size() * sizeof(glm::vec3),
            GL_STATIC_DRAW);
        m_vertexbuffer->upload(
            gl::buffer::ARRAY_BUFFER, m_vertices.data(), 0,
        ...
        m_uvbuffer.emplace();
        m_uvbuffer->allocate(
            gl::buffer::ARRAY_BUFFER, m_uvs.size() * sizeof(glm::vec2),
            GL_STATIC_DRAW);
        m_uvbuffer->upload(
            gl::buffer::ARRAY_BUFFER, m_uvs.data(), 0,
        ...
        m_indexbuffer->allocate(
            ...
            m_indices.size() * sizeof(unsigned short), GL_STATIC_DRAW);
        m_indexbuffer->upload(
            gl::buffer::ELEMENT_ARRAY_BUFFER, m_indices.data(), 0,
...
void opengl33_precipitation::render() {
    if (!m_shader) {
        return;
    }
    if (!m_vao) {
