...
void motiontelemetry::update() {
    ...
    if (conf.includegravity) {
        ...
        local_acc = glm::dvec3(
            -glm::dot(gravity, left), glm::dot(gravity, up),
            glm::dot(gravity, front));
    }
    ...
    local_acc.y +=
        glm::dot(acc, up) + t->Occupied()->AccVert * conf.axlebumpscale;
    ...
    glm::dvec3 right0 =
        glm::normalize(glm::cross(glm::dvec3(0.0, 1.0, 0.0), front));
