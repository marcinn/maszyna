...
class keyboard_input {
   ...
   public:
    ...
    bool key(int const Key, int const Action);
    int key(int const Key) const;
    void poll();
    inline user_command const command() const { return m_command; }
    bindingsetup_sequence& bindings() { return m_bindingsetups; }
    int binding(user_command const Command) const;
    std::string binding_hint(user_command const Command) const;
    void dump_bindings();
   ...
   protected:
    // methods
    virtual void default_bindings() = 0;
   ...
   private:
    ...
    // methods
    bool is_movement_key(int const Key) const;
