...
void basic_editor::translate(
    scene::basic_node *Node, glm::dvec3 const &Location,
    bool const Snaptoground) {
    auto &initiallocation{Node->location()};

    // fixup NaNs
    if (std::isnan(initiallocation.x)) initiallocation.x = Location.x;
    if (std::isnan(initiallocation.y)) initiallocation.y = Location.y;
    if (std::isnan(initiallocation.z)) initiallocation.z = Location.z;

    auto targetlocation{Location};
    if (false == Snaptoground) {
        targetlocation.y = initiallocation.y;
    }
    // NOTE: bit of a waste for single nodes, for the sake of less varied code
    // down the road
...
void basic_editor::translate(scene::basic_node *Node, float const Offset) {
    // NOTE: offset scaling is calculated early so the same multiplier can be
    // applied to potential whole group
    auto location{Node->location()};
    auto const distance{glm::length(location - glm::dvec3{Global.pCamera.Pos})};
    auto const offset{
        static_cast<float>(Offset * std::max(1.0, distance * 0.01))};
    ...
    if (Node->group() <= 1) {
        ...
        std::for_each(
            ...
            [&](auto *node) { translate_node(node, offset); });
    }
}

void basic_editor::translate_node(
    scene::basic_node *Node, glm::dvec3 const &Location) {
...
void basic_editor::translate_node(scene::basic_node *Node, float const Offset) {
    if (typeid(*Node) == typeid(TAnimModel)) {
...
void basic_editor::translate_instance(
    TAnimModel *Instance, glm::dvec3 const &Location) {
...
void basic_editor::translate_instance(
    TAnimModel *Instance, float const Offset) {
    auto location{Instance->location()};
    location.y += Offset;
    Instance->location(location);
}

void basic_editor::translate_memorycell(
    TMemCell *Memorycell, glm::dvec3 const &Location) {
...
void basic_editor::translate_memorycell(
    TMemCell *Memorycell, float const Offset) {
    auto location{Memorycell->location()};
    location.y += Offset;
    Memorycell->location(location);
}

void basic_editor::rotate(
    scene::basic_node *Node, glm::vec3 const &Angle, float const Quantization) {
    ...
    if ((Quantization > 0.f) && (typeid(*Node) == typeid(TAnimModel))) {
...
void basic_editor::rotate_instance(
    TAnimModel *Instance, glm::vec3 const &Angle) {
