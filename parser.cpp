...
// constructors
cParser::cParser(
    std::string const &Stream, buffertype const Type, std::string Path,
    bool const Loadtraction, std::vector<std::string> Parameters)
    : mPath(Path), LoadTraction(Loadtraction) {
...
template <>
glm::vec3 cParser::getToken(bool const ToLower, char const *Break) {
...
template <>
cParser &cParser::operator>>(std::string &Right) {
...
template <>
cParser &cParser::operator>>(bool &Right) {
    if (true == this->tokens.empty()) {
        return *this;
    }

    Right =
        ((this->tokens.front() == "true") || (this->tokens.front() == "yes") ||
         (this->tokens.front() == "1"));
    this->tokens.pop_front();
...
template <>
bool cParser::getToken<bool>(bool const ToLower, const char *Break) {
...
std::string cParser::readToken(bool ToLower, const char *Break) {
    ...
    if (true == token.empty()) {
        ...
        do {
            while (mStream->peek() != EOF &&
                ...
                token += c;
                if (findQuotes(
                        token))  // do glue together words enclosed in quotes
                    continue;
                if (trimComments(token))  // don't glue together words separated
                                          // with comment
                    break;
            }
            if (c == '\n') {
                ...
                ++mLine;
                 ...
                 mStream->peek() !=
                     EOF);  // double check in case of consecutive separators
    }
    ...
    if (false == parameters.empty()) {
        // if there's parameter list, check the token for potential parameters
        // to replace
        size_t pos;  // początek podmienianego ciągu
        while ((pos = token.find("(p")) != std::string::npos) {
            // check if the token is a parameter which should be replaced with
            // stored true value
            auto const parameter{token.substr(
                pos + 2, token.find(")", pos) - (pos + 2))};  // numer parametru
            token.erase(
                pos,
                token.find(")", pos) - pos + 1);  // najpierw usunięcie "(pN)"
            size_t nr = atoi(parameter.c_str()) - 1;
            if (nr < parameters.size()) {
                token.insert(
                    pos, parameters.at(nr));  // wklejenie wartości parametru
                if (ToLower)
                    for (; pos < parameters.at(nr).size(); ++pos)
                        token[pos] = tolower(token[pos]);
            } else
                token.insert(
                    pos, "none");  // zabezpieczenie przed brakiem parametru
        }
    }
    ...
    if (expandIncludes && token == "include") {
        ...
        if ((true == LoadTraction) ||
            ...
            if (Global.ParserLogIncludes) WriteLog("including: " + includefile);
            mIncludeParser = std::make_shared<cParser>(
                includefile, buffer_FILE, mPath, LoadTraction,
                readParameters(*this));
            mIncludeParser->autoclear(m_autoclear);
            if (mIncludeParser->mSize <= 0) {
                ErrorLog(
                    "Bad include: can't open file \"" + includefile + "\"");
            }
        } else {
            while (token != "end") {
                token = readToken(
                    true);  // minimize risk of case mismatch on comparison
            }
        }
        token = readToken(ToLower, Break);
    } else if (
        (std::strcmp(Break, "\n\r") == 0) &&
        (token.compare(0, 7, "include") == 0)) {
        // HACK: if the parser reads full lines we expect this line to contain
        // entire include directive, to make parsing easier
        cParser includeparser(token.substr(7));
        std::string includefile =
            includeparser.readToken(ToLower);  // nazwa pliku
        replace_slashes(includefile);
        if ((true == LoadTraction) ||
            ((false == contains(includefile, "tr/")) &&
             (false == contains(includefile, "tra/")))) {
...
std::vector<std::string> cParser::readParameters(cParser &Input) {
    ...
    std::string parameter =
        Input.readToken(false);  // w parametrach nie zmniejszamy
...
std::string cParser::readQuotes(
    char const Quote) {  // read the stream until specified char or stream end
    std::string token = "";
    char c{0};
    bool escaped = false;
...
void cParser::skipComment(
    std::string const
        &Endmark) {  // pobieranie znaków aż do znalezienia znacznika końca
    std::string input = "";
    char c{0};
    ...
    while (mStream->peek() != EOF) {
        ...
        if (input.size() >= endmarksize) {
            // keep the read text short, to avoid pointless string
            // re-allocations on longer comments
...
bool cParser::trimComments(std::string &String) {
    for (auto const &comment : mComments) {
        ...
        if (String.compare(
                String.size() - comment.first.size(), comment.first.size(),
                comment.first) == 0) {
...
void cParser::injectString(const std::string &str) {
    if (mIncludeParser) {
        ...
        mIncludeParser =
            std::make_shared<cParser>(str, buffer_TEXT, "", LoadTraction);
        mIncludeParser->autoclear(m_autoclear);
    }
}
...
int cParser::getProgress() const {
    return static_cast<int>(
        mStream->rdbuf()->pubseekoff(0, std::ios_base::cur) * 100 / mSize);
}
...
int cParser::getFullProgress() const {
    ...
    if (mIncludeParser)
        return progress +
               ((100 - progress) * (mIncludeParser->getProgress()) / 100);
    else
        return progress;
}
...
void cParser::addCommentStyle(
    std::string const &Commentstart, std::string const &Commentend) {
...
int cParser::LineMain() const { return mIncludeParser ? -1 : mLine; }
