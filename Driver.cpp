...
// finds point of specified track nearest to specified event. returns: distance
// to that point from the specified end of the track
// TODO: move this to file with all generic routines, too easy to forget it's
// here and it may come useful
double ProjectEventOnTrack(
    basic_event const *Event, TTrack const *Track, double const Direction) {
    auto const segment = Track->CurrentSegment();
    auto const nearestpoint =
        segment->find_nearest_point(Event->input_location());
    return (
        Direction > 0 ? nearestpoint * segment->GetLength()
                      :  // measure from point1
            (1.0 - nearestpoint) *
                segment->GetLength());  // measure from point2
};

double GetDistanceToEvent(
    TTrack const *track, basic_event const *event, double scan_dir,
    double start_dist, int iter = 0, bool back = false) {
    if (track == nullptr) {
        return start_dist;
    }
    ...
    double const dzielnik =
        1.0 / segment->GetLength();  // rozdzielczosc mniej wiecej 1m
    ...
    do {
        ...
        ++krok;
             ...
             (seg_len >= dzielnik && (seg_len <= (1.0 - dzielnik))));
    // trzeba sprawdzić czy seg_len nie osiągnął skrajnych wartości, bo wtedy
    //  trzeba sprawdzić tor obok
    if (1 == krok)
        sd = -sd;  // jeśli tylko jeden krok tzn, że event przy poprzednim
                   // sprawdzaym torze
    if (((1 == krok) || (seg_len <= dzielnik) ||
         (seg_len > (1.0 - dzielnik))) &&
        (iter < 3)) {  // przejście na inny tor
        track = track->Connected(int(sd), sd);
        start_dist += (1 == krok) ? 0
                      : back      ? -segment->GetLength()
                                  : segment->GetLength();
        if ((track != nullptr) && (track->eType == tt_Cross)) {
            // NOTE: tracing through crossroads currently poses risk of tracing
            // through wrong segment as it's possible to be performerd before
            // setting a route through the crossroads as a stop-gap measure we
            // don't trace through crossroads which should be reasonable in most
            // cases
            // TODO: establish route before the scan, or a way for the function
            // to know which route to pick
            return start_dist;
        } else {
            return GetDistanceToEvent(
                track, event, sd, start_dist, ++iter, 1 == krok ? true : false);
        }
    } else {  // obliczenie mojego toru
        seg_len -= scan_dir > 0 ? dzielnik
                                : -dzielnik;  // trzeba wrócić do pozycji len1
        seg_len = scan_dir < 0 ? 1 - seg_len : seg_len;
        seg_len =
            back ? 1 - seg_len : seg_len;  // odwracamy jeśli idzie do tyłu
...
// stałe
const double EasyReactionTime =
    0.5;  //[s] przebłyski świadomości dla zwykłej jazdy
...
std::string StopReasonTable[] = {
    ...
    "A command",         // stopComm, //otrzymano taką komendę (niewiadomego
                         // pochodzenia)
    "Out of station",    // stopOut, //komenda wyjazdu poza stację (raczej nie
                         // powinna zatrzymywać!)
...
TSpeedPos::TSpeedPos(
    basic_event *event, double dist, double length, TOrders order) {
...
void TSpeedPos::CommandCheck() {  // sprawdzenie typu komendy w evencie i
    ...
    switch (command) {
        case TCommandType::cm_ShuntVelocity:
            // prędkość manewrową zapisać, najwyżej AI zignoruje przy analizie
            // tabelki
            fVelNext = value1;  // powinno być value2, bo druga określa "za"?
            iFlags |= spShuntSemaphor;
            break;
        case TCommandType::cm_SetVelocity:
            // w semaforze typu "m" jest ShuntVelocity dla Ms2 i SetVelocity dla
            // S1 SetVelocity * 0    -> można jechać, ale stanąć przed
            // SetVelocity 0 20   -> stanąć przed, potem można jechać 20 (SBL)
            // SetVelocity -1 100 -> można jechać, przy następnym ograniczenie
            // (SBL) SetVelocity 40 -1  -> PutValues: jechać 40 aż do minięcia
            // (koniec ograniczenia(
            fVelNext = value1;
            iFlags &= ~(spShuntSemaphor | spPassengerStopPoint | spStopOnSBL);
            iFlags |= spSemaphor;   // nie manewrowa, nie przystanek, nie
                                    // zatrzymać na SBL, ale semafor
            if (value1 == 0.0)      // jeśli pierwsza zerowa
                if (value2 != 0.0)  // a druga nie
                {  // S1 na SBL, można przejechać po zatrzymaniu (tu nie mamy
                   // prędkości ani odległości)
                    fVelNext = value2;  // normalnie będzie zezwolenie na jazdę,
                                        // aby się usunął z tabelki
                    iFlags |= spStopOnSBL;  // flaga, że ma zatrzymać; na pewno
                                            // nie zezwoli na manewry
                }
            break;
        ...
        case TCommandType::cm_PassengerStopPoint:
            ...
            fVelNext = 0.0;
            iFlags |=
                spPassengerStopPoint;  // niestety nie da się w tym miejscu
                                       // współpracować z rozkładem
                                       /*
                                               // NOTE: not used for now as it might be unnecessary
                                               // special case, potentially override any set speed limits
                                          if requested
                                               // NOTE: we test it here because for the time being it's
                                          only used for passenger stops                            if( TestFlag( iFlags,
                                          spDontApplySpeedLimit ) ) {                            fVelNext = -1;
                                               }
                                       */
            break;
        ...
        case TCommandType::cm_OutsideStation:
            // w trybie manewrowym: skanować od niej wstecz i stanąć po
            // wyjechaniu za sygnalizator i zmienić kierunek w trybie
            // pociągowym: można przyspieszyć do wskazanej prędkości (po
        ...
        case TCommandType::cm_SecuritySystemMagnet:
            ...
            break;
        default:
            // inna komenda w evencie skanowanym powoduje zatrzymanie i wysłanie
            // tej komendy nie manewrowa, nie przystanek, nie zatrzymać na SBL
            // jak nieznana komenda w komórce sygnałowej, to zatrzymujemy
            fVelNext = 0.0;
            /*
                    fVelNext = (
                        evEvent->is_command() ? 0.0 : // ask for a stop if we
               have a command for the vehicle ( iFlags & ( spSemaphor |
               spShuntSemaphor )) != 0 ? fVelNext : // don't change semafor
               velocity -1.0 ); // otherwise don't be a bother
            */
            // TODO: check whether clearing spShuntSemaphor flag doesn't cause
            // problems potentially it can invalidate shunt semaphor used to
            // transmit timetable or similar command
            iFlags &= ~(spShuntSemaphor | spPassengerStopPoint | spStopOnSBL);
    }
};
...
bool TSpeedPos::Update() {
    ...
    if (iFlags & spTrack) {
        ...
        if (trTrack) {
            ...
            fVelNext = trTrack->VelocityGet();  // aktualizacja prędkości (może
                                                // być zmieniana eventem)
            ...
            if (trTrack->iCategoryFlag & 1) {
                ...
                if (iFlags & spSwitch) {
                    ...
                    if (((trTrack->GetSwitchState() & 1) != 0) !=
                        ...
                        if ((iFlags & spElapsed) == 0) {
                            ...
                            // problem jest chyba, jeśli zwrotnica się przełoży
                            // zaraz po zjechaniu z niej na Mydelniczce potrafi
                            // skanować na wprost mimo pojechania na bok
                            return true;
                        }
                    }
                }
                ...
                if (iFlags & 0xF0000000) {
                    ...
                    if ((iFlags & 0xF0000000) > 0x10000000) {
                        // ±1 to jazda na wprost, ±2 nieby też, ale z
                        // przecięciem głównej drogi - chyba że jest
                        // równorzędne...
                        // TODO: uzależnić prędkość od promienia;
                        // albo niech będzie ograniczona w skrzyżowaniu
                        // (velocity z ujemną wartością)
                        fVelNext = 30.0;
                    }
                }
            }
        ...
        // jeśli event
        if (((iFlags & spElapsed) == 0) || (fVelNext == 0.0)) {
            ...
            CommandCheck();  // sprawdzenie typu komendy w evencie i określenie
                             // prędkości
        }
    }
    return false;
...
std::string TSpeedPos::TableText() const {  // pozycja tabelki pr?dko?ci
    if (iFlags & spEnabled) {               // o ile pozycja istotna
        return to_hex_str(iFlags, 8) + "   " + to_string(fDist, 1, 6) + "   " +
               (fVelNext == -1.0
                    ? "  -"
                    : to_string(static_cast<int>(fVelNext), 0, 3)) +
               "   " + GetName();
    }
    return "Empty";
}

bool TSpeedPos::IsProperSemaphor(
    TOrders order) {  // sprawdzenie czy semafor jest zgodny z trybem jazdy
    if (order < Obey_train)  // Wait_for_orders, Prepare_engine,
                             // Change_direction, Connect, Disconnect, Shunt
    {
        ...
        if (iFlags & spSemaphor) return true;
    }
    return false;  // true gdy zatrzymanie, wtedy nie ma po co skanować dalej
}

bool TSpeedPos::Set(
    basic_event *event, double dist, double length,
    TOrders order) {  // zapamiętanie zdarzenia
    fDist = dist;
    iFlags = spEvent;
    evEvent = event;
    vPos = event->input_location();  // współrzędne eventu albo komórki pamięci
                                     // (zrzutować na tor?)
    // ignore events located behind the consist, but with exception of stop
    // points which may be needed to update freshly received timetable
    if ((dist + length < 0) &&
    ...
    CommandCheck();  // sprawdzenie typu komendy w evencie i określenie
                     // prędkości
    // zależnie od trybu sprawdzenie czy jest tutaj gdzieś semafor lub tarcza
    // manewrowa jeśli wskazuje stop wtedy wystawiamy true jako koniec
    // sprawdzania WriteLog("EventSet: Vel=" + AnsiString(fVelNext) + " iFlags="
    // + AnsiString(iFlags) + " order="+AnsiString(order));
    if (order < Obey_train)  // Wait_for_orders, Prepare_engine,
                             // Change_direction, Connect, Disconnect, Shunt
    {
        ...
        if (iFlags & spSemaphor && fVelNext == 0.0) return true;
    }
    return false;  // true gdy zatrzymanie, wtedy nie ma po co skanować dalej
};

void TSpeedPos::Set(
    TTrack *track, double dist,
    int flag) {    // zapamiętanie zmiany prędkości w torze
    fDist = dist;  // odległość do początku toru
    trTrack =
        track;  // TODO: (t) może być NULL i nie odczytamy końca poprzedniego :/
    if (trTrack) {
        iFlags =
            flag |
            (trTrack->eType == tt_Normal
                 ? spTrack
                 : (spTrack | spSwitch));  // zapamiętanie kierunku wraz z typem
        if (iFlags & spSwitch)
            if (trTrack->GetSwitchState() & 1) iFlags |= spSwitchStatus;
        fVelNext = trTrack->VelocityGet();
        if (trTrack->iDamageFlag & 128)
            fVelNext = 0.0;  // jeśli uszkodzony, to też stój
        if (iFlags & spEnd)
            fVelNext =
                (trTrack->iCategoryFlag & 1)
                    ? 0.0
                    : 20.0;  // jeśli koniec, to pociąg stój, a samochód zwolnij
        vPos = (iFlags & spReverse)
...
void TController::TableClear() {  // wyczyszczenie tablicy
    sSpeedTable.clear();
    iLast = -1;
...
std::vector<basic_event *> TController::CheckTrackEvent(
    TTrack *Track, double const fDirection)
    const {  // sprawdzanie eventów na podanym torze do podstawowego skanowania
    std::vector<basic_event *> events;
    auto const &eventsequence{
        (fDirection > 0 ? Track->m_events2 : Track->m_events1)};
    for (auto const &event : eventsequence) {
        if ((event.second != nullptr) && (event.second->m_passive)) {
...
bool TController::TableAddNew() {  // zwiększenie użytej tabelki o jeden rekord
    sSpeedTable.emplace_back();    // add a new slot
    iLast = sSpeedTable.size() - 1;
    return true;  // false gdy się nałoży
};

bool TController::TableNotFound(basic_event const *Event, double const Distance)
    const {  // sprawdzenie, czy nie został już dodany do tabelki (np. podwójne
             // W4 robi problemy)
    auto lookup = std::find_if(
        sSpeedTable.begin(), sSpeedTable.end(),
        [Event](TSpeedPos const &speedpoint) {
            return (
                (true == TestFlag(speedpoint.iFlags, spEnabled | spEvent)) &&
                (speedpoint.evEvent == Event));
        });

    if ((Global.iWriteLogEnabled & 8) && (lookup != sSpeedTable.end())) {
        WriteLog(
            "Speed table for " + OwnerName() + " already contains event " +
            lookup->evEvent->m_name);
    }

    // ignore duplicates which seem to be reasonably apart from each other, on
    // account of looping tracks NOTE: since supplied distance is only rough
    // approximation of distance to the event, we're using large safety margin
    return (
        (lookup == sSpeedTable.end()) || (Distance - lookup->fDist > 100.0));
};

void TController::TableTraceRoute(
    double fDistance,
    TDynamicObject
        *pVehicle) {  // skanowanie trajektorii na odległość (fDistance) od
                      // (pVehicle) w kierunku przodu składu i
    // uzupełnianie tabelki
    // WriteLog("Starting TableTraceRoute");

    TTrack *pTrack{nullptr};  // zaczynamy od ostatniego analizowanego toru
    double fTrackLength{
        0.0};  // długość aktualnego toru (krótsza dla pierwszego)
    ...
    if (iTableDirection != iDirection) {
        ...
        TableClear();
        /*
                // aktualna prędkość // changed to -1 to recognize speed limit,
           if any fLastVel = -1.0; sSpeedTable.clear(); iLast = -1; tLast =
           nullptr; //żaden nie sprawdzony SemNextIndex = -1; SemNextStopIndex =
           -1;
        */
        ...
        iTableDirection = iDirection;   // ustalenie w jakim kierunku jest
                                        // wypełniana tabelka względem pojazdu
        pTrack = pVehicle->GetTrack();  // odcinek, na którym stoi
        fTrackLength = pVehicle->RaTranslationGet();  // pozycja na tym torze
                                                      // (odległość od Point1)
        fLastDir =
            pVehicle->DirectionGet() *
            pVehicle
                ->RaDirectionGet();  // ustalenie kierunku skanowania na torze
        if (fLastDir < 0.0) {
            // jeśli w kierunku Point2 toru
            fTrackLength =
                pTrack->Length() -
                fTrackLength;  // przeskanowana zostanie odległość do Point2
        }
        // account for the fact tracing begins from active axle, not the actual
        // front of the vehicle NOTE: position of the couplers is modified by
        // track offset, but the axles ain't, so we need to account for this as
        // well
        fTrackLength -=
            (pVehicle->AxlePositionGet() - pVehicle->RearPosition() +
        ...
        fTrackLength =
            pTrack->Length();  // skasowanie zmian w zmiennej żeby poprawnie
                               // liczyło w dalszych krokach
        MoveDistanceReset();  // AI startuje 1s po zaczęciu jazdy i mógł już coś
                              // przejechać
    } else {
        if (iTableDirection == 0) {
            return;
        }
        // NOTE: provisory fix for BUG: sempahor indices no longer matching
        // table size
        // TODO: find and really fix the reason it happens
        if ((SemNextIndex != -1) && (SemNextIndex >= sSpeedTable.size())) {
            SemNextIndex = -1;
        }
        ...
        if ((SemNextStopIndex != -1) &&
            ...
            // znaleziono semafor lub tarczę lub tor z prędkością zero, trzeba
            // sprawdzić czy to nadał semafor jeśli jest następny semafor to
            // sprawdzamy czy to on nadał zero
            if ((OrderCurrentGet() & Obey_train) &&
                (sSpeedTable[SemNextStopIndex].iFlags & spSemaphor)) {
                return;
            } else {
                if ((OrderCurrentGet() < Obey_train) &&
                    (sSpeedTable[SemNextStopIndex].iFlags &
                     (spSemaphor | spShuntSemaphor | spOutsideStation))) {
                    return;
                }
            }
        }
        ...
        fCurrentDistance =
            lastspeedpoint.fDist;  // aktualna odleglosc do jego Point1
    ...
    while (fCurrentDistance < fDistance) {
        if (pTrack !=
            tLast)  // ostatni zapisany w tabelce nie był jeszcze sprawdzony
        {           // jeśli tor nie był jeszcze sprawdzany
            if (Global.iWriteLogEnabled & 8) {
                WriteLog(
                    "Speed table for " + OwnerName() +
                    " tracing through track " + pTrack->name());
            }

            auto const events{CheckTrackEvent(pTrack, fLastDir)};
            for (auto *pEvent : events) {
                if (pEvent != nullptr)  // jeśli jest semafor na tym torze
                {  // trzeba sprawdzić tabelkę, bo dodawanie drugi raz tego
                   // samego przystanku nie jest korzystne
                    if (TableNotFound(
                            pEvent, fCurrentDistance))  // jeśli nie ma
                    {
                        ...
                        /*
                                                if( newspeedpoint.Set(
                                                    pEvent,
                                                    fCurrentDistance +
                           ProjectEventOnTrack( pEvent, pTrack, fLastDir ),
                                                    OrderCurrentGet() ) ) {
                        */
                        if (newspeedpoint.Set(
                                pEvent,
                                GetDistanceToEvent(
                                    pTrack, pEvent, fLastDir, fCurrentDistance),
                                fLength, OrderCurrentGet())) {
                            fDistance =
                                newspeedpoint
                                    .fDist;  // jeśli sygnał stop, to nie ma
                            ...
                            if (Global.iWriteLogEnabled & 8) {
                                WriteLog(
                                    "(stop signal from " +
                                    (SemNextStopIndex != -1
                                         ? sSpeedTable[SemNextStopIndex]
                                               .GetName()
                                         : "unknown semaphor") +
                                    ")");
                            }
                        } else {
                            if ((true == newspeedpoint.IsProperSemaphor(
                                             OrderCurrentGet())) &&
                                (SemNextIndex == -1)) {
                                SemNextIndex =
                                    iLast;  // sprawdzamy czy pierwszy na drodze
                            }
                            if (Global.iWriteLogEnabled & 8) {
                                WriteLog(
                                    "(forward signal for " +
                                    (SemNextIndex != -1
                                         ? sSpeedTable[SemNextIndex].GetName()
                                         : "unknown semaphor") +
                                    ")");
                            }
                        }
            ...
            }  // event dodajemy najpierw, żeby móc sprawdzić, czy tor został
               // dodany po odczytaniu prędkości następnego

            if ((pTrack->VelocityGet() == 0.0)  // zatrzymanie
                || (pTrack->iAction)  // jeśli tor ma własności istotne dla
                                      // skanowania
                || (pTrack->VelocityGet() !=
                    fLastVel))  // następuje zmiana prędkości
            {  // odcinek dodajemy do tabelki, gdy jest istotny dla ruchu
                TableAddNew();
                sSpeedTable[iLast].Set(
                    pTrack, fCurrentDistance,
                    (fLastDir < 0 ? spEnabled | spReverse
                                  : spEnabled));  // dodanie odcinka do tabelki
                                                  // z flagą kierunku wejścia
                if (pTrack->eType == tt_Cross) {
                    // na skrzyżowaniach trzeba wybrać segment, po którym
                    // pojedzie pojazd dopiero tutaj jest ustalany kierunek
                    // segmentu na skrzyżowaniu
                    /*
                                        // NOTE: manual selection disabled due
                       to multiplayer int routewanted; if( false ==
                       AIControllFlag ) { routewanted = ( input::keys[
                       GLFW_KEY_LEFT ] != GLFW_RELEASE ? 1 : input::keys[
                       GLFW_KEY_RIGHT ] != GLFW_RELEASE ? 2 : 3 );
                                        }
                                        else {
                                            routewanted = 1 + std::floor(
                       Random( static_cast<double>( pTrack->RouteCount() ) -
                       0.001 ) );
                                        }
                    */
                    auto const routewanted{
                        1 + std::floor(Random(
                                static_cast<double>(pTrack->RouteCount()) -
                                0.001))};
                    sSpeedTable[iLast].iFlags |=
                        ((pTrack->CrossSegment(
                              (fLastDir < 0 ? tLast->iPrevDirection
                    ...
                    sSpeedTable[iLast].iFlags &=
                        ~spReverse;  // usunięcie flagi kierunku, bo może być
                                     // błędna
                    if (sSpeedTable[iLast].iFlags < 0) {
                        sSpeedTable[iLast].iFlags |=
                            spReverse;  // ustawienie flagi kierunku na
                    ...
                    /*
                                        if (AIControllFlag) {
                                            // dla AI na razie losujemy kierunek
                       na kolejnym skrzyżowaniu iRouteWanted = 1 + Random(3);
                                        }
                    */
                }
            } else if (
                (pTrack->fRadius !=
                 0.0)  // odległość na łuku lepiej aproksymować cięciwami
                || ((tLast != nullptr) &&
                    (tLast->fRadius != 0.0)))  // koniec łuku też jest istotny
            {  // albo dla liczenia odległości przy pomocy cięciw - te usuwać po
               // przejechaniu
                if (TableAddNew()) {
                    // dodanie odcinka do tabelki
                    sSpeedTable[iLast].Set(
                        pTrack, fCurrentDistance,
                        (fLastDir < 0 ? spEnabled | spCurve | spReverse
                                      : spEnabled | spCurve));
                }
            }
        }
        ...
        fCurrentDistance += fTrackLength;  // doliczenie kolejnego odcinka do
                                           // przeskanowanej długości
        tLast = pTrack;                    // odhaczenie, że sprawdzony
        fLastVel =
            pTrack
                ->VelocityGet();  // prędkość na poprzednio sprawdzonym odcinku
        pTrack = pTrack->Connected(
            (pTrack->eType == tt_Cross ? (sSpeedTable[iLast].iFlags >> 28)
                                       : static_cast<int>(fLastDir)),
            fLastDir);  // może być NULL

        if (pTrack != nullptr) {  // jeśli kolejny istnieje
            if (tLast != nullptr) {
                if ((tLast->VelocityGet() > 0) &&
                    ...
                    if ((iLast != -1) &&
                        ...
                        // if the track is already in the table we only need to
                        // mark it as relevant
                        ...
                        sSpeedTable[iLast].Set(
                            tLast,
                            fCurrentDistance -
                                fTrackLength,  // by now the current distance
                                               // points to beginning of next
                                               // track
            ...
            fTrackLength = pTrack->Length();
        } else {  // definitywny koniec skanowania, chyba że dalej puszczamy
                  // samochód po gruncie...
            if ((iLast == -1) ||
                ((false ==
                  TestFlag(sSpeedTable[iLast].iFlags, spEnabled | spEnd)) &&
                 (sSpeedTable[iLast].trTrack != tLast))) {
                // only if we haven't already marked end of the track and if the
                // new track doesn't duplicate last one
                if (TableAddNew()) {
                    // zapisanie ostatniego sprawdzonego toru
                    sSpeedTable[iLast].Set(
                        tLast,
                        fCurrentDistance -
                            fTrackLength,  // by now the current distance points
                                           // to beginning of next track,
                        (fLastDir < 0 ? spEnabled | spEnd | spReverse
                                      : spEnabled | spEnd));
                }
            } else if (sSpeedTable[iLast].trTrack == tLast) {
                // otherwise just mark the last added track as the final one
                // TODO: investigate exactly how we can wind up not marking the
                // last existing track as actual end
                if (false ==
                    TestFlag(
                        sSpeedTable[iLast].trTrack->iCategoryFlag, 0x100)) {
                    // don't mark portals, as these aren't exactly track ends,
                    // but teleport devices
                    sSpeedTable[iLast].iFlags |= (spEnabled | spEnd);
                }
            }
            // to ostatnia pozycja, bo NULL nic nie da, a może się podpiąć
            // obrotnica, czy jakieś transportery
            return;
        }
    }
    if (TableAddNew()) {
...
void TController::TableCheck(
    double fDistance) {  // przeliczenie odległości w tabelce, ewentualnie
                         // doskanowanie (bez analizy prędkości itp.)
    if (iTableDirection != iDirection) {
        // jak zmiana kierunku, to skanujemy od końca składu
        TableTraceRoute(fDistance, pVehicles[end::rear]);
        TableSort();
    } else if (iTableDirection) {  // trzeba sprawdzić, czy coś się zmieniło
        ...
        for (int i = 0; i < iLast;
             ++i) {  // aktualizacja rekordów z wyjątkiem ostatniego
            if (sSpeedTable[i].iFlags & spEnabled)  // jeśli pozycja istotna
            {
                if (sSpeedTable[i].Update()) {
                    if (Global.iWriteLogEnabled & 8) {
                        WriteLog(
                            "Speed table for " + OwnerName() +
                            " detected switch change at " +
                            sSpeedTable[i].trTrack->name() +
                            " (generating fresh trace)");
                    }
                    // usuwamy wszystko za tym torem
                    while (sSpeedTable.back().trTrack !=
                           sSpeedTable[i]
                               .trTrack) {  // usuwamy wszystko dopóki nie
                                            // trafimy na tą zwrotnicę
                ...
                if (sSpeedTable[i].iFlags & spTrack)  // jeśli odcinek
                {
                    if (sSpeedTable[i].fDist +
                            sSpeedTable[i].trTrack->Length() <
                        -fLength)  // a skład wyjechał całą długością poza
                    {              // degradacja pozycji
                        sSpeedTable[i].iFlags &= ~spEnabled;  // nie liczy się
                    } else if (
                        (sSpeedTable[i].iFlags & 0xF0000028) == spElapsed) {
                        // jest z tyłu (najechany) i nie jest zwrotnicą ani
                        // skrzyżowaniem
                        if (sSpeedTable[i].fVelNext <
                            0)  // a nie ma ograniczenia prędkości
                        {
                            sSpeedTable[i].iFlags =
                                0;  // to nie ma go po co trzymać (odtykacz
                                    // usunie ze środka)
                        }
                    }
                ...
                {
                    if (sSpeedTable[i].fDist <
                        (typeid(*(sSpeedTable[i].evEvent)) ==
                                 typeid(putvalues_event)
                             ? -fLength
                             : 0))  // jeśli jest z tyłu
                        if ((mvOccupied->CategoryFlag == 2) &&
                            (sSpeedTable[i].fDist <
                             -0.75)) {  // pociąg staje zawsze, a samochód tylko
                                        // jeśli nie przejedzie całą długością
                                        // (może być zaskoczony zmianą)
                                        // WriteLog("TableCheck: Event is
                                        // behind. Delete from table: " +
                                        // sSpeedTable[i].evEvent->asName);
                            sSpeedTable[i].iFlags &=
                                ~spEnabled;  // degradacja pozycji dla
                                             // samochodu;
                            // semafory usuwane tylko przy sprawdzaniu, bo
                            // wysyłają komendy
                        }
                }
            }
        ...
        // WriteLog("TableCheck: Upate last track. Dist=" +
        // AnsiString(sSpeedTable[iLast].fDist));
        if (sSpeedTable[iLast].fDist < fDistance) {
            TableTraceRoute(
                fDistance,
                pVehicles[end::rear]);  // doskanowanie dalszego odcinka
            TableSort();
        }
        // garbage collection
        TablePurger();
...
auto const passengerstopmaxdistance{
    400.0};  // maximum allowed distance between passenger stop point and
             // consist head

TCommandType TController::TableUpdate(
    double &fVelDes, double &fDist, double &fNext,
    double &fAcc) {  // ustalenie parametrów, zwraca typ komendy, jeśli sygnał
                     // podaje prędkość do jazdy
    // fVelDes - prędkość zadana
    // fDist - dystans w jakim należy rozważyć ruch
    // fNext - prędkość na końcu tego dystansu
    // fAcc - zalecane przyspieszenie w chwili obecnej - kryterium wyboru
    // dystansu
    ...
    auto speedlimitiscontinuous{
        true};  // stays true if potential active speed limit is unbroken to the
                // last (relevant) point in scan table
    eSignNext = nullptr;
    ...
    iDrivigFlags &=
        ~(moveTrackEnd | moveSwitchFound | moveSignalFound |
          /*moveSpeedLimitFound*/ moveStopPointFound);

    for (std::size_t idx = 0; idx < sSpeedTable.size(); ++idx) {
        // o ile dana pozycja tabelki jest istotna
        if ((sSpeedTable[idx].iFlags & spEnabled) == 0) {
            continue;
        }
        ...
        if (TestFlag(point.iFlags, spPassengerStopPoint)) {
            if (TableUpdateStopPoint(go, point, d_to_next_sem)) {
                continue;
            };
        }
        ...
        if (TestFlag(point.iFlags, spSwitch)) {
            ...
            iDrivigFlags |= moveSwitchFound;  // rozjazd z przodu/pod ogranicza
                                              // np. sens skanowania wstecz
        ...
        {  // jeżeli event, może być potrzeba wysłania komendy, aby ruszył
            if (TableUpdateEvent(v, go, point, d_to_next_sem, idx)) {
                continue;
            }
        }

        auto const railwaytrackend{
            (true == TestFlag(point.iFlags, spEnd)) && (is_train())};
        if ((v >= 0.0)  // pozycje z prędkością -1 można spokojnie pomijać
            || (railwaytrackend)) {
            d = point.fDist;

            if (v >= 0.0) {
                // points located in front of us can potentially affect our
                // acceleration and target speed
                if ((d > 0.0) && (false == TestFlag(point.iFlags, spElapsed))) {
                    // sygnał lub ograniczenie z przodu (+32=przejechane)
                    // 2014-02: jeśli stoi, a ma do przejechania kawałek, to
                    // niech jedzie
                    if ((mvOccupied->Vel < 0.01) &&
                        (true == TestFlag(
                                     point.iFlags, (spEnabled | spEvent |
                                                    spPassengerStopPoint))) &&
                        (false == IsAtPassengerStop)) {
                        // ma podjechać bliżej - czy na pewno w tym miejscu taki
                        // warunek?
                        a =
                            ((d > passengerstopmaxdistance) ||
                        ...
                        if (v >= 0.0) {
                            a = (v * v - mvOccupied->Vel * mvOccupied->Vel) /
                                (25.92 * d);
                            if ((mvOccupied->Vel < v) || (v == 0.0)) {
                                // if we're going slower than the target
                                // velocity and there's enough room for safe
                                // stop, speed up
                                auto const brakingdistance{
                                    1.2 * fBrakeDist *
                                    braking_distance_multiplier(v)};
                                if (brakingdistance > 0.0) {
                                    // maintain desired acc while we have enough
                                    // room to brake safely, when close enough
                                    // start paying attention try to make a
                                    // smooth transition instead of sharp change
                                    a = interpolate(
                                        a, AccPreferred,
                                        clamp(
                                            (d - brakingdistance) /
                            ...
                            if ((d < fMinProximityDist) && (v < fVelDes)) {
                                // jak jest już blisko, ograniczenie aktualnej
                                // prędkości
                                fVelDes = v;
                            }
                        }
                ...
                // points located behind can affect our current speed, but
                // little else
                else if (point.iFlags & spTrack)  // jeśli tor
                {  // tor ogranicza prędkość, dopóki cały skład nie przejedzie,
                    if (v >= 1.0) {  // EU06 się zawieszało po dojechaniu na
                                     // koniec toru postojowego
                    ...
                    if (v < fVelDes) {
                        // ograniczenie aktualnej prędkości aż do wyjechania za
                        // ograniczenie
                        fVelDes = v;
                    }
                    if (v < VelLimitLastDist.first) {
                        VelLimitLastDist.second =
                            d + point.trTrack->Length() + fLength;
                    } else if (
                        VelLimitLastDist.second >
                        0) {  // the speed limit can potentially start
                              // afterwards, so don't mark it as broken too soon
                        if ((point.iFlags &
                             (spSwitch | spPassengerStopPoint)) == 0) {
                    ...
                    // event trzyma tylko jeśli VelNext=0, nawet po przejechaniu
                    // (nie powinno dotyczyć samochodów?)
                    a =
                        (v > 0.0 ? fAcc
                         : mvOccupied->Vel < 0.01
                             ? 0.0
                             :  // already standing still so no need to bother
                                // with brakes
                             -2.0);  // ruszanie albo hamowanie
                }
            }
            // track can potentially end, which creates another virtual point of
            // interest with speed limit of 0 at the end of it TBD, TODO: when
            // tracing the route create a dedicated table entry for it, to
            // simplify the code?
            if (railwaytrackend) {
                // if the railway track ends here set the velnext accordingly as
                // well
                // TODO: test this with turntables and such
                auto const stopatendacceleration =
                    (-1.0 * mvOccupied->Vel * mvOccupied->Vel) /
                    (25.92 * (d + point.trTrack->Length()));
            ...
            if ((a <= fAcc) &&
                ((v < fNext) ||
                 (fNext <
                  0))) {  // filter out consecutive, farther out blocks with the
                          // same speed limit; they'd make us accelerate slower
                          // due to their lower a value
                // mniejsze przyspieszenie to mniejsza możliwość rozpędzenia się
                // albo konieczność hamowania jeśli droga wolna, to może być
                // a>1.0 i się tu nie załapuje
                fAcc = a;  // zalecane przyspieszenie (nie musi być uwzględniane
                           // przez AI)
            ...
            /*
                            ...
                            // jeśli nie ma wskazań do hamowania, można podać
               drogę i prędkość na jej końcu fNext = v; // istotna jest prędkość
               na końcu tego odcinka fDist = d; // dlugość odcinka (kolejne
               pozycje mogą wydłużać drogę, jeśli prędkość jest stała)
                        }
            */
            // we'll pick first scanned target speed as our goal
            // farther scan table points can override it through previous
            // clause, if they require lower acceleration or speed reduction
            ...
            if ((v < VelLimitLastDist.first) &&
                ((d < 0)  // the point counts as part of last speed limit either
                          // if it's behind us
                 || (VelLimitLastDist.second >
                     0))) {  // or if we already have the last limit ongoing
                VelLimitLastDist.second = d + fLength;
                if ((point.iFlags & spTrack) != 0) {
                    VelLimitLastDist.second += point.trTrack->Length();
                }
            } else {
                // if we found a point which isn't part of the current speed
                // limit mark the limit as broken NOTE: we exclude switches from
                // this rule, as speed limits generally extend through these
                if ((point.iFlags & (spSwitch | spPassengerStopPoint)) == 0) {
                    speedlimitiscontinuous = false;
                }
            }
        }  // if (v>=0.0)
        // finding a point which doesn't impose speed limit means any potential
        // active speed limit can't extend through entire scan table NOTE: we
        // test actual speed value for the given point, as events may receive (v
        // = -1) as a way to disable them in irrelevant modes
        if (point.fVelNext < 0) {
            // NOTE: we exclude switches from this rule, as speed limits
            // generally extend through these
            if ((point.iFlags & (spSwitch | spPassengerStopPoint)) == 0) {
                speedlimitiscontinuous = false;
            }
        }
        if (fNext >= 0.0) {  // jeśli ograniczenie
            if ((point.iFlags & (spEnabled | spEvent)) ==
                (spEnabled | spEvent)) {  // tylko sygnał przypisujemy
                if (eSignNext ==
                    nullptr) {  // jeśli jeszcze nic nie zapisane tam
    ...
    if ((VelSignalLast >= 0.0) &&
        ((iDrivigFlags &
          (moveSignalFound | moveSwitchFound | moveStopPointFound)) == 0) &&
        (true == TestFlag(OrderCurrentGet(), Obey_train))) {
        VelSignalLast = -1.0;
    }
    // if there's unbroken speed limit through our scan table take a note of it
    if ((VelLimitLastDist.second > 0) && (speedlimitiscontinuous)) {
        VelLimitLastDist.second = EU07_AI_SPEEDLIMITEXTENDSBEYONDSCANRANGE;
    }
    // take into account the effect switches have on duration of signal-imposed
    // speed limit, in calculation of speed limit end point
    if ((VelSignalLast >= 0.0) && (SwitchClearDist >= 0.0)) {
        VelLimitLastDist.second =
            std::max(VelLimitLastDist.second, SwitchClearDist);
    }
    ...
    FirstSemaphorDist =
        d_to_next_sem;  // przepisanie znalezionej wartosci do zmiennej
    return go;
};

bool TController::TableUpdateStopPoint(
    TCommandType &Command, TSpeedPos &Point, double const Signaldistance) {
    ...
    // first 19 chars of the command is expected to be "PassengerStopPoint:" so
    // we skip them
    if (ToLower(Point.evEvent->input_text())
            .compare(19, sizeof(asNextStop), ToLower(asNextStop)) !=
        0) {  // jeśli nazwa nie jest zgodna
        if ((false ==
             IsScheduledPassengerStopVisible)  // check if our next scheduled
                                               // stop didn't show up earlier in
                                               // the scan
            && (Point.fDist < (1.15 * fBrakeDist + 300)) &&
            (Point.fDist >
             0))  // tylko jeśli W4 jest blisko, przy dwóch może zaczać szaleć
        {
            // porównuje do następnej stacji, więc trzeba przewinąć do
            // poprzedniej nastepnie ustawić następną na aktualną tak żeby
            // prawidłowo ją obsłużył w następnym kroku
            if (true ==
                TrainParams.RewindTimeTable(Point.evEvent->input_text())) {
    ...
    } else if (iDrivigFlags & moveStopPoint)  // jeśli pomijanie W4, to nie
    ...
    {  // tylko gdy nazwa zatrzymania się zgadza
        if ((OrderCurrentGet() & (Obey_train | Bank)) != 0) {
            ...
            // NOTE: we don't do it in shunt mode, as shunting operations tend
            // to use dedicated radio channel NOTE: there's a risk radio channel
            // change was specified by a station which we skipped during
            // timetable rewind we ignore this for the time being as it's not a
            // high priority error
        ...
        IsScheduledPassengerStopVisible =
            true;  // block potential timetable rewind if the next stop shows up
                   // later in the scan
        if (false == TrainParams.IsStop()) {  // jeśli nie ma tu postoju
            ...
            if (Point.fDist < passengerstopmaxdistance * 0.5) {
                // zaliczamy posterunek w pewnej odległości przed (choć W4 nie
                // zasłania już semafora)
#if LOGSTOPS
                WriteLog(
                    pVehicle->asName + " as " + TrainParams.TrainName +
                    ": at " + std::to_string(simulation::Time.data().wHour) +
                    ":" + std::to_string(simulation::Time.data().wMinute) +
                    " passed " + asNextStop);  // informacja
#endif
                // przy jakim dystansie (stanie licznika) ma przesunąć na
                // następny postój
                fLastStopExpDist =
                    mvOccupied->DistCounter + 0.250 + 0.001 * fLength;
            ...
            if (false == Point.bMoved) {
                // potentially shift the stop point in accordance with its
                // defined parameters
                /*
                //
                https://rainsted.com/pl/Wersja/18.2.133#Okr.C4.99gi_dla_W4_i_W32
                Pierwszy parametr ujemny - preferowane zatrzymanie czoła składu
                (np. przed przejściem). Pierwszy parametr dodatni - preferowane
                zatrzymanie środka składu (np. przy wiacie, przejściu
                podziemnym). Drugi parametr ujemny - wskazanie zatrzymania dla
                krótszych składów (W32). Drugi paramer dodatni - długość peronu
                (W4).
                */
                auto L = 0.0;
                ...
                if ((Par2 >= 0) || (fLength < -Par2)) {  // użyj tego W4
                    ...
                    L = std::max(
                        0.0,
                        std::min(
                            L, std::abs(Par2) - fMinProximityDist - fLength));
            ...
            // for human-driven vehicles discard the stop point if they leave it
            // far enough behind
            if ((false == AIControllFlag) &&
                (Point.fDist <
                 -1 * std::max(fLength + 100, 250.0) - Point.fMoved)) {
                Point.iFlags =
            ...
            IsAtPassengerStop =
                ((Point.fDist <= passengerstopmaxdistance)
                 // Ra 2F1I: odległość plus długość pociągu musi być mniejsza od
                 // długości peronu, chyba że pociąg jest dłuższy, to wtedy
                 // minimalna. jeśli długość peronu
                 // ((sSpeedTable[i].evEvent->ValueGet(2)) nie podana, przyjąć
                 // odległość fMinProximityDist
                 && ((iDrivigFlags & moveStopCloser) != 0
                         ? Point.fDist + fLength +
                                   (Point.fMoved - fMinProximityDist * 0.5f) <=
                               std::max(
                                   std::abs(Point.evEvent->input_value(2)),
                                   2.0 * fMaxProximityDist + fLength)
                         :  // fmaxproximitydist typically equals ~50 m
            ...
            if (mvOccupied->Vel > EU07_AI_MOVEMENT) {
                // jeśli jedzie (nie trzeba czekać, aż się drgania wytłumią -
                // drzwi zamykane od 1.0) to będzie zatrzymanie
                Point.fVelNext = 0;
                // potentially announce pending stop
                if ((m_lastannouncement != announcement_t::approaching) &&
                    (Point.fDist < 750) && (Point.fDist > 250)) {
                    announce(announcement_t::approaching);
                }
            } else if (true == IsAtPassengerStop) {
                // jeśli się zatrzymał przy W4, albo stał w momencie zobaczenia
                // W4
                if (!AIControllFlag) {
                    // w razie przełączenia na AI ma nie podciągać do W4, gdy
                ...
                if (TrainParams.UpdateMTable(simulation::Time, asNextStop)) {
                    ...
                    if (TrainParams.StationIndex < TrainParams.StationCount) {
                        ...
                        // also ignore any horn cue that may be potentially set
                        // below 1 km/h and before the actual full stop
                        iDrivigFlags &= ~(moveStartHorn | moveStartHornNow);
                    }
                    UpdateDelayFlag();

                    // perform loading/unloading
                    // HACK: manual check if we didn't already do load exchange
                    // at this stop
                    // TODO: remove the check once the station system is in
                    // place
                    if (m_lastexchangestop != asNextStop) {
                        ...
                        m_lastexchangeplatforms =
                            static_cast<int>(std::floor(
                                std::abs(Point.evEvent->input_value(2)))) %
                            10;
                        auto const exchangetime{simulation::Station.update_load(
                            pVehicles[end::front], TrainParams,
                            m_lastexchangeplatforms)};
                    ...
                    if (TrainParams.DirectionChange()) {
                        ...
                        if (iDrivigFlags & movePushPull) {
                            ...
                            // a dla lokomotyw...
                            // pozwolenie na przejechanie za W4 przed czasem i
                            // nie ma stać
                            iDrivigFlags &= ~(moveStopPoint | moveStopHere);
                        }
                        // przejście do kolejnego rozkazu (zmiana kierunku,
                        // odczepianie)
                        JumpToNextOrder();
                        // ma nie podjeżdżać pod W4 po przeciwnej stronie
                        iDrivigFlags &= ~moveStopCloser;
                        // ten W4 nie liczy się już zupełnie (nie wyśle
                        // SetVelocity)
                        Point.iFlags = 0;
                        // jechać
                        Point.fVelNext = -1;
                ...
                if (TrainParams.StationIndex < TrainParams.StationCount) {
                    ...
                    if ((true == IsCargoTrain) ||
                        ...
                        (TrainParams.IsTimeToGo(
                            simulation::Time.data().wHour,
                            simulation::Time.data().wMinute +
                                simulation::Time.data().wSecond * 0.0167))) {
                        // z dalszą akcją czekamy do godziny odjazdu
                        // cargo trains and passenger trains at maintenance stop
                        // don't need to wait
                        IsAtPassengerStop = false;
                        // przy jakim dystansie (stanie licznika) ma przesunąć
                        // na następny postój
                        fLastStopExpDist =
                            mvOccupied->DistCounter + 0.050 + 0.001 * fLength;
                        TrainParams
                            .StationIndexInc();  // przejście do następnej
                        asNextStop =
                            TrainParams.NextStop();  // pobranie kolejnego
                                                     // miejsca zatrzymania
#if LOGSTOPS
                        WriteLog(
                            pVehicle->asName + " as " + TrainParams.TrainName +
                            ": at " +
                            std::to_string(simulation::Time.data().wHour) +
                            ":" +
                            std::to_string(simulation::Time.data().wMinute) +
                            " next " + asNextStop);  // informacja
#endif
                        // update consist weight, brake settings and ai braking
                        // tables NOTE: this calculation is expected to run
                        // after completing loading/unloading
                        CheckVehicles();  // nastawianie hamulca do jazdy
                                          // pociągowej

                        if (static_cast<int>(std::floor(
                                std::abs(Point.evEvent->input_value(1)))) %
                            2) {
                            // nie podjeżdżać do semafora, jeśli droga nie jest
                            // wolna
                        ...
                        iDrivigFlags |=
                            moveStopCloser;  // do następnego W4 podjechać
                                             // blisko (z dociąganiem)
                        Point.iFlags = 0;  // nie liczy się już zupełnie (nie
                                           // wyśle SetVelocity)
                        Point.fVelNext = -1;  // można jechać za W4
                        if ((Point.fDist <= 0.0) &&
                            (eSignNext == Point.evEvent)) {
                            // sanity check, if we're held by this stop point,
                            // let us go
                            VelSignalLast = -1;
                        }
                        if (Command ==
                            TCommandType::cm_Unknown) {  // jeśli nie było
                                                         // komendy wcześniej
                            Command =
                                TCommandType::cm_Ready;  // gotów do odjazdu z
                                                         // W4 (semafor może
                                                         // zatrzymać)
                        }
                        ...
                        if (TrainParams.IsTimeToGo(
                                simulation::Time.data().wHour,
                                simulation::Time.data().wMinute +
                                    simulation::Time.data().wSecond * 0.0167 +
                                    0.1)) {  // if is less than 6 sec to
                                             // departure
...
#endif
                    asNextStop =
                        TrainParams.NextStop();  // informacja o końcu trasy
                    TrainParams.NewName(
                        "none");  // czyszczenie nieaktualnego rozkładu
                    // ma nie podjeżdżać pod W4 i ma je pomijać
                    iDrivigFlags &= ~(moveStopCloser);
                    if (false == TestFlag(iDrivigFlags, movePushPull)) {
                        // if the consist can change direction through a simple
                        // cab change it doesn't need fiddling with recognition
                        // of passenger stops
                        iDrivigFlags &= ~(moveStopPoint);
                    }
                    fLastStopExpDist =
                        -1.0f;  // nie ma rozkładu, nie ma usuwania stacji
                    Point.iFlags =
                        0;  // W4 nie liczy się już (nie wyśle SetVelocity)
                    Point.fVelNext = -1;  // można jechać za W4
                    if ((Point.fDist <= 0.0) && (eSignNext == Point.evEvent)) {
                        // sanity check, if we're held by this stop point, let
                        // us go
                        VelSignalLast = -1;
                    }
...
bool TController::TableUpdateEvent(
    double &Velocity, TCommandType &Command, TSpeedPos &Point,
    double &Signaldistance, int const Pointindex) {
    // sprawdzanie eventów pasywnych miniętych
    if (Point.fDist < 0.0) {
        if (SemNextIndex == Pointindex) {
            if (Global.iWriteLogEnabled & 8) {
                WriteLog(
                    "Speed table update for " + OwnerName() +
                    ", passed semaphor " + sSpeedTable[SemNextIndex].GetName());
            }
            SemNextIndex =
                -1;  // jeśli minęliśmy semafor od ograniczenia to go kasujemy
                     // ze zmiennej sprawdzającej dla skanowania w przód
        }
        if (SemNextStopIndex == Pointindex) {
            if (Global.iWriteLogEnabled & 8) {
                WriteLog(
                    "Speed table update for " + OwnerName() +
                    ", passed semaphor " +
                    sSpeedTable[SemNextStopIndex].GetName());
            }
            SemNextStopIndex =
                -1;  // jeśli minęliśmy semafor od ograniczenia to go kasujemy
                     // ze zmiennej sprawdzającej dla skanowania w przód
        ...
        switch (Point.evEvent->input_command()) {
            // TBD, TODO: expand emergency_brake handling to a more generic
            ...
            case TCommandType::cm_SecuritySystemMagnet: {
                // NOTE: magnet induction calculation presumes the driver is
                // located in the front vehicle TBD, TODO: take into account
                // actual position of controlled/occupied vehicle in the
                // consist, whichever comes first
                auto const magnetlocation{
                    pVehicles[end::front]
                        ->MoverParameters->SecuritySystem.MagnetLocation};
                auto const magnetrange{1.0};
                auto const ismagnetpassed{
                    Point.fDist < -(magnetlocation + magnetrange)};
                if (Point.fDist < -(magnetlocation)) {
                    // NOTE: normally we'd activate the magnet once the leading
                    // vehicle passes it but on a fresh scan after direction
                    // change it would be detected as long as it's under
                    // consist, and meet the (simple) activation condition thus
                    // we're doing a more precise check in such situation (we
                    // presume direction change takes place only if the vehicle
                    // is standing still)
                    if ((mvOccupied->Vel > EU07_AI_NOMOVEMENT) ||
                        (false == ismagnetpassed)) {
                        mvOccupied->SecuritySystem.set_cabsignal_lock(
                            !AIControllFlag);  // don't make life difficult for
                                               // the ai, but a human driver is
                                               // a fair game
                        PutCommand(
                            Point.evEvent->input_text(),
                            Point.evEvent->input_value(1),
                            Point.evEvent->input_value(2), nullptr);
                    }
                }
                if (ismagnetpassed) {
                    Point.Clear();
    ...
    if (Point.fDist > 0.0) {
        ...
        if (Point.IsProperSemaphor(OrderCurrentGet())) {
            // special rule for cars: ignore stop signals at distance too short
            // to come to a stop as trying to stop in such situation is likely
            // to place the car on train tracks
            if ((is_car()) && (Point.fVelNext != -1.0) &&
                (Point.fVelNext < 1.0) &&
                (Point.fDist <
                 -0.5 + std::min(fBrakeDist * 0.2, mvOccupied->Vel * 0.2))) {
                Point.Clear();
                return true;
            }
            // jeśli jest mienięty poprzedni semafor a wcześniej
            // byl nowy to go dorzucamy do zmiennej, żeby cały czas widział
            // najbliższy
            if (SemNextIndex == -1) {
    ...
    if (Point.iFlags &
        spOutsideStation) {  // jeśli W5, to reakcja zależna od trybu jazdy
        if (OrderCurrentGet() &
            Obey_train) {  // w trybie pociągowym: można przyspieszyć do
                           // wskazanej prędkości (po zjechaniu z rozjazdów)
            Velocity = -1.0;               // ignorować?
            if (Point.fDist < 0.0) {       // jeśli wskaźnik został minięty
                VelSignalLast = Velocity;  // ustawienie prędkości na -1
            } else if ((iDrivigFlags & moveSwitchFound) == 0) {  // jeśli
                                                                 // rozjazdy już
                                                                 // minięte
                VelSignalLast = Velocity;  //!!! to też koniec ograniczenia
            }
        } else {  // w trybie manewrowym: skanować od niego wstecz, stanąć po
                  // wyjechaniu za sygnalizator i zmienić kierunek
            Velocity = 0.0;  // zmiana kierunku może być podanym sygnałem, ale
                             // wypadało by zmienić światło wcześniej
            if ((iDrivigFlags & moveSwitchFound) ==
                0) {  // jeśli nie ma rozjazdu
                // check for presence of a signal facing the opposite direction
                // if there's one, we'll want to pass it before changing
                // direction
                basic_event *foundevent = nullptr;
                if (Point.fDist - fMaxProximityDist > 0) {
                    auto scandistance{
                        Point.fDist + fLength - fMaxProximityDist};
                    auto *scanvehicle{pVehicles[end::rear]};
                    auto scandirection{
                        scanvehicle->DirectionGet() *
                        scanvehicle->RaDirectionGet()};
                    auto *foundtrack = BackwardTraceRoute(
                        scandistance, scandirection, scanvehicle, foundevent,
                        -1, end::front, false);
                }
                if (foundevent == nullptr) {
                    iDrivigFlags |=
                        moveTrackEnd;  // to dalsza jazda trwale ograniczona
                                       // (W5, koniec toru)
                }
            }
        }
        ...
        if (mvOccupied->Vel < 2.0) {
            ...
            // jest w maksymalnym zasięgu to można go pominąć (wziąć drugą
            // prędkosć) as long as there isn't any obstacle in arbitrary view
            // range
            if ((Point.fDist < fMaxProximityDist) &&
                (Obstacle.distance > 1000)) {
                eSignSkip = Point.evEvent;
                // jazda na widoczność - skanować możliwość kolizji i nie
                // podjeżdżać zbyt blisko usunąć flagę po podjechaniu blisko
                // semafora zezwalającego na jazdę ostrożnie interpretować
                // sygnały - semafor może zezwalać na jazdę pociągu z przodu!
                iDrivigFlags |= moveVisibility;
                // store the ordered restricted speed and don't exceed it until
                // the flag is cleared
        ...
        if (eSignSkip != Point.evEvent) {
            ...
            // TODO sprawdzić do której zmiennej jest przypisywane v i zmienić
            // to tutaj
        }
    } else if (Point.IsProperSemaphor(OrderCurrentGet())) {  // to semaphor
        if (Point.fDist < 0) {
            // for human-driven vehicles ignore the signal if it was passed by
            // sufficient distance
            if ((false == AIControllFlag) &&
            ...
            // for ai-driven vehicles always play by the rules
                ...
                VelSignalLast =
                    Point.fVelNext;  // minięty daje prędkość obowiązującą
            }
        } else {
            // jeśli z przodu to dajemy flagę, że jest
            iDrivigFlags |= moveSignalFound;
            Signaldistance = std::min(Point.fDist, Signaldistance);
            // if there's another vehicle closer to the signal, then it's likely
            // its intended recipient HACK: if so, make it a stop point, to
            // prevent non-signals farther down affect us
            auto const isforsomeoneelse{
                (is_train()) && (Obstacle.distance < Point.fDist)};
    ...
    } else if (Point.iFlags & spRoadVel) {  // to W6
        if (Point.fDist < 0) VelRoad = Point.fVelNext;
    } else if (Point.iFlags & spSectionVel) {  // to W27
        if (Point.fDist < 0)  // teraz trzeba sprawdzić inne warunki
        {
            if (Point.fSectionVelocityDist == 0.0) {
                if (Global.iWriteLogEnabled & 8) {
                    WriteLog(
                        "TableUpdate: Event is behind. SVD = 0: " +
                        Point.evEvent->m_name);
                }
                Point.Clear();  // jeśli punktowy to kasujemy i nie dajemy
                                // ograniczenia na stałe
            } else if (Point.fSectionVelocityDist < 0.0) {
                // ograniczenie obowiązujące do następnego
                if ((Point.fVelNext ==
                     min_speed(Point.fVelNext, VelLimitLast)) &&
                    (Point.fVelNext != VelLimitLast)) {
                    // jeśli ograniczenie jest mniejsze niż obecne to obowiązuje
                    // od zaraz
                    ...
                    Point.Clear();  // wyjechaliśmy poza poprzednie, można
                                    // skasować
                }
            } else {  // jeśli większe to ograniczenie ma swoją długość
                if ((Point.fVelNext ==
                     min_speed(Point.fVelNext, VelLimitLast)) &&
                    (Point.fVelNext != VelLimitLast)) {
                    // jeśli ograniczenie jest mniejsze niż obecne to obowiązuje
                    ...
                    Point.fDist < -fLength - Point.fSectionVelocityDist) {
                    VelLimitLast = -1.0;
                    if (Global.iWriteLogEnabled & 8) {
                        WriteLog(
                            "TableUpdate: Event is behind. SVD > 0: " +
    ...
    {  // zawalidrogi nie ma (albo pojazd jest samochodem), sprawdzić sygnał
        if (Point.iFlags &
            spShuntSemaphor)  // jeśli Tm - w zasadzie to sprawdzić komendę!
        {                     // jeśli podana prędkość manewrowa
            if ((Velocity == 0.0) &&
                (true == TestFlag(OrderCurrentGet(), Obey_train))) {
                // jeśli tryb pociągowy a tarcze ma ShuntVelocity 0 0
                Velocity =
                    -1;  // ignorować, chyba że prędkość stanie się niezerowa
                if (true == TestFlag(Point.iFlags, spElapsed)) {
                    // a jak przejechana to można usunąć, bo podstawowy automat
                    // usuwa tylko niezerowe
                    Point.Clear();
                }
            } else if (Command == TCommandType::cm_Unknown) {
                // jeśli jeszcze nie ma komendy
                // komenda jest tylko gdy ma jechać, bo stoi na podstawie
                // tabelki
                if (Velocity != 0.0) {
                    // jeśli nie było komendy wcześniej - pierwsza się liczy -
                    // ustawianie VelSignal
                    Command = TCommandType::
                        cm_ShuntVelocity;  // w trybie pociągowym tylko jeśli
                                           // włącza tryb manewrowy (v!=0.0)
                    // Ra 2014-06: (VelSignal) nie może być tu ustawiane, bo Tm
                    // może być daleko
                    if (VelSignal == 0.0) {
                        // aby stojący ruszył
                        VelSignal = Velocity;
                    ...
                    if (Point.fDist < 0.0) {
                        // jeśli przejechany
                        //!!! ustawienie, gdy przejechany jest lepsze niż wcale,
            ...
            if (Command == TCommandType::cm_Unknown) {
                ...
                if ((Velocity < 0.0) || (Velocity >= 1.0)) {
                    ...
                    // Ra 2014-06: (VelSignal) nie może być tu ustawiane, bo
                    // semafor może być daleko VelSignal=v; //nie do końca tak,
                    // to jest druga prędkość; -1 nie wpisywać...
                    if (VelSignal == 0.0) {
                        // aby stojący ruszył
                        VelSignal = -1.0;
                    }
                    if (Point.fDist < 0.0) {
                        ...
                        if (Point.iFlags & spEvent) {
                            ...
                            if ((Point.evEvent != eSignSkip) ||
                                ...
                                // ale inny niż ten, na którym minięto S1, chyba
                                // że się już zmieniło sygnał zezwalający na
                                // jazdę wyłącza jazdę na widoczność (po S1 na
                                // SBL)
                                iDrivigFlags &= ~moveVisibility;
                        ...
                        // jeśli nie jest ograniczeniem prędkości to można
                        // usunąć (nie mogą być usuwane w skanowaniu)
                        Point.Clear();
                    }
                    ...
                    // TBD, TODO: revise this check for bank/loose_shunt; we
                    // might want to ignore obstacles with no owner in these
                    // modes
                    if (Point.fDist < Obstacle.distance) {
                        if (true == TestFlag(iDrivigFlags, moveStopHere)) {
                            // jeśli ma stać, dostaje komendę od razu
                            Command =
                                TCommandType::cm_Command;  // komenda z komórki,
                                                           // do wykonania po
                                                           // zatrzymaniu
                        }
                        // NOTE: human drivers are likely to stop wherever and
                        // expect things to still work, so we give them more
                        // leeway
                        else if (
                            Point.fDist <=
                            (AIControllFlag
                                 ? fMaxProximityDist
                                 : std::max(250.0, fMaxProximityDist))) {
                            // jeśli ma dociągnąć, to niech dociąga
                            // (moveStopCloser dotyczy dociągania do W4, nie
                            // semafora)
                            Command =
                                TCommandType::cm_Command;  // komenda z komórki,
                                                           // do wykonania po
...
// modifies brake distance for low target speeds, to ease braking rate in such
// situations
float TController::braking_distance_multiplier(
    float const Targetvelocity) const {
    auto const frictionmultiplier{1.f / Global.FrictionWeatherFactor};

    if (Targetvelocity > 65.f) {
        return 1.f * frictionmultiplier;
    }
    if (Targetvelocity < 5.f) {
        // HACK: engaged automatic transmission means extra/earlier braking
        // effort is needed for the last leg before full stop
        if ((is_dmu()) && (mvOccupied->Vel < 40.0) && (Targetvelocity == 0.f)) {
        ...
        // HACK: cargo trains or trains going downhill with high braking
        // threshold need more distance to come to a full stop
        if ((fBrake_a0[1] > 0.2) &&
            ((true == IsCargoTrain) || (fAccGravity > 0.025))) {
            return (
                interpolate(
                    1.f, 2.f, clamp((fBrake_a0[1] - 0.2) / 0.2, 0.0, 1.0)) *
                frictionmultiplier);
        }

        return 1.f * frictionmultiplier;
    }
    // stretch the braking distance up to 3 times; the lower the speed, the
    // greater the stretch
    return (
        interpolate(3.f, 1.f, (Targetvelocity - 5.f) / 60.f) *
        frictionmultiplier);
}

void TController::TablePurger() {  // odtykacz: usuwa mniej istotne pozycje ze
                                   // środka tabelki, aby uniknąć zatkania
    //(np. brak ograniczenia pomiędzy zwrotnicami, usunięte sygnały, minięte
    //odcinki łuku)
    if (sSpeedTable.size() < 2) {
        return;
    }
    // simplest approach should be good enough for start -- just copy whatever
    // is still relevant, then swap do a trial run first, to see if we need to
    // bother at all
    ...
    for (std::size_t idx = 0; idx < sSpeedTable.size() - 1; ++idx) {
        ...
        if ((0 == (speedpoint.iFlags & spEnabled)) ||
            (((speedpoint.iFlags &
               (spElapsed | spTrack | spCurve | spSwitch)) ==
              (spElapsed | spTrack | spCurve)) &&
             (speedpoint.fVelNext < 0.0))) {
            // NOTE: we could break out early here, but running through entire
            // thing gives us exact size needed for new table
            ++trimcount;
        }
    }
    if (trimcount == 0) {
    ...
    std::vector<TSpeedPos> trimmedtable;
    trimmedtable.reserve(sSpeedTable.size() - trimcount);
    // we can only update pointers safely after new table is finalized, so
    // record their indices until then
    for (std::size_t idx = 0; idx < sSpeedTable.size() - 1; ++idx) {
        ...
        if ((0 == (speedpoint.iFlags & spEnabled)) ||
            (((speedpoint.iFlags &
              ...
              (spElapsed | spTrack | spCurve)) &&
             (speedpoint.fVelNext < 0.0))) {
            // if the trimmed point happens to be currently active semaphor we
            // need to invalidate their placements
            if (idx == SemNextIndex) {
                SemNextIndex = -1;
            }
            if (idx == SemNextStopIndex) {
    ...
    if (Global.iWriteLogEnabled & 8) {
        WriteLog(
            "Speed table garbage collection for " + OwnerName() + " cut away " +
            std::to_string(trimcount) +
            (trimcount == 1 ? " record" : " records"));
    }
    // update the data
...
void TController::TableSort() {
    if (sSpeedTable.size() < 3) {
        // we skip last slot and no point in checking if there's only one other
        // entry
        return;
    }
    ...
    for (int i = 0; i < (iLast - 1); ++i) {
        ...
        if (sSpeedTable[i].fDist >
            sSpeedTable[i + 1]
                .fDist) {  // jesli pozycja wcześniejsza jest dalej to źle
...
TController::TController(
    bool AI, TDynamicObject *NewControll, bool InitPsyche, bool Primary)
    :  // czy ma aktywnie prowadzić?
      AIControllFlag(AI),
      pVehicle(NewControll) {
    if (pVehicle != nullptr) {
        pVehicles[end::front] = pVehicle->GetFirstDynamic(
            end::front);  // pierwszy w kierunku jazdy (Np. Pc1)
        pVehicles[end::rear] = pVehicle->GetFirstDynamic(
            end::rear);  // ostatni w kierunku jazdy (końcówki)
    } else {
    ...
    if (mvOccupied != nullptr) {
        iDirectionOrder =
            mvOccupied->CabActive;  // 1=do przodu (w kierunku sprzęgu 0)
        VehicleName = mvOccupied->Name;

        if (is_car()) {  // samochody: na podst.
                         // http://www.prawko-kwartnik.info/hamowanie.html
            // fDriverBraking=0.0065; //mnożone przez (v^2+40*v) [km/h] daje
            // prawie drogę hamowania [m]
            fDriverBraking = 0.03;  // coś nie hamują te samochody zbyt dobrze
            fDriverDist = 5.0;      // 5m - zachowywany odstęp przed kolizją
            fVelPlus = 10.0;        // dopuszczalne przekroczenie prędkości na
                                    // ograniczeniu bez hamowania
            fVelMinus = 2.0;  // margines prędkości powodujący załączenie napędu
        } else {              // pociągi i statki
            fDriverBraking = 0.06;  // mnożone przez (v^2+40*v) [km/h] daje
                                    // prawie drogę hamowania [m]
            fDriverDist = 50.0;     // 50m - zachowywany odstęp przed kolizją
            fVelPlus = 5.0;         // dopuszczalne przekroczenie prędkości na
                                    // ograniczeniu bez hamowania
            fVelMinus = 5.0;  // margines prędkości powodujący załączenie napędu
        }

        // fAccThreshold może podlegać uczeniu się - hamowanie powinno być
        // rejestrowane, a potem analizowane próg opóźnienia dla zadziałania
        // hamulca
        fAccThreshold = (is_emu() ? -0.55 : is_dmu() ? -0.45 : -0.2);
        /*
                // HACK: emu with induction motors need to start their braking a
           bit sooner than the ones with series motors if( (
           mvOccupied->TrainType == dt_EZT )
                 && ( mvControlling->EngineType ==
           TEngineType::ElectricInductionMotor ) ) { fAccThreshold += 0.10;
                }
        */
    }
...
#endif
        LogFile.open(
            std::string("physicslog/" + VehicleName + ".dat"),
            std::ios::in | std::ios::out | std::ios::trunc);
#if LOGPRESS == 0
        LogFile
            << "Time[s] Velocity[m/s] Acceleration[m/ss] "
            << "Coupler.Dist[m] Coupler.Force[N] TractionForce[kN] "
               "FrictionForce[kN] BrakeForce[kN] "
    ...
    // HACK: give the simulation a small window to potentially replace the AI
    // with a human driver
    fActionTime = -5.0;
};
...
TController::~TController() {  // wykopanie mechanika z roboty
    CloseLog();
};

// zamiana kodu rozkazu na opis
std::string TController::Order2Str(TOrders Order) const {
    if (((Order & Change_direction) != 0) && (Order != Change_direction)) {
        // może być nałożona na inną i wtedy ma priorytet
        return Order2Str(Change_direction) + " + " +
...
std::string TController::OrderCurrent()
    const {  // pobranie aktualnego rozkazu celem wyświetlenia
    ...
    switch (static_cast<TOrders>(order & ~Change_direction)) {
        ...
        case Prepare_engine: {
            return STR("Start the engine");
        }
        case Release_engine: {
            return STR("Shut down the engine");
        }
        case Change_direction: {
            return STR("Change direction");
        }
        case Connect: {
            return STR("Couple to consist ahead");
        }
        case Disconnect: {
            if (iVehicleCount < 0) {
            ...
            if (iVehicleCount > 1) {
                std::snprintf(
                    ...
                    STR_C(
                        "the engine plus %d next vehicles"),  // TODO: implement
                                                              // plural forms
                    count);
            }
            ...
            std::snprintf(
                orderbuffer.data(), orderbuffer.size(), STR_C("Uncouple %s"),
                countstring.c_str());
            return orderbuffer.data();
        }
        case Shunt: {
            return STR("Shunt according to signals");
        }
        case Loose_shunt: {
            return STR("Loose shunt according to signals");
        }
        case Obey_train: {
            return STR("Drive according to signals and timetable");
        }
        case Bank: {
            return STR("Bank consist ahead");
        }
        default: {
            return {};
        }
...
void TController::OrdersClear() {  // czyszczenie tabeli rozkazów na starcie
                                   // albo po dojściu do końca
    OrderPos = 0;
    OrderTop = 1;  // szczyt stosu rozkazów
    for (int b = 0; b < maxorders; b++) OrderList[b] = Wait_for_orders;
#if LOGORDERS
    OrdersDump("OrdersClear", false);
#endif
};

void TController::Activation() {  // umieszczenie obsady w odpowiednim członie,
                                  // wykonywane wyłącznie gdy steruje AI
    iDirection = iDirectionOrder;  // kierunek (względem sprzęgów pojazdu z AI)
                                   // właśnie został ustalony (zmieniony)
    if (iDirection) {  // jeśli jest ustalony kierunek
        ...
        */
        ...
        mvOccupied->CabOccupied =
            mvOccupied
                ->CabActive;  // użytkownik moze zmienić CabOccupied wychodząc
        mvOccupied->CabDeactivisation();  // tak jest w Train.cpp
        if (TestFlag(
                pVehicle->MoverParameters
                    ->Couplers[iDirectionOrder < 0 ? end::rear : end::front]
                    .CouplingFlag,
                coupling::control)) {
            ZeroLocalBrake();
            ...
            // odcięcie na zaworze maszynisty, FVel6 po drugiej stronie nie
            // luzuje
            mvOccupied->BrakeLevelSet(mvOccupied->Handle->GetPos(
                bh_NP));  // odcięcie na zaworze maszynisty
            BrakeLevelSet(mvOccupied->BrakeCtrlPos);  // ustawienie zmiennej GBH
        }
        // przejście AI na drugą stronę EN57, ET41 itp.
        // TODO: clean this up, there's lot of redundancy with
        // TMoverParameters::ChangeCab() and TTrain::MoveToVehicle()
        {
            int movedirection{(iDirection < 0 ? end::rear : end::front)};
            auto *targetvehicle{
                pVehicle->FirstFind(movedirection, coupling::control)};
            if (pVehicle != targetvehicle) {
                auto *targetvehicledriver{
                    targetvehicle->Mechanik};  // zapamiętanie tego, co
                                               // ewentualnie tam siedzi, żeby w
                                               // razie dwóch zamienić miejscami
                // move to the new vehicle
                targetvehicle->Mechanik = this;  // na razie bilokacja
                targetvehicle->MoverParameters->SetInternalCommand(
                    "", 0, 0);  // usunięcie ewentualnie zalegającej komendy
                                // (Change_direction?)
                if (targetvehicle->DirectionGet() != pVehicle->DirectionGet()) {
                    // jeśli są przeciwne do siebie to będziemy jechać w drugą
                    // stronę względem zasiedzianego pojazdu
                    iDirection = -iDirection;
                }
                // move the other driver to our old vehicle
                pVehicle->Mechanik =
                    targetvehicledriver;  // wsadzamy tego, co ewentualnie był
                                          // (podwójna trakcja)
                // NOTE: having caboccupied != 0 (by swapping in the driver from
                // the target vehicle) may lead to dysfunctional brake
                // TODO: investigate and resolve if necessary
                pVehicle->MoverParameters->CabOccupied =
                    targetvehicle->MoverParameters->CabOccupied;
                if (pVehicle->Mechanik) {  // not guaranteed, as target vehicle
                                           // can be empty
                    pVehicle->Mechanik->BrakeLevelSet(
                        pVehicle->MoverParameters->BrakeCtrlPos);
                }
                // finish driver swap
        ...
        if (pVehicle !=
            initialvehicle) {  // jeśli zmieniony został pojazd prowadzony
            ControllingSet();  // utworzenie połączenia do sterowanego pojazdu
                               // (może się zmienić) - silnikowy dla EZT
        ...
        if (mvControlling->EngineType == TEngineType::DieselEngine) {
            // dla 2Ls150 - przed ustawieniem kierunku - można zmienić tryb
            // pracy
            if (mvControlling->ShuntModeAllow) {
                mvControlling->CurrentSwitch(
                    ((OrderCurrentGet() & (Shunt | Loose_shunt)) != 0) ||
                    (fMass > 224000.0));  // do tego na wzniesieniu może nie dać
                                          // rady na liniowym
            }
        }
        // Ra: to przełączanie poniżej jest tu bez sensu
        mvOccupied->CabOccupied =
            iDirection;  // aktywacja kabiny w prowadzonym pojeżdzie (silnikowy
                         // może być odwrotnie?)
        ...
        /*
                // NOTE: this won't restore local brake if the vehicle has
           integrated local brake control
                // TBD, TODO: fix or let the ai activate the brake again as part
           of its standard logic? if (initiallocalbrakelevel > 0.0) // hamowanie
           tylko jeśli był wcześniej zahamowany (bo możliwe, że jedzie!)
                    mvOccupied->LocalBrakePosA = initiallocalbrakelevel; //
           zahamuj jak wcześniej
        */
...
void TController::AutoRewident() {  // autorewident: nastawianie hamulców w
    ...
    if (r + g + p == 0)
        ...
        {           // to skład pasażerski - nastawianie pasażerskiego
            ...
            // inaczej R (2)
        } else {  // inaczej towarowy - nastawianie towarowego
            if ((fLength < 300.0) && (fMass < 600000.0))  //[kg]
                ...
                ustaw |=
                    bdelay_R;  // jeżeli długość<500 oraz masa<1300 to GP (2)
            else
                ustaw |= bdelay_G;  // inaczej G (1)
        }
        // zasadniczo na sieci PKP kilka lat temu na P/GP jeździły tylko
        // kontenerowce o rozkładowej 90 km/h. Pozostałe jeździły 70 km/h i były
        // nastawione na G.
    }
    d = pVehicles[0];  // pojazd na czele składu
    p = 0;       // będziemy tu liczyć wagony od lokomotywy dla nastawy GP
    while (d) {  // 3. Nastawianie
        if ((true == AIControllFlag) || (d != pVehicle)) {
            ...
            switch (ustaw) {
                case bdelay_P: {
                    ...
                    d->MoverParameters->BrakeDelaySwitch(
                        d->MoverParameters->Power > 1 ? bdelay_G : bdelay_P);
                    break;
                }
                case bdelay_G: {
                    // towarowy G - wszystko na G, jeśli nie ma to P (powinno
                    // się wyłączyć hamulec)
                    d->MoverParameters->BrakeDelaySwitch(
    ...
    // ustawianie trybu pracy zadajnika hamulca, wystarczy raz po inicjalizacji
    // AI
    if (true == AIControllFlag) {
        // if a human is in charge leave the brake mode up to them, otherwise do
        // as you like
        for (int i = 1; i <= 8; i *= 2) {
    ...
    if (OrderCurrentGet() & (Shunt | Loose_shunt)) {
        // for uniform behaviour and compatibility with older scenarios set
        // default acceleration table values for shunting
        fAccThreshold = (is_emu() ? -0.55 : is_dmu() ? -0.45 : -0.2);
        // HACK: emu with induction motors need to start their braking a bit
        // sooner than the ones with series motors
        if ((is_emu()) && (mvControlling->EngineType ==
                           TEngineType::ElectricInductionMotor)) {
            fAccThreshold += 0.10;
        }
    ...
    if (OrderCurrentGet() & (Obey_train | Bank)) {
        ...
        while (d) {
            for (int i = 0; i < BrakeAccTableSize; ++i) {
                fBrake_a0[i + 1] += d->MoverParameters->BrakeForceR(
                    0.25, velstep * (1 + 2 * i));
                fBrake_a1[i + 1] += d->MoverParameters->BrakeForceR(
                    1.00, velstep * (1 + 2 * i));
            }
            d = d->Next();  // kolejny pojazd, podłączony od tyłu (licząc od
                            // czoła)
        }
        for (int i = 0; i < BrakeAccTableSize; ++i) {
        ...
        if (is_emu()) {
            auto ep_factor{
                (BrakeSystem == TBrakeSystem::ElectroPneumatic ? 8 : 4)};
            if (mvControlling->EngineType ==
                TEngineType::ElectricInductionMotor) {
                // HACK: emu with induction motors need to start their braking a
                // bit sooner than the ones with series motors
                fNominalAccThreshold = std::max(
                    -0.60, -fBrake_a0[BrakeAccTableSize] -
                               ep_factor * fBrake_a1[BrakeAccTableSize]);
            } else {
                fNominalAccThreshold = std::max(
                    -0.75, -fBrake_a0[BrakeAccTableSize] -
                               ep_factor * fBrake_a1[BrakeAccTableSize]);
            }
            fBrakeReaction = 0.25;
        } else if (is_dmu()) {
            fNominalAccThreshold = std::max(
                -0.75, -fBrake_a0[BrakeAccTableSize] -
                           8 * fBrake_a1[BrakeAccTableSize]);
            fBrakeReaction = 0.25;
        } else if (ustaw > 16) {
            fNominalAccThreshold = -fBrake_a0[BrakeAccTableSize] -
                                   4 * fBrake_a1[BrakeAccTableSize];
            fBrakeReaction = 1.00 + fLength * 0.004;
        } else {
            fNominalAccThreshold = -fBrake_a0[BrakeAccTableSize] -
                                   1 * fBrake_a1[BrakeAccTableSize];
...
double TController::ESMVelocity(bool Main) {
    ...
    if (Main)
        ...
        SCPN += 1;
    if ((mvControlling->RList[MCPN].ScndAct < 255) &&
        (mvControlling->ScndCtrlActualPos == 0))
        SCPN = mvControlling->RList[MCPN].ScndAct;
    double FrictionMax =
        mvControlling->Mass * 9.81 *
        mvControlling->Adhesive(mvControlling->RunningTrack.friction) *
        fFrictionCoeff;
    ...
    for (int i = 0; i < 5; i++) {
        ...
        Fmax = MS * mvControlling->RList[MCPN].Bn *
               mvControlling->RList[MCPN].Mn * 2 /
               mvControlling->WheelDiameter * mvControlling->Transmision.Ratio;
        if (Fmax != 0.0) {
            IF = 0.5 * IF * (1 + FrictionMax / Fmax);
        } else {
            // NOTE: gets trimmed to actual highest acceptable value after the
            // loop
    ...
    double R = mvControlling->RList[MCPN].R + mvControlling->CircuitRes +
               mvControlling->RList[MCPN].Mn * mvControlling->WindingRes;
    double pole =
        mvControlling->MotorParam[SCPN].fi *
        std::max(
            abs(IF) / (abs(IF) + mvControlling->MotorParam[SCPN].Isat) -
                mvControlling->MotorParam[SCPN].fi0,
            0.0);
    double Us = abs(mvControlling->EngineVoltage) - IF * R;
    double ns = std::max(0.0, Us / (pole * mvControlling->RList[MCPN].Mn));
    ESMVel = ns * mvControlling->WheelDiameter * M_PI * 3.6 /
             mvControlling->Transmision.Ratio;
    return ESMVel;
}
...
bool TController::CheckVehicles(
    TOrders user) {     // sprawdzenie stanu posiadanych pojazdów w składzie i
                        // zapalenie świateł
    ...
    pVehicles[end::front] = p =
        pVehicle->FirstFind(d);  // pojazd na czele składu
    ...
    // Ra 2014-09: ustawić moveMultiControl, jeśli wszystkie są w ukrotnieniu (i
    // skrajne mają kabinę?)
    while (p) {  // sprawdzanie, czy jest głównym sterującym, żeby nie było
                 // konfliktu
        if ((p->Mechanik)  // jeśli ma obsadę
            &&
            (p->Mechanik !=
             this)) {  // ale chodzi o inny pojazd, niż aktualnie sprawdzający
            if (p->Mechanik->iDrivigFlags & movePrimary) {
                // a tamten ma priorytet
                // TODO: take into account drivers' operating modes, one or more
                // of them might be on banking duty
                if ((iDrivigFlags & movePrimary) && (mvOccupied->DirAbsolute) &&
                    ...
                    p->MoverParameters->BrakeLevelSet(
                        p->MoverParameters->Handle->GetPos(
                            bh_NP));  // odcięcie na zaworze maszynisty
                    p->Mechanik->BrakeLevelSet(
                        p->MoverParameters
                            ->BrakeCtrlPos);  // ustawienie zmiennej GBH
                } else {
        ...
        fMass +=
            p->MoverParameters->TotalMass;  // dodanie masy łącznie z ładunkiem
        fVelMax = min_speed(
            fVelMax, p->MoverParameters
                         ->Vmax);  // ustalenie maksymalnej prędkości dla składu
    ...
    if (main) {
        iDrivigFlags |=
            movePrimary;  // nie znaleziono innego, można się porządzić
    }

    ControllingSet();  // ustalenie członu do sterowania (może być inny niż
                       // zasiedziany)

    if (iDrivigFlags & movePrimary) {  // jeśli jest aktywnie prowadzącym
                                       // pojazd, może zrobić własny porządek
        auto pantmask = 1;
        p = pVehicles[end::front];
        // establish ownership and vehicle order
        while (p) {
            if (p->MoverParameters->EnginePowerSource.SourceType ==
                TPowerSource::
                    CurrentCollector) {  // jeśli pojazd posiada pantograf, to
                                         // przydzielamy mu maskę, którą będzie
                                         // informował o jeździe bezprądowej
                p->iOverheadMask = pantmask;
                pantmask = pantmask << 1;  // przesunięcie bitów, max. 32
                                           // pojazdy z pantografami w składzie
            }

            d = p->DirectionSet(
                d ? 1 : -1);  // zwraca położenie następnego
                              // (1=zgodny,0=odwrócony - względem czoła składu)
            p->ctOwner = this;  // dominator oznacza swoje terytorium
            p = p->Next();  // pojazd podłączony od tyłu (licząc od czoła)
        }
        // with the order established the virtual train manager can do their
        // work
        p = pVehicles[end::front];
        ...
        while (p) {
            if (p != pVehicle) {
                if (false == p->is_connected(pVehicle, coupling::control)) {
                    // NOTE: don't set battery in controllable vehicles, let the
                    // user/ai do it explicitly HACK: wagony muszą mieć baterię
                    // załączoną do otwarcia drzwi...
                    p->MoverParameters->BatterySwitch(true);
                    // enable heating and converter in carriages with can be
                    // heated
                    if (p->MoverParameters->HeatingPower > 0) {
                        p->MoverParameters->HeatingAllow = true;
                        p->MoverParameters->ConverterSwitch(
                            true, range_t::local);
                        hasheaters = true;
                    }
                } else {
            ...
            if (p->asDestination == "none") {
                p->DestinationSet(
                    TrainParams.Relation2,
                    TrainParams.TrainName);  // relacja docelowa, jeśli nie było
            }
        ...
        if ((user == Connect) && (true == main)) {
            // HACK: with additional vehicles in the consist ensure all linked
            // vehicles are set to move in the same direction
            if ((pVehicle->Prev(coupling::control) != nullptr) ||
                (pVehicle->Next(coupling::control) != nullptr)) {
                sync_consist_reversers();
            }
            // potentially sync compartment lighting state for the newly
            // connected vehicles
            if (mvOccupied->CompartmentLights.start_type == start_t::manual) {
                mvOccupied->CompartmentLightsSwitchOff(
                    mvOccupied->CompartmentLights.is_disabled);
                mvOccupied->CompartmentLightsSwitch(
                    mvOccupied->CompartmentLights.is_enabled);
            }
        }

        // kasowanie pamieci hamowania kontrolnego
        if (OrderCurrentGet() &
            (Shunt | Loose_shunt | Disconnect | Connect | Change_direction)) {
            DynamicBrakeTest = 0;
            DBT_VelocityBrake = DBT_VelocityRelease = DBT_VelocityFinish =
                DBT_BrakingTime = DBT_ReleasingTime = DBT_MidPointAcc = 0;
        }
        ...
        if (OrderCurrentGet() & (Shunt | Loose_shunt | Obey_train | Bank)) {
            ...
            /*
                        if( ( true == TestFlag( iDrivigFlags, moveConnect ) )
                            ...
                            iDrivigFlags &= ~moveConnect; // zdjęcie flagi
               doczepiania SetVelocity( 0, 0, stopJoin ); // wyłączyć
               przyspieszanie JumpToNextOrder(); // wykonanie następnej komendy
                        }
            */
        }

        // detect push-pull train configurations and mark them accordingly
        if (pVehicles[end::front]->is_connected(
                pVehicles[end::rear], coupling::control)) {
        ...
        // HACK: ensure vehicle lights are active from the beginning, if it had
        ...
        if (iEngineActive) {
            ...
            control_lights();
            // custom ai action for disconnect mode: switch off lights on
            // disconnected vehicle(s)
            if (is_train()) {
                if (true == TestFlag(OrderCurrentGet(), Disconnect)) {
                    if (AIControllFlag) {
                        // światła manewrowe (Tb1) tylko z przodu, aby nie
                        // pozostawić odczepionego ze światłem
                        if (mvOccupied->DirActive >=
                            0) {  // jak ma kierunek do przodu
            ...
            {
                auto const ispassengertrain{
                    (IsPassengerTrain) &&
                    (iVehicles - ControlledEnginesCount > 0)};
                // TODO: replace connection test with connection check between
                // last engine and first car, specifically
                auto const isheatingcouplingactive{
                    (ControlledEnginesCount == 1
                         ? pVehicles[end::front]->is_connected(
                               pVehicles[end::rear], coupling::heating)
                         : true)};
                ...
                if (mvControlling->HeatingAllow != isheatingneeded) {
                    cue_action(
                        isheatingneeded ? driver_hint::consistheatingon
                                        : driver_hint::consistheatingoff);
                }
            }
        }

        if ((user == Connect) || (user == Disconnect)) {
            // HACK: force route table update on consist change, new consist
            // length means distances to points of interest are now wrong
            iTableDirection = 0;
        }
...
void TController::Lights(int head, int rear) {  // zapalenie świateł w skłądzie
    pVehicles[end::front]->RaLightsSet(
        head, -1);  // zapalenie przednich w pierwszym
    pVehicles[end::rear]->RaLightsSet(
        -1, rear);  // zapalenie końcówek w ostatnim
}

void TController::DirectionInitial() {  // ustawienie kierunku po wczytaniu
                                        // trainset (może jechać na wstecznym
    mvOccupied
        ->CabActivisationAuto();  // załączenie rozrządu (wirtualne kabiny)
    if (mvOccupied->Vel > EU07_AI_NOMOVEMENT) {  // jeśli na starcie jedzie
        iDirection = iDirectionOrder =
            (mvOccupied->V > 0
                 ? 1
                 : -1);  // początkowa prędkość wymusza kierunek jazdy
        DirectionForward(
            mvOccupied->V * mvOccupied->CabActive >=
            0.0);  // a dalej ustawienie nawrotnika
    }
    CheckVehicles();  // sprawdzenie świateł oraz skrajnych pojazdów do
                      // skanowania
};
...
TBrakeSystem TController::consist_brake_system() const {
    if (mvOccupied->BrakeSystem != TBrakeSystem::ElectroPneumatic) {
        return mvOccupied->BrakeSystem;
    }

    auto isepcapable = true;
    if (pVehicles[end::front] != pVehicles[end::rear]) {
        // more detailed version, will use manual braking also for coupled sets
        // of controlled vehicles
        auto *vehicle = pVehicles[end::front];  // start from first
        while ((true == isepcapable) && (vehicle != nullptr)) {
            // NOTE: we could simplify this by doing only check of the rear
            // coupler, but this can be quite tricky in itself
            // TODO: add easier ways to access front/rear coupler taking into
            // account vehicle's direction
            isepcapable =
                (((vehicle->MoverParameters->Couplers[end::front].Connected ==
                   nullptr) ||
                  ((vehicle->MoverParameters->Couplers[end::front]
                        .CouplingFlag &
                    coupling::control) &&
                   (vehicle->MoverParameters->Couplers[end::front]
                        .Connected->Power > -1))) &&
                 ((vehicle->MoverParameters->Couplers[end::rear].Connected ==
                   nullptr) ||
                  ((vehicle->MoverParameters->Couplers[end::rear].CouplingFlag &
                    coupling::control) &&
                   (vehicle->MoverParameters->Couplers[end::rear]
                        .Connected->Power > -1))));
            vehicle = vehicle->Next();  // kolejny pojazd, podłączony od tyłu
                                        // (licząc od czoła)
        }
    }
    return (
        isepcapable ? TBrakeSystem::ElectroPneumatic : TBrakeSystem::Pneumatic);
}

int TController::OrderDirectionChange(
    int newdir, TMoverParameters
                    *Vehicle) {  // zmiana kierunku jazdy, niezależnie od kabiny
    int testd = newdir;
    if (Vehicle->Vel <
        0.5) {  // jeśli prawie stoi, można zmienić kierunek, musi być wykonane
                // dwukrotnie, bo za pierwszym razem daje na zero
        switch (
            newdir *
            Vehicle->CabActive) {  // DirectionBackward() i DirectionForward()
                                   // to zmiany względem kabiny
        ...
        if (testd == 0)
            VelforDriver =
                -1;  // kierunek został zmieniony na żądany, można jechać
    } else           // jeśli jedzie
        VelforDriver = 0;  // ma się zatrzymać w celu zmiany kierunku
    if ((Vehicle->DirActive == 0) &&
        (VelforDriver < Vehicle->Vel))  // Ra: to jest chyba bez sensu
    ...
    if (is_emu())
        if (Vehicle->DirActive > 0)
            // if () //tylko jeśli jazda pociągowa (tego nie wiemy w momencie
            // odpalania silnika)
...
void TController::WaitingSet(
    double Seconds) {      // ustawienie odczekania po zatrzymaniu (ustawienie w
                           // trakcie jazdy zatrzyma)
    fStopTime = -Seconds;  // ujemna wartość oznacza oczekiwanie (potem >=0.0)
}

void TController::SetVelocity(
    double NewVel, double NewVelNext,
    TStopReason r) {                   // ustawienie nowej prędkości
    WaitingTime = -WaitingExpireTime;  // przypisujemy -WaitingExpireTime, a
                                       // potem porównujemy z zerem
    if (NewVel == 0.0) {               // jeśli ma stanąć
        ...
        if ((is_train())  // tylko pociągi trąbią (unimogi tylko na torach, więc
                          // trzeba raczej sprawdzać tor)
            && ((OrderCurrentGet() & (Shunt | Loose_shunt | Obey_train | Bank |
                                      Connect | Prepare_engine)) !=
                0)  // jeśli jedzie w dowolnym trybie
            &&
            ((iDrivigFlags & moveStartHorn) != 0)  // jezeli trąbienie włączone
            &&
            ((iDrivigFlags & (moveStartHornDone | moveConnect)) ==
             0)  // jeśli nie zatrąbione i nie jest to moment podłączania składu
            &&
            (mvOccupied->Vel < 1.0)  // jesli stoi (na razie, bo chyba powinien
                                     // też, gdy hamuje przed semaforem)
            && ((NewVel >= 1.0) ||
                (NewVel <
                 0.0))) {  // o ile prędkość jest znacząca zatrąb po odhamowaniu
...
double TController::BrakeAccFactor() const {
    ...
    if ((fAccThreshold != 0.0) && (AccDesired < 0.0) &&
        ((ActualProximityDist > fMinProximityDist) ||
         (mvOccupied->Vel > VelDesired + fVelPlus))) {
        Factor +=
            (fBrakeReaction *
             (/*mvOccupied->BrakeCtrlPosR*/ BrakeCtrlPosition < 0.5 ? 1.5
                                                                    : 1)) *
            mvOccupied->Vel / (std::max(0.0, ActualProximityDist) + 1) *
            ((AccDesired - AbsAccS) / fAccThreshold);
    }
    /*
            if (mvOccupied->TrainType == dt_DMU && mvOccupied->Vel > 40 &&
       VelNext<40) Factor *= 1 + 0.25 * ( (1600 - VelNext * VelNext) /
       (mvOccupied->Vel * mvOccupied->Vel) );
    */
    return Factor;
}
...
void TController::SetDriverPsyche() {
    if ((Psyche == Aggressive) && (OrderCurrentGet() == Obey_train)) {
        ReactionTime =
            HardReactionTime;  // w zaleznosci od charakteru maszynisty
        if (is_car()) {
...
bool TController::PrepareEngine() {  // odpalanie silnika
    // HACK: don't immediately activate inert vehicle in case the simulation is
    // about to replace us with human driver
    if ((mvOccupied->Vel < 1.0) && (fActionTime < 0.0)) {
        return false;
    }
    ...
    ReactionTime =
        (mvOccupied->Vel < 5
             ? PrepareTime
             : EasyReactionTime);  // react faster with rolling start
    ...
    if ((mvPantographUnit->EnginePowerSource.SourceType ==
         TPowerSource::CurrentCollector) &&
        (mvPantographUnit->EnginePowerSource.CollectorParameters.CollectorsNo >
         0)) {
        // if our pantograph unit isn't a pantograph-devoid fallback
        auto const sufficientpantographpressure{
            (is_emu() ? 2.5 :  // Ra 2013-12: Niebugocław mówi, że w EZT
                               // podnoszą się przy 2.5
                 3.5) +
            0.1};  // leeway margin
        ...
        if (mvPantographUnit->PantPress < sufficientpantographpressure) {
            ...
            if (false == mvPantographUnit->PantAutoValve) {
                // odłączenie zbiornika głównego, bo z nim nie da rady
                // napompować
                cue_action(driver_hint::pantographairsourcesetauxiliary);
            }
            cue_action(
                driver_hint::pantographcompressoron);  // załączenie sprężarki
                                                       // pantografów
            ...
            if ((false == mvPantographUnit->bPantKurek3) ||
                (mvPantographUnit->PantPress <=
                 mvPantographUnit->ScndPipePress)) {  // kurek przełączony albo
                                                      // główna już pompuje
                cue_action(
                    driver_hint::pantographcompressoroff);  // sprężarkę
                                                            // pantografów można
                                                            // już wyłączyć
            }
        }
        // TODO: make pantograph setup a part of control_pantographs() and call
        // it here instead
        if ((fOverhead2 == -1.0) && (iOverheadDown == 0)) {
            cue_action(driver_hint::pantographsvalveon);
            cue_action(
                driver_hint::frontpantographvalveon, (iDirection >= 0 ? 5 : 0));
            cue_action(
                driver_hint::rearpantographvalveon, (iDirection >= 0 ? 0 : 5));
        }
    }

    auto const ispoweravailable =
        (mvControlling->EnginePowerSource.SourceType !=
         TPowerSource::CurrentCollector) ||
        (std::max(
             mvControlling->GetTrainsetHighVoltage(),
             mvControlling->PantographVoltage) >
         mvControlling->EnginePowerSource.CollectorParameters.MinV);

    bool isready = false;

    if ((IsHeatingTemperatureOK) && (ispoweravailable)) {
        ...
        if (IsAnyConverterOverloadRelayOpen) {
            ...
            cue_action(driver_hint::compressoroff);  // TODO: discern whether
                                                     // compressor needs
                                                     // converter to operate
            cue_action(driver_hint::converteroff);
            cue_action(
                driver_hint::primaryconverteroverloadreset);  // reset
                                                              // nadmiarowego
        }
        if (IsAnyLineBreakerOpen) {
            ...
            if (false == IsAnyConverterOverloadRelayOpen) {
                ...
                if ((mvControlling->ScndPipePress < 4.5) &&
                    (mvControlling->VeselVolume > 0.0)) {
            ...
            if (mvOccupied->BrakeHandle != TBrakeHandle::NoHandle) {
                std::unordered_map<int, int> const brakepositions{
                    {static_cast<int>(mvOccupied->Handle->GetPos(bh_RP)),
                     gbh_RP},
                    {static_cast<int>(mvOccupied->Handle->GetPos(bh_NP)),
                     gbh_NP},
                    {static_cast<int>(mvOccupied->Handle->GetPos(bh_FS)),
                     gbh_FS}};
                auto const lookup{brakepositions.find(
                    static_cast<int>(mvOccupied->fBrakeCtrlPos))};
                if (lookup != brakepositions.end()) {
                    BrakeLevelSet(lookup->second);  // GBH
                }
            }
            // set up train brake
            if (mvOccupied->fBrakeCtrlPos !=
                mvOccupied->Handle->GetPos(bh_RP)) {
                if ((!mvOccupied->Handle->Time) ||
                    (mvOccupied->Handle->GetCP() <
                     mvOccupied->HighPipePress - 0.05))
                    ;
                cue_action(driver_hint::trainbrakerelease);
            }
            // sync spring brake state across consist
            cue_action(
                mvOccupied->SpringBrake.Activate ? driver_hint::springbrakeon
                                                 : driver_hint::springbrakeoff);
        }
        isready = (false == IsAnyConverterOverloadRelayOpen) &&
                  (mvOccupied->DirActive != 0) &&
                  (false == IsAnyLineBreakerOpen) &&
                  ((false == IsAnyConverterPresent) ||
                   (true == IsAnyConverterEnabled)) &&
                  ((false == IsAnyCompressorPresent) ||
                   (true == IsAnyCompressorEnabled)) &&
                  ((mvControlling->ScndPipePress > 4.5) ||
                   (mvControlling->VeselVolume == 0.0)) &&
                  ((static_cast<int>(mvOccupied->fBrakeCtrlPos) ==
                    static_cast<int>(mvOccupied->Handle->GetPos(bh_RP))) ||
                   (static_cast<int>(mvOccupied->fBrakeCtrlPos) !=
...
// wyłączanie silnika (test wyłączenia, a część wykonawcza tylko jeśli steruje
// komputer)
bool TController::ReleaseEngine() {
    /*
        if( mvOccupied->Vel > 0.01 ) {
            ...
            if( true == AIControllFlag ) {
                // TBD, TODO: make a dedicated braking procedure out of it for
       potential reuse ZeroSpeed(); IncBrake(); ReactionTime = 0.1;
            }
    ...
    */
    ...
    if (OK) {
        ...
        iDrivigFlags &=
            ~moveActive;  // ma nie skanować sygnałów i nie reagować na komendy
...
bool TController::IncBrake() {  // zwiększenie hamowania
    auto OK{false};
    auto const bs{
    ...
    switch (bs) {
        case TBrakeSystem::Individual: {
            if (mvOccupied->LocalBrake == TLocalBrake::ManualBrake) {
                OK = mvOccupied->IncManualBrakeLevel(
                    1 +
                    static_cast<int>(std::floor(0.5 + std::fabs(AccDesired))));
            } else {
                OK = mvOccupied->IncLocalBrakeLevel(
                    std::floor(1.5 + std::abs(AccDesired)));
            }
            break;
        }
        case TBrakeSystem::Pneumatic: {
            ...
            // NOTE: can't perform just test whether connected vehicle ==
            // nullptr, due to virtual couplers formed with nearby vehicles
            ...
            if ((mvOccupied->TrainType == dt_ET41) ||
                ...
                // they'll break if someone does strange thing like turning
                // around the second unit
                if ((mvOccupied->Couplers[end::rear].CouplingFlag &
                     coupling::permanent) &&
                    (mvOccupied->Couplers[end::rear]
                         .Connected->Couplers[end::rear]
                         .Connected != nullptr)) {
                    standalone = false;
                }
                if ((mvOccupied->Couplers[end::front].CouplingFlag &
                     coupling::permanent) &&
                    (mvOccupied->Couplers[end::front]
                         .Connected->Couplers[end::front]
                         .Connected != nullptr)) {
                    standalone = false;
                }
            } else if (is_dmu()) {
                ...
                /*
                                standalone =
                                    ( ( mvOccupied->Couplers[ 0 ].CouplingFlag
                   == 0 )
                                   && ( mvOccupied->Couplers[ 1 ].CouplingFlag
                   == 0 ) );
                */
                if (pVehicles[end::front] != pVehicles[end::rear]) {
                    // more detailed version, will use manual braking also for
                    // coupled sets of controlled vehicles
                    auto *vehicle = pVehicles[end::front];  // start from first
                    while ((true == standalone) && (vehicle != nullptr)) {
                        // NOTE: we could simplify this by doing only check of
                        // the rear coupler, but this can be quite tricky in
                        // itself
                        // TODO: add easier ways to access front/rear coupler
                        // taking into account vehicle's direction
                        standalone =
                            (((vehicle->MoverParameters->Couplers[end::front]
                                   .Connected == nullptr) ||
                              ((vehicle->MoverParameters->Couplers[end::front]
                             ...
                             ((vehicle->MoverParameters->Couplers[end::rear]
                              ...
                              ((vehicle->MoverParameters->Couplers[end::rear]
                               ...
                               (vehicle->MoverParameters->Couplers[end::rear]
                                    .Connected->Power > 1))));
                        vehicle =
                            vehicle->Next();  // kolejny pojazd, podłączony od
                                              // tyłu (licząc od czoła)
                    }
                }
            }
            ...
            if ((true == standalone) && (false == ForcePNBrake)) {
                if (mvControlling->EIMCtrlType > 0) {
                    ...
                    OK = mvOccupied->IncLocalBrakeLevel(
                        1 + static_cast<int>(std::floor(
                                0.5 +
                                std::fabs(AccDesired))));  // hamowanie lokalnym
                                                           // bo luzem jedzie
                }
            } else {
                if (/*GBH mvOccupied->BrakeCtrlPos*/ BrakeCtrlPosition + 1 ==
                    ...
                    TDynamicObject *d;
                    d = pVehicles[0];  // pojazd na czele składu
                    while (d) {  // przeliczanie dodatkowego potrzebnego spadku
                                 // ciśnienia
                        if ((d->MoverParameters->Hamulec->GetBrakeStatus() &
                             b_dmg) == 0) {
                            pos_corr -=
                                (std::min(
                                     5.0,
                                 ...
                                 5.0) *
                                d->MoverParameters->TotalMass;
                        }
                        d = d->Next();  // kolejny pojazd, podłączony od tyłu
                                        // (licząc od czoła)
                    }
                    ...
                    double deltaAcc =
                        -AccDesired * BrakeAccFactor() -
                        (fBrake_a0[0] + 4.0 *
                                            (/*GBH mvOccupied->BrakeCtrlPosR*/
                                             BrakeCtrlPosition - 1 - pos_corr) *
                                            fBrake_a1[0]);

                    if (deltaAcc > fBrake_a1[0]) {
                        if (/*GBH mvOccupied->BrakeCtrlPosR*/
                            BrakeCtrlPosition < 0.1) {
                            OK = /*mvOccupied->*/ BrakeLevelAdd(
                                BrakingInitialLevel);  // GBH
                            // HACK: stronger braking to overcome SA134 engine
                            // behaviour
                            if ((is_dmu()) && (VelNext == 0.0) &&
                                (fBrakeDist < 200.0)) {
                                BrakeLevelAdd(
                            ...
                            OK = /*mvOccupied->*/ BrakeLevelAdd(
                                BrakingLevelIncrease);  // GBH
                            // brake harder if the acceleration is much higher
                            // than desired
                            /*if( ( deltaAcc > 2 * fBrake_a1[ 0 ] )
                             && ( mvOccupied->BrakeCtrlPosR +
                            BrakingLevelIncrease <= 5.0 ) ) {
                                mvOccupied->BrakeLevelAdd( BrakingLevelIncrease
                            ); }  GBH */
                            if ((deltaAcc > 2 * fBrake_a1[0]) &&
                                (BrakeCtrlPosition + BrakingLevelIncrease <=
                                 5.0)) {
        ...
        case TBrakeSystem::ElectroPneumatic: {
            while ((mvOccupied->BrakeOpModeFlag << 1) <=
                   mvOccupied->BrakeOpModes) {
                mvOccupied->BrakeOpModeFlag <<= 1;
            }
            if (mvOccupied->EngineType == TEngineType::ElectricInductionMotor) {
                if (mvOccupied->BrakeHandle == TBrakeHandle::MHZ_EN57) {
                    if (mvOccupied->BrakeCtrlPos <
                        mvOccupied->Handle->GetPos(bh_FB))
                ...
                auto const AccMax{std::min(
                    fBrake_a0[0] + 12 * fBrake_a1[0], mvOccupied->MED_amax)};
                mvOccupied->BrakeLevelSet(interpolate(
                    mvOccupied->Handle->GetPos(bh_EPR),
                    mvOccupied->Handle->GetPos(bh_EPB),
                    clamp(
                        -AccDesired / AccMax *
                            mvOccupied->AIHintLocalBrakeAccFactor,
                        0.0, 1.0)));
...
bool TController::IncBrakeEIM() {  // zwiększenie hamowania
    bool OK = false;
    switch (mvOccupied->EIMCtrlType) {
        case 0: {
            if (mvOccupied->MED_amax != 9.81) {
                auto const maxpos{mvOccupied->EIMCtrlEmergency ? 0.9 : 1.0};
                auto const brakelimit{
                    -2.2 * AccDesired / fMedAmax -
                    1.0};  // additional limit when hinted is too low
                auto const brakehinted{
                    -1.0 * mvOccupied->AIHintLocalBrakeAccFactor * AccDesired /
                    fMedAmax};  // preffered by AI
                auto const brakeposition{
                    maxpos *
                    clamp(std::max(brakelimit, brakehinted), 0.0, 1.0)};
        ...
        case 1: {
            ...
            if (OK) mvOccupied->MainCtrlPos = 0;
            break;
        }
        case 2: {
            OK = mvOccupied->MainCtrlPos > 1;
            if (OK) mvOccupied->MainCtrlPos = 1;
            break;
        }
        case 3: {
...
bool TController::DecBrake() {
    ...
    switch (bs) {
        case TBrakeSystem::Individual: {
            auto const positionchange{
                1 + std::floor(0.5 + std::abs(AccDesired))};
            OK =
        ...
        case TBrakeSystem::Pneumatic: {
            auto deltaAcc{-1.0};
            if ((fBrake_a0[0] != 0.0) || (fBrake_a1[0] != 0.0)) {
                auto const pos_diff{(is_dmu() ? 0.25 : 1.0)};
                deltaAcc =
                    -AccDesired * BrakeAccFactor() -
                    (fBrake_a0[0] +
                     4 *
                         (/*GBH mvOccupied->BrakeCtrlPosR*/ BrakeCtrlPosition -
                          pos_diff) *
                         fBrake_a1[0]);
            }
            if (deltaAcc < 0) {
...
bool TController::DecBrakeEIM() {  // zmniejszenie siły hamowania
    bool OK = false;
    switch (mvOccupied->EIMCtrlType) {
        case 0: {
            if (mvOccupied->MED_amax != 9.81) {
                auto const desiredacceleration{
                    (mvOccupied->Vel > EU07_AI_NOMOVEMENT
                         ? AccDesired
                         : std::max(0.0, AccDesired))};
                auto const brakeposition{clamp(
                    -1.0 * mvOccupied->AIHintLocalBrakeAccFactor *
                        desiredacceleration / mvOccupied->MED_amax,
                    0.0, 1.0)};
                OK = (brakeposition != mvOccupied->LocalBrakePosA);
                mvOccupied->LocalBrakePosA = brakeposition;
            } else {
        ...
        case 1: {
            ...
            if (OK) mvOccupied->MainCtrlPos = 2;
            break;
        }
        case 2: {
            OK = mvOccupied->MainCtrlPos < 3;
            if (OK) mvOccupied->MainCtrlPos = 3;
            break;
        }
        case 3: {
...
bool TController::IncSpeed() {  // zwiększenie prędkości; zwraca false, jeśli
                                // dalej się nie da zwiększać
    auto OK{false};
    switch (mvOccupied->EngineType) {
        ...
        case TEngineType::ElectricSeriesMotor:
            if (mvPantographUnit->EnginePowerSource.SourceType ==
                TPowerSource::CurrentCollector)  // jeśli pantografujący
            {
                if (fOverhead2 >= 0.0)  // a jazda bezprądowa ustawiana eventami
                                        // (albo opuszczenie)
                    return false;   // to nici z ruszania
                if (iOverheadZero)  // jazda bezprądowa z poziomu toru ustawia
                                    // bity
                    return false;   // to nici z ruszania
            }
            // TODO: move all fVoltage assignments to a single, more generic
            // place
            if (mvControlling->EnginePowerSource.SourceType ==
                TPowerSource::Accumulator) {
            ...
            if ((!IsAnyMotorOverloadRelayOpen) &&
                ...
                if ((mvControlling->IsMainCtrlNoPowerPos()) ||
                    (mvControlling
                         ->StLinFlag)) {  // youBy polecił dodać 2012-09-08 v367
                    // na pozycji 0 przejdzie, a na pozostałych będzie czekać,
                    // aż się załączą liniowe (zgaśnie DelayCtrlFlag)
                    if (Ready || (iDrivigFlags & movePress)) {
                        auto const usehighoverloadrelaythreshold{
                            (mvControlling->TrainType !=
                             dt_ET42)  // ET42 uses these variables for
                                       // different purpose. TODO: fix this
                            &&
                            (mvControlling->ImaxHi > mvControlling->ImaxLo) &&
                            (mvOccupied->Vel <
                             (mvControlling
                                      ->IsMotorOverloadRelayHighThresholdOn()
                                  ? 30.0
                                  : 20.0)) &&
                            (iVehicles - ControlledEnginesCount > 0)
                            //                     && ( std::fabs(
                            //                     mvControlling->Im ) > 0.85 *
                            //                     mvControlling->Imax )
                            && ((mvControlling->Imax *
                                 mvControlling->EngineVoltage *
                                 ControlledEnginesCount) /
                                    (fMass * (fAccGravity == 0.025
                                                  ? -0.01
                                                  :  // prevent div/0
                                                  fAccGravity - 0.025)) <
                                -2.8)};
                        ...
                        // to build up speed to 30/40 km/h for passenger/cargo
                        // train (10 km/h less if going uphill)
                        auto const sufficienttractionforce{
                            std::abs(mvControlling->Ft) *
                                ControlledEnginesCount >
                            (IsHeavyCargoTrain ? 75 : 50) * 1000.0};
                        auto const sufficientacceleration{
                            AbsAccS >= (IsHeavyCargoTrain ? 0.03
                                        : IsCargoTrain    ? 0.06
                                                          : 0.09)};
                        auto const seriesmodefieldshunting{
                            (mvControlling->ScndCtrlPos > 0) &&
                            (mvControlling->RList[mvControlling->MainCtrlPos]
                                 .Bn == 1)};
                        auto const parallelmodefieldshunting{
                            (mvControlling->ScndCtrlPos > 0) &&
                            (mvControlling->RList[mvControlling->MainCtrlPos]
                                 .Bn > 1)};
                        auto const minvoltage{
                            (mvPantographUnit->EnginePowerSource.SourceType ==
                                     TPowerSource::CurrentCollector
                                 ? mvPantographUnit->EnginePowerSource
                                       .CollectorParameters.MinV
                                 : 0.0)};
                        auto const maxvoltage{
                            (mvPantographUnit->EnginePowerSource.SourceType ==
                                     TPowerSource::CurrentCollector
                                 ? mvPantographUnit->EnginePowerSource
                                       .CollectorParameters.MaxV
                                 : 0.0)};
                        auto const seriesmodevoltage{interpolate(
                        ...
                        auto const useseriesmode =
                            ((mvControlling->Imax > mvControlling->ImaxLo) ||
                             ...
                             ((true == sufficientacceleration) &&
                              ...
                              (mvOccupied->Vel <=
                               (IsCargoTrain ? 40 : 30) +
                                   (seriesmodefieldshunting ? 5 : 0) -
                                   ((fAccGravity < -0.025) ? 10 : 0))));
                        // when not in series mode use the first available
                        // parallel mode configuration until 50/60 km/h for
                        // passenger/cargo train (if there's only one parallel
                        // mode configuration it'll be used regardless of
                        // current speed)
                        auto const usefieldshunting =
                            ((mvControlling->StLinFlag) &&
                             ...
                             (useseriesmode
                                  ? mvControlling
                                            ->RList[mvControlling->MainCtrlPos]
                                            .Bn == 1
                                  : ((true == sufficientacceleration) &&
                                             (true ==
                                              sufficienttractionforce) &&
                                             (mvOccupied->Vel <=
                                              (IsCargoTrain ? 60 : 50) +
                                                  (parallelmodefieldshunting
                                                       ? 5
                                                       : 0))
                                         ? mvControlling
                                                   ->RList[mvControlling
                                         ...
                                         : mvControlling->MainCtrlPos ==
                                               mvControlling->MainCtrlPosNo)));

                        // if needed enable high threshold for overload relay...
                        if (mvControlling->TrainType !=
                            dt_ET42) {  // ET42 uses these variables for
                                        // different purpose. TODO: fix this
                            if (usehighoverloadrelaythreshold) {
                                if (mvControlling->Imax <
                                    mvControlling->ImaxHi) {
                                    // to enable this setting we'll typically
                                    // need the main controller in series mode
                                    ...
                                    if (mvControlling
                                        ...
                                        while ((false ==
                                            ...
                                            mvControlling->DecMainCtrl(
                                                1);  // kręcimy nastawnik jazdy
                                                     // o 1 wstecz
                                        }
                                    }
                                    mvControlling->CurrentSwitch(
                                        true);  // rozruch wysoki (za to może
                                                // się ślizgać)
                                }
                            }
                            // ...or disable high threshold for overload relay
                            // if no longer needed
                            else {
                                if ((mvControlling
                                         ->IsMotorOverloadRelayHighThresholdOn()) &&
                                    (std::fabs(mvControlling->Im) <
                                     mvControlling->ImaxLo)) {
                                    mvControlling->CurrentSwitch(
                                        false);  // rozruch wysoki wyłącz
                                }
                            }
                        }

                        double Vs = 99999;
                        if (usefieldshunting ? (mvControlling->ScndCtrlPos <
                                                mvControlling->ScndCtrlPosNo)
                                             : (mvControlling->MainCtrlPos <
                                                mvControlling->MainCtrlPosNo)) {
                            Vs = ESMVelocity(!usefieldshunting);
                        }

                        if ((std::abs(mvControlling->Im) <
                             (fReady < 0.4 ? mvControlling->Imin
                                           : mvControlling->IminLo)) ||
                            (mvControlling->Vel > Vs)) {
                            // Ra: wywalał nadmiarowy, bo Im może być ujemne;
                            // jak nie odhamowany, to nie przesadzać z prądem
                            if (usefieldshunting) {
                                // to dać bocznik
                                // engage the shuntfield only if there's
                                // sufficient power margin to draw from
                                auto const sufficientpowermargin{
                                    fVoltage - seriesmodevoltage >
                                    (IsHeavyCargoTrain ? 100.0 : 75.0) *
                                        ControlledEnginesCount};
                                ...
                                // jeśli ustawiony bocznik to bocznik na zero po
                                // chamsku
                                ...
                                // keep from dropping into series mode when
                                // entering/using parallel mode, and from
                                // shutting down in the series mode
                                auto const sufficientpowermargin{
                                    fVoltage -
                                        (mvControlling
                                                     ->RList[std::min(
                                ...
                                OK =
                                    ((sufficientpowermargin &&
                                      (false == mvControlling->DelayCtrlFlag))
                                         ...
                                         : true);
                                // czekaj na 1 pozycji, zanim się nie włączą
                                // liniowe
                                ...
                                if ((mvControlling->Im == 0) &&
                                    ...
                                    // nie załączona lokomotywa albo wywalił
                                    // nadmiarowy
                                    Need_TryAgain = true;
                                }
                            }
            ...
            //        mvControlling->AutoRelayCheck(); // sprawdzenie logiki
            //        sterowania
            break;
        case TEngineType::Dumb:
            if (!IsAnyMotorOverloadRelayOpen)
                if (Ready || (iDrivigFlags &
                              movePress))  //{(BrakePress<=0.01*MaxBrakePress)}
                {
                    OK = IncSpeedEIM();
                    if (!OK) OK = mvControlling->IncMainCtrl(1);
                    if (!OK) OK = mvControlling->IncScndCtrl(1);
                }
            break;
        ...
        case TEngineType::ElectricInductionMotor:
            if (!IsAnyMotorOverloadRelayOpen) {
                if (Ready || (iDrivigFlags & movePress) ||
                    (mvOccupied
                         ->ShuntMode))  //{(BrakePress<=0.01*MaxBrakePress)}
                {
                    OK = IncSpeedEIM();
                    if ((mvControlling->SpeedCtrl) && (mvControlling->Mains)) {
                        // cruise control
                        auto const couplinginprogress{
                            (true == TestFlag(iDrivigFlags, moveConnect)) &&
                            (true == TestFlag(OrderCurrentGet(), Connect))};
                        auto const SpeedCntrlVel{
                            ((ActualProximityDist >
                              std::max(50.0, fMaxProximityDist)) ||
                                     (couplinginprogress)
                                 ? VelDesired
                                 : min_speed(VelDesired, VelNext))};
                        if ((SpeedCntrlVel >=
                             mvControlling->SpeedCtrlUnit.MinVelocity) &&
                            (SpeedCntrlVel -
                                 mvControlling->SpeedCtrlUnit.MinVelocity ==
                             quantize(
                                 SpeedCntrlVel -
                                     mvControlling->SpeedCtrlUnit.MinVelocity,
                                 mvControlling->SpeedCtrlUnit.VelocityStep))) {
        ...
        case TEngineType::WheelsDriven:
            if (!mvControlling->CabActive) mvControlling->CabActivisation();
            if (sin(mvControlling->eAngle) > 0)
                mvControlling->IncMainCtrl(
                    3 + 3 * floor(0.5 + fabs(AccDesired)));
            else
                mvControlling->DecMainCtrl(
                    3 + 3 * floor(0.5 + fabs(AccDesired)));
            break;
        case TEngineType::DieselEngine:
            if (mvControlling
                    ->ShuntModeAllow) {  // dla 2Ls150 można zmienić tryb pracy,
                                         // jeśli jest w liniowym i nie daje
                                         // rady (wymaga zerowania kierunku)
                                         // mvControlling->ShuntMode=(OrderList[OrderPos]&Shunt)||(fMass>224000.0);
            }
            if ((mvControlling->SpeedCtrl) &&
                (mvControlling->Mains)) {  // cruise control
                auto const couplinginprogress{
                    (true == TestFlag(iDrivigFlags, moveConnect)) &&
                    (true == TestFlag(OrderCurrentGet(), Connect))};
                auto const SpeedCntrlVel{(
                ...
                if ((SpeedCntrlVel >=
                     mvControlling->SpeedCtrlUnit.MinVelocity) &&
                    (SpeedCntrlVel - mvControlling->SpeedCtrlUnit.MinVelocity ==
                     quantize(
                         SpeedCntrlVel -
                             mvControlling->SpeedCtrlUnit.MinVelocity,
                         mvControlling->SpeedCtrlUnit.VelocityStep))) {
            ...
            if (mvControlling->EIMCtrlType > 0) {
                if (true == Ready) {
                    bool max = (mvControlling->Vel >
                                mvControlling->dizel_minVelfullengage) ||
                               (mvControlling->SpeedCtrl &&
                                mvControlling->ScndCtrlPos > 0);
                ...
                if (true == Ready) {
                    if ((mvControlling->Vel >
                         mvControlling->dizel_minVelfullengage) &&
                        (mvControlling->RList[mvControlling->MainCtrlPos].Mn >
                         0)) {
                        OK = mvControlling->IncMainCtrl(1);
                    }
                    if (mvControlling->RList[mvControlling->MainCtrlPos].Mn ==
                        0) {
                        OK = mvControlling->IncMainCtrl(1);
                    }
                }
...
bool TController::DecSpeed(
    bool force) {     // zmniejszenie prędkości (ale nie hamowanie)
    ...
    switch (mvOccupied->EngineType) {
        case TEngineType::None:  // McZapkie-041003: wagon sterowniczy
            ...
            if (force)  // przy aktywacji kabiny jest potrzeba natychmiastowego
                        // wyzerowania
                if (mvControlling->MainCtrlPosNo >
                    0)  // McZapkie-041003: wagon sterowniczy, np. EZT
                    mvControlling->DecMainCtrl(
                        std::min(mvControlling->MainCtrlPowerPos(), 2));
            //        mvControlling->AutoRelayCheck(); // sprawdzenie logiki
            //        sterowania
            return false;
        case TEngineType::ElectricSeriesMotor:
        ...
        case TEngineType::DieselElectric:
            ...
            if (!OK)
                OK = mvControlling->DecMainCtrl(std::min(
                    mvControlling->MainCtrlPowerPos(),
                    (2 + (mvControlling->MainCtrlPowerPos() / 2))));
            break;
        case TEngineType::ElectricInductionMotor:
            OK = DecSpeedEIM();
            break;
        ...
        case TEngineType::DieselEngine:
            if (mvControlling->EIMCtrlType > 0) {
                ...
                if ((VelDesired > 0.1) &&
                    (mvControlling->SpeedCtrlUnit.MinVelocity > VelDesired)) {
                    SpeedCntrl(0.0);  // wylacz od razu tempomat
                    mvControlling->DecScndCtrl(2);
                }
                break;
            ...
            if (force) {  // przy aktywacji kabiny jest potrzeba
                          // natychmiastowego wyzerowania
                OK = mvControlling->DecMainCtrl(
                    (mvControlling->MainCtrlPowerPos() > 2 ? 2 : 1));
                SpeedCntrl(0.0);  // wylacz od razu tempomat
                mvControlling->DecScndCtrl(2);
            }
            break;
...
bool TController::IncSpeedEIM() {
    ...
    switch (mvControlling->EIMCtrlType) {
        ...
        case 1:
            ...
            /*
                        // TBD, TODO: set position based on desired
               acceleration? OK = mvControlling->MainCtrlPos <
               mvControlling->MainCtrlPosNo; if( OK ) {
                            mvControlling->MainCtrlPos = clamp(
               mvControlling->MainCtrlPos + 1, 6, mvControlling->MainCtrlPosNo
               );
                        }
            */
            break;
...
bool TController::DecSpeedEIM(
    int const Amount) {  // zmniejszenie prędkości (ale nie hamowanie)
    bool OK = false;     // domyślnie false, aby wyszło z pętli while
    switch (mvControlling->EIMCtrlType) {
        case 0: {
        ...
        case 2: {
            if ((AccDesired > 0) && (mvControlling->SpeedCtrlUnit.IsActive) &&
                (mvControlling->SpeedCtrlUnit.PowerStep > 0) &&
                (mvControlling->SpeedCtrlUnit.DesiredPower >
                 mvControlling->SpeedCtrlUnit.MinPower)) {
...
void TController::BrakeLevelSet(
    double b) {  // ustawienie pozycji hamulca na wartość (b) w zakresie od -2
                 // do BrakeCtrlPosNo jedyny dopuszczalny sposób przestawienia
                 // hamulca zasadniczego
    if (BrakeCtrlPosition == b) return;  // nie przeliczać, jak nie ma zmiany
    BrakeCtrlPosition = clamp(b, (double)gbh_MIN, (double)gbh_MAX);
}

bool TController::BrakeLevelAdd(
    double b) {  // dodanie wartości (b) do pozycji hamulca (w tym ujemnej)
                 // zwraca false, gdy po dodaniu było by poza zakresem
    BrakeLevelSet(BrakeCtrlPosition + b);
    return b > 0.0
               ? (BrakeCtrlPosition < gbh_MAX)
               : (BrakeCtrlPosition > -1.0);  // true, jeśli można kontynuować
}

// Ra: regulacja prędkości, wykonywana w każdym przebłysku świadomości AI
// ma dokręcać do bezoporowych i zdejmować pozycje w przypadku przekroczenia
// prądu
void TController::SpeedSet() {
    ...
    switch (mvOccupied->EngineType) {
            ...
            if (mvControlling->MainCtrlPosNo > 0) {  // jeśli ma czym kręcić
                ...
                if ((iDrivigFlags & moveIncSpeed) == 0) {
                    ...
                    if (mvOccupied->DirActive > 0) {
                        mvOccupied->DirectionForward();  //żeby EN57 jechały na
                                                         //drugiej nastawie
                    }

                    if ((mvControlling->MainCtrlPos > 0) &&
                        (false == mvControlling->StLinFlag)) {
                        // jak niby jedzie, ale ma rozłączone liniowe to na zero
                        // i czekać na przewalenie kułakowego
                        mvControlling->DecMainCtrl(2);
                    } else {
                        // ruch nastawnika uzależniony jest od aktualnie
                        // ustawionej pozycji
                        switch (mvControlling->MainCtrlPos) {
                            case 0:
                                if (mvControlling->MainCtrlActualPos) {
                                    // jeśli kułakowy nie jest wyzerowany to
                                    // czekać na wyzerowanie
                                    break;
                                }
                                mvControlling->IncMainCtrl(
                                    1);  // przetok; bez "break", bo nie ma
                                         // czekania na 1. pozycji
                            case 1:
                                if (VelDesired >= 20)
        ...
        case TEngineType::ElectricSeriesMotor: {
            /*
                    if (Ready || (iDrivigFlags & movePress)) { // o ile może
            jechać
            //            if (fAccGravity < -0.10) // i jedzie pod górę większą
            niż 10 promil { // procedura wjeżdżania na ekstremalne wzniesienia
                            if( std::fabs( mvControlling->Im ) > 0.85 *
            mvControlling->Imax ) { // a prąd jest większy niż 85% nadmiarowego
                                if( mvControlling->Imax *
            mvControlling->EngineVoltage / ( fMass * fAccGravity ) < -2.8 ) // a
            na niskim się za szybko nie pojedzie { // włączenie wysokiego
            rozruchu;
                                    //
            (I*U)[A*V=W=kg*m*m/sss]/(m[kg]*a[m/ss])=v[m/s]; 2.8m/ss=10km/h if(
            mvControlling->RList[ mvControlling->MainCtrlPos ].Bn > 1 ) { //
            jeśli jedzie na równoległym, to zbijamy do szeregowego, aby włączyć
            wysoki rozruch if( mvControlling->ScndCtrlPos > 0 ) { // jeżeli jest
            bocznik mvControlling->DecScndCtrl( 2 ); // wyłączyć bocznik, bo
            może blokować skręcenie NJ
                                        }
                                        do { // skręcanie do bezoporowej na
            szeregowym mvControlling->DecMainCtrl( 1 ); // kręcimy nastawnik
            jazdy o 1 wstecz
                                        }
                                        while( ( false ==
            mvControlling->IsMainCtrlNoPowerPos() )
                                            && ( mvControlling->RList[
            mvControlling->MainCtrlPos ].Bn > 1 ) ); // oporowa zapętla
                                    }
                                    if( mvControlling->Imax <
            mvControlling->ImaxHi ) { // jeśli da się na wysokim
                                        mvControlling->CurrentSwitch( true ); //
            rozruch wysoki (za to może się ślizgać)
                                    }
                                    if( ReactionTime > 0.1 ) {
                                        ReactionTime = 0.1; // orientuj się
            szybciej
                                    }
                                } // if (Im>Imin)
                            }
                            if( std::fabs( mvControlling->Im ) > 0.96 *
            mvControlling->Imax ) {// jeśli prąd jest duży (można 690 na 750)
                                if( mvControlling->ScndCtrlPos > 0 ) { // jeżeli
            jest bocznik mvControlling->DecScndCtrl( 2 ); // zmniejszyć bocznik
                                }
                                else {
                                    mvControlling->DecMainCtrl( 1 ); // kręcimy
            nastawnik jazdy o 1 wstecz
                                }
                            }
                        }
            //            else // gdy nie jedzie ambitnie pod górę
                        { // sprawdzenie, czy rozruch wysoki jest potrzebny
                            if( ( mvControlling->Imax > mvControlling->ImaxLo )
                             && ( mvOccupied->Vel >= 30.0 ) ) { // jak się
            rozpędził
            //                        if (fAccGravity > -0.02) // a i pochylenie
            mnijsze niż 2‰ mvControlling->CurrentSwitch( false ); // rozruch
            wysoki wyłącz
                            }
                        }
                    }
        ...
        case TEngineType::DieselEngine: {
            ...
            auto const &motorparams{
                mvControlling->MotorParam[mvControlling->ScndCtrlPos]};
            auto const velocity{
                (mvControlling->ShuntMode ? mvControlling->AnPos : 1.0) *
                mvControlling->Vel};
            if (false == motorparams.AutoSwitch) {
                // gdy biegi ręczne
                // jak prędkość większa niż procent maksymalnej na danym biegu,
                // wrzucić wyższy
                if (velocity > 0.75 * motorparams.mfi) {
                    ...
                    if (mvControlling->ScndCtrlPos <
                        ...
                        while ((mvControlling->IncScndCtrl(1)) &&
                               (mvControlling
                                    ->MotorParam[mvControlling->ScndCtrlPos]
                                    .mIsat ==
                                0.0)) {  // jeśli bieg jałowy to kolejny
                            ;
                        }
                    }
                }
                // jak prędkość mniejsza niż minimalna na danym biegu, wrzucić
                // niższy
                else if (velocity < motorparams.fi) {
                    // ... but ensure we don't switch all way down to 0
                    if (mvControlling->ScndCtrlPos > 1) {
                        mvControlling->DecMainCtrl(2);
                        while ((mvControlling->ScndCtrlPos >
                                1)  // repeat the entry check as we're working
                                    // in a loop
...
void TController::SpeedCntrl(double DesiredSpeed) {
    if (false == mvControlling->SpeedCtrl) {
        return;
    }
    ...
    if (mvControlling->EngineType == TEngineType::DieselEngine) {
        ...
        mvControlling->RunCommand(
            "SpeedCntrl", DesiredSpeed, mvControlling->CabActive);
    } else if (mvControlling->ScndCtrlPosNo == 1) {
        mvControlling->IncScndCtrl(1);
        mvControlling->RunCommand(
            "SpeedCntrl", DesiredSpeed, mvControlling->CabActive);
    } else if (
        (mvControlling->ScndCtrlPosNo > 1) &&
        (!mvOccupied->SpeedCtrlTypeTime)) {
        int DesiredPos = 1 + mvControlling->ScndCtrlPosNo *
                                 ((DesiredSpeed - 1.0) / mvControlling->Vmax);
        while ((mvControlling->ScndCtrlPos > DesiredPos) &&
               (true == mvControlling->DecScndCtrl(1))) {
            ;
        }  // all work is done in the condition loop
        while ((mvControlling->ScndCtrlPos < DesiredPos) &&
               (true == mvControlling->IncScndCtrl(1))) {
            ;
        }  // all work is done in the condition loop
    }
    ...
    if ((mvControlling->SpeedCtrlUnit.PowerStep > 0) &&
        (mvControlling->ScndCtrlPos > 0)) {
        while (mvControlling->SpeedCtrlUnit.DesiredPower <
               mvControlling->SpeedCtrlUnit.MaxPower) {
...
void TController::SetTimeControllers() {
    // TBD, TODO: rework this method to use hint system and regardless of driver
    // type
    if (false == AIControllFlag || 0 == mvOccupied->CabActive) {
        return;
    }
    ...
    if (BrakeSystem == TBrakeSystem::Pneumatic || ForcePNBrake) {
        if (mvOccupied->Handle->Time) {
            auto const pressuredifference{
                mvOccupied->Handle->GetCP() -
                (mvOccupied->HighPipePress -
                 BrakeCtrlPosition * 0.25 * mvOccupied->DeltaPipePress)};
        ...
        if ((mvOccupied->BrakeHandle == TBrakeHandle::MHZ_K8P) ||
            ...
            if (BrakeCtrlPosition == 0) {
                ...
                mvOccupied->BrakeLevelSet(
                    round((BrakeCtrlPosition * 0.4 - 0.1) / 0.15));
            }
        }
    }
    ...
    /*
            if (mvOccupied->EIMCtrlType > 0)
            {
                    ...
                    {
                            if (mvOccupied->LocalBrakePosA > 0.95)
    ...
    */
    ...
    if (mvOccupied->EngineType == TEngineType::ElectricInductionMotor &&
        mvOccupied->ScndCtrlPosNo > 1 && mvOccupied->SpeedCtrlTypeTime) {
    ...
    if ((mvControlling->EngineType == TEngineType::DieselEngine) &&
        ...
        } else if (VelDesired > MinVel)       // more power for faster ride
        {
            ...
            if ((VelDesired <
                 0.5 *
                     mvControlling->Vmax)  // low velocity and reaching desired
                && (VelDesired - mvControlling->Vel < 10)) {
                DesiredPercentage = std::min(DesiredPercentage, 0.75);
            }
            DizelPercentage_Speed =
                std::round(DesiredPercentage * DizelPercentage);
        } else  // slow acceleration during shunting wth minimal velocity
        {
            DizelPercentage_Speed = std::min(
                DizelPercentage,
                mvControlling->Vel < 0.99 * VelDesired ? 1 : 0);
        }

        auto const DizelActualPercentage{
            int(100.4 * mvControlling->eimic_real)};
        ...
        if (std::abs(DizelPercentage_Speed - DizelActualPercentage) >
            (DizelPercentage > 1 ? 0 : 0)) {
            if ((PosDec > 0) &&
            ...
            auto const DesiredPos{
                (DizelPercentage_Speed > DizelActualPercentage ? PosInc
                                                               : PosDec)};
            while (mvControlling->MainCtrlPos > DesiredPos) {
                mvControlling->DecMainCtrl(1);
            }
            while (mvControlling->MainCtrlPos < DesiredPos) {
                mvControlling->IncMainCtrl(1);
        ...
        if (BrakeCtrlPosition < 0.1)  // jesli nie hamuje
            mvOccupied->BrakeLevelSet(
                mvControlling->UniCtrlList[mvControlling->MainCtrlPos]
                    .mode);  // zeby nie bruzdzilo machanie zespolonym
    } else
        // 5.2. Analog direct controller
        if ((mvControlling->EngineType == TEngineType::DieselEngine) &&
            (mvControlling->Vmax > 30)) {
            int MaxPos = mvControlling->MainCtrlPosNo;
            int MinPos = MaxPos;
            for (int i = MaxPos; (i > 1) && (mvControlling->RList[i].Mn > 0);
                 i--)
                MinPos = i;
            if ((MaxPos > MinPos) && (mvControlling->MainCtrlPos > 0) &&
                (AccDesired > 0)) {
                double Factor = 5 * (mvControlling->Vmax) /
                                (mvControlling->Vmax + mvControlling->Vel);
                int DesiredPos =
                    MinPos +
                    (MaxPos - MinPos) *
                        (VelDesired > mvControlling->Vel
    ...
    if ((mvControlling->EngineType == TEngineType::DieselElectric) &&
        ...
        if ((AccDesired >= 0.0) && (mvControlling->StLinFlag)) {
            auto const PosInc{mvControlling->MainCtrlPosNo};
            auto PosKeep{0};
            auto PosDec{0};
            for (int i = PosInc; i >= 0; --i) {
    ...
    if (std::abs(BrakeCtrlPosition - gbh_FS) < 0.5) {
        UniversalBrakeButtons |=
            (TUniversalBrake::ub_HighPressure | TUniversalBrake::ub_Overload);
    } else {
        UniversalBrakeButtons &=
            ~(TUniversalBrake::ub_HighPressure | TUniversalBrake::ub_Overload);
    }
    // 6.2. Setting buttons
    for (int i = 0; i < 3; i++) {
        mvOccupied->UniversalBrakeButton(
            i,
            (UniversalBrakeButtons & mvOccupied->UniversalBrakeButtonFlag[i]));
    }
};

void TController::CheckTimeControllers() {
    // TODO: rework this method to use hint system and regardless of driver type
    if (false == AIControllFlag || 0 == mvControlling->CabActive) {
        return;
    }

    // 1. Check the type of Main Brake Handle
    if (BrakeSystem == TBrakeSystem::ElectroPneumatic &&
        mvOccupied->Handle->TimeEP && !ForcePNBrake) {
        mvOccupied->BrakeLevelSet(mvOccupied->Handle->GetPos(bh_EPN));
    }
    if ((BrakeSystem == TBrakeSystem::Pneumatic || ForcePNBrake) &&
        mvOccupied->Handle->Time) {
        if (BrakeCtrlPosition > 0)
    ...
    if ((mvControlling->EngineType == TEngineType::DieselEngine) &&
        ...
        int NeutralPos =
            mvControlling->MainCtrlPosNo -
            1;  // przedostatnia powinna wstrzymywać - hipoteza robocza
        for (int i = mvControlling->MainCtrlPosNo; i >= 0; i--)
            if ((mvControlling->UniCtrlList[i].SetCtrlVal <= 0) &&
                (mvControlling->UniCtrlList[i].SpeedDown <
                 0.01))  // niby zero, ale nie zmniejsza procentów
            {
                NeutralPos = i;
                break;
            }
        if (BrakeCtrlPosition < 0.1)  // jesli nie hamuje
        {
            if ((DizelActualPercentage >= DizelPercentage_Speed) &&
                (mvControlling->MainCtrlPos > NeutralPos))
                while (mvControlling->MainCtrlPos > NeutralPos)
                    mvControlling->DecMainCtrl(1);
            if ((DizelActualPercentage <= DizelPercentage_Speed) &&
                (mvControlling->MainCtrlPos < NeutralPos))
                while (mvControlling->MainCtrlPos < NeutralPos)
...
/*         || ( pVehicle->MoverParameters->Doors.open_control == control_t::mixed ) */ ) {
    ...
    // zamykanie drzwi
        ...
        if ((false == doors_permit_active()) && (false == doors_open())) {
        ...
        if (true == doors_open()) {
            if ((true == mvOccupied->Doors.has_warning) &&
                ...
                cue_action(
                    driver_hint::departuresignalon);    // załącenie bzyczka
                fActionTime = -1.5 - 0.1 * Random(10);  // 1.5-2.5 second wait
            }
        }

        if ((false == AIControllFlag) || (fActionTime > -0.5)) {
            // ai doesn't close the door until it's free to depart, but human
            // driver has free reign to do stupid things
            if (true == doors_open()) {
                // consist-wide remote signals to close doors
                if ((pVehicle->MoverParameters->Doors.close_control ==
                     control_t::conductor) ||
                    (pVehicle->MoverParameters->Doors.close_control ==
                     control_t::driver) ||
                    (pVehicle->MoverParameters->Doors.close_control ==
                     control_t::mixed)) {
                    cue_action(driver_hint::doorrightclose);
            ...
            // if applicable close manually-operated doors in vehicles which may
            // ignore remote signals
            {
                auto *vehicle =
                    pVehicles[end::front];  // pojazd na czole składu
                while (vehicle != nullptr) {
                    // zamykanie drzwi w pojazdach - flaga zezwolenia była by
                    // lepsza
                    auto const ismanualdoor{
                        (vehicle->MoverParameters->Doors.auto_velocity ==
                         -1.f) &&
                        ((vehicle->MoverParameters->Doors.close_control ==
                          control_t::passenger) ||
                         (vehicle->MoverParameters->Doors.close_control ==
                          control_t::mixed))};
                    ...
                    if ((true == ismanualdoor) &&
                        ...
                        vehicle->MoverParameters->OperateDoors(
                            side::right, false, range_t::local);
                        vehicle->MoverParameters->OperateDoors(
                            side::left, false, range_t::local);
                    }
                    vehicle = vehicle->Next();  // pojazd podłączony z tyłu
                                                // (patrząc od czoła)
                }
            }
            fActionTime =
                -2.0 -
                0.1 *
                    Random(15);  // 2.0-3.5 sec wait, +potentially 0.5 remaining
            iDrivigFlags &=
                ~moveDoorOpened;  // zostały zamknięte - nie wykonywać drugi raz
...
bool TController::doors_open() const {
    return (IsAnyDoorOpen[side::right] || IsAnyDoorOpen[side::left]);
}

bool TController::doors_permit_active() const {
    return (
        IsAnyDoorPermitActive[side::right] ||
        IsAnyDoorPermitActive[side::left]);
}

void TController::announce(announcement_t const Announcement) {
    m_lastannouncement = Announcement;

    if (IsCargoTrain) {
        // cargo trains aren't likely to have announcement systems, so we can
...
void TController::RecognizeCommand() {  // odczytuje i wykonuje komendę
                                        // przekazaną lokomotywie
...
void TController::PutCommand(
    std::string NewCommand, double NewValue1, double NewValue2,
    const TLocation &NewLocation,
    TStopReason
        reason) {  // wysłanie komendy przez event PutValues, jak pojazd ma
                   // obsadę, to wysyła tutaj, a nie do pojazdu bezpośrednio
...
bool TController::PutCommand(
    std::string NewCommand, double NewValue1, double NewValue2,
    glm::dvec3 const *NewLocation, TStopReason reason) {  // analiza komendy
    if (NewCommand == "CabSignal") {  // SHP wyzwalane jest przez człon z
                                      // obsadą, ale obsługiwane przez silnikowy
        // nie jest to najlepiej zrobione, ale bez symulacji obwodów lepiej nie
        // będzie Ra 2014-04: jednak przeniosłem do rozrządczego
        mvOccupied->PutCommand(
            NewCommand, NewValue1, NewValue2, mvOccupied->Loc);
        mvOccupied->RunInternalCommand();  // rozpoznaj komende bo lokomotywa
                                           // jej nie rozpoznaje
        return true;                       // załatwione
    }

    if (NewCommand == "Overhead") {  // informacja o stanie sieci trakcyjnej
        fOverhead1 = NewValue1;  // informacja o napięciu w sieci trakcyjnej
                                 // (0=brak drutu, zatrzymaj!)
        fOverhead2 = NewValue2;  // informacja o sposobie jazdy (-1=normalnie,
                                 // 0=bez prądu, >0=z
    ...
    if (NewCommand ==
        "Emergency_brake")  // wymuszenie zatrzymania, niezależnie kto prowadzi
    ...
    if (NewCommand.compare(0, 10, "Timetable:") ==
        0) {  // przypisanie nowego rozkładu jazdy, również prowadzonemu przez
              // użytkownika
        NewCommand.erase(0, 10);  // zostanie nazwa pliku z rozkładem
#if LOGSTOPS
        WriteLog(
            "New timetable for " + pVehicle->asName + ": " +
            NewCommand);  // informacja
#endif
        TrainParams = TTrainParameters(NewCommand);  // rozkład jazdy
        tsGuardSignal = sound_source{
            sound_placement::internal,
            2 * EU07_SOUND_CABCONTROLSCUTOFFRANGE};  // wywalenie kierownika
        m_lastexchangestop.clear();
        if (NewCommand != "none") {
            if (false == TrainParams.LoadTTfile(
                             std::string(Global.asCurrentSceneryPath.c_str()),
                             floor(NewValue2 + 0.5),
                             NewValue1))  // pierwszy parametr to przesunięcie
                                          // rozkładu w czasie
            {
                if (ConversionError == -8)
                    ErrorLog("Missed timetable: " + NewCommand);
                WriteLog(
                    "Cannot load timetable file " + NewCommand + "\r\nError " +
                    std::to_string(ConversionError) + " in position " +
                    std::to_string(TrainParams.StationCount));
                NewCommand = "";  // puste, dla wymiennej tekstury
            } else {  // inicjacja pierwszego przystanku i pobranie jego nazwy
                // HACK: clear the potentially set door state flag to ensure
                // door get opened if applicable
                if (true == AIControllFlag) {
                    // simplified door closing procedure, to sync actual door
                    // state with the door state flag NOTE: this may result in
                    // visually ugly quick switch between closing and opening
                    // the doors, but eh
                    if ((pVehicle->MoverParameters->Doors.close_control ==
                         control_t::driver) ||
                        (pVehicle->MoverParameters->Doors.close_control ==
                         control_t::mixed)) {
                        pVehicle->MoverParameters->OperateDoors(
                            side::right, false);
                        pVehicle->MoverParameters->OperateDoors(
                            side::left, false);
                    }
                    if (pVehicle->MoverParameters->Doors.permit_needed) {
                        pVehicle->MoverParameters->PermitDoors(
                            side::right, false);
                        pVehicle->MoverParameters->PermitDoors(
                            side::left, false);
                    }
                ...
                TrainParams.UpdateMTable(
                    simulation::Time, TrainParams.NextStationName);
                TrainParams.StationIndexInc();  // przejście do następnej
                TrainParams.StationStart = TrainParams.StationIndex;
                asNextStop = TrainParams.NextStop();
                m_lastannouncement = announcement_t::idle;
                iDrivigFlags |=
                    movePrimary;  // skoro dostał rozkład, to jest teraz głównym
                //                NewCommand = Global.asCurrentSceneryPath +
                //                NewCommand;
                auto lookup = FileExists(
                    {Global.asCurrentSceneryPath + NewCommand,
                     szSoundPath + NewCommand},
                    {".ogg", ".flac", ".wav"});
                if (false == lookup.first.empty()) {
                    //  wczytanie dźwięku odjazdu podawanego bezpośrenido
                    tsGuardSignal =
                        sound_source(sound_placement::external, 75.f)
                            .deserialize(
                    ...
                    lookup = FileExists(
                        ...
                        {".ogg", ".flac", ".wav"});
                    if (false == lookup.first.empty()) {
                        //  wczytanie dźwięku odjazdu w wersji radiowej (słychać
                        //  tylko w kabinie)
                        tsGuardSignal = sound_source(
                                            sound_placement::internal,
                                            EU07_SOUND_HANDHELDRADIORANGE)
                                            .deserialize(
                                                lookup.first + lookup.second,
                                                sound_type::single);
                ...
                NewCommand =
                    TrainParams.Relation2;  // relacja docelowa z rozkładu
            }
            // jeszcze poustawiać tekstury na wyświetlaczach
            TDynamicObject *p = pVehicles[end::front];
            while (p) {
                p->DestinationSet(
                    NewCommand, TrainParams.TrainName);  // relacja docelowa
                p = p->Next();  // pojazd podłączony od tyłu (licząc od czoła)
            }
        }
        if (NewLocation)  // jeśli podane współrzędne eventu/komórki
                          // ustawiającej rozkład (trainset
        // nie podaje)
        {
            auto v = *NewLocation -
                     pVehicle->GetPosition();  // wektor do punktu sterującego
            auto d = pVehicle->VectorFront();  // wektor wskazujący przód
            iDirectionOrder = ((v.x * d.x + v.z * d.z) * NewValue1 > 0)
                                  ? 1
                                  : -1;  // do przodu, gdy iloczyn skalarny i
                                         // prędkość dodatnie
            /*
              if (NewValue1!=0.0) //jeśli ma jechać
               if (iDirectionOrder==0) //a kierunek nie był określony (normalnie
              określany przez reardriver/headdriver)
                iDirectionOrder=NewValue1>0?1:-1; //ustalenie kierunku jazdy
              względem sprzęgów else if (NewValue1<0) //dla ujemnej prędkości
                 iDirectionOrder=-iDirectionOrder; //ma jechać w drugą stronę
            */
            if (AIControllFlag)  // jeśli prowadzi komputer
                Activation();  // umieszczenie obsługi we właściwym członie,
                               // ustawienie nawrotnika w
            // przód
        }
        /*
          else
        ...
        */
        ...
        // if (mvOccupied->CabActive*mvOccupied->V*NewValue1<0) //jeśli zadana
        // prędkość niezgodna z aktualnym kierunkiem jazdy
        //  DirectionForward(false); //jedziemy do tyłu (nawrotnik do tyłu)
        // CheckVehicles(); //sprawdzenie składu, AI zapali światła
        TableClear();  // wyczyszczenie tabelki prędkości, bo na nowo trzeba
                       // określić kierunek i
        // sprawdzić przystanki
        OrdersInit(fabs(NewValue1));  // ustalenie tabelki komend wg rozkładu
                                      // oraz prędkości początkowej
        // if (NewValue1!=0.0) if (!AIControllFlag)
        // DirectionForward(NewValue1>0.0); //ustawienie nawrotnika
        // użytkownikowi (propaguje się do członów) if (NewValue1>0)
        // TrainNumber=floor(NewValue1); //i co potem ???
        return true;  // załatwione
    }

    if (NewCommand == "SetVelocity") {
        if (NewLocation) vCommandLocation = *NewLocation;
        if ((NewValue1 != 0.0) &&
            (OrderCurrentGet() != Obey_train)) {  // o ile jazda
            if (false == iEngineActive) {
                // trzeba odpalić silnik najpierw, światła ustawi
                OrderNext(Prepare_engine);
            }
            OrderNext(Obey_train);  // to uruchomić jazdę pociągową (od razu
                                    // albo po odpaleniu silnika
            OrderCheck();  // jeśli jazda pociągowa teraz, to wykonać niezbędne
                           // operacje
        }
        if (NewValue1 != 0.0)               // jeśli jechać
            ...
            iDrivigFlags |=
                moveStopHere;  // stać do momentu podania komendy jazdy
        SetVelocity(
            NewValue1, NewValue2,
            reason);  // bylo: nic nie rob bo SetVelocity zewnetrznie
    ...
    if (NewCommand == "ShuntVelocity") {  // uruchomienie jazdy manewrowej bądź
                                          // zmiana prędkości
        if (NewLocation) vCommandLocation = *NewLocation;
        ...
        OrderNext(Shunt);  // zamieniamy w aktualnej pozycji, albo dodajey za
                           // odpaleniem silnika
        if (NewValue1 != 0.0) {
            // if (iVehicleCount>=0) WriteLog("Skasowano ilosć wagonów w
            // ShuntVelocity!");
            iVehicleCount = -2;  // wartość neutralna
            CheckVehicles();     // zabrać to do OrderCheck()
        }
        // dla prędkości ujemnej przestawić nawrotnik do tyłu? ale -1=brak
        // ograniczenia !!!!
        ...
        if (fabs(NewValue1) > 2.0)  // o ile wartość jest sensowna (-1 nie jest
                                    // konkretną wartością)
            fShuntVelocity = fabs(NewValue1);  // zapamiętanie obowiązującej
                                               // prędkości dla manewrów

        return true;
    }

    if (NewCommand == "Wait_for_orders") {  // oczekiwanie; NewValue1 - czas
                                            // oczekiwania, -1 = na inną komendę
        if (NewValue1 > 0.0 ? NewValue1 > fStopTime : false)
            fStopTime = NewValue1;  // Ra: włączenie czekania bez zmiany komendy
        else
            OrderList[OrderPos] =
                Wait_for_orders;  // czekanie na komendę (albo dać OrderPos=0)

        return true;
    }

    if (NewCommand == "Prepare_engine") {  // włączenie albo wyłączenie silnika
                                           // (w szerokim sensie)
        OrdersClear();  // czyszczenie tabelki rozkazów, aby nic dalej nie robił
        if (NewValue1 == 0.0)
            OrderNext(Release_engine);  // wyłączyć silnik (przygotować pojazd
                                        // do jazdy)
        else if (NewValue1 > 0.0)
            OrderNext(Prepare_engine);  // odpalić silnik (wyłączyć wszystko, co
                                        // się da)
        // po załączeniu przejdzie do kolejnej komendy, po wyłączeniu na
        // Wait_for_orders
        return true;
    }
    ...
    if (NewCommand == "Change_direction") {
        ...
        else if (NewLocation)  // jeśli podane współrzędne eventu/komórki
                               // ustawiającej rozkład
        // (trainset nie podaje)
        {
            auto v = *NewLocation -
                     pVehicle->GetPosition();  // wektor do punktu sterującego
        ...
        if (mvOccupied->Vel >= EU07_AI_MOVEMENT)  // jeśli jedzie
            iDrivigFlags &=
                ~moveStartHorn;  // to bez trąbienia po ruszeniu z zatrzymania
        // Change_direction wykona się samo i następnie przejdzie do kolejnej
        // komendy
        return true;
    }
    ...
    if ((NewCommand == "Shunt") ||
        (NewCommand ==
         "Loose_shunt")) {  // NewValue1 - ilość wagonów (-1=wszystkie);
                            // NewValue2: 0=odczep, 1..63=dołącz, -1=bez zmian
        //-3,-y - podłączyć do całego stojącego składu (sprzęgiem y>=1), zmienić
        //kierunek i czekać w trybie pociągowym -2,-y - podłączyć do całego
        //stojącego składu (sprzęgiem y>=1), zmienić kierunek i czekać -2, y -
        //podłączyć do całego stojącego składu (sprzęgiem y>=1) i czekać -1,-y -
        //podłączyć do całego stojącego składu (sprzęgiem y>=1) i jechać w
        //powrotną stronę -1, y - podłączyć do całego stojącego składu
        //(sprzęgiem y>=1) i jechać dalej -1, 0 - tryb manewrowy bez zmian
        //(odczepianie z pozostawieniem wagonów nie ma sensu)
        // 0, 0 - odczepienie lokomotywy
        // x,-y - podłączyć się do składu (sprzęgiem y>=1), a następnie odczepić
        // i zabrać (x) wagonów 1, 0 - odczepienie lokomotywy z jednym wagonem
        iDrivigFlags &= ~moveStopHere;  // podjeżanie do semaforów zezwolone
        if (false == iEngineActive) {
            OrderNext(Prepare_engine);  // trzeba odpalić silnik najpierw
        }
        if (NewValue2 != 0)  // jeśli podany jest sprzęg
        {
            ...
            {                      // to po podpięciu należy się odczepić
                ...
                OrderPush(Change_direction);    // najpierw zmień kierunek, bo
                                                // odczepiamy z tyłu
                OrderPush(Disconnect);  // a odczep już po zmianie kierunku
                if ((NewValue2 > 0.0) && (NewCommand == "Loose_shunt")) {
                    // after decoupling continue pushing in the original
                    // direction NOTE: for backward compatibility this option
                    // isn't supported for basic shunting mode
                    iDirectionOrder = iDirection;  // back to pushing
                    OrderPush(Change_direction);
                }
            } else if (NewValue2 < 0.0)  // jeśli wszystkie, a sprzęg ujemny, to
                                         // tylko zmiana kierunku po
                                         // podczepieniu
            {                                   // np. Shunt -1 -3
                iDirectionOrder = -iDirection;  // jak się podczepi, to jazda w
                                                // przeciwną stronę
                OrderNext(Change_direction);
            }
            WaitingTime = 0.0;  // nie ma co dalej czekać, można zatrąbić i
                                // jechać, chyba że już jedzie
        } else {                // if (NewValue2==0.0) //zerowy sprzęg
            if (NewValue1 >= 0.0) {
                // jeśli ilość wagonów inna niż wszystkie będzie odczepianie,
                // ale jeśli wagony są z przodu, to trzeba najpierw zmienić
                // kierunek
                if ((mvOccupied
                         ->Couplers
                             [mvOccupied->DirAbsolute > 0 ? end::rear
                                                          : end::front]
                         .Connected == nullptr)  // z tyłu nic
                    && (mvOccupied
                            ->Couplers
                                [mvOccupied->DirAbsolute > 0 ? end::front
                                                             : end::rear]
                            .Connected != nullptr)) {  // a z przodu skład
                    iDirectionOrder = -iDirection;     // zmiana na ciągnięcie
                    OrderNext(Change_direction);  // najpierw zmień kierunek
                                                  // (zastąpi Disconnect)
                    OrderPush(Disconnect);  // a odczep już po zmianie kierunku
                } else if (
                    mvOccupied
                        ->Couplers
                            [mvOccupied->DirAbsolute > 0 ? end::rear
                                                         : end::front]
                        .Connected != nullptr) {  // z tyłu coś
                    OrderNext(Disconnect);  // jak ciągnie, to tylko odczep
                                            // (NewValue1) wagonów
                } else {
                    // both couplers empty
                    // NOTE: this condition implements legacy behavior (mis)used
                    // by some scenarios to keep vehicle idling without moving
                    SetVelocity(0, 0, stopJoin);
                    iDrivigFlags |=
                        moveStopHere;  // nie podjeżdżać do semafora, jeśli
                                       // droga nie jest wolna
                }
                WaitingTime = 0.0;  // nie ma co dalej czekać, można zatrąbić i
                                    // jechać, chyba że już jedzie
            }
        ...
        if (NewValue1 == -1.0) {
            ...
            WaitingTime =
                0.0;  // nie ma co dalej czekać, można zatrąbić i jechać
        }
        if (NewValue1 <
            -1.5) {  // jeśli -2/-3, czyli czekanie z ruszeniem na sygnał
            iDrivigFlags |= moveStopHere;  // nie podjeżdżać do semafora, jeśli
                                           // droga nie jest wolna
        }
        // (nie dotyczy Connect)
    ...
    if (NewCommand == "Warning_signal") {
        if ((NewValue1 > 0) && (NewValue2 > 0)) {
            ...
            cue_action(
                driver_hint::hornon, NewValue2);  // horn combination flag
        }
        return true;
    }

    if (NewCommand == "Radio_channel") {
        // wybór kanału radiowego (którego powinien używać AI, ręczny maszynista
        // musi go ustawić sam)
        if (NewValue1 >= 0) {
        ...
        // NewValue2 może zawierać dodatkowo oczekiwany kod odpowiedzi, np. dla
        // W29 "nawiązać łączność radiową z dyżurnym ruchu odcinkowym"
        return true;
    }
...
void TController::PhysicsLog() {  // zapis logu - na razie tylko wypisanie
                                  // parametrów
    if (LogFile.is_open()) {
#if LOGPRESS == 0
        /*
        << "Time[s] Velocity[m/s] Acceleration[m/ss] "
        << "Coupler.Dist[m] Coupler.Force[N] TractionForce[kN] FrictionForce[kN]
        BrakeForce[kN] "
        << "BrakePress[MPa] PipePress[MPa] MotorCurrent[A]    "
        ...
        */
        LogFile << ElapsedTime << " "
                ...
                << mvOccupied->Couplers[1].CForce << " " << mvOccupied->Ft
                << " " << mvOccupied->Ff << " " << mvOccupied->Fb << " "
                << mvOccupied->BrakePress << " " << mvOccupied->PipePress << " "
                << mvControlling->Im << " " << int(mvControlling->MainCtrlPos)
                << " " << int(mvControlling->ScndCtrlPos) << " "
                << mvOccupied->fBrakeCtrlPos << " "
                << mvOccupied->LocalBrakePosA << " "
                << int(mvControlling->DirActive) << " "
                << (mvOccupied->CommandIn.Command.empty()
                        ? "none"
                        : mvOccupied->CommandIn.Command.c_str())
...
void TController::Update(double const Timedelta) {
    ...
    /*
        ...
        // NOTE: this section moved all cars to the edge of their respective
       roads
        ...
        if( mvOccupied->Vel > 1.0 ) {
            // przy prowadzeniu samochodu trzeba każdą oś odsuwać oddzielnie,
       inaczej kicha wychodzi if (mvOccupied->CategoryFlag & 2) // jeśli
       samochód
                // if (fabs(mvOccupied->OffsetTrackH)<mvOccupied->Dim.W) //Ra:
       szerokość drogi tu powinna być? if (!mvOccupied->ChangeOffsetH(-0.01 *
       mvOccupied->Vel * dt)) // ruch w poprzek drogi
                    mvOccupied->ChangeOffsetH(0.01 * mvOccupied->Vel * dt); //
       Ra: co to miało być, to nie wiem
        }
    */
    update_hints();
    ...
    // vicinity check
    auto const awarenessrange{std::max(
        750.0, mvOccupied->Vel > EU07_AI_MOVEMENT
                   ? 400 + fBrakeDist
...
// configures vehicle heating given current situation; returns: true if vehicle
// can be operated normally, false otherwise
bool TController::PrepareHeating() {
    switch (mvControlling->EngineType) {
        ...
        case TEngineType::DieselEngine: {
            ...
            // if the heater has configured maximum temperature, it'll disable
            // itself automatically, so we can leave it always running otherwise
            // enable the heater only to maintain minimum required temperature
            auto const lowtemperature{
                (((heat.water.config.temp_min > 0) &&
                  (heat.temperatura1 <
                   heat.water.config.temp_min +
                       (mvControlling->WaterHeater.is_active ? 5 : 0))) ||
                 ((heat.water_aux.config.temp_min > 0) &&
                  (heat.temperatura2 <
                   heat.water_aux.config.temp_min +
                       (mvControlling->WaterHeater.is_active ? 5 : 0))) ||
                 ((heat.oil.config.temp_min > 0) &&
                  (heat.To <
                   heat.oil.config.temp_min +
                       (mvControlling->WaterHeater.is_active ? 5 : 0))))};
            auto const heateron{
                ((mvControlling->WaterHeater.config.temp_max > 0) ||
                 (true == lowtemperature))};
            if (true == heateron) {
                // make sure the water pump is running before enabling the
                // heater
...
void TController::PrepareDirection() {
    if (iDirection == 0) {
        // jeśli nie ma ustalonego kierunku
        if (mvOccupied->Vel <
            EU07_AI_NOMOVEMENT) {                // ustalenie kierunku, gdy stoi
            iDirection = mvOccupied->CabActive;  // wg wybranej kabiny
                                                 /*
                                                             if( iDirection == 0 ) {
                                                                 // jeśli nie ma ustalonego kierunku
                                                                 if( mvOccupied->Couplers[ end::rear ].Connected ==
                                                    nullptr ) {
                                                                     // jeśli z tyłu nie ma nic
                                                                     iDirection = -1; // jazda w kierunku sprzęgu 1
                                                                 }
                                                                 if( mvOccupied->Couplers[ end::front ].Connected ==
                                                    nullptr ) {
            ...
            iDirection =
                (mvOccupied->V >= 0 ? 1 :  // jazda w kierunku sprzęgu 0
                     -1);                  // jazda w kierunku sprzęgu 1
        }
    }
    cue_action(driver_hint::mastercontrollersetreverserunlock);
    cue_action(
        iDirection > 0 ? driver_hint::directionforward
                       : driver_hint::directionbackward);
}

void TController::JumpToNextOrder(
    bool const Ignoremergedchangedirection) {  // wykonanie kolejnej komendy z
                                               // tablicy rozkazów
    if (OrderList[OrderPos] != Wait_for_orders) {
        if (((OrderList[OrderPos] & Change_direction) !=
             0)  // jeśli zmiana kierunku
            &&
            ...
            if (false == Ignoremergedchangedirection) {
                ...
                OrderList[OrderPos] =
                    TOrders(OrderList[OrderPos] & ~Change_direction);
                OrderCheck();
                return;
            }
        }
...
void TController::JumpToFirstOrder() {  // taki relikt
    OrderPos = 1;
...
void TController::OrderCheck() {  // reakcja na zmianę rozkazu
    ...
    if (OrderCurrentGet() &
        (Shunt | Loose_shunt | Connect | Obey_train | Bank)) {
        CheckVehicles();  // sprawdzić światła
    }
    ...
    if (OrderList[OrderPos] &
        Change_direction)  // może być nałożona na inną i wtedy ma priorytet
        iDirectionOrder =
            -iDirection;  // trzeba zmienić jawnie, bo się nie domyśli
    else if (OrderList[OrderPos] == Obey_train)
        iDrivigFlags |= moveStopPoint;  // W4 są widziane
    else if (OrderList[OrderPos] == Disconnect)
        iVehicleCount =
            iVehicleCount < 0 ? 0 : iVehicleCount;  // odczepianie lokomotywy
    else if (OrderList[OrderPos] == Connect)
        iDrivigFlags &= ~moveStopPoint;  // podczas jazdy na połączenie nie
                                         // zwracać uwagi na W4
    else if (OrderList[OrderPos] == Wait_for_orders)
        OrdersClear();  // czyszczenie rozkazów i przeskok do zerowej pozycji
}

void TController::OrderNext(
    TOrders NewOrder) {  // ustawienie rozkazu do wykonania jako następny
    ...
    {  // ale może być zajęty chwilowymi operacjami
        while ((OrderList[OrderTop] != Wait_for_orders) &&
               (OrderList[OrderTop] < Shunt))  // jeśli coś robi
            ++OrderTop;  // pomijamy wszystkie tymczasowe prace
    } else {  // jeśli ma ustawioną jazdę, to wyłączamy na rzecz operacji
        while (OrderList[OrderTop] ? (OrderList[OrderTop] < Shunt) &&
                                         (OrderList[OrderTop] != NewOrder)
...
void TController::OrderPush(
    TOrders NewOrder) {  // zapisanie na stosie kolejnego rozkazu do wykonania
    if (OrderPos == OrderTop)  // jeśli miałby być zapis na aktalnej pozycji
        if (OrderList[OrderPos] < Shunt)  // ale nie jedzie
            ++OrderTop;  // niektóre operacje muszą zostać najpierw dokończone
                         // => zapis na kolejnej
...
#if LOGORDERS
    OrdersDump(
        "OrderPush: [" + Order2Str(NewOrder) +
        "]");  // normalnie nie ma po co tego wypisywać
#endif
}

void TController::OrdersDump(
    std::string const Neworder,
    bool const Verbose) {  // wypisanie kolejnych rozkazów do logu
    ...
    for (int b = 0; b < maxorders; ++b) {
        WriteLog(
            (OrderPos == b ? ">" : " ") +
            (std::to_string(b) + ": " + Order2Str(OrderList[b])));
...
void TController::OrdersInit(
    double fVel) {  // wypełnianie tabelki rozkazów na podstawie rozkładu
    ...
    if (TrainParams.StationCount == 0) {  // brak rozkładu to jazda manewrowa
        //        if (fVel > 0.05) // typowo 0.1 oznacza gotowość do jazdy, 0.01
        //        tylko załączenie silnika
        OrderPush(Shunt);  // jeśli nie ma rozkładu, to manewruje
    } else {               // jeśli z rozkładem, to jedzie na szlak
        if ((fVel > 0.0) && (fVel < 0.02))
            OrderPush(Shunt);  // dla prędkości 0.01 włączamy jazdę manewrową
        else if (
            TrainParams.DirectionChange()
                ?  //  jeśli obrót na pierwszym przystanku
                ((iDrivigFlags & movePushPull)
                     ?  // SZT również! SN61 zależnie od wagonów...
                     (TrainParams.TimeTable[1].StationName ==
                      TrainParams.Relation1)
                     : false)
                : false)
            OrderPush(Shunt);  // a teraz start będzie w manewrowym, a tryb
                               // pociągowy włączy W4
        else
            // jeśli start z pierwszej stacji i jednocześnie jest na niej zmiana
            // kierunku, to EZT ma mieć Shunt
            OrderPush(
                Obey_train);  // dla starych scenerii start w trybie pociagowym
        ...
        for (int i = 0; i <= TrainParams.StationCount; ++i) {
            ...
            if (DebugModeFlag) {
                ...
                WriteLog(
                    t->StationName + " " + std::to_string(t->Ah) + ":" +
            ...
            if (contains(
                ...
                {  // jeśli skład zespolony, wystarczy zmienić kierunek jazdy
                    OrderPush(Change_direction);  // zmiana kierunku
                } else {  // dla zwykłego składu wagonowego odczepiamy
                          // lokomotywę
    ...
    if (fVel == 0.0) {
        ...
        if ((fVel >= 1.0) || (fVel < 0.02)) {
            // jeśli ma jechać - dla 0.01 ma podjechać manewrowo po podaniu
            // sygnału to do następnego W4 ma podjechać blisko
        ...
        if (fVel >= 1.0)            // jeśli ma jechać
            ...
            SetVelocity(
                0, 0,
                stopSleep);  // prędkość w przedziale (0;1) oznacza, że ma stać
    }
#if LOGORDERS
    OrdersDump(
        "OrdersInit", DebugModeFlag);  // wypisanie kontrolne tabelki rozkazów
#endif
...
std::string TController::StopReasonText()
    const {                // informacja tekstowa o przyczynie zatrzymania
...
// Ra: stare funkcje skanujące, używane podczas manewrów do szukania
// sygnalizatora z tyłu
//- nie reagują na PutValues, bo nie ma takiej potrzeby
//- rozpoznają tylko zerową prędkość (jako koniec toru i brak podstaw do
//dalszego skanowania)
//----------------------------------------------------------------------------------------------------------------------

bool TController::IsOccupiedByAnotherConsist(
    TTrack *Track,
    double const Distance = 0) {  // najpierw sprawdzamy, czy na danym torze są
                                  // pojazdy z innego składu
    if (false == Track->Dynamics.empty()) {
        for (auto dynamic : Track->Dynamics) {
            if (dynamic->ctOwner != this) {
                // jeśli jest jakiś cudzy to tor jest zajęty i skanowanie nie
                // obowiązuje
                if (Distance == 0) {
                    return true;
                } else {
                    // based on provided position of scanning vehicle and scan
                    // direction, filter out vehicles on irrelevant end
                    auto const scandirection{
                        Distance > 0 ? 1
                                     : -1};  // positive value means scan
                                             // towards point2 end of the track
                    auto const obstaclelocation{
                        scandirection > 0
                            ? Track->Length() - dynamic->RaTranslationGet()
                            : dynamic->RaTranslationGet()};
                    // if detected vehicle is closer to the end of the track in
                    // scanned direction than we are, it means it's blocking our
                    // way
...
basic_event *TController::CheckTrackEventBackward(
    double fDirection, TTrack *Track, TDynamicObject *Vehicle,
    int const Eventdirection,
    end const End) {  // sprawdzanie eventu w torze, czy jest sygnałowym -
                      // skanowanie do tyłu
    // NOTE: this method returns only one event which meets the conditions, due
    // to limitations in the caller TBD, TODO: clean up the caller and return
    // all suitable events, as in theory things will go awry if the track has
    // more than one signal
    auto const dir{Vehicle->VectorFront() * Vehicle->DirectionGet()};
    auto const pos{
        End == end::front ? Vehicle->RearPosition() : Vehicle->HeadPosition()};
    auto const &eventsequence{(
        fDirection * Eventdirection > 0 ? Track->m_events2 : Track->m_events1)};
    for (auto const &event : eventsequence) {
        if ((event.second != nullptr) && (event.second->m_passive) &&
            (typeid(*(event.second)) == typeid(getvalues_event))) {
            // since we're checking for events behind us discard the sources in
            // front of the scanning vehicle
            auto const sl{
                event.second->input_location()};  // położenie komórki pamięci
            auto const sem{
                sl - pos};  // wektor do komórki pamięci od końca składu
...
TTrack *TController::BackwardTraceRoute(
    double &fDistance, double &fDirection, TDynamicObject *Vehicle,
    basic_event *&Event, int const Eventdirection, end const End,
    bool const Untiloccupied) {  // szukanie sygnalizatora w kierunku przeciwnym
                                 // jazdy (eventu odczytu komórki pamięci)
    auto *Track = Vehicle->GetTrack();
    double fCurrentDistance =
        Vehicle->RaTranslationGet();  // aktualna pozycja na torze
    if (fDirection > 0)               // jeśli w kierunku Point2 toru
        fCurrentDistance = Track->Length() - fCurrentDistance;
    double s = 0;

    if (Track->VelocityGet() ==
        0.0) {  // jak prędkosć 0 albo uszkadza, to nie ma po co skanować
    ...
    if (Untiloccupied &&
        IsOccupiedByAnotherConsist(Track, fCurrentDistance * fDirection)) {
        fDistance = s;   // to na tym torze stoimy
        return nullptr;  // stop, skanowanie nie dało sensownych rezultatów
    }
    if ((Event = CheckTrackEventBackward(
             fDirection, Track, Vehicle, Eventdirection, End)) !=
        nullptr) {      // jeśli jest semafor na tym torze
        fDistance = s;  // to na tym torze stoimy
        return Track;
    }
    ...
    while (s < fDistance) {
        ...
        s += fCurrentDistance;  // doliczenie kolejnego odcinka do
                                // przeskanowanej długości
        if (fDirection > 0) {   // jeśli szukanie od Point1 w kierunku Point2
            if (Track->iNextDirection) fDirection = -fDirection;
            Track = Track->CurrentNext();  // może być NULL
        } else                             // if (fDirection<0)
        {  // jeśli szukanie od Point2 w kierunku Point1
            if (!Track->iPrevDirection) fDirection = -fDirection;
            Track = Track->CurrentPrev();  // może być NULL
        }
        if (Track == pTrackFrom) Track = nullptr;  // koniec, tak jak dla torów
        ...
        if ((Event = CheckTrackEventBackward(
                 fDirection, Track, Vehicle, Eventdirection, End)) !=
            nullptr) {  // znaleziony tor z eventem
            fDistance = s;
            return Track;
        }
    }
...
void TController::SetProximityVelocity(
    double dist, double vel,
    glm::dvec3 const *pos) {  // Ra:przeslanie do AI prędkości
    /*
...
TCommandType TController::BackwardScan(
    double const Range) {  // sprawdzanie zdarzeń semaforów z tyłu pojazdu,
                           // zwraca komendę
    // dzięki temu będzie można stawać za wskazanym sygnalizatorem, a zwłaszcza
    // jeśli będzie jazda na kozioł ograniczenia prędkości nie są wtedy istotne,
    // również koniec toru jest do niczego nie przydatny zwraca true, jeśli
    // należy odwrócić kierunek jazdy pojazdu
    if ((OrderCurrentGet() & ~(Shunt | Loose_shunt | Connect))) {
        // skanowanie sygnałów tylko gdy jedzie w trybie manewrowym albo czeka
        // na rozkazy
    ...
    // skanowanie toru w poszukiwaniu eventów GetValues (PutValues nie są
    // przydatne) Ra: przy wstecznym skanowaniu prędkość nie ma znaczenia
    double scandist = Range;   // zmodyfikuje na rzeczywiście przeskanowane
    basic_event *e = nullptr;  // event potencjalnie od semafora
    // opcjonalnie może być skanowanie od "wskaźnika" z przodu, np. W5, Tm=Ms1,
    // koniec toru wg drugiej osi w kierunku ruchu
    auto const *scantrack{
        BackwardTraceRoute(scandist, scandir, pVehicles[end::front], e)};
    auto const dir{
        startdir * pVehicles[end::front]
                       ->VectorFront()};  // wektor w kierunku jazdy/szukania

    // jeśli wstecz wykryto koniec toru to raczej nic się nie da w takiej
    // sytuacji zrobić
    if (e == nullptr) {
        return TCommandType::cm_Unknown;
    }
...
#if LOGBACKSCAN
    std::string edir{
        "Backward scan by " + pVehicle->asName + " - " +
...
#endif
    {
        ...
        // przeliczamy odległość od semafora - potrzebne by były współrzędne
        // początku składu
    ...
    if (e->input_command() == TCommandType::cm_SetVelocity) {
        if ((scanvel == 0.0)
            ...
            if ((scandist > fMinProximityDist) &&
                ...
                // zmianie prędkości jeśli jedzie manewrowo, musi dostać
                // SetVelocity, żeby sie na pociągowy przełączył
#if LOGBACKSCAN
                ...
                return (
                    scanvel > 0 ? TCommandType::cm_SetVelocity
                                : TCommandType::cm_Unknown);
            } else {
                // ustawiamy prędkość tylko wtedy, gdy ma ruszyć, stanąć albo ma
                // stać if ((MoverParameters->Vel==0.0)||(scanvel==0.0)) //jeśli
                // stoi lub ma stanąć/stać semafor na tym torze albo lokomtywa
                // stoi, a ma ruszyć, albo ma stanąć, albo nie ruszać stop
                // trzeba powtarzać, bo inaczej zatrąbi i pojedzie sam
                // PutCommand("SetVelocity",scanvel,e->Params[9].asMemCell->Value2(),&sl,stopSem);
#if LOGBACKSCAN
                WriteLog(
                    edir + " - [SetVelocity] [" + to_string(scanvel, 2) +
                          ...
                          : true)  // w Wait_for_orders też widzi tarcze
    {
        if (move ||
            e->input_command() ==
                TCommandType::cm_ShuntVelocity) {  // jeśli powyżej było
                                                   // SetVelocity 0 0, to
                                                   // dociągamy pod S1
                                                   /*
                                                               if ((scandist > fMinProximityDist) &&
                                                                   ((mvOccupied->Vel > EU07_AI_NOMOVEMENT) || (scanvel
                                                   == 0.0)))
                                                               {
                                                                   // jeśli tarcza jest daleko, to:
                                                                   //- jesli pojazd jedzie, to informujemy o zmianie
                                                   prędkości
                                                                   //- jeśli stoi, to z własnej inicjatywy może
                                                   podjechać pod zamkniętą tarczę                                        if (mvOccupied->Vel >
                                                   EU07_AI_NOMOVEMENT) // tylko jeśli jedzie                                        { //
                                                   ...
                                                   #endif
                                                                       // SetProximityVelocity(scandist,scanvel,&sl);
                                                                       // jeśli jedzie na W5 albo koniec toru, to można
                                                   zmienić kierunek                                        return (iDrivigFlags & moveTrackEnd) ?
                                                   TCommandType::cm_ChangeDirection :                                        TCommandType::cm_Unknown;
                                                                       // TBD: request direction change also if VelNext
                                                   in current direction is 0,
                                                                       // while signal behind requests movement?
                                                                   }
                                                               }
                                                               else
                                                               {
                                                                   // ustawiamy prędkość tylko wtedy, gdy ma ruszyć,
                                                   albo stanąć albo ma stać pod
                                                                   // tarczą stop trzeba powtarzać, bo inaczej zatrąbi
                                                   i pojedzie sam if
                                                                   // ((MoverParameters->Vel==0.0)||(scanvel==0.0))
                                                   //jeśli jedzie lub ma stanąć/stać                                        { // nie dostanie komendy jeśli
                                                   jedzie i ma jechać
                                                                     //
                                                   PutCommand("ShuntVelocity",scanvel,e->Params[9].asMemCell->Value2(),&sl,stopSem);
                                                   #if LOGBACKSCAN
                                                                       WriteLog(edir + " - [ShuntVelocity] [" +
                                                   to_string(scanvel, 2) + "] [" +                                        to_string(e->input_value(2), 2) +
                                                   "]");                                        #endif                                        return (scanvel > 0 ? TCommandType::cm_ShuntVelocity :
                                                                                             TCommandType::cm_Unknown);
                                                                   }
                                                               }
                                                               ...
                                                               {
                                                                   // jeśli Tm w odległości do 100m podaje zezwolenie
                                                   na jazdę, to od razu ją
                                                                   // ignorujemy, aby móc szukać kolejnej eSignSkip=e;
                                                   //wtedy uznajemy ignorowaną przy
                                                                   // poszukiwaniu nowej
                                                   #if LOGBACKSCAN
                                                   ...
                                                   */
            return (
                (VelNext > 0) ? TCommandType::cm_Unknown
                              :  // no need to bother if we can continue in
                                 // current direction
                    (scanvel == 0) ? TCommandType::cm_Unknown
                                   :  // no need to bother with a stop signal
                    TCommandType::cm_ShuntVelocity);  // otherwise report a
                                                      // relevant shunt signal
                                                      // behind
        }                                             // if (move?...
    }  // if (OrderCurrentGet()==Shunt)

    return (
        ((e->m_passive) && (e->is_command())) ? TCommandType::cm_Command
                                              : TCommandType::cm_Unknown);
};

std::string TController::NextStop()
    const {  // informacja o następnym zatrzymaniu, wyświetlane pod [F1]
...
void TController::TakeControl(
    bool const Aidriver,
    bool const Forcevehiclecheck) {  // przejęcie kontroli przez AI albo oddanie
    ...
    {              // teraz AI prowadzi
        ...
        TableClear();  // ponowne utworzenie tabelki, bo człowiek mógł pojechać
                       // niezgodnie z sygnałami
        if (action() != TAction::actSleep) {
            // gdy zgaszone światła, flaga podjeżdżania pod semafory pozostaje
            // bez zmiany conditional below disabled to get around the situation
            // where the AI train does nothing ever because it is waiting for
            // orders which don't come until the engine is engaged, i.e.
            // effectively never
            if (OrderCurrentGet()) {
                // jeśli coś robi
                ...
                // jeśli nic nie robi
                OrderNext(Prepare_engine);
                if (pVehicle->MoverParameters->iLights[(
                        mvOccupied->CabActive < 0 ? end::rear : end::front)] &
                    (light::headlight_left | light::headlight_right |
                     light::headlight_upper))  // któreś ze świateł zapalone?
                {   // od wersji 357 oczekujemy podania komend dla AI przez
                    // scenerię
                    /*
                                        if( pVehicle->MoverParameters->iLights[
                       mvOccupied->CabActive < 0 ? end::rear : end::front ] &
                       light::headlight_upper ) // górne światło zapalone
                                            OrderNext( Obey_train ); // jazda
                       pociągowa else OrderNext( Shunt ); // jazda manewrowa
                    */
                    if (mvOccupied->Vel >=
                        1.0)  // jeśli jedzie (dla 0.1 ma stać)
                        iDrivigFlags &= ~moveStopHere;  // to ma nie czekać na
                                                        // sygnał, tylko jechać
                    else
                        iDrivigFlags |=
                            moveStopHere;  // a jak stoi, to niech czeka
                }
            }
            CheckVehicles();  // ustawienie świateł
        }
    } else {  // a teraz użytkownik
...
void TController::DirectionForward(
    bool forward) {  // ustawienie jazdy do przodu dla true i do tyłu dla false
                     // (zależy od kabiny)
    ZeroSpeed(true);  // TODO: check if force switch is needed anymore here
    // HACK: make sure the master controller isn't set in position which
    // prevents direction change
    mvControlling->MainCtrlPos = std::min(
        mvControlling->MainCtrlPos, mvControlling->MainCtrlMaxDirChangePos);
...
Mtable::TTrainParameters const &TController::TrainTimetable() const {
    return TrainParams;
}

std::string TController::Relation() const {  // zwraca relację pociągu
    return TrainParams.ShowRelation();
};

const std::string &TController::TrainName() const {  // zwraca numer pociągu
    return TrainParams.TrainName;
};

int TController::StationCount()
    const {  // zwraca ilość stacji (miejsc zatrzymania)
    return TrainParams.StationCount;
};

int TController::StationIndex()
    const {  // zwraca indeks aktualnej stacji (miejsca zatrzymania)
    return TrainParams.StationIndex;
};

bool TController::IsStop()
    const {  // informuje, czy jest zatrzymanie na najbliższej stacji
    return TrainParams.IsStop();
};

// returns most recently calculated distance to potential obstacle ahead
double TController::TrackObstacle() const { return Obstacle.distance; }

void TController::MoveTo(TDynamicObject *to) {  // przesunięcie AI do innego
                                                // pojazdu (przy zmianie kabiny)
    if ((to->Mechanik != nullptr) && (to->Mechanik != this)) {
...
void TController::ControllingSet() {  // znajduje człon do sterowania w EZT
                                      // będzie to silnikowy
    // problematyczne jest sterowanie z członu biernego, dlatego damy AI
    // silnikowy dzięki temu będzie wirtualna kabina w silnikowym, działająca w
    // rozrządczym w plikach FIZ zostały zgubione ujemne maski sprzęgów, stąd
    // problemy z EZT
    mvOccupied =
        pVehicle->MoverParameters;  // domyślny skrót do obiektu parametrów
    BrakeSystem = mvOccupied->BrakeSystem;  // domyślny sposób hamowania
    mvControlling = pVehicle->FindPowered()
                        ->MoverParameters;  // poszukiwanie członu sterowanego
    {
        auto *lookup{pVehicle->FindPantographCarrier()};
        mvPantographUnit =
            (lookup != nullptr ? lookup->MoverParameters : mvControlling);
    }
    BrakeSystem = consist_brake_system();
};

std::string TController::TableText(
    std::size_t const Index) const {  // pozycja tabelki prędkości
...
int TController::CrossRoute(
    TTrack *tr) {  // zwraca numer segmentu dla skrzyżowania (tr)
    // pożądany numer segmentu jest określany podczas skanowania drogi
    // droga powinna być określona sposobem przejazdu przez skrzyżowania albo
    // współrzędnymi miejsca docelowego
    for (std::size_t i = 0; i < sSpeedTable.size();
         ++i) {  // trzeba przejrzeć tabelę skanowania w poszukiwaniu (tr)
        // i jak się znajdzie, to zwrócić zapamiętany numer segmentu i kierunek
        // przejazdu
        // (-6..-1,1..6)
        if ((true == TestFlag(sSpeedTable[i].iFlags, spEnabled | spTrack)) &&
            ...
            return (
                sSpeedTable[i].iFlags >>
                28);  // najstarsze 4 bity jako liczba -8..7
        }
    }
     ...
     && ( iRouteWanted != d ) ) { // nowy kierunek
        ...
        if( mvOccupied->CategoryFlag & 2 ) {
            ...
            for( std::size_t i = 0; i < sSpeedTable.size(); ++i ) {
                // szukanie pierwszego skrzyżowania i resetowanie kierunku na
nim if( true == TestFlag( sSpeedTable[ i ].iFlags, spEnabled | spTrack ) ) {
                    // jeśli pozycja istotna (1) oraz odcinek (2)
                    if( false == TestFlag( sSpeedTable[ i ].iFlags, spElapsed )
) {
                        // odcinek nie może być miniętym
                        if( sSpeedTable[ i ].trTrack->eType == tt_Cross ) //
jeśli skrzyżowanie
                        {
                            while( sSpeedTable.size() >= i ) {
                                // NOTE: we're ignoring semaphor flags and not
resetting them like we do for train route trimming
...
void TController::update_timers(double dt) {
    ElapsedTime += dt;
    WaitingTime += dt;
    fBrakeTime -= dt;  // wpisana wartość jest zmniejszana do 0, gdy ujemna
                       // należy zmienić nastawę hamulca
    ...
    fActionTime +=
        dt;  // czas używany przy regulacji prędkości i zamykaniu drzwi
    LastReactionTime += dt;
...
void TController::update_logs(double const dt) {
    // log vehicle data
    LastUpdatedTime += dt;
    if ((WriteLogFlag) && (LastUpdatedTime > deltalog)) {
        // zapis do pliku DAT
        PhysicsLog();
...
void TController::determine_consist_state() {
    // ABu-160305 testowanie gotowości do jazdy
    // Ra: przeniesione z DynObj, skład użytkownika też jest testowany, żeby mu
    // przekazać, że ma odhamować

    if (mvOccupied->CabActive == 0 && mvOccupied->Power24vIsAvailable) {
        cue_action(driver_hint::cabactivation);
    } else if (
        !mvOccupied->AutomaticCabActivation &&
        ((mvOccupied->CabActive == -mvOccupied->CabOccupied) ||
         (!mvOccupied->CabMaster) || (!mvOccupied->Power24vIsAvailable))) {
        cue_action(driver_hint::cabdeactivation);
    }

    int index =
        double(BrakeAccTableSize) * (mvOccupied->Vel / mvOccupied->Vmax);
    ...
    if ((is_emu()) || (is_dmu())) {
        ...
        fAccThreshold = fNominalAccThreshold * Coeff -
                        fBrake_a0[BrakeAccTableSize] * (1.0 - Coeff);
    }
        ...
        (((mvOccupied->BrakeSystem == TBrakeSystem::ElectroPneumatic) &&
          (false == ForcePNBrake))
             ...
             : mvOccupied->PipePress <
                   std::max(
                       3.9, mvOccupied->BrakePressureActual.PipePressureVal) +
                       0.1);
    ...
    IsAnyDoorPermitActive[side::right] = IsAnyDoorPermitActive[side::left] =
        false;
    ConsistShade = 0.0;
    auto *p{pVehicles[end::front]};  // pojazd na czole składu
    double dy;   // składowa styczna grawitacji, w przedziale <0,1>
    while (p) {  // sprawdzenie odhamowania wszystkich połączonych pojazdów
        auto *vehicle{p->MoverParameters};
        auto const bp{std::max(
            0.0, vehicle->BrakePress -
                     (vehicle->SpeedCtrlUnit.Parking
                          ? vehicle->MaxBrakePress[0] * vehicle->StopBrakeDecc
                          : 0.0))};
        if (Ready) {
            // bo jak coś nie odhamowane, to dalej nie ma co sprawdzać
            if ((TestFlagAny(
                    vehicle->Hamulec->GetBrakeStatus(), (b_hld | b_on))) ||
                ((vehicle->Vel < 1.0 ? (bp > 0.4)
                                     :  // ensure the brakes are sufficiently
                                        // released when starting to move
                      (vehicle->Fb * 0.001 >
                       10.0)))) {  // once in motion we can make a more lenient
                                   // check
                Ready = false;
            }
            // Ra: odluźnianie przeładowanych lokomotyw, ciągniętych na zimno -
            // prowizorka...
            if (bp >= 0.4) {  // wg UIC określone sztywno na 0.04
                if (AIControllFlag || (Global.AITrainman &&
                    ...
                    if ((BrakeCtrlPosition == gbh_RP)  // jest pozycja jazdy
                        && (false == TestFlag(
                                         vehicle->Hamulec->GetBrakeStatus(),
                                         b_dmg))  // brake isn't broken
                        && (vehicle->PipePress - mvOccupied->Handle->GetRP() >
                            -0.1)  // jeśli ciśnienie jak dla jazdy
                        &&
                        (vehicle->Hamulec->GetCRP() >
                         vehicle->PipePress + 0.12)) {  // za dużo w zbiorniku
            ...
            if (bp < 0.1) {
                if (AIControllFlag || Global.AITrainman) {
                    if ((false == TestFlag(
                        ...
                        && (vehicle->Hamulec->GetCRP() <
                            vehicle->PipePress -
                                0.1)) {  // już nie jest za dużo w zbiorniku
        ...
        if ((dy = p->VectorFront().y) != 0.0) {
            ...
            fAccGravity -= vehicle->TotalMassxg * dy *
                           (p->DirectionGet() == iDirection ? 1 : -1);
        }
        // check coupler state
        ...
        // check door state
        {
            auto const switchsides{
                p->DirectionGet() !=
                (iDirection == 0 ? mvOccupied->CabOccupied : iDirection)};
            auto const &rightdoor{
                vehicle->Doors
                    .instances[(switchsides ? side::left : side::right)]};
            auto const &leftdoor{
                vehicle->Doors
                    .instances[(switchsides ? side::right : side::left)]};
            ...
            if (vehicle->Doors.close_control != control_t::autonomous) {
                IsAnyDoorOnlyOpen[side::right] |=
                    (false == rightdoor.is_door_closed);
                IsAnyDoorOnlyOpen[side::left] |=
                    (false == leftdoor.is_door_closed);
            }
    ...
    pVehicle->for_each(coupling::control, [this](TDynamicObject *Vehicle) {
        ...
        IsAnyCompressorEnabled |=
            (vehicle->CompressorSpeed > 0.0
                 ? (vehicle->CompressorAllow ||
                    vehicle->CompressorStart == start_t::automatic) &&
                       (vehicle->CompressorAllowLocal)
                 : false);
        IsAnyCompressorExplicitlyEnabled |=
            (vehicle->CompressorSpeed > 0.0
                 ? (vehicle->CompressorAllow && vehicle->CompressorAllowLocal)
                 : false);
        IsAnyConverterPresent |= (vehicle->ConverterStart != start_t::disabled);
        IsAnyConverterEnabled |=
            (vehicle->ConverterAllow ||
             vehicle->ConverterStart == start_t::automatic) &&
            (vehicle->ConverterAllowLocal);
        IsAnyConverterExplicitlyEnabled |=
            (vehicle->ConverterAllow && vehicle->ConverterAllowLocal);
        if (vehicle->Power > 0.01) {
            IsAnyLineBreakerOpen |= !(vehicle->Mains);
        }
    });

    // siłę generują pojazdy na pochyleniu ale działa ona całość składu, więc
    // a=F/m
    fAccGravity *= (iDirection >= 0 ? 1 : -1);
    fAccGravity /= fMass;
    {
        auto absaccs{fAccGravity};  // Ra 2014-03: jesli skład stoi, to działa
                                    // na niego składowa styczna grawitacji
        if (mvOccupied->Vel > EU07_AI_NOMOVEMENT) {
            absaccs = 0;
            auto *d = pVehicles[end::front];  // pojazd na czele składu
            while (d) {
                absaccs += d->MoverParameters->TotalMass *
                           d->MoverParameters->AccS *
                           (d->DirectionGet() == iDirection ? 1 : -1);
                d = d->Next();  // kolejny pojazd, podłączony od tyłu (licząc od
                                // czoła)
            }
...
#if LOGVELOCITY
    WriteLog(
        "Vel=" + to_string(DirectionalVel()) + ", AbsAccS=" +
        to_string(AbsAccS) + ", AccGrav=" + to_string(fAccGravity));
#endif

    if ((!Ready)  // v367: jeśli wg powyższych warunków skład nie jest
                  // odhamowany
        ...
        if (has_diesel_engine()) {
            Ready =
                ((vehicle->Vel > EU07_AI_MOVEMENT)  // already moving
                 ...
                 || (vehicle->enrot >
                     0.8 * (vehicle->EngineType == TEngineType::DieselEngine
                                ? vehicle->dizel_nmin
                                : vehicle->DElist[0].RPM / 60.0)));
        }
        p = p->Next();  // pojazd podłączony z tyłu (patrząc od czoła)
    }
    // test consist state for external events and/or weird things human user
    // could've done
    iEngineActive &=
...
void TController::control_wheelslip() {
    // wheel slip
    if (mvControlling->SlippingWheels) {
        ...
        cue_action(
            driver_hint::sandingon);  // piaskujemy tory, coby się nie ślizgać
    } else {
...
void TController::control_pantographs() {
    if (mvPantographUnit->EnginePowerSource.SourceType !=
        TPowerSource::CurrentCollector) {
        return;
    }
    ...
    if ((false == mvPantographUnit->PantAutoValve) &&
        ...
        // gdy główna sprężarka bezpiecznie nabije ciśnienie to można przestawić
        // kurek na zasilanie pantografów z głównej pneumatyki
        cue_action(driver_hint::pantographairsourcesetmain);
    }

    // uśrednione napięcie sieci: przy spadku poniżej wartości minimalnej
    // opóźnić rozruch o losowy czas
    fVoltage = 0.5 * (fVoltage + std::max(
                                     mvControlling->GetTrainsetHighVoltage(),
                                     mvControlling->PantographVoltage));
    if (fVoltage < mvControlling->EnginePowerSource.CollectorParameters.MinV) {
        ...
        if (fActionTime >= PrepareTime) {
            // jeśli czas oczekiwania nie został ustawiony, losowy czas
            // oczekiwania przed ponownym załączeniem jazdy
    ...
    if (mvOccupied->Vel > EU07_AI_NOMOVEMENT) {
                ...
                if (mvOccupied->AIHintPantstate == 0) {
                    ...
                    if ((iDirection >= 0) && (useregularpantographlayout)) {
                        ...
                        if ((mvPantographUnit->PantRearVolt == 0.0)
                            // filter out cases with single _other_ working
                            // pantograph so we don't try to raise something we
                            // can't
                            && ((mvPantographUnit->PantographVoltage == 0.0) ||
                                (mvPantographUnit->EnginePowerSource
                                     .CollectorParameters.CollectorsNo > 1))) {
                    ...
                    if (mvOccupied->Vel > 5) {
                        // opuszczenie przedniego po rozpędzeniu się o ile jest
                        // więcej niż jeden
                        if (mvPantographUnit->EnginePowerSource
                                .CollectorParameters.CollectorsNo > 1) {
                            if ((iDirection >= 0) &&
                                (useregularpantographlayout))  // jak jedzie w
                                                               // kierunku
                                                               // sprzęgu 0
                            {  // poczekać na podniesienie tylnego
                                if ((mvPantographUnit->PantFrontVolt != 0.0) &&
                                    (mvPantographUnit->PantRearVolt !=
                                     0.0)) {  // czy jest napięcie zasilające na
                                              // tylnym?
                                    cue_action(
                                        driver_hint::
                                            frontpantographvalveoff);  // opuszcza
                                                                       // od
                                                                       // sprzęgu
                                                                       // 0
                                }
                                ...
                                if ((mvPantographUnit->PantRearVolt != 0.0) &&
                                    (mvPantographUnit->PantFrontVolt !=
                                     0.0)) {  // czy jest napięcie zasilające na
                                              // przednim?
                                    cue_action(
                                        driver_hint::
                                            rearpantographvalveoff);  // opuszcza
                                                                      // od
                                                                      // sprzęgu
                                                                      // 1
                                }
        ...
        if ((mvOccupied->AIHintPantUpIfIdle) &&
            (IdleTime > 45.0)
            // NOTE: abs(stoptime) covers either at least 15 sec remaining for a
            // scheduled stop, or 15+ secs spent at a basic stop
            && (std::abs(fStopTime) > 15.0)) {
            // spending a longer at a stop, raise also front pantograph
            if (mvPantographUnit->EnginePowerSource.CollectorParameters
                    .CollectorsNo > 1) {
                if ((iDirection >= 0) && (useregularpantographlayout)) {
                    // jak jedzie w kierunku sprzęgu 0
                    if (mvPantographUnit->PantFrontVolt == 0.0) {
                        cue_action(
                            driver_hint::frontpantographvalveon,
                            5);  // discard the hint if speed exceeds 5 km/h
                    }
                } else {
                    if (mvPantographUnit->PantRearVolt == 0.0) {
                        cue_action(
...
void TController::control_horns(double const Timedelta) {
    // horn control
    ...
    if (mvOccupied->Vel > EU07_AI_MOVEMENT) {
        ...
        iDrivigFlags &=
            ~moveStartHornDone;  // zatrąbi dopiero jak następnym razem stanie
        iDrivigFlags |= moveStartHorn;  // i trąbić przed następnym ruszeniem
    }

    if ((true == TestFlag(iDrivigFlags, moveStartHornNow)) && (true == Ready) &&
        (true == iEngineActive) && (mvControlling->MainCtrlPowerPos() > 0)) {
...
void TController::control_security_system(double const Timedelta) {
    if (mvOccupied->SecuritySystem.is_blinking()) {
        // jak zadziałało CA/SHP
        if ((false == is_emu()) && (mvOccupied->DirActive == 0)) {
            cue_action(driver_hint::directionforward);
        }
        cue_action(driver_hint::securitysystemreset);  // to skasuj
        if (BrakeCtrlPosition ==
                0  // TODO: verify whether it's 0 in all vehicle types
            && AccDesired > 0.0 && mvOccupied->SecuritySystem.is_braking()) {
            cue_action(driver_hint::trainbrakerelease);
        }
    }
    ...
    if ((true == mvOccupied->RadioStopFlag)  // radio-stop
        ...
        // turning off the radio should reset the flag, during security system
        // check
        if (m_radiocontroltime >
            5.0) {  // arbitrary delay between stop and disabling the radio
            cue_action(driver_hint::radiooff);
        } else {
            m_radiocontroltime += Timedelta;
        }
    }
    if ((iEngineActive) && (false == mvOccupied->Radio) &&
...
void TController::control_handles() {
    switch (mvControlling->EngineType) {
        case TEngineType::ElectricSeriesMotor: {
            ...
            // if the power station is heavily burdened drop down to series mode
            // to reduce the load
            auto const useseriesmodevoltage{interpolate(
                mvControlling->EnginePowerSource.CollectorParameters.MinV,
            ...
            // sanity check
            if ((false == Ready) && (mvControlling->MainCtrlPowerPos() > 1)) {
                cue_action(driver_hint::mastercontrollersetzerospeed);
            }
            break;
        }
...
void TController::control_lights() {
    if (is_train()) {
        ...
        if (true == TestFlag(OrderCurrentGet(), Obey_train)) {
            ...
            if (m_lighthints[end::front] == -1) {
                ...
                (light::redmarker_left | light::headlight_right |
                 light::headlight_upper)) {
...
void TController::control_compartment_lights() {
    // compartment lights
    if (mvOccupied->CompartmentLights.start_type != start_t::manual) {
        return;
    }
    ...
    auto const desiredlightstate{
        (currentlightstate ? lightlevel < 0.40
                           :       // turn off if lighting level goes above 0.4
             lightlevel < 0.35)};  // turn on if lighting level goes below 0.35
    if (desiredlightstate != currentlightstate) {
        cue_action(
            (desiredlightstate ? driver_hint::consistlightson
                               : driver_hint::consistlightsoff));
    }
}

void TController::control_doors() {
    if (false == doors_open()) {
        return;
    }
    // jeżeli jedzie
    // nie zamykać drzwi przy drganiach, bo zatrzymanie na W4 akceptuje
    // niewielkie prędkości
    if (mvOccupied->Vel > EU07_AI_MOVEMENT) {
        Doors(false);
        return;
    }
    // HACK: crude way to deal with automatic door opening on W4 preventing
    // further ride for human-controlled vehicles with no door control and
    // dynamic brake auto-activating with door open
...
void TController::UpdateCommand() {
...
void TController::UpdateNextStop() {
    if ((fLastStopExpDist > 0.0) &&
        ...
        if (true == m_makenextstopannouncement) {
            ...
            m_makenextstopannouncement =
                false;  // keep next stop announcements suppressed until another
                        // scheduled stop
        }
    }
}

void TController::determine_braking_distance() {
    // przybliżona droga hamowania
    // NOTE: we're ensuring some minimal braking distance to reduce ai flipping
    // states between starting and braking
    ...
    if ((mvOccupied->Vel > 15.0) &&
        ...
        (is_emu())) {
        // HACK: make the induction motor powered EMUs start braking slightly
        // earlier
        fBrakeDist += 10.0;
    }
    /*
        // take into account effect of gravity (but to stay on safe side of
       calculations, only downhill) if( fAccGravity > 0.025 ) { fBrakeDist *=
       ( 1.0 + fAccGravity );
            // TBD: use version which shortens route going uphill, too
            //fBrakeDist = std::max( fBrakeDist, fBrakeDist * ( 1.0 +
       fAccGravity ) );
        }
    */
}

void TController::scan_route(double const Range) { TableCheck(Range); }

// check for potential collisions
void TController::scan_obstacles(double const Range) {
    // HACK: vehicle order in the consist is based on intended travel direction
    // if our actual travel direction doesn't match that, we should be scanning
    // from the other end of the consist we cast to int to avoid getting
    // confused by microstutters
    auto *frontvehicle{pVehicles[(
        static_cast<int>(mvOccupied->V) * iDirection >= 0 ? end::front
                                                          : end::rear)]};

    int routescandirection;
    // for moving vehicle determine heading from velocity; for standing fall
    // back on the set direction
    if ((std::abs(frontvehicle->MoverParameters->V) > 0.5
             ?  // ignore potential micro-stutters in oposite direction during
                // "almost stop"
    ...
    */
    ...
    auto const lookup{
        frontvehicle->find_vehicle(routescandirection, obstaclescanrange)};
    ...
    if (std::get<bool>(lookup) == true) {
        ...
        if (Obstacle.distance < (is_car() ? 50 : 100)) {
            ...
            Obstacle.distance = TMoverParameters::CouplerDist(
                frontvehicle->MoverParameters,
                Obstacle.vehicle->MoverParameters);
        }
    }
}

void TController::determine_proximity_ranges() {
    // ustalenie dystansów w pozostałych przypadkach
    switch (OrderCurrentGet()) {
        case Connect: {
            // podłączanie do składu
            if (iDrivigFlags & moveConnect) {
                // jeśli stanął już blisko, unikając zderzenia i można próbować
                // podłączyć
                fMinProximityDist = -1.0;
                fMaxProximityDist = 0.0;  //[m] dojechać maksymalnie
                fVelPlus = 1.0;  // dopuszczalne przekroczenie prędkości na
                                 // ograniczeniu bez hamowania
                fVelMinus =
                    0.5;  // margines prędkości powodujący załączenie napędu
            } else {
                // jak daleko, to jazda jak dla Shunt na kolizję
                fMinProximityDist = 2.0;
                fMaxProximityDist =
                    5.0;  //[m] w takim przedziale odległości powinien stanąć
                fVelPlus = 2.0;  // dopuszczalne przekroczenie prędkości na
                                 // ograniczeniu bez hamowania
                fVelMinus =
                    1.0;  // margines prędkości powodujący załączenie napędu
            }
            break;
        }
        case Disconnect: {
        ...
        case Shunt: {
            ...
            // HACK: modern vehicles might brake slower at low speeds, increase
            // safety margin as crude counter
            if (mvControlling->EIMCtrlType > 0) {
                fMinProximityDist += 5.0;
                fMaxProximityDist += 5.0;
            }
            // take into account weather conditions
            if ((Global.FrictionWeatherFactor < 1.f) && (iVehicles > 1)) {
                fMinProximityDist += 5.0;
                fMaxProximityDist += 5.0;
            }
            fVelPlus = 2.0;  // dopuszczalne przekroczenie prędkości na
        ...
        case Obey_train: {
            ...
            if (is_train()) {
                ...
                if (IsCargoTrain) {
                    // increase distances for cargo trains to take into account
                    // slower reaction to brakes
                    fMinProximityDist += 10.0;
                    fMaxProximityDist += 10.0;
                    /*
                                    if( IsHeavyCargoTrain ) {
                                        // cargo trains with high braking
                       threshold may require even larger safety margin
                ...
                if (iDrivigFlags & moveLate) {
                    ...
                    fVelMinus = 1.0;
                    // dopuszczalne przekroczenie prędkości na ograniczeniu bez
                    // hamowania
                    fVelPlus = 5.0;
                } else {
                    // gdy nie musi się sprężać
                    // margines prędkości powodujący załączenie napędu; min 1.0
                    // żeby nie ruszał przy 0.1
                    fVelMinus = clamp(std::round(0.05 * VelDesired), 1.0, 5.0);
                    // normalnie dopuszczalne przekroczenie to 5% prędkości ale
                    // nie więcej niż 5km/h bottom margin raised to 2 km/h to
                    // give the AI more leeway at low speed limits
            ...
            // samochod (sokista też)
                ...
                fMaxProximityDist =
                    std::max(9.5, mvOccupied->Vel * 0.375);  //[m]
...
void TController::check_departure() {
    if ((OrderCurrentGet() &
         (Shunt | Loose_shunt | Obey_train | Bank | Connect)) == 0) {
        return;
    }
    ...
    if ((VelSignal == 0.0) && (WaitingTime > 0.0) &&
        ...
        if ((OrderCurrentGet() & (Shunt | Loose_shunt | Obey_train | Bank)) &&
            ...
            } else {  // samochód ma stać, aż dostanie odjazd, chyba że stoi
                      // przez kolizję
        ...
        (VelSignal == 0.0) && (VelNext > 0.0) && (mvOccupied->Vel < 1.0)) {
...
void TController::check_load_exchange() {
    ExchangeTime = 0.f;
    ...
    while (vehicle != nullptr) {
        ...
        DoesAnyDoorNeedOpening |=
            ((vehicleexchangetime > 0) && (vehicle->LoadExchangeSpeed() == 0));
        ExchangeTime = std::max(ExchangeTime, vehicleexchangetime);
        vehicle = vehicle->Next();
    }

    if (pVehicle->DirectionGet() != m_lastexchangedirection) {
        // generally means the ai driver moved to the opposite end of the
        // consist
        // TODO: investigate whether user playing with the reverser can mess
        // this up
...
void TController::UpdateChangeDirection() {
    // TODO: rework into driver mode independent routine
    if (false == TestFlag(OrderCurrentGet(), Change_direction)) {
        return;
    }
    ...
    if (true == AIControllFlag) {
        ...
        if (mvOccupied->Vel < EU07_AI_NOMOVEMENT) {
            // jeśli się zatrzymał, to zmieniamy kierunek jazdy, a nawet
            // kabinę/człon
            Activation();  // ustawienie zadanego wcześniej kierunku i
                           // ewentualne przemieszczenie AI
        }                  // Change_direction (tylko dla AI)
    }
    // shared part of the routine, implement if direction matches what was
    // requested
    if ((mvOccupied->Vel < EU07_AI_NOMOVEMENT) &&
        (iDirection == iDirectionOrder)) {
        // NOTE: we can't be sure there's a visible signal within scan range
        // after direction change which would normally overwrite the old limit,
        // so we reset signal value manually here
        VelSignal = -1;
        VelSignalNext = -1;
        PrepareEngine();
        JumpToNextOrder();  // następnie robimy, co jest do zrobienia (Shunt
                            // albo Obey_train)
        if (OrderCurrentGet() & (Shunt | Loose_shunt | Connect)) {
...
void TController::UpdateLooseShunt() {
    // when loose shunting try to detect situations where engaged brakes in a
    // consist to be pushed prevent movement
    // TODO: run also for potential settings-based virtual assistant
    auto const autobrakerelease{(true == AIControllFlag)};

    if ((autobrakerelease) && (Obstacle.distance < 1.0) && (AccDesired > 0.1) &&
        (mvOccupied->Vel < 1.0)) {
        auto *vehicle{Obstacle.vehicle};
        auto const direction{
            (vehicle->Prev() != nullptr ? end::front : end::rear)};
        while (vehicle != nullptr) {
            if (vehicle->MoverParameters->BrakePress > 0.2) {
                vehicle->MoverParameters->BrakeLevelSet(
                    0);  // hamulec na zero, aby nie hamował
                vehicle->MoverParameters->BrakeReleaser(
            ...
            vehicle =
                (direction == end::front ? vehicle->Prev() : vehicle->Next());
        }
    }
}

void TController::UpdateObeyTrain() {
    UpdateNextStop();

    if (((iDrivigFlags & moveGuardSignal) != 0) && (VelDesired > 0.0)) {
        ...
        if (false == tsGuardSignal.empty()) {
            tsGuardSignal.stop();
            // w zasadzie to powinien mieć flagę, czy jest dźwiękiem radiowym,
            // czy bezpośrednim albo trzeba zrobić dwa dźwięki, jeden
            // bezpośredni, słyszalny w pobliżu, a drugi radiowy, słyszalny w
            // innych lokomotywach na razie zakładam, że to nie jest dźwięk
            // radiowy, bo trzeba by zrobić obsługę kanałów radiowych itd.
            if (iGuardRadio == 0) {
                // jeśli nie przez radio
                tsGuardSignal.owner(pVehicle);
                // place virtual conductor some distance away
                tsGuardSignal.offset(
                    {pVehicle->MoverParameters->Dim.W * -0.75f, 1.7f,
                     std::min(-20.0, -0.2 * fLength)});
                tsGuardSignal.play(sound_flags::exclusive);
                // NOTE: we can't rely on is_playing() check as sound playback
                // is based on distance from local camera
                fActionTime = -5.0;  // niech trochę potrzyma
            } else {
                // radio message
                tsGuardSignal.owner(pVehicle);
                /*
                                tsGuardSignal.offset( { 0.f, 2.f,
                   pVehicle->MoverParameters->Dim.L * 0.4f * (
                   pVehicle->MoverParameters->CabOccupied < 0 ? -1 : 1 ) } );
                                tsGuardSignal.play( sound_flags::exclusive );
                */
...
void TController::UpdateConnect() {
    // podłączanie do składu
    if (iDrivigFlags & moveConnect) {
        // sprzęgi sprawdzamy w pierwszej kolejności, bo jak połączony, to
        // koniec
            ...
            if (neighbour.vehicle != nullptr) {
                if (neighbour.distance < 10) {
                    ...
                    if (coupler.type() != TCouplerType::Automatic) {
                        auto const &othercoupler =
                            neighbour.vehicle->MoverParameters->Couplers[(
                                neighbour.vehicle_end != 2
                                    ? neighbour.vehicle_end
                                    : coupler.ConnectedNr)];
                        if (othercoupler.type() == TCouplerType::Automatic) {
                            coupleriscompatible = false;
                            cue_action(
                                driver_hint::couplingadapterattach,
                                couplingend);
                        }
                    }
                    // próba podczepienia
                    if (AIControllFlag || Global.AITrainman) {
                        if ((coupleriscompatible) && (neighbour.distance < 2)) {
                            vehicleparameters->Attach(
                                ...
                                neighbour.vehicle->MoverParameters, iCoupler);
                        }
                    }
                }
            }
        }
        // NOTE: no else as the preceeding block can potentially establish
        // expected connection
        if (vehicleparameters->Couplers[couplingend].CouplingFlag == iCoupler) {
        ...
        if (Obstacle.distance <= 20.0) {
            if (!iCouplingVehicle) {
                // write down which vehicle should be coupled with the target
                // consist, so we don't lose track of it if the user does
                // something unexpected
                iCouplingVehicle = {
                    pVehicles[end::front],
                    (pVehicles[end::front]->DirectionGet() > 0 ? end::front
                                                               : end::rear)};
...
void TController::GuardOpenDoor() {
    if ((iDrivigFlags & moveGuardOpenDoor) != 0) {
        auto *vehicle{pVehicles[end::front]};
        while (vehicle != nullptr &&
               vehicle->MoverParameters->Doors.range == 0) {
            vehicle = vehicle->Next();
        }
        if (vehicle != nullptr && vehicle->MoverParameters->Doors.range > 0) {
            ...
            if (m_lastexchangeplatforms & lewe) {
                vehicle->MoverParameters->OperateDoors(
                    side::left, true, range_t::local);
            }
            if (m_lastexchangeplatforms & prawe) {
                vehicle->MoverParameters->OperateDoors(
                    side::right, true, range_t::local);
            }
        } else {
            iDrivigFlags &= ~moveGuardOpenDoor;
        }
...
int TController::unit_count(int const Threshold) const {
    auto *vehicle{pVehicle};
    auto unitcount{1};
    do {
        auto const decoupledend{
            (vehicle->DirectionGet() > 0
                 ?  // numer sprzęgu od strony czoła składu
                 end::rear
                 : end::front)};
        auto const coupling{
            vehicle->MoverParameters->Couplers[decoupledend].CouplingFlag};
...
void TController::UpdateDisconnect() {
    if (iVehicleCount >= 0) {
        // early test for human drivers, who might disregard the proper
        // procedure, or perform it before they receive the order to
        if (false == AIControllFlag) {
            // iVehicleCount = 0 means the consist should be reduced to (1)
            // leading unit, thus we increase our test values by 1
        ...
        // regular uncoupling procedure, performed by ai and naively expected
        // from the human drivers 3rd stage: change direction, compress buffers
        // and uncouple
        ...
        if (((iDrivigFlags & movePress) != 0) &&
            ...
            if (iVehicleCount >= 0) {
                ...
                WriteLog(
                    "Uncoupling [" + mvOccupied->Name +
                    "]: actuating and compressing buffers...");
                cue_action(driver_hint::independentbrakerelease);
                cue_action(driver_hint::bufferscompress);
            }

            WriteLog(
                "Uncoupling [" + mvOccupied->Name + "]: from " +
                (mvOccupied->DirAbsolute > 0 ? "front" : "rear"));
            auto *decoupledvehicle{
                pVehicle};     // pojazd do odczepienia, w (pVehicle) siedzi AI
            int decoupledend;  // numer sprzęgu, który sprawdzamy albo
                               // odczepiamy
            auto vehiclecount{iVehicleCount};  // ile wagonów ma zostać
            // szukanie pojazdu do odczepienia
            do {
                decoupledend = decoupledvehicle->DirectionGet() > 0
                                   ?  // numer sprzęgu od strony czoła składu
                                   end::front
                                   : end::rear;
                // jeżeli sprzęg zablokowany to liczymy człony jako jeden
                if (decoupledvehicle->MoverParameters->Couplers[decoupledend]
                        .CouplingFlag &
                    coupling::permanent) {
                    ++vehiclecount;
                }
                if (decoupledvehicle != pVehicle) {
                    decoupledvehicle->MoverParameters->BrakeReleaser(
                        1);  // wyluzuj pojazd, aby dało się dopychać
                }
                if (vehiclecount) {  // jeśli jeszcze nie koniec
                    decoupledvehicle =
                        decoupledvehicle
                            ->Prev();  // kolejny w stronę czoła składu (licząc
                                       // od tyłu), bo dociskamy
                    if (decoupledvehicle == nullptr) {
                        vehiclecount = 0;  // nie ma co dalej sprawdzać,
                                           // odczepianie zakończone
                    }
                }
            } while (vehiclecount--);
            if (decoupledvehicle == nullptr) {
                ...
                WriteLog(
                    "Uncoupling [" + mvOccupied->Name +
                    "]: didn't find anything to disconnect");
                iVehicleCount = -2;  // odczepiono, co było do odczepienia
            } else {
                if (AIControllFlag || Global.AITrainman) {
                ...
                // tylko jeśli odepnie
                if (decoupledvehicle->MoverParameters->Couplers[decoupledend]
                        .CouplingFlag == coupling::faux) {
                    WriteLog(
                        "Uncoupling [" + mvOccupied->Name + "]: uncoupled");
                    iVehicleCount = -2;
                    ...
                    /*
                                        if(
                       decoupledvehicle->MoverParameters->Couplers[ decoupledend
                       ].has_adapter() ) {
                                            decoupledvehicle->remove_coupler_adapter(
                       decoupledend );
                                        }
                    */
                    if (pVehicles[end::front]
                            ->MoverParameters->Couplers[decoupledend]
                            .has_adapter()) {
                        cue_action(
                            driver_hint::couplingadapterremove, decoupledend);
                    }
                }
        ...
        if ((iDrivigFlags & movePress) == 0) {
            ...
            // pozycji 3, zwyle 0.37)
                ...
                WriteLog(
                    "Uncoupling [" + mvOccupied->Name + "]: direction change");
                /* // TODO: test if this block is needed
                                if( BrakeSystem ==
                   TBrakeSystem::ElectroPneumatic ) {
                                    // wyłączenie EP, gdy wystarczy (może nie
                   być potrzebne, bo na początku jest)
                                    mvOccupied->BrakeLevelSet( 0 );
                                }
                */
                iDirectionOrder = -iDirection;  // zmiana kierunku jazdy na
                                                // przeciwny (dociskanie)
    ...
    if (iVehicleCount < 0) {
        ...
        if (iDirection != iDirectionOrder) {
            ...
            cue_action(driver_hint::mastercontrollersetreverserunlock);
            cue_action(driver_hint::directionother);  // zmiana kierunku jazdy
                                                      // na właściwy
        }
...
void TController::handle_engine() {
    // HACK: activate route scanning if an idling vehicle is activated by a
    // human user
    ...
    if ((OrderCurrentGet() & (Change_direction | Connect | Disconnect | Shunt |
                              Loose_shunt | Obey_train | Bank)) &&
        (false == iEngineActive)) {
...
void TController::handle_orders() {
    switch (OrderCurrentGet()) {
        case Release_engine: {
        ...
        default: {
            // special case, change_direction potentially received during
            // partially completed coupling operation this is unlikely to happen
            // for the AI, but can happen for a (slower) human user
...
void TController::pick_optimal_speed(double const Range) {
    //    if( ( OrderCurrentGet() & ( Shunt | Loose_shunt | Obey_train | Bank |
    //    Connect | Disconnect | Change_direction ) ) == 0 ) { return; }

    // set initial velocity and acceleration values
    SetDriverPsyche();     // ustawia AccPreferred (potrzebne tu?)
    VelDesired = fVelMax;  // wstępnie prędkość maksymalna dla pojazdu(-ów),
                           // będzie następnie ograniczana
    AccDesired = AccPreferred;  // AccPreferred wynika z osobowości mechanika

    // nie przekraczać rozkladowej
    if (((OrderCurrentGet() & Obey_train) != 0) && (TrainParams.TTVmax > 0.0)) {
        VelDesired = min_speed(VelDesired, TrainParams.TTVmax);
    }

    //    VelNext = VelDesired; // maksymalna prędkość wynikająca z innych
    //    czynników niż trajektoria ruchu
    // HACK: -1 means we can pick up 0 speed limits in ( point.velnext > velnext
    // ) tests aimed to find highest next speed, but also doubles as 'max speed'
    // in min_speed tests, so it shouldn't interfere if we don't find any speed
    // limits
    VelNext = -1.0;
    ...
    if ((OrderCurrentGet() & (Obey_train | Bank)) == 0) {  // spokojne manewry
        SetVelocity(fShuntVelocity, fShuntVelocity);
        VelDesired = min_speed(VelDesired, fShuntVelocity);
    }
    ...
    if ((OrderCurrentGet() & Disconnect) != 0) {
        if (iVehicleCount >= 0) {
            ...
            if (((iDrivigFlags & movePress) != 0) &&
                (iDirection == iDirectionOrder)) {
                SetVelocity(
                    2, 0);  // jazda w ustawionym kierunku z prędkością 2
            }
    ...
    SwitchClearDist = -1;
    ActualProximityDist =
        Range;  // funkcja Update() może pozostawić wartości bez zmian
    ...
    // Ra: odczyt (ActualProximityDist), (VelNext) i (AccPreferred) z tabelki
    // prędkosci
    ...
    // Ra 2F1I: wyłączyć kiedyś to uśrednianie i przeanalizować skanowanie,
    // czemu migocze
    if (AccDesired >
        EU07_AI_NOACCELERATION) {  // hamowania lepeiej nie uśredniać
    ...
    AccDesired =
        clamp(AccDesired, (is_car() ? -2.0 : -0.9), (is_car() ? 2.0 : 0.9));

    // if the route ahead is blocked we might need to head the other way
    check_route_behind(1000);  // NOTE: legacy scan range value
}

void TController::adjust_desired_speed_for_obstacles() {
    // prędkość w kierunku jazdy, ujemna gdy jedzie w przeciwną stronę niż
    // powinien
    ...
    if (Obstacle.distance < 5000) {
        ...
        if (k - vel < 5) {
            ...
            ActualProximityDist =
                std::min<double>(ActualProximityDist, Obstacle.distance);

            if (Obstacle.distance <= ((mvOccupied->CategoryFlag & 2) ? 100.0
                                                                     :  // cars
                                          250.0)) {  // others
                // regardless of driving mode at close distance take precaution
                // measures: match the other vehicle's speed or slow down if the
                // other vehicle is stopped
                VelDesired = min_speed(
                    VelDesired,
                    std::max(
                        k,
                        (mvOccupied->CategoryFlag & 2) ? 40.0 :  // cars
                            20.0));                              // others
            ...
            double const distance =
                Obstacle.distance - fMaxProximityDist -
                (fBrakeDist *
                 1.15);  // odległość bezpieczna zależy od prędkości
            if (distance < 0.0) {
                ...
                {  // jeśli tamten porusza się z niewielką prędkością albo stoi
                    ...
                    VelDesired = std::floor(min_speed(
                        VelDesired,
                        (Obstacle.distance > 100 ? k + 20.0
                                                 : std::min(8.0, k + 4.0))));

                    if ((OrderCurrentGet() & (Connect | Loose_shunt)) != 0) {
                        // jeśli spinanie, to jechać dalej
                        AccPreferred =
                            std::min(0.35, AccPreferred);  // nie hamuj
                        VelNext = std::floor(
                            std::min(8.0, k + 2.0));  // i pakuj się na tamtego
                    } else {
                        ...
                        if ((mvOccupied->CategoryFlag & 1) &&
                            ...
                            // trains which move normally should try to stop at
                            // safe margin
                    ...
                    if (Obstacle.distance <
                        ...
                        if (Obstacle.distance <=
                            ((mvOccupied->CategoryFlag & 2) ? fMaxProximityDist
                                                            :  // cars
                                 2.0 * fMaxProximityDist)) {   // others
                            // try to force speed change if obstacle is really
                            // close
                            VelDesired = VelNext;
                        }
                    }
                ...
                if (OrderCurrentGet() & Connect) {
                    ...
                    // speed up too much
                    VelDesired = min_speed(
                        VelDesired, (Obstacle.distance > 100 ? 20.0 : 4.0));
                }
            }
        }
    }
...
void TController::adjust_desired_speed_for_limits() {
    ...
    check_load_exchange();
    if ((ExchangeTime > 0) ||
        (mvOccupied->Vel > 2.0)) {  // HACK: force timer reset if the load
                                    // exchange is cancelled due to departure
        WaitingSet(ExchangeTime);
    }
    if (fStopTime < 0) {
    ...
    if ((OrderCurrentGet() & (Shunt | Loose_shunt | Obey_train | Bank)) != 0) {
        ...
        if (((iDrivigFlags & moveStopHere) != 0) && (mvOccupied->Vel < 0.01) &&
            (VelSignalNext == 0.0)) {
...
void TController::adjust_desired_speed_for_target_speed(double const Range) {
    // ustalanie zadanego przyspieszenia
    //(ActualProximityDist) - odległość do miejsca zmniejszenia prędkości
    //(AccPreferred) - wynika z psychyki oraz uwzglęnia już ewentualne zderzenie
    //z pojazdem z przodu, ujemne gdy należy hamować (AccDesired) - uwzględnia
    //sygnały na drodze ruchu, ujemne gdy należy hamować (fAccGravity) -
    //chwilowe przspieszenie grawitacyjne, ujemne działa przeciwnie do zadanego
    //kierunku jazdy (AbsAccS) - chwilowe przyspieszenie pojazu (uwzględnia
    //grawitację), ujemne działa przeciwnie do zadanego kierunku jazdy
    ...
    if ((VelNext >= 0.0) && (ActualProximityDist <= Range) &&
        (vel >= VelNext)) {
        // gdy zbliża się i jest za szybki do nowej prędkości, albo stoi na
        // zatrzymaniu
        if (vel > EU07_AI_NOMOVEMENT) {
            // jeśli jedzie
            if ((vel < VelNext) &&
                (ActualProximityDist > fMaxProximityDist * (1.0 + 0.1 * vel))) {
                // jeśli jedzie wolniej niż można i jest wystarczająco daleko,
                ...
                // jedzie szybciej, niż trzeba na końcu ActualProximityDist, ale
                // jeszcze jest daleko
                if (ActualProximityDist < fMaxProximityDist) {
                    // jak minął już maksymalny dystans po prostu hamuj (niski
                    // stopień) ma stanąć, a jest w drodze hamowania albo ma
                    // jechać
                    /*
                                        VelDesired = min_speed( VelDesired,
                       VelNext );
                    */
                    if (VelNext == 0.0) {
                        if (mvOccupied->CategoryFlag & 1) {
                            ...
                            if ((OrderCurrentGet() & (Shunt | Connect)) &&
                                ...
                                // crude detection of edge case, if approaching
                                // another vehicle coast slowly until min
                                // distance this should allow to bunch up
                                // trainsets more on sidings
                                ...
                                AccDesired =
                                    ...
                                    (25.92 * (ActualProximityDist + 0.1 -
                                              0.5 * fMinProximityDist));
                                AccDesired =
                                    std::min(AccDesired, fAccThreshold);
                            }
                        } else {
                            // for cars (and others) coast at low speed until we
                            // hit min proximity range
                    ...
                    AccDesired = AccPreferred;
                    if (vel > min_speed(
                                  (ActualProximityDist > 10.0 ? 10.0 : 5.0),
                                  VelDesired)) {
                        // allow to coast at reasonably low speed
                        auto const brakingdistance{
                            fBrakeDist * braking_distance_multiplier(VelNext)};
                        auto const slowdowndistance{
                            (mvOccupied->CategoryFlag == 2
                                 ?  // cars can stop on a dime, for bigger
                                    // vehicles we enforce some minimal braking
                                    // distance
                                 brakingdistance
                                 : std::max(
                                       ...
                                       brakingdistance))};
                        if ((brakingdistance +
                             std::max(slowdowndistance, fMaxProximityDist)) >=
                            (ActualProximityDist - fMaxProximityDist)) {
                            // don't slow down prematurely; as long as we have
                            // room to come to a full stop at a safe distance,
                            // we're good ensure some minimal coasting speed,
                            // otherwise a vehicle entering this zone at very
                            // low speed will be crawling forever
                                ...
                                (VelNext * VelNext - vel * vel) /
                                    (25.92 * std::max(
                                                 ...
                                                 std::min(
                                                     ActualProximityDist,
                                                     brakingpointoffset)) +
                                     0.1));  // najpierw hamuje mocniej, potem
                                             // zluzuje
                        }
                    }
                }
                ...
                */
                if (VelNext == 0.0) {
                    ...
                    if (vel <= VelNext + fVelPlus) {
                        ...
                        AccDesired = std::max(
                            0.0, AccPreferred);  // to olej (zacznij luzować)
                    }
                }
            ...
            if (VelNext > 0.0) {
                ...
                if (ActualProximityDist >
                    ((mvOccupied->CategoryFlag & 2) ? fMinProximityDist
                                                    :  // cars
...
void TController::adjust_desired_speed_for_current_speed() {
    ...
    if (is_train()) {
        ...
        if (vel > VelDesired) {
            ...
            if (VelDesired == 0.0) {
                ...
                if (vel > (VelDesired + fVelPlus)) {
                    ...
                    AccDesired = std::min(
                        AccDesired,  // but don't override decceleration for
                                     // VelNext
        ...
        if (fAccGravity > 0.025) {
            // going sharply downhill we may need to start braking sooner than
            // usual try to estimate increase of current velocity before engaged
            // brakes start working
            auto const speedestimate =
                vel + (1.0 - fBrake_a0[0]) * 30.0 * AbsAccS;
            if (speedestimate > VelDesired) {
                ...
                if (VelDesired == 0.0) {
                    ...
                    // if it looks like we'll exceed maximum speed start
                    // thinking about slight slowing down
                    AccDesired = std::min(AccDesired, -0.05 + fAccThreshold);
                    // HACK: for cargo trains with high braking threshold ensure
                    // we cross that threshold
                    if ((true == IsCargoTrain) && (fBrake_a0[0] > 0.2)) {
                ...
                AccDesired = std::min(
                    ...
                    interpolate(  // ease off as you close to the target
                                  // velocity
                        EU07_AI_NOACCELERATION, AccPreferred,
                        clamp(VelDesired - speedestimate, 0.0, fVelMinus) /
                            fVelMinus));
            }
            // final tweaks
            if (vel > EU07_AI_NOMOVEMENT) {
                ...
                // HACK: if the max allowed speed was exceeded something went
                // wrong; brake harder
                AccDesired -= 0.15 * clamp(vel - VelDesired, 0.0, 5.0);
            }
        }
        // HACK: limit acceleration for cargo trains, to reduce probability of
        // breaking couplers on sudden jolts
        auto MaxAcc{
            0.5 *
            mvOccupied
                ->Couplers[(
                    mvOccupied->DirAbsolute >= 0 ? end::rear : end::front)]
                .FmaxC /
            fMass};
        ...
        AccDesired = std::min(
            AccDesired,
            clamp(MaxAcc, HeavyCargoTrainAcceleration, AccPreferred));
        // TBD: expand this behaviour to all trains with car(s) exceeding
        // certain weight?
        /*
if( ( IsPassengerTrain ) && ( iVehicles - ControlledEnginesCount > 0 ) ) {
    AccDesired = std::min( AccDesired, ( iVehicles - ControlledEnginesCount > 8
? HeavyPassengetTrainAcceleration : PassengetTrainAcceleration ) );
}
...
if( ( IsHeavyCargoTrain ) && ( iVehicles - ControlledEnginesCount > 0 ) ) {
    ...
    // NOTE: as a stop-gap measure the routine is limited to trains only while
    // car calculations seem off
    if (is_train()) {
        if (vel < VelDesired) {
...
// hamowanie kontrolne
void TController::adjust_desired_speed_for_braking_test() {
    //    if( false == Global.DynamicBrakeTest ) { return; }
    if (DynamicBrakeTest >= 5) {
        return;
    }  // all done
    if (false == is_train()) {
        return;
    }  // not applicable
    if (false == TestFlag(OrderCurrentGet(), Obey_train)) {
        return;
    }  // not applicable
    if (false == AIControllFlag) {
        return;
    }  // TBD: add notification about braking test and enable it for human
       // driver as well?
    ...
    switch (DynamicBrakeTest) {
        ...
        case 3: {
            AccDesired =
                clamp(-AbsAccS, fAccThreshold * 1.01, fAccThreshold * 1.21);
            VelDesired = DBT_VelocityBrake;
...
void TController::control_tractive_and_braking_force() {
#if LOGVELOCITY
    WriteLog(
        "Dist=" + FloatToStrF(ActualProximityDist, ffFixed, 7, 1) +
        ", VelDesired=" + FloatToStrF(VelDesired, ffFixed, 7, 1) +
        ", AccDesired=" + FloatToStrF(AccDesired, ffFixed, 7, 3) +
        ", VelSignal=" + AnsiString(VelSignal) +
        ", VelNext=" + AnsiString(VelNext));
#endif

    if (iDriverFailCount > maxdriverfails) {
        Psyche = Easyman;
        if (iDriverFailCount > maxdriverfails * 2) SetDriverPsyche();
    }
    ...
    if ((true == mvOccupied->RadioStopFlag)           // radio-stop
        ...
        cue_action(driver_hint::mastercontrollersetzerospeed);  // just throttle
                                                                // down...
        return;  // ...and don't touch any other controls
    }
    // if we're slipping leave the controls alone, there's a separate logic
    // section trying to deal with it
...
#if LOGVELOCITY
    WriteLog(
        "BrakePos=" + AnsiString(mvOccupied->BrakeCtrlPos) +
        ", MainCtrl=" + AnsiString(mvControlling->MainCtrlPos));
#endif
}
...
void TController::control_tractive_force() {
    ...
    if ((AccDesired > EU07_AI_NOACCELERATION)  // don't add power if not asked
                                               // for actual speed-up
        && ((AbsAccS <
             AccDesired /*- std::min( 0.05, 0.01 * iDriverFailCount )*/) ||
            (mvOccupied->SpeedCtrlUnit.IsActive &&
             velocity < mvOccupied->SpeedCtrlUnit.FullPowerVelocity)) &&
        (false == TestFlag(iDrivigFlags, movePress))) {
        // ...jeśli prędkość w kierunku czoła jest mniejsza od dozwolonej o
        // margines...
        if (velocity < (VelDesired == 1.0
                            ?  // work around for trains getting stuck on tracks
                               // with speed limit = 1
                            VelDesired
                            : VelDesired - fVelMinus)) {
            // within proximity margin don't exceed next scheduled speed,
            // outside of the margin accelerate as you like
            if ((ActualProximityDist > (is_car()
                                            ? fMinProximityDist
                                            :  // cars are allowed to move
                                               // within min proximity distance
                                            fMaxProximityDist)
                     ?  // other vehicle types keep wider margin
                     true
    ...
    // margines dla prędkości jest doliczany tylko jeśli oczekiwana prędkość
    // jest większa od 5km/h
    if (false == TestFlag(iDrivigFlags, movePress)) {
        double SpeedCtrlMargin =
            (mvControlling->SpeedCtrlUnit.IsActive && VelDesired > 5) ? 3 : 0;
        // jeśli nie dociskanie
        if (AccDesired <= EU07_AI_NOACCELERATION) {
            cue_action(driver_hint::mastercontrollersetzerospeed);
        } else if (
            (velocity > VelDesired + SpeedCtrlMargin) ||
            (fAccGravity < -0.01 ? AccDesired < 0.0
...
void TController::control_braking_force() {
    // don't touch the brakes when compressing buffers during
    // coupling/uncoupling
    if (TestFlag(iDrivigFlags, movePress)) {
        return;
    }
    ...
    if (is_emu() && (!ForcePNBrake)) {
        ...
        auto const accthreshold{
            (fAccGravity < 0.025 ?  // HACK: when going downhill be more
                                    // responsive to desired deceleration
                 fAccThreshold
                                 : std::max(-0.2, fAccThreshold))};
        auto const AccMax{
            std::min(fBrake_a0[0] + 12 * fBrake_a1[0], mvOccupied->MED_amax)};
        auto const accmargin =
            ((AccMax > 1.1 * AccDesired) && (fAccGravity < 0.025)) ? 0.05 : 0.0;
        if ((AccDesired < accthreshold)  // jeśli hamować - u góry ustawia się
                                         // hamowanie na fAccThreshold
            &&
            ((AbsAccS > AccDesired + accmargin) || (BrakeCtrlPosition < 0))) {
            // hamować bardziej, gdy aktualne opóźnienie hamowania mniejsze niż
            // (AccDesired)
            cue_action(driver_hint::brakingforceincrease);
        } else if (OrderCurrentGet() != Disconnect) {  // przy odłączaniu nie
                                                       // zwalniamy tu hamulca
            if (AbsAccS < AccDesired - 0.05) {
                // jeśli opóźnienie większe od wymaganego (z histerezą)
                // luzowanie, gdy za dużo TBD: check if the condition isn't
                // redundant with the DecBrake() code
                if (BrakeCtrlPosition >= 0) {
                    if (VelDesired >
                        0.0) {  // sanity check to prevent unintended brake
                                // release on sharp slopes
                        cue_action(driver_hint::brakingforcedecrease);
                    }
                }
            } else {
        ...
        if (((AccDesired < fAccGravity - 0.1) &&
             (AbsAccS > AccDesired + fBrake_a1[0]))  // regular braking
            || ((fAccGravity < -0.05) &&
                (velocity < -0.1))) {  // also brake if uphill and slipping back
            // u góry ustawia się hamowanie na fAccThreshold
            if ((fBrakeTime < 0.0) || (AccDesired < fAccGravity - 0.5) ||
                (BrakeCtrlPosition <= 0)) {
                // jeśli upłynął czas reakcji hamulca, chyba że nagłe albo
                // luzował
                // TODO: check whether brake delay variable still has any
                // purpose
                cue_action(
                    driver_hint::brakingforceincrease,
                    // Ra: ten czas należy zmniejszyć, jeśli czas dojazdu do
                    // zatrzymania jest mniejszy
                    (3.0 + 0.5 * ((mvOccupied->BrakeDelayFlag > bdelay_G
        ...
        if ((AccDesired < fAccGravity - 0.05) &&
            ...
            if (OrderCurrentGet() !=
                Disconnect) {  // przy odłączaniu nie zwalniamy tu hamulca
                if (VelDesired > 0.0) {  // sanity check to prevent unintended
                                         // brake release on sharp slopes
                    // TODO: check whether brake delay variable still has any
                    // purpose
                    cue_action(
        ...
        // stop-gap measure to ensure cars actually brake to stop even when
        // above calculactions go awry instead of releasing the brakes and
        // creeping into obstacle at 1-2 km/h
        if (is_car()) {
            if ((VelDesired == 0.0) && (velocity > VelDesired) &&
                (ActualProximityDist <= fMinProximityDist) &&
    ...
    if ((mvOccupied->Vel < EU07_AI_NOMOVEMENT) &&
        ((VelDesired == 0.0) || (AccDesired <= EU07_AI_NOACCELERATION))) {
        if (((OrderCurrentGet() & (Disconnect | Connect | Change_direction)) ==
             0)  // przy (p)odłączaniu nie zwalniamy tu hamulca
            && (std::abs(fAccGravity) <
                0.01)) {  // only do this on flats, on slopes keep applied the
                          // train brake
            apply_independent_brake_only();
        }
        // if told to change direction don't confuse human driver with request
        // to leave applied brake in the cab they're about to leave
...
void TController::apply_independent_brake_only() {
    if (mvOccupied->LocalBrake != TLocalBrake::ManualBrake) {
        if ((false == mvOccupied->ShuntMode)  // no need for independent brake
                                              // if autobrake is active
            && (is_equal(
                   mvOccupied->fBrakeCtrlPos, mvOccupied->Handle->GetPos(bh_RP),
                   0.2))) {
...
void TController::control_releaser() {
    // HACK: don't immediately touch releaser in inert vehicle, in case the
    // simulation is about to replace us with human driver
    if ((mvOccupied->Vel < 1.0) && (fActionTime < 0.0)) {
        return;
    }
    // TODO: combine all releaser handling in single decision tree instead of
    // having bits all over the place
    // TODO: replace handle and system conditions with flag indicating releaser
    // presence
    ...
    auto const hasreleaser{
        (false == (is_dmu() || is_emu()))  // TODO: a more refined test than
                                           // rejecting these types wholesale
...
void TController::control_main_pipe() {
    // unlocking main pipe
    if ((AccDesired > -0.03) && (true == mvOccupied->LockPipe)) {
    ...
    if ((mvOccupied->BrakeHandle == TBrakeHandle::FV4a) ||
        ...
        (mvOccupied->BrakeHandle == TBrakeHandle::MHZ_K8P) ||
        (mvOccupied->BrakeHandle == TBrakeHandle::M394)) {
        if ((iDrivigFlags & moveOerlikons) || (true == IsCargoTrain)) {
            if ((BrakeCtrlPosition == gbh_RP) && (AbsAccS < 0.03) &&
                (AccDesired > -0.03) && (VelDesired - mvOccupied->Vel > 2.0)) {
                if ((fReady > 0.35) &&
                    (mvOccupied->EqvtPipePress <
                     4.5)  // don't charge with a risk of overcharging the main
                           // pipe
                    && (mvOccupied->Compressor >=
                        7.5)  // don't charge without sufficient pressure in the
                              // tank
                    && (BrakeChargingCooldown >=
                        0.0)  // don't charge while cooldown is active
                    &&
                    ((ActualProximityDist >
                      100.0)  // don't charge if we're about to be braking soon
                     || (min_speed(mvOccupied->Vel, VelNext) >=
                         mvOccupied->Vel))) {
                    BrakeLevelSet(gbh_FS);
                    // don't charge the brakes too often, or we risk
                    // overcharging
...
void TController::check_route_ahead(double const Range) {
    auto const comm{
        TableUpdate(VelDesired, ActualProximityDist, VelNext, AccDesired)};
    ...
    switch (comm) {
        ...
        case TCommandType::cm_Ready: {  // W4 zezwolił na jazdę
            ...
            TableCheck(Range);
            TableUpdate(
                VelDesired, ActualProximityDist, VelNext,
        ...
        case TCommandType::cm_SetVelocity: {  // od wersji 357 semafor nie budzi
                                              // wyłączonej lokomotywy
            if ((OrderCurrentGet() &
                 ~(Shunt | Loose_shunt | Obey_train | Bank)) ==
                0) {  // jedzie w dowolnym trybie albo Wait_for_orders
                if (std::fabs(VelSignal) >=
                    1.0) {  // 0.1 nie wysyła się do samochodow, bo potem nie
                            // ruszą
                    PutCommand(
                        "SetVelocity", VelSignal, VelNext,
                        nullptr);  // komenda robi dodatkowe operacje
                }
            }
            break;
        }
        case TCommandType::cm_ShuntVelocity: {  // od wersji 357 Tm nie budzi
                                                // wyłączonej lokomotywy
            if ((OrderCurrentGet() &
                 ~(Shunt | Loose_shunt | Obey_train | Bank)) ==
                0) {  // jedzie w dowolnym trybie albo Wait_for_orders
        ...
        case TCommandType::cm_Command: {  // komenda z komórki
            if ((OrderCurrentGet() &
                 ~(Shunt | Loose_shunt | Obey_train | Bank)) == 0) {
                // jedzie w dowolnym trybie albo Wait_for_orders
                if (mvOccupied->Vel < 0.1) {
                    // dopiero jak stanie
                    /*
                                    PutCommand( eSignNext->text(),
                       eSignNext->value( 1 ), eSignNext->value( 2 ), nullptr );
                                    eSignNext->StopCommandSent(); // się
                       wykonało już
...
void TController::check_route_behind(double const Range) {
    if (VelNext != 0.0) {
        return;
    }  // not if we can move ahead
    if (TestFlag(iDrivigFlags, moveConnect)) {
        return;
    }  // not if we're in middle of coupling operation

    if ((OrderCurrentGet() & ~(Shunt | Loose_shunt | Connect)) == 0) {
        // jedzie w Shunt albo Connect, albo Wait_for_orders
        // w trybie Connect skanować do tyłu tylko jeśli przed kolejnym sygnałem
        // nie ma taboru do podłączenia
        auto const couplinginprogress{
        ...
        if (commandbehind != TCommandType::cm_Unknown) {
            ...
            // sygnalizator i dopiero wtedy zmienić kierunek jazdy, oczekując
            // podania prędkości >0.5
            if (commandbehind == TCommandType::cm_Command) {
                // jeśli komenda Shunt to ją odbierz bez przemieszczania się
                // (np. odczep wagony po dopchnięciu do końca toru)
                iDrivigFlags |= moveStopHere;
            }
...
void TController::UpdateBrakingHelper() {
    if ((HelperState > 0) && (-AccDesired < fBrake_a0[0] + 2 * fBrake_a1[0]) &&
        (mvOccupied->Vel > EU07_AI_NOMOVEMENT)) {
        HelperState = 0;
    }
