...
network::server_manager::server_manager() {
    backbuffer = std::make_shared<std::fstream>(
        "backbuffer.bin",
...
command_queue::commands_map network::server_manager::pop_commands() {
    ...
    for (auto srv : servers) add_to_dequemap(map, srv->pop_commands());

    return map;
}

void network::server_manager::push_delta(
    double render_dt, double dt, double sync,
    const command_queue::commands_map &commands) {
    if (dt == 0.0 && commands.empty()) return;
    ...
    for (auto srv : servers) srv->push_delta(msg);

    serialize_message(msg, *backbuffer.get());
}

void network::server_manager::create_server(
    const std::string &backend, const std::string &conf) {
...
void network::manager::update() {
    for (auto &backend : backend_list()) backend.second->update();
...
// std::unordered_map<std::string, std::shared_ptr<network::backend_manager>>
// network::backend_list;
