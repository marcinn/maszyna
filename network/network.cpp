...
void network::connection::set_handler(
...
void network::server::push_delta(const frame_info &msg) {
    for (auto it = clients.begin(); it != clients.end();) {
        ...
        if ((*it)->state == connection::ACTIVE) (*it)->send_message(msg);
...
void network::server::handle_message(
    std::shared_ptr<connection> conn, const message &msg) {
    ...
    if (msg.type == message::CLIENT_HELLO) {
        ...
        reply.config =
            0;  // TODO: pass bitfield with state of relevant setting switches
        ...
        for (auto const &kv : cmd.commands) client_commands_queue.emplace(kv);
    }
}
...
// client
std::tuple<double, double, command_queue::commands_map>
network::client::get_next_delta(int counter) {
    ...
    if (delta_queue.empty()) {
        // buffer underflow
        return std::tuple<double, double, command_queue::commands_map>(
            0.0, 0.0, command_queue::commands_map());
    }
    ...
    float mult =
        entry.render_dt /
        std::chrono::duration_cast<std::chrono::duration<float>>(frame_time)
            .count();

    if (counter == 1 && size < MAX_BUFFER_SIZE * 2.0f) {
        last_target = last_target * TARGET_MIX +
                      (std::min(
                          TARGET_MIN + jitteriness * JITTERINESS_MULTIPIER,
                          MAX_BUFFER_SIZE)) *
                          (1.0f - TARGET_MIX);
    ...
    float last_rcv_diff =
        std::chrono::duration_cast<std::chrono::duration<float>>(now - last_rcv)
            .count();

    if (size > MAX_BUFFER_SIZE || consume_counter > mult ||
        last_rcv_diff > 1.0f) {
        if (consume_counter > mult) {
            consume_counter = std::clamp(
                consume_counter - mult, -MAX_BUFFER_SIZE, MAX_BUFFER_SIZE);
        }
...
void network::client::send_commands(command_queue::commands_map commands) {
    if (!conn || conn->state == connection::DEAD || commands.empty()) return;
...
void network::client::handle_message(
    std::shared_ptr<connection> conn, const message &msg) {
    if (msg.type >= message::TYPE_MAX) {
        conn->disconnect();
        return;
    ...
    if (msg.type == message::SERVER_HELLO) {
        ...
        if (!Global.ready_to_load) {
            ...
            // TODO: configure simulation settings according to received
            // cmd.config
    ...
    if (conn->state != connection::ACTIVE) return;
