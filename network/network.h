...
class connection {
   ...
   public:
    ...
    virtual void send_messages(
        const std::vector<std::shared_ptr<message>> &messages) = 0;
    ...
    enum peer_state { AWAITING_HELLO, CATCHING_UP, ACTIVE, DEAD };
    peer_state state;
};
...
class client {
   ...
   public:
    ...
    int get_frame_counter() { return resume_frame_counter; }
    int get_awaiting_frames() { return delta_queue.size(); }
};

class backend_manager {
   public:
    virtual std::shared_ptr<server> create_server(
        std::shared_ptr<std::fstream>, const std::string &conf) = 0;
