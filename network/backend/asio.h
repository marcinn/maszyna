...
class connection : public network::connection {
   ...
   public:
    connection(
        asio::io_context &io_ctx, bool client = false, size_t counter = 0);
    ~connection();

    virtual void connected() override;
    virtual void disconnect() override;
    virtual void send_messages(
        const std::vector<std::shared_ptr<message> > &messages) override;
...
class asio_manager : public network::backend_manager {
   ...
   public:
    ...
    virtual std::shared_ptr<network::server> create_server(
        std::shared_ptr<std::fstream>, const std::string &conf) override;
    virtual std::shared_ptr<network::client> create_client(
