...
bool gfx_renderer_factory::register_backend(
    const std::string &backend, gfx_renderer_factory::create_method func) {
    backends[backend] = func;
    return true;
}

std::unique_ptr<gfx_renderer> gfx_renderer_factory::create(
...
gfx_renderer_factory *gfx_renderer_factory::get_instance() {
    if (!instance) instance = new gfx_renderer_factory();
