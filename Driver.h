...
enum TOrders {            // rozkazy dla AI
    ...
    Connect =
        1 << 3,  // podłączanie wagonów (z częściowym skanowaniem sygnalizacji)
...
enum TMovementStatus {  // flagi bitowe ruchu (iDrivigFlags)
    moveStopCloser = 1,  // podjechać blisko W4 (nie podjeżdżać na początku ani
                         // po zmianie czoła)
    moveStopPoint = 2,  // stawać na W4 (wyłączone podczas zmiany czoła)
    moveActive = 4,     // pojazd jest załączony i skanuje
    movePress =
        8,  // dociskanie przy odłączeniu (zamiast zmiennej Prepare2press)
    moveConnect =
        0x10,  // jest blisko innego pojazdu i można próbować podłączyć
    movePrimary = 0x20,  // ma priorytet w składzie (master)
    moveLate = 0x40,     // flaga spóźnienia, włączy bardziej
    moveStopHere =
        0x80,  // nie podjeżdżać do semafora, jeśli droga nie jest wolna
    ...
    moveTrackEnd =
        0x2000,  // dalsza jazda do przodu trwale ograniczona (W5, koniec toru)
    ...
    moveDoorOpened =
        0x20000,  // drzwi zostały otwarte - doliczyć czas na zamknięcie
    movePushPull = 0x40000,  // zmiana czoła przez zmianę kabiny - nie odczepiać
                             // przy zmianie kierunku
    moveSignalFound =
        0x80000,  // na drodze skanowania został znaleziony semafor
    ...
    /*
        ...
        moveTrainInsideStation = 0x200000, // pociąg między semaforem a
       rozjazdami lub następnym semaforem moveSpeedLimitFound = 0x400000 //
       pociąg w ograniczeniu z podaną jego długością
    */
};

enum TStopReason {  // powód zatrzymania, dodawany do SetVelocity 0 - w zasadzie
                    // do usunięcia
...
enum class TAction : int {  // przechowanie aktualnego stanu AI od poprzedniego
...
enum TSpeedPosFlag {  // wartości dla iFlag w TSpeedPos
    spNone = 0x0,
    ...
    spProximityVelocity =
        0x40000  // odcinek z ograniczeniem i podaną jego długościa
    //    spDontApplySpeedLimit = 0x10000 // this point won't apply its speed
    //    limit. potentially set by the scanning vehicle
};

class TSpeedPos {  // pozycja tabeli prędkości dla AI
   public:
    ...
    bool bMoved{
        false};  // czy przesunięty (dotyczy punktu zatrzymania w peronie)
    double fMoved{
        0.0};  // ile przesunięty (dotyczy punktu zatrzymania w peronie)
    Math3D::vector3 vPos;  // współrzędne XYZ do liczenia odległości
    struct {
        TTrack *trTrack{nullptr};  // wskaźnik na tor o zmiennej prędkości
                                   // (zwrotnica, obrotnica)
        basic_event *evEvent{
            nullptr};  // połączenie z eventem albo komórką pamięci
    };
    void CommandCheck();
   ...
   public:
    ...
    inline void UpdateDistance(double dist) { fDist -= dist; }
    bool Set(
        basic_event *e, double d, double length,
        TOrders order = Wait_for_orders);
...
static const int maxdriverfails =
    4;  // ile błędów może zrobić AI zanim zmieni nastawienie
...
class TController {
   ...
   public:
    TController(
        bool AI, TDynamicObject *NewControll, bool InitPsyche,
        bool primary = true);
    ~TController();

    // ai operations logic
    // types
    using hintpredicate = std::function<bool(float)>;  // returns true if action
                                                       // hint can be removed
   ...
   public:
    ...
    inline bool primary(bool const Primary) {
        SetFlag(iDrivigFlags, (Primary ? movePrimary : -movePrimary));
        return primary();
    }
    inline bool primary() const { return ((iDrivigFlags & movePrimary) != 0); };
    inline TMoverParameters const *Controlling() const { return mvControlling; }
    inline TMoverParameters const *Occupied() const { return mvOccupied; }
    void DirectionInitial();
    void DirectionChange();
    inline int Direction() const { return iDirection; }
    inline TAction &action() { return eAction; }
    inline TAction const &action() const { return eAction; }
    inline bool is_active() const { return TestFlag(iDrivigFlags, moveActive); }
    inline bool is_train() const {
        return TestFlag(mvOccupied->CategoryFlag, 1);
    }
    inline bool is_car() const { return TestFlag(mvOccupied->CategoryFlag, 2); }
    inline bool is_emu() const { return (mvControlling->TrainType == dt_EZT); }
    inline bool is_dmu() const { return (mvControlling->TrainType == dt_DMU); }
    inline bool has_diesel_engine() const {
        return (
   ...
   private:
    ...
    void SetTimeControllers(); /*setting state of time controllers depending of
                                  desired action*/
    void CheckTimeControllers(); /*checking state of time controllers to reset
                                    them to stable position*/
    double ESMVelocity(bool Main);
    void PrepareDirection();
    bool PrepareHeating();
    // uaktualnia informacje o prędkości
    void SetVelocity(
        double NewVel, double NewVelNext, TStopReason r = stopNone);
    int CheckDirection();
    ...
    // modifies brake distance for low target speeds, to ease braking rate in
    // such situations
    float braking_distance_multiplier(float const Targetvelocity) const;
    inline int DrivigFlags() const { return iDrivigFlags; };
    inline double DirectionalVel() const {
    ...
    void handle_orders();
    void UpdateNextStop();
    void check_load_exchange();  // returns: estimated remaining time of load
                                 // exchange, in seconds
    ...
    void hint(
        driver_hint const Value, hintpredicate const Predicate,
        float const Predicateparameter = 0.f);
    void update_hints();
    ...
    void cue_action(
        driver_hint const Action, float const Actionparameter = 0.f);
    // members
   public:
    bool AIControllFlag = false;  // rzeczywisty/wirtualny maszynista
    int iOverheadZero = 0;  // suma bitowa jezdy bezprądowej, bity ustawiane
                            // przez pojazdy z podniesionymi pantografami
    int iOverheadDown =
        0;  // suma bitowa opuszczenia pantografów, bity ustawiane przez pojazdy
            // z podniesionymi pantografami
    double BrakeCtrlPosition =
        0.0;  // intermediate position of main brake controller
    int UniversalBrakeButtons =
        0.0;  // flag of which universal buttons need to be pressed
    int DizelPercentage = 0;  // oczekiwane procenty jazdy/hamowania szynobusem
    int DizelPercentage_Speed =
        0;  // oczekiwane procenty jazdy/hamowania szynobusem w związku z
            // osiąganiem VelDesired
   ...
   private:
    ...
    TMoverParameters *mvControlling =
        nullptr;  // jakim pojazdem steruje (może silnikowym w EZT)
    TMoverParameters *mvOccupied = nullptr;  // jakim pojazdem hamuje
    TMoverParameters *mvPantographUnit =
        nullptr;  // pantograph equipped vehicle in the occupied/controlled set
    std::string VehicleName;
    std::array<int, 2> m_lighthints = {-1, -1};  // suggested light patterns
    double AccPreferred =
        0.0;  // preferowane przyspieszenie (wg psychiki kierującego,
              // zmniejszana przy wykryciu kolizji)
    double AccDesired = 0.0;  // przyspieszenie, jakie ma utrzymywać (<0:nie
                              // przyspieszaj,<-0.1:hamuj)
    double VelDesired = 0.0;  // predkość, z jaką ma jechać, wynikająca z
                              // analizy tableki; <=VelSignal
    double fAccDesiredAv =
        0.0;  // uśrednione przyspieszenie z kolejnych przebłysków świadomości,
              // żeby ograniczyć migotanie
    double VelforDriver = -1.0;  // prędkość, używana przy zmianie kierunku
                                 // (ograniczenie przy nieznajmości szlaku?)
    double VelSignal =
        0.0;  // ograniczenie prędkości z kompilacji znaków i sygnałów //
              // normalnie na początku ma stać, no chyba że jedzie
    double VelLimit =
        -1.0;  // predkość zadawana przez event jednokierunkowego ograniczenia
               // prędkości // -1: brak ograniczenia prędkości
    double VelSignalLast = -1.0;  // prędkość zadana na ostatnim semaforze //
                                  // ostatni semafor też bez ograniczenia
    ...
    double VelLimitLast = -1.0;   // prędkość zadana przez ograniczenie //
                                  // ostatnie ograniczenie bez ograniczenia
    double VelRoad =
        -1.0;  // aktualna prędkość drogowa (ze znaku W27) (PutValues albo
               // komendą) // prędkość drogowa bez ograniczenia
    double VelNext =
        120.0;  // prędkość, jaka ma być po przejechaniu długości ProximityDist
    double VelRestricted =
        -1.0;  // speed of travel after passing a permissive signal at stop
    double FirstSemaphorDist =
        10000.0;  // odległość do pierwszego znalezionego semafora
    double ActualProximityDist = 1.0;  // odległość brana pod uwagę przy
                                       // wyliczaniu prędkości i przyspieszenia
    std::pair<double, double> VelLimitLastDist{
        0.0, 0.0};  // distance to velocity change point
    double SwitchClearDist{
        0.0};            // distance to point after farthest detected switch
    int iDirection = 0;  // kierunek jazdy względem sprzęgów pojazdu, w którym
                         // siedzi AI (1=przód,-1=tył)
    int iDirectionOrder = 0;  //żadany kierunek jazdy (służy do zmiany kierunku)
    std::optional<int>
        iDirectionBackup;  // consist direction to be restored after
                           // coupling/uncoupling and similar direction-changing
                           // operations
    std::optional<std::pair<TDynamicObject *, int>>
        iCouplingVehicle;  // <vehicle, coupler> to be coupled with another
                           // consist in current coupling operation
    int iVehicleCount =
        -2;  // wartość neutralna // ilość pojazdów do odłączenia albo zabrania
             // ze składu (-1=wszystkie)
    int iCoupler = 0;  // maska sprzęgu, jaką należy użyć przy łączeniu (po
                       // osiągnięciu trybu Connect), 0 gdy jazda bez łączenia
    ...
    bool IsAtPassengerStop{
        false};  // true if the consist is within acceptable range of w4 post
    bool IsScheduledPassengerStopVisible{false};
    double fMinProximityDist =
        30.0;  // stawanie między 30 a 60 m przed przeszkodą // minimalna
               // oległość do przeszkody, jaką należy zachować
    double fOverhead1 = 3000.0;  // informacja o napięciu w sieci trakcyjnej
                                 // (0=brak drutu, zatrzymaj!)
    double fOverhead2 =
        -1.0;  // informacja o sposobie jazdy (-1=normalnie, 0=bez prądu, >0=z
               // opuszczonym i ograniczeniem prędkości)
    double fVoltage = 0.0;  // uśrednione napięcie sieci: przy spadku poniżej
                            // wartości minimalnej opóźnić rozruch o losowy czas
    double fMaxProximityDist =
        50.0;  // stawanie między 30 a 60 m przed przeszkodą // akceptowalna
               // odległość stanięcia przed przeszkodą
    TStopReason eStopReason =
        stopSleep;  // powód zatrzymania przy ustawieniu zerowej prędkości // na
                    // początku śpi
    double fVelPlus = 0.0;  // dopuszczalne przekroczenie prędkości na
                            // ograniczeniu bez hamowania
    double fVelMinus =
        0.0;  // margines obniżenia prędkości, powodujący załączenie napędu
    double fWarningDuration = 0.0;  // ile czasu jeszcze trąbić
    double WaitingTime =
        0.0;  // zliczany czas oczekiwania do samoistnego ruszenia
    double WaitingExpireTime =
        31.0;  // tyle ma czekać, zanim się ruszy // maksymlany czas oczekiwania
               // do samoistnego ruszenia
    double IdleTime{};  // keeps track of time spent at a stop
    double fStopTime =
        0.0;  // czas postoju przed dalszą jazdą (np. na przystanku)
    float ExchangeTime{0.0};  // time needed to finish current load exchange
    double fShuntVelocity =
        40.0;            // prędkość manewrowania, zależy m.in. od składu
    int iDrivigFlags =   // flagi bitowe ruchu
        moveStopPoint |  // podjedź do W4 możliwie blisko
        moveStopHere |   // nie podjeżdżaj do semafora, jeśli droga nie jest
                         // wolna
        moveStartHorn;   // podaj sygnał po podaniu wolnej drogi
    double fDriverBraking =
        0.0;  // po pomnożeniu przez v^2 [km/h] daje ~drogę hamowania [m]
    double fDriverDist =
        0.0;  // dopuszczalna odległość podjechania do przeszkody
    double fVelMax =
        -1.0;  // maksymalna prędkość składu (sprawdzany każdy pojazd)
    double fBrakeDist = 0.0;  // przybliżona droga hamowania
    double fBrakeReaction =
        1.0;  // opóźnienie zadziałania hamulca - czas w s / (km/h)
    double fNominalAccThreshold =
        0.0;  // nominalny próg opóźnienia dla zadziałania hamulca
    double fAccThreshold =
        0.0;  // aktualny próg opóźnienia dla zadziałania hamulca
    double AbsAccS = 0.0;  // próg opóźnienia dla zadziałania hamulca
    // dla fBrake_aX:
    // indeks [0] - wartości odpowiednie dla aktualnej prędkości
    // a potem jest 20 wartości dla różnych prędkości zmieniających się co 5 %
    // Vmax pojazdu obsadzonego
    double fBrake_a0[BrakeAccTableSize + 1] = {
        0.0};  // opóźnienia hamowania przy ustawieniu zaworu maszynisty w
               // pozycji 1.0
    double fBrake_a1[BrakeAccTableSize + 1] = {
        0.0};  // przyrost opóźnienia hamowania po przestawieniu zaworu
               // maszynisty o 0,25 pozycji
    double BrakingInitialLevel{1.0};
    double BrakingLevelIncrease{0.25};
    double ReactionTime =
        0.0;  // czas reakcji Ra: czego i na co? świadomości AI
    double fBrakeTime = 0.0;  // wpisana wartość jest zmniejszana do 0, gdy
                              // ujemna należy zmienić nastawę hamulca
    double BrakeChargingCooldown{};  // prevents the ai from trying to charge
                                     // the train brake too frequently
    TBrakeSystem BrakeSystem = TBrakeSystem::Individual;  // type of main brake
    bool ForcePNBrake =
        false;  // is it necessary to use PN brake instead of EP brake
    int DynamicBrakeTest =
        0;  // is it necessary to make brake test while driving
    ...
    int StaticBrakeTest =
        0;  // is it necessary to make brake test while standing
    double LastReactionTime = 0.0;
    double fActionTime =
        0.0;  // czas używany przy regulacji prędkości i zamykaniu drzwi
    double m_radiocontroltime{
        0.0};  // timer used to control speed of radio operations
    TAction eAction{TAction::actUnknown};  // aktualny stan
    std::list<std::tuple<driver_hint, hintpredicate, float>>
        m_hints;  // queued ai operations
   ...
   public:
    ...
    bool PutCommand(
        std::string NewCommand, double NewValue1, double NewValue2,
        glm::dvec3 const *NewLocation, TStopReason reason = stopComm);
    // defines assignment data
    inline auto assignment() -> std::string & { return m_assignment; }
    inline auto assignment() const -> std::string const & {
        return m_assignment;
    }
   ...
   private:
    ...
    Math3D::vector3 vCommandLocation;  // polozenie wskaznika, sygnalizatora lub
                                       // innego obiektu do ktorego odnosi sie
                                       // komenda // NOTE: not used
    TOrders OrderList[maxorders];      // lista rozkazów
    int OrderPos = 0,
        OrderTop =
            0;  // rozkaz aktualny oraz wolne miejsce do wstawiania nowych
   ...
   private:
    ...
    std::vector<basic_event *> CheckTrackEvent(
        TTrack *Track, double const fDirection) const;
    bool TableAddNew();
    ...
    TCommandType TableUpdate(
        double &fVelDes, double &fDist, double &fNext, double &fAcc);
    bool TableUpdateStopPoint(
        TCommandType &Command, TSpeedPos &Point, double const Signaldistance);
    bool TableUpdateEvent(
    ...
    void TablePurger();
    void TableSort();
    inline double MoveDistanceGet() const { return dMoveLen; }
    inline void MoveDistanceReset() { dMoveLen = 0.0; }
    ...
    basic_event *CheckTrackEventBackward(
        double fDirection, TTrack *Track, TDynamicObject *Vehicle,
        int const Eventdirection = 1, end const End = end::rear);
    TTrack *BackwardTraceRoute(
        double &fDistance, double &fDirection, TDynamicObject *Vehicle,
        basic_event *&Event, int const Eventdirection = 1,
        end const End = end::rear, bool const Untiloccupied = true);
    ...
    */
    // members
    int iLast{0};  // ostatnia wypełniona pozycja w tabeli <iFirst (modulo
                   // iSpeedTableSize)
    int iTableDirection{
        0};  // kierunek zapełnienia tabelki względem pojazdu z AI
    ...
    double dMoveLen =
        0.0;  // odległość przejechana od ostatniego sprawdzenia tabelki
    basic_event *eSignNext =
        nullptr;  // sygnał zmieniający prędkość, do pokazania na [F2]
   ...
   private:
    ...
    // members
    Mtable::TTrainParameters
        TrainParams;         // rozkład jazdy zawsze jest, nawet jeśli pusty
    std::string asNextStop;  // nazwa następnego punktu zatrzymania wg rozkładu
    //    int iStationStart = 0; // numer pierwszej stacji pokazywanej na
    //    podglądzie rozkładu
    std::string m_lastexchangestop;  // HACK: safeguard to prevent multiple load
                                     // exchanges per station
    int m_lastexchangeplatforms{
        0};  // cached station platforms for last exchange
    int m_lastexchangedirection{0};  //
    double fLastStopExpDist =
        -1.0;               // odległość wygasania ostateniego przystanku
    int iRadioChannel = 1;  // numer aktualnego kanału radiowego
    int iGuardRadio =
        0;  // numer kanału radiowego kierownika (0, gdy nie używa radia)
   ...
   private:
    ...
    double fMass =
        0.0;  // całkowita masa do liczenia stycznej składowej grawitacji
    double fAccGravity = 0.0;  // przyspieszenie składowej stycznej grawitacji
    int iVehicles = 0;         // ilość pojazdów w składzie
    int ControlledEnginesCount{
        0};  // number of powered vehicles under driver's control
    ...
    bool Ready = false;   // ABu: stan gotowosci do odjazdu - sprawdzenie
                          // odhamowania wagonow
    bool IsConsistBraked{false};
    double ConsistShade{
        1.0};  // averaged amount of sunlight received by the consist
    TDynamicObject *pVehicles[2];  // skrajne pojazdy w składzie (niekoniecznie
                                   // bezpośrednio sterowane)
    bool DoesAnyDoorNeedOpening{false};
    bool IsAnyDoorOpen[2];      // state of door in the consist
    bool IsAnyDoorOnlyOpen[2];  // state of door in the consist regardless of
                                // step
    bool IsAnyDoorPermitActive[2];  // state of door permit in the consist
    bool IsAnyLineBreakerOpen{
        false};  // state of line breaker in all powered vehicles under control
    bool IsAnyConverterOverloadRelayOpen{
        false};  // state of converter overload relays in all vehicles under
                 // control
    bool IsAnyMotorOverloadRelayOpen{
        false};  // state of motor overload relays in all vehicles under control
    bool IsAnyGroundRelayOpen{false};
    bool IsAnyCompressorPresent{false};
    bool IsAnyCompressorEnabled{false};
    bool IsAnyCompressorExplicitlyEnabled{
        false};  // only takes into account manually controlled devices
    bool IsAnyConverterPresent{false};
    bool IsAnyConverterEnabled{false};
    bool IsAnyConverterExplicitlyEnabled{
        false};  // only takes into account manually controlled devices
    bool IsAnyCouplerStretched{false};  // whether there's a coupler in the
                                        // consist stretched above limit
   ...
   public:
    ...
    TDynamicObject *Vehicle(end const Side) const { return pVehicles[Side]; }
   ...
   private:
    ...
    // leftovers
    /*
        int iRouteWanted = 3; // oczekiwany kierunek jazdy
       (0-stop,1-lewo,2-prawo,3-prosto) np. odpala migacz lub czeka na stan
       zwrotnicy
    */
};
