...
bool keyboard_input::recall_bindings() {
    ...
    while (true == bindingparser.getTokens(1, true, "\n\r")) {
        ...
        if (true == entryparser.getTokens(1, true, "\n\r\t ")) {
            ...
            if (lookup == nametocommandmap.end()) {
                WriteLog(
                    "Keyboard binding defined for unknown command, \"" +
                    commandname + "\"");
            } else {
                int binding{0};
                while (entryparser.getTokens(1, true, "\n\r\t ")) {
                    ...
                    if (bindingkeyname == "shift") {
                        binding |= keymodifier::shift;
                    } else if (bindingkeyname == "ctrl") {
                        binding |= keymodifier::control;
                    } else if (bindingkeyname == "none") {
                        binding = 0;
                    } else {
                        // regular key, convert it to glfw key code
                        auto const keylookup =
                            nametokeymap.find(bindingkeyname);
                        if (keylookup == nametokeymap.end()) {
                            WriteLog(
                                "Keyboard binding included unrecognized key, "
                                "\"" +
                                bindingkeyname + "\"");
                        } else {
                            // replace any existing binding, preserve modifiers
                            // (protection from cases where there's more than
                            // one key listed in the entry)
                            binding =
                                keylookup->second | (binding & 0xffff0000);
                        }
                    }

                    if ((binding & 0xffff) != 0) {
                        m_bindingsetups.insert_or_assign(
                            lookup->second, binding);
                    }
                }
            }
        }
...
void keyboard_input::dump_bindings() {
    ...
    for (const std::pair<user_command, int> &binding : m_bindingsetups) {
        stream << simulation::Commands_descriptions[static_cast<std::size_t>(
                                                        binding.first)]
                      .name
               << ' ';
        ...
        if (it != keytonamemap.end()) {
            if (binding.second & keymodifier::control) stream << "ctrl ";
            if (binding.second & keymodifier::shift) stream << "shift ";
...
bool keyboard_input::key(int const Key, int const Action) {
    bool modifier(false);

    if ((Key == GLFW_KEY_LEFT_SHIFT) || (Key == GLFW_KEY_RIGHT_SHIFT)) {
        // update internal state, but don't bother passing these
        input::key_shift = (Action == GLFW_RELEASE ? false : true);
        modifier = true;
        // whenever shift key is used it may affect currently pressed movement
        // keys, so check and update these
    }
    if ((Key == GLFW_KEY_LEFT_CONTROL) || (Key == GLFW_KEY_RIGHT_CONTROL)) {
        // update internal state, but don't bother passing these
        input::key_ctrl = (Action == GLFW_RELEASE ? false : true);
        modifier = true;
    }
    if ((Key == GLFW_KEY_LEFT_ALT) || (Key == GLFW_KEY_RIGHT_ALT)) {
        // update internal state, but don't bother passing these
        input::key_alt = (Action == GLFW_RELEASE ? false : true);
    }
    ...
    if (true == is_movement_key(Key)) {
        // if the received key was one of movement keys, it's been handled and
        // we don't need to bother further
        return true;
    }

    // include active modifiers for currently pressed key, except if the key is
    // a modifier itself
    auto const key =
        Key | (modifier ? 0 : (input::key_shift ? keymodifier::shift : 0)) |
    ...
    // as we haven't yet implemented either item id system or multiplayer, the
    // 'local' controlled vehicle and entity have temporary ids of 0
...
int keyboard_input::key(int const Key) const { return input::keys[Key]; }

void keyboard_input::bind() {
    m_bindings.clear();
...
int keyboard_input::binding(user_command const Command) const {
...
bool keyboard_input::is_movement_key(int const Key) const {
    bool const ismovementkey =
        ((Key == m_bindingscache.forward) || (Key == m_bindingscache.back) ||
         (Key == m_bindingscache.left) || (Key == m_bindingscache.right) ||
         (Key == m_bindingscache.up) || (Key == m_bindingscache.down));

    return ismovementkey;
}

void keyboard_input::poll() {
    ...
    if ((movementhorizontal.x != 0.f || movementhorizontal.y != 0.f) ||
        ...
        m_relay.post(
            (true == Global.ctrlState ? user_command::movehorizontalfast
                                      : user_command::movehorizontal),
            movementhorizontal.x, movementhorizontal.y, GLFW_PRESS, 0);
    }
    ...
    if ((movementvertical != 0.f) || (m_movementvertical != 0.f)) {
        m_relay.post(
            (true == Global.ctrlState ? user_command::moveverticalfast
                                      : user_command::movevertical),
            movementvertical, 0, GLFW_PRESS, 0);
    }
...
std::string keyboard_input::binding_hint(user_command const Command) const {
