...
void locale::init() {
    std::fstream stream(
        "lang/" + Global.asLang + ".po",
        std::ios_base::in | std::ios_base::binary);

    if (!stream.is_open()) {
        WriteLog(
            "translation: cannot open lang file: lang/" + Global.asLang +
            ".po");
        return;
    }
    ...
    WriteLog(
        "translation: " + std::to_string(lang_mapping.size()) +
        " strings loaded");
}
...
const std::string &locale::lookup_s(const std::string &msg, bool constant) {
    ...
    if (it != lang_mapping.end()) {
        if (constant) pointer_cache.emplace(&msg, &(it->second));
        return it->second;
    }

    if (constant) pointer_cache.emplace(&msg, &msg);
    return msg;
}
...
const char *locale::lookup_c(const char *msg, bool constant) {
    if (constant) {
        auto it = pointer_cache.find(msg);
        if (it != pointer_cache.end()) return (const char *)(it->second);
    }

    auto it = lang_mapping.find(std::string(msg));
    if (it != lang_mapping.end()) {
        if (constant) pointer_cache.emplace(msg, it->second.c_str());
