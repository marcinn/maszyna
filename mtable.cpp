...
double TTrainParameters::CheckTrainLatency() {
    if ((LastStationLatency > 1.0) || (LastStationLatency < 0))
        return LastStationLatency; /*spoznienie + lub do przodu - z tolerancja 1
                                      min*/
    else
        return 0;
}

double TTrainParameters::WatchMTable(
    double
        DistCounter) {  // zwraca odleglość do najblizszej stacji z zatrzymaniem
    double dist;
...
std::string TTrainParameters::NextStop()
    const {  // pobranie nazwy następnego miejsca zatrzymania
...
sound_source TTrainParameters::next_stop_sound() const {
    if (StationIndex > StationCount) {
        return {sound_placement::engine};
    }
    for (auto stationidx{StationIndex}; stationidx < StationCount + 1;
         ++stationidx) {
...
sound_source TTrainParameters::last_stop_sound() const {
    return TimeTable[StationCount].name_sound;
}

bool TTrainParameters::IsStop()
    const {  // zapytanie, czy zatrzymywać na następnym punkcie rozkładu
    if ((StationIndex <= StationCount))
...
bool TTrainParameters::UpdateMTable(
    scenario_time const &Time, std::string const &NewName) {
    return UpdateMTable(
        Time.data().wHour, Time.data().wMinute + Time.data().wSecond * 0.0167,
        NewName);
}

bool TTrainParameters::UpdateMTable(
    double hh, double mm, std::string const &NewName)
/*odfajkowanie dojechania do stacji (NewName) i przeliczenie opóźnienia*/
{
    bool OK;
    OK = false;
    if (StationIndex <= StationCount)  // Ra: "<=", bo ostatni przystanek jest
                                       // traktowany wyjątkowo
    {
        ...
        {  // Ra: wywołanie może być powtarzane, jak stoi na W4
            ...
            LastStationLatency = CompareTime(
                hh, mm, TimeTable[StationIndex].Dh, TimeTable[StationIndex].Dm);
            // inc(StationIndex); //przejście do następnej pozycji
            // StationIndex<=StationCount Ra: "<", bo dodaje 1 przy przejściu do
            // następnej stacji
            if (StationIndex < StationCount) {
                // jeśli nie ostatnia stacja
                NextStationName = TimeTable[StationIndex + 1]
                                      .StationName;  // zapamiętanie nazwy
...
bool Mtable::TTrainParameters::RewindTimeTable(std::string actualStationName) {
    ...
    for (auto i = 1; i <= StationCount; ++i) {
        // przechodzimy po całej tabelce i sprawdzamy nazwy stacji (bez
        // pierwszej)
        if (ToLower(TimeTable[i].StationName) == actualStationName) {
            // nazwa stacji zgodna więc ustawiamy na poprzednią, żeby w
            // następnym kroku poprawnie obsłużyć
            StationIndex = i;
...
void TTrainParameters::StationIndexInc() {  // przejście do następnej pozycji
...
// StationIndex to numer następnego po dodarciu do aktualnego
{
    ...
    else if ((StationIndex < StationCount)) {  // oprócz ostatniego przystanku
        if ((TimeTable[StationIndex].Ah < 0))  // odjazd z poprzedniego
            return true;  // czas przyjazdu nie był podany - przelot
        else
            return CompareTime(
                       hh, mm, TimeTable[StationIndex].Dh,
                       TimeTable[StationIndex].Dm) <= 0;
    } else             // gdy rozkład się skończył
        return false;  // dalej nie jechać
}

// returns: difference between specified time and scheduled departure from
// current stop, in seconds
double TTrainParameters::seconds_until_departure(
    double const Hour, double const Minute) const {
    if ((TimeTable[StationStart].Ah < 0)) {  // passthrough
        return 0;
    }
    return (
        60.0 * CompareTime(
                   Hour, Minute, TimeTable[StationStart].Dh,
                   TimeTable[StationStart].Dm));
}

std::string TTrainParameters::ShowRelation() const
/*zwraca informację o relacji*/
{
    // if (Relation1=TimeTable[1].StationName) and
    // (Relation2=TimeTable[StationCount].StationName)
...
// bool TTrainParameters::LoadTTfile(std::string scnpath, int iPlus, double
// vmax)
//{
//	return false;
//}
...
// wczytanie pliku-tabeli z rozkładem przesuniętym o (fPlus); (vMax) nie ma
// znaczenia
{
    std::string lines;
    std::string s;
    ...
    EndTable = false;
    if ((TrainName == "")) {  // jeśli pusty rozkład
                              // UpdateVelocity(StationCount,vMax);
                              // //ograniczenie do prędkości startowej
    } else {
        ConversionError = 666;
        vActual = -1;
        s = scnpath + TrainName + ".txt";
        replace_slashes(s);
        // Ra 2014-09: ustalić zasady wyznaczenia pierwotnego pliku przy
        // przesuniętych rozkładach (kolejny pociąg dostaje numer +2)
        fin.open(s.c_str());  // otwieranie pliku
        replace_slashes(s);

        if (!fin.is_open()) {                // jeśli nie ma pliku
            vmax = atoi(TrainName.c_str());  // nie ma pliku ale jest liczba
            if ((vmax > 10) && (vmax < 200)) {
                TTVmax = vmax;  // Ra 2014-07: zamiast rozkładu można podać Vmax
                UpdateVelocity(
                    StationCount, vmax);  // ograniczenie do prędkości startowej
                ConversionError = 0;
            } else
                ConversionError = -8; /*Ra: ten błąd jest niepotrzebny*/
        } else {                      /*analiza rozkładu jazdy*/
            ConversionError = 0;
            while (fin.good() && !((ConversionError != 0) || EndTable)) {
                std::getline(fin, lines); /*wczytanie linii*/
                if (contains(
                        lines, "___________________")) /*linia pozioma górna*/
                {
                    fin >> s;
                    if (s == "[") /*lewy pion*/
                    {
                ...
                fin >> s; /*nazwa pociągu*/
                // if LowerCase(s)<>ExtractFileName(TrainName) then {musi być
                // taka sama, jak nazwa pliku} ConversionError:=-7 {błąd
                // niezgodności}
                TrainName =
                    s;  // nadanie nazwy z pliku TXT (bez ścieżki do pliku)
                while (fin >> s || fin.bad()) {
                ...
                { /*czytaj naglowek*/
                    while (fin >> s || !fin.bad()) {
                        if (s == "[") break;
                    }  // while (!() || (fin.eof())); /*pierwsza linia z
                       // relacją*/
                    while (fin >> s || !fin.bad()) {
                        if (s != "|") break;
                    }  // while (!(() || fin.eof()));
                    ...
                    while (!fin.bad() && !EndTable) {
                        ...
                        TMTableLine *record = &TimeTable[StationCount];
                        {
                            ...
                            if (contains(
                                    s,
                                    "|_____|")) /*zmiana predkosci szlakowej*/
                            ...
                            while (false == contains(s, "|")) fin >> s;
                            fin >> record->StationName;
                            // get rid of non-ascii chars. TODO: run correct
                            // version based on locale
                            ...
                            if (s != "|") {
                                if (contains(s, hrsd)) {
                                    record->Ah = atoi(
                                        s.substr(0, s.find(hrsd))
                                            .c_str());  // godzina przyjazdu
                                    record->Am = atof(
                                        s.substr(s.find(hrsd) + 1, s.length())
                                            .c_str());  // minuta przyjazdu
                                } else {
                                    record->Ah =
                                        TimeTable[StationCount - 1]
                                            .Ah;  // godzina z poprzedniej
                                                  // pozycji
                                    record->Am = atof(
                            ...
                            if (s != "]") record->tm = atof(s.c_str());
                            do {
                                fin >> s;
                            } while (!((s == "[") || fin.bad()));
                            fin >> s;
                            if (false == contains(s, "|")) {
                                /*tu s moze byc miejscem zmiany predkosci
                                 * szlakowej*/
                                fin >> s;
                            }
                            if (s.find("|_____|") !=
                                std::string::npos) /*zmiana predkosci
                                                      szlakowej*/
                                UpdateVelocity(StationCount, vActual);
                            else {
                                fin >> s;
                            ...
                            {
                                auto const stationware{
                                    Split(record->StationWare, ',')};
                                for (auto const &entry : stationware) {
                                    if (entry.front() != 'R') {
                                        continue;
                                    }
                                    auto const entrysplit{
                                        split_string_and_number(entry)};
                                    if ((entrysplit.first == "R") &&
                                        (entrysplit.second <= 10)) {
                                        auto const radiochannel{
                                            entrysplit.second};
                                        if ((record->radio_channel == -1) ||
                                            (radiochannel !=
                                             activeradiochannel)) {
                                            // if the station has more than one
                                            // radiochannel listed, it generally
                                            // means we should switch to the one
                                            // we weren't using so far
                                            // TODO: reverse this behaviour
                                            // (keep the channel used so far)
                                            // once W28 signs are included in
                                            // the system
                                            record->radio_channel =
                                                radiochannel;
                                        }
                                    }
                                }
                            ...
                            if (s != "|") {
                                if (contains(s, hrsd)) {
                                    record->Dh =
                                        atoi(s.substr(0, s.find(hrsd))
                                                 .c_str());  // godzina odjazdu
                                    record->Dm = atof(
                                        s.substr(s.find(hrsd) + 1, s.length())
                                ...
                                record->Dh =
                                    record
                                        ->Ah;  // odjazd o tej samej, co
                                               // przyjazd (dla ostatniego też)
                                record->Dm =
                                    record->Am;  // bo są używane do wyliczenia
                                                 // opóźnienia po dojechaniu
                            }
                            do {
                                fin >> s;
                            } while (!((s != "|") || (fin.bad())));
                            ...
                            while (false == contains(s, "|")) fin >> s;
                            while ((false == contains(s, "]"))) fin >> s;
                            if (contains(s, "_|_")) EndTable = true;
                        } /*timetableline*/
                    }
                }
            } /*while eof*/
    ...
    if (ConversionError == 0) {
        if ((TimeTable[1].StationName ==
             Relation1))  // jeśli nazwa pierwszego zgodna z relacją
    ...
    auto const timeoffset{
        static_cast<int>(Global.ScenarioTimeOffset * 60) + iPlus};
    if (timeoffset != 0)  // jeżeli jest przesunięcie rozkładu
    {
        long i_end = StationCount + 1;
        float adjustedtime;  // do zwiększania czasu
        for (auto i = 1; i < i_end;
             ++i)  // bez with, bo ciężko się przenosi na C++
        {
            if ((TimeTable[i].Ah >= 0)) {
                adjustedtime = clamp_circular<float>(
                    TimeTable[i].Ah * 60 + TimeTable[i].Am + timeoffset,
                    24 * 60);  // nowe minuty
            ...
            if ((TimeTable[i].Dh >= 0)) {
                adjustedtime = clamp_circular<float>(
                    TimeTable[i].Dh * 60 + TimeTable[i].Dm + timeoffset,
                    24 * 60);  // nowe minuty
...
void TTrainParameters::load_sounds() {
    for (auto stationidx = 1; stationidx < StationCount + 1; ++stationidx) {
        ...
        auto const lookup{FileExists(
            {Global.asCurrentSceneryPath + stationname,
             std::string{szSoundPath} + "sip/" + stationname},
            {".ogg", ".flac", ".wav"})};
        if (lookup.first.empty()) {
            continue;
        }
        //  wczytanie dźwięku odjazdu w wersji radiowej (słychać tylko w
        //  kabinie)
        station.name_sound =
            sound_source{
                sound_placement::engine, EU07_SOUND_CABANNOUNCEMENTCUTOFFRANGE}
...
{
    if ((StationIndex > 0) &&
        (StationIndex < StationCount))  // dla ostatniej stacji nie
        if (contains(TimeTable[StationIndex].StationWare, '@')) return true;
    return false;
}
...
void TTrainParameters::serialize(dictionary_source *Output) const {
    ...
    if (StationCount > 0) {
        ...
        for (auto stationidx = 1; stationidx <= StationCount; ++stationidx) {
            auto const stationlabel{
                "train_station" + std::to_string(stationidx) + "_"};
            auto const &timetableline{TimeTable[stationidx]};
            Output->insert(
                (stationlabel + "name"), Bezogonkow(timetableline.StationName));
            Output->insert(
                (stationlabel + "fclt"), Bezogonkow(timetableline.StationWare));
