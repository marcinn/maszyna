...
const int iPrawo4[4] = {
    -4, -6, -3, -5};  // segmenty (1..6) do skręcania w prawo
...
TSwitchExtension::TSwitchExtension(
    TTrack *owner, int const what) {  // na początku wszystko puste
    pNexts[0] = nullptr;              // wskaźniki do kolejnych odcinków ruchu
    pNexts[1] = nullptr;
    pPrevs[0] = nullptr;
    pPrevs[1] = nullptr;
    fOffset1 = fOffset = fDesiredOffset =
        -fOffsetDelay;  // położenie zasadnicze
    fOffset2 = 0.f;     // w zasadniczym wewnętrzna iglica dolega
    Segments[0] = std::make_shared<TSegment>(owner);  // z punktu 1 do 2
    Segments[1] = std::make_shared<TSegment>(
        owner);  // z punktu 3 do 4 (1=3 dla zwrotnic; odwrócony dla skrzyżowań,
                 // ewentualnie 1=4)
    Segments[2] = (what >= 3) ? std::make_shared<TSegment>(owner)
                              : nullptr;  // z punktu 2 do 4       skrzyżowanie
                                          // od góry:      wersja "-1":
    Segments[3] =
        (what >= 4)
    ...
    Segments[4] =
        (what >= 5) ? std::make_shared<TSegment>(owner)
                    : nullptr;  // z punktu 1 do 3            4 x 3   3 3 x 2 2
    Segments[5] = (what >= 6)
                      ? std::make_shared<TSegment>(owner)
                      : nullptr;  // z punktu 3 do 2              2       2 1 1
}
TSwitchExtension::~TSwitchExtension() {  // nie ma nic do usuwania
}
/*
TIsolated::TIsolated()
{ // utworznie pustego
...
TIsolated *TIsolated::Find(
    std::string const &n) {  // znalezienie obiektu albo utworzenie nowego
    TIsolated *p = pRoot;
...
void TIsolated::Modify(int i, TDynamicObject *o) {  // dodanie lub odjęcie osi
    if (iAxles) {                                   // grupa zajęta
        iAxles += i;
        if (!iAxles) {  // jeśli po zmianie nie ma żadnej osi na odcinku
                        // izolowanym
            if (evFree)
                simulation::Events.AddToQuery(
                    evFree, o);       // dodanie zwolnienia do kolejki
            if (Global.iMultiplayer)  // jeśli multiplayer
                multiplayer::WyslijString(
                    asName, 10);  // wysłanie pakietu o zwolnieniu
            if (pMemCell)         // w powiązanej komórce
                pMemCell->UpdateValues(
                    "", 0, int(pMemCell->Value2()) & ~0xFF,
        ...
        if (iAxles) {
            if (evBusy)
                simulation::Events.AddToQuery(
                    evBusy, o);       // dodanie zajętości do kolejki
            if (Global.iMultiplayer)  // jeśli multiplayer
            {
                auto const *owner =
                    (((o->Mechanik != nullptr) && (o->Mechanik->primary()))
                         ? o->Mechanik
                         : o->ctOwner);
                auto textline = owner != nullptr
                                    ? Bezogonkow(owner->TrainName(), true)
                                    : "none";
                ...
                multiplayer::WyslijString(
                    asName + textline, 11);  // wysłanie pakietu o zajęciu
            }
            if (pMemCell)  // w powiązanej komórce
                pMemCell->UpdateValues(
                    "", 0, int(pMemCell->Value2()) | 1,
                    basic_event::flags::value2);  // zmieniamy ostatnią wartość
                                                  // na nieparzystą
        }
    }

    if (i > 0 && evInc) simulation::Events.AddToQuery(evInc, o);
    if (i < 0 && evDec) simulation::Events.AddToQuery(evDec, o);
...
TTrack::~TTrack() {  // likwidacja odcinka
    if (eType == tt_Cross) {
        delete SwitchExtension->vPoints;  // skrzyżowanie może mieć punkty
    }
}

void TTrack::Init() {  // tworzenie pomocniczych danych
    switch (eType) {
        case tt_Switch:
            SwitchExtension = std::make_shared<TSwitchExtension>(
                this, 2);  // na wprost i na bok
            break;
        case tt_Cross:  // tylko dla skrzyżowania dróg
            SwitchExtension =
                std::make_shared<TSwitchExtension>(this, 6);  // 6 po³¹czeñ
        ...
        case tt_Table:  // oba potrzebne
            SwitchExtension = std::make_shared<TSwitchExtension>(
                this, 1);  // kopia oryginalnego toru
            Segment = std::make_shared<TSegment>(this);
            break;
    }
}
...
bool TTrack::sort_by_material(TTrack const *Left, TTrack const *Right) {
    return std::tie(Left->m_material1, Left->m_material2) <
           std::tie(Right->m_material1, Right->m_material2);
}

TTrack *TTrack::Create400m(
    int what,
    double
        dx) {  // tworzenie toru do wstawiania taboru podczas konwersji na E3D
    scene::node_data nodedata;
    nodedata.name = "auto_400m";  // track isn't visible so only name is needed
    auto *trk = new TTrack(nodedata);
    trk->m_visible =
        false;  // nie potrzeba pokazywać, zresztą i tak nie ma tekstur
    trk->iCategoryFlag = what;  // taki sam typ plus informacja, że dodatkowy
    trk->Init();                // utworzenie segmentu
    trk->Segment->Init(
        Math3D::vector3(-dx, 0, 0), Math3D::vector3(-dx, 0, 400), 10.0, 0,
        0);                                  // prosty
...
TTrack *TTrack::NullCreate(
    int dir) {  // tworzenie toru wykolejającego od strony (dir), albo pętli dla
                // samochodów
    ...
    trk->m_visible =
        false;  // nie potrzeba pokazywać, zresztą i tak nie ma tekstur
    trk->iCategoryFlag = (iCategoryFlag & 15) |
                         0x80;  // taki sam typ plus informacja, że dodatkowy
    float r1, r2;
    ...
    if (iCategoryFlag & 1) {           // tylko dla kolei
        ...
        switch (dir) {                 //łączenie z nowym torem
            ...
            case 3:  // na razie nie możliwe
                p1 = SwitchExtension->Segments[1]
                         ->FastGetPoint_1();  // koniec toru drugiego zwrotnicy
                p2 = p1 -
                     450.0 *
                         Normalize(
                             SwitchExtension->Segments[1]
                                 ->GetDirection2());  // przedłużenie na wprost
                trk->Segment->Init(
                    p1, p2, 5, RadToDeg(r2),
                    70.0);  // bo prosty, kontrolne wyliczane przy zmiennej
                            // przechyłce
                ConnectNextPrev(trk, 0);
                // trk->ConnectPrevNext(trk,dir);
                SetConnections(1);  // skopiowanie połączeń
                Switch(1);  // bo się przełączy na 0, a to coś chce się przecież
                            // wykoleić na bok
                break;  // do drugiego zwrotnicy... nie zadziała?
        }
    } else {                    // tworznie pętelki dla samochodów
        trk->fVelocity = 20.0;  // zawracanie powoli
        trk->fRadius = 20.0;  // promień, aby się dodawało do tabelki prędkości
                              // i liczyło narastająco
        trk->Init();  // utworzenie segmentu
        ...
        trk2->fRadius = 20.0;  // promień, aby się dodawało do tabelki prędkości
                               // i liczyło narastająco
        ...
        switch (dir) {  //łączenie z nowym torem
            case 0:
                ...
                cv1 =
                    -20.0 *
                    Normalize(
                        Segment->GetDirection1());  // pierwszy wektor kontrolny
                p2 = p1 + cv1 + cv1;                // 40m
                // bo prosty, kontrolne wyliczane przy zmiennej przechyłce
                trk->Segment->Init(
                    p1, p1 + cv1, p2 + Math3D::vector3(-cv1.z, cv1.y, cv1.x),
                    p2, 2, -RadToDeg(r1), 0.0);
                ConnectPrevPrev(trk, 0);
                // bo prosty, kontrolne wyliczane przy zmiennej przechyłce
                trk2->Segment->Init(
                    p1, p1 + cv1, p2 + Math3D::vector3(cv1.z, cv1.y, -cv1.x),
            ...
            case 1:
                ...
                cv1 =
                    -20.0 *
                    Normalize(
                        Segment->GetDirection2());  // pierwszy wektor kontrolny
                p2 = p1 + cv1 + cv1;
                // bo prosty, kontrolne wyliczane przy zmiennej przechyłce
                trk->Segment->Init(
                    p1, p1 + cv1, p2 + Math3D::vector3(-cv1.z, cv1.y, cv1.x),
                    p2, 2, RadToDeg(r2), 0.0);
                ConnectNextPrev(trk, 0);
                // bo prosty, kontrolne wyliczane przy zmiennej przechyłce
                trk2->Segment->Init(
                    p1, p1 + cv1, p2 + Math3D::vector3(cv1.z, cv1.y, -cv1.x),
        ...
        trk2->trPrev = this;
        trk->ConnectNextNext(
            trk2, 1);  // połączenie dwóch dodatkowych odcinków punktami 2
    }
    // trzeba jeszcze dodać do odpowiedniego segmentu, aby się renderowały z
    // niego pojazdy
    trk->location(
        glm::dvec3{0.5 * (p1 + p2)});  //środek, aby się mogło wyświetlić
    simulation::Paths.insert(trk);
    simulation::Region->insert(trk);
    if (trk2) {
...
void TTrack::ConnectPrevPrev(
    TTrack *pTrack,
    int typ) {     //łączenie torów - Point1 własny do Point1 cudzego
    if (pTrack) {  //(pTrack) może być zwrotnicą, a (this) tylko zwykłym
                   //odcinkiem
        trPrev = pTrack;
...
void TTrack::ConnectPrevNext(
    TTrack *pTrack,
    int typ) {  //łaczenie torów - Point1 własny do Point2 cudzego
    if (pTrack) {
        trPrev = pTrack;
        iPrevDirection =
            typ | 1;  // 1:zwykły lub pierwszy zwrotnicy, 3:drugi zwrotnicy
        pTrack->trNext = this;
            ...
            if (pTrack->m_visible)
                if (eType == tt_Normal)  // jeśli łączone są dwa normalne
                    if (pTrack->eType == tt_Normal)
                        if ((fTrackWidth !=
                             pTrack->fTrackWidth)  // Ra: jeśli kolejny ma inne
                                                   // wymiary
...
void TTrack::ConnectNextPrev(
    TTrack *pTrack,
    int typ) {  //łaczenie torów - Point2 własny do Point1 cudzego
    if (pTrack) {
        trNext = pTrack;
...
void TTrack::ConnectNextNext(
    TTrack *pTrack,
    int typ) {  //łaczenie torów - Point2 własny do Point2 cudzego
    if (pTrack) {
        trNext = pTrack;
        iNextDirection =
            typ | 1;  // 1:zwykły lub pierwszy zwrotnicy, 3:drugi zwrotnicy
...
void TTrack::Load(
    cParser *parser,
    glm::dvec3 const &pOrigin) {  // pobranie obiektu trajektorii ruchu
    Math3D::vector3 pt, vec, p1, p2, cp1, cp2, p3, p4, cp3,
        cp4;  // dodatkowe punkty potrzebne do skrzyżowań
    double a1, a2, r1, r2, r3, r4;
    std::string str;
    size_t i;  //,state; //Ra: teraz już nie ma początkowego stanu zwrotnicy we
               //wpisie
    std::string token;
    ...
    *parser >> str;  // typ toru
    ...
    if (str == "normal") {
        ...
        eType = tt_Unknown;
    if (Global.iWriteLogEnabled & 4) WriteLog(str);
    parser->getTokens(4);
    float discard{};
    *parser >> discard >> fTrackWidth >> fFriction >> fSoundDistance;
    fTrackWidth2 =
        fTrackWidth;  // rozstaw/szerokość w punkcie 2, na razie taka sama
    parser->getTokens(2);
    ...
    *parser >> token;
    ...
    if (m_visible) {
        ...
        *parser >> str;  // railtex
        m_material1 =
            (str == "none" ? null_handle : GfxRenderer->Fetch_Material(str));
        parser->getTokens();
        ...
        *parser >> fTexHeight1 >> fTexWidth >> fTexSlope;
        ...
        if (iCategoryFlag & 4)
            fTexHeight1 =
                -fTexHeight1;  // rzeki mają wysokość odwrotnie niż drogi
    ...
    switch (eType) {
        ...
        case tt_Normal: {
            ...
            if (iCategoryFlag & 1) {  // zero na główce szyny
                                      // TODO: delay these calculations unti
                                      // rail profile and thus height is known
            ...
            if (fRadius != 0) {
                ...
                segsize = clamp(
                    ...
                    2.0 / Global.SplineFidelity, 10.0 / Global.SplineFidelity);
            } else {
                // HACK: crude check whether claimed straight is an actual
                // straight piece
                if ((cp1 == Math3D::vector3()) && (cp2 == Math3D::vector3())) {
                    segsize = 10.0;  // for straights, 10m per segment works
                                     // good enough
                } else {
                    // HACK: divide roughly in 10 segments.
                    segsize = clamp(
                        (p1 - p2).Length() * 0.1, 2.0 / Global.SplineFidelity,
            ...
            {  // SwitchExtension=new TSwitchExtension(this,1); //dodatkowe
               // zmienne dla obrotnicy
                SwitchExtension->Segments[0]->Init(
                    p1, p2, segsize, r1, r2);  // kopia oryginalnego toru
            } else if (iCategoryFlag & 2)
                if (m_material1 && fTexLength) {  // dla drogi trzeba ustalić
                                                  // proporcje boków nawierzchni
                    auto const &texture1{GfxRenderer->Texture(
                        GfxRenderer->Material(m_material1).textures[0])};
                    if (texture1.height() > 0) {
                        fTexRatio1 = static_cast<float>(texture1.width()) /
                                     static_cast<float>(
                                         texture1.height());  // proporcja boków
                    }
                    auto const &texture2{GfxRenderer->Texture(
                        GfxRenderer->Material(m_material2).textures[0])};
                    if (texture2.height() > 0) {
                        fTexRatio2 = static_cast<float>(texture2.width()) /
                                     static_cast<float>(
                                         texture2.height());  // proporcja boków
                    }
                }
            break;
        }
        case tt_Cross: {
            ...
            //        segsize = 1.0; // specjalne segmentowanie ze względu na
            //        małe promienie
        }
        case tt_Tributary:  // dopływ
        case tt_Switch: {   // zwrotnica
            iAction |= 1;   // flaga zmiany położenia typu zwrotnica lub
                            // skrzyżowanie dróg
            // problemy z animacją iglic powstaje, gdzy odcinek prosty ma
            // zmienną przechyłkę wtedy dzieli się na dodatkowe odcinki (po
            // 0.2m, bo R=0) i animację diabli biorą Ra: na razie nie podejmuję
            // się przerabiania iglic

            // SwitchExtension=new TSwitchExtension(this,eType==tt_Cross?6:2);
            // //zwrotnica ma doklejkę
            auto const &path{m_paths[0]};
            p1 = path.points[segment_data::point::start];
            // pobranie współrzędnych punktów kontrolnych
            cp1 = path.points[segment_data::point::control1];
            ...
            r2 = path.rolls[1];
            fRadiusTable[0] =
                std::abs(path.radius);  // we wpisie może być ujemny
            ...
            if ((cp1 == Math3D::vector3(0, 0, 0)) &&
                ...
                // gdy łuk (ustawia bCurve=true)
                SwitchExtension->Segments[0]->Init(
                    p1, cp1 + p1, cp2 + p2, p2, segsize, r1, r2);
            }
            ...
            r4 = path2.rolls[1];
            fRadiusTable[1] =
                std::abs(path2.radius);  // we wpisie może być ujemny
            ...
            if (fRadiusTable[1] != 0) {
                ...
                if ((cp3 == Math3D::vector3()) && (cp4 == Math3D::vector3())) {
                    ...
                    // HACK: divide roughly in 10 segments.
                    segsize = clamp(
                        (p3 - p4).Length() * 0.1, 2.0 / Global.SplineFidelity,
                        10.0 / Global.SplineFidelity);
                }
            }
            ...
            if ((cp3 == Math3D::vector3(0, 0, 0)) &&
                ...
                if (eType != tt_Cross) {
                    SwitchExtension->Segments[1]->Init(
                        p3, p3 + cp3, p4 + cp4, p4, segsize, r3, r4);
                } else {
                    // dla skrzyżowania dróg dać odwrotnie końce, żeby brzegi
                    // generować lewym
                    SwitchExtension->Segments[1]->Init(
                        p4, p4 + cp4, p3 + cp3, p3, segsize, r4,
                        r3);  // odwrócony
                }
            }

            if (eType == tt_Cross) {  // Ra 2014-07: dla skrzyżowań będą
                                      // dodatkowe segmenty
                SwitchExtension->Segments[2]->Init(
                    p2, cp2 + p2, cp4 + p4, p4, segsize, r2,
                    r4);  // z punktu 2 do 4
                if (LengthSquared3(p3 - p1) <
                    0.01)  // gdy mniej niż 10cm, to mamy skrzyżowanie trzech
                           // dróg
                    SwitchExtension->iRoads = 3;
                else  // dla 4 dróg będą dodatkowe 3 segmenty
                {
                    SwitchExtension->Segments[3]->Init(
                        p4, p4 + cp4, p1 + cp1, p1, segsize, r4,
                        r1);  // z punktu 4 do 1
                    SwitchExtension->Segments[4]->Init(
                        p1, p1 + cp1, p3 + cp3, p3, segsize, r1,
                        r3);  // z punktu 1 do 3
                    SwitchExtension->Segments[5]->Init(
                        p3, p3 + cp3, p2 + cp2, p2, segsize, r3,
                        r2);  // z punktu 3 do 2
                }
            }

            Switch(0);  // na stałe w położeniu 0 - nie ma początkowego stanu
                        // zwrotnicy we wpisie
            ...
            // Ra: zamienić później na iloczyn wektorowy
            {
                ...
                while (a2 < -M_PI) a2 = a2 + 2 * M_PI;
                SwitchExtension->RightSwitch =
                    a2 < 0;  // lustrzany układ OXY...
            }
            break;
        }
    }
    ...
    *parser >> token;
        ...
        if (str == "event0") {
            ...
            *parser >> fVelocity;  //*0.28; McZapkie-010602
            if (SwitchExtension)   // jeśli tor ruchomy
                if (std::abs(fVelocity) >=
                    1.0)  //żeby zero nie ograniczało dożywotnio
                    // zapamiętanie głównego ograniczenia; a np. -40 ogranicza
                    // tylko na bok
                    SwitchExtension->fVelocity = static_cast<float>(fVelocity);
        } else if (str == "isolated") {  // obwód izolowany, do którego tor
                                         // należy
            parser->getTokens();
            *parser >> token;
            ...
            *parser >> token;
            // nFouling[1]=
        } else if (str == "overhead") {  // informacja o stanie sieci: 0-jazda
                                         // bezprądowa, >0-z opuszczonym i
                                         // ograniczeniem prędkości
            parser->getTokens();
            *parser >> fOverhead;
            if (fOverhead > 0.0)
                iAction |= 0x40;  // flaga opuszczenia pantografu (tor
                                  // uwzględniany w skanowaniu jako
            ...
            *parser >> fVerticalRadius;
            ...
            if (eType == tt_Switch) {
                SwitchExtension->m_material3 =
                    GfxRenderer->Fetch_Material(trackbedtexture);
            }
            ...
            ErrorLog(
                "Bad track: unknown property: \"" + str +
                "\" defined for track \"" + m_name + "\"");
        parser->getTokens();
        *parser >> token;
        str = token;
    }
    ...
    if (!pIsolated)
        if ((i = m_name.find("@")) != std::string::npos)
            ...
            {
                pIsolated =
                    TIsolated::Find(m_name.substr(i + 1, m_name.length()));
...
bool TTrack::AssignEvents() {
    ...
    for (auto &eventsequence : eventsequences) {
        for (auto &event : *(eventsequence.second)) {
            ...
            if (event.second != nullptr) {
                ...
                ErrorLog(
                    "Bad track: " +
                    (m_name.empty() ? "unnamed track" : "\"" + m_name + "\"") +
                    " can't find assigned event \"" + event.first + "\"");
                lookupfail = true;
            }
        }
    }
    ...
    if ((Global.iHiddenEvents & 1) && (false == trackname.empty())) {
        // jeśli podana jest nazwa torów, można szukać eventów skojarzonych
        // przez nazwę
        for (auto &eventsequence : eventsequences) {
            auto *event = simulation::Events.FindEvent(
                trackname + ':' + eventsequence.first);
            if (event != nullptr) {
                // HACK: auto-associated events come with empty lookup string,
                // to avoid including them in the text format export
...
bool TTrack::AssignForcedEvents(
    basic_event *NewEventPlus,
    basic_event *NewEventMinus) {  // ustawienie eventów sygnalizacji rozprucia
    if (SwitchExtension) {
        if (NewEventPlus) SwitchExtension->evPlus = NewEventPlus;
        if (NewEventMinus) SwitchExtension->evMinus = NewEventMinus;
        return true;
    }
    return false;
};

void TTrack::QueueEvents(
    event_sequence const &Events, TDynamicObject const *Owner) {
...
std::string TTrack::IsolatedName() {  // podaje nazwę odcinka izolowanego, jesli
                                      // nie ma on jeszcze przypisanych zdarzeń
    if (pIsolated)
        if (!pIsolated->evBusy && !pIsolated->evFree) return pIsolated->asName;
    return "";
};

// ABu: przeniesione z Path.h i poprawione!!!
bool TTrack::AddDynamicObject(
    TDynamicObject *Dynamic) {  // dodanie pojazdu do trajektorii
    ...
    if (Dynamic->iOverheadMask) {
        // jeśli ma pantografy
        Dynamic->OverheadTrack(fOverhead);  // przekazanie informacji o jeździe
                                            // bezprądowej na tym odcinku toru
    }
    return true;
};
...
bool TTrack::CheckDynamicObject(
    TDynamicObject
        *Dynamic) {  // sprawdzenie, czy pojazd jest przypisany do toru
...
bool TTrack::RemoveDynamicObject(
    TDynamicObject
        *Dynamic) {  // usunięcie pojazdu z listy przypisanych do toru
    bool result = false;
    if (*Dynamics.begin() == Dynamic) {
        ...
        for (auto idx = Dynamics.begin(); idx != Dynamics.end();
             /*iterator advancement is inside the loop*/) {
            if (*idx == Dynamic) {
...
bool TTrack::InMovement() {  // tory animowane (zwrotnica, obrotnica) mają
                             // SwitchExtension
    if (SwitchExtension) {
        if (eType == tt_Switch)
            ...
            if (SwitchExtension->pModel) {
                ...
                auto ac =
                    (SwitchExtension->pModel
                         ...
                         : nullptr);
                return ac ? (ac->AngleGet() != SwitchExtension->fOffset) ||
...
void TTrack::RaAssign(
    TAnimModel *am, basic_event *done,
    basic_event *joined) {  // Ra: wiązanie toru z modelem obrotnicy
    if (eType == tt_Table) {
        SwitchExtension->pModel = am;
        SwitchExtension->evMinus =
            done;  // event zakończenia animacji (zadanie nowej przedłuża)
        SwitchExtension->evPlus =
            joined;  // event potwierdzenia połączenia (gdy nie znajdzie, to się
                     // nie połączy)
        if ((am != nullptr) && (am->GetContainer())) {  // może nie być?
            am->GetContainer()->EventAssign(
                done);  // zdarzenie zakończenia animacji
        }
    }
};

void TTrack::create_map_geometry(
    std::vector<gfx::basic_vertex> &Bank,
    const gfx::geometrybank_handle Extra) {
    if (iCategoryFlag != 1) return;  // only tracks for now
    ...
    switch (eType) {
        case tt_Normal: {
            ...
            std::copy(
                vertices.begin(), vertices.end(), std::back_inserter(Bank));
        ...
        case tt_Switch: {
            ...
            std::copy(
                vertices.begin(), vertices.end(), std::back_inserter(Bank));
            SwitchExtension->map_geometry[0] =
                GfxRenderer->Insert(vertices, Extra, GL_LINES);

            vertices.clear();
            SwitchExtension->Segments[1]->render_lines(vertices, 0.5f);
            std::copy(
                vertices.begin(), vertices.end(), std::back_inserter(Bank));
            SwitchExtension->map_geometry[1] =
                GfxRenderer->Insert(vertices, Extra, GL_LINES);
            break;
        }
        default:
...
void TTrack::get_map_paths_for_state(map_colored_paths &handles, int state) {
    if (iCategoryFlag != 1 || eType != tt_Switch) return;
...
glm::vec3 TTrack::get_nearest_point(const glm::dvec3 &point) const {
    if (eType == tt_Normal) {
        ...
        for (size_t i = 0; i < 2; i++) {
            glm::dvec3 p =
                SwitchExtension->Segments[i]->get_nearest_point(point);
...
void TTrack::create_geometry(gfx::geometrybank_handle const &Bank) {
    switch (iCategoryFlag & 15) {
        case 1:  // tor
        {
            // zwykla szyna: //Ra: czemu główki są asymetryczne na wysokości
            // 0.140?
            gfx::vertex_array rpts1, rpts2;
            create_track_rail_profile(rpts1, rpts2);
            switch (eType)  // dalej zależnie od typu
            {
                case tt_Table:  // obrotnica jak zwykły tor, tylko animacja
                                // dochodzi
                case tt_Normal:
                    if (m_material2) {  // podsypka z podkładami jest tylko dla
                                        // zwykłego toru
                        ...
                        Segment->RenderLoft(
                            vertices, m_origin, bpts1, iTrapezoid > 0,
                            texturelength);
                        if ((Bank != 0) && (true == Geometry2.empty())) {
                            Geometry2.emplace_back(GfxRenderer->Insert(
                                vertices, Bank, GL_TRIANGLE_STRIP));
                        }
                        if ((Bank == 0) && (false == Geometry2.empty())) {
                            // special variant, replace existing data for a
                            // turntable track
                            GfxRenderer->Replace(
                                vertices, Geometry2[0], GL_TRIANGLE_STRIP);
                        }
                    }
                    if (m_material1) {  // szyny - generujemy dwie, najwyżej
                                        // rysować się będzie jedną
                        auto const nnumPts{
                            track_rail_profile(m_profile1.second).size() / 2};
                        auto const texturelength{texture_length(m_material1)};
                        gfx::vertex_array vertices;
                        if ((Bank != 0) && (true == Geometry1.empty())) {
                            Segment->RenderLoft(
                                vertices, m_origin, rpts1, iTrapezoid > 0,
                                texturelength);
                            Geometry1.emplace_back(GfxRenderer->Insert(
                                vertices, Bank, GL_TRIANGLE_STRIP));
                            vertices.clear();  // reuse the scratchpad
                            Segment->RenderLoft(
                                vertices, m_origin, rpts2, iTrapezoid > 0,
                                texturelength);
                            Geometry1.emplace_back(GfxRenderer->Insert(
                                vertices, Bank, GL_TRIANGLE_STRIP));
                        }
                        if ((Bank == 0) && (false == Geometry1.empty())) {
                            ...
                            Segment->RenderLoft(
                                ...
                                texturelength);
                            GfxRenderer->Replace(
                                vertices, Geometry1[0], GL_TRIANGLE_STRIP);
                            vertices.clear();  // reuse the scratchpad
                            Segment->RenderLoft(
                                vertices, m_origin, rpts2, iTrapezoid > 0,
                                texturelength);
                            GfxRenderer->Replace(
                                vertices, Geometry1[1], GL_TRIANGLE_STRIP);
                        }
                    }
                    break;
                case tt_Switch:  // dla zwrotnicy dwa razy szyny
                    if (m_material1 || m_material2) {
                        ...
                        // TODO, TBD: change all track geometry to triangles, to
                        // allow packing data in less, larger buffers
                        auto const nnumPts{
                            track_rail_profile(m_profile1.second).size() / 2};
                        auto const bladelength{static_cast<int>(std::ceil(
                            SwitchExtension->Segments[0]->RaSegCount() *
                            0.65))};
                        // positive jointlength: the switch is typically used
                        // along the main track, negative: along the diverging
                        // track
                        // TODO: determine this from names of textures assigned
                        // to the tracks
                        //                auto const jointlength {
                        //                static_cast<int>( std::ceil(
                        //                SwitchExtension->Segments[ 0
                        //                ]->RaSegCount() * 0.15 ) ) };
                        auto const jointlength{
                            0};  // temporary until implementation of the above
                        if (SwitchExtension
                                ->RightSwitch) {  // nowa wersja z SPKS, ale
                                                  // odwrotnie lewa/prawa
                            gfx::vertex_array vertices;
                            if (m_material1) {
                                auto const texturelength{
                                    texture_length(m_material1)};
                                // left blade
                                // composed from two parts: transition from
                                // blade to regular rail, and regular rail
                                SwitchExtension->Segments[0]->RenderLoft(
                                    vertices, m_origin, rpts3, true,
                                    texturelength, 1.0, 0, bladelength / 2,
                                    {SwitchExtension->fOffset2,
                                     SwitchExtension->fOffset2 / 2});
                                SwitchExtension->Segments[0]->RenderLoft(
                                    vertices, m_origin, rpts1, false,
                                    texturelength, 1.0, bladelength / 2,
                                    bladelength,
                                    {SwitchExtension->fOffset2 / 2, 0.f});
                                Geometry1.emplace_back(GfxRenderer->Insert(
                                    vertices, Bank, GL_TRIANGLE_STRIP));
                                vertices.clear();
                                // fixed parts
                                SwitchExtension->Segments[0]->RenderLoft(
                                    vertices, m_origin, rpts1, false,
                                    texturelength, 1.0, bladelength);
                                Geometry1.emplace_back(GfxRenderer->Insert(
                                    vertices, Bank, GL_TRIANGLE_STRIP));
                                vertices.clear();
                                if (jointlength > 0) {
                                    // part of the diverging rail touched by
                                    // wheels of vehicle going straight
                                    SwitchExtension->Segments[1]->RenderLoft(
                                        vertices, m_origin, rpts1, false,
                                        texturelength, 1.0, 0, jointlength);
                                    Geometry1.emplace_back(GfxRenderer->Insert(
                                        vertices, Bank, GL_TRIANGLE_STRIP));
                                    vertices.clear();
                                }
                            ...
                            if (m_material2) {
                                auto const texturelength{
                                    texture_length(m_material2)};
                                // right blade
                                // composed from two parts: transition from
                                // blade to regular rail, and regular rail
                                SwitchExtension->Segments[1]->RenderLoft(
                                    vertices, m_origin, rpts4, true,
                                    texturelength, 1.0, 0, bladelength / 2,
                                    {-fMaxOffset + SwitchExtension->fOffset1,
                                     (-fMaxOffset + SwitchExtension->fOffset1) /
                                         2});
                                SwitchExtension->Segments[1]->RenderLoft(
                                    vertices, m_origin, rpts2, false,
                                ...
                                Geometry2.emplace_back(GfxRenderer->Insert(
                                    vertices, Bank, GL_TRIANGLE_STRIP));
                                vertices.clear();
                                // diverging rail, potentially minus part
                                // touched by wheels of vehicle going straight
                                SwitchExtension->Segments[1]->RenderLoft(
                                    vertices, m_origin, rpts1, false,
                                    texturelength, 1.0, jointlength);
                                Geometry2.emplace_back(GfxRenderer->Insert(
                                    vertices, Bank, GL_TRIANGLE_STRIP));
                                vertices.clear();
                            }
                    ...
                    if (true == Global.CreateSwitchTrackbeds) {
                        ...
                        SwitchExtension->Geometry3 = GfxRenderer->Insert(
                            vertices, Bank, GL_TRIANGLE_STRIP);
                        vertices.clear();
                    }
        ...
        case 2:             // McZapkie-260302 - droga - rendering
            switch (eType)  // dalej zależnie od typu
            {
                ...
                {
                    gfx::vertex_array bpts1;  // punkty głównej płaszczyzny
                                              // przydają się do robienia boków
                    if (m_material1 || m_material2) {
                        // punkty się przydadzą, nawet jeśli nawierzchni nie ma
                        create_road_profile(bpts1);
                    }
                    if (m_material1)  // jeśli podana była tekstura, generujemy
                                      // trójkąty
                    ...
                    if (m_material2) {  // pobocze drogi - poziome przy
                                        // przechyłce (a może krawężnik i
                                        // chodnik zrobić jak w Midtown Madness
                                        // 2?)
                        auto const side{std::abs(
                            fTexWidth)};  // szerokść podsypki na zewnątrz szyny
                                          // albo pobocza
                        auto const slop{
                            std::abs(fTexSlope)};  // brzeg zewnętrzny
                        auto const texturelength{texture_length(m_material2)};
                        gfx::vertex_array rpts1,
                            rpts2;  // współrzędne przekroju i mapowania dla
                ...
                {  // ustalenie współrzędnych środka - przecięcie Point1-Point2
                   // z CV4-Point4
                    double a[4];           // kąty osi ulic wchodzących
                    Math3D::vector3 p[4];  // punkty się przydadzą do obliczeń
                    // na razie połowa odległości pomiędzy Point1 i Point2,
                    // potem się dopracuje
                    a[0] = a[1] =
                        0.5;  // parametr do poszukiwania przecięcia łuków
                    // modyfikować a[0] i a[1] tak, aby trafić na przecięcie
                    // odcinka 34
                    p[0] = SwitchExtension->Segments[0]->FastGetPoint(
                        a[0]);  // współrzędne środka pierwszego odcinka
                    p[1] = SwitchExtension->Segments[1]->FastGetPoint(
                        a[1]);  //-//- drugiego
                    // p[2]=p[1]-p[0]; //jeśli różne od zera, przeliczyć a[0] i
                    // a[1] i wyznaczyć nowe punkty
                    Math3D::vector3 oxz =
                        p[0];  // punkt mapowania środka tekstury skrzyżowania
                    p[0] = SwitchExtension->Segments[0]
                               ->GetDirection1();  // Point1 - pobranie wektorów
                                                   // kontrolnych
                    p[1] = SwitchExtension->Segments[1]
                               ->GetDirection2();  // Point3 (bo zamienione)
                    p[2] = SwitchExtension->Segments[0]
                               ->GetDirection2();  // Point2
                    p[3] = SwitchExtension->Segments[1]
                               ->GetDirection1();   // Point4 (bo zamienione)
                    a[0] = atan2(-p[0].x, p[0].z);  // kąty stycznych osi dróg
                    a[1] = atan2(-p[1].x, p[1].z);
                    a[2] = atan2(-p[2].x, p[2].z);
                    a[3] = atan2(-p[3].x, p[3].z);
                    p[0] = SwitchExtension->Segments[0]
                               ->FastGetPoint_0();  // Point1 - pobranie
                                                    // współrzędnych końców
                    p[1] = SwitchExtension->Segments[1]
                               ->FastGetPoint_1();  // Point3
                    p[2] = SwitchExtension->Segments[0]
                               ->FastGetPoint_1();  // Point2
                    p[3] =
                        SwitchExtension->Segments[1]
                            ->FastGetPoint_0();  // Point4 - przy trzech drogach
                                                 // pokrywa się z Point1
                    // 2014-07: na początek rysować brzegi jak dla łuków
                    // punkty brzegu nawierzchni uzyskujemy podczas renderowania
                    // boków (bez sensu, ale najszybciej było zrobić)
                    int pointcount;
                    ...
                    if (!SwitchExtension
                             ->vPoints) {  // jeśli tablica punktów nie jest
                                           // jeszcze utworzona, zliczamy punkty
                                           // i tworzymy ją
                        // we'll need to add couple extra points for the
                        // complete fan we'll build
                        SwitchExtension->vPoints =
                            new glm::vec3[pointcount + SwitchExtension->iRoads];
                    }
                    glm::vec3 *b =
                        SwitchExtension->bPoints
                            ? nullptr
                            : SwitchExtension
                                  ->vPoints;  // zmienna robocza, NULL gdy
                                              // tablica punktów już jest
                                              // wypełniona
                    gfx::vertex_array bpts1;  // punkty głównej płaszczyzny
                                              // przydają się do robienia boków
                    if (m_material1 ||
                        m_material2) {  // punkty się przydadzą, nawet jeśli
                                        // nawierzchni nie ma
                        create_road_profile(bpts1, true);
                    }
                    // najpierw renderowanie poboczy i zapamiętywanie punktów
                    // problem ze skrzyżowaniami jest taki, że teren chce się
                    // pogrupować wg tekstur, ale zaczyna od nawierzchni sama
                    // nawierzchnia nie wypełni tablicy punktów, bo potrzebne są
                    // pobocza ale pobocza renderują się później, więc
                    // nawierzchnia nie załapuje się na renderowanie w swoim
                    // czasie
                    if (m_material2) {  // pobocze drogi - poziome przy
                                        // przechyłce (a może krawężnik i
                                        // chodnik zrobić jak w Midtown Madness
                        ...
                        // Ra 2014-07: trzeba to przerobić na pętlę i pobierać
                        // profile (przynajmniej 2..4) z sąsiednich dróg
                        bool render =
                            (m_material2 !=
                             0);  // renderować nie trzeba, ale trzeba wyznaczyć
                        ...
                        auto const texturelength{texture_length(m_material2)};
                        gfx::vertex_array vertices;
                        if (SwitchExtension->iRoads ==
                            4) {  // pobocza do trapezowatej nawierzchni -
                                  // dodatkowe punkty z drugiej strony odcinka
                            if ((fTexHeight1 >= 0.0) || (side != 0.0)) {
                                SwitchExtension->Segments[2]->RenderLoft(
                                    vertices, m_origin, rpts2, true,
                                    texturelength, 1.0, 0, 0, {}, &b, render);
                                if (true == render) {
                                    Geometry2.emplace_back(GfxRenderer->Insert(
                                        vertices, Bank, GL_TRIANGLE_STRIP));
                                    vertices.clear();
                            ...
                            // punkt 3 pokrywa się z punktem 1, jak w zwrotnicy;
                            // połączenie 1->2 nie musi być prostoliniowe
                            if ((fTexHeight1 >= 0.0) || (side != 0.0)) {
                                SwitchExtension->Segments[2]->RenderLoft(
                                    vertices, m_origin, rpts2, true,
                                    texturelength, 1.0, 0, 0, {}, &b,
                                    render);  // z P2 do P4
                                if (true == render) {
                                    Geometry2.emplace_back(GfxRenderer->Insert(
                                        vertices, Bank, GL_TRIANGLE_STRIP));
                    ...
                    double u, v;
                    if ((false == SwitchExtension->bPoints)  // jeśli tablica
                                                             // nie wypełniona
                        && (b != nullptr)) {
                        SwitchExtension->bPoints =
                            true;  // tablica punktów została wypełniona
                    }
                    ...
                    if (m_material1) {
                        ...
                        vertices.emplace_back(
                            glm::vec3{
                                oxz.x - m_origin.x, oxz.y - m_origin.y,
                                oxz.z - m_origin.z},
                            glm::vec3{0.0f, 1.0f, 0.0f}, glm::vec2{0.5f, 0.5f});
                        // ...and add one extra vertex to close the fan...
                        u = (SwitchExtension->vPoints[0].x - oxz.x +
                             m_origin.x) /
                            texturelength;
                        v = (SwitchExtension->vPoints[0].z - oxz.z +
                             m_origin.z) /
                            (fTexRatio1 * texturelength);
                        vertices.emplace_back(
                            glm::vec3{
                        ...
                        for (int i = pointcount + SwitchExtension->iRoads - 1;
                             i >= 0; --i) {
                            // mapowanie we współrzędnych scenerii
                            u = (SwitchExtension->vPoints[i].x - oxz.x +
                                 m_origin.x) /
                                texturelength;
                            v = (SwitchExtension->vPoints[i].z - oxz.z +
                                 m_origin.z) /
                                (fTexRatio1 * texturelength);
                            vertices.emplace_back(
                                glm::vec3{
...
void TTrack::RenderDynSounds() {  // odtwarzanie dźwięków pojazdów jest
                                  // niezależne od ich wyświetlania
    for (auto dynamic : Dynamics) {
        dynamic->RenderSounds();
    }
};
//---------------------------------------------------------------------------
bool TTrack::SetConnections(
    int i) {  // przepisanie aktualnych połączeń toru do odpowiedniego segmentu
    if (SwitchExtension) {
        ...
        if (eType == tt_Switch) {  // zwrotnica jest wyłącznie w punkcie 1, więc
                                   // tor od strony Prev jest zawsze ten sam
...
bool TTrack::Switch(
    int i, float const t,
    float const d) {      // przełączenie torów z uruchomieniem animacji
    if (SwitchExtension)  // tory przełączalne mają doklejkę
        if (eType == tt_Switch) {  // przekładanie zwrotnicy jak zwykle
            if (t > 0.f)           // prędkość liniowa ruchu iglic
                SwitchExtension->fOffsetSpeed =
                    t;  // prędkość łatwiej zgrać z animacją modelu
            if (d >=
                0.f)  // dodatkowy ruch drugiej iglicy (zamknięcie nastawnicze)
                SwitchExtension->fOffsetDelay = d;
            i &= 1;  // ograniczenie błędów !!!!
            SwitchExtension->fDesiredOffset =
                i ? fMaxOffset + SwitchExtension->fOffsetDelay
                  : -SwitchExtension->fOffsetDelay;
            SwitchExtension->CurrentIndex = i;
            Segment =
                SwitchExtension
                    ->Segments[i];  // wybranie aktywnej drogi - potrzebne to?
            ...
            if (SwitchExtension->fVelocity <= -2) {
                //-1 oznacza maksymalną prędkość, a dalsze ujemne to
                //ograniczenie na bok
            ...
            if (SwitchExtension->pOwner
                    ? SwitchExtension->pOwner->RaTrackAnimAdd(this)
                    : true)  // jeśli nie dodane do animacji
            {                // nie ma się co bawić
                SwitchExtension->fOffset = SwitchExtension->fDesiredOffset;
                // przeliczenie położenia iglic; czy zadziała na niewyświetlanym
                // sektorze w VBO?
                RaAnimate();
            }
            return true;
        } else if (eType == tt_Table) {  // blokowanie (0, szukanie torów) lub
                                         // odblokowanie (1, rozłączenie)
                                         // obrotnicy
            if (i)  // NOTE: this condition seems opposite to intention/comment?
                    // TODO: investigate this
            {       // 0: rozłączenie sąsiednich torów od obrotnicy
                if (trPrev)              // jeśli jest tor od Point1 obrotnicy
                    if (iPrevDirection)  // 0:dołączony Point1, 1:dołączony
                                         // Point2
                ...
                if (trNext)              // jeśli jest tor od Point2 obrotnicy
                    if (iNextDirection)  // 0:dołączony Point1, 1:dołączony
                                         // Point2
                        trNext->trNext = NULL;  // rozłączamy od Point2
                    else
                        trNext->trPrev = NULL;  // rozłączamy od Point1
                trNext = trPrev =
                    NULL;  // na końcu rozłączamy obrotnicę (wkaźniki do
                           // sąsiadów już niepotrzebne)
                fVelocity = 0.0;  // AI, nie ruszaj się!
                if (SwitchExtension->pOwner)
                    SwitchExtension->pOwner->RaTrackAnimAdd(
                        this);  // dodanie do listy animacyjnej
                // TODO: unregister path ends in the owner cell
            } else {  // 1: ustalenie finalnego położenia (gdy nie było
                      // animacji)
                RaAnimate();  // ostatni etap animowania
                // zablokowanie pozycji i połączenie do sąsiednich torów
                // TODO: register new position of the path endpoints with the
                // region
                ...
                if (trNext || trPrev) {
                    ...
                    if ((trPrev) && (trPrev->fVelocity == 0.0)) {
                        ...
                        trPrev->VelocitySet(
                            6.0);  // odblokowanie dołączonego toru do jazdy
                    }
                    if ((trNext) && (trNext->fVelocity == 0.0)) {
                        trNext->VelocitySet(6.0);
                    }
                    if (SwitchExtension
                            ->evPlus) {  // w starych sceneriach może nie być
                        // potwierdzenie wykonania (np. odpala WZ)
                        simulation::Events.AddToQuery(
                            SwitchExtension->evPlus, nullptr);
                    }
                }
            }
            SwitchExtension->CurrentIndex =
                i;  // zapamiętanie stanu zablokowania
            return true;
        } else if (eType == tt_Cross) {  // to jest przydatne tylko do łączenia
                                         // odcinków
            i &= 1;
            SwitchExtension->CurrentIndex = i;
            Segment =
    ...
    if (iCategoryFlag == 1)
        iDamageFlag =
            (iDamageFlag & 127) + 128 * (i & 1);  // przełączanie wykolejenia
    else
        Error("Cannot switch normal track");
    return false;
};

bool TTrack::SwitchForced(int i, TDynamicObject *o) {  // rozprucie rozjazdu
    if (SwitchExtension)
        if (eType == tt_Switch) {  //
            if (i != SwitchExtension->CurrentIndex) {
                ...
                Switch(i);  // jeśli się tu nie przełączy, to każdy pojazd
                            // powtórzy event rozrprucia
            }
        } else if (eType == tt_Cross) {  // ustawienie wskaźnika na wskazany
                                         // segment
...
int TTrack::CrossSegment(
    int from, int into) {  // ustawienie wskaźnika na segement w pożądanym
                           // kierunku (into) od strony (from)
    // zwraca kod segmentu, z kierunkiem jazdy jako znakiem ±
    int i = 0;
    switch (into) {
        case 0:  // stop
            //        WriteLog( "Stopping in P" + to_string( from + 1 ) + " on "
            //        + name() );
            break;
        case 1:  // left
            //        WriteLog( "Turning left from P" + to_string( from + 1 ) +
            //        " on " + name() );
            i = (SwitchExtension->iRoads == 4) ? iLewo4[from] : iLewo3[from];
            break;
        case 2:  // right
            //        WriteLog( "Turning right from P" + to_string( from + 1 ) +
            //        " on " + name() );
            i = (SwitchExtension->iRoads == 4) ? iPrawo4[from] : iPrawo3[from];
            break;
        case 3:  // stright
            //        WriteLog( "Going straight from P" + to_string( from + 1 )
            //        + " on " + name() );
            i = (SwitchExtension->iRoads == 4) ? iProsto4[from]
                                               : iProsto3[from];
            break;
    }
    if (i) {
...
void TTrack::RaAnimListAdd(TTrack *t) {  // dodanie toru do listy animacyjnej
    if ((t != nullptr) && (SwitchExtension != nullptr)) {
        ...
        if (SwitchExtension->pNextAnim) {
            ...
            t->SwitchExtension->pNextAnim =
                NULL;  // nowo dodawany nie może mieć ogona
        }
    }
};

TTrack *TTrack::RaAnimate() {  // wykonanie rekurencyjne animacji, wywoływane
                               // przed wyświetleniem sektora
    ...
    {
        auto const v = SwitchExtension->fDesiredOffset -
                       SwitchExtension->fOffset;  // kierunek
        SwitchExtension->fOffset +=
            sign(v) * Timer::GetDeltaTime() * SwitchExtension->fOffsetSpeed;
        ...
        if (SwitchExtension->fOffset1 > fMaxOffset)
            SwitchExtension->fOffset1 =
                fMaxOffset;  // ograniczenie animacji zewnętrznej iglicy
        if (SwitchExtension->fOffset2 < 0.f)
            SwitchExtension->fOffset2 =
                0.f;    // ograniczenie animacji wewnętrznej iglicy
        ...
        // skip the geometry update if no geometry for this track was generated
        // yet
        if (((m_material1 != 0) || (m_material2 != 0)) &&
            ((false == Geometry1.empty()) || (false == Geometry2.empty()))) {
            // iglice liczone tylko dla zwrotnic
            gfx::vertex_array rpts1, rpts2;
            create_track_rail_profile(rpts1, rpts2);
            gfx::vertex_array rpts3, rpts4;
            create_track_blade_profile(rpts3, rpts4);
            gfx::vertex_array vertices;
            auto const bladelength{static_cast<int>(
                std::ceil(SwitchExtension->Segments[0]->RaSegCount() * 0.65))};
            auto const nnumPts{
                track_rail_profile(m_profile1.second).size() / 2};
            if (SwitchExtension->RightSwitch) {  // nowa wersja z SPKS, ale
                                                 // odwrotnie lewa/prawa
                if (m_material1) {
                    auto const texturelength{texture_length(m_material1)};
                    // left blade
                    // composed from two parts: transition from blade to regular
                    // rail, and regular rail
                    SwitchExtension->Segments[0]->RenderLoft(
                        vertices, m_origin, rpts3, true, texturelength, 1.0, 0,
                        bladelength / 2,
                        {SwitchExtension->fOffset2,
                         SwitchExtension->fOffset2 / 2});
                    SwitchExtension->Segments[0]->RenderLoft(
                        vertices, m_origin, rpts1, false, texturelength, 1.0,
                        bladelength / 2, bladelength,
                        {SwitchExtension->fOffset2 / 2, 0.f});
                    GfxRenderer->Replace(
                        vertices, Geometry1[0], GL_TRIANGLE_STRIP);
                    vertices.clear();
                ...
                if (m_material2) {
                    ...
                    // rail, and regular rail
                    SwitchExtension->Segments[1]->RenderLoft(
                        vertices, m_origin, rpts4, true, texturelength, 1.0, 0,
                        bladelength / 2,
                        {-fMaxOffset + SwitchExtension->fOffset1,
                         (-fMaxOffset + SwitchExtension->fOffset1) / 2});
                    SwitchExtension->Segments[1]->RenderLoft(
                        vertices, m_origin, rpts2, false, texturelength, 1.0,
                        bladelength / 2, bladelength,
                        {(-fMaxOffset + SwitchExtension->fOffset1) / 2, 0.f});
                    GfxRenderer->Replace(
                        vertices, Geometry2[0], GL_TRIANGLE_STRIP);
                    vertices.clear();
                }
        ...
        {  // trzeba każdorazowo porównywać z kątem modelu
           // //pobranie kąta z modelu
            auto ac =
                (SwitchExtension->pModel
                     ? SwitchExtension->pModel->GetContainer()
                     :  // pobranie głównego submodelu
                     nullptr);
            if (ac) {
                if ((ac->AngleGet() != SwitchExtension->fOffset) ||
                    !(ac->TransGet() ==
                      SwitchExtension->vTrans)) {  // czy przemieściło się od
                                                   // ostatniego sprawdzania

                    double hlen = 0.5 * SwitchExtension->Segments[0]
                                            ->GetLength();  // połowa długości
                    SwitchExtension->fOffset =
                        SwitchExtension->pModel->Angles()
                            .y  // take into account orientation of the model
                        + ac->AngleGet();  // pobranie kąta z submodelu
                    double sina =
                               -hlen *
                               std::sin(glm::radians(SwitchExtension->fOffset)),
                           cosa =
                               -hlen *
                               std::cos(glm::radians(SwitchExtension->fOffset));
                    SwitchExtension->vTrans = ac->TransGet();
                    auto middle =
                        location() +
                        SwitchExtension
                            ->vTrans;  // SwitchExtension->Segments[0]->FastGetPoint(0.5);
                    Segment->Init(
                    ...
                    // NOTE: passing empty handle is a bit of a hack here. could
                    // be refactored into something more elegant
                    create_geometry({});
                }  // animacja trwa nadal
            }
        } else
    ...
    return m ? this
             : SwitchExtension->pNextAnim;  // zwraca obiekt do dalszej animacji
};
//---------------------------------------------------------------------------
void TTrack::RadioStop() {  // przekazanie pojazdom rozkazu zatrzymania
    for (auto dynamic : Dynamics) {
        dynamic->RadioStop();
    }
};

double TTrack::WidthTotal() {  // szerokość z poboczem
    if (iCategoryFlag & 2)     // jesli droga
        if (fTexHeight1 >=
            0.0)  // i ma boki zagięte w dół (chodnik jest w górę)
            return 2.0 * fabs(fTexWidth) +
                   0.5 * fabs(fTrackWidth + fTrackWidth2);  // dodajemy pobocze
    return 0.5 * fabs(
                     fTrackWidth +
                     fTrackWidth2);  // a tak tylko zwykła średnia szerokość
};

bool TTrack::IsGroupable() {  // czy wyświetlanie toru może być zgrupwane z
                              // innymi
...
bool Equal(
    Math3D::vector3 v1,
    Math3D::vector3 *v2) {  // sprawdzenie odległości punktów
    // Ra: powinno być do 100cm wzdłuż toru i ze 2cm w poprzek (na prostej może
    // nie być długiego kawałka) Ra: z automatycznie dodawanym stukiem, jeśli
    // dziura jest większa niż 2mm.
...
int TTrack::TestPoint(
    Math3D::vector3 *Point) {  // sprawdzanie, czy tory można połączyć
    switch (eType) {
        case tt_Normal:  // zwykły odcinek
            if (trPrev == NULL)
                if (Equal(Segment->FastGetPoint_0(), Point)) return 0;
            if (trNext == NULL)
                if (Equal(Segment->FastGetPoint_1(), Point)) return 1;
            break;
        case tt_Switch:  // zwrotnica
        {
            int state = GetSwitchState();  // po co?
            // Ra: TODO: jak się zmieni na bezpośrednie odwołania do segmentow
            // zwrotnicy, to się wykoleja, ponieważ trNext zależy od przełożenia
            Switch(0);
            if (trPrev == NULL)
                // if
                // (Equal(SwitchExtension->Segments[0]->FastGetPoint_0(),Point))
            ...
            if (trNext == NULL)
                // if
                // (Equal(SwitchExtension->Segments[0]->FastGetPoint_1(),Point))
            ...
            if (trPrev == NULL)  // Ra: z tym chyba nie potrzeba łączyć
                // if
                // (Equal(SwitchExtension->Segments[1]->FastGetPoint_0(),Point))
                if (Equal(Segment->FastGetPoint_0(), Point)) {
            ...
            if (trNext == NULL)  // TODO: to zależy od przełożenia zwrotnicy
                // if
                // (Equal(SwitchExtension->Segments[1]->FastGetPoint_1(),Point))
                if (Equal(Segment->FastGetPoint_1(), Point)) {
                    Switch(state);  // Switch(0);
                    return 5;
                }
...
float TTrack::radius_() {
    ...
    for (auto &point : points) {
        radius = std::max(
            radius,
            static_cast<float>(glm::length(
                m_area.center - point)));  // extra margin to prevent driven
                                           // vehicle from flicking
    }
    return radius;
}
...
void TTrack::export_as_text_(std::ostream &Output) const {
    ...
    Output << "track ";
    // type
    Output << (eType == tt_Normal      ? (iCategoryFlag == 1   ? "normal"
           ...
           << ' ';
    // basic attributes
    Output << Length() << ' ' << fTrackWidth << ' ' << fFriction << ' '
           << fSoundDistance << ' ' << iQualityFlag << ' ' << iDamageFlag
           << ' ';
    // environment
    Output << (eEnvironment == e_flat        ? "flat"
           ...
           << ' ';
    // visibility
    // NOTE: 'invis' would be less wrong than 'unvis', but potentially
    // incompatible with old 3rd party tools
    Output << (m_visible ? "vis" : "unvis") << ' ';
    if (m_visible) {
        // texture parameters are supplied only if the path is set as visible
        auto texturefile{(
            m_material1 != null_handle ? GfxRenderer->Material(m_material1).name
                                       : "none")};
        ...
        Output << (fTexHeight1 - fTexHeightOffset) *
                      ((iCategoryFlag & 4) ? -1 : 1)
               << ' ' << fTexWidth << ' ' << fTexSlope << ' ';
    }
    // path data
    for (auto const &path : m_paths) {
        Output << path.points[segment_data::point::start].x << ' '
    ...
    // optional attributes
    std::vector<std::pair<std::string, event_sequence const *> > const
        eventsequences{{"event0", &m_events0}, {"eventall0", &m_events0all},
                       {"event1", &m_events1}, {"eventall1", &m_events1all},
                       {"event2", &m_events2}, {"eventall2", &m_events2all}};
    ...
    for (auto &eventsequence : eventsequences) {
        for (auto &event : *(eventsequence.second)) {
            ...
            // cached name, if it was renamed in the editor therefore on export
            // we pull the name from the event itself, if the binding isn't
            // broken
            Output << eventsequence.first << ' '
                   << (event.second != nullptr ? event.second->m_name
                                               : event.first)
                   << ' ';
        }
    }
    ...
    if ((eType == tt_Switch) && (SwitchExtension->m_material3 != null_handle)) {
...
// locates specified profile in the profile database, potentially loading it
// from a file returns: pair <profile name, profile handle>
std::pair<std::string, int> TTrack::fetch_track_rail_profile(
    ...
    // ... and if it fails try to add the profile to the database from a data
    // file
    auto profilehandle{
        0};  // fallback link to default profile if loading it fails
    auto profiledata{deserialize_profile(railprofilepath + Profile)};
    if (false == profiledata.empty()) {
        // if we get the profile data add it to the database and calculate a
        // link to it
...
void TTrack::fetch_default_profiles() {
...
gfx::vertex_array TTrack::deserialize_profile(std::string const &Profile) {
    ...
    while (input.getTokens(5, true, "\n\r\t ;,{}")) {
        ...
        input >> vertex.position.x >> vertex.position.y >> vertex.normal.x >>
            vertex.normal.y >> vertex.texture.s;
...
gfx::vertex_array const &TTrack::track_rail_profile(int const Profile) {
    return m_profiles[Profile];
}

float TTrack::texture_length(material_handle const Material) {
...
void TTrack::MovedUp1(
    float const dh) {  // poprawienie przechyłki wymaga wydłużenia podsypki
    fTexHeight1 += dh;
    fTexHeightOffset += dh;
};

// ustawienie prędkości z ograniczeniem do pierwotnej wartości (zapisanej w
// scenerii)
void TTrack::VelocitySet(float v) {
    // TBD, TODO: add a variable to preserve potential speed limit set by the
    // track configuration on basic track pieces
    if ((SwitchExtension) && (SwitchExtension->fVelocity != -1)) {
        // zwrotnica może mieć odgórne ograniczenie, nieprzeskakiwalne eventem
        fVelocity = min_speed<float>(
            v,
            (SwitchExtension->fVelocity > 0
                 ? SwitchExtension->fVelocity
                 :  // positive limit applies to both switch tracks
                 (SwitchExtension->CurrentIndex == 0
                      ? -1
                      :  // negative limit applies only to the diverging track
                      -SwitchExtension->fVelocity)));
    } else {
...
double
TTrack::VelocityGet() {  // pobranie dozwolonej prędkości podczas skanowania
    return (
        (iDamageFlag & 128) ? 0.0
                            : fVelocity);  // tor uszkodzony = prędkość zerowa
};
...
float TTrack::Friction() const {
    if (m_friction.second == nullptr) {
        ...
        return clamp(
            fFriction * m_friction.second->Value1() +
                m_friction.second->Value2(),
            0.0, 1.0);
    }
}

void TTrack::ConnectionsLog() {  // wypisanie informacji o połączeniach
    int i;
...
TTrack *TTrack::Connected(
    ...
    if (eType != tt_Cross) {  // jeszcze trzeba sprawdzić zgodność
        t = (s > 0) ? trNext : trPrev;
        if (t)  // o ile jest na co przejść, zmieniamy znak kierunku na nowym
                // torze
            if (t->eType ==
                tt_Cross) {  // jeśli wjazd na skrzyżowanie, trzeba ustalić
                             // segment, bo od tego zależy zmiana kierunku (d)
                             // if (r) //gdy nie podano (r), to nie zmieniać (d)
                             // if
                             // (s*t->CrossSegment(((s>0)?iNextDirection:iPrevDirection),r)<0)
                             //  d=-d;
            } else {
                if ((s > 0) ? iNextDirection : !iPrevDirection)
                    d = -d;  // następuje zmiana kierunku wózka albo kierunku
                             // skanowania
                // s=((s>0)?iNextDirection:iPrevDirection)?-1:1; //kierunek toru
                // po zmianie
            }
        return (t);  // zwrotnica ma odpowiednio ustawione (trNext)
    }
    switch ((SwitchExtension->iRoads == 4)
                ? iEnds4[s + 6]
                : iEnds3[s + 6])  // numer końca 0..3, -1 to błąd
    {                             // zjazd ze skrzyżowania
        case 0:
            if (SwitchExtension->pPrevs[0])
                if ((s > 0) == SwitchExtension->iPrevDirection[0]) d = -d;
            return SwitchExtension->pPrevs[0];
        case 1:
            if (SwitchExtension->pNexts[0])
                if ((s > 0) == SwitchExtension->iNextDirection[0]) d = -d;
            return SwitchExtension->pNexts[0];
        case 2:
            if (SwitchExtension->pPrevs[1])
                if ((s > 0) == SwitchExtension->iPrevDirection[1]) d = -d;
            return SwitchExtension->pPrevs[1];
        case 3:
            if (SwitchExtension->pNexts[1])
                if ((s > 0) == SwitchExtension->iNextDirection[1]) d = -d;
...
void TTrack::create_track_rail_profile(
    gfx::vertex_array &Right, gfx::vertex_array &Left) {
    auto const fHTW{0.5f * std::abs(fTrackWidth)};

    float roll1{0.f}, roll2{0.f};

    if (Segment) {
        Segment->GetRolls(roll1, roll2);
    }

    float const sin1{std::sin(roll1)}, cos1{std::cos(roll1)},
        sin2{std::sin(roll2)}, cos2{std::cos(roll2)};

    auto const &railprofile{track_rail_profile(m_profile1.second)};
    // NOTE: rail profile defines both regular rail and switch blade profiles,
    // so we halve total point count
    ...
    for (int i = 0; i < pointcount; ++i) {
        Right[i] = {
            ...
            {szyna[i].normal.x * cos1 + szyna[i].normal.y * sin1,
             ...
             szyna[i].normal.z},
            // texture
            {szyna[i].texture.s, szyna[i].texture.t}};
...
void TTrack::create_track_blade_profile(
    gfx::vertex_array &Right, gfx::vertex_array &Left) {
    auto const fHTW{0.5f * std::abs(fTrackWidth)};
    float const fHTW2{
        ((iTrapezoid & 2) != 0
             ?  // ten bit oznacza, że istnieje odpowiednie pNext
             0.5f * std::fabs(trNext->fTrackWidth)
             :  // połowa rozstawu/nawierzchni
             fHTW)};
    ...
    for (int i = 0; i < pointcount; ++i) {
        Right[i] = {
            {+(fHTW + iglica[i].position.x) * cos1 +
                 iglica[i].position.y * sin1,
             -(fHTW + iglica[i].position.x) * sin1 +
                 iglica[i].position.y * cos1,
             0.f},
            {iglica[i].normal},
        ...
        Left[pointcount * 2 - 1 - i] = {
            {(-fHTW2 - szyna[i].position.x) * cos2 + szyna[i].position.y * sin2,
             -(-fHTW2 - szyna[i].position.x) * sin2 +
                 szyna[i].position.y * cos2,
             0.f},
...
void TTrack::create_track_bed_profile(
    gfx::vertex_array &Output, TTrack const *Previous, TTrack const *Next) {
    // geometry parameters
    auto *profilesource =
    ...
    auto const fHTW{0.5f * std::abs(fTrackWidth)};
    auto const side{std::abs(
        texwidth)};  // szerokść podsypki na zewnątrz szyny albo pobocza
    auto const slop{std::abs(texslope)};  // brzeg zewnętrzny
    auto const rozp{fHTW + side + slop};  // brzeg zewnętrzny

    auto hypot1{std::hypot(
        slop, texheight1)};  // rozmiar pochylenia do liczenia normalnych
    if (hypot1 == 0.f) hypot1 = 1.f;

    glm::vec3 normal1{
        texheight1 / hypot1, texslope / hypot1, 0.f};  // wektor normalny
    ...
    if ((Next != nullptr) && (Next->eType != tt_Switch) &&
        ((iTrapezoid & 2)  // ten bit oznacza, że istnieje odpowiednie pNext
         || (eType == tt_Switch))) {
        fHTW2 =
            0.5f * std::abs(Next->fTrackWidth);  // połowa rozstawu/nawierzchni
    ...
    auto const railheight{
        std::abs(track_rail_profile(m_profile1.second).front().position.y)};
    if (texturelength == 4.f) {
        // stare mapowanie z różną gęstością pikseli i oddzielnymi teksturami na
        // każdy profil
        auto const normalx = std::cos(glm::radians(75.f));
        auto const normaly = std::sin(glm::radians(75.f));
        if (transition) {
            // trapez albo przechyłki
            // ewentualnie poprawić mapowanie, żeby środek mapował się
            // na 1.435/4.671 ((0.3464,0.6536) bo się tekstury podsypki
            // rozjeżdżają po zmianie proporcji profilu
            Output[0] = {
            ...
            Output[2] = {
                ...
                {0.5f, 0.f}};  // middle
            Output[3] = {
                {-Output[1].position.x, +(fHTW + side) * sin1 - railheight,
                 0.f},
                {-normalx, normaly, 0.f},
            ...
            Output[5] = {
                ...
                {0.00f, 0.f}};  // lewy brzeg
            Output[6] = {
                {(fHTW2 + side2) * cos2, -(fHTW2 + side2) * sin2 - railheight,
                 0.f},
                {normalx, normaly, 0.f},
                {0.33f, 0.f}};  // krawędź załamania
            Output[7] = {
                ...
                {0.5f, 0.f}};  // middle
            Output[8] = {
                {-Output[6].position.x, +(fHTW2 + side2) * sin2 - railheight,
                 0.f},
                {-normalx, normaly, 0.f},
                {0.67f, 0.f}};  // prawy brzeg początku symetrycznie
            Output[9] = {
        ...
        // fTexLength
        auto const max =
            fTexRatio2 * texturelength;  // szerokość proporcjonalna do długości
        auto const map11 =
            max > 0.f ? (fHTW + side) / max : 0.25f;  // załamanie od strony 1
        auto const map12 = max > 0.f ? (fHTW + side + hypot1) / max
                                     : 0.5f;  // brzeg od strony 1
        if (transition) {
            // trapez albo przechyłki
            auto const map21 = max > 0.f ? (fHTW2 + side2) / max
                                         : 0.25f;  // załamanie od strony 2
            auto const map22 = max > 0.f ? (fHTW2 + side2 + hypot2) / max
                                         : 0.5f;  // brzeg od strony 2
            // ewentualnie poprawić mapowanie, żeby środek mapował się
            // na 1.435/4.671
            // ((0.3464,0.6536)
...
void TTrack::create_road_profile(
    gfx::vertex_array &Output, bool const Forcetransition) {
    auto const fHTW{0.5f * std::abs(fTrackWidth)};
    float const fHTW2{
        ((iTrapezoid & 2) != 0
             ?  // ten bit oznacza, że istnieje odpowiednie pNext
    ...
    auto const max =
        fTexRatio1 *
        texturelength;  // test: szerokość proporcjonalna do długości
    auto const map1 =
        max > 0.f ? fHTW / max : 0.5f;  // obcięcie tekstury od strony 1
    auto const map2 =
        max > 0.f ? fHTW2 / max : 0.5f;  // obcięcie tekstury od strony 2
    ...
    if (transition) {
        ...
        Output[0] = {{fHTW, 0.f, 0.f}, normalup, {0.5f - map1, 0.f}};
        Output[1] = {{-fHTW, 0.f, 0.f}, normalup, {0.5f + map1, 0.f}};
    }
}

void TTrack::create_road_side_profile(
    gfx::vertex_array &Right, gfx::vertex_array &Left,
    gfx::vertex_array const &Road, bool const Forcetransition) {
    auto const fHTW{0.5f * std::abs(fTrackWidth)};
    auto const side{std::abs(
        fTexWidth)};  // szerokść podsypki na zewnątrz szyny albo pobocza
    ...
    if (iTrapezoid & 2) {
        ...
        // Ra: jest OK
        fHTW2 = 0.5f *
                std::fabs(trNext->fTrackWidth);  // połowa rozstawu/nawierzchni
    ...
    if (fTexHeight1 >=
        0.f) {  // standardowo: od zewnątrz pochylenie, a od wewnątrz poziomo
        Right[0] = {
    ...
    } else {  // wersja dla chodnika: skos 1:3.75, każdy chodnik innej
              // szerokości
        // mapowanie propocjonalne do szerokości chodnika
        // krawężnik jest mapowany od 31/64 do 32/64 lewy i od 32/64 do 33/64
        // prawy
        auto const d =
            -fTexHeight1 /
            3.75f;  // krawężnik o wysokości 150mm jest pochylony 40mm
        auto const max =
            fTexRatio2 *
            texturelength;  // test: szerokość proporcjonalna do długości
        auto const map1l =
        ...
        if (transition) {
            ...
            auto const map2l =
                (max > 0.f ? side2 / max : 0.484375f);  // obcięcie tekstury od
                                                        // lewej strony punktu 2
            auto const map2r =
                (max > 0.f ? slop2 / max
                           : 0.484375f);  // obcięcie tekstury od prawej strony
                                          // punktu 2
...
void TTrack::create_switch_trackbed(gfx::vertex_array &Output) {
    // try to get trackbed material from a regular track connected to the
    // primary path
    ...
    // main trackbed
    create_track_bed_profile(
        trackbedprofile, SwitchExtension->pPrevs[0],
        SwitchExtension->pNexts[0]);
    SwitchExtension->Segments[0]->RenderLoft(
        trackbedvertices1, m_origin, trackbedprofile, true, texturelength);
    // side trackbed
    create_track_bed_profile(
        trackbedprofile, SwitchExtension->pPrevs[1],
        SwitchExtension->pNexts[1]);
    SwitchExtension->Segments[1]->RenderLoft(
        trackbedvertices2, m_origin, trackbedprofile, true, texturelength);
...
material_handle TTrack::copy_adjacent_trackbed_material(TTrack const *Exclude) {
    if (iCategoryFlag != 1) {
        return null_handle;
    }  // tracks only

    auto &material{
        eType == tt_Switch ? SwitchExtension->m_material3 : m_material2};
    ...
    switch (eType) {
        case tt_Normal: {
            // for regular tracks don't set the trackbed texture if we aren't
            // sitting next to a part of a double slip
            /*
...
path_table::~path_table() { TIsolated::DeleteAll(); }
...
void path_table::InitTracks() {
...
#else
    // NOTE: legacy code peformed item operations last-to-first due to way the
    // items were added to the list this had impact in situations like two
    // possible connection candidates, where only the first one would be used
    for (auto first = std::rbegin(m_items); first != std::rend(m_items);
         ++first) {
        auto *track = *first;
#endif
        track->AssignEvents();
        ...
        switch (track->eType) {
            case tt_Table: {
                ...
                track->RaAssign(
                    instance, simulation::Events.FindEvent(trackname + ":done"),
                    simulation::Events.FindEvent(trackname + ":joined"));
                if (instance == nullptr) {
                    // jak nie ma modelu to pewnie jest wykolejnica, a ta jest
                    // domyślnie zamknięta i wykoleja
                    break;
                }
                // no break on purpose:
            ...
            case tt_Normal: {
                // tylko proste są podłączane do rozjazdów, stąd dwa rozjazdy
                // się nie połączą ze sobą
                if (track->CurrentPrev() == nullptr) {
                    // tylko jeśli jeszcze nie podłączony
                    std::tie(matchingtrack, connection) =
                        simulation::Region->find_path(
                            track->CurrentSegment()->FastGetPoint_0(), track);
                    switch (connection) {
                        case -1:  // Ra: pierwsza koncepcja zawijania samochodów
                                  // i statków
                            // if ((Track->iCategoryFlag&1)==0) //jeśli nie jest
                            // torem szynowym Track->ConnectPrevPrev(Track,0);
                            // //łączenie końca odcinka do samego siebie
                            break;
                        case 0:
                        ...
                        case 1:
                            ...
                            break;
                        case 2:
                            track->ConnectPrevPrev(
                                matchingtrack, 0);  // do Point1 pierwszego
                            matchingtrack->SetConnections(
                                0);  // zapamiętanie ustawień w Segmencie
                            break;
                        case 3:
                            track->ConnectPrevNext(
                                matchingtrack, 1);  // do Point2 pierwszego
                            matchingtrack->SetConnections(
                                0);  // zapamiętanie ustawień w Segmencie
                            break;
                        case 4:
                            ...
                            track->ConnectPrevPrev(
                                matchingtrack, 2);  // do Point1 drugiego
                            matchingtrack->SetConnections(
                                1);  // robi też Switch(0)
                        ...
                        case 5:
                            matchingtrack->Switch(1);
                            track->ConnectPrevNext(
                                matchingtrack, 3);  // do Point2 drugiego
                            matchingtrack->SetConnections(
                                1);  // robi też Switch(0)
                            matchingtrack->Switch(0);
                            break;
                ...
                if (track->CurrentNext() == nullptr) {
                    ...
                    std::tie(matchingtrack, connection) =
                        simulation::Region->find_path(
                            track->CurrentSegment()->FastGetPoint_1(), track);
                    switch (connection) {
                        case -1:  // Ra: pierwsza koncepcja zawijania samochodów
                                  // i statków
                            // if ((Track->iCategoryFlag&1)==0) //jeśli nie jest
                            // torem szynowym Track->ConnectNextNext(Track,1);
                            // //łączenie końca odcinka do samego siebie
                            break;
                        case 0:
...

        {
            ...
            if (false == inputcellname.empty()) {
                track->m_friction.second =
                    simulation::Memory.find(inputcellname);
                if (track->m_friction.second == nullptr) {
                    ErrorLog(
                        "Bad track: " +
                        (trackname.empty() ? "unnamed track"
                                           : "\"" + trackname + "\"") +
                        " can't find assigned memory cell \"" + inputcellname +
                        "\"");
                }
            }
        }
    ...
    if (Global.CreateSwitchTrackbeds) {
        // do this after all connections are established, otherwise missing
        // switch connections may prevent us from obtaining texture data for
        // basic track from 'across' a switch
        for (auto *track : m_items) {
            // try to assign missing trackbed materials for switches and tracks
    ...
    while (isolated) {
        ...
        auto *memorycell = simulation::Memory.find(
            isolated->asName);  // czy jest komóka o odpowiedniej nazwie
        if (memorycell == nullptr) {
            // utworzenie automatycznej komórki
            // TODO: determine suitable location for this one, create and add
            // world reference node
            scene::node_data nodedata;
            nodedata.name = isolated->asName;
            memorycell =
                new TMemCell(nodedata);  // to nie musi mieć nazwy, nazwa w
                                         // wyszukiwarce wystarczy
            // NOTE: autogenerated cells aren't exported; they'll be
            // autogenerated anew when exported file is loaded
...
void path_table::IsolatedBusy(std::string const &Name) const {
    ...
    multiplayer::WyslijString(
        Name, 10);  // wolny (technically not found but, eh)
}
