...
class buffer_manager {
   public:
    // constructors
    buffer_manager() {
        m_buffers.emplace_back(openal_buffer());
    }  // empty bindings for null buffer
       // destructor
    ~buffer_manager();
    // methods
    // creates buffer object out of data stored in specified file. returns:
    // handle to the buffer or null_handle if creation failed
    buffer_handle create(std::string const &Filename);
   ...
   private:
    ...
    buffer_handle emplace(std::string Filename);
    // checks whether specified buffer is in the buffer bank. returns: buffer
    // handle, or null_handle.
    buffer_handle find_buffer(std::string const &Buffername) const;
