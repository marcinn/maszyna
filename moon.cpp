...
cMoon::cMoon() {
    setLocation(
        19.00f, 52.00f);        // default location roughly in centre of Poland
...
void cMoon::init() {
    ...
    // NOTE: we're calculating phase just once, because it's unlikely simulation
    // will last a few days, plus a sudden texture change would be pretty
    // jarring
    phase();
}

void cMoon::update(bool const Includephase) {
    ...
    position = glm::rotateX(
        position, glm::radians(static_cast<float>(m_body.elevref)));
    position =
        glm::rotateY(position, glm::radians(static_cast<float>(-m_body.hrang)));
...
glm::vec3 cMoon::getDirection() { return m_position; }

float cMoon::getAngle() const { return (float)m_body.elevref; }
...
float cMoon::getIntensity() {
    ...
    auto const phasefactor =
        1.0f - std::abs(m_phase - 29.53f * 0.5f) / (29.53f * 0.5f);
    return static_cast<float>(
        (m_body.etr / 1399.0) * phasefactor *
        0.15);  // arbitrary scaling factor taken from etrn value
}

void cMoon::setLocation(float const Longitude, float const Latitude) {
    // convert fraction from geographical base of 6o minutes
    m_observer.longitude =
        (int)Longitude + (Longitude - (int)(Longitude)) * 100.0 / 60.0;
    m_observer.latitude =
        (int)Latitude + (Latitude - (int)(Latitude)) * 100.0 / 60.0;
}
...
void cMoon::setPressure(float const Pressure) { m_observer.press = Pressure; }
...
void cMoon::move() {
    ...
    if (m_observer.hour >= 0) {
        localtime.wHour = m_observer.hour;
    }
    if (m_observer.minute >= 0) {
        localtime.wMinute = m_observer.minute;
    }
    if (m_observer.second >= 0) {
        localtime.wSecond = m_observer.second;
    }
    ...
    double daynumber = 367 * localtime.wYear -
                       ...
                       275 * localtime.wMonth / 9 + localtime.wDay - 730530 +
                       (localut / 24.0);
    ...
    // moon parameters
    double longascnode =
        clamp_circular(125.1228 - 0.0529538083 * daynumber);  // N, degrees
    double const inclination = 5.1454;                        // i, degrees
    double const mndistance = 60.2666;  // a, in earth radii
    // argument of perigee
    double const perigeearg =
        clamp_circular(318.0634 + 0.1643573223 * daynumber);  // w, degrees
    // mean anomaly
    m_body.mnanom =
        clamp_circular(115.3654 + 13.0649929509 * daynumber);  // M, degrees
    // eccentricity
    double const e = 0.054900;
    // eccentric anomaly
    double E0 =
        m_body.mnanom + radtodeg * e * std::sin(degtorad * m_body.mnanom) *
                            (1.0 + e * std::cos(degtorad * m_body.mnanom));
    double E1 =
        E0 - (E0 - radtodeg * e * std::sin(degtorad * E0) - m_body.mnanom) /
                 (1.0 - e * std::cos(degtorad * E0));
    while (std::abs(E0 - E1) >
           0.005) {  // arbitrary precision tolerance threshold
        E0 = E1;
        E1 =
            E0 - (E0 - radtodeg * e * std::sin(degtorad * E0) - m_body.mnanom) /
                     (1.0 - e * std::cos(degtorad * E0));
    ...
    double const yv =
        mndistance * std::sin(degtorad * E) * std::sqrt(1.0 - e * e);
    // distance
    ...
    double const xeclip =
        m_body.distance *
        (std::cos(degtorad * longascnode) * std::cos(vpluswinrad) -
         std::sin(degtorad * longascnode) * std::sin(vpluswinrad) *
             std::cos(degtorad * inclination));
    double const yeclip =
        m_body.distance *
        (std::sin(degtorad * longascnode) * std::cos(vpluswinrad) +
         std::cos(degtorad * longascnode) * std::sin(vpluswinrad) *
             std::cos(degtorad * inclination));
    double const zeclip = m_body.distance * std::sin(vpluswinrad) *
                          std::sin(degtorad * inclination);
    // ecliptic coordinates
    double ecliplat =
        radtodeg *
        std::atan2(zeclip, std::sqrt(xeclip * xeclip + yeclip * yeclip));
    m_body.eclong = clamp_circular(radtodeg * std::atan2(yeclip, xeclip));
    // distance
    m_body.distance =
        std::sqrt(xeclip * xeclip + yeclip * yeclip + zeclip * zeclip);
    // perturbations
    // NOTE: perturbation calculation can be safely disabled if we don't mind
    // error of 1-2 degrees Sun's  mean anomaly:          Ms     (already
    // computed)
    double const sunmnanom =
        clamp_circular(356.0470 + 0.9856002585 * daynumber);  // M
    // Sun's  mean longitude:        Ls     (already computed)
    double const sunphlong = clamp_circular(282.9404 + 4.70935e-5 * daynumber);
    double const sunmnlong =
        clamp_circular(sunphlong + sunmnanom);  // L = w + M
    ...
    double const pertevection =
        -1.274 *
        std::sin(degtorad * (m_body.mnanom - 2.0 * mnelong));  // Evection
    double const pertvariation =
        +0.658 * std::sin(degtorad * (2.0 * mnelong));  // Variation
    double const pertyearlyeqt =
        -0.186 * std::sin(degtorad * sunmnanom);  // Yearly equation
    // latitude perturbations
    double const pertlat =
        -0.173 * std::sin(degtorad * (arglat - 2.0 * mnelong));

    m_body.eclong += pertevection + pertvariation + pertyearlyeqt;
    ecliplat += pertlat;
    // declination
    m_body.declin = radtodeg * std::asin(
                                   std::sin(m_body.oblecl * degtorad) *
                                   std::sin(m_body.eclong * degtorad));

    // right ascension
    double top =
        std::cos(degtorad * m_body.oblecl) * std::sin(degtorad * m_body.eclong);
    ...
    if (m_body.hrang < -180.0)
        m_body.hrang += 360.0;  // (force it between -180 and 180 degrees)
    else if (m_body.hrang > 180.0)
        m_body.hrang -= 360.0;
        ...
        if (m_body.elevetr >= 5.0)
            ...
            refcor = 1735.0 +
                     m_body.elevetr *
                         (-518.2 +
                          m_body.elevetr *
                              (103.4 + m_body.elevetr *
                                           (-12.79 + m_body.elevetr * 0.711)));
        else
            refcor = -20.774 / tanelev;

        prestemp =
            (m_observer.press * 283.0) / (1013.0 * (273.0 + m_observer.temp));
...
void cMoon::irradiance() {
    ...
    double cd =
        cos(degtorad * m_body.dayang);  // cosine of the day angle or delination
...
void cMoon::phase() {
    ...
    if ((lt.wMonth == 5) && (lt.wDay == 4))  // May the forth be with you!
        ...
        float ip = normalize(
            (simulation::Time.julian_day() - 2451550.1f) / 29.530588853f);
        m_phase = ip * 29.53f;
    }
}

// normalize values to range 0...1
float cMoon::normalize(const float Value) const {
