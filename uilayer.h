...
// GuiLayer -- basic user interface class. draws requested information on top of
// openGL screen
...
class ui_layer {
   public:
    ...
    static void set_unit(GLint const Textureunit) {
        m_textureunit = GL_TEXTURE0 + Textureunit;
    }
    static void shutdown();
    // potentially processes provided input key. returns: true if the input was
    // processed, false otherwise
    virtual bool on_key(int const Key, int const Action);
    // potentially processes provided mouse movement. returns: true if the input
    // was processed, false otherwise
    virtual bool on_cursor_pos(double const Horizontal, double const Vertical);
    // potentially processes provided mouse button. returns: true if the input
    // was processed, false otherwise
    virtual bool on_mouse_button(int const Button, int const Action);
    ...
    // begins new UI frame
    // (this is separate from render() to allow for debug GUI outside of proper
    // UI framework)
    void begin_ui_frame();
    static void begin_ui_frame_internal();
    //
    static void set_cursor(int const Mode);
    // stores operation progress
    void set_progress(
        float const Progress = 0.0f, float const Subtaskprogress = 0.0f);
   ...
   private:
    ...
    // draws a quad between coordinates x,y and z,w with uv-coordinates spanning
    // 0-1
    ...
    float m_progress{0.0f};  // percentage of filled progres bar, to indicate
                             // lengthy operations.
    float m_subtaskprogress{0.0f};  // percentage of filled progres bar, to
                                    // indicate lengthy operations.
    std::string m_progresstext;     // label placed over the progress bar

    texture_handle m_background{
        null_handle};  // path to texture used as the background. size depends
                       // on mAspect.
