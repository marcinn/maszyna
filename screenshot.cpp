...
void screenshot_manager::screenshot_save_thread(char *img, int w, int h) {
    ...
    if (Global.gfx_usegles) {
        ...
        for (int y = 0; y < h; y++)
            for (int x = 0; x < w; x++) img[(y * w + x) * 4 + 3] = 0xFF;
    } else {
    ...
    if (png_image_write_to_file(
            &png, filename.c_str(), 0, img, stride, nullptr) == 1)
...
void screenshot_manager::make_screenshot() {
    ...
    glReadPixels(
        0, 0, Global.fb_size.x, Global.fb_size.y,
        Global.gfx_usegles ? GL_RGBA : GL_RGB, GL_UNSIGNED_BYTE, (GLvoid *)img);
    // m7t: use pbo

    std::thread t(
        screenshot_save_thread, img, Global.fb_size.x, Global.fb_size.y);
    t.detach();
}
