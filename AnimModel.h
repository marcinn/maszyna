...
class TAnimVocaloidFrame {  // ramka animacji typu Vocaloid Motion Data z
...
class TAnimContainer
    : std::enable_shared_from_this<
          TAnimContainer> {  // opakowanie submodelu, określające animację
                             // egzemplarza - obsługiwane jako lista
    friend TAnimModel;
   ...
   private:
    ...
    //+0x100: pierwszy stopień IK - obrócić w stronę pierwszego potomnego
    //(dziecka) +0x200: drugi stopień IK - dostosować do pozycji potomnego
    //potomnego (wnuka)
    basic_event *evDone;  // ewent wykonywany po zakończeniu animacji, np.
                          // zapór, obrotnicy
   public:
    // wyświetlania
    TAnimContainer();
    ...
    inline void WillBeAnimated() {
        if (pSubModel) pSubModel->WillBeAnimated();
    };
    void EventAssign(basic_event *ev);
    inline basic_event *Event() { return evDone; };
};
...
class TAnimModel : public scene::basic_node {
   ...
   public:
    ...
    std::optional<std::tuple<float, float, std::optional<glm::vec3>>> LightGet(
        int const n);
    int TerrainCount();
    TSubModel *TerrainSquare(int n);
    int Flags();
    inline material_data const *Material() const { return &m_materialdata; }
    inline TModel3d *Model() const { return pModel; }
    inline void Angles(glm::vec3 const &Angles) { vAngle = Angles; }
    inline glm::vec3 Angles() const { return vAngle; }
    // members
    std::list<std::shared_ptr<TAnimContainer>> m_animlist;

    // lista animacji z eventem, które muszą być przeliczane również bez
    // wyświetlania
   ...
   private:
    ...
    void RaAnimate(
        unsigned int const Framestamp);  // przeliczenie animacji egzemplarza
    ...
    // members
    std::shared_ptr<TAnimContainer>
        pRoot;  // pojemniki sterujące, tylko dla aniomowanych submodeli
    ...
    int iNumLights{0};
    std::array<TSubModel *, iMaxNumLights>
        LightsOn{};  // Ra: te wskaźniki powinny być w ramach TModel3d
    ...
    float fOffTime{1.f / 2};  // { 60.f / 45.f / 2 }; // były stałymi, teraz
                              // mogą być zmienne dla każdego egzemplarza
