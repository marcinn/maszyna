...
void TGauge::Init(
    TSubModel *Submodel, TSubModel *Submodelon, TGaugeAnimation Type,
    float Scale, float Offset, float Friction, float Value,
    float const Endvalue, float const Endscale,
    bool const Interpolatescale) {  // ustawienie parametrów animacji submodelu
    SubModel = Submodel;
    ...
    auto const offset{model_offset()};
    {
        std::vector<sound_source *> soundfxs = {
            &m_soundfxincrease, &m_soundfxdecrease, &m_soundfxon,
            &m_soundfxoff};
...
void TGauge::Load(
    cParser &Parser, TDynamicObject const *Owner, double const mul) {
    ...
    if (Parser.peek() != "{") {
        ...
        Parser >> scale >> offset >> friction;
        if ((gaugetypename == "rotvar") || (gaugetypename == "movvar")) {
            interpolatescale = true;
            Parser.getTokens(2, false);
            Parser >> endvalue >> endscale;
        }
    } else {
        ...
        // new, variable length section
        {
            ...
            // set provided custom soundproofing to assigned sounds (for sounds
            // without their own custom soundproofing)
            if (scratchpad.soundproofing) {
                if (!m_soundfxincrease.soundproofing()) {
                    m_soundfxincrease.soundproofing() =
                        scratchpad.soundproofing;
                }
                if (!m_soundfxdecrease.soundproofing()) {
                    m_soundfxdecrease.soundproofing() =
                        scratchpad.soundproofing;
                }
                ...
                for (auto &soundfxrecord : m_soundfxvalues) {
                    if (!soundfxrecord.second.soundproofing()) {
                        soundfxrecord.second.soundproofing() =
                            scratchpad.soundproofing;
                    }
                }
            }
    ...
    for (auto const *source : sources) {
        ...
        if ((source != nullptr) && (submodel = source->GetFromName(
                                        submodelname + "_off")) != nullptr) {
            // got what we wanted, bail out
            break;
        }
    }
    if (submodel == nullptr) {
        ErrorLog(
            "Bad model: failed to locate sub-model \"" + submodelname +
                "\" in 3d model(s) of \"" + Owner->name() + "\"",
            logtype::model);
    }
    // see if we can locate optional submodel for active state, with _on suffix
    TSubModel *submodelon{nullptr};
    for (auto const *source : sources) {
        if ((source != nullptr) && (submodelon = source->GetFromName(
                                        submodelname + "_on")) != nullptr) {
            // got what we wanted, bail out
            break;
        }
    }
    ...
    auto const type =
        (lookup != gaugetypes.end() ? lookup->second
                                    : TGaugeAnimation::gt_Unknown);

    Init(
        submodel, submodelon, type, scale, offset, friction, 0, endvalue,
        endscale, interpolatescale);

    //    return md2 != nullptr; // true, gdy podany model zewnętrzny, a w
    //    kabinie nie było
};

bool TGauge::Load_mapping(cParser &Input, TGauge::scratch_data &Scratchpad) {
    ...
    if (key == "type:") {
        ...
        // sounds assigned to specific gauge values, defined by key soundX:
        // where X = value
        ...
        if (indexstart != std::string::npos) {
            m_soundfxvalues.emplace(
                ...
                sound_source(m_soundtemplate)
                    .deserialize(Input, sound_type::single));
        }
    }
...
// ustawienie wartości docelowej. plays provided fallback sound, if no sound was
// defined in the control itself
bool TGauge::UpdateValue(
    float fNewDesired, std::optional<sound_source> &Fallbacksound) {
...
bool TGauge::UpdateValue(float fNewDesired) {
    auto const desiredtimes100 =
        static_cast<int>(std::round(100.0 * fNewDesired));
    if (desiredtimes100 ==
        static_cast<int>(std::round(100.0 * m_targetvalue))) {
        return true;
    }
    ...
    if (fullinteger) {
        ...
        // toggle the control to continous range/exclusive sound mode from now
        // on
        m_soundtype = sound_flags::exclusive;
    }
    // ...and if there isn't any, fall back on the basic set...
    auto const currentvalue = GetValue();
    // HACK: crude way to discern controls with continuous and quantized value
    // range
...
void TGauge::PutValue(
    float fNewDesired) {  // McZapkie-281102: natychmiastowe wpisanie wartosci
    m_targetvalue = fNewDesired;
    m_value = m_targetvalue;
};

float TGauge::GetValue() const {
    // we feed value in range 0-1 so we should be getting it reported in the
    // same range
    return m_value;
}
...
void TGauge::Update(bool const Power) {
    ...
    if (m_value != m_targetvalue) {
        ...
        if ((m_friction > 0) && (dt < 0.5 * m_friction)) {
            // McZapkie-281102: zabezpieczenie przed oscylacjami dla dlugich
            // czasow
            ...
            if (std::abs(m_targetvalue - m_value) <= 0.0001) {
                ...
                m_value = m_targetvalue;  // set it exactly as requested just in
                                          // case it matters
            }
        } else {
            m_value = m_targetvalue;
...
void TGauge::UpdateValue() {  // ustawienie wartości docelowej z parametru
    switch (m_datatype) {  // to nie jest zbyt optymalne, można by zrobić osobne
                           // funkcje
        case 'f': {
            UpdateValue(*fData);
            break;
...
void TGauge::AssignState(bool const *State) { m_stateinput = State; }
...
float TGauge::GetScaledValue() const {
    return (
        (false == m_interpolatescale)
            ...
            : m_value * interpolate(
                            m_scale, m_endscale,
                            clamp(m_value / m_endvalue, 0.f, 1.f)) +
                  m_offset);
}

void TGauge::UpdateAnimation(TSubModel *Submodel) {
    if (Submodel == nullptr) {
        return;
    }
    ...
    switch (m_animation) {
        ...
        case TGaugeAnimation::gt_Wiper: {
            ...
            if (sm) {
                ...
                sm = sm->ChildGet();
                if (sm) sm->SetRotate(float3(0, 1, 0), scaledvalue * 360.0);
            }
            break;
        }
        case TGaugeAnimation::gt_Digital: {
            ...
            auto *sm = Submodel->ChildGet();
            /*  			std::string n = FormatFloat( "0000000000", floor( fValue )
             * ); // na razie tak trochę bez sensu
             */
            std::string n(
                "000000000" +
                std::to_string(static_cast<int>(std::floor(GetScaledValue()))));
            if (n.length() > 10) {
                n.erase(0, n.length() - 10);
            }     // also dumb but should work for now
            do {  // pętla po submodelach potomnych i obracanie ich o kąt zależy
                  // od cyfry w (fValue)
                if ((sm->pName.size()) && (std::isdigit(sm->pName[0]))) {
                    sm->SetRotate(
                        float3(0, 1, 0),
...
glm::vec3 TGauge::model_offset() const {
    return (SubModel != nullptr ? SubModel->offset(1.f) : glm::vec3());
}

TGaugeType TGauge::type() const { return m_type; }
//---------------------------------------------------------------------------
