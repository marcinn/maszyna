...
bool gamepad_input::init() {
    m_inputaxes.clear();
    m_inputbuttons.clear();
    // NOTE: we're only checking for joystick_1 and rely for it to stay
    // connected throughout. not exactly flexible, but for quick hack it'll do
...
void gamepad_input::bind(
    std::vector<std::reference_wrapper<user_command> > &Targets, cParser &Input,
    std::unordered_map<std::string, user_command> const &Translator,
    ...
    for (auto &bindingtarget : Targets) {
        ...
        if (true == bindingcommandname.empty()) {
            ...
            WriteLog(
                "Gamepad binding for " + Point +
                " didn't specify associated command(s)");
            // ...can't quit outright though, as provided references are likely
            // to be unitialized
...
bool gamepad_input::recall_bindings() {
    ...
    while (true == bindingparser.getTokens(1, true, "\n\r")) {
        ...
        if (splitbindingpoint.first == "axis") {
            ...
            // input types:
            // -- range commandname IDX; axis value is passed as paramIDX of
            // commandname
            // -- 3state commandname commandname; positive axis value issues
            // first commandname, negative value issues second commandname

            auto const axisindex{splitbindingpoint.second};
            // sanity check, connected gamepad isn't guaranteed to have that
            // many axes
            if (axisindex >= m_inputaxes.size()) {
                continue;
            }

            int controlmode{-1};  // unless stated otherwise the set will be
                                  // associated with the default control mode
            ...
            while (true == entryparser.getTokens(1, true, "\n\r\t ")) {
                ...
                if (typelookup == nametotypemap.end()) {
                    WriteLog(
                        "Gamepad binding for " + bindingpoint +
                        " specified unknown control type, \"" +
                        bindingtypename + "\"");
                } else {
                    std::vector<std::reference_wrapper<user_command> >
                        bindingtargets;
                    auto const bindingtype{typelookup->second};
                    std::get<0>(m_inputaxes[axisindex].bindings[controlmode]) =
                        bindingtype;
                    // retrieve regular commands associated with the axis and
                    // mode
                    switch (bindingtype) {
                        case input_type::value:
                        case input_type::value_invert: {
                            bindingtargets.emplace_back(std::ref(std::get<1>(
                                m_inputaxes[axisindex].bindings[controlmode])));
                            break;
                        }
                        case input_type::threestate: {
                            bindingtargets.emplace_back(std::ref(std::get<1>(
                                m_inputaxes[axisindex].bindings[controlmode])));
                            bindingtargets.emplace_back(std::ref(std::get<2>(
                                m_inputaxes[axisindex].bindings[controlmode])));
                            break;
                        }
                        default: {
                    ...
                    bind(
                        bindingtargets, entryparser, nametocommandmap,
                        bindingpoint);
                    // handle potential remaining input type-specific parameters
                    switch (bindingtype) {
                        case input_type::value:
                        case input_type::value_invert: {
                            auto const paramidxname{
                            ...
                            std::get<2>(
                                m_inputaxes[axisindex].bindings[controlmode]) =
                                static_cast<user_command>(paramidx);
                            break;
                        }
                        default: {
                            break;
            ...
            // sanity check, connected gamepad isn't guaranteed to have that
            // many buttons
            if (buttonindex >= m_inputbuttons.size()) {
                continue;
            }
            ...
            if (bindingtype == "mode") {
                ...
                if (true == entryparser.getTokens(1, true, "\n\r\t ")) {
                    ...
                    WriteLog(
                        "Gamepad binding for " + bindingpoint +
                        " didn't specify mode index");
                }
            } else {
                // regular button, single bound command
                std::vector<std::reference_wrapper<user_command> >
                    bindingtargets;
                bindingtargets.emplace_back(
                    std::ref(m_inputbuttons[buttonindex].binding));
                bind(
                    bindingtargets, entryparser, nametocommandmap,
                    bindingpoint);
            }
...
void gamepad_input::on_button(int const Button, int const Action) {
...
void gamepad_input::process_axes() {
    input_type inputtype;
    user_command boundcommand1, boundcommand2;
    auto binding{std::tie(inputtype, boundcommand1, boundcommand2)};

    // since some commands can potentially collect values from two different
    // axes we can't post them directly instead, we use a small scratchpad to
    // first put these commands together, then post them in their completed
    // state
    std::unordered_map<user_command, std::tuple<double, double, int> > commands;
    // HACK: generate movement reset, it'll be eiter overriden by actual
    // movement command, or issued if another control mode was activated
    ...
    for (auto &axis : m_inputaxes) {
        ...
        switch (inputtype) {
            case input_type::threestate: {
                // TODO: separate multiplier for each mode, to allow different,
                // customizable sensitivity for each control
                auto const deltatime{Timer::GetDeltaTime() * 15.0};
                if (axis.state >= 0.0f) {
                    // first bound command selected
                    if (axis.accumulator < 0.0f) {
                        // we were issuing the other command, post notification
                        // that's no longer the case
                        if (boundcommand2 != user_command::none) {
                            m_relay.post(boundcommand2, 0, 0, GLFW_RELEASE, 0);
                            axis.accumulator = 0.0f;
                        }
                    }
                    if (boundcommand1 != user_command::none) {
                        if (axis.state > m_deadzone) {
                            axis.accumulator += (axis.state - m_deadzone) /
                                                (1.0 - m_deadzone) * deltatime;
                            // we're making sure there's always a positive
                            // charge left in the accumulator, to more reliably
                            // decect when the stick goes from active to dead
                            // zone, below
                            while (axis.accumulator > 1.0f) {
                                // send commands if the accumulator(s) was
                                // filled
                                m_relay.post(
                                    boundcommand1, 0, 0, GLFW_PRESS, 0);
                                axis.accumulator -= 1.0f;
                            }
                        } else {
                            // if the accumulator isn't empty it's an indicator
                            // the stick moved from active to neutral zone
                            // indicate it with proper RELEASE command
                            m_relay.post(boundcommand1, 0, 0, GLFW_RELEASE, 0);
                            axis.accumulator = 0.0f;
                        }
                    }
                } else {
                    ...
                    if (boundcommand1 != user_command::none) {
                        if (axis.state < -m_deadzone) {
                            ...
                            while (axis.accumulator < -1.0f) {
                                ...
                                // filled
                                m_relay.post(
                                    boundcommand2, 0, 0, GLFW_PRESS, 0);
                                axis.accumulator += 1.0f;
                            }
                        } else {
                            // if the accumulator isn't empty it's an indicator
            ...
            case input_type::value_invert: {
                ...
                auto &param{
                    (static_cast<int>(boundcommand2) == 0
                         ?  // for this type boundcommand2 stores param index
                         std::get<0>(command)
                         : std::get<1>(command))};
                param = axis.state;
                ...
                switch (boundcommand1) {
                    ...
                    default: {
                        // commands generally expect their parameter to be in
                        // 0:1 range
                        param = param * 0.5 + 0.5;
                        break;
                    }
    ...
    for (auto const &command : commands) {
        ...
        if ((param1 != 0.0) || (std::get<0>(lastparams) != 0.0) ||
            ...
            m_relay.post(
                command.first, param1, param2, std::get<2>(command.second),
                // as we haven't yet implemented either item id system or
                // multiplayer, the 'local' controlled vehicle and entity have
                // temporary ids of 0
                // TODO: pass correct entity id once the missing systems are in
