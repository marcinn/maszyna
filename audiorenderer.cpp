...
float const EU07_SOUND_CUTOFFRANGE{
    3000.f};  // 2750 m = max expected emitter spawn range, plus safety margin
float const EU07_SOUND_VELOCITYLIMIT{
    250 / 3.6f};  // 343 m/sec ~= speed of sound; arbitrary limit of 250 km/h

// potentially clamps length of provided vector to 343 meters
// TBD: make a generic method for utilities out of this
glm::vec3 limit_velocity(glm::vec3 const &Velocity) {
...
void openal_source::play() {
    if (id == audio::null_resource) {
        return;
    }  // no implementation-side source to match, no point
...
void openal_source::stop() {
    ...
    loop(false);
    // NOTE: workaround for potential edge cases where ::alSourceStop() doesn't
    // set source which wasn't yet started to AL_STOPPED
    int state;
    ::alGetSourcei(id, AL_SOURCE_STATE, &state);
    if (state == AL_INITIAL) {
        play();
...
void openal_source::update(
    double const Deltatime, glm::vec3 const &Listenervelocity) {
    update_deltatime = Deltatime;  // cached for time-based processing of data
                                   // from the controller
    if (sound_range < 0.0) {
        sound_velocity =
            Listenervelocity;  // cached for doppler shift calculation
    }
    /*
        // HACK: if the application gets stuck for long time loading assets the
       audio can gone awry.
    ...
    */
    if (id != audio::null_resource) {
        ...
        // for multipart sounds trim away processed buffers until only one
        // remains, the last one may be set to looping by the controller TBD,
        // TODO: instead of change flag move processed buffer ids to separate
        // queue, for accurate tracking of longer buffer sequences
        ALuint discard;
        while ((sound_index > 0) && (sounds.size() > 1)) {
            ::alSourceUnqueueBuffers(id, 1, &discard);
            sounds.erase(std::begin(sounds));
            --sound_index;
            sound_change = true;
            // potentially adjust starting point of the last buffer (to reduce
            // chance of reverb effect with multiple, looping copies playing)
...
void openal_source::sync_with(sound_properties const &State) {
    if (id == audio::null_resource) {
        // no implementation-side source to match, return sync error so the
        // controller can clean up on its end
        sync = sync_state::bad_resource;
        return;
    }
    // velocity
    if ((update_deltatime > 0.0) && (sound_range >= 0) &&
        (properties.location != glm::dvec3())) {
        // after sound position was initialized we can start velocity
        // calculations
        sound_velocity = limit_velocity(
            (State.location - properties.location) / update_deltatime);
    }
    // NOTE: velocity at this point can be either listener velocity for global
    // sounds, actual sound velocity, or 0 if sound position is yet unknown
    ...
    if (sound_range != -1) {
        ...
        // NOTE: since we're comparing squared distances we can ignore that
        // sound range can be negative
        auto const cutoffrange =
            (is_multipart
                 ? EU07_SOUND_CUTOFFRANGE
                 :  // we keep multi-part sounds around longer, to minimize
                    // restarts as the sounds get out and back in range
                 sound_range * 7.5f);
        if (glm::length2(sound_distance) >
            std::min(
                (cutoffrange * cutoffrange),
                (EU07_SOUND_CUTOFFRANGE * EU07_SOUND_CUTOFFRANGE))) {
            stop();
            sync = sync_state::bad_distance;  // flag sync failure for the
                                              // controller
            return;
        }
    }
    if (sound_range >= 0) {
        ...
        // sounds with 'unlimited' or negative range are positioned on top of
        // the listener
        ::alSourcefv(id, AL_POSITION, glm::value_ptr(glm::vec3()));
    }
    // gain
    auto const gain{
        State.gain * State.soundproofing *
        (State.category == sound_category::vehicle ? Global.VehicleVolume
         : State.category == sound_category::local
             ? Global.EnvironmentPositionalVolume
         : State.category == sound_category::ambient
             ? Global.EnvironmentAmbientVolume
             : 1.f)};
    if ((State.gain != properties.gain) ||
        ...
        auto const range{
            (sound_range >= 0 ? sound_range
                              : 5)};  // range of -1 means sound of unlimited
                                      // range, positioned at the listener
        ::alSourcef(
            id, AL_REFERENCE_DISTANCE,
            range * (1.f / 16.f) * State.soundproofing);
    }
    if (sound_range != -1) {
        auto const rangesquared{sound_range * sound_range};
        auto const distancesquared{glm::length2(sound_distance)};
        if ((distancesquared > rangesquared) || (false == is_in_range)) {
            // if the emitter is outside of its nominal hearing range or was
            // outside of it during last check adjust the volume to a suitable
            // fraction of nominal value
            auto const fadedistance{sound_range * 0.75f};
            auto const rangefactor{interpolate(
                1.f, 0.f,
                clamp<float>(
    ...
    if (State.pitch != properties.pitch) {
        ...
        ::alSourcef(
            id, AL_PITCH, clamp(State.pitch * pitch_variation, 0.1f, 10.f));
    }
    // all synced up
    properties = State;
...
void openal_source::range(float const Range) {
    // NOTE: we cache actual specified range, as we'll be giving 'unlimited'
    // range special treatment
    sound_range = Range;
    ...
    auto const range{
        (Range >= 0 ? Range : 5)};  // range of -1 means sound of unlimited
                                    // range, positioned at the listener
...
void openal_source::pitch(float const Pitch) {
...
void openal_source::loop(bool const State) {
    if (id == audio::null_resource) {
        return;
    }  // no implementation-side source to match, no point
    if (is_looping == State) {
...
void openal_source::clear() {
    ...
    // ...and reset reset the properties, except for the id of the allocated
    // source NOTE: not strictly necessary since except for the id the source
    // data typically get discarded in next step
...
audio::buffer_handle openal_renderer::fetch_buffer(
    std::string const &Filename) {
    return m_buffers.create(Filename);
}

// provides direct access to a specified buffer
audio::openal_buffer const &openal_renderer::buffer(
    audio::buffer_handle const Buffer) const {
    return m_buffers.buffer(Buffer);
}

// initializes the service
bool openal_renderer::init() {
    if (true == m_ready) {
...
void openal_renderer::erase(sound_source const *Controller) {
    ...
    while (source != std::end(m_sources)) {
        if (source->controller == Controller) {
            ...
            if (source->id != audio::null_resource) {
                // keep around functional sources, but no point in doing it with
                // the above-the-limit ones
...
void openal_renderer::update(double const Deltatime) {
    ...
    if (Deltatime == 0.0) {
        if (alcDevicePauseSOFT) alcDevicePauseSOFT(m_device);
        return;
    }

    if (alcDeviceResumeSOFT) alcDeviceResumeSOFT(m_device);
    ...
    auto cameraposition =
        Global.pCamera.Pos + (Global.viewport_move * glm::mat3(cameramatrix));
    cameramatrix =
        glm::dmat4(glm::inverse(Global.viewport_rotate)) * cameramatrix;
    ...
    ::alListenerfv(
        AL_ORIENTATION, reinterpret_cast<ALfloat const *>(orientation));
    // velocity
    if (Deltatime > 0) {
        ...
        if ((simulation::Train != nullptr) &&
            (simulation::Train->iCabn != m_activecab)) {
        ...
        ::alListenerfv(
            AL_VELOCITY, reinterpret_cast<ALfloat const *>(
                             glm::value_ptr(m_listenervelocity)));
    }
    ...
    while (source != std::end(m_sources)) {
        ...
        // if after the update the source isn't playing, put it away on the
        // spare stack, it's done
...
audio::openal_source openal_renderer::fetch_source() {
    ...
    if (newsource.id == audio::null_resource) {
        alGetError();
        // if we still don't have a working source, see if we can sacrifice an
        // already active one under presumption it's more important to play new
        // sounds than keep the old ones going TBD, TODO: for better results we
        // could use range and/or position for the new sound to better weight
        // whether the new sound is really more important
        auto leastimportantsource{std::end(m_sources)};
        auto leastimportantweight{std::numeric_limits<float>::max()};

        for (auto source{std::begin(m_sources)}; source != std::cend(m_sources);
             ++source) {
            if ((source->id == audio::null_resource) ||
            ...
            auto const sourceweight{
                (source->sound_range != -1
                     ? (source->sound_range * source->sound_range) /
                           (glm::length2(source->sound_distance) + 1)
        ...
        if ((leastimportantsource != std::end(m_sources)) &&
            ...
            // only accept the candidate if it's outside of its nominal hearing
            // range
    ...
    if (newsource.id == audio::null_resource) {
        // for sources with functional emitter reset emitter parameters from
        // potential last use
...
bool openal_renderer::init_caps() {
    ...
    // NOTE: default value of audio renderer variable is empty string, meaning
    // argument of NULL i.e. 'preferred' device
    ...
    auto const oalversion{
        std::to_string(versionmajor) + "." + std::to_string(versionminor)};

    std::string al_renderer(
        (char *)::alcGetString(m_device, ALC_DEVICE_SPECIFIER));
    crashreport_add_info("openal_renderer", al_renderer);
    crashreport_add_info("openal_version", oalversion);

    WriteLog(
        "Audio Renderer: " + al_renderer + " OpenAL Version: " + oalversion);

    WriteLog(
        "Supported extensions: " +
        std::string{(char *)::alcGetString(m_device, ALC_EXTENSIONS)});

    ALCint attr[3] = {
        ALC_MONO_SOURCES, Global.audio_max_sources, 0};  // request more sounds
    ...
    if (alIsExtensionPresent("AL_SOFT_deferred_updates")) {
        ...
        alProcessUpdatesSOFT =
            (void (*)())alGetProcAddress("alProcessUpdatesSOFT");
    }
    if (!alDeferUpdatesSOFT || !alProcessUpdatesSOFT)
        WriteLog(
            "sound: warning: extension AL_SOFT_deferred_updates not found");

    if (alcIsExtensionPresent(m_device, "ALC_SOFT_pause_device")) {
        alcDevicePauseSOFT = (void (*)(ALCdevice *))alcGetProcAddress(
            m_device, "alcDevicePauseSOFT");
        alcDeviceResumeSOFT = (void (*)(ALCdevice *))alcGetProcAddress(
            m_device, "alcDeviceResumeSOFT");
    }
