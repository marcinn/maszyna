...
// jeśli pierwsza jest aktualna, a druga rozkładowa, to ujemna oznacza
// opóżnienie na dłuższą metę trzeba uwzględnić datę, jakby opóżnienia miały
// przekraczać 12h (towarowych)
double CompareTime(double t1h, double t1m, double t2h, double t2m) {
    if ((t2h < 0))
        return 0;
    else {
        auto t = (t2h - t1h) * 60 + t2m -
                 t1m;  // jeśli t2=00:05, a t1=23:50, to różnica wyjdzie ujemna
...
bool FuzzyLogic(double Test, double Threshold, double Probability) {
    if ((Test > Threshold) && (!DebugModeFlag))
        return (
            Random() < Probability * Threshold * 1.0 /
                           Test) /*im wiekszy Test tym wieksza szansa*/;
    else
        return false;
}
...
std::string ExchangeCharInString(
    std::string const &Source, char const From, char const To) {
...
std::vector<std::string> Split(
    const std::string &s, char delim) {  // dzieli tekst na wektor tekstow
...
std::vector<std::string> Split(
    const std::string
        &s) {  // dzieli tekst na wektor tekstow po białych znakach
    std::vector<std::string> elems;
    std::stringstream ss(s);
    std::string item;
    while (ss >> item) {
...
std::string to_string(
    double const Value, int const Precision, int const Width) {
    std::ostringstream converter;
    converter << std::setw(Width) << std::fixed << std::setprecision(Precision)
              << Value;
    return converter.str();
};
...
std::string to_hex_str(int const Value, int const Width) {
    std::ostringstream converter;
    converter << "0x" << std::uppercase << std::setfill('0') << std::setw(Width)
              << std::hex << Value;
    return converter.str();
};
...
bool string_ends_with(const std::string &string, const std::string &ending) {
    ...
    return string.compare(
               string.length() - ending.length(), ending.length(), ending) == 0;
}

bool string_starts_with(const std::string &string, const std::string &begin) {
    if (string.length() < begin.length()) return false;

    return string.compare(0, begin.length(), begin) == 0;
}

std::string const fractionlabels[] = {" ",   u8"¹", u8"²", u8"³", u8"⁴",
                                      u8"⁵", u8"⁶", u8"⁷", u8"⁸", u8"⁹"};
...
std::string to_minutes_str(
    ...
    auto minutes =
        (std::string(width - 1, ' ') +
         (Leadingzero ? to_string(100 + minutesintegral).substr(1, 2)
                      : to_string(minutesintegral, 0)));
    return (
        minutes.substr(minutes.size() - width, width) +
        fractionlabels[static_cast<int>(
            std::floor(minutesfractional * 10 + 0.1))]);
}
...
std::string ToLower(std::string const &text) {
    auto lowercase{text};
    std::transform(
        std::begin(text), std::end(text), std::begin(lowercase),
...
std::string ToUpper(std::string const &text) {
    auto uppercase{text};
    std::transform(
        std::begin(text), std::end(text), std::begin(uppercase),
...
void win1250_to_ascii(std::string &Input) {
    std::unordered_map<char, char> const charmap{
        {165, 'A'}, {198, 'C'}, {202, 'E'}, {163, 'L'}, {209, 'N'}, {211, 'O'},
        {140, 'S'}, {143, 'Z'}, {175, 'Z'}, {185, 'a'}, {230, 'c'}, {234, 'e'},
        {179, 'l'}, {241, 'n'}, {243, 'o'}, {156, 's'}, {159, 'z'}, {191, 'z'}};
...
std::string win1250_to_utf8(const std::string &Input) {
    std::unordered_map<char, std::string> const charmap{
        {165, u8"Ą"}, {198, u8"Ć"}, {202, u8"Ę"}, {163, u8"Ł"}, {209, u8"Ń"},
        {211, u8"Ó"}, {140, u8"Ś"}, {143, u8"Ź"}, {175, u8"Ż"}, {185, u8"ą"},
        {230, u8"ć"}, {234, u8"ę"}, {179, u8"ł"}, {241, u8"ń"}, {243, u8"ó"},
        {156, u8"ś"}, {159, u8"ź"}, {191, u8"ż"}};
    std::string output;
...
bool erase_extension(std::string &Filename) {
    ...
    if (extensionpos != Filename.rfind("..") + 1) {
        // we can get extension for .mat or, in legacy files, some image format.
        // just trim it and set it to material file extension
...
std::ptrdiff_t len_common_prefix(
    ...
    return (
        Right.size() <= Left.size()
            ? std::distance(
                  right, std::mismatch(right, right + Right.size(), left).first)
            : std::distance(
                  left, std::mismatch(left, left + Left.size(), right).first));
}
...
bool ends_with(std::string_view String, std::string_view Suffix) {
    return (String.size() >= Suffix.size()) &&
           (0 == String.compare(
                     String.size() - Suffix.size(), Suffix.size(), Suffix));
}
...
// TODO: review and clean up the helper routines, there's likely some redundant
// ones
...
// extracts a group of tokens from provided data stream, returns one of them
// picked randomly
std::string deserialize_random_set(cParser &Input, char const *Break) {
    ...
    // if instead of a single token we've encountered '[' this marks a beginning
    // of a random set we retrieve all entries, then return a random one
    ...
    if (false == tokens.empty()) {
        std::shuffle(
            std::begin(tokens), std::end(tokens), Global.random_engine);
        return tokens.front();
    } else {
...
int count_trailing_zeros(uint32_t val) {
    ...
    for (uint32_t shift = 1; !(val & shift); shift <<= 1) r++;
