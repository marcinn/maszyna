...
class render_task {
   public:
    // constructors
    render_task(
        PyObject *Renderer, dictionary_source *Input,
...
class python_taskqueue {
   public:
    ...
    void exit();
    // adds specified task along with provided collection of data to the work
    ...
    auto run_file(std::string const &File, std::string const &Path = "")
        -> bool;
   ...
   private:
    ...
    void run(
        ...
        uploadtask_sequence &Upload_Tasks,
        threading::condition_variable &Condition, std::atomic<bool> &Exit);
    void error();
    // members
    ...
    std::unordered_map<std::string, PyObject *>
        m_renderers;  // cache of python classes
